...
{
    sc_dt::uint64 base_Freq;

    /* Base clock source of PCLK */
    base_Freq = PCLK.read();

    SetCLKFreq("PCLK", base_Freq, "Hz");
...
void HAGE::DMA_Read_Payload(const unsigned int addr, const unsigned int read_size, bool is_debug)
{
    ...
    common_re_printf("info", "Read data from source address 0x%08X", addr);
    /* wait to switch state*/
    //wait(t_time);
}
...
void HAGE::SetCLKFreq(const std::string clock_name, const sc_dt::uint64 freq, const std::string unit)
{
    std::vector<std::string> clock_vector;
    std::string unit_out;
    SeparateString(clock_vector, clock_name);
    if (clock_vector.size() == 1)
    {
        if (clock_vector[0] == "PCLK")
        {
            /* Convert requency GHz, MHz, KHz, Hz */
            ConvertClockFreq(mPCLKClock, unit_out, freq, unit);
            if (mPCLKClock > 0)
            {
                /* Calculate number of PCLK periodic
                * Peroid = (1/f)*time_resolution
                */
                mPCLKPeriod = (sc_dt::uint64) ((1 / (double)mPCLKClock) * (double)mTimeResolutionVal);
                
                /* Update HASH periodic with PLL configure */
                ExchangePLL();
            }
            else 
            {
        ...
        {
            common_re_printf("warning", "Clock name (%s) is not support!\n", clock_vector[0].c_str());
        }
    }
    else
    {
...
void HAGE::GetCLKFreq(const std::string clock_name)
{
}

bool HAGE::CheckClockPeriod(const std::string clock_name)
{
    return false;
}

void HAGE::ExchangePLL(void)
{
    uint32_t pllVal;
    /* Get PLL value: 0, 1, 2 */
    pllVal = mpHAGE_Func->GetPLL();
    switch (pllVal)
    {
    case 0:
        mHASHPeriod = mPCLKPeriod;
        break;
    case 1:
        mHASHPeriod = mPCLKPeriod / 4;
        break;
    case 2:
        mHASHPeriod = mPCLKPeriod / 8;
        break;
    default:
        break;
    }
