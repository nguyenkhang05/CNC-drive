...
//------------------------------------------------------------------------------
// floorf.duw reg2, reg3 implementation
//------------------------------------------------------------------------------

rh850g4::floorf_dot_duw_reg2_reg3_instr_t::floorf_dot_duw_reg2_reg3_instr_t
(
...
void rh850g4::floorf_dot_duw_reg2_reg3_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850g4::floorf_dot_duw_reg2_reg3_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  disass_string << "floorf.duw " << reg2_name << ", " << reg3_name; 
  return disass_string.str();
}

unsigned int rh850g4::floorf_dot_duw_reg2_reg3_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850g4::floorf_dot_duw_reg2_reg3_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg2 = get_reg2_u8_12_15_shift1(opcode);
  GR_index_t reg3 = get_reg3_u8_27_31(opcode);
  ...
  //    - For a positive floating point number, if it is greater than 2^32-1 (represented as 0x41efffffffe00000 in double precision floating point)
  set_gen_point(check_saturation_branch);
  fastiss::i1_t *is_saturated = gen_fcmp_ogt(reg2_pair_float64_flushed, gen_f64_cast(gen_i64_constant(0x41efffffffe00000ULL)));
  ...
  gen_update_dest_and_fpsr(false, reg3, gen_i32_constant(0x00000000), gen_i8_constant(0x10), continue_block);

  // Handle saturation case
  set_gen_point(saturation_branch);
  gen_update_dest_and_fpsr(false, reg3, gen_i32_constant(0xffffffff), gen_i8_constant(0x10), continue_block);  

  // Handle general operand case
  set_gen_point(perform_calc_branch);   

  // Convert the operand to a 32-bit integer value
  fastiss::i32_with_exception_flags_t convert_result = gen_i32_unsigned_convert(reg2_pair_float64_flushed, gen_i8_constant(1));
