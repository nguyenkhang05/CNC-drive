...
//------------------------------------------------------------------------------
// st.h reg3, disp23 [reg1] implementation
//------------------------------------------------------------------------------

rh850::st_dot_h_reg3_disp23_ind_reg1_instr_t::st_dot_h_reg3_disp23_ind_reg1_instr_t
(
...
void rh850::st_dot_h_reg3_disp23_ind_reg1_instr_t::decode(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(6);  
}

std::string rh850::st_dot_h_reg3_disp23_ind_reg1_instr_t::disassemble(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  if (disp23 < 0)
  {
    disass_string << "st.h " << reg3_name << ", -0x" << std::hex << -disp23 << "[" << reg1_name << "]";
  }
  else
  {
    disass_string << "st.h " << reg3_name << ", 0x" << std::hex << disp23 << "[" << reg1_name << "]";
...
unsigned int rh850::st_dot_h_reg3_disp23_ind_reg1_instr_t::length(rh850::opcode_t opcode, unsigned int trans_PC)
{
  return 6;
}

void rh850::st_dot_h_reg3_disp23_ind_reg1_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg3 = get_reg3_u8_27_31(opcode);
  ...
  if (is_mdp_enable()){
    ...
    fastiss::i32_t *mpu_func_result = call_helper(
      ...
      addr,
      gen_or(gen_shl(gen_i32_constant(reg1), gen_i32_constant(16)), gen_i32_constant(0x205)),
      NULL
    );
    gen_cond_branch(
  ...
  if(is_link_func_enable()){
    ...
    call_helper("clear_monitor_helper", gen_read_link_manager_ptr(), gen_read_HTCFG0_PEID(), addr, gen_i32_constant(2), NULL);
  }
