...
std::vector<std::vector<std::string>> softfloat_cvtf_sw;

char *test_data_dir;


class CVTFSWTest : public testing::Test,
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfSW, CVTFSWTest,
        ::testing::ValuesIn(softfloat_cvtf_sw.begin(),
                            softfloat_cvtf_sw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_sw.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cvtf_sw = parseTable(inFile, ",");
    inFile.close();
  }
