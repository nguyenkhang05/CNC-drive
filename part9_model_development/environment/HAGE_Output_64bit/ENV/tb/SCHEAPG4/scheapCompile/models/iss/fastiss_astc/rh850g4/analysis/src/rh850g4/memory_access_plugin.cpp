...
rh850g4::analysis_predicate_response memory_access_predicate(MemoryAccessPluginCallbackInfo *mapcbi)
{
  ...
  memory_access_callback_func_t read_func)
{
  if (analysis_context && core_context)
  {
    ...
    if (read_func)
    {
      ...
      // Attach to load analysis points
      mapi->m_attachments.push_back(core->attach("immediate_load", (rh850g4::analysis_callback)memory_access_callback, load_info));
      mapi->m_attachments.push_back(core->attach("generative_load", (rh850g4::analysis_predicate)memory_access_predicate, load_info));
    }
    ...
    if (write_func)
    {
      ...
      // Attach to store analysis points
      mapi->m_attachments.push_back(core->attach("immediate_store", (rh850g4::analysis_callback)memory_access_callback, store_info));
      mapi->m_attachments.push_back(core->attach("generative_store", (rh850g4::analysis_predicate)memory_access_predicate, store_info));
    }
  }
  ...
  void* analysis_context)
{
  if (analysis_context)
  {
    ...
    // Detach attachments
    for (std::list<rh850g4::analysis_attachment_descriptor *>::iterator att_it = mapi->m_attachments.begin(); att_it != mapi->m_attachments.end(); ++att_it )
    {
...
{
  aii->m_memory_access_callback = aii->m_core->register_analysis_callback((rh850g4::analysis_callback) memory_access_callback);
}
