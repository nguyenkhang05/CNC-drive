/**
 * @file test_mpu_ismpupermittedei.cpp
 *
...
protected:
  ...
  virtual void TearDown() {
    ...
    for (MockSRegMPAT_N* element : m_mpat_vec) { delete element; };
    m_mpat_vec.clear();
    delete m_dir0;
    delete m_hvcfg;
    delete m_pswh;
...
/*!
 * bool IsMpuPermittedEI<>(PhysAddr vrtl_addr, ByteSize size);
 * @brief Check IsMpuPermittedEI method.
 */
TEST_P(MpuMethodCheck, CheckIsMpuPermittedEI) {
  ...
  switch (cmd) {
  case DATA_WRITE:
    res = m_mpu->IsMpuPermittedEI<DATA_WRITE>(addr, size);
    break;
  case DATA_READ:
    res = m_mpu->IsMpuPermittedEI<DATA_READ>(addr, size);
    break;
  default:
    FAIL() << "We shouldn't get here.";
  }
...
std::tuple<MemAccessCommand, bool, bool, bool> const ParametersTable[] = {
  ...
  /* 0000 */ std::make_tuple(DATA_READ,  false, true, true),   // Exit (a)
  /* 0001 */ std::make_tuple(DATA_READ,  false, false, false), // Exit (b)
  /* 0002 */ std::make_tuple(DATA_WRITE, false, true, true),   // Exit (a)
  /* 0003 */ std::make_tuple(DATA_WRITE, false, false, false), // Exit (b)
  /* 0004 */ std::make_tuple(DATA_WRITE, true,  false, true),  // Exit ()
  /* 0005 */ std::make_tuple(DATA_WRITE, true,  true, true),   // Exit ()
  /* 0006 */ std::make_tuple(DATA_READ,  true,  true, true),   // Exit ()
  /* 0007 */ std::make_tuple(DATA_READ,  true,  false, true),  // Exit ()
