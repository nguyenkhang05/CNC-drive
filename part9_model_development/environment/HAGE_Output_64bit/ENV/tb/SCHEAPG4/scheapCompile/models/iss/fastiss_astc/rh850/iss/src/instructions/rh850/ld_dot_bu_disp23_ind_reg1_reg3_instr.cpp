...
//------------------------------------------------------------------------------
// ld.bu disp23 [reg1], reg3 implementation
//------------------------------------------------------------------------------

rh850::ld_dot_bu_disp23_ind_reg1_reg3_instr_t::ld_dot_bu_disp23_ind_reg1_reg3_instr_t
(
...
void rh850::ld_dot_bu_disp23_ind_reg1_reg3_instr_t::decode(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(6);  
}

std::string rh850::ld_dot_bu_disp23_ind_reg1_reg3_instr_t::disassemble(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  if (disp23 < 0)
  {
    disass_string << "ld.bu -0x" << std::hex << -disp23 << "[" << reg1_name << "], " << reg3_name;
  }
  else
  {
    disass_string << "ld.bu 0x" << std::hex << disp23 << "[" << reg1_name << "], " << reg3_name;
...
unsigned int rh850::ld_dot_bu_disp23_ind_reg1_reg3_instr_t::length(rh850::opcode_t opcode, unsigned int trans_PC)
{
  return 6;
}

void rh850::ld_dot_bu_disp23_ind_reg1_reg3_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  if (is_mdp_enable()){
    ...
    fastiss::i32_t *mpu_func_result = call_helper(
      ...
      address,
      gen_or(gen_shl(gen_i32_constant(reg3), gen_i32_constant(16)), gen_i32_constant(0x104)),
      NULL
    );
    gen_cond_branch(
