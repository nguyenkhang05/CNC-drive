...
G0MevReg::G0MevReg ( const char* name ):
  GRegDataBase<GRegData32>( name )
{
  dmsgmv( GetName() << ".G0MevReg()" );
  SetRegPtr( &mData );
}
...
/// <param name="name">module name</param>
/// <returns>none</returns>
G0MevFunc::G0MevFunc( const char* name ):
  BusSlaveFuncIf()
{
  dmsgmv( name << ".G0MevFunc()" );
  strcpy( mModName, name );
  char inst_name[128];
  sprintf( inst_name, "%s.G0MEV", name );
  mpG0MevReg = new G0MevReg( inst_name );
}

/// <summary>destructor</summary>
/// <param name="">none</param>
/// <returns>none</returns>
G0MevFunc::~G0MevFunc( void )
{
  dmsgmv( mModName << ".~G0MevFunc()" );
  delete mpG0MevReg;
}
  ...
  bool             debug )
{
  ...
  if ( debug ) {
    mpG0MevReg->ReadBody( (unsigned int)trans.get_address(), pData, (G_REG_SIZE_T)size );
  }
  else {
    mpG0MevReg->Read( (unsigned int)trans.get_address(), pData, (G_REG_SIZE_T)size );
  }
  ...
  bool             debug )
{
  ...
  if ( debug ) {
    mpG0MevReg->WriteBody( (unsigned int)trans.get_address(), pData, (G_REG_SIZE_T)size );
  }
  else {
    mpG0MevReg->Write( (unsigned int)trans.get_address(), pData, (G_REG_SIZE_T)size );
  }
