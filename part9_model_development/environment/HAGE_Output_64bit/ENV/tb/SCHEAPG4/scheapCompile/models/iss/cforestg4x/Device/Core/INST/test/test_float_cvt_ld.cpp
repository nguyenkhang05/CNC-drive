...
std::vector<std::vector<std::string>> softfloat_cvtf_ld;

char *test_data_dir;


class CVTFLDTest : public testing::Test,
...
TEST_P(CVTFLDTest, CheckCVTFLD) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), Cvt_i64_f64(_)).Times(1).After(inits);
  ...
  FpuRegDouble ret;
  int64_t reg1_i64 = static_cast<int64_t> (reg1);
  float64_t ret_f64 = m_sf->Cvt_i64_f64(reg1_i64);

  uint32_t fpu_exception_flag = m_sf->GetExceptionFlags();

  EXPECT_EQ(expect_evzoui, (fpu_exception_flag & 0x0000003f));
  EXPECT_EQ(expect, ret_f64.v);
}


INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfLD, CVTFLDTest,
        ::testing::ValuesIn(softfloat_cvtf_ld.begin(),
                            softfloat_cvtf_ld.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_ld_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Cold not open test data for input");
    }
    softfloat_cvtf_ld = parseTable(inFile, ",");
    inFile.close();
  }
