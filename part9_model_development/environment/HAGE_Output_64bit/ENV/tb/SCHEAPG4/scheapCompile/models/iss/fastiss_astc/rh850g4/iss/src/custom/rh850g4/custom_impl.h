...
/**
 * \file custom_impl.h
 * \brief Defines the Custom Generator implementation class
 */

#ifndef _RH850G4_CUSTOM_IMPL_H_
#define _RH850G4_CUSTOM_IMPL_H_
...
#include "rh850g4/memory_bridge.h"
#include "rh850g4/custom_if.h"
#include "rh850g4/cpu_state_bridge.h"
#include "rh850g4/analysis_point_if.h"

//------------------------------------------------------------------------------
// Custom extension point implementation class
//------------------------------------------------------------------------------

namespace rh850g4
{
  /**
   * \brief Class implementing the custom extension interface. This class
   * inherits from the bridge classes of the other generators in order to make
   * those interfaces available directly.
   */
  class custom_impl_t :
    public rh850g4::custom_if_t,
    public fastiss::code_gen_bridge_t,
    public rh850g4::memory_bridge_t,
    public rh850g4::cpu_state_bridge_t
  {
  public:
    /**
     * \brief Custom implementor constructor
     * \param code_gen_impl A pointer to the code generator implementor
     * \param memory_impl A pointer to the memory access implementor
     * \param cpu_state_impl A pointer to the CPU state implementor
     * \param interrupt_accepted_analysis_point A pointer to the interrupt accepted analysis point
     * \param interrupt_completed_analysis_point A pointer to the interrupt completed analysis point
     * \param pending_fpu_imprecise_exception_flag A pointer to the ISS core
     * \param link_manager The pointer of LinkManager
     * \param link_func_enable The flag of link function
     */
    custom_impl_t
    (
    ...
    /**
     * \brief Custom implementor destructor
     */
    virtual ~custom_impl_t();
     ...
     */
    virtual fastiss::i8_t *gen_read_round_mode(void);

    /**
     * \brief Generates clear FPI flag,
     */
    virtual void gen_clear_fpi_flag(void);
    ...
    /**
     * \brief Generates an FP SIMD covert single to unsigned word operation,,
     * \param input_val_1 A Input operand 1 
     * \param res_val A result of covert culculation
     * \param res_exp A result of exception
     * \param round_mode A round mode
  ...
  private:
    /**
     * \brief A pointer to the exit block for block translation
     */          
    fastiss::block_t *m_exit_block;
    /**
     * \brief A pointer to the interrupt accepted analysis point
     */          
    rh850g4::analysis_point_if *m_interrupt_accepted_analysis_point;
    /**
     * \brief A pointer to the interrupt completed analysis point
     */          
    rh850g4::analysis_point_if *m_interrupt_completed_analysis_point;

    /**
     * \brief A pointer to the ISS core flag which indicates that there is a
     * pending floating point imprecise exception
     */          
    void *m_pending_fpu_imprecise_exception_flag;

    /**
     * \brief A pointer to the LinkManager instance's pointer
     */          
    void *m_link_manager;

    /**
     * \brief A flag whether the Link-function is enable or not
     */          
    bool m_link_func_enable;

    /**
     * \brief A flag whether the Hardware-assisted Virtualization is supported or not
     */          
    bool m_hv_func_support;
  };
}

#endif // _RH850G4_CUSTOM_IMPL_H_
