...
//------------------------------------------------------------------------------
// floorf.sul reg2, reg3 implementation
//------------------------------------------------------------------------------

rh850g4::floorf_dot_sul_reg2_reg3_instr_t::floorf_dot_sul_reg2_reg3_instr_t
(
...
void rh850g4::floorf_dot_sul_reg2_reg3_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850g4::floorf_dot_sul_reg2_reg3_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  disass_string << "floorf.sul " << reg2_name << ", " << reg3_name; 
  return disass_string.str();
}

unsigned int rh850g4::floorf_dot_sul_reg2_reg3_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850g4::floorf_dot_sul_reg2_reg3_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg2 = get_reg2_u8_11_15(opcode);
  GR_index_t reg3 = get_reg3_u8_28_31_shift1(opcode);
  ...
  //    - For a positive floating point number, if it is greater than 2^64-1 (represented as 0x5f800000 in single precision floating point)
  set_gen_point(check_saturation_branch);
  fastiss::i1_t *is_saturated = gen_fcmp_oge(reg2_float32_flushed, gen_f32_cast(gen_i32_constant(0x5f800000)));
  ...
  gen_update_dest_and_fpsr(true, reg3, gen_i64_constant(0x0000000000000000ULL), gen_i8_constant(0x10), continue_block);

  // Handle saturation case
  set_gen_point(saturation_branch);
  gen_update_dest_and_fpsr(true, reg3, gen_i64_constant(0xffffffffffffffffULL), gen_i8_constant(0x10), continue_block);  

  // Handle general operand case
  set_gen_point(perform_calc_branch);   

  // Convert the operand to a 64-bit integer value
  fastiss::i64_with_exception_flags_t convert_result = gen_i64_unsigned_convert(reg2_float32_flushed, gen_i8_constant(1));
