...
std::vector<std::vector<std::string>> softfloat_trncf_dw;

char *test_data_dir;


class TRNCFDWTest : public testing::Test,
...
/*!
 * @brief Check trncf.dw
 * SoftFloat-3b modified
 */
TEST_P(TRNCFDWTest, CheckTRNCFDW) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatTrncfDW, TRNCFDWTest,
        ::testing::ValuesIn(softfloat_trncf_dw.begin(),
                            softfloat_trncf_dw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_trncf_dw_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_trncf_dw = parseTable(inFile, ",");
    inFile.close();
  }
