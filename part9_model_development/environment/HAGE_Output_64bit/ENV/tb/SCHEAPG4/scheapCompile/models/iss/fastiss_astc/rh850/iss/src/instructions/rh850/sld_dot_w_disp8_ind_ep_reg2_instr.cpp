...
//------------------------------------------------------------------------------
// sld.w disp8 [ep], reg2 implementation
//------------------------------------------------------------------------------

rh850::sld_dot_w_disp8_ind_ep_reg2_instr_t::sld_dot_w_disp8_ind_ep_reg2_instr_t
(
...
void rh850::sld_dot_w_disp8_ind_ep_reg2_instr_t::decode(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(2);  
}

std::string rh850::sld_dot_w_disp8_ind_ep_reg2_instr_t::disassemble(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  disass_string << "sld.w 0x" << std::hex << disp8 << "[ep], " << reg2_name;
  return disass_string.str();
}

unsigned int rh850::sld_dot_w_disp8_ind_ep_reg2_instr_t::length(rh850::opcode_t opcode, unsigned int trans_PC)
{
  return 2;
}

void rh850::sld_dot_w_disp8_ind_ep_reg2_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  if (is_mdp_enable()){
    ...
    fastiss::i32_t *mpu_func_result = call_helper(
      ...
      address,
      gen_or(gen_shl(gen_i32_constant(reg2), gen_i32_constant(16)), gen_i32_constant(0x400)),
      NULL
    );
    gen_cond_branch(
