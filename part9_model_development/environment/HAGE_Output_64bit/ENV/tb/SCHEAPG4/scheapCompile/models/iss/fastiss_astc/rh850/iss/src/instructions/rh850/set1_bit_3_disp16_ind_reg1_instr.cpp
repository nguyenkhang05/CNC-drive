...
//------------------------------------------------------------------------------
// set1 bit#3, disp16 [reg1] implementation
//------------------------------------------------------------------------------

rh850::set1_bit_3_disp16_ind_reg1_instr_t::set1_bit_3_disp16_ind_reg1_instr_t
(
...
void rh850::set1_bit_3_disp16_ind_reg1_instr_t::decode(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850::set1_bit_3_disp16_ind_reg1_instr_t::disassemble(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  if (disp16 < 0)
  {
    disass_string << "set1 0x" << (unsigned int)bit << ", -0x" << std::hex << -disp16 << "[" << reg1_name << "]";
  }
  else
  {
    disass_string << "set1 0x" << (unsigned int)bit << ", 0x" << std::hex << disp16 << "[" << reg1_name << "]";
...
unsigned int rh850::set1_bit_3_disp16_ind_reg1_instr_t::length(rh850::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850::set1_bit_3_disp16_ind_reg1_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
...
unsigned int rh850::set1_bit_3_disp16_ind_reg1_instr_t::cycle_count(opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}
