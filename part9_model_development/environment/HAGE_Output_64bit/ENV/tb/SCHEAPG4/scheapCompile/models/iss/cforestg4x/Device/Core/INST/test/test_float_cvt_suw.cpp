...
std::vector<std::vector<std::string>> softfloat_cvtf_suw;

char *test_data_dir;


class CVTFSUWTest : public testing::Test,
...
/*!
 * @brief Check cvtf.suw
 * SoftFloat-3b modified
 */
TEST_P(CVTFSUWTest, CheckCVTFSUW) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfSUW, CVTFSUWTest,
        ::testing::ValuesIn(softfloat_cvtf_suw.begin(),
                            softfloat_cvtf_suw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_suw.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cvtf_suw = parseTable(inFile, ",");
    inFile.close();
  }
