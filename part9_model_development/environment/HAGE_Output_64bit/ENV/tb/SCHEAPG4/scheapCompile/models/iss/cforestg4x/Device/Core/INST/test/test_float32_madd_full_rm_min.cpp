/**
 * @file test_float32_madd_full_rm_min.cpp
 *
...
std::vector<std::vector<std::string>> softfloat_muladd_min;
...
INSTANTIATE_TEST_CASE_P(Romeo_FMAF_min_1M_0, F32_Muladd_Test,
                        ::testing::ValuesIn(softfloat_muladd_min.begin(),
                                            softfloat_muladd_min.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/Romeo_FMAF_min_1M_0.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_muladd_min = parseTable(inFile, ",");
    inFile.close();
  }
