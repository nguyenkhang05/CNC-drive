...
std::vector<std::vector<std::string>> softfloat_ceilf_sw;

char *test_data_dir;


class CEILFSWTest : public testing::Test,
...
/*!
 * @brief Check ceilf.sw
 * SoftFloat-3b modified
 */
TEST_P(CEILFSWTest, CheckCEILFSW) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfSW, CEILFSWTest,
        ::testing::ValuesIn(softfloat_ceilf_sw.begin(),
                            softfloat_ceilf_sw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_ceilf_sw.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_ceilf_sw = parseTable(inFile, ",");
    inFile.close();
  }
