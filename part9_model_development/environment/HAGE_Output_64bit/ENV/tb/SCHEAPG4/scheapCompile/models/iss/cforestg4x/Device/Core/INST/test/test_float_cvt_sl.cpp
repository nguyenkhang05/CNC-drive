...
std::vector<std::vector<std::string>> softfloat_cvtf_sl;

char *test_data_dir;


class CVTFSLTest : public testing::Test,
...
TEST_P(CVTFSLTest, CheckCVTFSL) {
  ...
  uint64_t ret = static_cast<uint64_t> (m_sf->Cvt_f32_i64(reg1_f32));
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfSL, CVTFSLTest,
        ::testing::ValuesIn(softfloat_cvtf_sl.begin(),
                            softfloat_cvtf_sl.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_sl_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cvtf_sl = parseTable(inFile, ",");
    inFile.close();
  }
