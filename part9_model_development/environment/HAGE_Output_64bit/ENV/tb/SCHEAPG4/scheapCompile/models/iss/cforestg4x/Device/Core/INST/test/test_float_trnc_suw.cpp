...
std::vector<std::vector<std::string>> softfloat_trncf_suw;

char *test_data_dir;


class TRNCFSUWTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  TRNCFSUWTest() {}
  virtual ~TRNCFSUWTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check trncf.suw
 * SoftFloat-3b modified
 */
TEST_P(TRNCFSUWTest, CheckTRNCFSUW) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatTrncfSUW, TRNCFSUWTest,
        ::testing::ValuesIn(softfloat_trncf_suw.begin(),
                            softfloat_trncf_suw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_trncf_suw.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_trncf_suw = parseTable(inFile, ",");
    inFile.close();
  }
