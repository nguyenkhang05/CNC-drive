...
std::vector<std::vector<std::string>> softfloat_roundf_sl;

char *test_data_dir;


class ROUNDFSLTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  ROUNDFSLTest() {}
  virtual ~ROUNDFSLTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check roundf.sl
 * SoftFloat-3b modified
 */
TEST_P(ROUNDFSLTest, CheckROUNDFSL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatRoundSL, ROUNDFSLTest,
        ::testing::ValuesIn(softfloat_roundf_sl.begin(),
                            softfloat_roundf_sl.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_roundf_sl_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_roundf_sl = parseTable(inFile, ",");
    inFile.close();
  }
