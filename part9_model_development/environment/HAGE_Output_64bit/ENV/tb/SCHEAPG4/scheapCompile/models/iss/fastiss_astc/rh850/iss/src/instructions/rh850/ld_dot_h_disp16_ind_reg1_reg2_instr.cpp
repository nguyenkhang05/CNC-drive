...
//------------------------------------------------------------------------------
// ld.h disp16 [reg1], reg2 implementation
//------------------------------------------------------------------------------

rh850::ld_dot_h_disp16_ind_reg1_reg2_instr_t::ld_dot_h_disp16_ind_reg1_reg2_instr_t
(
...
void rh850::ld_dot_h_disp16_ind_reg1_reg2_instr_t::decode(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850::ld_dot_h_disp16_ind_reg1_reg2_instr_t::disassemble(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  if (disp16 < 0)
  {
    disass_string << "ld.h -0x" << std::hex << -disp16 << "[" << reg1_name << "], " << reg2_name;
  }
  else
  {
    disass_string << "ld.h 0x" << std::hex << disp16 << "[" << reg1_name << "], " << reg2_name;
...
unsigned int rh850::ld_dot_h_disp16_ind_reg1_reg2_instr_t::length(rh850::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850::ld_dot_h_disp16_ind_reg1_reg2_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  if (is_mdp_enable()){
    ...
    fastiss::i32_t *mpu_func_result = call_helper(
      ...
      address,
      gen_or(gen_shl(gen_i32_constant(reg2) ,gen_i32_constant(16)), gen_i32_constant(0x202)),
      NULL
    );
    gen_cond_branch(
