...
std::vector<std::vector<std::string>> softfloat_cvtf_duw;

char *test_data_dir;


class CVTFDUWTest : public testing::Test,
...
/*!
 * @brief Check cvtf.duw
 * SoftFloat-3b modified
 */
TEST_P(CVTFDUWTest, CheckCVTFDUW) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfDUW, CVTFDUWTest,
        ::testing::ValuesIn(softfloat_cvtf_duw.begin(),
                            softfloat_cvtf_duw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_duw_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cvtf_duw = parseTable(inFile, ",");
    inFile.close();
  }
