...
std::vector<std::vector<std::string>> softfloat_trncf_sul;

char *test_data_dir;


class TRNCFSULTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  TRNCFSULTest() {}
  virtual ~TRNCFSULTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check trncf.sul
 * SoftFloat-3b modified
 */
TEST_P(TRNCFSULTest, CheckTRNCFSUL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatTrncSUL, TRNCFSULTest,
        ::testing::ValuesIn(softfloat_trncf_sul.begin(),
                            softfloat_trncf_sul.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_trncf_sul_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_trncf_sul = parseTable(inFile, ",");
    inFile.close();
  }
