...
std::vector<std::vector<std::string>> softfloat_rsqrtfs;

char *test_data_dir;


class RSQRTFSTest : public testing::Test,
 ...
 */
TEST_P(RSQRTFSTest, CheckRSQRTFS) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f32_rsqrt(_)).Times(1).After(inits);
  ...
  float32_t reg1_val;
  reg1_val.v = reg1;
  float32_t ret_f32 = m_sf->f32_rsqrt(reg1_val);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloat32Rsqrt, RSQRTFSTest,
        ::testing::ValuesIn(softfloat_rsqrtfs.begin(),
                            softfloat_rsqrtfs.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_rsqrtf_s.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_rsqrtfs = parseTable(inFile, ",");
    inFile.close();
  }
