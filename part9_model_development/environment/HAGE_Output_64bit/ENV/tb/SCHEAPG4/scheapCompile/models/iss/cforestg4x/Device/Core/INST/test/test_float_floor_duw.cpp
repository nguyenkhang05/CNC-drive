...
std::vector<std::vector<std::string>> softfloat_floorf_duw;

char *test_data_dir;


class FLOORFDUWTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  FLOORFDUWTest() {}
  virtual ~FLOORFDUWTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check floorf.duw
 * SoftFloat-3b modified
 */
TEST_P(FLOORFDUWTest, CheckFLOORFDUW) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), Cvt_f64_ui32(_)).Times(1).After(inits);
  ...
  uint32_t ret = m_sf->Cvt_f64_ui32(v1);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatFloorfDUW, FLOORFDUWTest,
        ::testing::ValuesIn(softfloat_floorf_duw.begin(),
                            softfloat_floorf_duw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_floorf_duw_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_floorf_duw = parseTable(inFile, ",");
    inFile.close();
  }
