...
//------------------------------------------------------------------------------
// satsubr reg1, reg2 implementation
//------------------------------------------------------------------------------

rh850g4::satsubr_reg1_reg2_instr_t::satsubr_reg1_reg2_instr_t
(
...
void rh850g4::satsubr_reg1_reg2_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(2);  
}

std::string rh850g4::satsubr_reg1_reg2_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  disass_string << "satsubr " << reg1_name << ", " << reg2_name;
  return disass_string.str();
}

unsigned int rh850g4::satsubr_reg1_reg2_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 2;
}

void rh850g4::satsubr_reg1_reg2_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg1 = get_reg1_u8_0_4(opcode);
  GR_index_t reg2 = get_reg2_u8_11_15(opcode);
  ...
  // Perform the SUB
  fastiss::i32_with_carry_overflow_t sub_result_struct = gen_sub_carry_overflow(reg1_load, reg2_load);

  // If overflow update result to either int32_max or int32_min
  fastiss::i32_t *sat_value = gen_select(gen_icmp_slt(reg2_load, gen_i32_constant(0)), gen_i32_constant(INT32_MAX), gen_i32_constant(INT32_MIN));
