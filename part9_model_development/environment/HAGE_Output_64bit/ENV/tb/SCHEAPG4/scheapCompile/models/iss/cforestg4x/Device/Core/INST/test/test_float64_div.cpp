/**
 * @file test_float64_div.cpp
 *
 * @date Created on: 2017
 * @author Can Vy
 * @brief UNIT TEST for float64_div.
...
std::vector<std::vector<std::string>> softfloat_divfd;

char *test_data_dir;


class DivfdTest : public testing::Test,
...
/*!
 * @brief Check f64_divfd
 * SoftFloat-3b
 */
TEST_P(DivfdTest, CheckF64Div) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f64_divfd(_,_)).Times(1).After(inits);
  ...
  float64_t ret = m_sf->f64_divfd(v2, v1);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatDivfd, DivfdTest,
                        ::testing::ValuesIn(softfloat_divfd.begin(), softfloat_divfd.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
                                                 ...
                                                 ::testing::internal::FilePath("data/UT_divf_d_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_divfd = parseTable(inFile, ",");
    inFile.close();
  }
