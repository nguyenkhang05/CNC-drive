...
std::vector<std::vector<std::string>> softfloat_floorf_dl;

char *test_data_dir;


class FLOORFDLTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  FLOORFDLTest() {}
  virtual ~FLOORFDLTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check floorf.dl
 * SoftFloat-3b modified
 */
TEST_P(FLOORFDLTest, CheckFLOORFDL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatFloorfDL, FLOORFDLTest,
        ::testing::ValuesIn(softfloat_floorf_dl.begin(),
                            softfloat_floorf_dl.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_floorf_dl_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_floorf_dl = parseTable(inFile, ",");
    inFile.close();
  }
