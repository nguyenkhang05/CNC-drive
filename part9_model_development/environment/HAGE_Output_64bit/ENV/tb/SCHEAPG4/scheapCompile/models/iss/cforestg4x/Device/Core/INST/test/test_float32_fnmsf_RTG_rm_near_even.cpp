...
std::vector<std::vector<std::string>> softfloat_fnmsfs;

char *test_data_dir;
char *test_data_file;


 class FNMSFSTest : public testing::Test,
  ...
  * SoftFloat-3b
  */
 TEST_P(FNMSFSTest, CheckF32Fnmsfs) {
   ...
   // call softfloat func
   op1 = EXPECT_CALL((*m_sf), f32_mulSub(_,_,_)).Times(1).After(inits);
   op2 = EXPECT_CALL((*m_sf), f32_Mneg(_)).Times(1).After(op1);
   // get exception flag from softfloat
   EXPECT_CALL((*m_sf), GetExceptionFlags()).Times(1).After(op2);
   // read IF bit from softfloat to fpsr
   EXPECT_CALL((*m_sf), GetInputFlushFlag()).Times(AtLeast(0)).After(op2);
 ...
 INSTANTIATE_TEST_CASE_P(TestNewSoftFloatFnmafs, FNMSFSTest,
         ::testing::ValuesIn(softfloat_fnmsfs.begin(),
                             softfloat_fnmsfs.end()));
...
int main(int argc, char **argv) {
  ...
  ::test_data_file = argv[argc - 1];
  {
    ...
    softfloat_fnmsfs = parseTable(inFile, ",");
    inFile.close();
  }
