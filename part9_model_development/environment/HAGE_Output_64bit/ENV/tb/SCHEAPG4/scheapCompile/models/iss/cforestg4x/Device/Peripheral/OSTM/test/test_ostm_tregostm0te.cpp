...
public:
  ...
  PhysAddr start_addr = 0xFFEC0010;
...
TEST_F(OstmRegAccessTest, CheckTE_GetName) {
    ...
    TRegOSTMnTE* OSTM0TE = new TRegOSTMnTE(m_ostm0_core);

    ASSERT_STREQ("OSTMnTE ", OSTM0TE->GetName());
...
TEST_F(OstmRegAccessTest, CheckTE_InitalValue) {
    for (ostmch = 0; ostmch < MAX_OSTM_NUM; ostmch++){
        ...
        ASSERT_EQ(expect_data, OSTM0TE->GetVal());
        delete OSTM0TE;
        delete m_ostm0_core;
        delete m_ostm[ostmch];
    }
...
TEST_F(OstmRegAccessTest, CheckTE_ReadWrite_1) {
    for (ostmch = 0; ostmch < MAX_OSTM_NUM; ostmch++){
        pe_id = ostmch;
        PhysAddr base_addr_pe = base_addr + 0x1000*ostmch;
        ...
        OSTM0TE->Write(sideband, start_addr_pe, wdata, SIZE_8BIT);
        OSTM0TE->Read(sideband, start_addr_pe, (uint8_t*)data, SIZE_8BIT);
        uint32_t expect_data = 0x0;
...
TEST_F(OstmRegAccessTest, CheckTE_ReadWrite_2) {
    for (ostmch = 0; ostmch < MAX_OSTM_NUM; ostmch++){
        pe_id = ostmch;
        PhysAddr base_addr_pe = base_addr + 0x1000*ostmch;
        PhysAddr start_addr_pe = start_addr + 0x1000*ostmch;
...
std::tuple<PhysAddr, int32_t, ByteSize, ByteSize, int32_t> const RW_ParametersTable[] = {
  /* 0000 */ std::make_tuple(0xFFEC0010 /* TE */, 0xf3f2ffff, SIZE_16BIT, SIZE_16BIT, 0x0), //16bit access
  /* 0000 */ std::make_tuple(0xFFEC0012 /* TE */, 0xf3f2ffff, SIZE_16BIT, SIZE_16BIT, 0x0), //16bit access
  /* 0000 */ std::make_tuple(0xFFEC0010 /* TE */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
  /* 0000 */ std::make_tuple(0xFFEC0010 /* TE */, 0xf3f2ffff, SIZE_32BIT, SIZE_32BIT, 0x0), //32bit access
  /* 0000 */ std::make_tuple(0xFFEC0011 /* TE */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
  /* 0000 */ std::make_tuple(0xFFEC0012 /* TE */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
  /* 0000 */ std::make_tuple(0xFFEC0013 /* TE */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
