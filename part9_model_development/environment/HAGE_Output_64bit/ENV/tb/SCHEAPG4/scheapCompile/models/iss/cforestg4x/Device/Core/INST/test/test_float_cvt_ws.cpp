...
std::vector<std::vector<std::string>> softfloat_cvtf_ws;

char *test_data_dir;


class CVTFWSTest : public testing::Test,
...
TEST_P(CVTFWSTest, CheckCVTFWS) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), Cvt_i32_f32(_)).Times(1).After(inits);
  ...
  float32_t ret_f32 = m_sf->Cvt_i32_f32(reg1_i32);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfWS, CVTFWSTest,
        ::testing::ValuesIn(softfloat_cvtf_ws.begin(),
                            softfloat_cvtf_ws.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_ws.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cvtf_ws = parseTable(inFile, ",");
    inFile.close();
  }
