...
//------------------------------------------------------------------------------
// sld.hu disp5 [ep], reg2 implementation
//------------------------------------------------------------------------------

rh850::sld_dot_hu_disp5_ind_ep_reg2_instr_t::sld_dot_hu_disp5_ind_ep_reg2_instr_t
(
...
void rh850::sld_dot_hu_disp5_ind_ep_reg2_instr_t::decode(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(2);  
}

std::string rh850::sld_dot_hu_disp5_ind_ep_reg2_instr_t::disassemble(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  unsigned int disp5 = get_disp5_u32_0_3_shift1(opcode);
  GR_index_t reg2 = get_reg2_u8_11_15(opcode);
  std::string reg2_name = register_name_GR(reg2);
  
  // Render and return result
  std::ostringstream disass_string;
  disass_string << "sld.hu 0x" << std::hex << disp5 << "[ep], " << reg2_name;
  return disass_string.str();
}

unsigned int rh850::sld_dot_hu_disp5_ind_ep_reg2_instr_t::length(rh850::opcode_t opcode, unsigned int trans_PC)
{
  return 2;
}

void rh850::sld_dot_hu_disp5_ind_ep_reg2_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  fastiss::i32_t *disp5 = gen_get_disp5_u32_0_3_shift1(opcode);
  GR_index_t reg2 = get_reg2_u8_11_15(opcode);
  ...
  fastiss::i32_t *address = gen_add(ep_load, disp5);
  ...
  if (is_mdp_enable()){
    ...
    fastiss::i32_t *mpu_func_result = call_helper(
      ...
      address,
      gen_or(gen_shl(gen_i32_constant(reg2),gen_i32_constant(16)), gen_i32_constant(0x300)),
      NULL
    );
    gen_cond_branch(
