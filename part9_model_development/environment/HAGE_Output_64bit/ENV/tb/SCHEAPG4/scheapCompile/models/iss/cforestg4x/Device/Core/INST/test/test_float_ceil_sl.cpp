...
std::vector<std::vector<std::string>> softfloat_ceilf_sl;

char *test_data_dir;


class CEILFSLTest : public testing::Test,
...
/*!
 * @brief Check ceilf.sl
 * SoftFloat-3b modified
 */
TEST_P(CEILFSLTest, CheckCEILFSL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCeilSL, CEILFSLTest,
        ::testing::ValuesIn(softfloat_ceilf_sl.begin(),
                            softfloat_ceilf_sl.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_ceilf_sl_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_ceilf_sl = parseTable(inFile, ",");
    inFile.close();
  }
