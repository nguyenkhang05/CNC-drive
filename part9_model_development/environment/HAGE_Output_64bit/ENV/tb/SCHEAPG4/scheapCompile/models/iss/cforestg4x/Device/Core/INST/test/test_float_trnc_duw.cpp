...
std::vector<std::vector<std::string>> softfloat_trncf_duw;

char *test_data_dir;


class TRNCFDUWTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  TRNCFDUWTest() {}
  virtual ~TRNCFDUWTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check trncf.duw
 * SoftFloat-3b modified
 */
TEST_P(TRNCFDUWTest, CheckTRNCFDUW) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatTrncfDUW, TRNCFDUWTest,
        ::testing::ValuesIn(softfloat_trncf_duw.begin(),
                            softfloat_trncf_duw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_trncf_duw_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_trncf_duw = parseTable(inFile, ",");
    inFile.close();
  }
