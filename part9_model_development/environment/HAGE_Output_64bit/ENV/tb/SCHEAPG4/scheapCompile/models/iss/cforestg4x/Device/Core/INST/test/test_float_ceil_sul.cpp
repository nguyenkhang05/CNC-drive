...
std::vector<std::vector<std::string>> softfloat_ceilf_sul;

char *test_data_dir;


class CEILFSULTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  CEILFSULTest() {}
  virtual ~CEILFSULTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check ceilf.sul
 * SoftFloat-3b modified
 */
TEST_P(CEILFSULTest, CheckCEILFSUL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCeilSUL, CEILFSULTest,
        ::testing::ValuesIn(softfloat_ceilf_sul.begin(),
                            softfloat_ceilf_sul.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_ceilf_sul_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_ceilf_sul = parseTable(inFile, ",");
    inFile.close();
  }
