/**
 * @file test_float32_fmsf_rm_near_even.cpp
 *
...
std::vector<std::vector<std::string>> softfloat_fmsfs;
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatFmafs, FMAFSTest,
        ::testing::ValuesIn(softfloat_fmsfs.begin(),
                            softfloat_fmsfs.end()));
...
int main(int argc, char **argv) {
  ...
  ::test_data_file = argv[argc - 1];
  {
    ...
    softfloat_fmsfs = parseTable(inFile, ",");
    inFile.close();
  }
