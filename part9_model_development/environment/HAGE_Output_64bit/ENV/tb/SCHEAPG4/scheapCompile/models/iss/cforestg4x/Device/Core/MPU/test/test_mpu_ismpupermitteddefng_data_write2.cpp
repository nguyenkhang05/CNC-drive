/**
 * @file test_mpu_ismpupermitteddefng_data_write2.cpp
 *
 ...
 *        condition: MPE=0 & no guest entry
 */
TEST_P(MpuMethodCheck, CheckIsMpuPermittedDefNG_DATA_WRITE_3) {
  ...
  // Test Condition
  {
    ...
    if (!gent & !hent) {
      if ((gmpe) && (gm || um || svp)) { SET_NOT_HIT(HOST_ENTRY);    } // Host: not hit
    } else if (!gent & hent) {
      if ((gmpe) && (gm || um || svp)) { SET_HIT_PERMIT(HOST_ENTRY); } // Host: hit & permit
    } else if (gent & !hent) {
      if ((gmpe) && (gm || um || svp)) { SET_NOT_HIT(HOST_ENTRY);    } // Host: not hit
    } else { // if (gent && hent)
      if ((gmpe) && (gm || um || svp)) { SET_HIT_PERMIT(HOST_ENTRY); } // Host: hit & permit
...
/*!
 * @brief Check IsMpuPermittedDefNG method. (DATE_READ)
 *        condition MPE=0 & no guest entry
 */
TEST_P(MpuMethodCheck, CheckIsMpuPermittedDefNG_DATA_WRITE_4) {
  bool gm = std::get<0>(GetParam());
  bool mpe = std::get<1>(GetParam());
  bool gmpe = std::get<2>(GetParam());
  bool um = std::get<3>(GetParam());
  ...
  // Test Condition
  {
    ...
    if (!gent & !hent) {
      if ((gmpe) && (gm || um || svp)) { SET_HIT_NOT_PERMIT(HOST_ENTRY); } // Host: hit & not permit
    } else if (!gent & hent) {
      if ((gmpe) && (gm || um || svp)) { SET_HIT_PERMIT(HOST_ENTRY);     } // Host: hit & permit
    } else if (gent & !hent) {
      if ((gmpe) && (gm || um || svp)) { SET_HIT_NOT_PERMIT(HOST_ENTRY); } // Host: hit & not permit
