/**
 * @file test_float32_fma_RTG_rm_near_even.cpp
 *
 * @date Created on: Mar 3, 2017
 * @author Hung Nguyen
 * @brief UNIT TEST for float32_fmafs.
...
std::vector<std::vector<std::string>> softfloat_fmafs;

char *test_data_dir;
char *test_data_file;


class FMAFSTest : public testing::Test,
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatFmafs, FMAFSTest,
        ::testing::ValuesIn(softfloat_fmafs.begin(),
                            softfloat_fmafs.end()));
...
int main(int argc, char **argv) {
  ...
  ::test_data_file = argv[argc - 1];
  {
    ...
    softfloat_fmafs = parseTable(inFile, ",");
    inFile.close();
  }
