...
std::vector<std::vector<std::string>> softfloat_addfs;

char *test_data_dir;


class ADDFSTest : public testing::Test,
...
TEST_P(ADDFSTest, CheckF32Add) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f32_addfs(_,_)).Times(1).After(inits);
  ...
  float32_t ret = m_sf->f32_addfs(v2, v1);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatAddfs, ADDFSTest,
        ::testing::ValuesIn(softfloat_addfs.begin(),
                            softfloat_addfs.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_addf_s.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_addfs = parseTable(inFile, ",");
    inFile.close();
  }
