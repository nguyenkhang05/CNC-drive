...
//------------------------------------------------------------------------------
// st.b reg3, disp23 [reg1] implementation
//------------------------------------------------------------------------------

rh850::st_dot_b_reg3_disp23_ind_reg1_instr_t::st_dot_b_reg3_disp23_ind_reg1_instr_t
(
...
void rh850::st_dot_b_reg3_disp23_ind_reg1_instr_t::decode(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(6);  
}

std::string rh850::st_dot_b_reg3_disp23_ind_reg1_instr_t::disassemble(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg3 = get_reg3_u8_27_31(opcode);
  std::string reg3_name = register_name_GR(reg3);
  int disp23 = get_disp23_s32_32_47_20_26(opcode);
  ...
  if (disp23 < 0)
  {
    disass_string << "st.b " << reg3_name << ", -0x" << std::hex << -disp23 << "[" << reg1_name << "]";
  }
  else
  {
    disass_string << "st.b " << reg3_name << ", 0x" << std::hex << disp23 << "[" << reg1_name << "]";
...
unsigned int rh850::st_dot_b_reg3_disp23_ind_reg1_instr_t::length(rh850::opcode_t opcode, unsigned int trans_PC)
{
  return 6;
}

void rh850::st_dot_b_reg3_disp23_ind_reg1_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg3 = get_reg3_u8_27_31(opcode);
  fastiss::i32_t *disp23 = gen_get_disp23_s32_32_47_20_26(opcode);
  GR_index_t reg1 = get_reg1_u8_0_4(opcode);
  ...
  if (is_mdp_enable()){
    ...
    fastiss::i32_t *mpu_func_result = call_helper(
      ...
      addr,
      gen_or(gen_shl(gen_i32_constant(reg1),gen_i32_constant(16)), gen_i32_constant(0x5)),
      NULL
    );
    gen_cond_branch(
