...
//------------------------------------------------------------------------------
// cvtf.ds reg2, reg3 implementation
//------------------------------------------------------------------------------

rh850g4::cvtf_dot_ds_reg2_reg3_instr_t::cvtf_dot_ds_reg2_reg3_instr_t
(
...
void rh850g4::cvtf_dot_ds_reg2_reg3_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850g4::cvtf_dot_ds_reg2_reg3_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  disass_string << "cvtf.ds " << reg2_name << ", " << reg3_name; 
  return disass_string.str();
}

unsigned int rh850g4::cvtf_dot_ds_reg2_reg3_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850g4::cvtf_dot_ds_reg2_reg3_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg2 = get_reg2_u8_12_15_shift1(opcode);
  GR_index_t reg3 = get_reg3_u8_27_31(opcode);
  ...
  gen_update_dest_and_fpsr(false, reg3, gen_i32_constant(0x7fffffff), gen_i8_constant(0x10), continue_block);
  ...
  // Truncate the operand to 32-bits - always round to nearest
  fastiss::f32_with_exception_flags_t trunc_result = gen_f32_trunc(reg2_pair_float64_flushed, gen_read_round_mode());
  ...
  trunc_result.inexact = gen_inexact_denorm_update(trunc_result.invalid, trunc_result.divide_by_zero, trunc_result.inexact, reg2_pair_float64);
  ...
  gen_update_dest_and_fpsr(false, reg3, gen_i32_cast(trunc_result.result), trunc_result_xp, continue_block);
