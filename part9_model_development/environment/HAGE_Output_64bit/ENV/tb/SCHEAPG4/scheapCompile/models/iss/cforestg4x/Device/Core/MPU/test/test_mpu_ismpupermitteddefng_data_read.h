/**
 * @file test_mpu_ismpupermitteddefng_data_read.h
 *
...
#define SET_HIT_PERMIT(INDEX) \
        ...
        if (um) {EXPECT_CALL(*(m_mpat_vec[INDEX]), GetUR()).WillOnce(Return(true));} \
        else {EXPECT_CALL(*(m_mpat_vec[INDEX]), GetSR()).WillOnce(Return(true));}
#define SET_HIT_NOT_PERMIT(INDEX) \
        EXPECT_CALL(*(m_mpla_vec[INDEX]), ReadBody()).WillRepeatedly(Return(addr)); \
        EXPECT_CALL(*(m_mpua_vec[INDEX]), ReadBody()).WillRepeatedly(Return(addr + size - 1)); \
        if (um) {EXPECT_CALL(*(m_mpat_vec[INDEX]), GetUR()).WillOnce(Return(false));} \
        else {EXPECT_CALL(*(m_mpat_vec[INDEX]), GetSR()).WillOnce(Return(false));}
