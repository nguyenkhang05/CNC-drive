...
//------------------------------------------------------------------------------
// ldm.mp [reg1], eh-et implementation
//------------------------------------------------------------------------------

rh850g4::ldm_dot_mp_ind_reg1_eh_dec_et_instr_t::ldm_dot_mp_ind_reg1_eh_dec_et_instr_t
(
...
void rh850g4::ldm_dot_mp_ind_reg1_eh_dec_et_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850g4::ldm_dot_mp_ind_reg1_eh_dec_et_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  disass_string << "ldm.mp [" << reg1_name << "], " << eh << "-" << et; 
  return disass_string.str();
}

unsigned int rh850g4::ldm_dot_mp_ind_reg1_eh_dec_et_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850g4::ldm_dot_mp_ind_reg1_eh_dec_et_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg1 = get_reg1_u8_0_4(opcode);
  SR_SELID_5_index_t eh = get_eh_u8_11_14(opcode);
  ...
  // Load the system registers (MPLA, MPUA, MPAT) from the memory
  if(eh <= et){
    ...
    while (cur <= end) {
      ...
      fastiss::i32_t *loaded_val_mpla = gen_i32_load(addr);
      gen_write_MPLA(loaded_val_mpla);  

      addr = gen_add(addr, gen_i32_constant(4));
      fastiss::i32_t *loaded_val_mpua = gen_i32_load(addr);
      gen_write_MPUA(loaded_val_mpua);  

      addr = gen_add(addr, gen_i32_constant(4));
      fastiss::i32_t *loaded_val_mpat = gen_i32_load(addr);
      gen_write_MPAT(loaded_val_mpat);  
...
unsigned int rh850g4::ldm_dot_mp_ind_reg1_eh_dec_et_instr_t::cycle_count(opcode_t opcode, unsigned int trans_PC)
{
  return 1;
}
