...
std::vector<std::vector<std::string>> softfloat_ceilf_dul;

char *test_data_dir;


class CEILFDULTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  CEILFDULTest() {}
  virtual ~CEILFDULTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check ceilf.dul
 * SoftFloat-3b modified
 */
TEST_P(CEILFDULTest, CheckCEILFDUL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCeilfDUL, CEILFDULTest,
        ::testing::ValuesIn(softfloat_ceilf_dul.begin(),
                            softfloat_ceilf_dul.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_ceilf_dul_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_ceilf_dul = parseTable(inFile, ",");
    inFile.close();
  }
