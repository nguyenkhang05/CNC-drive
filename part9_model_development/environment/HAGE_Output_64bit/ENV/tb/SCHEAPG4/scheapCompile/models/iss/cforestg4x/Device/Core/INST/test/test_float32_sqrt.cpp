...
std::vector<std::vector<std::string>> softfloat_sqrtfs;

char *test_data_dir;


class SQRTFSTest : public testing::Test,
...
/*!
 * @brief Check f32_sqrt
 * SoftFloat-3a
 */
TEST_P(SQRTFSTest, CheckF32Sqrt) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f32_sqrtfs(_)).Times(1).After(inits);
  ...
  float32_t ret = m_sf->f32_sqrtfs(v2);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatSqrtfs, SQRTFSTest,
        ::testing::ValuesIn(softfloat_sqrtfs.begin(),
                            softfloat_sqrtfs.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_sqrtf_s.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_sqrtfs = parseTable(inFile, ",");
    inFile.close();
  }
