...
namespace rh850
{
  void eiret_helper(rh850::immediate_base_t *instr_base, unsigned int ispr_val)
  {
    for(int i=0; i<16; i++){
      if(ispr_val & (0x1 << i)){
...
rh850::eiret_instr_t::eiret_instr_t
(
  ...
  rh850::instruction_base_impl_t(code_gen_impl, memory_impl, operand_impl, cpu_state_impl, custom_impl)
{
  ...
  code_gen_impl->append_i32(eiret_helper_args);
  code_gen_impl->add_helper("eiret_helper", (void*)&rh850::eiret_helper, code_gen_impl->create_void_func(eiret_helper_args));
...
void rh850::eiret_instr_t::decode(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850::eiret_instr_t::disassemble(rh850::opcode_t opcode, unsigned int trans_PC)
{
...
unsigned int rh850::eiret_instr_t::length(rh850::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850::eiret_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Create sub-blocks
  ...
  gen_cond_branch(gen_and(gen_icmp_eq(gen_read_INTCFG_ISPC(), gen_i1_constant(0)), gen_icmp_eq(psw_ep_val, gen_i1_constant(0))), update_ispr_block, keep_ispr_block);
...
bool rh850::eiret_instr_t::terminates_block(opcode_t opcode, unsigned int trans_PC)
{
  return true;
}

unsigned int rh850::eiret_instr_t::cycle_count(opcode_t opcode, unsigned int trans_PC)
{
  return 7;
}
