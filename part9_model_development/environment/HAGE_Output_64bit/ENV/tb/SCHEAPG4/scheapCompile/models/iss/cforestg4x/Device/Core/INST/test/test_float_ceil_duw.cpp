...
std::vector<std::vector<std::string>> softfloat_ceilf_duw;

char *test_data_dir;


class CEILFDUWTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  CEILFDUWTest() {}
  virtual ~CEILFDUWTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check ceilf.duw
 * SoftFloat-3b modified
 */
TEST_P(CEILFDUWTest, CheckCEILFDUW) {
  ...
  m_sf->SetRoundingMode(SoftFloat::RM_softfloat_round_max);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCeilfDUW, CEILFDUWTest,
        ::testing::ValuesIn(softfloat_ceilf_duw.begin(),
                            softfloat_ceilf_duw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_ceilf_duw_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_ceilf_duw = parseTable(inFile, ",");
    inFile.close();
  }
