...
std::vector<std::vector<std::string>> softfloat_negfs;

char *test_data_dir;


class NEGFSTest : public testing::Test,
...
TEST_P(NEGFSTest, CheckF32Neg) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f32_negfs(_)).Times(1).After(inits);
  ...
  float32_t ret = m_sf->f32_negfs(v2);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatNegfs, NEGFSTest,
        ::testing::ValuesIn(softfloat_negfs.begin(),
                            softfloat_negfs.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_negf_s.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_negfs = parseTable(inFile, ",");
    inFile.close();
  }
