...
std::vector<std::vector<std::string>> softfloat_cvtf_sul;

char *test_data_dir;


class CVTFSULTest : public testing::Test,
...
/*!
 * @brief Check cvtf.sul
 * SoftFloat-3b modified
 */
TEST_P(CVTFSULTest, CheckCVTFSUL) {
  ...
  uint64_t ret = m_sf->Cvt_f32_ui64(reg1_f32);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfSUL, CVTFSULTest,
        ::testing::ValuesIn(softfloat_cvtf_sul.begin(),
                            softfloat_cvtf_sul.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_sul_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cvtf_sul = parseTable(inFile, ",");
    inFile.close();
  }
