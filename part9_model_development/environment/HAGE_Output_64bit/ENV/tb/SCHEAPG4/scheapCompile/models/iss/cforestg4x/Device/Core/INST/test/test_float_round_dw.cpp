...
std::vector<std::vector<std::string>> softfloat_roundf_dw;

char *test_data_dir;


class ROUNDFDWTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  ROUNDFDWTest() {}
  virtual ~ROUNDFDWTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check roundf.dw
 * SoftFloat-3b modified
 */
TEST_P(ROUNDFDWTest, CheckROUNDFDW) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatRoundfDW, ROUNDFDWTest,
        ::testing::ValuesIn(softfloat_roundf_dw.begin(),
                            softfloat_roundf_dw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_roundf_dw_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_roundf_dw = parseTable(inFile, ",");
    inFile.close();
  }
