...
std::vector<std::vector<std::string>> softfloat_trncf_dl;

char *test_data_dir;


class TRNCFDLTest : public testing::Test,
...
/*!
 * @brief Check trncf.dl
 * SoftFloat-3b modified
 */
TEST_P(TRNCFDLTest, CheckTRNCFDL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatTrncfDL, TRNCFDLTest,
        ::testing::ValuesIn(softfloat_trncf_dl.begin(),
                            softfloat_trncf_dl.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_trncf_dl_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_trncf_dl = parseTable(inFile, ",");
    inFile.close();
  }
