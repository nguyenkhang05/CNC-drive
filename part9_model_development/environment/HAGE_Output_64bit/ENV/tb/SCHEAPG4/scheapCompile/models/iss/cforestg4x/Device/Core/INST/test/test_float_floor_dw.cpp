...
std::vector<std::vector<std::string>> softfloat_floorf_dw;

char *test_data_dir;


class FLOORFDWTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  FLOORFDWTest() {}
  virtual ~FLOORFDWTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check floorf.dw
 * SoftFloat-3b modified
 */
TEST_P(FLOORFDWTest, CheckFLOORFDW) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), Cvt_f64_i32(_)).Times(1).After(inits);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatFloorfDW, FLOORFDWTest,
        ::testing::ValuesIn(softfloat_floorf_dw.begin(),
                            softfloat_floorf_dw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_floorf_dw_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_floorf_dw = parseTable(inFile, ",");
    inFile.close();
  }
