...
std::vector<std::vector<std::string>> softfloat_floorf_sul;

char *test_data_dir;


class FLOORFSULTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  FLOORFSULTest() {}
  virtual ~FLOORFSULTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check floorf.sul
 * SoftFloat-3b modified
 */
TEST_P(FLOORFSULTest, CheckFLOORFSUL) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), Cvt_f32_ui64(_)).Times(1).After(inits);
  ...
  uint64_t ret = m_sf->Cvt_f32_ui64(v1);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatFloorSUL, FLOORFSULTest,
        ::testing::ValuesIn(softfloat_floorf_sul.begin(),
                            softfloat_floorf_sul.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_floorf_sul_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_floorf_sul = parseTable(inFile, ",");
    inFile.close();
  }
