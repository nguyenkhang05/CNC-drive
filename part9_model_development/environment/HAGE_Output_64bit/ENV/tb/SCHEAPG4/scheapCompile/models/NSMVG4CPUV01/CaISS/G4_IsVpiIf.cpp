...
/// <param name="vpiIf">pointer pointer of vpiIf</param>
/// <param name="num">I/F number</param>
G4_IsVpiIf::G4_IsVpiIf( G4_CaIss *pCaIss, 
		  BusMasterIf<BUS_WIDTH_VCI> **vpiIf,
		  unsigned int num ):
  mpCaIss( pCaIss ), 
  G4_IsIfBase(),
  mPacketId(1)
...
void G4_IsVpiIf::notifyTransSend( TlmTransactionType &trans, BusTime_t &t )
{
#if defined(BUSIF_TEST_LOG) || defined(DWACC_TEST)
  printf("\n");
  printf("G4_IsVpiIf::notifyTransSend: time=%s\n", t.to_string().c_str());
  fflush(stdout);
#endif
}
...
/// <param name="t">transport time</param>
/// <returns>none</returns>
void G4_IsVpiIf::notifyTransComp( TlmTransactionType &trans, BusTime_t &t )
{
#ifdef BUSIF_TEST_LOG
  printf("\n");
  printf("G4_IsVpiIf::notifyTransComp: time=%s\n", t.to_string().c_str());
  fflush(stdout);
#endif
...
/// <returns>none</returns>
void G4_IsVpiIf::CyclicHandler (void)
{
}
...
SysErrCause G4_IsVpiIf::TargetWrite (SideBand sideband, PhysAddr addr, uint8_t* p_data, ByteSize size)
{
  unsigned char peid = getBmId(sideband);

#if defined(BUSIF_TEST_LOG) || defined(DWACC_TEST)
  printf("\n");
  printf("G4_IsVpiIf::TargetWrite(Normal_S): ");
  ...
  unsigned char   *data          = new unsigned char[VPI_MAX_DATA];
  ...
  if(mPacketId != 0xffffffff) mPacketId++;
  else                        mPacketId = 1;
  ...
  isNotFull = mpVpiIf[peid]->write( *basic_payload,   // payload pointer
				      false,            // debug?
				      t,                // local time
				      false );          // with wait?

#ifdef BUSIF_TEST_LOG
  printf("G4_IsVpiIf::TargetWrite(Normal_E): Under operating transaction num=%d\n", mTransInfoQueue.size());
  printf("     ");
  for (int i=0; i<size; i++){
  ...
  printf("\n\n");
  fflush(stdout);
  printf("G4_IsVpiIf::TargetWrite(Normal_E): VCI Exp Payload: requset type=%d\n", (unsigned int)vci_ext->getRequestType());
  fflush(stdout);
#endif
...
/// <param name="size">data length</param>
/// <returns>SYSERR cause</returns>
SysErrCause G4_IsVpiIf::TargetRead  (SideBand sideband, PhysAddr addr, uint8_t* p_data, ByteSize size)
{
  unsigned char peid = getBmId(sideband);

#if defined(BUSIF_TEST_LOG) || defined(DWACC_TEST)
  printf("\n");
  printf("G4_IsVpiIf::TargetRead(Debug_S): ");
  printf("sideband(peid(0x%x),tcid(0x%x),vcid(0x%x),spid(0x%x),vm(%d),um(%d),bitpos(0x%0x),bitop(0x%x)), ", 
	 sideband.GetBMID(), 
	 0, 
	 0, 
  ...
  mpVpiIf[peid]->read( mDbgTrans,       // payload pointer
			true );          // debug?
  
#ifdef BUSIF_TEST_LOG
  printf("G4_IsVpiIf::TargetRead(Debug_E): ");
  for (int i=0; i<size; i++){
    printf("data%d=0x%08x, ", i, *(p_data+i));
  }
  printf("\n");
  printf("G4_IsVpiIf::TargetRead(Normal_S): ");
  printf("sideband(peid(0x%x),tcid(0x%x),vcid(0x%x),spid(0x%x),vm(%d),um(%d),bitpos(0x%0x),bitop(0x%x)), ", 
	 sideband.GetBMID(), 
	 0, 
	 0, 
  ...
  isNotFull = mpVpiIf[peid]->read( *basic_payload, // payload pointer
				     false,         // debug?
				     t,             // local time
				     false );       // with wait?

#ifdef BUSIF_TEST_LOG
  printf("G4_IsVpiIf::TargetRead(Normal_E): Under operating transaction num=%d\n", mTransInfoQueue.size());
  printf("\n");
  fflush(stdout);
  printf("G4_IsVpiIf::TargetRead(Normal_E): VCI Exp Payload: requset type=%d\n", (unsigned int)vci_ext->getRequestType());
  fflush(stdout);
#endif
...
/// <param name="size">data length</param>
/// <returns>SYSERR cause</returns>
SysErrCause G4_IsVpiIf::TargetFetch  (SideBand sideband, PhysAddr addr, uint8_t* p_data, ByteSize size)
{
  unsigned char peid = getBmId(sideband);
  for(int i=0; i<size; i++){
    *(p_data+i) = 0;
...
/// <param name="size">data length</param>
/// <returns>SYSERR cause</returns>
SysErrCause G4_IsVpiIf::TargetWriteDebug (PhysAddr addr, uint8_t* p_data, ByteSize size)
{
  setTransBasic( addr, size, p_data, mDbgTrans );
  setTransG3m( 0, 0, DEBUG_MASTER_BMID, 0, 0, 0, 0, mDbgTrans, mDbgG3mExt );
  setTransVci( TlmVciExtension::CMD_WRITE, mDbgTrans, mDbgVciExt );
  mpVpiIf[0]->write( mDbgTrans,       // payload pointer
		    true );          // debug?
...
/// <param name="size">data length</param>
/// <returns>SYSERR cause</returns>
SysErrCause G4_IsVpiIf::TargetReadDebug  (PhysAddr addr, uint8_t* p_data, ByteSize size)
{
  setTransBasic( addr, size, p_data, mDbgTrans );
  setTransG3m( 0, 0, DEBUG_MASTER_BMID, 0, 0, 0, 0, mDbgTrans, mDbgG3mExt );
  setTransVci( TlmVciExtension::CMD_READ, mDbgTrans, mDbgVciExt );
  mpVpiIf[0]->read( mDbgTrans,       // payload pointer
		   true );          // debug?
...
/// <returns>none</returns>
void G4_IsVpiIf::HardReset (void)
{
#ifdef TEST_RESET_DBG
  printf("%s G4_IsVpiIf::HardReset\n", sc_time_stamp().to_string().c_str()); 
  fflush(stdout);
#endif
  for( int j=0; j<VPI_MAX_DATA; j++ ){
    mDataDbg[j] = 0;
  }
}
...
/// <param name="trans">transaction</param>
/// <returns>none</returns>
void G4_IsVpiIf::DeleteTrans ( TlmTransactionType &trans )
{
...
/// <summary>get vpi transaction request type </summary>
/// <param name="sideband">sideband</param>
/// <returns>vci transaction requset type</returns>
TlmVciExtension::VciRequest_t G4_IsVpiIf::getReqType ( SideBand sideband, TlmVciExtension::VciRequest_t def_req )
{
