/**
 * @file test_float64_cmp.cpp
 *
 * @date Created on: 2017
 * @author Can Vy
 * @brief UNIT TEST for float64_cmp.
...
std::vector<std::vector<std::string>> softfloat_cmpfd;

char *test_data_dir;


class CmpfdTest : public testing::Test,
...
/*!
 * @brief Check f64_cmpfd
 * SoftFloat-3b
 */
TEST_P(CmpfdTest, CheckF64Cmp) {
  std::vector<std::string> cell = GetParam();
  uint32_t fcond = s2ul_32(cell[0]);
  uint64_t reg1 = s2ull_64(cell[1]);
  uint64_t reg2 = s2ull_64(cell[2]);
  uint32_t rm   = s2ul_32(cell[3]);  // FPSR.RM
  uint32_t fn   = s2ul_32(cell[4]);  // FPSR.FN
  uint32_t fs   = s2ul_32(cell[5]);  // FPSR.FS
  uint32_t xe_o = s2ul_32(cell[6]);  // FPSR.XE.O
  uint32_t xe_u = s2ul_32(cell[7]);  // FPSR.XE.U
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f64_cmp(_,_,_)).Times(1).After(inits);
  ...
  bool ret = m_sf->f64_cmp(v2, v1, fcond);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCmpfd, CmpfdTest,
                        ::testing::ValuesIn(softfloat_cmpfd.begin(), softfloat_cmpfd.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cmpf_d_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cmpfd = parseTable(inFile, ",");
    inFile.close();
  }
