...
std::vector<std::vector<std::string>> softfloat_roundf_duw;

char *test_data_dir;


class ROUNDFDUWTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  ROUNDFDUWTest() {}
  virtual ~ROUNDFDUWTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check roundf.duw
 * SoftFloat-3b modified
 */
TEST_P(ROUNDFDUWTest, CheckROUNDFDUW) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatRoundfDUW, ROUNDFDUWTest,
        ::testing::ValuesIn(softfloat_roundf_duw.begin(),
                            softfloat_roundf_duw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_roundf_duw_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_roundf_duw = parseTable(inFile, ",");
    inFile.close();
  }
