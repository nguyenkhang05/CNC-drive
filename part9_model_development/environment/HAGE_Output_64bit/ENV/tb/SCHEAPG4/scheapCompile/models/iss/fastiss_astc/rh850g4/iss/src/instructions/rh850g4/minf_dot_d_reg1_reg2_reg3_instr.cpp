...
//------------------------------------------------------------------------------
// minf.d reg1, reg2, reg3 implementation
//------------------------------------------------------------------------------

rh850g4::minf_dot_d_reg1_reg2_reg3_instr_t::minf_dot_d_reg1_reg2_reg3_instr_t
(
...
void rh850g4::minf_dot_d_reg1_reg2_reg3_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850g4::minf_dot_d_reg1_reg2_reg3_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  disass_string << "minf.d " << reg1_name << ", " << reg2_name << ", " << reg3_name; 
  return disass_string.str();
}

unsigned int rh850g4::minf_dot_d_reg1_reg2_reg3_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850g4::minf_dot_d_reg1_reg2_reg3_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg1 = get_reg1_u8_1_4_shift1(opcode);
  GR_index_t reg2 = get_reg2_u8_12_15_shift1(opcode);
  ...
  // Determine the MIN result
  fastiss::f64_t *min_result = gen_select(gen_is_quiet_nan(reg1_pair_float64), 
					  reg2_pair_float64, 
					  gen_select(gen_is_quiet_nan(reg2_pair_float64), 
						     reg1_pair_float64,
						     gen_select(gen_fcmp_ule(reg1_pair_float64, reg2_pair_float64), 
								reg1_pair_float64,
								reg2_pair_float64)));
  
  // Update the result, FPSR.XP, and handle exceptions
  gen_update_dest_and_fpsr(false, reg3, gen_i64_cast(min_result), gen_i8_constant(0), continue_block);
