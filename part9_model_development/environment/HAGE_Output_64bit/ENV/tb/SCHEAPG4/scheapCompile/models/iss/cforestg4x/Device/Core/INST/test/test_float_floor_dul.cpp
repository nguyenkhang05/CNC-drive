...
std::vector<std::vector<std::string>> softfloat_floorf_dul;

char *test_data_dir;


class FLOORFDULTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  FLOORFDULTest() {}
  virtual ~FLOORFDULTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check floorf.dul
 * SoftFloat-3b modified
 */
TEST_P(FLOORFDULTest, CheckFLOORFDUL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatFloorfDUL, FLOORFDULTest,
        ::testing::ValuesIn(softfloat_floorf_dul.begin(),
                            softfloat_floorf_dul.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_floorf_dul_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_floorf_dul = parseTable(inFile, ",");
    inFile.close();
  }
