...
std::vector<std::vector<std::string>> softfloat_cvtf_ls;

char *test_data_dir;


class CVTFLSTest : public testing::Test,
...
TEST_P(CVTFLSTest, CheckCVTFLS) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), Cvt_i64_f32(_)).Times(1).After(inits);
  ...
  float32_t ret_f32 = m_sf->Cvt_i64_f32(reg1_i64);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfLS, CVTFLSTest,
        ::testing::ValuesIn(softfloat_cvtf_ls.begin(),
                            softfloat_cvtf_ls.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_ls_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cvtf_ls = parseTable(inFile, ",");
    inFile.close();
  }
