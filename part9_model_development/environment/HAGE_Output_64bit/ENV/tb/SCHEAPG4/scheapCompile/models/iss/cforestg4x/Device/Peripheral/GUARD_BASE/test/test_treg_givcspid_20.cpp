/**
 * @file test_treg_givcspid_20.cpp
 *
 * @date Created on: 2016
 * @author Author: Hoanh Ho <hoanh.ho.px@renesas.com>
 * @brief UNIT TEST for GIVCSPID. (SAMPLE)
...
// ----------------------------------------
class TRegGIVCSPID1 : public testing::Test,
                     public testing::WithParamInterface<std::tuple<PhysAddr, uint8_t *, uint32_t>> {
protected:
    TRegGIVCSPID1() {}
    virtual ~TRegGIVCSPID1() {}
    virtual void SetUp() {
        m_treg_gspid   = new GIVCSPID (reg_size_gspid, min_access_size_gspid, m_reg_addr, m_channel, "GIVCSPID", &m_guard_internal);
    }
...
TEST_F(TRegGIVCSPID1, Check_Initial_Setup) {
    ASSERT_STREQ("GIVCSPID", m_treg_gspid->GetName());
 ...
 */
TEST_F(TRegGIVCSPID1, Check_Reset) {
    m_treg_gspid->Reset();
    uint8_t data[4] = {0x01, 0x01, 0x01, 0x01};
    uint32_t init_val = 0x00000000U;
    m_treg_gspid->ReadBody(m_reg_addr, data, SIZE_32BIT);
...
TEST_P(TRegGIVCSPID1, Check_Write) {
    ...
    if ((size == SIZE_8BIT) || (size == SIZE_16BIT) || (size == SIZE_32BIT)) {
        ...
        switch (size) {
        case SIZE_16BIT:
            EXPECT_EQ(0x0000FFFFU, m_treg_gspid->GetIvcSPID());
            break;
        case SIZE_8BIT:
            EXPECT_EQ(0x000000FFU, m_treg_gspid->GetIvcSPID());
            break;
        case SIZE_32BIT:
        default:
            EXPECT_EQ(0xFFFFFFFFU, m_treg_gspid->GetIvcSPID());
            break;
        }
    } else {
...
std::tuple<PhysAddr, uint8_t *, uint32_t> const GetSet_ParametersTable[] = {
    // INPUT_DATA of GIVCSPID(SideBand, PhysAddr, uint8_t *, ByteSize=)
...
INSTANTIATE_TEST_CASE_P(Test_Write_WithParameter, TRegGIVCSPID1,
                        ::testing::ValuesIn(GetSet_ParametersTable));


// ----------------------------------------
class TRegGIVCSPID2 : public testing::Test,
                     public testing::WithParamInterface<std::tuple<PhysAddr, uint8_t *, uint32_t, bool>> {
protected:
    TRegGIVCSPID2() {}
    virtual ~TRegGIVCSPID2() {}
    virtual void SetUp() {
        m_treg_gspid   = new GIVCSPID (reg_size_gspid, min_access_size_gspid, m_reg_addr, m_channel, "GIVCSPID", &m_guard_internal);
    }
    virtual void TearDown() {
...
TEST_F(TRegGIVCSPID2, Check_EnWrite_val) {
    ...
    EXPECT_EQ(0x00000000U, m_treg_gspid->GetIvcSPID());
...
TEST_P(TRegGIVCSPID2, Check_WriteBody) {
    {
        SideBand cur_sideband = sideband;
        PhysAddr cur_reg_addr = std::get<0>(GetParam());
        uint8_t *data     = std::get<1>(GetParam());
            ...
            if (is_write) {
                if (size == SIZE_32BIT) {
                    ...
                    if (addr_check == 0x0) {
                        ...
                        EXPECT_EQ(0xFFFF0000U, m_treg_gspid->GetIvcSPID());
                    }
                    ...
                    case 0x0:
                        ...
                        break;
                    case 0x1:
                        EXPECT_EQ(0x0000FF00U, m_treg_gspid->GetIvcSPID());
                        break;
                    case 0x2:
                        EXPECT_EQ(0x00FF0000U, m_treg_gspid->GetIvcSPID());
                        break;
                    case 0x3:
                    default:
                        EXPECT_EQ(0xFF000000U, m_treg_gspid->GetIvcSPID());
                        break;
                    }
                }
            } else {
...
std::tuple<PhysAddr, uint8_t *, uint32_t, bool> const GetSet_ParametersTable2[] = {
    // INPUT_DATA of GIVCSPID(PhysAddr, uint8_t *, ByteSize=)
...
INSTANTIATE_TEST_CASE_P(Test_WriteBody_WithParameter, TRegGIVCSPID2,
...
TEST_F(TRegGIVCSPID2, Check_SPID_val) {
    RegData gen_val = 0xCAFECAFEU;
    m_treg_gspid->SetIvcSPID(gen_val);
    EXPECT_EQ(gen_val, m_treg_gspid->GetIvcSPID());
    gen_val = 0xA5A5A5A5U;
    m_treg_gspid->SetIvcSPID(gen_val);
    EXPECT_EQ(gen_val, m_treg_gspid->GetIvcSPID());
}
