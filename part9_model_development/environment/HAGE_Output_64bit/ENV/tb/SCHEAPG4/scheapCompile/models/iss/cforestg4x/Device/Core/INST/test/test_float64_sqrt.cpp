...
std::vector<std::vector<std::string>> softfloat_sqrtfd;

char *test_data_dir;


class SQRTFDTest : public testing::Test,
...
/*!
 * @brief Check f64_sqrt
 * SoftFloat-3a
 */
TEST_P(SQRTFDTest, CheckF64Sqrt) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f64_sqrtfd(_)).Times(1).After(inits);
  ...
  float64_t ret = m_sf->f64_sqrtfd(v2);

  uint32_t fpu_exception_flag = m_sf->GetExceptionFlags();
  // with xe filter (See fpu_device.cpp: FpuOpeDevice::Sqrt
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatSqrtfd, SQRTFDTest,
        ::testing::ValuesIn(softfloat_sqrtfd.begin(),
                            softfloat_sqrtfd.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_sqrtf_d_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_sqrtfd = parseTable(inFile, ",");
    inFile.close();
  }
