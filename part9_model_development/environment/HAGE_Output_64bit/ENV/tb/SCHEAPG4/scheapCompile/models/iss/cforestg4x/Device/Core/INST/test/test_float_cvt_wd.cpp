...
std::vector<std::vector<std::string>> softfloat_cvtf_wd;

char *test_data_dir;


class CVTFWDTest : public testing::Test,
...
TEST_P(CVTFWDTest, CheckCVTFWD) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), Cvt_i32_f64(_)).Times(1).After(inits);
  ...
  FpuRegDouble ret;
  int32_t reg1_i32 = static_cast<int32_t> (reg1);
  float64_t ret_f64 = m_sf->Cvt_i32_f64(reg1_i32);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfWD, CVTFWDTest,
        ::testing::ValuesIn(softfloat_cvtf_wd.begin(),
                            softfloat_cvtf_wd.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_wd_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cvtf_wd = parseTable(inFile, ",");
    inFile.close();
  }
