...
std::vector<std::vector<std::string>> softfloat_floorf_sw;

char *test_data_dir;


class FLOORFSWTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  FLOORFSWTest() {}
  virtual ~FLOORFSWTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check floorf.sw
 * SoftFloat-3b modified
 */
TEST_P(FLOORFSWTest, CheckFLOORFSW) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfSW, FLOORFSWTest,
        ::testing::ValuesIn(softfloat_floorf_sw.begin(),
                            softfloat_floorf_sw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_floorf_sw.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_floorf_sw = parseTable(inFile, ",");
    inFile.close();
  }
