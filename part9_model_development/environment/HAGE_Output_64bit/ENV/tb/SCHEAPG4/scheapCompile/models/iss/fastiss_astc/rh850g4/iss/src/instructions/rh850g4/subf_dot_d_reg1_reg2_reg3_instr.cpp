...
//------------------------------------------------------------------------------
// subf.d reg1, reg2, reg3 implementation
//------------------------------------------------------------------------------

rh850g4::subf_dot_d_reg1_reg2_reg3_instr_t::subf_dot_d_reg1_reg2_reg3_instr_t
(
...
void rh850g4::subf_dot_d_reg1_reg2_reg3_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850g4::subf_dot_d_reg1_reg2_reg3_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  disass_string << "subf.d " << reg1_name << ", " << reg2_name << ", " << reg3_name; 
  return disass_string.str();
}

unsigned int rh850g4::subf_dot_d_reg1_reg2_reg3_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850g4::subf_dot_d_reg1_reg2_reg3_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg1 = get_reg1_u8_1_4_shift1(opcode);
  GR_index_t reg2 = get_reg2_u8_12_15_shift1(opcode);
  ...
  // Check if both operands are infinity with the same sign
  ...
  fastiss::f64_with_exception_flags_t sub_result = gen_f64_sub(reg2_float80, reg1_float80, gen_read_round_mode());
  ...
  (
    ...
    gen_f64_cast(gen_and(gen_i64_cast(sub_result.result), gen_i64_constant(0x8000000000000000ULL))),
    sub_result.result
  );
  ...
  sub_result.inexact = gen_inexact_denorm_update(sub_result.invalid, sub_result.divide_by_zero, sub_result.inexact, reg1_pair_float64, reg2_pair_float64);

  // Update the result, FPSR.XP, and handle exceptions
  sub_result.result = gen_select(gen_is_quiet_nan(reg2_pair_float64),
				 reg2_pair_float64,
  ...
  gen_update_dest_and_fpsr(false, reg3, gen_i64_cast(sub_result.result), sub_result_xp, continue_block);
