...
std::vector<std::vector<std::string>> softfloat_roundf_sw;

char *test_data_dir;


class ROUNDFSWTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  ROUNDFSWTest() {}
  virtual ~ROUNDFSWTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check roundf.sw
 * SoftFloat-3b modified
 */
TEST_P(ROUNDFSWTest, CheckROUNDFSW) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfSW, ROUNDFSWTest,
        ::testing::ValuesIn(softfloat_roundf_sw.begin(),
                            softfloat_roundf_sw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_roundf_sw.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_roundf_sw = parseTable(inFile, ",");
    inFile.close();
  }
