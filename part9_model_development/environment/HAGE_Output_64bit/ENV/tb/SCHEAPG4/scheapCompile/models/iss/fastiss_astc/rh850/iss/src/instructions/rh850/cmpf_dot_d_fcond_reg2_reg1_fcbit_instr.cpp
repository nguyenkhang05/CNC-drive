...
rh850::cmpf_dot_d_fcond_reg2_reg1_fcbit_instr_t::cmpf_dot_d_fcond_reg2_reg1_fcbit_instr_t
(
...
void rh850::cmpf_dot_d_fcond_reg2_reg1_fcbit_instr_t::decode(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850::cmpf_dot_d_fcond_reg2_reg1_fcbit_instr_t::disassemble(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
...
unsigned int rh850::cmpf_dot_d_fcond_reg2_reg1_fcbit_instr_t::length(rh850::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850::cmpf_dot_d_fcond_reg2_reg1_fcbit_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg1 = get_reg1_u8_1_4_shift1(opcode);
  GR_index_t reg2 = get_reg2_u8_12_15_shift1(opcode);
  ...
  fastiss::block_t *fpi_branch = create_basic_block();
  fastiss::block_t *no_excep_branch = create_basic_block();
  ...
  gen_update_dest_and_fpsr(false, GR_INDEX_R0_0, gen_i32_constant(0), gen_shl(gen_i8_zext(invalid_bit), gen_i8_constant(4)), continue_block);
  ...
  gen_cond_branch(gen_icmp_eq(gen_read_FPEC(), gen_i32_constant(1)), fpi_branch, no_excep_branch);

  // no exception case
  set_gen_point(no_excep_branch); 
  ...
  gen_branch(fpi_branch);

  // fpi case
  set_gen_point(fpi_branch); 
