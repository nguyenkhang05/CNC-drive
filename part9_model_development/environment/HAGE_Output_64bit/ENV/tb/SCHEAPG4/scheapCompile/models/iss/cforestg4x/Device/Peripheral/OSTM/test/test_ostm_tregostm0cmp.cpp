...
public:
  ...
  PhysAddr start_addr = 0xFFEC0000;
...
TEST_F(OstmRegAccessTest, CheckCMP_GetName) {
    ...
    TRegOSTMnCMP* OSTMnCMP = new TRegOSTMnCMP(m_ostm0_core);

    ASSERT_STREQ("OSTMnCMP", OSTMnCMP->GetName());
...
TEST_F(OstmRegAccessTest, CheckCMP_InitalValue) {
    for (ostmch = 0; ostmch < MAX_OSTM_NUM; ostmch++){
        ...
        ASSERT_EQ(expect_data, OSTM0CMP->GetVal());
        delete OSTM0CMP;
        delete m_ostm0_core;
        delete m_ostm[ostmch];
    }
...
TEST_F(OstmRegAccessTest, CheckCMP_ReadWrite_1) {
    for (ostmch = 0; ostmch < MAX_OSTM_NUM; ostmch++){
        pe_id = ostmch;
        PhysAddr base_addr_pe = base_addr + 0x1000*ostmch;
        ...
        OSTM0CMP->Write(sideband, start_addr_pe, wdata, SIZE_32BIT);
        OSTM0CMP->Read(sideband, start_addr_pe, (uint8_t*)data, SIZE_32BIT);
...
TEST_F(OstmRegAccessTest, CheckCMP_ReadWrite_2) {
    for (ostmch = 0; ostmch < MAX_OSTM_NUM; ostmch++){
        pe_id = ostmch;
        PhysAddr base_addr_pe = base_addr + 0x1000*ostmch;
        PhysAddr start_addr_pe = start_addr + 0x1000*ostmch;
        ...
        uint32_t data_input = 0x0A0A0A0A;
        uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);

        OSTM0CMP->Write(sideband, start_addr_pe, wdata, SIZE_32BIT);
        OSTM0CMP->Read(sideband, start_addr_pe, (uint8_t*)data, SIZE_32BIT);

        uint32_t expect_data = 0x0A0A0A0A;
        memcpy (&m_data32, data, sizeof(m_data32));
        ASSERT_EQ(expect_data, m_data32);
...
std::tuple<PhysAddr, int32_t, ByteSize, ByteSize, int32_t> const RW_ParametersTable[] = {
  /* 0000 */ std::make_tuple(0xFFEC0000 /* CMP */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
  /* 0000 */ std::make_tuple(0xFFEC0001 /* CMP */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
  /* 0000 */ std::make_tuple(0xFFEC0002 /* CMP */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
  /* 0000 */ std::make_tuple(0xFFEC0003 /* CMP */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
  /* 0000 */ std::make_tuple(0xFFEC0000 /* CMP */, 0xf3f2ffff, SIZE_16BIT, SIZE_16BIT, 0x0), //16bit access
  /* 0000 */ std::make_tuple(0xFFEC0002 /* CMP */, 0xf3f2ffff, SIZE_16BIT, SIZE_16BIT, 0x0), //16bit access
  /* 0000 */ std::make_tuple(0xFFEC0000 /* CMP */, 0xf3f2ffff, SIZE_32BIT, SIZE_32BIT, 0xF3F2FFFF), //32bit access
