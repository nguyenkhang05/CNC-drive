/**
 * @file test_intc2_tregeic_intc2_20.cpp
 *
...
public:
  ...
  TRegEIC* eic;
  uint32_t pe_id = 0;
  const PhysAddr eic_base_addr = 0xFFF80000UL;
  const uint32_t INTC2_CHANNEL = 33;
  const uint32_t INTC2_EIC_SIZE = 0x02;
...
TEST_F(TRegEICAccessPermission, IsRegWriteEnable) {
    cintc2->HardReset();
    eic = new TRegEIC (SIZE_16BIT, SIZE_8BIT, eic_base_addr + INTC2_CHANNEL * INTC2_EIC_SIZE, INTC2_CHANNEL, pe_id, cintc2);
    eic->Reset();
...
TEST_F(TRegEICAccessPermission, IsRegReadEnable) {
    uint32_t ch = 0;
    cintc2->HardReset();
    eic = new TRegEIC (SIZE_16BIT, SIZE_8BIT, eic_base_addr + INTC2_CHANNEL * INTC2_EIC_SIZE, INTC2_CHANNEL, pe_id, cintc2);
    eic->Reset();
...
TEST_F(TRegEICBitSetTest, CheckEIC_UpdateEICHighPriority_1) {
    ...
    ASSERT_EQ(0x20U, cintc2->m_intc2_priority[pe_id]->GetChannelEiint());
