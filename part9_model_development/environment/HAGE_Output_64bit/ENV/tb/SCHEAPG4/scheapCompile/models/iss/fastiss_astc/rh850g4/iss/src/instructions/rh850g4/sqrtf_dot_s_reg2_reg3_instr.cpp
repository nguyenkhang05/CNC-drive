...
//------------------------------------------------------------------------------
// sqrtf.s reg2, reg3 implementation
//------------------------------------------------------------------------------

rh850g4::sqrtf_dot_s_reg2_reg3_instr_t::sqrtf_dot_s_reg2_reg3_instr_t
(
...
void rh850g4::sqrtf_dot_s_reg2_reg3_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850g4::sqrtf_dot_s_reg2_reg3_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  disass_string << "sqrtf.s " << reg2_name << ", " << reg3_name; 
  return disass_string.str();
}

unsigned int rh850g4::sqrtf_dot_s_reg2_reg3_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850g4::sqrtf_dot_s_reg2_reg3_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg2 = get_reg2_u8_11_15(opcode);
  GR_index_t reg3 = get_reg3_u8_27_31(opcode);
  ...
  fastiss::i1_t* is_zero = gen_is_zero(reg2_float32_flushed);
  fastiss::i1_t* is_quiet_nan = gen_is_quiet_nan(reg2_float32);
  ...
  gen_update_dest_and_fpsr(true, reg3, gen_i32_cast(reg2_float32_flushed), gen_i8_zext(gen_is_denorm(reg2_float32)), continue_block);  
  ...
  fastiss::f32_with_exception_flags_t sqrt_result = gen_f32_sqrt(reg2_float80, gen_read_round_mode());
  ...
  sqrt_result.inexact = gen_inexact_denorm_update(sqrt_result.invalid, sqrt_result.divide_by_zero, sqrt_result.inexact, reg2_float32);
  ...
  gen_update_dest_and_fpsr(true, reg3, gen_i32_cast(sqrt_result.result), sqrt_result_xp, continue_block);
...
unsigned int rh850g4::sqrtf_dot_s_reg2_reg3_instr_t::cycle_count(opcode_t opcode, unsigned int trans_PC)
{
  return 14;
}
