...
std::vector<std::vector<std::string>> softfloat_ceilf_dw;

char *test_data_dir;


class CEILFDWTest : public testing::Test,
...
/*!
 * @brief Check ceilf.dw
 * SoftFloat-3b modified
 */
TEST_P(CEILFDWTest, CheckCEILFDW) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCeilfDW, CEILFDWTest,
        ::testing::ValuesIn(softfloat_ceilf_dw.begin(),
                            softfloat_ceilf_dw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_ceilf_dw_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_ceilf_dw = parseTable(inFile, ",");
    inFile.close();
  }
