...
std::vector<std::vector<std::string>> softfloat_trncf_sl;

char *test_data_dir;


class TRNCFSLTest : public testing::Test,
...
/*!
 * @brief Check trncf.sl
 * SoftFloat-3b modified
 */
TEST_P(TRNCFSLTest, CheckTRNCFSL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatTrncSL, TRNCFSLTest,
        ::testing::ValuesIn(softfloat_trncf_sl.begin(),
                            softfloat_trncf_sl.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_trncf_sl_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_trncf_sl = parseTable(inFile, ",");
    inFile.close();
  }
