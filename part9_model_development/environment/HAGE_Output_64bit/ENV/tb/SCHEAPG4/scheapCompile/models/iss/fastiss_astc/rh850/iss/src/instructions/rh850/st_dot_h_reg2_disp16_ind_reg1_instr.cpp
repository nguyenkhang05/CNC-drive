...
//------------------------------------------------------------------------------
// st.h reg2, disp16 [reg1] implementation
//------------------------------------------------------------------------------

rh850::st_dot_h_reg2_disp16_ind_reg1_instr_t::st_dot_h_reg2_disp16_ind_reg1_instr_t
(
...
void rh850::st_dot_h_reg2_disp16_ind_reg1_instr_t::decode(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850::st_dot_h_reg2_disp16_ind_reg1_instr_t::disassemble(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  if (disp16 < 0)
  {
    disass_string << "st.h " << reg2_name << ", -0x" << std::hex << -disp16 << "[" << reg1_name << "]";
  }
  else
  {
    disass_string << "st.h " << reg2_name << ", 0x" << std::hex << disp16 << "[" << reg1_name << "]";
...
unsigned int rh850::st_dot_h_reg2_disp16_ind_reg1_instr_t::length(rh850::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850::st_dot_h_reg2_disp16_ind_reg1_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg2 = get_reg2_u8_11_15(opcode);
  ...
  if (is_mdp_enable()){
    ...
    fastiss::i32_t *mpu_func_result = call_helper(
      ...
      addr,
      gen_or(gen_shl(gen_i32_constant(reg1),gen_i32_constant(16)), gen_i32_constant(0x203)),
      NULL
    );
    gen_cond_branch(
