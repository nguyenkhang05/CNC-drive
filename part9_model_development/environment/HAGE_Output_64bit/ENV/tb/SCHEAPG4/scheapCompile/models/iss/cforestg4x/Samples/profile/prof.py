...
start_node="_CallFuncs"
bench_name='output/cft'
# Only the output without running: True
# Normal operation: False
FLAG_ONLY_OUTPUT = False

# import pdb; pdb.set_trace() # for debug

ml('./cforest_test1.elf')
result,entry_point=getSymbolAddress('__start')
if result == 0:
   setreg("PC", entry_point)
   print("ENTRY POINT: %08x" % entry_point)
   print("PC: %08x" % getreg("PC"))
else:
   print("Can't find entry point")
   quit()

prof=profiler()
prof.set_plugin('callgraph', plugin_callgraph())
prof.set_plugin('cycle', plugin_cycle())

# Setup profiler
prof.setup()
# Set start point
prof.set_profile_function(start_node)

if not FLAG_ONLY_OUTPUT:
   # Start profiler
   prof.start()
   # save data
   save_plugin_data(bench_name)

# reload the data
load_plugin_data(bench_name)

# Save the data in CSV format
save_csv(bench_name)

# Save the call-graph data in SVG format
save_callgraph(bench_name)
