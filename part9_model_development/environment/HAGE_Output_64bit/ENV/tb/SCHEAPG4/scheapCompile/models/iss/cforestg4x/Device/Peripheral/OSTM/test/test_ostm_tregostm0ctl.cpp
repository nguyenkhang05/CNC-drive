...
public:
  ...
  PhysAddr start_addr = 0xFFEC0020;
...
TEST_F(OstmRegAccessTest, CheckCTL_GetName) {
    ...
    TRegOSTMnCTL* OSTM0CTL = new TRegOSTMnCTL(m_ostm0_core);

    ASSERT_STREQ("OSTMnCTL", OSTM0CTL->GetName());
...
TEST_F(OstmRegAccessTest, CheckCTL_InitalValue) {
    for (ostmch = 0; ostmch < MAX_OSTM_NUM; ostmch++){
        ...
        ASSERT_EQ(expect_data, OSTM0CTL->GetVal());
        delete OSTM0CTL;
        delete m_ostm0_core;
        delete m_ostm[ostmch];
    }
...
std::tuple<PhysAddr, int32_t, ByteSize, ByteSize, int32_t> const RW_ParametersTable[] = {
  /* 0000 */ std::make_tuple(0xFFEC0020 /* CTL */, 0xf3f2ffff, SIZE_16BIT, SIZE_16BIT, 0x0), //16bit access
  /* 0000 */ std::make_tuple(0xFFEC0022 /* CTL */, 0xf3f2ffff, SIZE_16BIT, SIZE_16BIT, 0x0), //16bit access
  /* 0000 */ std::make_tuple(0xFFEC0020 /* CTL */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x87), //8bit access
  /* 0000 */ std::make_tuple(0xFFEC0020 /* CTL */, 0xf3f2ffff, SIZE_32BIT, SIZE_32BIT, 0x0), //32bit access
  /* 0000 */ std::make_tuple(0xFFEC0021 /* CTL */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
  /* 0000 */ std::make_tuple(0xFFEC0022 /* CTL */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
  /* 0000 */ std::make_tuple(0xFFEC0023 /* CTL */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
...
TEST_F(OstmFunctionTest, CheckOstmFunction_MD1_0_MD2_0) {
  int32_t temp = 0x0;  //MD1 = 0, MD2 = 0
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&temp);

  int32_t temp1 = 0x1;  //TS = 1
  ...
  for (ostmch = 0; ostmch < MAX_OSTM_NUM; ostmch++){
      ...
      m_ostm[ostmch]->GetOstmCore()->SetOSTMnCMP(0xA);
      ...
      ASSERT_NO_THROW(m_ostm[ostmch]->TargetWrite(sideband, start_addr_pe_TS, wdata1, SIZE_8BIT));
      ASSERT_EQ(m_ostm[ostmch]->GetOstmCore()->GetOSTMnCNT(), m_ostm[ostmch]->GetOstmCore()->GetOSTMnCMP());
      delete m_ostm[ostmch];
  }

}

TEST_F(OstmFunctionTest, CheckOstmFunction_MD1_1_MD2_0) {
  int32_t temp = 0x2;  //MD1 = 1, MD2 = 0
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&temp);
  ...
  for (ostmch = 0; ostmch < MAX_OSTM_NUM; ostmch++){
      ...
      ASSERT_NO_THROW(m_ostm[ostmch]->TargetWrite(sideband, start_addr_pe, wdata, SIZE_8BIT));
      ASSERT_NO_THROW(m_ostm[ostmch]->TargetWrite(sideband, start_addr_pe_TS, wdata1, SIZE_8BIT));
      ASSERT_EQ(0x0U,m_ostm[ostmch]->GetOstmCore()->GetOSTMnCNT());

      delete m_ostm[ostmch];
  }
}

TEST_F(OstmFunctionTest, CheckOstmFunction_MD1_0_MD2_1) {
  int32_t temp = 0x4;  //MD1 = 0, MD2 = 1
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&temp);
...
TEST_F(OstmFunctionTest, CheckOstmFunction_MD1_1_MD2_1) {
  int32_t temp = 0x6;  //MD1 = 1, MD2 = 1
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&temp);
