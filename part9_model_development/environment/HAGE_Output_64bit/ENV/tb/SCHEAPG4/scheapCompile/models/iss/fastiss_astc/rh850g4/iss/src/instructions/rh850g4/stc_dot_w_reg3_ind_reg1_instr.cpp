...
rh850g4::stc_dot_w_reg3_ind_reg1_instr_t::stc_dot_w_reg3_ind_reg1_instr_t
(
...
void rh850g4::stc_dot_w_reg3_ind_reg1_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850g4::stc_dot_w_reg3_ind_reg1_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
...
unsigned int rh850g4::stc_dot_w_reg3_ind_reg1_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850g4::stc_dot_w_reg3_ind_reg1_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg3 = get_reg3_u8_27_31(opcode);
  GR_index_t reg1 = get_reg1_u8_0_4(opcode);
  ...
  if(is_link_func_enable()){
    ...
    link_stat = call_helper("is_link_valid_helper", gen_read_link_manager_ptr(), gen_read_PEID(), addr, gen_i32_constant(4), NULL);
  }
  ...
  if(is_link_func_enable()){
    // clear the link information
    // temp code !!!!!!! need re-check for Link
    call_helper("clear_link_helper", gen_read_link_manager_ptr(), gen_read_PEID(), addr, gen_i32_constant(4), NULL);
  }
...
unsigned int rh850g4::stc_dot_w_reg3_ind_reg1_instr_t::cycle_count(opcode_t opcode, unsigned int trans_PC)
{
  return 8;
}
