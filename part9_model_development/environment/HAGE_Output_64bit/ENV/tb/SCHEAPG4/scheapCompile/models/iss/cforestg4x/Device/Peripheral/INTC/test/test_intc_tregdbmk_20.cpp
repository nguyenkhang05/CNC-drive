/**
 * @file test_intc_tregdbmk_20.cpp
 *
...
TEST_F(TregDBMKAccessTest, CheckDBMK_IsRegReadEnable) {
  intc1->HardReset();
  TRegDBMK *dbmk;
  dbmk = new TRegDBMK (reg_size, min_access_size, base_addr, intc1);
  dbmk->SetEI(0x0);
  ...
  sideband.SetGM(0);
  sideband.SetUM(0);
  ASSERT_EQ(true, !dbmk->IsRegReadEnable(sideband));
...
TEST_F(TregDBMKAccessTest, CheckDBMK_IsRegWriteEnable) {
  intc1->HardReset();
  TRegDBMK *dbmk;
  dbmk = new TRegDBMK (reg_size, min_access_size, base_addr, intc1);
  dbmk->SetEI(0x0);
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_HV) {
  ...
  SysErrCause syserr = dbmk->Write(sideband, base_addr, wdata, SIZE_16BIT);
  ASSERT_EQ(0x0U, syserr);
  syserr = dbmk->Read(sideband, base_addr, (uint8_t*)data, SIZE_16BIT);
  ASSERT_EQ(0x0U, syserr);
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_HV_32bits) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
  ...
  SysErrCause syserr = dbmk->Write(sideband, base_addr, wdata, SIZE_32BIT);
  ASSERT_EQ(0x0U, syserr);
  syserr = dbmk->Read(sideband, base_addr, (uint8_t*)data, SIZE_32BIT);
  ASSERT_EQ(0x0U, syserr);
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_HV_64bits) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
  ...
  SysErrCause syserr = dbmk->Write(sideband, base_addr, wdata, SIZE_64BIT);
  ASSERT_EQ(0x0U, syserr);
  syserr = dbmk->Read(sideband, base_addr, (uint8_t*)data, SIZE_64BIT);
  ASSERT_EQ(0x0U, syserr);
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_HV_128bits) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
  ...
  SysErrCause syserr = dbmk->Write(sideband, base_addr, wdata, SIZE_128BIT);
  ASSERT_EQ(0x0U, syserr);
  syserr = dbmk->Read(sideband, base_addr, (uint8_t*)data, SIZE_128BIT);
  ASSERT_EQ(0x0U, syserr);
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest2_HV) {
  ...
  SysErrCause syserr = dbmk->Write(sideband, base_addr, wdata, SIZE_8BIT);
  ASSERT_EQ(0x0U, syserr);
  syserr = dbmk->Read(sideband, base_addr, (uint8_t*)data, SIZE_8BIT);
  ASSERT_EQ(0x0U, syserr);
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest3_HV) {
  uint16_t data_input = 0xff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
  ...
  SysErrCause syserr = dbmk->Write(sideband, base_addr + 1, wdata, SIZE_8BIT);
  ASSERT_EQ(0x0U, syserr);
  syserr = dbmk->Read(sideband, base_addr + 1, (uint8_t*)data, SIZE_8BIT);
  ASSERT_EQ(0x0U, syserr);
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_SV) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_SV_32bits) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_SV_64bits) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_SV_128bits) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest2_SV) {
  uint16_t data_input = 0xff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest3_SV) {
  uint16_t data_input = 0xff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_UM) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_UM_32bits) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_UM_64bits) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_UM_128bits) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest2_UM) {
  uint16_t data_input = 0xff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest3_UM) {
  uint16_t data_input = 0xff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_DM_HV) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest2_DM_HV) {
  uint16_t data_input = 0xff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest3_DM_HV) {
  uint16_t data_input = 0xff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest1_SV_DM) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest2_SV_DM) {
  uint16_t data_input = 0xff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTest3_SV_DM) {
  uint16_t data_input = 0xff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
...
TEST_F(TregDBMKAccessTest, CheckDBMK_AccessTestIHVE0) {
  uint32_t data_input = 0xffff;
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&data_input);
  intc1->HardReset();
  intc1->SetIHVEFromIHVCFG(0);
  ...
  sideband.SetDM(0);
  sideband.SetUM(1);
  syserr = dbmk->Write(sideband, base_addr, wdata, SIZE_16BIT);
  ASSERT_EQ(0x0U, syserr);
  syserr = dbmk->Read(sideband, base_addr, (uint8_t*)data, SIZE_16BIT);
  ASSERT_EQ(0x0U, syserr);
