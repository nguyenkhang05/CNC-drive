...
rh850::pushsp_rh_dec_rt_instr_t::pushsp_rh_dec_rt_instr_t
(
...
void rh850::pushsp_rh_dec_rt_instr_t::decode(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850::pushsp_rh_dec_rt_instr_t::disassemble(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
...
unsigned int rh850::pushsp_rh_dec_rt_instr_t::length(rh850::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850::pushsp_rh_dec_rt_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t rh = get_rh_u8_0_4(opcode);
  GR_index_t rt = get_rt_u8_27_31(opcode);
  ...
  if(rh <= rt){
    GR_index_t cur = rh;
    GR_index_t end = rt;
    //fastiss::block_t *continue_block[32];
    while (cur <= end)
    {
...
unsigned int rh850::pushsp_rh_dec_rt_instr_t::cycle_count(opcode_t opcode, unsigned int trans_PC)
{
  GR_index_t rh = get_rh_u8_0_4(opcode);
  GR_index_t rt = get_rt_u8_27_31(opcode);

  // Determine the number of pops
  unsigned int num_pops = ((unsigned int)((rh <= rt) ? (rt - rh) : 0) / 2);
  return (num_pops + 1);
}
