...
std::vector<std::vector<std::string>> softfloat_roundf_dl;

char *test_data_dir;


class ROUNDFDLTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  ROUNDFDLTest() {}
  virtual ~ROUNDFDLTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check roundf.dl
 * SoftFloat-3b modified
 */
TEST_P(ROUNDFDLTest, CheckROUNDFDL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatRoundfDL, ROUNDFDLTest,
        ::testing::ValuesIn(softfloat_roundf_dl.begin(),
                            softfloat_roundf_dl.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_roundf_dl_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_roundf_dl = parseTable(inFile, ",");
    inFile.close();
  }
