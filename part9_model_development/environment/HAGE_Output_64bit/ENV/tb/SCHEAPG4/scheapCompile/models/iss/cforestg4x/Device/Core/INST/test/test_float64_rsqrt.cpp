...
std::vector<std::vector<std::string>> softfloat_rsqrtfd;

char *test_data_dir;


class RSQRTFDTest : public testing::Test,
...
/*!
 * @brief Check rsqrtf.s
 * SoftFloat-3b modified
 */
TEST_P(RSQRTFDTest, CheckRSQRTFD) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f64_rsqrt(_)).Times(1).After(inits);
  ...
  float64_t reg1_val;
  reg1_val.v = reg1;
  float64_t ret_f64 = m_sf->f64_rsqrt(reg1_val);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloat64Rsqrt, RSQRTFDTest,
        ::testing::ValuesIn(softfloat_rsqrtfd.begin(),
                            softfloat_rsqrtfd.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_rsqrtf_d_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_rsqrtfd = parseTable(inFile, ",");
    inFile.close();
  }
