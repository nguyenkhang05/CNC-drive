...
class G4_IsVpiIf : public BusMasterFuncIf, public CModuleVc, public G4_IsIfBase
{
  public:
    G4_IsVpiIf( G4_CaIss *pCaIss, 
		BusMasterIf<BUS_WIDTH_VCI> **vpiIf, 
		unsigned int num = PE_MAX_NUM);
    ~G4_IsVpiIf( void );
  ...
  private:
    ...
    BusMasterIf<BUS_WIDTH_VCI> *mpVpiIf[PE_MAX_NUM];    // for the pointer of BusMasterIf<BUS_WIDTH_VPI>'s instance
    std::list<trans_info*>      mTransInfoQueue;        // for transaction information list

    unsigned char               mDataDbg[VPI_MAX_DATA]; // for data (debug access)
    ...
    unsigned int                mPacketId;              // for TlmVpiExtension's packet ID
}; // end of IsVpiIf definition
