...
public:
  ...
  PhysAddr start_addr = 0xFFEC0018;
...
TEST_F(OstmRegAccessTest, CheckTT_GetName) {
    ...
    TRegOSTMnTT* OSTM0TT = new TRegOSTMnTT(m_ostm0_core);

    ASSERT_STREQ("OSTMnTT", OSTM0TT->GetName());
...
TEST_F(OstmRegAccessTest, CheckTT_InitalValue) {
    for (ostmch = 0; ostmch < MAX_OSTM_NUM; ostmch++){
        ...
        ASSERT_EQ(expect_data, OSTM0TT->GetVal());
        delete OSTM0TT;
        delete m_ostm0_core;
        delete m_ostm[ostmch];
    }
...
std::tuple<PhysAddr, int32_t, ByteSize, ByteSize, int32_t> const RW_ParametersTable[] = {
  /* 0000 */ std::make_tuple(0xFFEC0018 /* TT */, 0xf3f2ffff, SIZE_16BIT, SIZE_16BIT, 0x0), //16bit access
  /* 0000 */ std::make_tuple(0xFFEC001A /* TT */, 0xf3f2ffff, SIZE_16BIT, SIZE_16BIT, 0x0), //16bit access
  /* 0000 */ std::make_tuple(0xFFEC0018 /* TT */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
  /* 0000 */ std::make_tuple(0xFFEC0018 /* TT */, 0xf3f2ffff, SIZE_32BIT, SIZE_32BIT, 0x0), //32bit access
  /* 0000 */ std::make_tuple(0xFFEC0019 /* TT */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
  /* 0000 */ std::make_tuple(0xFFEC001A /* TT */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
  /* 0000 */ std::make_tuple(0xFFEC001B /* TT */, 0xf3f2ffff, SIZE_8BIT, SIZE_8BIT, 0x0), //8bit access
...
TEST_F(OstmFunctionTest, CheckOstmFunction_TT_0) {
  int32_t temp = 0x2;  //TT = 0
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&temp);

  for (ostmch = 0; ostmch < MAX_OSTM_NUM; ostmch++){
      pe_id = ostmch;
      PhysAddr base_addr_pe = base_addr + 0x1000*ostmch;
      PhysAddr start_addr_pe_TT = start_addr + 0x1000*ostmch;
      m_ostm[ostmch] = new OstmIO(pe_id, intc1, cintc2, base_addr_pe, intch, ostmch, mock_fsbus, INIT_OSTM_RATIO);
      m_ostm[ostmch]->HardReset();

      m_ostm[ostmch]->GetOstmCore()->SetOSTMnTE(0x1);
      ASSERT_NO_THROW(m_ostm[ostmch]->GetOstmCore()->SetRUN());   //Status: OSTM_RUN
      ASSERT_NO_THROW(m_ostm[ostmch]->TargetWrite(sideband, start_addr_pe_TT, wdata, SIZE_8BIT));
...
TEST_F(OstmFunctionTest, CheckOstmFunction_TT_1) {
  int32_t temp = 0x1;  //TT =1
  uint8_t *wdata = reinterpret_cast<uint8_t *>(&temp);
