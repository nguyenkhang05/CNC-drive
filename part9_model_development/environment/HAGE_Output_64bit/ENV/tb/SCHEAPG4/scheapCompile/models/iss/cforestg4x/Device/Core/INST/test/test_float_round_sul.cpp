...
std::vector<std::vector<std::string>> softfloat_roundf_sul;

char *test_data_dir;


class ROUNDFSULTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  ROUNDFSULTest() {}
  virtual ~ROUNDFSULTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check roundf.sul
 * SoftFloat-3b modified
 */
TEST_P(ROUNDFSULTest, CheckROUNDFSUL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatRoundSUL, ROUNDFSULTest,
        ::testing::ValuesIn(softfloat_roundf_sul.begin(),
                            softfloat_roundf_sul.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_roundf_sul_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_roundf_sul = parseTable(inFile, ",");
    inFile.close();
  }
