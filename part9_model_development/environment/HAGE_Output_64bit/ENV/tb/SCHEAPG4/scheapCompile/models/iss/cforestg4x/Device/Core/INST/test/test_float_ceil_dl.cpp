...
std::vector<std::vector<std::string>> softfloat_ceilf_dl;

char *test_data_dir;


class CEILFDLTest : public testing::Test,
...
/*!
 * @brief Check ceilf.dl
 * SoftFloat-3b modified
 */
TEST_P(CEILFDLTest, CheckCEILFDL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCeilfDL, CEILFDLTest,
        ::testing::ValuesIn(softfloat_ceilf_dl.begin(),
                            softfloat_ceilf_dl.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_ceilf_dl_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_ceilf_dl = parseTable(inFile, ",");
    inFile.close();
  }
