/**
 * @file test_float32_recipfs.cpp
 *
 * @date Created on: 2017
 * @author Can Vy
 * @brief UNIT TEST for float32_recipfs.
...
std::vector<std::vector<std::string>> softfloat_recipfs;

char *test_data_dir;


class RECIPFSTest : public testing::Test,
...
/*!
 * @brief Check f32_recipfs
* SoftFloat-3b
*/
TEST_P(RECIPFSTest, CheckF32Recip) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f32_recipfs(_)).Times(1).After(inits);
  ...
  float32_t ret = m_sf->f32_recipfs(v2);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatRecipfs, RECIPFSTest,
        ::testing::ValuesIn(softfloat_recipfs.begin(),
                            softfloat_recipfs.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_recipf_s.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_recipfs = parseTable(inFile, ",");
    inFile.close();
  }
