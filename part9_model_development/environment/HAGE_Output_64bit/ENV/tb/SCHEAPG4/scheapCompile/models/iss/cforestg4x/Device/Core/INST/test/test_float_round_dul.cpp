...
std::vector<std::vector<std::string>> softfloat_roundf_dul;

char *test_data_dir;


class ROUNDFDULTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  ROUNDFDULTest() {}
  virtual ~ROUNDFDULTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check roundf.dul
 * SoftFloat-3b modified
 */
TEST_P(ROUNDFDULTest, CheckROUNDFDUL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfDUL, ROUNDFDULTest,
        ::testing::ValuesIn(softfloat_roundf_dul.begin(),
                            softfloat_roundf_dul.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_roundf_dul_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_roundf_dul = parseTable(inFile, ",");
    inFile.close();
  }
