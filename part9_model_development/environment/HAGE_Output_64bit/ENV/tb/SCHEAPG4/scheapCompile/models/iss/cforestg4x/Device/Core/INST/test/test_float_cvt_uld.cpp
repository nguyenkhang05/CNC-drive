...
std::vector<std::vector<std::string>> softfloat_cvtf_uld;

char *test_data_dir;


class CVTFULDTest : public testing::Test,
...
/*!
 * @brief Check cvtf.uld
 * SoftFloat-3b modified
 */
TEST_P(CVTFULDTest, CheckCVTFULD) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), Cvt_ui64_f64(_)).Times(1).After(inits);
  ...
  FpuRegDouble ret;
  float64_t ret_f64 = m_sf->Cvt_ui64_f64(reg1);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfULD, CVTFULDTest,
        ::testing::ValuesIn(softfloat_cvtf_uld.begin(),
                            softfloat_cvtf_uld.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_uld_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cvtf_uld = parseTable(inFile, ",");
    inFile.close();
  }
