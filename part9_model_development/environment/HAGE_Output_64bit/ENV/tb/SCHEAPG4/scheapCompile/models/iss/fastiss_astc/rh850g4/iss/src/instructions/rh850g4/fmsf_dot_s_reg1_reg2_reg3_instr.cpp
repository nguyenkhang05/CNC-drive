...
//------------------------------------------------------------------------------
// fmsf.s reg1, reg2, reg3 implementation
//------------------------------------------------------------------------------

rh850g4::fmsf_dot_s_reg1_reg2_reg3_instr_t::fmsf_dot_s_reg1_reg2_reg3_instr_t
(
...
void rh850g4::fmsf_dot_s_reg1_reg2_reg3_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850g4::fmsf_dot_s_reg1_reg2_reg3_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  disass_string << "fmsf.s " << reg1_name << ", " << reg2_name << ", " << reg3_name; 
  return disass_string.str();
}

unsigned int rh850g4::fmsf_dot_s_reg1_reg2_reg3_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850g4::fmsf_dot_s_reg1_reg2_reg3_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg1 = get_reg1_u8_0_4(opcode);
  GR_index_t reg2 = get_reg2_u8_11_15(opcode);
  ...
  fastiss::i1_t* is_mul_result_invalid = gen_and(is_reg3_not_qnan, gen_or(gen_and(is_reg1_zero, is_reg2_infinity), gen_and(is_reg1_infinity, is_reg2_zero)));
  
  fastiss::i1_t* is_reg1_or_reg2_infinity = gen_or(gen_and(is_reg1_infinity, is_reg2_not_qnan), gen_and(is_reg2_infinity, is_reg1_not_qnan));
  ...
  fastiss::i1_t* is_sub_result_invalid = gen_or(gen_and(is_reg3_positive_infinity, is_mul_result_positive_infinity), gen_and(is_reg3_negative_infinity, is_mul_result_negative_infinity));

  // Branch to the invalid quiet Nan case if any operand is a signalling NaN
  // or if the mul or sub results are invalid
  gen_cond_branch(gen_or(is_any_signalling_nan, gen_or(is_mul_result_invalid, is_sub_result_invalid)), invalid_qnan_branch, perform_calc_branch);
  ...
  fastiss::f32_with_exception_flags_t sub_result = gen_f32_sub(mul_result.result, reg3_float80, gen_read_round_mode());
  ...
  // Check if the operand is a denormaled value and FPSR.FS=0
  gen_cond_branch(gen_and(sub_result.underflow, gen_icmp_eq(gen_read_FPSR_FS(), gen_i1_constant(0))), 
  ...
  sub_result.inexact = gen_inexact_enabled_overflow_underflow_update(sub_result.overflow, sub_result.underflow, sub_result.inexact);
  
  // Check if the inexact flag must be explicitly set
  sub_result.inexact = gen_inexact_denorm_update(sub_result.invalid, sub_result.divide_by_zero, sub_result.inexact, reg1_float32, reg2_float32, reg3_float32);

  // Update the result, FPSR.XP, and handle exceptions
  sub_result.result = gen_select(gen_is_quiet_nan(reg2_float32), 
				 reg2_float32,
				 gen_select(gen_is_quiet_nan(reg1_float32),
					    reg1_float32,
					    gen_select(gen_is_quiet_nan(reg3_float32),
						       reg3_float32,
						       sub_result.result)));
  fastiss::i8_t* sub_result_xp = gen_i8_compose(gen_false(), gen_false(), gen_false(), sub_result.invalid, sub_result.divide_by_zero, sub_result.overflow, sub_result.underflow, sub_result.inexact);
  gen_update_dest_and_fpsr(true, reg3, gen_i32_cast(sub_result.result), sub_result_xp, continue_block);
