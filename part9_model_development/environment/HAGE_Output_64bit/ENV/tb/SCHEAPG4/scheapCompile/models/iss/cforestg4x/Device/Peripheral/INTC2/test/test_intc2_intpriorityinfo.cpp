...
TEST_F(IntPriorityInfoTest, Check_Invalidate) {
    ...
    EXPECT_EQ(NO_REQ_CHANNEL, m_test_priority->GetChannel());
    EXPECT_EQ(LOW_PRIORITY, m_test_priority->GetPriority());
}
...
TEST_F(IntPriorityInfoTest, Check_UpdateChannel) {
    ...
    ASSERT_NO_THROW (m_test_priority->UpdateChannel(update_channel, update_priority, update_priority_by_bind));
    EXPECT_EQ(update_channel, m_test_priority->GetChannel());
    EXPECT_EQ(update_priority, m_test_priority->GetPriority());
}
