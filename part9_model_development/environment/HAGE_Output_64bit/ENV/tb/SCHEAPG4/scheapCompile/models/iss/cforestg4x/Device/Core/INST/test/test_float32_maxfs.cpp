/**
 * @file test_float32_maxfs.cpp
 *
 * @date Created on: 2016
 * @author Can Vy
 * @brief UNIT TEST for float32_maxfs.
...
std::vector<std::vector<std::string>> softfloat_maxfs;

char *test_data_dir;


class MAXFSTest : public testing::Test,
...
/*!
 * @brief Check f32_maxfs
* SoftFloat-3b
*/
TEST_P(MAXFSTest, CheckF32Max) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f32_maxfs(_,_)).Times(1).After(inits);
  ...
  float32_t ret = m_sf->f32_maxfs(v2, v1);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatMaxfs, MAXFSTest,
        ::testing::ValuesIn(softfloat_maxfs.begin(),
                            softfloat_maxfs.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_maxf_s.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_maxfs = parseTable(inFile, ",");
    inFile.close();
  }
