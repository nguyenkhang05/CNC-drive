...
rh850::sst_dot_h_reg2_disp8_ind_ep_instr_t::sst_dot_h_reg2_disp8_ind_ep_instr_t
(
...
void rh850::sst_dot_h_reg2_disp8_ind_ep_instr_t::decode(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(2);  
}

std::string rh850::sst_dot_h_reg2_disp8_ind_ep_instr_t::disassemble(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
...
unsigned int rh850::sst_dot_h_reg2_disp8_ind_ep_instr_t::length(rh850::opcode_t opcode, unsigned int trans_PC)
{
  return 2;
}

void rh850::sst_dot_h_reg2_disp8_ind_ep_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  if (is_mdp_enable()){
    ...
    fastiss::i32_t *mpu_func_result = call_helper(
      ...
      addr,
      gen_or(gen_shl(gen_i32_constant(30), gen_i32_constant(16)), gen_i32_constant(0x201)),
      NULL
    );
    gen_cond_branch(
