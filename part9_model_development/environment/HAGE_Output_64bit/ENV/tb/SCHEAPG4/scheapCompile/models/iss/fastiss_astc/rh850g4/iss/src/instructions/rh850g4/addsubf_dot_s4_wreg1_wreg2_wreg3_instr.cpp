...
//------------------------------------------------------------------------------
// addsubf.s4 wreg1, wreg2, wreg3 implementation
//------------------------------------------------------------------------------

rh850g4::addsubf_dot_s4_wreg1_wreg2_wreg3_instr_t::addsubf_dot_s4_wreg1_wreg2_wreg3_instr_t
(
...
void rh850g4::addsubf_dot_s4_wreg1_wreg2_wreg3_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850g4::addsubf_dot_s4_wreg1_wreg2_wreg3_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  WR_index_t wreg1 = get_wreg1_u8_0_4(opcode);
  std::string wreg1_name = register_name_WR(WR_index_t(wreg1*1));
  ...
  disass_string << "addsubf.s4 " << wreg1_name << ", " << wreg2_name << ", " << wreg3_name; 
  return disass_string.str();
}

unsigned int rh850g4::addsubf_dot_s4_wreg1_wreg2_wreg3_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850g4::addsubf_dot_s4_wreg1_wreg2_wreg3_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  WR_index_t wreg1 = get_wreg1_u8_0_4(opcode);
  WR_index_t wreg2 = get_wreg2_u8_11_15(opcode);
  ...
  fastiss::i32_t *w0_res = gen_i32_extract(wreg3lo_load, gen_i64_constant(0));
  fastiss::i32_t *w1_res = gen_i32_extract(wreg3lo_load, gen_i64_constant(1));
  fastiss::i32_t *w2_res = gen_i32_extract(wreg3up_load, gen_i64_constant(0));
  fastiss::i32_t *w3_res = gen_i32_extract(wreg3up_load, gen_i64_constant(1));
  fastiss::i8_t *w0_exp  = gen_i8_constant(0);
  fastiss::i8_t *w1_exp  = gen_i8_constant(0);
  fastiss::i8_t *w2_exp  = gen_i8_constant(0);
  fastiss::i8_t *w3_exp  = gen_i8_constant(0);
  ...
  // for w0
  gen_fp_simd_sub(wr2_w0_float32, wr1_w0_float32, &w0_res, &w0_exp, gen_i1_constant(0));

  // for w1
  gen_fp_simd_add(wr2_w1_float32, wr1_w1_float32, &w1_res, &w1_exp, gen_i1_constant(0));

  // for w2
  gen_fp_simd_sub(wr2_w2_float32, wr1_w2_float32, &w2_res, &w2_exp, gen_i1_constant(0));

  // for w3
  gen_fp_simd_add(wr2_w3_float32, wr1_w3_float32, &w3_res, &w3_exp, gen_i1_constant(0));

  fastiss::i64_t *lo_result = gen_i64_compose(w1_res, w0_res);
  fastiss::i64_t *up_result = gen_i64_compose(w3_res, w2_res);

  // Update the result, FXSR.XP, and handle exceptions
  gen_update_dest_and_fxsr(true, wreg3lo, lo_result, up_result, w0_exp, w1_exp, w2_exp, w3_exp, continue_block);
...
unsigned int rh850g4::addsubf_dot_s4_wreg1_wreg2_wreg3_instr_t::cycle_count(opcode_t opcode, unsigned int trans_PC)
{
  return 2;
}
