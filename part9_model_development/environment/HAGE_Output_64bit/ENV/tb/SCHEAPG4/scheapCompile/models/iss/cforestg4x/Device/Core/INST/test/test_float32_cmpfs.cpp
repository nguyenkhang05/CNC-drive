...
std::vector<std::vector<std::string>> softfloat_cmpfs;

char *test_data_dir;


class CMPFSTest : public testing::Test,
...
/*!
 * @brief Check f32_cmpfs
* SoftFloat-3b
*/
TEST_P(CMPFSTest, CheckF32Min) {
  std::vector<std::string> cell = GetParam();
  uint32_t fcond = s2ul_32(cell[0]);
  uint32_t reg1 = s2ul_32(cell[1]);
  uint32_t reg2 = s2ul_32(cell[2]);
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f32_cmpfs(_,_,_)).Times(1).After(inits);
  ...
  bool ret = m_sf->f32_cmpfs(v2, v1, fcond);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCmpfs, CMPFSTest,
        ::testing::ValuesIn(softfloat_cmpfs.begin(),
                            softfloat_cmpfs.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cmpf_s.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cmpfs = parseTable(inFile, ",");
    inFile.close();
  }
