...
rh850::sld_dot_bu_disp4_ind_ep_reg2_instr_t::sld_dot_bu_disp4_ind_ep_reg2_instr_t
(
...
void rh850::sld_dot_bu_disp4_ind_ep_reg2_instr_t::decode(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(2);  
}

std::string rh850::sld_dot_bu_disp4_ind_ep_reg2_instr_t::disassemble(rh850::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
...
unsigned int rh850::sld_dot_bu_disp4_ind_ep_reg2_instr_t::length(rh850::opcode_t opcode, unsigned int trans_PC)
{
  return 2;
}

void rh850::sld_dot_bu_disp4_ind_ep_reg2_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  if (is_mdp_enable()){
    ...
    fastiss::i32_t *mpu_func_result = call_helper(
      ...
      address,
      gen_or(gen_shl(gen_i32_constant(reg2), gen_i32_constant(16)), gen_i32_constant(0x100)),
      NULL
    );
    gen_cond_branch(
