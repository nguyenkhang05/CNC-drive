...
//------------------------------------------------------------------------------
// cmpf.s fcond, reg2, reg1, fcbit implementation
//------------------------------------------------------------------------------

rh850g4::cmpf_dot_s_fcond_reg2_reg1_fcbit_instr_t::cmpf_dot_s_fcond_reg2_reg1_fcbit_instr_t
(
...
void rh850g4::cmpf_dot_s_fcond_reg2_reg1_fcbit_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850g4::cmpf_dot_s_fcond_reg2_reg1_fcbit_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  disass_string << "cmpf.s " << fpu_condition_code_name[fcond] << ", " << reg2_name << ", " << reg1_name << ", 0x" << std::hex << (unsigned int)fcbit;
  return disass_string.str();
}

unsigned int rh850g4::cmpf_dot_s_fcond_reg2_reg1_fcbit_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850g4::cmpf_dot_s_fcond_reg2_reg1_fcbit_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg1 = get_reg1_u8_0_4(opcode);
  GR_index_t reg2 = get_reg2_u8_11_15(opcode);
  ...
  fastiss::i1_t *is_reg1_nan = gen_is_nan(reg1_float32);
  fastiss::i1_t *is_reg2_nan = gen_is_nan(reg2_float32);
  ...
  gen_update_dest_and_fpsr(true, GR_INDEX_R0_0, gen_i32_constant(0), res_exp, continue_block);

  // Handle no NaN case
  set_gen_point(no_nan_branch);
  
  // Determine the comparison result - use ordered comparison operations
  fastiss::i1_t *is_less_than = gen_fcmp_olt(reg2_float32, reg1_float32);
  fastiss::i1_t *is_equal = gen_fcmp_oeq(reg2_float32, reg1_float32);
