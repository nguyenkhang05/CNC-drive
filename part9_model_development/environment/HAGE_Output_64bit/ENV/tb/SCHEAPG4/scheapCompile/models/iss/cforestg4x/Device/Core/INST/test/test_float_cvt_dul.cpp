...
std::vector<std::vector<std::string>> softfloat_cvtf_dul;

char *test_data_dir;


class CVTFDULTest : public testing::Test,
...
/*!
 * @brief Check cvtf.dul
 * SoftFloat-3b modified
 */
TEST_P(CVTFDULTest, CheckCVTFDUL) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfDUL, CVTFDULTest,
        ::testing::ValuesIn(softfloat_cvtf_dul.begin(),
                            softfloat_cvtf_dul.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_dul_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cvtf_dul = parseTable(inFile, ",");
    inFile.close();
  }
