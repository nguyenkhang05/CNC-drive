/**
 * @file test_float32_divfs.cpp
 *
 * @date Created on: 2017
 * @author Can Vy
 * @brief UNIT TEST for float32_divfs.
...
std::vector<std::vector<std::string>> softfloat_divfs;

char *test_data_dir;


class DIVFSTest : public testing::Test,
...
/*!
 * @brief Check f32_divfs
* SoftFloat-3b
*/
TEST_P(DIVFSTest, CheckF32Min) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f32_divfs(_,_)).Times(1).After(inits);
  ...
  float32_t ret = m_sf->f32_divfs(v2, v1);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatDivfs, DIVFSTest,
        ::testing::ValuesIn(softfloat_divfs.begin(),
                            softfloat_divfs.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_divf_s.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_divfs = parseTable(inFile, ",");
    inFile.close();
  }
