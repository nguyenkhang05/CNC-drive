/**
 * @file test_float64_max.cpp
 *
 * @date Created on: 2017
 * @author Can Vy
 * @brief UNIT TEST for float64_max.
...
std::vector<std::vector<std::string>> softfloat_maxfd;

char *test_data_dir;


class MaxfdTest : public testing::Test,
...
/*!
 * @brief Check f64_maxfd
 * SoftFloat-3b
 */
TEST_P(MaxfdTest, CheckF64Max) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f64_max(_,_)).Times(1).After(inits);
  ...
  float64_t ret = m_sf->f64_max(v2, v1);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatMaxfd, MaxfdTest,
                        ::testing::ValuesIn(softfloat_maxfd.begin(), softfloat_maxfd.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths (
        ...
        ::testing::internal::FilePath("data/UT_maxf_d_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_maxfd = parseTable(inFile, ",");
    inFile.close();
  }
