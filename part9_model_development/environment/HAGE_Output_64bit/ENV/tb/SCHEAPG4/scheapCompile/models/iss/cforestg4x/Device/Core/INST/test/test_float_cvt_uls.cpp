...
std::vector<std::vector<std::string>> softfloat_cvtf_uls;

char *test_data_dir;


class CVTFULSTest : public testing::Test,
...
/*!
 * @brief Check cvtf.uls
 * SoftFloat-3b modified
 */
TEST_P(CVTFULSTest, CheckCVTFULS) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), Cvt_ui64_f32(_)).Times(1).After(inits);
  ...
  float32_t ret_f32 = m_sf->Cvt_ui64_f32(reg1);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfULS, CVTFULSTest,
        ::testing::ValuesIn(softfloat_cvtf_uls.begin(),
                            softfloat_cvtf_uls.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_uls_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cvtf_uls = parseTable(inFile, ",");
    inFile.close();
  }
