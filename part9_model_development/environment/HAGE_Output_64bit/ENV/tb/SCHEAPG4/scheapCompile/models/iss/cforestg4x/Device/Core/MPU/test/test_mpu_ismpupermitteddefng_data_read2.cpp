/**
 * @file test_mpu_ismpupermitteddefng_data_read2.cpp
 *
 ...
 */
TEST_P(MpuMethodCheck, CheckIsMpuPermittedDefNG_DATA_READ_3) {
 ...
 *        condition MPE=0 & no guest entry
 */
TEST_P(MpuMethodCheck, CheckIsMpuPermittedDefNG_DATA_READ_4) {
  bool gm = std::get<0>(GetParam());
  bool mpe = std::get<1>(GetParam());
  bool gmpe = std::get<2>(GetParam());
  bool um = std::get<3>(GetParam());
