...
std::vector<std::vector<std::string>> softfloat_negfd;

char *test_data_dir;


class NEGFDTest : public testing::Test,
...
/*!
 * @brief Check f64_negfd
 * SoftFloat-3a
 */
TEST_P(NEGFDTest, CheckF64Neg) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f64_negfd(_)).Times(1).After(inits);
  ...
  float64_t ret = m_sf->f64_negfd(v2);

  uint32_t fpu_exception_flag = m_sf->GetExceptionFlags();
  // with xe filter (See fpu_device.cpp: FpuOpeDevice::Neg
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatNegfd, NEGFDTest,
        ::testing::ValuesIn(softfloat_negfd.begin(),
                            softfloat_negfd.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_negf_d_mod64.csv"));
//        ::testing::internal::FilePath filepath = ::testing::internal::FilePath("/shsv/MPU/user/hungnguyen/work/UT_training/161208/Device/Core/INST/test/UT_negf_s.csv");
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_negfd = parseTable(inFile, ",");
    inFile.close();
  }
