...
std::vector<std::vector<std::string>> softfloat_subfs;
...
TEST_P(SUBFSTest, CheckF32Sub) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f32_subfs(_,_)).Times(1).After(inits);
  ...
  float32_t ret = m_sf->f32_subfs(v2, v1);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatSubfs, SUBFSTest,
        ::testing::ValuesIn(softfloat_subfs.begin(),
                            softfloat_subfs.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_subf_s.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_subfs = parseTable(inFile, ",");
    inFile.close();
  }
