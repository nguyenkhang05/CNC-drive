...
std::vector<std::vector<std::string>> softfloat_cvtf_uwd;

char *test_data_dir;


class CVTFUWDTest : public testing::Test,
...
/*!
 * @brief Check cvtf.uwd
 * SoftFloat-3b modified
 */
TEST_P(CVTFUWDTest, CheckCVTFUWD) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), Cvt_ui32_f64(_)).Times(1).After(inits);
  ...
  FpuRegDouble ret;
  float64_t ret_f64 = m_sf->Cvt_ui32_f64(reg1);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfUWD, CVTFUWDTest,
        ::testing::ValuesIn(softfloat_cvtf_uwd.begin(),
                            softfloat_cvtf_uwd.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_uwd_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cvtf_uwd = parseTable(inFile, ",");
    inFile.close();
  }
