...
std::vector<std::vector<std::string>> softfloat_floorf_suw;

char *test_data_dir;


class FLOORFSUWTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  FLOORFSUWTest() {}
  virtual ~FLOORFSUWTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check floorf.suw
 * SoftFloat-3b modified
 */
TEST_P(FLOORFSUWTest, CheckFLOORFSUW) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatFloorfSUW, FLOORFSUWTest,
        ::testing::ValuesIn(softfloat_floorf_suw.begin(),
                            softfloat_floorf_suw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_floorf_suw.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_floorf_suw = parseTable(inFile, ",");
    inFile.close();
  }
