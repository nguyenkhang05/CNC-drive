...
//------------------------------------------------------------------------------
// add imm5, reg2 implementation
//------------------------------------------------------------------------------

rh850g4::add_imm5_reg2_instr_t::add_imm5_reg2_instr_t
(
...
void rh850g4::add_imm5_reg2_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(2);  
}

std::string rh850g4::add_imm5_reg2_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  if (imm5 < 0)
  {
    disass_string << "add -0x" << std::hex << -imm5 << ", " << reg2_name;
  }
  else
  {
    disass_string << "add 0x" << std::hex << imm5 << ", " << reg2_name;
...
unsigned int rh850g4::add_imm5_reg2_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 2;
}

void rh850g4::add_imm5_reg2_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  fastiss::i32_with_carry_overflow_t add_result_struct = gen_add_carry_overflow(imm5, reg2_load);
