/**
 * @file test_mpu_ismpupermitteddefng_data_write.h
 *
...
#define SET_NOT_HIT(INDEX) EXPECT_CALL(*(m_mpla_vec[INDEX]), ReadBody()).WillRepeatedly(Return(addr + size))
#define SET_HIT_PERMIT(INDEX) \
        ...
        if (um) {EXPECT_CALL(*(m_mpat_vec[INDEX]), GetUW()).WillOnce(Return(true));} \
        else {EXPECT_CALL(*(m_mpat_vec[INDEX]), GetSW()).WillOnce(Return(true));}
#define SET_HIT_NOT_PERMIT(INDEX) \
        EXPECT_CALL(*(m_mpla_vec[INDEX]), ReadBody()).WillRepeatedly(Return(addr)); \
        EXPECT_CALL(*(m_mpua_vec[INDEX]), ReadBody()).WillRepeatedly(Return(addr + size - 1)); \
        if (um) {EXPECT_CALL(*(m_mpat_vec[INDEX]), GetUW()).WillOnce(Return(false));} \
        else {EXPECT_CALL(*(m_mpat_vec[INDEX]), GetSW()).WillOnce(Return(false));}
