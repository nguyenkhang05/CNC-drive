...
//------------------------------------------------------------------------------
// addf.d reg1, reg2, reg3 implementation
//------------------------------------------------------------------------------

rh850g4::addf_dot_d_reg1_reg2_reg3_instr_t::addf_dot_d_reg1_reg2_reg3_instr_t
(
...
void rh850g4::addf_dot_d_reg1_reg2_reg3_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850g4::addf_dot_d_reg1_reg2_reg3_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  disass_string << "addf.d " << reg1_name << ", " << reg2_name << ", " << reg3_name; 
  return disass_string.str();
}

unsigned int rh850g4::addf_dot_d_reg1_reg2_reg3_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850g4::addf_dot_d_reg1_reg2_reg3_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg1 = get_reg1_u8_1_4_shift1(opcode);
  GR_index_t reg2 = get_reg2_u8_12_15_shift1(opcode);
  ...
  fastiss::i1_t* is_both_infinity = gen_and(gen_is_infinity(reg1_pair_float64), gen_is_infinity(reg2_pair_float64));
  fastiss::i1_t* is_opposite_sign = gen_icmp_ne(gen_is_positive(reg1_pair_float64), gen_is_positive(reg2_pair_float64));
  ...
  fastiss::f80_t *reg1_float80 = gen_f80_ext(reg1_pair_float64_flushed);
  fastiss::f80_t *reg2_float80 = gen_f80_ext(reg2_pair_float64_flushed);
  
  // Determine the ADD result - always round to nearest
  fastiss::f64_with_exception_flags_t add_result = gen_f64_add(reg1_float80, reg2_float80, gen_read_round_mode());
  ...
  gen_cond_branch(gen_and(add_result.underflow, gen_icmp_eq(gen_read_FPSR_FS(), gen_i1_constant(0))), 
  ...
  (
    ...
    gen_f64_cast(gen_and(gen_i64_cast(add_result.result), gen_i64_constant(0x8000000000000000ULL))),
    add_result.result
  );

  // Check if the inexact flag must be explicitly cleared due to an enabled overflow/underflow
  add_result.inexact = gen_inexact_enabled_overflow_underflow_update(add_result.overflow, add_result.underflow, add_result.inexact);

  // Check if the inexact flag must be explicitly set
  add_result.inexact = gen_inexact_denorm_update(add_result.invalid, add_result.divide_by_zero, add_result.inexact, reg1_pair_float64, reg2_pair_float64);

  // Update the result, FPSR.XP, and handle exceptions
  add_result.result = gen_select(gen_is_quiet_nan(reg2_pair_float64),
				 reg2_pair_float64,
				 gen_select(gen_is_quiet_nan(reg1_pair_float64),
					    reg1_pair_float64,
					    add_result.result));
  fastiss::i8_t* add_result_xp = gen_i8_compose(gen_false(), gen_false(), gen_false(), add_result.invalid, add_result.divide_by_zero, add_result.overflow, add_result.underflow, add_result.inexact);
  gen_update_dest_and_fpsr(false, reg3, gen_i64_cast(add_result.result), add_result_xp, continue_block);
