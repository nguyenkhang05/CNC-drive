...
std::vector<std::vector<std::string>> softfloat_floorf_sl;

char *test_data_dir;


class FLOORFSLTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  FLOORFSLTest() {}
  virtual ~FLOORFSLTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check floorf.sl
 * SoftFloat-3b modified
 */
TEST_P(FLOORFSLTest, CheckFLOORFSL) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), Cvt_f32_i64(_)).Times(1).After(inits);
  ...
  uint64_t ret = static_cast<uint64_t> (m_sf->Cvt_f32_i64(v1));
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatFloorSL, FLOORFSLTest,
        ::testing::ValuesIn(softfloat_floorf_sl.begin(),
                            softfloat_floorf_sl.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_floorf_sl_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_floorf_sl = parseTable(inFile, ",");
    inFile.close();
  }
