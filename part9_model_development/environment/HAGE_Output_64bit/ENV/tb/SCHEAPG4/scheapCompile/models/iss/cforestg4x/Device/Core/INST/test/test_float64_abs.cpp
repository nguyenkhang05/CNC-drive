...
std::vector<std::vector<std::string>> softfloat_absfd;

char *test_data_dir;


class ABSFDTest : public testing::Test,
...
TEST_P(ABSFDTest, CheckF64Abs) {
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f64_absfd(_)).Times(1).After(inits);
  ...
  float64_t ret = m_sf->f64_absfd(v2);
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatAbsfd, ABSFDTest,
        ::testing::ValuesIn(softfloat_absfd.begin(),
                            softfloat_absfd.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_absf_d_mod64.csv"));
//        ::testing::internal::FilePath filepath = ::testing::internal::FilePath("/shsv/MPU/user/hungnguyen/work/UT_training/161208/Device/Core/INST/test/UT_absf_s.csv");
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_absfd = parseTable(inFile, ",");
    inFile.close();
  }
