...
#include "rh850/execution_plugin.h"
...
public:
  ...
  // Predicate handlers
  rh850::analysis_predicate_response post_decode_predicate_handler(void); 
...
rh850::analysis_predicate_response pre_translate_loop_predicate(FunctionCallPluginCallbackInfo *fcpcbi)
{
...
rh850::analysis_predicate_response post_decode_predicate(FunctionCallPluginCallbackInfo *fcpcbi)
{
  return fcpcbi->post_decode_predicate_handler();
}

rh850::analysis_predicate_response FunctionCallPluginCallbackInfo::post_decode_predicate_handler(void)
{
  // Create null callback structure 
  rh850::analysis_predicate_response ret_val; 
    ...
    func_query_callback_func_t query_func)
{
  if (analysis_context && core_context && call_func && return_func && query_func)
  {
    ...
    fcpi->m_attachments.push_back(core->attach("post_decode", (rh850::analysis_predicate)post_decode_predicate, fcpcbi)); 
    fcpi->m_attachments.push_back(core->attach("pre_translate_loop",  (rh850::analysis_predicate)pre_translate_loop_predicate, fcpcbi)); 
    fcpi->m_attachments.push_back(core->attach("interrupt_accepted", (rh850::analysis_callback)interrupt_accepted_callback, fcpcbi)); 
  }
...
extern "C" bool remove_func_call_cb(void *analysis_context)
{
  if (analysis_context)
  {
    ...
    // Detach attachments
    for (std::list<rh850::analysis_attachment_descriptor *>::iterator att_it = fcpi->m_attachments.begin(); att_it != fcpi->m_attachments.end(); ++att_it )
    {
...
{
  // Register callbacks 
  aii->m_start_symbol_callback = aii->m_core->register_analysis_callback((rh850::analysis_callback)start_symbol_callback);
  aii->m_known_symbol_callback = aii->m_core->register_analysis_callback((rh850::analysis_callback)known_symbol_callback);
  aii->m_unknown_symbol_callback = aii->m_core->register_analysis_callback((rh850::analysis_callback)unknown_symbol_callback); 
}
