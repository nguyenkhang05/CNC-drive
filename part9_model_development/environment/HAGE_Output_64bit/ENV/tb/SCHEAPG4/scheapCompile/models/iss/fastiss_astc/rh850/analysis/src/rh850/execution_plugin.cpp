...
rh850::analysis_predicate_response execution_predicate(ExecutionPluginCallbackInfo *epcbi)
{
  AnalysisInterfaceInfo *aii =(AnalysisInterfaceInfo *)epcbi->m_analysis_interface_info;
  rh850::core_base_t *core = epcbi->m_core;
    ...
    pc_query_callback_func_t pc_query_func)
{
  if (analysis_context && core_context && pc_exec_func && pc_query_func)
  {
    ...
    epi->m_attachments.push_back(core->attach("post_decode", (rh850::analysis_predicate)execution_predicate, epcbi));
  }
...
extern "C" bool remove_execution_cb(void *analysis_context)
{
  if (analysis_context)
  {
    ...
    // Detach attachments
    for (std::list<rh850::analysis_attachment_descriptor *>::iterator att_it = epi->m_attachments.begin(); att_it != epi->m_attachments.end(); ++att_it )
    {
...
{
  aii->m_execute_callback = aii->m_core->register_analysis_callback((rh850::analysis_callback) execution_callback);
}
