...
class CVTF_SHTest : public testing::Test,
...
/*!
 * @brief Check f32_to_f16
 * SoftFloat-3b
 */
TEST_P(CVTF_SHTest, Check_CVTF_SH) {
  ...
  uint16_t expect = s2ul_16(cell[6]);
  ...
  inits += EXPECT_CALL((*m_sf), SetInputFlushFlag(_)).Times(1);
  ...
  // call softfloat func
  op = EXPECT_CALL((*m_sf), f32_to_f16(_)).Times(1).After(inits);
...
INSTANTIATE_TEST_CASE_P(quebec_cvtf_sh, CVTF_SHTest,
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_sh.csv"));
