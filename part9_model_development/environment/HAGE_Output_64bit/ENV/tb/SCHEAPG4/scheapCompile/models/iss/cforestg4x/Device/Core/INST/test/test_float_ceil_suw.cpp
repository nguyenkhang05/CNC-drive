...
std::vector<std::vector<std::string>> softfloat_ceilf_suw;

char *test_data_dir;


class CEILFSUWTest : public testing::Test,
public testing::WithParamInterface<std::vector<std::string>> {
protected:
  CEILFSUWTest() {}
  virtual ~CEILFSUWTest() {}
  virtual void SetUp() {
...
/*!
 * @brief Check ceilf.suw
 * SoftFloat-3b modified
 */
TEST_P(CEILFSUWTest, CheckCEILFSUW) {
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCeilfSUW, CEILFSUWTest,
        ::testing::ValuesIn(softfloat_ceilf_suw.begin(),
                            softfloat_ceilf_suw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_ceilf_suw.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_ceilf_suw = parseTable(inFile, ",");
    inFile.close();
  }
