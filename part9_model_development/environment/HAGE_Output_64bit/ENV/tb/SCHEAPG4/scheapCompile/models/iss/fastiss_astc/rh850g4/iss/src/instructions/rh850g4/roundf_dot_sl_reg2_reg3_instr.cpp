...
//------------------------------------------------------------------------------
// roundf.sl reg2, reg3 implementation
//------------------------------------------------------------------------------

rh850g4::roundf_dot_sl_reg2_reg3_instr_t::roundf_dot_sl_reg2_reg3_instr_t
(
...
void rh850g4::roundf_dot_sl_reg2_reg3_instr_t::decode(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Increment PC
  increment_trans_PC(4);  
}

std::string rh850g4::roundf_dot_sl_reg2_reg3_instr_t::disassemble(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  ...
  disass_string << "roundf.sl " << reg2_name << ", " << reg3_name; 
  return disass_string.str();
}

unsigned int rh850g4::roundf_dot_sl_reg2_reg3_instr_t::length(rh850g4::opcode_t opcode, unsigned int trans_PC)
{
  return 4;
}

void rh850g4::roundf_dot_sl_reg2_reg3_instr_t::execute(opcode_t opcode, unsigned int trans_PC)
{
  // Extract operands
  GR_index_t reg2 = get_reg2_u8_11_15(opcode);
  GR_index_t reg3 = get_reg3_u8_28_31_shift1(opcode);
  ...
  gen_cond_branch(gen_is_nan(reg2_float32), invalid_nan_branch, check_saturation_branch);
  
  // Check if the conversion will saturate. Saturation occurs when:
  //    - For a positive floating point number, if it is greater than 2^63-1 (represented as 0x5f000000 in single precision floating point)
  //    - For a negative floating point number, if it is less than -2^63 (represented as 0xdf000000 in single precision floating point)
  set_gen_point(check_saturation_branch);
  fastiss::i1_t *is_positive = gen_is_positive(reg2_float32);
  fastiss::i1_t *is_saturated = gen_select(is_positive, gen_fcmp_oge(reg2_float32_flushed, gen_f32_cast(gen_i32_constant(0x5f000000))), gen_fcmp_ole(reg2_float32_flushed, gen_f32_cast(gen_i32_constant(0xdf000000))));
  ...
  gen_update_dest_and_fpsr(true, reg3, gen_i64_constant(0x8000000000000000ULL), gen_i8_constant(0x10), continue_block);
  ...
  gen_update_dest_and_fpsr(true, reg3, saturate_val, gen_i8_constant(0x10), continue_block);  

  // Handle general operand case
  set_gen_point(perform_calc_branch);   

  // Convert the operand to a 32-bit integer value
  fastiss::i64_with_exception_flags_t convert_result = gen_i64_signed_convert(reg2_float32_flushed, gen_i8_constant(0));
