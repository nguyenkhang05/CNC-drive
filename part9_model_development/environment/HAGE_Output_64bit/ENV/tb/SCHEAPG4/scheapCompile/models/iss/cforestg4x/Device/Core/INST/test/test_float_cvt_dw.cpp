...
std::vector<std::vector<std::string>> softfloat_cvtf_dw;

char *test_data_dir;


class CVTFDWTest : public testing::Test,
...
INSTANTIATE_TEST_CASE_P(TestNewSoftFloatCvtfDW, CVTFDWTest,
        ::testing::ValuesIn(softfloat_cvtf_dw.begin(),
                            softfloat_cvtf_dw.end()));
...
int main(int argc, char **argv) {
  ::test_data_dir = argv[argc - 1];
  {
    ::testing::internal::FilePath filepath = ::testing::internal::FilePath::ConcatPaths(
        ...
        ::testing::internal::FilePath("data/UT_cvtf_dw_mod64.csv"));
    std::ifstream inFile(filepath.string());
    if (!inFile) {
      throw std::runtime_error("Could not open test data for input");
    }
    softfloat_cvtf_dw = parseTable(inFile, ",");
    inFile.close();
  }
