// -----------------------------------------------------------------------------
// $Id: ecccnt_cfs_regif.cpp,v 1.1 2019/09/26 10:06:59 landang Exp $
//
...
//
// Input file : ./input/CFLASH/ecccnt_cfs_regif.txt
////////////////////////////////////////////////////////////////////////////////
// %MODULE ecccnt_cfs
//     #              name     offset_size
...
////////////////////////////////////////////////////////////////////////////////
/// @file ecccnt_cfs_regif.cpp
/// @brief Register IF class of model ECCCNT_CFS
/// $Id: ecccnt_cfs_regif.cpp,v 1.1 2019/09/26 10:06:59 landang Exp $
...
/// @return none
Cecccnt_cfs_regif::Cecccnt_cfs_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    CFSECCCTL    = new vpcl::re_register(0x0000, this, "CFSECCCTL"   , name.c_str());
    CFSECCTSTCTL = new vpcl::re_register(0x0010, this, "CFSECCTSTCTL", name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_CFSECCCTL     , CFSECCCTL->name()   , CFSECCCTL->addr()   );
    cw_instantiate_reg(CW_CFSECCTSTCTL  , CFSECCTSTCTL->name(), CFSECCTSTCTL->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(CFSECCCTL     , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(CFSECCTSTCTL  , mCurReg, 0,  4, "8|16|32", "8|16|32");

    mRegList = mCurReg;

    (*CFSECCCTL     ) (24, 24, "BLANKMASKENABLE", 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CFSECCCTL     ) ( 1,  1, "SECDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CFSECCCTL     ) ( 0,  0, "ECCDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CFSECCTSTCTL  ) ( 1,  1, "ECCTST"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["CFSECCCTL" ]["BLANKMASKENABLE"] = &Cecccnt_cfs_regif::cb_CFSECCCTL_BLANKMASKENABLE;
    mWrCbAPI["CFSECCCTL" ]["ECCDIS"    ] = &Cecccnt_cfs_regif::cb_CFSECCCTL_ECCDIS;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cecccnt_cfs_regif::~Cecccnt_cfs_regif()
{
...
Cecccnt_cfs_regif::uint Cecccnt_cfs_regif::bit_select(
...
bool Cecccnt_cfs_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cecccnt_cfs_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_cfs_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                   const unsigned char *p_data,   ///< [in] Writing data
                                   cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_cfs_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                   unsigned char *p_data,   ///< [out] Reading data
                                   cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_cfs_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                       const unsigned char *p_data,  ///< [in] Writing data
                                       cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_cfs_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                       unsigned char *p_data,   ///< [out] Reading data
                                       cuint size,              ///< [in]  Data size (byte)
                                       bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cecccnt_cfs_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                    unsigned char *p_data,          ///< [out] Reading data
                                    cuint size,                     ///< [in]  Data size (byte)
...
bool Cecccnt_cfs_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                    const unsigned char *p_data,      ///< [in] Writing data
                                    cuint size,                       ///< [in] Data size (byte)
                                    cuint reg_index,                  ///< [in] Register index
...
bool Cecccnt_cfs_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cecccnt_cfs_regif::CommandInit()
{
...
/// @return string
std::string Cecccnt_cfs_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cecccnt_cfs_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cecccnt_cfs_regif::InitLocalVal()
{
    // Initialize local variables
    CFSECCCTL_BLANKMASKENABLE = 0x0;
    CFSECCCTL_SECDIS = 0x0;
    CFSECCCTL_ECCDIS = 0x0;
    CFSECCTSTCTL_ECCTST = 0x0;
}

/// Update bit value to local value
/// @return None
void Cecccnt_cfs_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        CFSECCCTL_BLANKMASKENABLE        = (*CFSECCCTL   )["BLANKMASKENABLE"];
        CFSECCCTL_SECDIS                 = (*CFSECCCTL   )["SECDIS"      ];
        CFSECCCTL_ECCDIS                 = (*CFSECCCTL   )["ECCDIS"      ];
        return;
    }
    if (addr == 0x0010) {
        CFSECCTSTCTL_ECCTST              = (*CFSECCTSTCTL)["ECCTST"      ];
        return;
    }
}
...
/// @return None
void Cecccnt_cfs_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*CFSECCCTL   )["BLANKMASKENABLE"] = CFSECCCTL_BLANKMASKENABLE;
        (*CFSECCCTL   )["SECDIS"      ] = CFSECCCTL_SECDIS;
        (*CFSECCCTL   )["ECCDIS"      ] = CFSECCCTL_ECCDIS;
        return;
    }
    if (addr == 0x0010) {
        (*CFSECCTSTCTL)["ECCTST"      ] = CFSECCTSTCTL_ECCTST;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cecccnt_cfs_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cecccnt_cfs_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cecccnt_cfs_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cecccnt_cfs_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cecccnt_cfs_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cecccnt_cfs_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cecccnt_cfs_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cecccnt_cfs_regif::uint Cecccnt_cfs_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cecccnt_cfs_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cecccnt_cfs_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cecccnt_cfs_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cecccnt_cfs_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cecccnt_cfs_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cecccnt_cfs_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cecccnt_cfs_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
