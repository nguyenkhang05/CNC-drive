...
//  %CMD_ACTION DumpStatInfo : void @ Dumps statistical information of AIR
...
std::string handleCommand (const std::vector<std::string>& args)
{
  ...
  if (args[0] == "command") {
    ...
    ret = pAirCore->regif_handle_command(_args);
    if (ret != "") {
      ret += "\n";
    }
  }
  ...
  // send a command to all targets
    ...
    baseid_message = pAirCore->regif_handle_command(_args);
    if (baseid_message.find("is invalid", 0) == std::string::npos) {
      cmd_found = true;
      if (baseid_message != "") {
        ret += baseid_message;
...
#endif // EnableDumpResult
    ret += "    DumpStatInfo                              Dumps statistical information of AIR\n";
