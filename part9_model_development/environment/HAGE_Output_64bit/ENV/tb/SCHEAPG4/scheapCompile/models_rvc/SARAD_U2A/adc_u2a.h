...
#ifndef __ADC_U2A_H__
#define __ADC_U2A_H__
#include "systemc.h"
#include <iterator>
#include <cerrno>
#include <string>
...
/// ADC model class
class Cadc_u2a: public sc_module
           ,public Cadc_u2a_regif
{

#include "adc_u2a_cmdif.h"
friend class Csarad_u2a;

    /// TIMER model class
    class Ctimer: public Cgeneral_timer
    {
    friend class Cadc_u2a;
...
public:
    SC_HAS_PROCESS(Cadc_u2a);
    Ctimer *mTIMER [2];
    Cadc_u2a(sc_module_name name, unsigned int adc_instance);
    ~Cadc_u2a();
...
//         ,emTxSELNum   = 6
    ...
    bool mIsRetainedADsyn [emGroupNum];
    bool mIsRetainedSW [emGroupNum];
    bool mIsRetainedSWTHA [emGroupNum];
    bool mIsRetainedSWTHB [emGroupNum];
    bool mIsRetainedHWTHA [emGroupNum];
    bool mIsRetainedHWTHB [emGroupNum];
    bool mIsRetainedPWD;
    double mAVccMax;
    double mAVccMin;
    double mEVccMax;
    ...
    unsigned int mHWTriggerSelectNum;    //Trigger input for ADC2
    ...
    sc_event mADSYNTriggerEvent[emGroupNum];
    ...
    double ADCTimeCalculation (const bool is_start_SG,const unsigned int vc_number, const unsigned int vc_add);
    ...
    unsigned int CheckVCULLMTBR(const unsigned int index, const bool isUpper, const unsigned int lmtb, const unsigned int vcAddNumber);
    double SelfDiagData(unsigned int index, unsigned int tdlv);
    bool HighAccuracyCheck(unsigned int index);
    ...
    void TriggerADC2Method (const unsigned int index);
