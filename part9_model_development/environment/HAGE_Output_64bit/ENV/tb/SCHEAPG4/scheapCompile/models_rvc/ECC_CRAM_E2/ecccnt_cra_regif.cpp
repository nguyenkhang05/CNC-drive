// -----------------------------------------------------------------------------
// $Id: ecccnt_cra_regif.cpp,v 1.1 2019/09/26 10:07:06 landang Exp $
//
...
//
// Input file : ecccnt_cra_regif.txt
////////////////////////////////////////////////////////////////////////////////
// %MODULE ecccnt_cra
//     #              name     offset_size
...
////////////////////////////////////////////////////////////////////////////////
/// @file ecccnt_cra_regif.cpp
/// @brief Register IF class of model ECCCNT_CRA
/// $Id: ecccnt_cra_regif.cpp,v 1.1 2019/09/26 10:07:06 landang Exp $
/// $Date: 2019/09/26 10:07:06 $
/// $Revison$
/// $Author: landang $
////////////////////////////////////////////////////////////////////////////////
...
/// @return none
Cecccnt_cra_regif::Cecccnt_cra_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
    ...
    #endif
{
    ...
    mWrCbAPI["CRAECCCTL"]["ECCDIS" ] = &Cecccnt_cra_regif::cb_CRAECCCTL_ECCDIS;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cecccnt_cra_regif::~Cecccnt_cra_regif()
{
...
Cecccnt_cra_regif::uint Cecccnt_cra_regif::bit_select(
...
bool Cecccnt_cra_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cecccnt_cra_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_cra_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                   const unsigned char *p_data,   ///< [in] Writing data
                                   cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_cra_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                   unsigned char *p_data,   ///< [out] Reading data
                                   cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_cra_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                       const unsigned char *p_data,  ///< [in] Writing data
                                       cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_cra_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                       unsigned char *p_data,   ///< [out] Reading data
                                       cuint size,              ///< [in]  Data size (byte)
                                       bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cecccnt_cra_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                    unsigned char *p_data,          ///< [out] Reading data
                                    cuint size,                     ///< [in]  Data size (byte)
...
bool Cecccnt_cra_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                    const unsigned char *p_data,      ///< [in] Writing data
                                    cuint size,                       ///< [in] Data size (byte)
                                    cuint reg_index,                  ///< [in] Register index
...
bool Cecccnt_cra_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cecccnt_cra_regif::CommandInit()
{
...
/// @return string
std::string Cecccnt_cra_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cecccnt_cra_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cecccnt_cra_regif::InitLocalVal()
{
    // Initialize local variables
...
/// @return None
void Cecccnt_cra_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
...
/// @return None
void Cecccnt_cra_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
...
/// @return vector
std::vector<std::string> Cecccnt_cra_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cecccnt_cra_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cecccnt_cra_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cecccnt_cra_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cecccnt_cra_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cecccnt_cra_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cecccnt_cra_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cecccnt_cra_regif::uint Cecccnt_cra_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cecccnt_cra_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cecccnt_cra_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cecccnt_cra_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cecccnt_cra_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cecccnt_cra_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cecccnt_cra_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cecccnt_cra_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
