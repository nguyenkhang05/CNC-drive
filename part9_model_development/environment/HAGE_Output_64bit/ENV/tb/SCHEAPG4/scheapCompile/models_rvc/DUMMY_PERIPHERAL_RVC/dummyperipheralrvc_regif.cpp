    ...
    #endif
{
    ...
    for(uint i = 0; i < emNUM_PCLK_REG; i++) {
        str_tmp.str("");
        str_tmp<<"PCLK_REG"<< i;
        PCLK_REG [i] = new vpcl::re_register((0x0010) + 4*i, this, str_tmp.str(), name.c_str());
    }
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_JUDGE_REG  , JUDGE_REG->name(), JUDGE_REG->addr());
    cw_instantiate_reg(CW_RESET_REG  , RESET_REG->name(), RESET_REG->addr());
    for(uint i = 0; i < emNUM_PCLK_REG ; i++) cw_instantiate_reg(CW_PCLK_REG   [i], PCLK_REG [i]->name(), PCLK_REG [i]->addr());
    #endif
    ...
    for(uint i = 0; i < emNUM_PCLK_REG ; i++) mCurReg = mRegArray[index++] = new SRegList(PCLK_REG   [i], mCurReg, i,  4,      "32", "8|16|32");

    mRegList = mCurReg;

    (*JUDGE_REG  ) ( 0,  0, "JUDGE"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*RESET_REG  ) ( 0,  0, "PRESETn" , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
    for(uint i = 0; i < emNUM_PCLK_REG; i++) {
        (*PCLK_REG   [i]) (31,  0, "PCLK"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    ...
    mWrCbAPI["RESET_REG"]["PRESETn"] = &Cdummyperipheralrvc_regif::cb_RESET_REG_PRESETn;

    for(uint i = 0; i < emNUM_PCLK_REG; i++) {
        str_tmp.str("");
        str_tmp<<"PCLK_REG"<< i;
        mWrCbAPI[str_tmp.str()]["PCLK"] = &Cdummyperipheralrvc_regif::cb_PCLK_REG_PCLK;
    }
    InitLocalVal();
}
...
Cdummyperipheralrvc_regif::uint Cdummyperipheralrvc_regif::bit_select(
...
bool Cdummyperipheralrvc_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cdummyperipheralrvc_regif::InitLocalVal()
{
    // Initialize local variables
    JUDGE_REG_JUDGE = 0x0;
    RESET_REG_PRESETn = 0x1;
    for(uint i = 0; i < emNUM_PCLK_REG; i++) {
        PCLK_REG_PCLK[i] = 0x0;
    }
}
...
/// @return None
void Cdummyperipheralrvc_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        JUDGE_REG_JUDGE               = (*JUDGE_REG)["JUDGE"    ];
        return;
    }
    if (addr == 0x0004) {
        RESET_REG_PRESETn             = (*RESET_REG)["PRESETn"  ];
        return;
    }
    if ((0x0010 <= addr) && (addr <= (0x0010)+(4*1)) && ((addr-0x0010)%4 == 0)) {
        uint i = (addr - 0x0010)/4;
        PCLK_REG_PCLK                [i] = (*(PCLK_REG [i]))["PCLK"     ];
        return;
    }
}
...
/// @return None
void Cdummyperipheralrvc_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*JUDGE_REG)["JUDGE"    ] = JUDGE_REG_JUDGE;
        return;
    }
    if (addr == 0x0004) {
        (*RESET_REG)["PRESETn"  ] = RESET_REG_PRESETn;
        return;
    }
    if ((0x0010 <= addr) && (addr <= (0x0010)+(4*1)) && ((addr-0x0010)%4 == 0)) {
        uint i = (addr - 0x0010)/4;
        (*(PCLK_REG [i]))["PCLK"     ] = PCLK_REG_PCLK[i];
        return;
    }
}
...
std::string Cdummyperipheralrvc_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
void Cdummyperipheralrvc_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
