// -----------------------------------------------------------------------------
// $Id: pwbrga0_regif.cpp,v 1.4 2020/02/17 10:06:29 landang Exp $
//
...
// 
////////////////////////////////////////////////////////////////////////////////
/// @file pwbrga0_regif.cpp
/// @brief Register IF class of model PWBRGA0
/// $Id: pwbrga0_regif.cpp,v 1.4 2020/02/17 10:06:29 landang Exp $
/// $Date: 2020/02/17 10:06:29 $
/// $Revison$
/// $Author: landang $
////////////////////////////////////////////////////////////////////////////////
...
/// @return none
Cpwbrga0_regif::Cpwbrga0_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
...
{
    CommandInit();
    PWBA0TE   = new vpcl::re_register(0x0010, this, "PWBA0TE"  , name.c_str());
    PWBA0TS   = new vpcl::re_register(0x0014, this, "PWBA0TS"  , name.c_str());
    PWBA0TT   = new vpcl::re_register(0x0018, this, "PWBA0TT"  , name.c_str());
    PWBA0EMU  = new vpcl::re_register(0x001C, this, "PWBA0EMU" , name.c_str());
    std::ostringstream str_tmp;
    for(uint i = 0; i < emNUM_PWBA0BRS; i++) {
        str_tmp.str("");
        str_tmp<<"PWBA0BRS"<< i;
        PWBA0BRSn[i] = new vpcl::re_register((0x0000) + 4*i, this, str_tmp.str(), name.c_str());
    }
    ...
    index = 0;
    #ifdef USR_CWR_SYSTEMC
    for(uint i = 0; i < emNUM_PWBA0BRS ; i++) cw_instantiate_reg(CW_PWBA0BRSn  [i], PWBA0BRSn[i]->name(), PWBA0BRSn[i]->addr());
    cw_instantiate_reg(CW_PWBA0TE    , PWBA0TE->name()  , PWBA0TE->addr()  );
    cw_instantiate_reg(CW_PWBA0TS    , PWBA0TS->name()  , PWBA0TS->addr()  );
    cw_instantiate_reg(CW_PWBA0TT    , PWBA0TT->name()  , PWBA0TT->addr()  );
    cw_instantiate_reg(CW_PWBA0EMU   , PWBA0EMU->name() , PWBA0EMU->addr() );
    #endif
    for(uint i = 0; i < emNUM_PWBA0BRS ; i++) mCurReg = mRegArray[index++] = new SRegList(PWBA0BRSn  [i], mCurReg, i,  4,      "16", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PWBA0TE    , mCurReg, 0,  4,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PWBA0TS    , mCurReg, 0,  4,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PWBA0TT    , mCurReg, 0,  4,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PWBA0EMU   , mCurReg, 0,  4,       "8", "8|16|32");

    mRegList = mCurReg;

    (*PWBA0TE    ) ( 3,  0, "TE"      , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PWBA0TS    ) ( 3,  0, "TS"      , 0x0       , "W"     , vpcl::SPP_ENABLE);
    (*PWBA0TT    ) ( 3,  0, "TT"      , 0x0       , "W"     , vpcl::SPP_ENABLE);
    (*PWBA0EMU   ) ( 7,  7, "SVSDIS"  , 0x0       , "W"     , vpcl::SPP_ENABLE);
    for(uint i = 0; i < emNUM_PWBA0BRS; i++) {
        (*PWBA0BRSn  [i]) (10,  0, "BRS"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    ...
    mWrCbAPI["PWBA0TS"]["TS"     ] = &Cpwbrga0_regif::cb_PWBA0TS_TS;
    mWrCbAPI["PWBA0TT"]["TT"     ] = &Cpwbrga0_regif::cb_PWBA0TT_TT;
    mWrCbAPI["PWBA0EMU"]["SVSDIS" ] = &Cpwbrga0_regif::cb_PWBA0EMU_SVSDIS;

    for(uint i = 0; i < emNUM_PWBA0BRS; i++) {
        str_tmp.str("");
        str_tmp<<"PWBA0BRS"<< i;
        mWrCbAPI[str_tmp.str()]["BRS"] = &Cpwbrga0_regif::cb_PWBA0BRSn_BRS;
    }
    InitLocalVal();
}
...
Cpwbrga0_regif::uint Cpwbrga0_regif::bit_select(
...
bool Cpwbrga0_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cpwbrga0_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cpwbrga0_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                const unsigned char *p_data,   ///< [in] Writing data
                                cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cpwbrga0_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                unsigned char *p_data,   ///< [out] Reading data
                                cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cpwbrga0_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                    const unsigned char *p_data,  ///< [in] Writing data
                                    cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cpwbrga0_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                    unsigned char *p_data,   ///< [out] Reading data
                                    cuint size,              ///< [in]  Data size (byte)
                                    bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cpwbrga0_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                 unsigned char *p_data,          ///< [out] Reading data
                                 cuint size,                     ///< [in]  Data size (byte)
...
bool Cpwbrga0_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                 const unsigned char *p_data,      ///< [in] Writing data
                                 cuint size,                       ///< [in] Data size (byte)
                                 cuint reg_index,                  ///< [in] Register index
...
bool Cpwbrga0_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cpwbrga0_regif::CommandInit()
{
...
/// @return string
std::string Cpwbrga0_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cpwbrga0_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cpwbrga0_regif::InitLocalVal()
{
    // Initialize local variables
    for(uint i = 0; i < emNUM_PWBA0BRS; i++) {
        PWBA0BRSn_BRS[i] = 0x0;
...
/// @return None
void Cpwbrga0_regif::UpdateLocalVal(cuint addr)
{
    if ((addr <= (0x0000)+(4*3)) && ((addr-0x0000)%4 == 0)) {
        uint i = (addr - 0x0000)/4;
        PWBA0BRSn_BRS                [i] = (*(PWBA0BRSn[i]))["BRS"      ];
        return;
    }
    if (addr == 0x0010) {
        PWBA0TE_TE                    = (*PWBA0TE  )["TE"       ];
        return;
    }
    if (addr == 0x0014) {
        PWBA0TS_TS                    = (*PWBA0TS  )["TS"       ];
        return;
    }
    if (addr == 0x0018) {
        PWBA0TT_TT                    = (*PWBA0TT  )["TT"       ];
        return;
    }
    if (addr == 0x001C) {
        PWBA0EMU_SVSDIS               = (*PWBA0EMU )["SVSDIS"   ];
        return;
    }
}
...
/// @return None
void Cpwbrga0_regif::UpdateRegVal(cuint addr)
{
    if ((addr <= (0x0000)+(4*3)) && ((addr-0x0000)%4 == 0)) {
        uint i = (addr - 0x0000)/4;
        (*(PWBA0BRSn[i]))["BRS"      ] = PWBA0BRSn_BRS[i];
        return;
    }
    if (addr == 0x0010) {
        (*PWBA0TE  )["TE"       ] = PWBA0TE_TE;
        return;
    }
    if (addr == 0x0014) {
        (*PWBA0TS  )["TS"       ] = PWBA0TS_TS;
        return;
    }
    if (addr == 0x0018) {
        (*PWBA0TT  )["TT"       ] = PWBA0TT_TT;
        return;
    }
    if (addr == 0x001C) {
        (*PWBA0EMU )["SVSDIS"   ] = PWBA0EMU_SVSDIS;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cpwbrga0_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cpwbrga0_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cpwbrga0_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cpwbrga0_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cpwbrga0_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cpwbrga0_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cpwbrga0_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Cpwbrga0_regif::uint Cpwbrga0_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return none
void Cpwbrga0_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cpwbrga0_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cpwbrga0_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cpwbrga0_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Cpwbrga0_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cpwbrga0_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cpwbrga0_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
