// ---------------------------------------------------------------------
// $Id: ECCCNT_DMDAn_Func.cpp,v 1.8 2019/09/26 10:07:19 landang Exp $
//
...
//========================================================
/// Constructor
ECCCNT_DMDAn_Func::ECCCNT_DMDAn_Func (std::string name, unsigned int sub_id, ECC_DMADTS_RAM_E2_AgentController* DMADTS_RAMAgentController):
                Cecccnt_dmdan_regif(name, 32)
                ,BusSlaveFuncIf()
{//{{{
    ...
    mProtectedRegMap.insert(std::pair<unsigned int, std::string>(0x0, "DAECCCTL"));

    Cecccnt_dmdan_regif::set_instance_name(name);
}//}}}

/// Destructor
ECCCNT_DMDAn_Func::~ECCCNT_DMDAn_Func()
{//{{{
}//}}}

/// Reset Func and registers
void ECCCNT_DMDAn_Func::EnableReset(const std::string reset_name, const bool is_active)
{//{{{
    if (reset_name == "sys_resetz"){
        Cecccnt_dmdan_regif::EnableReset(is_active);
    }
}//}}}

/// Read API (target socket)
void ECCCNT_DMDAn_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    // Get extension info
...
void ECCCNT_DMDAn_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void ECCCNT_DMDAn_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cecccnt_dmdan_regif::reg_handle_command(cmd);
                if (msg_return != "") {
            ...
            msg_return = Cecccnt_dmdan_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
unsigned int ECCCNT_DMDAn_Func::GetDAECCCTLReg()
{//{{{ 
    return (unsigned int)(*DAECCCTL);
}//}}}
...
void ECCCNT_DMDAn_Func::cb_DAECCCTL_ECCDIS(RegCBstr str)
{//{{{ 
    mpDMADTS_RAMAgentController->WriteDisPort_DA(mSubId, (unsigned int)(*DAECCCTL));
}//}}}
