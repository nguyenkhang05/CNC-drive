// ---------------------------------------------------------------------
// $Id: ATU5_TIMERC_SUB.cpp 1196 2019-10-17 05:33:04Z chuonghoangle $
//
...
///*********************************
/// Function     : ATU5_TIMERC_SUB
/// Description  : Constructor of ATU5_TIMERC_SUB class
///*********************************
ATU5_TIMERC_SUB::ATU5_TIMERC_SUB (sc_module_name name,
            ATU5_TIMERC *ATU5_TIMERC_Ptr,
            unsigned int sub_id):
                sc_module(name),
                ATU5_TIMERC_SUB_AgentController(),
                PCLK("PCLK"),
                nc_clk("nc_clk"),
                ...
                INT_OVI("INT_OVI")
{//{{{
    assert(ATU5_TIMERC_Ptr != NULL);
    mTIMERC = ATU5_TIMERC_Ptr;
    ...
    mTIC_SUBFunc = new ATU5_TIMERC_SUB_Func((std::string)name, this, sub_id);
    sc_assert(mTIC_SUBFunc != NULL);
    INT_OVI.initialize(false);
    Initialize();
    // move here from Initialize() to fix bug Redmine #84445 (start)
    for (unsigned int i = 0; i < 2; i++) {   
    mCLKNOISE_nPeriod[i] = 0;
    }
    // move here from Initialize() to fix bug Redmine #84445 (end)
    ...
    TIOC0.initialize(false);
    TIOC1.initialize(false);
    TIOC2.initialize(false);
    TIOC3.initialize(false);

    ////////////////////////////////////////////
    mFreeRunCnt = new ATU5_Timer("mFreeRunCnt",mTIMERC->mATU5);
    ...
    for (unsigned int index = 0; index <= 3; index++) {
        ...
        mRunCntGRC[index] = new ATU5_Timer(name.str().c_str(),mTIMERC->mATU5);
    ...
    for (unsigned int index = 0; index <= 3; index++) {
        ...
        mRunCntOCRC[index] = new ATU5_Timer(name.str().c_str(),mTIMERC->mATU5);
    ...
    for (unsigned int index = 0; index <= 3; index++) {
        ...
        mNoiseCanceler[index] = new ATU5_NoiseCanceler(port_name.str().c_str(), mTIMERC->mATU5);
    ...
    // Define Method
    //For Noise Canceler
    for (unsigned int index = 0; index < ATU5_TIMERC_SUB_AgentController::emChannel; index++) {
        sc_core::sc_spawn_options opt;
        opt.spawn_method();
        opt.set_sensitivity(&mNoiseCanceler[index]->mResultOutEvent);
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::OutputNoiseMethod, this, index),
    ...
    for (unsigned int index = 0; index < ATU5_TIMERC_SUB_AgentController::emChannel; index++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::SetNCNTCMethod, this, index),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::CaptureOCRCMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::CaptureGRCMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::WriteTIOCOutputMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::RisingInterruptGRCMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::RisingInterruptOCRCMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::FallingInterruptGRCMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::FallingInterruptOCRCMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::WriteTOCEPortMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::HandleCmpGRCMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::HandleOvfGRCMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::HandleCmpOCRCMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::HandleOvfOCRCMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::HandleFlagAndInterruptOCRCMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::HandleFlagAndInterruptGRCMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::StartCntGRCMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::StartCntOCRCMethod, this, signal_id),
                          sc_core::sc_gen_unique_name("StartCntOCRCMethod"), &opt);
    }

    for (unsigned int signal_id = 0; signal_id < 4; signal_id++) { // Added for ver 0.080
        sc_core::sc_spawn_options opt;
        opt.spawn_method();
        opt.set_sensitivity(&mPriorityEvent[signal_id]);
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERC_SUB::HandlePriorityMethod, this, signal_id),
                          sc_core::sc_gen_unique_name("HandlePriorityMethod"), &opt);
        }
...
///*********************************
/// Function     : ~ATU5_TIMERC_SUB
/// Description  : Destructor of ATU5_TIMERC_SUB class
///*********************************
ATU5_TIMERC_SUB::~ATU5_TIMERC_SUB (void)
{//{{{
    delete mTIC_SUBFunc;
...
///*********************************
void ATU5_TIMERC_SUB::Initialize(void)
{//{{{
    ...
    mPre_CntClkPeriod = 0;
    mPCLKPeriod = 0;
    mUpdatedTCNTC = 0; // fix at ver 0.080
    for (unsigned int i = 0; i < 6; i++) {
        mCLKBUS_nPeriod[i] = 0;
    mTimeCLKStart[i] = 0; // added for ver 0.080
    mCurCLKCntPeriod[i] = 0; // added for ver 0.080
    }
    ...
    for (unsigned int i = 0; i < emChannel; i++) {
        ...
        mTIOC[i] = false;
        mCurTIC[i] = false;
    mCurTICevent[i] = false; // added for ver 0.080 
    ...
    mIsOCRCoccur[i] = false; // added for update version 0.080 181
    mIsTSOCRCoccur[i] = false; 
    mIsGRCoccur[i] = false;    
    mIsTSGRCoccur[i] = false;
    }
}//}}}
...
***********************************/
void ATU5_TIMERC_SUB::EnableReset(const bool is_active)
{//{{{
...
***********************************/
void ATU5_TIMERC_SUB::CancelEvent(void)
{//{{{
    for (unsigned channel = 0; channel < emChannel; channel++) {
        ...
        mPriorityEvent[channel].cancel(); // added ver 0.080 
    }
...
***********************************/
bool ATU5_TIMERC_SUB::IsModelResetActive(void)
{//{{{
    bool ret = false;
...
***********************************/
void ATU5_TIMERC_SUB::RunningCounter(const bool is_run)
{//{{{
...
***********************************/
double ATU5_TIMERC_SUB::CalculateClockEdge (const std::string clock_name, const bool is_pos, const bool add_period, const double time_point, const double time_change_point)
{//{{{
    double period_num = 0;
    ...
    if (clock_name == "TCNTCCLK") {
        //zero_clock = (mCLKBUSPeriod == 0) ? true : false;
        if(mCLKBUSPeriod == 0){
            zero_clock=true;
        }else{
            zero_clock=false;
       ...
       // zero_clock = (mTIMERC->mPCLKPeriod == 0) ? true : false;
        if(mTIMERC->mPCLKPeriod == 0){
            zero_clock=true;
        }else{
            zero_clock=false;
        }
    ...
    sc_dt::uint64 mod_period_num = (sc_dt::uint64) period_num; // modified for ver 0.080 
    if (!add_period) {
        if (is_pos) {
...
Description     : TSTRC set counter start
***********************************/
void ATU5_TIMERC_SUB::SetCounterPeriod(const double period)
{//{{{
    if (period != 0) {
...
**********************************/
unsigned int ATU5_TIMERC_SUB::GetCounterValue(void)
{//{{{
...
***********************************/
void ATU5_TIMERC_SUB::StartStopTimer (const bool is_start)
{//{{{
    mIsTSTRCTrue = is_start;
    bool temp = is_start;
    mCLKBUSPeriod = mCLKBUS_nPeriod[mTIC_SUBFunc->GetCLKBusSel()];
    mTimeCLKChangePoint = mTimeCLKStart[mTIC_SUBFunc->GetCLKBusSel()]; 
    SetCounterPeriod((double)mCLKBUSPeriod);//Set Counter clock
    if (temp && ((mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) == false)) { // Modified for bug #49836
        temp = false;
    }
    if (temp) { //Start Counter 
        if (mChannelMode[0] == emPWM) { //PWM mode
            re_printf("info","PWM mode.\n");
            mLimitValue = (unsigned int) mGRCValue[0];
            mFreeRunCnt->SetCompareMatchValue(mLimitValue);
        } else { //Other mode
            if (mIs_Upper_Limit){
               if (((unsigned int)(mTIC_SUBFunc->GetCUCRC()) < (unsigned int)mTIC_SUBFunc->GetTCNTC())) {  // added ver 0.080 When writing CUCRCx < TCNTCx it 'll be FFFF_FFFF
                  mLimitValue = 0xFFFFFFFF; 
               } else { 
       ...
       if (mCLKBUSPeriod != 0){
           mStartTimerEvent.notify(CalculateClockEdge("TCNTCCLK", true,false, GetCurTime(), mTimeCLKChangePoint),mTIMERC->mATU5->mTimeResolutionUnit); 
        }   
    } else { //Stop Counter
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::StartTimerMethod (void)
{//{{{
    ...
    if (mIsTimerRunning) {
        ...
        if ((mLimitValue == (1+(unsigned int)(mTIC_SUBFunc->GetTCNTC()))) || (((unsigned int)mTIC_SUBFunc->GetTCNTC() >= mLimitValue) && (mChannelMode[0] == emPWM))) { //Added for reset at PWM mode if set TCNTC > GRC0 
            ...
            for (unsigned int channel = 0; channel <= 3; channel++) { //Start the virtual counter
                if ((mChannelValue[channel] == 7) || mIsOCRCWaitStart[channel] ||
                    ...
                    if ((mChannelMode[channel] == emOneshot) && mIsWaitOCRCMatching[channel] && (mPre_OCRC[channel] == (1 + mTIC_SUBFunc->GetTCNTC()))) { // add 0819
                        ...
                        mRunCntOCRC[channel]->SetCounterPeriod((double)mCLKBUSPeriod);
                ...
                if (((mChannelMode[channel] == emComp) && (0 < mChannelValue[channel]) && (mChannelValue[channel] < 4)) || 
                    ...
                    if ((mChannelMode[channel] == emOneshot) && (mIsGRCWaitOvf[channel] || mIsGRCWaitStart[channel] || mIsWaitGRCMatching[channel]) && 
                        ...
                        mRunCntGRC[channel]->SetCounterPeriod((double)mCLKBUSPeriod); //close for tp03_06_05
...
**********************************/
void ATU5_TIMERC_SUB::RangeCmpOCRC(const double range_value, const unsigned int channel)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
        unsigned int mTCNTCTemp = GetCounterValue();
       // Check range compare function
       if (mChannelMode[channel] == emPWM || mChannelValue[channel] == 5 || mChannelValue[channel] == 6) { // added for ver 0.080
           re_printf("warning", "The Range Compare function is prohibited \n");
       } else if ((mOCRCValue[channel] + range_value) <= mLimitValue) {
       if ((mOCRCValue[channel] <= mTCNTCTemp) && (mTCNTCTemp <= (mOCRCValue[channel] + range_value))) {
              mHandleCmpOCRCEvent[channel].notify(SC_ZERO_TIME); // add for ver  0.080
          mCaptureOCRCEvent[channel].notify(mTIMERC->mPCLKPeriod,mTIMERC->mATU5->mTimeResolutionUnit);
       }
       } else {
       if (((mOCRCValue[channel] <= mTCNTCTemp) && (mTCNTCTemp <= mLimitValue)) ||
         ((0 <= mTCNTCTemp) && (mTCNTCTemp <= (mOCRCValue[channel] + range_value - mLimitValue)))) {
             mCaptureOCRCEvent[channel].notify(mTIMERC->mPCLKPeriod,mTIMERC->mATU5->mTimeResolutionUnit);
             mHandleCmpOCRCEvent[channel].notify(SC_ZERO_TIME); //add for ver 0.080
           }
       }
...
**********************************/
void ATU5_TIMERC_SUB::RangeCmpGRC(const double range_value, const unsigned int channel)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
       unsigned int mTCNTCTemp = GetCounterValue();
    // Check range compare function
       if ((mChannelMode[channel] == emPWM) || (mChannelMode[channel] == emCapture)) { // added for ver 0.080
              re_printf("warning", "The Range Compare Function is prohibited \n");
       } else if ((mGRCValue[channel] + range_value) <= mLimitValue) {
          if ((mGRCValue[channel] <= mTCNTCTemp) && (mTCNTCTemp <= (mGRCValue[channel] + range_value))) {
              mCaptureGRCEvent[channel].notify(mTIMERC->mPCLKPeriod,mTIMERC->mATU5->mTimeResolutionUnit);
              mHandleCmpGRCEvent[channel].notify(SC_ZERO_TIME);
      }
       } else {
       if (((mGRCValue[channel] <= mTCNTCTemp) && (mTCNTCTemp <= mLimitValue)) ||
              ((0 <= mTCNTCTemp) && (mTCNTCTemp <= (mGRCValue[channel] + range_value - mLimitValue)))) {
              mCaptureGRCEvent[channel].notify(mTIMERC->mPCLKPeriod,mTIMERC->mATU5->mTimeResolutionUnit);
              mHandleCmpGRCEvent[channel].notify(SC_ZERO_TIME);
           }
        }
...
**********************************/
void ATU5_TIMERC_SUB::RestartTimerMethod(void)
{//{{{
...
**********************************/
void ATU5_TIMERC_SUB::StartCntOCRCMethod(const unsigned int channel)
{//{{{
    mIsTimerRunning = mIsTSTRCTrue;
    mCLKBUSPeriod = mCLKBUS_nPeriod[mTIC_SUBFunc->GetCLKBusSel()];
    mTimeCLKChangePoint = mTimeCLKStart[mTIC_SUBFunc->GetCLKBusSel()];
    ...
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB") && (mTIMERC->mPCLKPeriod != 0)) {
        if (mChannelMode[channel] == emOneshot) {
...
// NOte         : This method is called to load the value of FreeRunCnt when Cnt is running
**********************************/
void ATU5_TIMERC_SUB::StartCntGRCMethod(const unsigned int channel)
{//{{{
    mIsTimerRunning = mIsTSTRCTrue;
    mIsWaitGRCMatching[channel] = true; //add 0818
    mCLKBUSPeriod = mCLKBUS_nPeriod[mTIC_SUBFunc->GetCLKBusSel()];
    mTimeCLKChangePoint = mTimeCLKStart[mTIC_SUBFunc->GetCLKBusSel()];
    SetCounterPeriod((double)mCLKBUSPeriod);//Set Counter clock
    mRunCntGRC[channel]->SetCounterPeriod((double)mCLKBUSPeriod);
    mRunCntGRC[channel]->SetCounterInitialValue(1+(unsigned int) mTIC_SUBFunc->GetTCNTC());
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB") && (mTIMERC->mPCLKPeriod != 0)) {
        if (mChannelMode[channel] == emOneshot) {
...
**********************************/
void ATU5_TIMERC_SUB::SetCounterValue(const unsigned int count)
{//{{{
...
**********************************/
void ATU5_TIMERC_SUB::UpdatingCntClock(void)
{//{{{
...
**********************************/
void ATU5_TIMERC_SUB::UpdateInRunning(const std::string name)
{//{{{
    if (name == "TCNTC") {
        mTIC_SUBFunc->SetCounterStatus(false);
        ...
        mStartTimerEvent.notify(CalculateClockEdge("TCNTCCLK", true,true, GetCurTime(), mTimeCLKChangePoint),mTIMERC->mATU5->mTimeResolutionUnit); // add 0818
    } else if (name == "clock") {
            mCLKBUSPeriod = mCLKBUS_nPeriod[mTIC_SUBFunc->GetCLKBusSel()];
            SetCounterPeriod((double)mCLKBUSPeriod);
            mTimeCLKChangePoint = mTIMERC->mATU5->mClkBusStartTime[mTIC_SUBFunc->GetCLKBusSel()];
            //Restart counter with updated setting
        if (mCLKBUSPeriod != 0) {
            re_printf("info", "Counter is counted with updated clock.\n");
            mStartTimerEvent.notify(CalculateClockEdge("TCNTCCLK", true,false, GetCurTime(), mTimeCLKChangePoint), mTIMERC->mATU5->mTimeResolutionUnit);//edited 0813 true -> false
        }
    }
}//}}}
...
**********************************/
void ATU5_TIMERC_SUB::SetCmpOCRCValue(const unsigned int channel, const unsigned int value)
{//{{{
    if (mIsTSTRCTrue) {
        if (((0 <= mChannelValue[channel]) && (mChannelValue[channel] <= 4)) || (mChannelValue[channel] == 7) ||
            ...
            ((mChannelValue[channel] >= 12) && (mChannelValue[channel] <= 15))) { // Modified for ver 0.080
            if ((value == (1+(unsigned int)mFreeRunCnt->GetCounterValue())) && mIsTSTRCTrue && (mChannelMode[channel] != emOneshot)) { //add if 0809
                mHandleCmpOCRCEvent[channel].notify(CalculateClockEdge("TCNTCCLK", true,true, GetCurTime(), mTimeCLKChangePoint),mTIMERC->mATU5->mTimeResolutionUnit);
            } else {
...
**********************************/
void ATU5_TIMERC_SUB::SetCmpGRCValue(const unsigned int channel, const unsigned int value)
{//{{{
    if (mIsTSTRCTrue) {
        if (((0 < mChannelValue[channel]) && (mChannelValue[channel] <= 3)) ||
            ...
            if ((value == (1+(unsigned int)mTIC_SUBFunc->GetTCNTC())) && mIsTSTRCTrue && (mChannelMode[channel] != emOneshot) && (mChannelMode[channel] != emPWM)) { //add if 0809
                mHandleCmpGRCEvent[channel].notify(CalculateClockEdge("TCNTCCLK", true,true, GetCurTime(), mTimeCLKChangePoint),mTIMERC->mATU5->mTimeResolutionUnit);
            } else {
                mRunCntGRC[channel]->SetCompareMatchValue(mGRCValue[channel]);
            }
        }
...
**********************************/
void ATU5_TIMERC_SUB::HandleCntClkInputMethod(void)
{//{{{
    if (!IsModelResetActive()) {
        ...
        for (unsigned int i = 0; i < 6; i++) {
            if (mCurCLKCntPeriod[i] != mCLKBUS_nPeriod[i]) {
                ...
                mCurCLKCntPeriod[i] = mCLKBUS_nPeriod[i]; // fixed bug 55267
            }
        }
        if (mIsTSTRCTrue && (mCLKBUSPeriod != mCLKBUS_nPeriod[mTIC_SUBFunc->GetCLKBusSel()])) {
            ...
            if (mPre_CntClkPeriod == 0) {
                ...
                mRestartTimerEvent.notify((double)mCLKBUSPeriod,mTIMERC->mATU5->mTimeResolutionUnit);
            }
...
// Description  : Clear counter value as clock for CUCRC with PCLK, CLRCSELCx = 0, upperlimit value = 1
**********************************/
void ATU5_TIMERC_SUB::ClearTimeMethod(void)
{//{{{
    if ((mCLKBUSPeriod != 0) && mIsTSTRCTrue) {
        if (CalculateClockEdge("TCNTCCLK",true,false, GetCurTime(), mTimeCLKChangePoint) == 0) { // Added for version 0.080 when reset and count same time counter equal 1.
        ...
        mRestartCUCRCEvent.notify(CalculateClockEdge("TCNTCCLK",true,true, GetCurTime(), mTimeCLKChangePoint),mTIMERC->mATU5->mTimeResolutionUnit); //edit 0811 for tp03_01_005 
    }
}//}}}
...
**********************************/
void ATU5_TIMERC_SUB::RestartCUCRCMethod(void)
{//{{{
...
**********************************/
void ATU5_TIMERC_SUB::HandleCmpOvfMethod(void)
{//{{{
    
    if (mTIMERC->mATU5->CheckClockPeriod("CLK_LSB") && !IsModelResetActive() && (mCLKBUSPeriod != 0)) { //Check Clock & reset
        mIsTimerRunning = false;
        ...
        mTIC_SUBFunc->SetTCNTC(mGRCValue[0]);
            mTIC_SUBFunc->SetCounterStatus(false);
            mHandlePWMEvent.notify(CalculateClockEdge("PCLK", true, true, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]), mTIMERC->mATU5->mTimeResolutionUnit);
        } else {
            mIsTimerRunning = false; //edit 0814 true -> false       
        if (mIs_Upper_Limit){        
                mTIC_SUBFunc->SetTCNTC(GetCounterValue()); 
                if ((bool) mTIC_SUBFunc->TimingClearCounter() == false) { // Modified at ver 0.080 for check reset synschronize with PCLK. 
                     mClearTimeEvent.notify(CalculateClockEdge("PCLK", true, true, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]), mTIMERC->mATU5->mTimeResolutionUnit);
                     mInterruptWithPCLKEvent.notify(CalculateClockEdge("PCLK", true, true, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]), mTIMERC->mATU5->mTimeResolutionUnit); //Syns with PCLK when CLRCSELC == 0
                } else {    
                    if (mTIC_SUBFunc->IsInterruptOVF( )) {
                        mRisingInterruptOVFEvent.notify(CalculateClockEdge("TCNTCCLK", true, true, GetCurTime(), mTimeCLKChangePoint), mTIMERC->mATU5->mTimeResolutionUnit);//Added for output interrupt 
                    }
                        mIsOvf = true; // set for turn on flag  
                        mStartTimerEvent.notify(CalculateClockEdge("TCNTCCLK", true, true, GetCurTime(), mTimeCLKChangePoint), mTIMERC->mATU5->mTimeResolutionUnit);
                      }
            } else { 
                if (mTIC_SUBFunc->IsInterruptOVF()) {
                    mRisingInterruptOVFEvent.notify(CalculateClockEdge("TCNTCCLK", true, true, GetCurTime(), mTimeCLKChangePoint), mTIMERC->mATU5->mTimeResolutionUnit);// syn with CLKBUS;
                }
                mIsOvf = true; // set for turn on flag 
...
// Description  : Handle Compare Event
**********************************/
void ATU5_TIMERC_SUB::HandleCmpGRCMethod(const unsigned int channel)
{//{{{
    if ((mGRCMatchTime[channel] == sc_time_stamp().to_double()) && (mIsGRC1stMatch[channel])) {
        ...
        if (mChannelMode[channel] == emComp) {
            if (mRunCntGRC[channel]->GetBitOVF() && (mGRCValue[channel] == 0)) {
                ...
                mWriteTIOCOutputEvent[channel].notify((CalculateClockEdge("PCLK", true, false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]) + mTIMERC->mPCLKPeriod),
                                                        mTIMERC->mATU5->mTimeResolutionUnit);//syn with clock
                mHandleFlagAndInterruptGRCEvent[channel].notify((CalculateClockEdge("PCLK", true, false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]) + mTIMERC->mPCLKPeriod),
                                                                    mTIMERC->mATU5->mTimeResolutionUnit);//syn with clock
            ...
            switch (mChannelValue[channel]) {
                ...
                case 10:
                    mTIOC[channel] = 0; 
                    mHandleFlagAndInterruptGRCEvent[channel].notify(mTIMERC->mPCLKPeriod,mTIMERC->mATU5->mTimeResolutionUnit);
            mRisingInterruptGRCEvent[channel].notify(SC_ZERO_TIME);//current 
                    break;
                case 11:
                    mTIOC[channel] = 1; 
                    mHandleFlagAndInterruptGRCEvent[channel].notify(mTIMERC->mPCLKPeriod,mTIMERC->mATU5->mTimeResolutionUnit); 
            ...
            mRunCntGRC[channel]->StartStop(false);
            mWriteTIOCOutputEvent[channel].notify(mTIMERC->mPCLKPeriod,mTIMERC->mATU5->mTimeResolutionUnit);
            ...
            //The signal duty cycle is output
                ...
                mRunCntGRC[channel]->StartStop(false); //add 0810 for tp03_05_14
                    ...
                    mWriteTIOCOutputEvent[channel].notify(CalculateClockEdge("PCLK", true, true, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]), 
                                                            mTIMERC->mATU5->mTimeResolutionUnit);//syn with clock
                    if ((mChannelValue[channel] != 8) && (mChannelValue[channel] != 0)) {
                        mHandleFlagAndInterruptGRCEvent[channel].notify(CalculateClockEdge("PCLK", true, true, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]), 
                                                            mTIMERC->mATU5->mTimeResolutionUnit);//syn with clock
                    }
            }
        }
...
// Description  : Handle Compare Event
**********************************/
void ATU5_TIMERC_SUB::HandleCmpOCRCMethod(const unsigned int channel)
{//{{{
    if ((mOCRCMatchTime[channel] == sc_time_stamp().to_double()) && (mIsOCRC1stMatch[channel])) {
        ...
        if (((0 <= mChannelValue[channel]) && (mChannelValue[channel] <= 4)) || 
            ...
            if (mRunCntOCRC[channel]->GetBitOVF() && (mOCRCValue[channel] == 0)) {
                ...
                mHandleFlagAndInterruptOCRCEvent[channel].notify((CalculateClockEdge("PCLK", true, false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]) + mTIMERC->mPCLKPeriod),
                                                                    mTIMERC->mATU5->mTimeResolutionUnit);
            }
            ...
            switch (temp) {
                case 10:
                    mTIOC[channel] = 1;
                    if (mGRCValue[channel] != mOCRCValue[channel]) {
                        mWriteTIOCOutputEvent[channel].notify(mTIMERC->mPCLKPeriod, mTIMERC->mATU5->mTimeResolutionUnit);//syn with clock
                        mHandleFlagAndInterruptOCRCEvent[channel].notify(mTIMERC->mPCLKPeriod,mTIMERC->mATU5->mTimeResolutionUnit);
                        mRisingInterruptOCRCEvent[channel].notify(SC_ZERO_TIME); // added for ver 0.080
                    }
                    break;
                case 11:
                    mTIOC[channel] = 0;
            ...
            if (mOCRCValue[channel] > mGRCValue[channel]) {
                ...
                mHandleFlagAndInterruptGRCEvent[channel].notify(mTIMERC->mPCLKPeriod, mTIMERC->mATU5->mTimeResolutionUnit);//syn with clock
            }
        }
...
**********************************/
void ATU5_TIMERC_SUB::InterruptWithPCLKMethod(void)
{//{{{
    mTIC_SUBFunc->TurnOVFFlag(); // Modified for fix bug not check enable interrupt for occur flag ovf.
    if (mTIC_SUBFunc->IsInterruptOVF()) {
...
**********************************/
void ATU5_TIMERC_SUB::HandleFlagAndInterruptGRCMethod(const unsigned int channel)
{//{{{

    mIsGRCoccur[channel] = true; // added for  ver 0.080 181 
    mPriorityEvent[channel].notify(SC_ZERO_TIME); // added for 04/07 0.080 181    
    bool temp = false;
    if ((mChannelValue[channel] == 0) || (mChannelValue[channel] == 8) || (mChannelValue[channel] == 9)) {   // modified for ver 0.080
         temp = true;
    } 
    if (mTIC_SUBFunc->IsInterruptGRC(channel) && !temp && (mChannelMode[channel] != emOneshot)) {
        mRisingInterruptGRCEvent[channel].notify(SC_ZERO_TIME);
    }
}//}}}
...
**********************************/
void ATU5_TIMERC_SUB::HandleFlagAndInterruptOCRCMethod(const unsigned int channel)
{//{{{

    mIsOCRCoccur[channel] = true; // added for  ver 0.080 181 
    mPriorityEvent[channel].notify(SC_ZERO_TIME); // added for ver 0.080 181
    if ((mTIC_SUBFunc->IsInterruptOCRC(channel)) && (mChannelMode[channel] != emOneshot))  {  // modified for ver
...
***********************************/
double ATU5_TIMERC_SUB::GetCurTime(void)
{//{{{
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::WriteTIOCOutputMethod(const unsigned int channel)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) { //Check reset/clock
        switch (channel) {
            case 0:
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::RisingInterruptGRCMethod(const unsigned int channel)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) { //Check reset/clock
        INT_GRC[channel]->write(true);
        mFallingInterruptGRCEvent[channel].notify(mTIMERC->mPCLKPeriod, mTIMERC->mATU5->mTimeResolutionUnit);
    }
}//}}}
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::FallingInterruptGRCMethod(const unsigned int channel)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) { //Check reset/clock
        INT_GRC[channel]->write(false);
    }
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::RisingInterruptOCRCMethod(const unsigned int channel)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) { //Check reset/clock
        INT_OCRC[channel]->write(true);
        mFallingInterruptOCRCEvent[channel].notify(mTIMERC->mPCLKPeriod,mTIMERC->mATU5->mTimeResolutionUnit);
    }
}//}}}
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::FallingInterruptOCRCMethod(const unsigned int channel)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) { //Check reset/clock
        INT_OCRC[channel]->write(false);
    }
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::RisingInterruptOVFMethod(void)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")){
        INT_OVI.write(true);
        mFallingInterruptOVFEvent.notify(mTIMERC->mPCLKPeriod, mTIMERC->mATU5->mTimeResolutionUnit);
    }
}//}}}
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::FallingInterruptOVFMethod(void)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")){
        INT_OVI.write(false);
    }
...
**********************************/
void ATU5_TIMERC_SUB::HandleTIOC0Method(void)
{//{{{
    if (mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
        SetCLKNoisePeriod(0);
...
**********************************/
void ATU5_TIMERC_SUB::HandleTIOC1Method(void)
{//{{{
    if (mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
        SetCLKNoisePeriod(1);
...
**********************************/
void ATU5_TIMERC_SUB::HandleTIOC2Method(void)
{//{{{
    if (mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
        SetCLKNoisePeriod(2);
...
**********************************/
void ATU5_TIMERC_SUB::HandleTIOC3Method(void)
{//{{{
    if (mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
        SetCLKNoisePeriod(3);
...
// Description  : Handle TIOC0 input
**********************************/
void ATU5_TIMERC_SUB::HandleInputChannel0Method(void)
{//{{{
        ...
        if (mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
            if ((mChannelMode[0] == emCapture)) {
                ...
                if ((value == 4) || (value == 12)) {
                    ...
                    re_printf ("info","Disable Input Capture.\n");
                } else if ((value == 5) && (mCurTICevent[0] == 1)) {
                    mCurTICevent[0] = false;
                    if (mTIOC0 == 1) { // modified by for ver 0.080
                        mCaptureGRCEvent[0].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit); //after 2 cycle
                    } else {
                        mCaptureOCRCEvent[0].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit); //after 2 cycle
                    }
                } else if (value == 6 && (mCurTICevent[0] == 1)) { // modified for ver 0.080
                   mCurTICevent[0] = false; 
           if (mTIOC0 == 1) {
                        mCaptureOCRCEvent[0].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit); //after 2 cycle
                ...
                } else if (value == 7 && (mCurTICevent[0] == 1)) { // modified for ver 0.080
                   mCurTICevent[0] = false;  
                   mCaptureGRCEvent[0].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit); //after 2 cycle
                } else if (value == 13 && i_event_1_0.event()) { // Modified for ver 0.080
                    if (mIA1 == 1) {
                        mCaptureGRCEvent[0].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit);
                    }
                } else if (value == 14 && i_event_2A.event()) { // modified for ver 0.080
                    if (mIA2A == 1) {
                        mCaptureGRCEvent[0].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit);
                    }
                } else if (value == 15 && i_event_2B.event()) { // modified for ver 0.080
                    if (mIA2B == 1) {
                        mCaptureGRCEvent[0].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit);
                    }
...
// Description  : Handle TIOC1 input
**********************************/
void ATU5_TIMERC_SUB::HandleInputChannel1Method(void)
{//{{{
    if (!IsModelResetActive()) {//Check reset
        if (mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) { 
            if (mChannelMode[1] == emCapture) {
                ...
                } else if ((value == 5) && (mCurTICevent[1] == 1)) {
                    mCurTICevent[1] = false;
                    if (mTIOC1 == 1) {  // modified for ver0.080
                        mCaptureGRCEvent[1].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit); //after 2 cycle
                    } else {
                        mCaptureOCRCEvent[1].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit); //after 2 cycle
                    }
                } else if (value == 6 && (mCurTICevent[1] == 1)) { // modified for ver 0.080
            mCurTICevent[1] = false;
                    if (mTIOC1 ==1) {
                        mCaptureOCRCEvent[1].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit); //after 2 cycle
                ...
                } else if (value == 7 && (mCurTICevent[1] == 1)) { // modified for ver 0.080
                   mCurTICevent[1] = false;
                   mCaptureGRCEvent[1].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit);     //after 2 cycle
                } else if (value == 13 && i_event_1_0.event()) {
                    if (mIA1 == 1) {
                        mCaptureGRCEvent[1].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit);
                    }
                } else if (value == 14 && i_event_2A.event() ) {
                    if (mIA2A == 1) {
                        mCaptureGRCEvent[1].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit);
                    }
                } else if (value == 15 && i_event_2B.event()) {
                    if (mIA2B == 1) {
                        mCaptureGRCEvent[1].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod)),
                                                    mTIMERC->mATU5->mTimeResolutionUnit);
                    }
...
// Description  : Handle TIOC2 input
**********************************/
void ATU5_TIMERC_SUB::HandleInputChannel2Method(void)
{//{{{
    if (!IsModelResetActive()) {//Check reset
        if (mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) { 
            if (mChannelMode[2] == emCapture) {
                ...
                if ((value == 4) || (value == 12)) {
                    //disable input capture
                    re_printf ("info","Disable Input Catpure.\n");
                } else if (value == 5 && (mCurTICevent[2] == 1)) { // modified for ver 0.080
                   mCurTICevent[2] = false;
                    if (mTIOC2 ==1) {
                        mCaptureGRCEvent[2].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                        mTIMERC->mATU5->mTimeResolutionUnit); //after 2 cycle
                    } else {
                        mCaptureOCRCEvent[2].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                        mTIMERC->mATU5->mTimeResolutionUnit); //after 2 cycle
                    }
                } else if ((value == 6) && (mCurTICevent[2] == 1)) { // modified for ver 0.080
                   mCurTICevent[2] = false;
                    if (mTIOC2 ==1) {
                        mCaptureOCRCEvent[2].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                        mTIMERC->mATU5->mTimeResolutionUnit); //after 2 cycle
                ...
                } else if ((value == 7) && (mCurTICevent[2] == 1)) { // modified for ver 0.080
                   mCurTICevent[2] = false;
                    mCaptureGRCEvent[2].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                        mTIMERC->mATU5->mTimeResolutionUnit);     //after 2 cycle
                } else if (value == 13 && i_event_1_0.event()) {
                    if (mIA1 == 1) {
                        mCaptureGRCEvent[2].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod)),
                                                        mTIMERC->mATU5->mTimeResolutionUnit);
                    }
                } else if (value == 14 && i_event_2A.event()) {
                    if (mIA2A == 1) {
...
// Description  : Handle TIOC3 input
**********************************/
void ATU5_TIMERC_SUB::HandleInputChannel3Method(void)
{//{{{
    if (!IsModelResetActive()) {//Check reset
        if (mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) { 
            if (mChannelMode[3] == emCapture) {
                ...
                if ((value == 4) || (value == 12)) {
                    //disable input capture
                    re_printf ("info","Disable Input Catpure.\n");
                } else if ((value == 5) && (mCurTICevent[3] == 1)) {// modified for ver 0.080
                   mCurTICevent[3] = false;
                    if (mTIOC3 == 1) {
                        mCaptureGRCEvent[3].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                        mTIMERC->mATU5->mTimeResolutionUnit); //after 2 cycle
                    } else {
                        mCaptureOCRCEvent[3].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                        mTIMERC->mATU5->mTimeResolutionUnit); //after 2 cycle
                    }
                } else if ((value == 6) && (mCurTICevent[3] == 1)) {// modified for ver 0.080
                   mCurTICevent[3] = false;
                    if (mTIOC3 == 1) {
                        mCaptureOCRCEvent[3].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]) + (mTIMERC->mPCLKPeriod * 2))),
                                                        mTIMERC->mATU5->mTimeResolutionUnit); //after 2 cycle
                ...
                } else if ((value == 7) && (mCurTICevent[3] == 1)) { // modified for ver 0.080
                   mCurTICevent[3] = false;
                    mCaptureGRCEvent[3].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod * 2)),
                                                        mTIMERC->mATU5->mTimeResolutionUnit);     //after 2 cycle
                } else if (value == 13 && i_event_1_0.event()) { // modified for ver 0.080
                    if (mIA1 == 1) {
                        mCaptureGRCEvent[3].notify(((CalculateClockEdge("PCLK", true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0])) + (mTIMERC->mPCLKPeriod)),
                                                        mTIMERC->mATU5->mTimeResolutionUnit);
                    }
                } else if (value == 14 && i_event_2A.event()) { // modified for ver 0.080
                    if (mIA2A == 1) {
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::CaptureGRCMethod(const unsigned int channel)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
        ...
        if (mChannelMode[channel] == emCapture){ // added  for ver 0.080
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::CaptureOCRCMethod(const unsigned int channel)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
        unsigned int value_cap = GetCounterValue();
...
*********************************/
void ATU5_TIMERC_SUB::SetOCRCValue (const unsigned int channel, const unsigned int value)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
        ...
        if (!mIsTimerRunning) {
            ...
            if (mChannelMode[channel] != emOneshot) {
                ...
                if (mOCRCValue[channel] == (1+ (unsigned int)mRunCntOCRC[channel]->GetCounterValue())) {
                    mHandleCmpOCRCEvent[channel].notify(CalculateClockEdge("TCNTCCLK",true,true, GetCurTime(), mTimeCLKChangePoint),mTIMERC->mATU5->mTimeResolutionUnit); //add 0818
                } else {
                    mStartCntOCRCEvent[channel].notify(CalculateClockEdge("TCNTCCLK",true,true, GetCurTime(), mTimeCLKChangePoint),mTIMERC->mATU5->mTimeResolutionUnit); //edit 0818
                }
            }
        }
    }
...
*********************************/
void ATU5_TIMERC_SUB::SetGRCValue (const unsigned int channel, const unsigned int value)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
        ...
        if (!mIsTSTRCTrue) {
            ...
            if (!mIsTimerRunning) { // Timer not Run 
                SetCmpGRCValue(channel,value);
            } else {
                mTIC_SUBFunc->SetTCNTC(GetCounterValue());     
                if ((channel == 0) && (mChannelMode[channel] == emPWM)) {   // setting for GRCO 
                    mIsTimerRunning = false;
                    ...
                    mFreeRunCnt->SetCompareMatchValue(mLimitValue);
                    mRestartTimerEvent.notify(CalculateClockEdge("TCNTCCLK",true,false, GetCurTime(), mTimeCLKChangePoint),mTIMERC->mATU5->mTimeResolutionUnit);
                } else {  // Setting for GRC0->3
                    mRunCntGRC[channel]->StartStop(false); // GRC1-2-3
                    SetCmpGRCValue(channel,value);
                    //----Checking Initial Value and Cmp Value are equal--------------------------
                    if (mGRCValue[channel] == (1+ (unsigned int)mTIC_SUBFunc->GetTCNTC())) {
                        mHandleCmpGRCEvent[channel].notify(CalculateClockEdge("TCNTCCLK",true,true, GetCurTime(), mTimeCLKChangePoint),mTIMERC->mATU5->mTimeResolutionUnit);
                    } else {
                        mStartCntGRCEvent[channel].notify(CalculateClockEdge("TCNTCCLK",true,false, GetCurTime(), mTimeCLKChangePoint),mTIMERC->mATU5->mTimeResolutionUnit);
                    }
                }
            }
        }
...
*********************************/
void ATU5_TIMERC_SUB::OffCntUpperLimit(void)
{//{{{
    if (mIsTimerRunning && !IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
        ...
        mTIC_SUBFunc->SetTCNTC(GetCounterValue());
        mStartTimerEvent.notify(CalculateClockEdge("TCNTCCLK",true,true, GetCurTime(), mTimeCLKChangePoint),mTIMERC->mATU5->mTimeResolutionUnit); //true,true
    }
}//}}}
...
*********************************/
void ATU5_TIMERC_SUB::SetModeDuringRunning(void)
{//{{{
    for (unsigned int channel = 0; channel < emChannel; channel++) {
        mRunCntGRC[channel]->StartStop(false);
        mRunCntOCRC[channel]->StartStop(false);
...
// Description  : Set the counter upper limit function 
*********************************/
void ATU5_TIMERC_SUB::SetUpperLimit (const unsigned int value)
{//{{{
...
// Note         : Add 0806
*********************************/
void ATU5_TIMERC_SUB::StartOneShotMethod(const unsigned int channel, const unsigned int value)
{//{{{
    mGRCValue[channel] = value;
    if (!mIsOneShotRunning[channel]) { //Check the OneShot has not pulsed
        ...
        if (!mIsTimerRunning) { //Counter is still wait to started (TSTRC has been 1 already)
            ...
            mTIC_SUBFunc->SetTCNTC(GetCounterValue());
            if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB") && mIsTSTRCTrue) { //Check clock and reset
                if ((mChannelValue[channel] == 10) || (mChannelValue[channel] == 11)) {
                    SetCmpOCRCValue(channel,mOCRCValue[channel]);
                    if (mGRCValue[channel] >= mOCRCValue[channel]) {
                    ...
                    mStartCntOCRCEvent[channel].notify(CalculateClockEdge("TCNTCCLK",true,true, GetCurTime(), mTimeCLKChangePoint),mTIMERC->mATU5->mTimeResolutionUnit);//false ->true
                } else {
                    mIsWaitOCRCMatching[channel] = false; //add 0814
                    SetCmpGRCValue(channel,value);
                    mStartCntGRCEvent[channel].notify(CalculateClockEdge("TCNTCCLK",true,true, GetCurTime(), mTimeCLKChangePoint),mTIMERC->mATU5->mTimeResolutionUnit);//Add 0806//false -> true
                }
            }
        }
...
*********************************/
void ATU5_TIMERC_SUB::UpdateGRCOneShot(const unsigned int channel)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB") && mIsTSTRCTrue) {
...
*********************************/
void ATU5_TIMERC_SUB::HandleForcedCmpMatchMethod(const unsigned int channel)
{//{{{

    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) { //Check clock and reset
        if ((mChannelMode[channel] == emPWM)) {   // Modyfied  for ver0.08
            mIsForcedGRC[channel] = false;                      
        mIsForcedOCRC[channel] = false; ////Cancel
        re_printf("info", "Setting Prohibited. Force compare match cann't set.\n");
        } else {
            if (mIsForcedGRC[channel] && mIsForcedOCRC[channel] && mIsOneShotRunning[channel]) {  // Disable One shot Pulse
                ...
                switch (mChannelValue[channel]) {
                 case 10: mTIOC[channel] = 0;
                        mHandleFlagAndInterruptGRCEvent[channel].notify(CalculateClockEdge("PCLK", true, false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]), mTIMERC->mATU5->mTimeResolutionUnit);
                        mHandleFlagAndInterruptOCRCEvent[channel].notify(CalculateClockEdge("PCLK", true, false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]), mTIMERC->mATU5->mTimeResolutionUnit);
                        break;
               ...
               mWriteTIOCOutputEvent[channel].notify(CalculateClockEdge("PCLK",true,false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]), mTIMERC->mATU5->mTimeResolutionUnit);
            } else {
                if (mIsForcedGRC[channel]) {  //ForceGRC
                    mIsForcedGRC[channel] = false;
                    if ((mChannelMode[channel] != emCapture) && (mChannelValue[channel] != 0)) { // Modified for ver 0.080
                    ...
                    if (mIsTSTRCTrue) {
                        mRunCntGRC[channel]->StartStop(false);
            if ((0 != mChannelValue[channel]) && (mChannelMode[channel] != emCapture)) {
                mHandleFlagAndInterruptGRCEvent[channel].notify(CalculateClockEdge("PCLK", true, false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]), mTIMERC->mATU5->mTimeResolutionUnit);
                        }
                    } else { //add 0810 for case While 1shot is pulse, turn off TSTRC -> dump interrupt
            if ((mIsOneShotRunning[channel] && (mChannelMode[channel] == emOneshot)) || ((0 < mChannelValue[channel]) && (mChannelValue[channel] < 4))) { 
                           mHandleFlagAndInterruptGRCEvent[channel].notify(CalculateClockEdge("PCLK", true, false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]), mTIMERC->mATU5->mTimeResolutionUnit);
                        }
            }
                }    
                if (mIsForcedOCRC[channel]) {
                    mIsForcedOCRC[channel] = false;
                    mRunCntOCRC[channel]->StartStop(false); 
                    if (mChannelMode[channel] == emOneshot ) { // Modified for ver 0.080
                    ...
                    if ((mChannelValue[channel] != 5) || (mChannelValue[channel] != 6)) { // added for ver 0.080 
                         mHandleFlagAndInterruptOCRCEvent[channel].notify(CalculateClockEdge("PCLK", true, false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]), mTIMERC->mATU5->mTimeResolutionUnit);
                    } 
                }
            }
...
*********************************/
void ATU5_TIMERC_SUB::HandlePWMMethod(void)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB") && mIsTSTRCTrue) {  
        if ((CalculateClockEdge("TCNTCCLK",true,false, GetCurTime(), mTimeCLKChangePoint) == 0) &&
            (CalculateClockEdge("PCLK", true, false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]) == 0)) { // Added for version 0.080 when reset and count same time counter equal 1.
             mTIC_SUBFunc->SetTCNTC(1);
        } else { 
             mTIC_SUBFunc->SetTCNTC(0);
        }
...
// Description  : PWM Method
*********************************/
void ATU5_TIMERC_SUB::OffPWM(void)
{//{{{
    if (mIs_Upper_Limit){
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::SetCLKNoisePeriod (const unsigned int chan_num)
{//{{{
    ...
    if (noiseclk_sel == 0) {
        ...
        start_point = mTIMERC->mATU5->mClkBusStartTime[5];
    }
    bool is_clk_from_b = (noiseclk_sel == 1) && mTIMERC->mATU5->GetClkBus5Setting();   //modified is_clk_from_b = 1 when select clock bus5 and CB5SEL = 1
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::OutputNoiseMethod(const unsigned int chan_num)
{//{{{
    if (mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::SetNCNTCMethod (const unsigned int chan_num)
{//{{{
    if (mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) {
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::UpdateNoiseCounterValue (const unsigned int channel, const unsigned int value)
{//{{{
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::SetNoiseCancelMode (const unsigned int mode)
{//{{{
...
// Return value : None
**********************************/
void ATU5_TIMERC_SUB::SetNoiseCORVal (const unsigned int chan_num, const unsigned int value)
{//{{{
...
///******************************
unsigned int ATU5_TIMERC_SUB::GetNoiseCounterValue(const unsigned int channel)
{//{{{ 
    //Edited 0809
...
///******************************
bool ATU5_TIMERC_SUB::IsNoiseCancelDuration(void)
{//{{{
    bool temp = false;
    for (unsigned int channel = 0; channel < 4; channel++) {
...
///******************************
void ATU5_TIMERC_SUB::EnableNoiseCancel(const unsigned int channel, const bool is_enable)
{//{{{
...
///******************************
void ATU5_TIMERC_SUB::SetNoiseEachCancelMode (const unsigned int channel, const unsigned int mode)
{//{{{
    mNoiseCanceler[channel]->SetNoiseCancelMode(mode);
}//}}}

void ATU5_TIMERC_SUB::CheckCmpGRCOCRC(void)
{//{{{
    for (unsigned int channel = 0; channel <= 3; channel++) {
        ...
        if (((mChannelMode[channel] == emComp) && (0 < mChannelValue[channel]) && (mChannelValue[channel] <= 3))) {  // Modified for ver 0.080
...
//update TOCE port
void ATU5_TIMERC_SUB::ControlTOCE(const unsigned int channel, const bool value)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) { //Check reset/clock
        mTOCE[channel] = value;
        if (mCurTOCE[channel] != value) {
            mWriteTOCEPortEvent[channel].notify(CalculateClockEdge("PCLK", true, false, GetCurTime(), mTIMERC->mATU5->mTimeCLKChangePoint[0]), mTIMERC->mATU5->mTimeResolutionUnit);
        }
    }
}//}}}

//update TOCE port
void ATU5_TIMERC_SUB::WriteTOCEPortMethod(const unsigned int channel)
{//{{{
    if (!IsModelResetActive() && mTIMERC->mATU5->CheckClockPeriod("CLK_LSB")) { //Check reset/clock
        switch (channel) {
            case 0:
...
void ATU5_TIMERC_SUB::HandleOvfGRCMethod (const unsigned int channel)
{//{{{
...
void ATU5_TIMERC_SUB::HandleOvfOCRCMethod (const unsigned int channel)
{//{{{
...
void ATU5_TIMERC_SUB::HandlePriorityMethod(const unsigned int channel) 
{//{{{    // Added by for ver 0.080 181      
    if (mIsOCRCoccur[channel] && !mIsTSOCRCoccur[channel]) {
        mTIC_SUBFunc->TurnOCRCFlag(channel, 1);
    }    
    if (mIsGRCoccur[channel] && !mIsTSGRCoccur[channel]) {
        mTIC_SUBFunc->TurnGRCFlag(channel, 1);
    }
    mIsOCRCoccur[channel] = false; 
    mIsTSOCRCoccur[channel] = false; 
    mIsGRCoccur[channel] = false;
    mIsTSGRCoccur[channel] = false;
}//}}}

void ATU5_TIMERC_SUB::SetPriority(const unsigned int channel, const bool value) // added for ver 0.080
{//{{{
    bool  mtemp = value;
    if (mtemp == 0) { 
        mIsTSOCRCoccur[channel] = true;    
    } else { 
        mIsTSGRCoccur[channel] = true; 
    }
    mPriorityEvent[channel].notify(SC_ZERO_TIME);
}//}}}
