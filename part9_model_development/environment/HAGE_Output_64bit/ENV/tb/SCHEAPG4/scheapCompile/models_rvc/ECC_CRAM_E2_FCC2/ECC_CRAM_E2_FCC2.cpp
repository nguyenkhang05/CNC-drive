// ---------------------------------------------------------------------
// $Id: ECC_CRAM_E2_FCC2.cpp,v 1.17 2019/10/23 08:29:01 landang Exp $
//
...
#include "ECC_def.h"
#include "ECC_CRAM_E2_FCC2.h"
#include "ECCCNT_CRCn_FCC2_Func.h"
#include "ECCCNT_CRA_FCC2_Func.h"
#include "MECCCAP_CR_FCC2_Func.h"
#include "Common.h"
#include <iostream>
/// Constructor and Destructor

ECC_CRAM_E2_FCC2::ECC_CRAM_E2_FCC2(sc_module_name name, const uint8_t iPE_NUM, const unsigned int rLatency, const unsigned int wLatency)
        :sc_module(name)
        ,ECC_CRAM_E2_FCC2_AgentController()
        ,rvc_common_model()
        ,PE_NUM(iPE_NUM)
        // Clock/Reset
        ,pclk("pclk")
...
{//{{{ 
    ...
    mpECCCNT_CRCFunc = new ECCCNT_CRCn_FCC2_Func*[PE_MAX_NUM];
    for (unsigned int i = 0; i < PE_MAX_NUM; i++){
        mpECCCNT_CRCFunc[i] = new ECCCNT_CRCn_FCC2_Func((std::string)name, i, this);
    }
    mpECCCNT_CRAFunc = new ECCCNT_CRA_FCC2_Func((std::string)name, this);
    mpMECCCAP_CRFunc = new MECCCAP_CR_FCC2_Func((std::string)name, this);
    ...
    mAWIDTH_MSB = CRAM_DEFAULT_AWIDTH_MSB;
    mAWIDTH_LSB = CRAM_DEFAULT_AWIDTH_LSB;
    mCAP_DEPTH  = CRAM_DEFAULT_CAP_DEPTH;
    mERRINT_INIT= CRAM_DEFAULT_ERRINT_INIT;
    ...
    for (unsigned int reset_id = 0; reset_id < emNumReset; reset_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_CRAM_E2_FCC2::HandleResetHardMethod, this, reset_id),
    ...
    for (unsigned int reset_id = 0; reset_id < emNumReset; reset_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_CRAM_E2_FCC2::HandleResetCmdMethod, this, reset_id),
    ...
    for (unsigned int reset_id = 0; reset_id < emNumReset; reset_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_CRAM_E2_FCC2::CancelResetCmdMethod, this, reset_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_CRAM_E2_FCC2::WriteDisPort_CRCMethod, this, pe_id),
...
ECC_CRAM_E2_FCC2::~ECC_CRAM_E2_FCC2()
{//{{{
...
void ECC_CRAM_E2_FCC2::CreateEDLMap()
{//{{{
    std::string edl_name;
    for (unsigned int i = 0; i < PE_NUM; i++){
        // PE, pe_index
        ...
        // S2M2L pe_index
        edl_name = CombineStringUint("S2M2L", i);
        mEDLmap.insert(std::pair<std::string, unsigned int>(edl_name, 0xC));
        // S2M2U pe_index
        edl_name = CombineStringUint("S2M2U", i);
        mEDLmap.insert(std::pair<std::string, unsigned int>(edl_name, 0xD));
        // Cram0L, pe_index
        ...
        // Cram2L, pe_index
        edl_name = CombineStringUint("Cram2L", i);
        mEDLmap.insert(std::pair<std::string, unsigned int>(edl_name, 0x14));
        // Cram2U, pe_index
        edl_name = CombineStringUint("Cram2U", i);
        mEDLmap.insert(std::pair<std::string, unsigned int>(edl_name, 0x15));
    } 
}//}}}

/// Initialize when reset activated

void ECC_CRAM_E2_FCC2::Initialize(void)
{//{{{
...
void ECC_CRAM_E2_FCC2::WriteDisPort_CRC(unsigned int pe_id, unsigned int value)
{//{{{
...
void ECC_CRAM_E2_FCC2::WriteDisPort_CRA(unsigned int value)
{//{{{
...
unsigned int ECC_CRAM_E2_FCC2::GetCapDepth()
{//{{{
    return mCAP_DEPTH;
}//}}}
...
void ECC_CRAM_E2_FCC2::AssertERROVFOUT0_CR()
{//{{{
    mERROVFOUT0_CR = true;
    double current_time = sc_time_stamp().to_double();
...
void ECC_CRAM_E2_FCC2::AssertERROVFOUT1_CR()
{//{{{
    mERROVFOUT1_CR = true;
    double current_time = sc_time_stamp().to_double();
...
bool ECC_CRAM_E2_FCC2::GetKeyOpen()
{//{{{
    return mKeyOpen;
}//}}}
...
unsigned int ECC_CRAM_E2_FCC2::GetAWIDTH_MSB()
{//{{{
    return mAWIDTH_MSB;
}//}}}
...
void ECC_CRAM_E2_FCC2::MonitorpclkMethod()
{//{{{
...
void ECC_CRAM_E2_FCC2::Monitorsys_clkMethod()
{//{{{
...
void ECC_CRAM_E2_FCC2::MonitorpresetnMethod()
{//{{{
...
void ECC_CRAM_E2_FCC2::Monitorsys_resetzMethod()
{//{{{
...
void ECC_CRAM_E2_FCC2::HandleResetHardMethod (const unsigned int reset_id)
{//{{{
...
void ECC_CRAM_E2_FCC2::HandleResetCmdMethod (const unsigned int reset_id)
{//{{{
...
void ECC_CRAM_E2_FCC2::CancelResetCmdMethod (const unsigned int reset_id)
{//{{{
    std::string reset_name = "presetn";
    if (reset_id == empresetnId) {
        reset_name = "presetn";
...
void ECC_CRAM_E2_FCC2::EnableReset (const std::string reset_name, const bool is_active)
{//{{{
...
void ECC_CRAM_E2_FCC2::CancelEvents()
{//{{{
    for (unsigned int i = 0; i < PE_NUM; i++){
...
bool ECC_CRAM_E2_FCC2::GetResetStatus (const std::string reset_name)
{//{{{
...
bool ECC_CRAM_E2_FCC2::CheckClockPeriod (const std::string clock_name)
{//{{{
...
void ECC_CRAM_E2_FCC2::MonitorkeyopenMethod()
{//{{{
...
void ECC_CRAM_E2_FCC2::WriteDisPort_CRCMethod(unsigned int pe_id)
{//{{{ 
...
void ECC_CRAM_E2_FCC2::WriteDisPort_CRAMethod()
{//{{{
...
void ECC_CRAM_E2_FCC2::WriteERROVFOUT0_CRMethod()
{//{{{
...
void ECC_CRAM_E2_FCC2::WriteERROVFOUT1_CRMethod()
{//{{{
...
void ECC_CRAM_E2_FCC2::WriteSEDERROUT_CRMethod()
{//{{{
...
void ECC_CRAM_E2_FCC2::AssertSEDERROUT_CRMethod()
{//{{{
...
void ECC_CRAM_E2_FCC2::NegateSEDRROUT_CRMethod()
{//{{{
    mSEDERROUT_CR = false;
    mWriteSEDERROUT_CREvent.notify();
}//}}}
...
void ECC_CRAM_E2_FCC2::NegateERROVFOUT0_CRMethod()
{//{{{
    mERROVFOUT0_CR = false;
    mWriteERROVFOUT0_CREvent.notify();
}//}}}
...
void ECC_CRAM_E2_FCC2::NegateERROVFOUT1_CRMethod()
{//{{{
    mERROVFOUT1_CR = false;
    mWriteERROVFOUT1_CREvent.notify();
}//}}}
...
void ECC_CRAM_E2_FCC2::Set1bitErrorStatusThread()
{//{{{
    while (true){
...
void ECC_CRAM_E2_FCC2::Process1bitErrorMethod()
{//{{{
    // Update 1bit error FIFO
    for (unsigned int portindex = 0; portindex <= CRAM_HIGHEST_PORT_INDEX; portindex++){// With fix priority
        for (unsigned int i = 0; i < mSize1bitErrorList; i++){// Scan list to find high priority
            ...
            if (m1bitErrorList[i].edl_code == portindex){
                ...
                if (m1bitErrorFIFO.size() < (PE_NUM+12)) {//emTotalSlavePorts){// Fixed FIFO size = emTotalSlavePorts
...
void ECC_CRAM_E2_FCC2::ProcessFatalErrorMethod()
{//{{{
...
void ECC_CRAM_E2_FCC2::SetFatalErrorStatusMethod()
{//{{{
    if (mHas2bitError){
...
void ECC_CRAM_E2_FCC2::NotifyFatalErrorMethod()
{//{{{
...
void ECC_CRAM_E2_FCC2::NegateDEDERROUT_CRMethod()
{//{{{
    mDEDERROUT_CR = false;
    mWriteDEDERROUT_CREvent.notify();
}//}}}
...
void ECC_CRAM_E2_FCC2::NegateAFEERROUT_CRMethod()
{//{{{
    mAFEERROUT_CR = false;
    mWriteAFEERROUT_CREvent.notify();
}//}}}
...
void ECC_CRAM_E2_FCC2::WriteDEDERROUT_CRMethod()
{//{{{
...
void ECC_CRAM_E2_FCC2::WriteAFEERROUT_CRMethod()
{//{{{
...
bool ECC_CRAM_E2_FCC2::CheckNoFatalStatus()
{//{{{
...
bool ECC_CRAM_E2_FCC2::CheckMaskAddress( const unsigned int address)
{//{{{
...
bool ECC_CRAM_E2_FCC2::Check1bitAddressNotCaptured(unsigned int mod_address)
{//{{{ 
...
void ECC_CRAM_E2_FCC2::Update1bitErrorList()
{//{{{
...
void ECC_CRAM_E2_FCC2::UpdateFatalErrorList()
{//{{{
...
std::string ECC_CRAM_E2_FCC2::CombineStringUint(std::string name, unsigned int index)
{//{{{
...
unsigned int ECC_CRAM_E2_FCC2::ShiftAddress(unsigned int address)
{//{{{
...
void ECC_CRAM_E2_FCC2::SetMessageLevel (const std::string msg_lv)
{//{{{
...
void ECC_CRAM_E2_FCC2::DumpRegisterRW (const std::string is_enable)
{//{{{
    std::vector<std::string> cmd;
    cmd.push_back("reg");
...
void ECC_CRAM_E2_FCC2::DumpInterrupt (const std::string is_enable)
{//{{{
    ...
    if (word_vector.size() == 1) {
        if (word_vector[0] == "true") {
            ...
            re_printf("warning","Invalid argument: ECC_CRAM_E2_FCC2_DumpInterrupt %s\n", is_enable.c_str());
        }
        ...
        re_printf("info","ECC_CRAM_E2_FCC2_DumpInterrupt %s\n",temp.c_str());
    } else {
        re_printf("warning","Invalid argument: ECC_CRAM_E2_FCC2_DumpInterrupt %s\n", is_enable.c_str());
    }
}//}}}
...
void ECC_CRAM_E2_FCC2::AWIDTH_MSB(const unsigned int value)
{//{{{
    if ((mAWIDTH_LSB <= value) && (value <= MAX_ADDWIDTH_MSB)){
        mAWIDTH_MSB = value;
    }else{
        re_printf("warning", "Invalid AWIDTH_MSB (%d) of the ECC_CRAM_E2_FCC2_AWIDTH_MSB parameter. Valid range is %d-%d. Value %d is set as default.\n", value, mAWIDTH_LSB, MAX_ADDWIDTH_MSB, CRAM_DEFAULT_AWIDTH_MSB);
    }
}//}}}

/// Set AWIDTH_LSB

void ECC_CRAM_E2_FCC2::AWIDTH_LSB(const unsigned int value)
{//{{{
    if ((MIN_ADDWIDTH_LSB <= (int)value) && (value <= mAWIDTH_MSB)){
        mAWIDTH_LSB = value;
    }else{
        re_printf("warning", "Invalid AWIDTH_LSB (%d) of the ECC_CRAM_E2_FCC2_AWIDTH_LSB parameter. Valid range is %d-%d. Value %d is set as default.\n", value, MIN_ADDWIDTH_LSB, mAWIDTH_MSB, CRAM_DEFAULT_AWIDTH_LSB);
    }
}//}}}

/// Set CAP_DEPTH

void ECC_CRAM_E2_FCC2::CAP_DEPTH(const unsigned int value)
{//{{{
    if ((MIN_CAP_DEPTH <= value) && (value <= MAX_CAP_DEPTH)){
        mCAP_DEPTH = value;
    }else{
        re_printf("warning", "Invalid CAP_DEPTH (%d) of the ECC_CRAM_E2_FCC2_CAP_DEPTH parameter. Valid range is %d-%d. Value %d is set as default.\n", value, MIN_CAP_DEPTH, MAX_CAP_DEPTH, CRAM_DEFAULT_CAP_DEPTH);
    }
}//}}}

/// Set ERRINT_INIT

void ECC_CRAM_E2_FCC2::ERRINT_INIT(const unsigned int value)
{//{{{
    if ((MIN_ERRINT_INIT <= (int)value) && (value <= MAX_ERRINT_INIT)){
        mERRINT_INIT = value;
    }else{
        re_printf("warning", "Invalid ERRINT_INIT (%d) of the ECC_CRAM_E2_FCC2_ERRINT_INIT parameter. Valid range is %d-%d. Value %d is set as default.\n", value, MIN_ERRINT_INIT, MAX_ERRINT_INIT, CRAM_DEFAULT_ERRINT_INIT);
    }
}//}}}

/// Issue error to ECC

void ECC_CRAM_E2_FCC2::IssueError(const unsigned int errortype)
{//{{{
    ...
    if ((errortype == em1bitError) || (errortype == em2bitError) || (errortype == emAddressFeedbackError)){
        ...
        re_printf("warning", "Invalid error type (%d) of ECC_CRAM_E2_FCC2_IssueError command.\n", errortype);
    }
}//}}}

/// Config error address

void ECC_CRAM_E2_FCC2::SetErrorAddr(const unsigned int addr_value)
{//{{{
...
std::string ECC_CRAM_E2_FCC2::CalculateEDLCode()
{//{{{
    std::string edl_name = "";
...
void ECC_CRAM_E2_FCC2::SetEDL (const std::string edl, const unsigned int pe_index)
{//{{{
    ...
    if (!mIsEDLConfig){
        re_printf("warning", "Invalid EDL (%s, %d) of ECC_CRAM_E2_FCC2_SetEDL command.\n", edl.c_str(), pe_index);
    }
}//}}}

/// Dump Status info

void ECC_CRAM_E2_FCC2::DumpStatusInfo()
{//{{{
    std::string type = "StatInfo";
    printf("PROFILE(%s): ECC_CRAM_E2_FCC2: Info [%20s] (%10s):\n", type.c_str(), sc_time_stamp().to_string().c_str(), this->name());
    printf("PROFILE(%s): ECC_CRAM_E2_FCC2:   CR_SERSTR: 0x%08X\n", type.c_str(), mpMECCCAP_CRFunc->GetCR_SERSTR());
    printf("PROFILE(%s): ECC_CRAM_E2_FCC2:   CR_DERSTR: 0x%08X\n", type.c_str(), mpMECCCAP_CRFunc->GetCR_DERSTR());
    printf("PROFILE(%s): ECC_CRAM_E2_FCC2:   CR_OVFSTR: 0x%08X\n", type.c_str(), mpMECCCAP_CRFunc->GetCR_OVFSTR());
    for (unsigned int i = 0; i < mCAP_DEPTH; i++){
        ...
        printf("PROFILE(%s): ECC_CRAM_E2_FCC2:   CR_%2dSEADR: SEDL = 0x%02X, SEADR = 0x%08X\n", type.c_str(), i, sedl, seadr);
    }
    unsigned int dedl = mpMECCCAP_CRFunc->GetDEDLBit();
    unsigned int deadr = mpMECCCAP_CRFunc->GetDEADRBit() >> (mAWIDTH_LSB - MIN_AWIDTH_LSB);
    printf("PROFILE(%s): ECC_CRAM_E2_FCC2:   CR_00DEADR: DEDL = 0x%02X, DEADR = 0x%08X\n", type.c_str(), dedl, deadr);
}//}}}

/// Command to assert reset

void ECC_CRAM_E2_FCC2::AssertReset (const std::string reset_name, const double start_time, const double period)
{//{{{
    if (reset_name == "presetn") {
...
void ECC_CRAM_E2_FCC2::SetCLKFreq (const std::string clock_name, const sc_dt::uint64 freq, const std::string unit)
{//{{{
    std::vector<std::string> word_vector;
...
void ECC_CRAM_E2_FCC2::GetCLKFreq (const std::string clock_name)
{//{{{
    std::vector<std::string> word_vector;
    SeparateString(word_vector, clock_name);
    if (word_vector.size() == 1) {
...
void ECC_CRAM_E2_FCC2::ForceRegister (const std::string block_name, const unsigned int block_index, const std::string reg_name, const unsigned int reg_value)
{//{{{ 
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ECC_CRAM_E2_FCC2::ReleaseRegister (const std::string block_name, const unsigned int block_index, const std::string reg_name)
{//{{{
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ECC_CRAM_E2_FCC2::WriteRegister (const std::string block_name, const unsigned int block_index, const std::string reg_name, const unsigned int reg_value)
{//{{{
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ECC_CRAM_E2_FCC2::ReadRegister (const std::string block_name, const unsigned int block_index, const std::string reg_name)
{//{{{
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ECC_CRAM_E2_FCC2::ListRegister (void)
{//{{{
    std::vector<std::string> cmd;
    cmd.push_back("reg");
    for (unsigned int i = 0; i < PE_NUM; i++){
...
void ECC_CRAM_E2_FCC2::Help (const std::string type)
{//{{{
    std::vector<std::string> word_vector;
    SeparateString(word_vector, type);
    if (word_vector.size() == 1) {
        if (word_vector[0] == "parameters") {
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_MessageLevel (\"ECC_CRAM_E2_FCC2 instance\", \"fatal|error|warning|info\")                          Set debug message level (Default: fatal|error)");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_DumpRegisterRW (\"ECC_CRAM_E2_FCC2 instance\", \"true/false\")                                      Enable/disable dumping access register (Default: false)");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_DumpInterrupt (\"ECC_CRAM_E2_FCC2 instance\", \"true/false\")                                       Enable/disable dumping interrupt info (Default: false)");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_AWIDTH_MSB (\"ECC_CRAM_E2_FCC2 instance\", value)                                                   Set address bit width MSB (Default: 19). Valid range to be set: 7-26");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_AWIDTH_LSB (\"ECC_CRAM_E2_FCC2 instance\", value)                                                   Set address bit width LSB (Default: 2). Valid range to be set: 2-4");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_CAP_DEPTH (\"ECC_CRAM_E2_FCC2 instance\", value)                                                    Set number of address registers (Default: 8). Valid range to be set: 1-32");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_ERRINT_INIT (\"ECC_CRAM_E2_FCC2 instance\", value)                                                  Set initial value of ERRINT register (Default: 0x8F). Valid range to be set: 0x0-0xFF");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_EnableRegisterMessage (\"ECC_CRAM_E2_FCC2 instance\", \"true/false\")             Enable/disable info/warning/error message of register IF (Default: true)");
        } else if (word_vector[0] == "commands") {
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_IssueError (\"ECC_CRAM_E2_FCC2 instance\", error_type)                                              Issue pseudo error to the ECC_CRAM_E2_FCC2 model");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_SetErrorAddr (\"ECC_CRAM_E2_FCC2 instance\", addr_value)                                            Set error detection address");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_SetEDL (\"ECC_CRAM_E2_FCC2 instance\", \"edl\", pe_index)                                           Set error detection location");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_DumpStatusInfo (\"ECC_CRAM_E2_FCC2 instance\")                                                      Dump information of the error status registers of the ECC_CRAM_E2_FCC2 model");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_AssertReset (\"ECC_CRAM_E2_FCC2 instance\", \"reset_name\", start_time, period)                     Assert and de-assert reset signal to the ECC_CRAM_E2_FCC2 model");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_SetCLKFreq (\"ECC_CRAM_E2_FCC2 instance\", \"clock_name\", freq, \"unit\")                          Set clock frequency value to ECC_CRAM_E2_FCC2 model. Valid clock_name is pclk or sys_clk");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_GetCLKFreq (\"ECC_CRAM_E2_FCC2 instance\", \"clock_name\")                                          Get clock frequency value of ECC_CRAM_E2_FCC2 model. Valid clock_name is pclk or sys_clk");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_ForceRegister (\"ECC_CRAM_E2_FCC2 instance\", \"block_name\", block_index, \"reg_name\", reg_value) Force register with setting value");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_ReleaseRegister (\"ECC_CRAM_E2_FCC2 instance\", \"block_name\", block_index, \"reg_name\")          Release register from force value");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_WriteRegister (\"ECC_CRAM_E2_FCC2 instance\", \"block_name\", block_index, \"reg_name\", reg_value) Write a value to a register");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_ReadRegister (\"ECC_CRAM_E2_FCC2 instance\", \"block_name\", block_index, \"reg_name\")             Read a value from a register");
            SC_REPORT_INFO(this->basename(),"ECC_CRAM_E2_FCC2_ListRegister (\"ECC_CRAM_E2_FCC2 instance\")                                                        Dump name of all registers");
        } else {
...
void ECC_CRAM_E2_FCC2::EnableRegisterMessage(const std::string is_enable)
{//{{{
    std::vector<std::string> word_vector;
    SeparateString(word_vector, is_enable);
    if (word_vector.size() == 1) {
        if (word_vector[0] == "true") {
            ...
            re_printf("warning","Invalid argument: ECC_CRAM_E2_FCC2_EnableRegisterMessage %s.\n", is_enable.c_str());
        }
    } else if (word_vector.size() == 0) {
        std::string temp = "false";
        ...
        re_printf("info","ECC_CRAM_E2_FCC2_EnableRegisterMessage %s\n", temp.c_str());
    } else {
        re_printf("warning","Invalid argument: ECC_CRAM_E2_FCC2_EnableRegisterMessage %s.\n", is_enable.c_str());
    }
}//}}}
