// -----------------------------------------------------------------------------
// $Id: dnf_regif.cpp,v 1.3 2020/02/05 04:31:04 ngathunguyen Exp $
//
...
/// @return none
Cdnf_regif::Cdnf_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef USR_CWR_SYSTEMC
    , cwmem("register", scml_memsize(0xE/2))
    #endif
{
    CommandInit();
    DNFACTL = new vpcl::re_register(0x0000, this, "DNFACTL", name.c_str());
    DNFAEN  = new vpcl::re_register(0x0004, this, "DNFAEN" , name.c_str());
    DNFAENH = new vpcl::re_register(0x0008, this, "DNFAENH", name.c_str());
    DNFAENL = new vpcl::re_register(0x000C, this, "DNFAENL", name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_DNFACTL_, "DNFACTL_", 0x0000);
    CW_DNFACTL = new scml2::bitfield<unsigned short>(DNFACTL->name(), CW_DNFACTL_      ,  0,  8);
    cw_instantiate_reg(CW_DNFAENH_, "DNFAENH_", 0x0008);
    CW_DNFAENH = new scml2::bitfield<unsigned short>(DNFAENH->name(), CW_DNFAENH_      ,  0,  8);
    cw_instantiate_reg(CW_DNFAENL_, "DNFAENL_", 0x000C);
    CW_DNFAENL = new scml2::bitfield<unsigned short>(DNFAENL->name(), CW_DNFAENL_      ,  0,  8);
    cw_instantiate_reg(CW_DNFAEN   , DNFAEN->name() , DNFAEN->addr() );
    #endif
    mCurReg = mRegArray[index++] = new SRegList(DNFACTL  , mCurReg, 0,  1,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DNFAEN   , mCurReg, 0,  2,      "16", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DNFAENH  , mCurReg, 0,  1,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DNFAENL  , mCurReg, 0,  1,       "8", "8|16|32");

    mRegList = mCurReg;

    (*DNFACTL  ) ( 6,  5, "NFSTS"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*DNFACTL  ) ( 2,  0, "PRS"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*DNFAEN   ) (15,  8, "NFENH"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*DNFAEN   ) ( 7,  0, "NFENL"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*DNFAENH  ) ( 7,  0, "NFENH"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*DNFAENL  ) ( 7,  0, "NFENL"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["DNFAEN"]["NFENL"] = &Cdnf_regif::cb_DNFAEN_NFENL;
    mWrCbAPI["DNFAENH"]["NFENH"] = &Cdnf_regif::cb_DNFAENH_NFENH;
    mWrCbAPI["DNFAENL"]["NFENL"] = &Cdnf_regif::cb_DNFAENL_NFENL;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cdnf_regif::~Cdnf_regif()
{
...
Cdnf_regif::uint Cdnf_regif::bit_select(
...
bool Cdnf_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cdnf_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cdnf_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                            const unsigned char *p_data,   ///< [in] Writing data
                            cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cdnf_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                            unsigned char *p_data,   ///< [out] Reading data
                            cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cdnf_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                const unsigned char *p_data,  ///< [in] Writing data
                                cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cdnf_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                unsigned char *p_data,   ///< [out] Reading data
                                cuint size,              ///< [in]  Data size (byte)
                                bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cdnf_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                             unsigned char *p_data,          ///< [out] Reading data
                             cuint size,                     ///< [in]  Data size (byte)
...
bool Cdnf_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                             const unsigned char *p_data,      ///< [in] Writing data
                             cuint size,                       ///< [in] Data size (byte)
                             cuint reg_index,                  ///< [in] Register index
...
bool Cdnf_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return string
std::string Cdnf_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cdnf_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return None
void Cdnf_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        DNFACTL_NFSTS               = (*DNFACTL)["NFSTS"  ];
        DNFACTL_PRS                 = (*DNFACTL)["PRS"    ];
        return;
    }
    if (addr == 0x0004) {
        DNFAEN_NFENH                = (*DNFAEN )["NFENH"  ];
        DNFAEN_NFENL                = (*DNFAEN )["NFENL"  ];
        return;
    }
    if (addr == 0x0008) {
        DNFAENH_NFENH               = (*DNFAENH)["NFENH"  ];
        return;
    }
    if (addr == 0x000C) {
        DNFAENL_NFENL               = (*DNFAENL)["NFENL"  ];
        return;
    }
}
...
/// @return None
void Cdnf_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*DNFACTL)["NFSTS"  ] = DNFACTL_NFSTS;
        (*DNFACTL)["PRS"    ] = DNFACTL_PRS;
        return;
    }
    if (addr == 0x0004) {
        (*DNFAEN )["NFENH"  ] = DNFAEN_NFENH;
        (*DNFAEN )["NFENL"  ] = DNFAEN_NFENL;
        return;
    }
    if (addr == 0x0008) {
        (*DNFAENH)["NFENH"  ] = DNFAENH_NFENH;
        return;
    }
    if (addr == 0x000C) {
        (*DNFAENL)["NFENL"  ] = DNFAENL_NFENL;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cdnf_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cdnf_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cdnf_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cdnf_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cdnf_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cdnf_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cdnf_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Cdnf_regif::uint Cdnf_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return none
void Cdnf_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cdnf_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cdnf_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cdnf_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
unsigned short Cdnf_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
unsigned short Cdnf_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(unsigned short));
    return sizeof(unsigned short);
}
void Cdnf_regif::cw_instantiate_reg(scml2::reg<unsigned short> * reg, std::string reg_name, int offset)
{
