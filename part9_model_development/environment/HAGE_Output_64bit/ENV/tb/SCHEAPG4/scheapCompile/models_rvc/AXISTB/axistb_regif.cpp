// -----------------------------------------------------------------------------
// $Id: axistb_regif.cpp,v 1.1 2014/10/09 08:49:59 dungvannguyen Exp $
//
...
//     %%REG      HDMACTL              HDMACTL             8|16|32     32          0x2000      R|W         0x0     TRUE        W
//     %%REG      HDMASTAT             HDMASTAT            8|16|32     32          0x2004      R           0x0     TRUE        -
// 
// %REG_NAME HDMACTL        
//     %%TITLE     name        upper   lower   init    access  support     callback
//     %%BIT       SICSRQ      1       1       0       R|W     TRUE        -
//     %%BIT       GRISRQ      0       0       0       R|W     TRUE        W
//     
// %REG_NAME HDMASTAT    
//     %%TITLE     name        upper   lower   init    access  support     callback
//     %%BIT       SICSAK      1       1       0       R       TRUE        -
//     %%BIT       GRISAK      0       0       0       R       TRUE        -
////////////////////////////////////////////////////////////////////////////////
/// @file axistb_regif.cpp
/// @brief Register IF class of model AXISTB
/// $Id: axistb_regif.cpp,v 1.1 2014/10/09 08:49:59 dungvannguyen Exp $
/// $Date: 2014/10/09 08:49:59 $
/// $Revison$
...
/// @return none
Caxistb_regif::Caxistb_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
    , cwmem("register", scml_memsize(0x2008/4))
    #endif
{
    CommandInit();
    HDMACTL  = new vpcl::re_register(0x2000, this, "HDMACTL" , name.c_str());
    HDMASTAT = new vpcl::re_register(0x2004, this, "HDMASTAT", name.c_str());
    ...
    mRegMap[0x2000]     = index;
    mRegMap[0x2000 + 1] = index;
    mRegMap[0x2000 + 2] = index;
    mRegMap[0x2000 + 3] = index++;
    mRegMap[0x2004]     = index;
    mRegMap[0x2004 + 1] = index;
    mRegMap[0x2004 + 2] = index;
    mRegMap[0x2004 + 3] = index++;
    mTotalRegNum = index;
    mRegArray = new SRegList* [mTotalRegNum];
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_HDMACTL   , HDMACTL->name() , HDMACTL->addr() );
    cw_instantiate_reg(CW_HDMASTAT  , HDMASTAT->name(), HDMASTAT->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(HDMACTL   , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(HDMASTAT  , mCurReg, 0,  4, "8|16|32", "8|16|32");

    mRegList = mCurReg;

    (*HDMACTL   ) ( 1,  1, "SICSRQ"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*HDMACTL   ) ( 0,  0, "GRISRQ"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*HDMASTAT  ) ( 1,  1, "SICSAK"  , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*HDMASTAT  ) ( 0,  0, "GRISAK"  , 0x0       , "R"     , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["HDMACTL"]["GRISRQ"] = &Caxistb_regif::cb_HDMACTL_GRISRQ;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Caxistb_regif::~Caxistb_regif()
{
...
Caxistb_regif::uint Caxistb_regif::bit_select(
...
bool Caxistb_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Caxistb_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Caxistb_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                               const unsigned char *p_data,   ///< [in] Writing data
                               cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Caxistb_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                               unsigned char *p_data,   ///< [out] Reading data
                               cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Caxistb_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                   const unsigned char *p_data,  ///< [in] Writing data
                                   cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Caxistb_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                   unsigned char *p_data,   ///< [out] Reading data
                                   cuint size,              ///< [in]  Data size (byte)
                                   bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Caxistb_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                unsigned char *p_data,          ///< [out] Reading data
                                cuint size,                     ///< [in]  Data size (byte)
...
bool Caxistb_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                const unsigned char *p_data,      ///< [in] Writing data
                                cuint size,                       ///< [in] Data size (byte)
                                cuint reg_index,                  ///< [in] Register index
...
bool Caxistb_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Caxistb_regif::CommandInit()
{
...
/// @return string
std::string Caxistb_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Caxistb_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Caxistb_regif::InitLocalVal()
{
    // Initialize local variables
    HDMACTL_SICSRQ = 0x0;
    HDMACTL_GRISRQ = 0x0;
...
/// @return None
void Caxistb_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x2000) {
        HDMACTL_SICSRQ               = (*HDMACTL )["SICSRQ"  ];
        HDMACTL_GRISRQ               = (*HDMACTL )["GRISRQ"  ];
        return;
    }
    if (addr == 0x2004) {
        HDMASTAT_SICSAK              = (*HDMASTAT)["SICSAK"  ];
        HDMASTAT_GRISAK              = (*HDMASTAT)["GRISAK"  ];
        return;
    }
}
...
/// @return None
void Caxistb_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x2000) {
        (*HDMACTL )["SICSRQ"  ] = HDMACTL_SICSRQ;
        (*HDMACTL )["GRISRQ"  ] = HDMACTL_GRISRQ;
        return;
    }
    if (addr == 0x2004) {
        (*HDMASTAT)["SICSAK"  ] = HDMASTAT_SICSAK;
        (*HDMASTAT)["GRISAK"  ] = HDMASTAT_GRISAK;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Caxistb_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Caxistb_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Caxistb_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Caxistb_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Caxistb_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Caxistb_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Caxistb_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Caxistb_regif::uint Caxistb_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Caxistb_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Caxistb_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Caxistb_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Caxistb_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Caxistb_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Caxistb_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Caxistb_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
