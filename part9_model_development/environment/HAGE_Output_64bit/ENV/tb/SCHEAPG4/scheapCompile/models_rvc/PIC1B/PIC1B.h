// ---------------------------------------------------------------------
// $Id: PIC1B.h,v 1.3 2015/08/21 13:15:32 huepham Exp $
//
...
class PIC1B_Port: public sc_module
{
friend class PIC1B;

public:
    sc_in<bool>  *InputPort      [PIC1B_AgentController::emInputNum];
    sc_out<bool> *OutputPort     [PIC1B_AgentController::emOutputNum];
    sc_out<bool> *SyncOutputPort [PIC1B_AgentController::emSyncOutputNum];
    
    SC_HAS_PROCESS (PIC1B_Port);
    PIC1B_Port (sc_module_name name, PIC1B *PIC_Ptr);
    ~PIC1B_Port (void);
    
private:
    PIC1B *mPIC_Ptr;

    bool mWriteValue [PIC1B_AgentController::emOutputNum];
    bool mWriteSyncValue [PIC1B_AgentController::emSyncOutputNum];
    bool mDT_Input [PIC1B_AgentController::emDTnum];           // Store the value of DTnm inputs
    bool mRS_SetInput [PIC1B_AgentController::emRSnum];        // Store the value of S input of RSnm
    bool mRS_ResetInput [PIC1B_AgentController::emRSnum];      // Store the value of R input of RSnm
    bool mWNG_CKENInput [2][PIC1B_AgentController::emWNGnum];  // Store the value of CKEN input of WNGnm
    ...
    sc_event mHandleInputChangeEvent  [PIC1B_AgentController::emInputNum];
    sc_event mWriteOutputPortEvent    [PIC1B_AgentController::emOutputNum];
    sc_event mAssertSyncOutputEvent   [PIC1B_AgentController::emSyncOutputNum];
    sc_event mDeassertSyncOutputEvent [PIC1B_AgentController::emSyncOutputNum];
    sc_event mWriteSyncOutputEvent    [PIC1B_AgentController::emSyncOutputNum];
...
class PIC1B: public sc_module,
            public PIC1B_AgentController,
            public BusSlaveBase<32,1>
{
friend class PIC1B_Port;
...
private:
    ...
    PIC1B_Func *mPIC1B_Func;             //PIC1B_Func class instance
    PIC1B_Port *mPIC1B_Port;             //PIC1B_Port class instance
    ...
    unsigned int mDelayTSSTCycle;        //Cycle of TSST delay on PCLK clock
    ...
    sc_event mDelayTSSTEndEvent;        //Call DelayTSSTEndMethod() when PIC0SST is set
    
    sc_event mUpdateFlagEvent [2][PIC1B_AgentController::emWNGnum];             //Call UpdateFlagMethod() to update WNGFn flag
    sc_event mUpdateWNG_FFiOuputEvent [2][PIC1B_AgentController::emWNGnum];     //Call UpdateWNG_FFiOuputMethod() to update FFi output
    sc_event mUpdateWNG_FFgOuputEvent [2][PIC1B_AgentController::emWNGnum];     //Call UpdateWNG_FFgOuputMethod() to update FFg output
    sc_event mUpdateREG513_FFOuputEvent [2][PIC1B_AgentController::emFFnum];    //Call UpdateREG513_FFOuputMethod() to update FF output of PIC0REG5n[13]
    sc_event mUpdateREG514_FFOuputEvent [2][PIC1B_AgentController::emFFnum];    //Call UpdateREG514_FFOuputMethod() to update FF output of PIC0REG5n[14]
    sc_event mUpdateTSGOUTCTR_FFOuputEvent [2][PIC1B_AgentController::emFFnum]; //Call UpdateTSGOUTCTR_FFOuputMethod() to update FF output of PIC0TSGOUTCTRn[1-6]
    sc_event mHandleResetRSEvent [PIC1B_AgentController::emRSnum];              //Call HandleResetRSMethod() to reset RSnm
    sc_event mHandleResetDTEvent [PIC1B_AgentController::emDTnum];              //Call HandleResetRSMethod() to reset DTnm
    sc_event mHandleResetRTOEvent [2][PIC1B_AgentController::emRTOnum];         //Call HandleResetRTOMethod() to reset RTOnm
...
public:
    ...
    SC_HAS_PROCESS (PIC1B);
    PIC1B (sc_module_name name,
    ...
    void SetDelayTSSTCycle (const unsigned int cycle_num);
...
private:
    ...
    /// Virtual functions of PIC1B_AgentController class
    bool GetResetStatus (void);
    bool CheckClockPeriod (const std::string clock_name);
    void ActiveDelayTSST(void);
