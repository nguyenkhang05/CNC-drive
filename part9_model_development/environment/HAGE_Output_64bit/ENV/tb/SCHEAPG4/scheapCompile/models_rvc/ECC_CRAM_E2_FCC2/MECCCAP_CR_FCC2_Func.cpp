// ---------------------------------------------------------------------
// $Id: MECCCAP_CR_FCC2_Func.cpp,v 1.9 2019/09/26 10:07:11 landang Exp $
//
...
//========================================================
/// Constructor
MECCCAP_CR_FCC2_Func::MECCCAP_CR_FCC2_Func (std::string name, ECC_CRAM_E2_FCC2_AgentController* CRAMAgentController):
                Cmecccap_cr_fcc2_regif(name, 32)
                ,BusSlaveFuncIf()
{//{{{
    ...
    Cmecccap_cr_fcc2_regif::set_instance_name(name);
}//}}}

/// Destructor
MECCCAP_CR_FCC2_Func::~MECCCAP_CR_FCC2_Func()
{//{{{
}//}}}

/// Reset Func and registers
void MECCCAP_CR_FCC2_Func::EnableReset(const std::string reset_name, const bool is_active)
{//{{{
    if (reset_name == "sys_resetz"){
        Cmecccap_cr_fcc2_regif::EnableReset(is_active);
    }
}//}}}

/// Read API (target socket)
void MECCCAP_CR_FCC2_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void MECCCAP_CR_FCC2_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void MECCCAP_CR_FCC2_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cmecccap_cr_fcc2_regif::reg_handle_command(cmd);
                if (msg_return != "") {
            ...
            msg_return = Cmecccap_cr_fcc2_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
bool MECCCAP_CR_FCC2_Func::Set1bitStatusAndAddress(unsigned int edl_code, unsigned int mod_address)
{//{{{
    unsigned int cap_depth = mpCRAMAgentController->GetCapDepth();
...
void MECCCAP_CR_FCC2_Func::SetFlagAddressBufferOverflow()
{//{{{
...
bool MECCCAP_CR_FCC2_Func::CheckHasRoomFor1bit()
{//{{{
    unsigned int cap_depth = mpCRAMAgentController->GetCapDepth();
    unsigned int status = (unsigned int)(*CR_SERSTR);
    bool is_find_room = false;
...
/// Set FIFO overflow
void MECCCAP_CR_FCC2_Func::SetFIFOOverflow ()
{//{{{
...
/// Get SEDL bit
unsigned int MECCCAP_CR_FCC2_Func::GetSEDLBit(unsigned int index)
{//{{{
    return (unsigned int)(*CR_nSEADR[index])["SEDL"];
}//}}}

/// Get SEADR bit
unsigned int MECCCAP_CR_FCC2_Func::GetSEADRBit(unsigned int index)
{//{{{
    return (unsigned int)(*CR_nSEADR[index])["SEADR0"];
}//}}}

/// Get DEDL bit
unsigned int MECCCAP_CR_FCC2_Func::GetDEDLBit()
{//{{{
    return (unsigned int)(*CR_00DEADR)["DEDL"];
}//}}}

/// Get DEADR bit
unsigned int MECCCAP_CR_FCC2_Func::GetDEADRBit()
{//{{{
    return (unsigned int)(*CR_00DEADR)["DEADR0"];
}//}}}

/// Get Start mask address
unsigned int MECCCAP_CR_FCC2_Func::GetERMSA(unsigned int index)
{//{{{
...
unsigned int MECCCAP_CR_FCC2_Func::GetERMEA(unsigned int index)
{//{{{
    unsigned int ret_val = 0;
    if (index == 0){
...
unsigned int MECCCAP_CR_FCC2_Func::GetERM_EN(unsigned int index)
{//{{{
    unsigned int ret_val = 0;
    if (index == 0){
...
bool MECCCAP_CR_FCC2_Func::CheckNoFatalStatus()
{//{{{
    return ((unsigned int)(*CR_DERSTR) == 0);
}//}}}

/// Set 2bit error status
void MECCCAP_CR_FCC2_Func::Set2bitErrorStatus()
{//{{{
    (*CR_DERSTR)["DEDF00"] = 1;
}//}}}

/// Set address parity error status
void MECCCAP_CR_FCC2_Func::SetAddrFeedbackErrorStatus()
{//{{{
    (*CR_DERSTR)["AFEF00"] = 1;
}//}}}

/// Set fatal error address and edl code according
void MECCCAP_CR_FCC2_Func::SetFatalAddress(unsigned int edl_code, unsigned int mod_address)
{//{{{
...
unsigned int MECCCAP_CR_FCC2_Func::GetCR_SERSTR()
{//{{{
    return (unsigned int)(*CR_SERSTR);
}//}}}

/// Get value of CR_DERSTR register
unsigned int MECCCAP_CR_FCC2_Func::GetCR_DERSTR()
{//{{{
    return (unsigned int)(*CR_DERSTR);
}//}}}

/// Get value of CR_OVFSTR register
unsigned int MECCCAP_CR_FCC2_Func::GetCR_OVFSTR()
{//{{{
    return (unsigned int)(*CR_OVFSTR);
}//}}}

/// Write ERRINT_INIT to ERRINT reg after reset
void MECCCAP_CR_FCC2_Func::WriteERRINT_INIT(unsigned int value)
{//{{{
...
/// Get ERRINT value
unsigned int MECCCAP_CR_FCC2_Func::GetERRINT()
{//{{{
...
void MECCCAP_CR_FCC2_Func::cb_CR_SSTCLR_CR_SSTCLR(RegCBstr str)
{//{{{ 
...
void MECCCAP_CR_FCC2_Func::cb_CR_DSTCLR_DSTCLR00(RegCBstr str)
{//{{{
    // clear DED.
...
void MECCCAP_CR_FCC2_Func::cb_CR_OVFCLR_SERROVFCLR0(RegCBstr str)
{//{{{ 
...
void MECCCAP_CR_FCC2_Func::cb_CR_ERMSA0_ERMSA(RegCBstr str)
{//{{{
...
void MECCCAP_CR_FCC2_Func::cb_CR_ERMEA0_ERMEA(RegCBstr str)
{//{{{
...
void MECCCAP_CR_FCC2_Func::cb_CR_ERMSA1_ERMSA(RegCBstr str)
{//{{{
...
void MECCCAP_CR_FCC2_Func::cb_CR_ERMEA1_ERMEA(RegCBstr str)
{//{{{
