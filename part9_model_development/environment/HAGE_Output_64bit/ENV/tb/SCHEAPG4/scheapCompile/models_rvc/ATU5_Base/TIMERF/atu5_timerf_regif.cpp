// -----------------------------------------------------------------------------
// $Id: atu5_timerf_regif.cpp 1196 2019-10-17 05:33:04Z chuonghoangle $
//
...
//
// Input file : atu5_timerf_regif.txt
////////////////////////////////////////////////////////////////////////////////
// %MODULE ATU5_TIMERF
//     #              name      offset_size
...
// %REG_NAME        NCMCR1F
// %%TITLE          name      upper     lower     init         access    support   callback
// %%BIT            NCM1F     15        0         0            R|W       TRUE      -
// 
// 
// %REG_NAME        NCMCR2F
// %%TITLE          name      upper     lower     init         access    support   callback
// %%BIT            NCM2F     15        0         0            R|W       TRUE      -
// 
// 
// %REG_NAME        NCCRF
// %%TITLE          name      upper     lower     init         access    support   callback
// %%BIT            NCEF      15        0         0            R|W       TRUE      -
// 
// 
// %REG_NAME        PVFCRF
// %%TITLE          name      upper     lower     init         access    support   callback
...
// 
////////////////////////////////////////////////////////////////////////////////
/// @file atu5_timerf_regif.cpp
/// @brief Register IF class of model ATU5_TIMERF
/// $Id: atu5_timerf_regif.cpp 1196 2019-10-17 05:33:04Z chuonghoangle $
...
/// @return none
Catu5_timerf_regif::Catu5_timerf_regif(std::string name, uint buswidth , unsigned int subblock)
    :vpcl::reg_super()
    ...
    #endif
{
    ...
    (*TSTRF    ) ( msbF - 1,  0, "STRF"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);

    (*NCMCR1F  ) (msbF - 1,  0, "NCM1F"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*NCMCR2F  ) (msbF - 1,  0, "NCM2F"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*NCCRF    ) (msbF - 1,  0, "NCEF"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["TSTRF"]["STRF"] = &Catu5_timerf_regif::cb_TSTRF_STRF;

    mWrCbAPI["PVFCRF"]["PVFCRKEY"] = &Catu5_timerf_regif::cb_PVFCRF_PVFCRKEY;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Catu5_timerf_regif::~Catu5_timerf_regif()
{
...
Catu5_timerf_regif::uint Catu5_timerf_regif::bit_select(
...
bool Catu5_timerf_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Catu5_timerf_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Catu5_timerf_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                    const unsigned char *p_data,   ///< [in] Writing data
                                    cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Catu5_timerf_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                    unsigned char *p_data,   ///< [out] Reading data
                                    cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Catu5_timerf_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                        const unsigned char *p_data,  ///< [in] Writing data
                                        cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Catu5_timerf_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                        unsigned char *p_data,   ///< [out] Reading data
                                        cuint size,              ///< [in]  Data size (byte)
                                        bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Catu5_timerf_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                     unsigned char *p_data,          ///< [out] Reading data
                                     cuint size,                     ///< [in]  Data size (byte)
...
bool Catu5_timerf_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                     const unsigned char *p_data,      ///< [in] Writing data
                                     cuint size,                       ///< [in] Data size (byte)
                                     cuint reg_index,                  ///< [in] Register index
...
bool Catu5_timerf_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Catu5_timerf_regif::CommandInit()
{
...
/// @return string
std::string Catu5_timerf_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Catu5_timerf_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Catu5_timerf_regif::InitLocalVal()
{
    // Initialize local variables
    TSTRF_STRF = 0x0;
...
/// @return None
void Catu5_timerf_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x5000) {
        TSTRF_STRF                 =  (*TSTRF  )["STRF"  ];
        return;
    }
    if (addr == 0x5004) {
...
/// @return None
void Catu5_timerf_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x5000) {
        (*TSTRF  )["STRF"  ] = TSTRF_STRF;
        return;
    }
    if (addr == 0x5004) {
...
/// @return vector
std::vector<std::string> Catu5_timerf_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Catu5_timerf_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Catu5_timerf_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Catu5_timerf_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Catu5_timerf_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Catu5_timerf_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Catu5_timerf_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Catu5_timerf_regif::uint Catu5_timerf_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return none
void Catu5_timerf_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Catu5_timerf_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Catu5_timerf_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Catu5_timerf_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Catu5_timerf_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Catu5_timerf_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Catu5_timerf_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
