// -----------------------------------------------------------------------------
// $Id: ecccnt_cfp_pen_regif.cpp,v 1.1 2019/09/26 10:06:59 landang Exp $
//
...
//
// Input file : ./input/CFLASH/ecccnt_cfp_pen_regif.txt
////////////////////////////////////////////////////////////////////////////////
// %MODULE ecccnt_cfp_pen
//     #              name     offset_size
//     %%REG_INSTANCE reg_def  7 
// 
// %REG_CHANNEL reg_def                                                       
//     %%TITLE    name                           reg_name                     wsize   rsize        length  offset  factor_start    factor_end  factor_index    factor_step access   init   support  callback
//     %%REG      CFPECCCTL                      CFPECCCTL                    8|16|32 8|16|32      32      0x00    -               -           -               -           R/W      0x0     TRUE     -
//     %%REG      CFPECCTSTCTL                   CFPECCTSTCTL                 8|16|32 8|16|32      32      0x10    -               -           -               -           R/W      0x0     TRUE     -
// %REG_NAME CFPECCCTL                                                            
//     %%TITLE  name               upper   lower   init    access  support     callback
//     %%BIT    SECDIS             1        1      0       R|W     TRUE        -
//     %%BIT    ECCDIS             0        0      0       R|W     TRUE        W
// %REG_NAME CFPECCTSTCTL                                                            
//     %%TITLE  name               upper   lower   init    access  support     callback
//     %%BIT    ECCTST             1        1      0       R|W     TRUE        -
////////////////////////////////////////////////////////////////////////////////
/// @file ecccnt_cfp_pen_regif.cpp
/// @brief Register IF class of model ECCCNT_CFP_PEN
/// $Id: ecccnt_cfp_pen_regif.cpp,v 1.1 2019/09/26 10:06:59 landang Exp $
...
/// @return none
Cecccnt_cfp_pen_regif::Cecccnt_cfp_pen_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
    , cwmem("register", scml_memsize(0x14/4))
    #endif
{
    CommandInit();
    CFPECCCTL    = new vpcl::re_register(0x0000, this, "CFPECCCTL"   , name.c_str());
    CFPECCTSTCTL = new vpcl::re_register(0x0010, this, "CFPECCTSTCTL", name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_CFPECCCTL     , CFPECCCTL->name()   , CFPECCCTL->addr()   );
    cw_instantiate_reg(CW_CFPECCTSTCTL  , CFPECCTSTCTL->name(), CFPECCTSTCTL->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(CFPECCCTL     , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(CFPECCTSTCTL  , mCurReg, 0,  4, "8|16|32", "8|16|32");

    mRegList = mCurReg;

    (*CFPECCCTL     ) ( 1,  1, "SECDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CFPECCCTL     ) ( 0,  0, "ECCDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CFPECCTSTCTL  ) ( 1,  1, "ECCTST"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["CFPECCCTL" ]["ECCDIS"    ] = &Cecccnt_cfp_pen_regif::cb_CFPECCCTL_ECCDIS;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cecccnt_cfp_pen_regif::~Cecccnt_cfp_pen_regif()
{
...
Cecccnt_cfp_pen_regif::uint Cecccnt_cfp_pen_regif::bit_select(
...
bool Cecccnt_cfp_pen_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cecccnt_cfp_pen_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_cfp_pen_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                       const unsigned char *p_data,   ///< [in] Writing data
                                       cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_cfp_pen_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                       unsigned char *p_data,   ///< [out] Reading data
                                       cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_cfp_pen_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                           const unsigned char *p_data,  ///< [in] Writing data
                                           cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_cfp_pen_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                           unsigned char *p_data,   ///< [out] Reading data
                                           cuint size,              ///< [in]  Data size (byte)
                                           bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cecccnt_cfp_pen_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                        unsigned char *p_data,          ///< [out] Reading data
                                        cuint size,                     ///< [in]  Data size (byte)
...
bool Cecccnt_cfp_pen_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                        const unsigned char *p_data,      ///< [in] Writing data
                                        cuint size,                       ///< [in] Data size (byte)
                                        cuint reg_index,                  ///< [in] Register index
...
bool Cecccnt_cfp_pen_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cecccnt_cfp_pen_regif::CommandInit()
{
...
/// @return string
std::string Cecccnt_cfp_pen_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cecccnt_cfp_pen_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cecccnt_cfp_pen_regif::InitLocalVal()
{
    // Initialize local variables
    CFPECCCTL_SECDIS = 0x0;
    CFPECCCTL_ECCDIS = 0x0;
    CFPECCTSTCTL_ECCTST = 0x0;
}

/// Update bit value to local value
/// @return None
void Cecccnt_cfp_pen_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        CFPECCCTL_SECDIS                 = (*CFPECCCTL   )["SECDIS"      ];
        CFPECCCTL_ECCDIS                 = (*CFPECCCTL   )["ECCDIS"      ];
        return;
    }
    if (addr == 0x0010) {
        CFPECCTSTCTL_ECCTST              = (*CFPECCTSTCTL)["ECCTST"      ];
        return;
    }
}
...
/// @return None
void Cecccnt_cfp_pen_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*CFPECCCTL   )["SECDIS"      ] = CFPECCCTL_SECDIS;
        (*CFPECCCTL   )["ECCDIS"      ] = CFPECCCTL_ECCDIS;
        return;
    }
    if (addr == 0x0010) {
        (*CFPECCTSTCTL)["ECCTST"      ] = CFPECCTSTCTL_ECCTST;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cecccnt_cfp_pen_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cecccnt_cfp_pen_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cecccnt_cfp_pen_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cecccnt_cfp_pen_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cecccnt_cfp_pen_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cecccnt_cfp_pen_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cecccnt_cfp_pen_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cecccnt_cfp_pen_regif::uint Cecccnt_cfp_pen_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cecccnt_cfp_pen_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cecccnt_cfp_pen_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cecccnt_cfp_pen_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cecccnt_cfp_pen_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cecccnt_cfp_pen_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cecccnt_cfp_pen_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cecccnt_cfp_pen_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
