// ---------------------------------------------------------------------
// $Id: ECCCNT_CRCn_FCC2_Func.cpp,v 1.9 2019/09/26 10:07:11 landang Exp $
//
...
//========================================================
/// Constructor
ECCCNT_CRCn_FCC2_Func::ECCCNT_CRCn_FCC2_Func (std::string name, unsigned int pe_id, ECC_CRAM_E2_FCC2_AgentController* CRAMAgentController):
                Cecccnt_crcn_fcc2_regif(name, 32)
                ,BusSlaveFuncIf()
{//{{{
    ...
    mProtectedRegMap.insert(std::pair<unsigned int, std::string>(0x0, "CRCAFCTL"));

    Cecccnt_crcn_fcc2_regif::set_instance_name(name);
}//}}}

/// Destructor
ECCCNT_CRCn_FCC2_Func::~ECCCNT_CRCn_FCC2_Func()
{//{{{
}//}}}

/// Reset Func and registers
void ECCCNT_CRCn_FCC2_Func::EnableReset(const std::string reset_name, const bool is_active)
{//{{{
    if (reset_name == "sys_resetz"){
        Cecccnt_crcn_fcc2_regif::EnableReset(is_active);
    }
}//}}}

/// Read API (target socket)
void ECCCNT_CRCn_FCC2_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    // Get extension info
...
void ECCCNT_CRCn_FCC2_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void ECCCNT_CRCn_FCC2_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cecccnt_crcn_fcc2_regif::reg_handle_command(cmd);
                if (msg_return != "") {
            ...
            msg_return = Cecccnt_crcn_fcc2_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
unsigned int ECCCNT_CRCn_FCC2_Func::GetCRCAFCTLReg()
{//{{{
    return (unsigned int)(*CRCAFCTL);
}//}}}

//========================================================
//============ Callback function  ========================
//========================================================
/// Callback function of LR0ECCCTL_AFEDIS bit
void ECCCNT_CRCn_FCC2_Func::cb_CRCAFCTL_AFEDIS(RegCBstr str)
{//{{{ 
    mpCRAMAgentController->WriteDisPort_CRC(mPeId, (unsigned int)(*CRCAFCTL));
}//}}}
