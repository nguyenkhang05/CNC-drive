// ---------------------------------------------------------------------
// $Id: IPG_DataHandlerIFU.cpp,v 1.3 2014/04/17 03:09:28 dungvannguyen Exp $
//
...
///Constructor of IPG_DataHandlerIFU class
IPG_DataHandlerIFU::IPG_DataHandlerIFU(std::string name,
...
void IPG_DataHandlerIFU::read(unsigned int offsetAddress,TlmBasicPayload& trans,BusTime_t* t,bool debug)
{//{{{
    ...
    if (!debug) {//Normal transaction
        if (mIPG_AgentController->GetResetStatus()) {//lvcirreset is active
            ...
            mIPG_AgentController->print_msg("error","The IFU reading transaction is fail because the reset is active\n");
        }
    }
    if (is_continue_to_transfer) {
        if ((mIPG_AgentController->CheckClockPeriod("pclk"))&&(mIPG_AgentController->CheckClockPeriod("lvcclk"))) {
            if (vcid == 0) {
                if (mIPG_AgentController->CheckAccessPermission(true, trans)) {
...
///Overwrite virtual functions of BusBridgeCoreBase class
void IPG_DataHandlerIFU::deleteTrans( void )
{//{{{
}//}}}
