// ---------------------------------------------------------------------
// $Id: ATU5010_TIMERD_SUB.cpp,v 1.2 2018/01/26 10:15:45 binhnguyen Exp $
//
...
/// Description  : Constructor of ATU5010_TIMERD_SUB class
///*********************************
ATU5010_TIMERD_SUB::ATU5010_TIMERD_SUB (sc_module_name name,
            ATU5010_TIMERD *ATU5010_TIMERD_Ptr,
            unsigned int sub):
                sc_module(name),
                ATU5010_TIMERD_SUB_AgentController(),
                //input ports
                TAEO2A("TAEO2A"),
                TAEO2B("TAEO2B"),
                PHU0DFE("PHU0DFE"),
...
{//{{{
   mTIMERD = ATU5010_TIMERD_Ptr;
   subnum = sub;
    std::ostringstream port_name;
    for (unsigned int index = 0; index < ATU5010_TIMERD_SUB_AgentController::emChannel; index++) {
        port_name.str("");
    ...
    mATU5010_TIMERD_SUBFunc = new ATU5010_TIMERD_SUB_Func((std::string)name, this);
    sc_assert(mATU5010_TIMERD_SUBFunc != NULL);

    //TCNTD1
    std::ostringstream namecnt1;
    namecnt1.str("");namecnt1 << "mTimer_TCNT1D_CUCR1D";
    mTimer_TCNT1D_CUCR1D = new ATU5010_Timer(namecnt1.str().c_str(), mTIMERD->mATU5010);
    sc_assert(mTimer_TCNT1D_CUCR1D != NULL);

    for (unsigned int index = 0; index <= 3; index++) {
        std::ostringstream name;
        name.str("");name << "mTimer_TCNT1D_OCR1D" << index;
        mTimer_TCNT1D_OCR1D[index] = new ATU5010_Timer(name.str().c_str(), mTIMERD->mATU5010);
        sc_assert(mTimer_TCNT1D_OCR1D[index] != NULL);

        name.str("");name << "mTimer_TCNT1D_OCR2D" << index;
        mTimer_TCNT1D_OCR2D[index] = new ATU5010_Timer(name.str().c_str(), mTIMERD->mATU5010);
    ...
    mTimer_TCNT2D_CUCR2D = new ATU5010_Timer(namecnt2.str().c_str(), mTIMERD->mATU5010);
    sc_assert(mTimer_TCNT2D_CUCR2D != NULL);

    for (unsigned int index = 0; index <= 3; index++) {
        std::ostringstream name;
        name.str("");name << "mTimer_TCNT2D_OCR1D" << index;
        mTimer_TCNT2D_OCR1D[index] = new ATU5010_Timer(name.str().c_str(), mTIMERD->mATU5010);
        sc_assert(mTimer_TCNT2D_OCR1D[index] != NULL);

        name.str("");name << "mTimer_TCNT2D_OCR2D" << index;
        mTimer_TCNT2D_OCR2D[index] = new ATU5010_Timer(name.str().c_str(), mTIMERD->mATU5010);
    ...
    for (unsigned int index = 0; index <= 3; index++) {
        ...
        mTimer_DCNTD[index] = new ATU5010_Timer(name.str().c_str(), mTIMERD->mATU5010);
    ...
    for (unsigned int index = 0; index <= 3; index++) {
        ...
        mTimer_OFMICNTD[index] = new ATU5010_Timer(name.str().c_str(), mTIMERD->mATU5010);
        sc_assert(mTimer_OFMICNTD[index] != NULL);

        name.str("");name << "mTimer_ONMICNTD" << index;
        mTimer_ONMICNTD[index] = new ATU5010_Timer(name.str().c_str(), mTIMERD->mATU5010);
        sc_assert(mTimer_ONMICNTD[index] != NULL);

        name.str("");name << "mTimer_OTONMICNTD" << index;
        mTimer_OTONMICNTD[index] = new ATU5010_Timer(name.str().c_str(), mTIMERD->mATU5010);
        sc_assert(mTimer_OTONMICNTD[index] != NULL);
    }

    //Initialize ports
    for(unsigned int ch_id = 0; ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel; ch_id++) {
   ...
   // Compare match A counters
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::PreHandleCmpMatchCNT1OCR1DMethod, this, signal_id),
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::HandleCmpMatchCNT1OCR1DMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::PreHandleCmpMatchCNT2OCR1DMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::HandleCmpMatchCNT2OCR1DMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::HandleInterruptCMPAMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::ClearInterruptCMPAMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::PreHandleCmpMatchCNT1OCR2DMethod, this, signal_id),
     ...
     for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::HandleCmpMatchCNT1OCR2DMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::PreHandleCmpMatchCNT2OCR2DMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::HandleCmpMatchCNT2OCR2DMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::HandleInterruptCMPBMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::ClearInterruptCMPBMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 2; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::WriteCompareMatchValueMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 2; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::WriteCounterValueMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::HandleInterruptFEIEDMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::HandleInterruptREIEDMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::ClearInterruptFEIEDMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::ClearInterruptREIEDMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::StartTimerOFMICNTDMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++)
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::StartTimerONMICNTDMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::StartTimerOTOMICNTDMethod, this, signal_id),
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::StopTimerOFMICNTDMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++)
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::StopTimerONMICNTDMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::StopTimerOTOMICNTDMethod, this, signal_id),
    ...
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        sc_core::sc_spawn_options opt;
        opt.spawn_method();
        opt.set_sensitivity(&(mWriteOFMICNTDEvent[signal_id]));
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::WriteTimerOFMICNTDMethod, this, signal_id),
                          sc_core::sc_gen_unique_name("WriteTimerOFMICNTDMethod"), &opt);
    }
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++)
        {
        sc_core::sc_spawn_options opt;
        opt.spawn_method();
        opt.set_sensitivity(&(mWriteONMICNTDEvent[signal_id]));
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::WriteTimerONMICNTDMethod, this, signal_id),
                          sc_core::sc_gen_unique_name("WriteTimerONMICNTDMethod"), &opt);
    }
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        sc_core::sc_spawn_options opt;
        opt.spawn_method();
        opt.set_sensitivity(&(mWriteOTOMICNTDEvent[signal_id]));
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::WriteTimerOTOMICNTDMethod, this, signal_id),
                          sc_core::sc_gen_unique_name("WriteTimerOTOMICNTDMethod"), &opt);
    }
    // TODA port
    for (unsigned int signal_id = 0; signal_id < 4; signal_id ++) {
        sc_core::sc_spawn_options opt;
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::HandleOutputPortTODAMethod, this, signal_id),
      ...
      for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::TriggerDCNTMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::AssertmIsCMAMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::UpdateDCNTDMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::AssertmIsCMBMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.set_sensitivity(&mTimer_DCNTD[ch_id]->mUnderflowEvent);
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::AssertmIsUNFMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::StartDcntMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::StartDCNTDMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::StopDcntMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::ClearDSR1D_DSFDMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::WriteTODBMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::ProcessMIGMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::WriteTODMIGMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.set_sensitivity(&mAssertTODB_normalEvent[ch_id]);
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::AssertTODB_normalMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.set_sensitivity(&mDeassertTODB_normalEvent[ch_id]);
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::DeassertTODB_normalMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::AssertINT_UDFMethod, this, ch_id),
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERD_SUB::DeassertINT_UDFMethod, this, ch_id),
...
///*********************************
/// Function     : ~ATU5010_TIMERD_SUB
/// Description  : Destructor of ATU5010_TIMERD_SUB class
///*********************************
ATU5010_TIMERD_SUB::~ATU5010_TIMERD_SUB (void)
{//{{{
    delete mATU5010_TIMERD_SUBFunc;
...
***********************************/
double ATU5010_TIMERD_SUB::CalculateClockEdge (const std::string clock_name, const bool is_pos, const bool add_period, const double time_point, const double time_change_point)
{//{{{
    double period_num = 0;
    if ( (clock_name == mCNT1CLKName) && ((double)mCNT1CLKPeriod != 0)) {
        period_num = (time_point - time_change_point) / (double)mCNT1CLKPeriod;
    } else if ((clock_name == mCNT2CLKName) && ((double)mCNT2CLKPeriod !=0)) {
        period_num = (time_point - time_change_point) / (double)mCNT2CLKPeriod;
    } else if ((clock_name == mDCNTCLKName) && ((double)mDCNTCLKPeriod !=0)) {
        period_num = (time_point - time_change_point) / (double)mDCNTCLKPeriod;
    } else if ((clock_name == mPCLKName) && ((double)mTIMERD->mPCLKPeriod!= 0)) {
...
***********************************/
void ATU5010_TIMERD_SUB::RunningCounter(const bool is_run)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if (is_run) {
            mCounterEnable = true;
            StartStopTimer(true);
        } else {
...
***********************************/
unsigned int ATU5010_TIMERD_SUB::GetCounterValue(unsigned int counter, unsigned int channel)
{//{{{
        unsigned int val = 0;
        if (counter == ATU5010_TIMERD_SUB_AgentController::emCounter1) {
            val = mTimer_TCNT1D_CUCR1D->GetCounterValue();
        } else if (counter == ATU5010_TIMERD_SUB_AgentController::emCounter2) {
            val = mTimer_TCNT2D_CUCR2D->GetCounterValue();
        } else if (counter == ATU5010_TIMERD_SUB_AgentController::emDownCounter) {
            val = mTimer_DCNTD[channel]->GetCounterValue();
        } else if (counter == ATU5010_TIMERD_SUB_AgentController::emOFMICNTDCounter) {
            val = mTimer_OFMICNTD[channel]->GetCounterValue();
        } else if (counter == ATU5010_TIMERD_SUB_AgentController::emONMICNTDCounter) {
            val = mTimer_ONMICNTD[channel]->GetCounterValue();
        } else if (counter == ATU5010_TIMERD_SUB_AgentController::emOTOMICNTDCounter) {
...
Description     :
***********************************/
void ATU5010_TIMERD_SUB::WriteCounterValueMethod(const unsigned int counter)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        double cur_timer = sc_time_stamp().to_double();
        if (mIsTimer1Running && (counter == ATU5010_TIMERD_SUB_AgentController::emCounter1)) {
        StopTimerFunction(ATU5010_TIMERD_SUB_AgentController::emCounter1,false);
        mStartTimer1Event.notify(CalculateClockEdge(mCNT1CLKName, true, false, cur_timer,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter1]),mTIMERD->mATU5010->mTimeResolutionUnit);
        } else if(mIsTimer2Running && (counter == ATU5010_TIMERD_SUB_AgentController::emCounter2)){
        StopTimerFunction(ATU5010_TIMERD_SUB_AgentController::emCounter2,false);
        mStartTimer2Event.notify(CalculateClockEdge(mCNT2CLKName, true, false , cur_timer,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter2]),mTIMERD->mATU5010->mTimeResolutionUnit);
        }
    }
}//}}}
/***********************************
...
Description     :
***********************************/
void ATU5010_TIMERD_SUB::WriteCompareMatchValueMethod(const unsigned int counter)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        double cur_timer = sc_time_stamp().to_double();
        if (mIsTimer1Running && (counter == ATU5010_TIMERD_SUB_AgentController::emCounter1)) {
        StopTimerFunction(ATU5010_TIMERD_SUB_AgentController::emCounter1,true);
        mStartTimer1Event.notify(CalculateClockEdge(mCNT1CLKName, true, true, cur_timer,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter1]),mTIMERD->mATU5010->mTimeResolutionUnit);
        } else if(mIsTimer2Running && (counter == ATU5010_TIMERD_SUB_AgentController::emCounter2)){
        StopTimerFunction(ATU5010_TIMERD_SUB_AgentController::emCounter2,true);
        mStartTimer2Event.notify(CalculateClockEdge(mCNT2CLKName, true, true, cur_timer,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter2]),mTIMERD->mATU5010->mTimeResolutionUnit);
        }
    }
}//}}}
/***********************************
...
***********************************/
void ATU5010_TIMERD_SUB::StartStopTimer (const bool is_start)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        double cur_timer = sc_time_stamp().to_double();
...
Description     : Call to start or stop counter
***********************************/
void ATU5010_TIMERD_SUB::StartTimer1Method (void)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        StartTimerFunction(ATU5010_TIMERD_SUB_AgentController::emCounter1);
   }
}//}}}
/***********************************
...
Description     : Call to start or stop counter
***********************************/
void ATU5010_TIMERD_SUB::StartTimer2Method (void)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        StartTimerFunction(ATU5010_TIMERD_SUB_AgentController::emCounter2);
   }
}//}}}

// measurement counter
void ATU5010_TIMERD_SUB::StartTimerOFMICNTDMethod (const unsigned int channel)
{//{{{
    #if 0
    ...
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        ...
        unsigned int SetCounterValue = 0;
            if (!mIsTimerOFMICNTDRunning[channel]) {
                ...
                SetCounterValue = mATU5010_TIMERD_SUBFunc->GetOFMINDRegValue(channel);
                mATU5010_TIMERD_SUBFunc->SetOFMICNTDRegValue(channel,SetCounterValue);

                // set count period
                mTimer_OFMICNTD[channel]->SetCounterMode(true);
                mTimer_OFMICNTD[channel]->SetCounterPeriod(mTIMERD->mPCLKPeriod);
...
void ATU5010_TIMERD_SUB::StartTimerONMICNTDMethod (const unsigned int channel)
{//{{{
    #if 0
    ...
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        ...
        unsigned int SetCounterValue = 0;
            if (!mIsTimerONMICNTDRunning[channel]) {
                ...
                SetCounterValue = mATU5010_TIMERD_SUBFunc->GetONMINDRegValue(channel);
                mATU5010_TIMERD_SUBFunc->SetONMICNTDRegValue(channel,SetCounterValue);

                // set count period
                mTimer_ONMICNTD[channel]->SetCounterMode(true);
                mTimer_ONMICNTD[channel]->SetCounterPeriod(mTIMERD->mPCLKPeriod);
...
void ATU5010_TIMERD_SUB::StartTimerOTOMICNTDMethod (const unsigned int channel)
{//{{{
    #if 0
    ...
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        ...
        unsigned int SetCounterValue = 0;
            if (!mIsTimerOTONMICNTDRunning[channel]) {
                ...
                SetCounterValue = mATU5010_TIMERD_SUBFunc->GetOTOMINDRegValue(channel);
                mATU5010_TIMERD_SUBFunc->SetOTONMICNTDRegValue(channel,SetCounterValue);

                // set count period
                mTimer_OTONMICNTD[channel]->SetCounterMode(true);
                mTimer_OTONMICNTD[channel]->SetCounterPeriod(mTIMERD->mPCLKPeriod);
...
void ATU5010_TIMERD_SUB::StopTimerOFMICNTDMethod (const unsigned int channel)
{//{{{
    #if 0
    ...
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        ...
        unsigned int SetCounterValue = 0;
            if (mIsTimerOFMICNTDRunning[channel]) {
                ...
                mATU5010_TIMERD_SUBFunc->SetOFMICNTDRegValue(channel,0x0);
                mTimer_OFMICNTD[channel]->StartStop(mIsTimerOFMICNTDRunning[channel]);
                mOFMIZeroEventFunc(channel);
            }
        }
}//}}}
void ATU5010_TIMERD_SUB::StopTimerONMICNTDMethod (const unsigned int channel)
{//{{{
    #if 0
    ...
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        ...
        unsigned int SetCounterValue = 0;
            if (mIsTimerONMICNTDRunning[channel]) {
                ...
                mATU5010_TIMERD_SUBFunc->SetONMICNTDRegValue(channel,0x0);
                mTimer_ONMICNTD[channel]->StartStop(mIsTimerONMICNTDRunning[channel]);
                mONMIZeroEventFunc(channel);
            }
        }
}//}}}
void ATU5010_TIMERD_SUB::StopTimerOTOMICNTDMethod (const unsigned int channel)
{//{{{
    #if 0
    ...
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        ...
        unsigned int SetCounterValue = 0;
            if (mIsTimerOTONMICNTDRunning[channel]) {
                ...
                mATU5010_TIMERD_SUBFunc->SetOTONMICNTDRegValue(channel,0x0);
                mTimer_OTONMICNTD[channel]->StartStop(mIsTimerOTONMICNTDRunning[channel]);
                mOTONMIZeroEventFunc(channel);
            }
        }
}//}}}
void ATU5010_TIMERD_SUB::WriteTimerOFMICNTDMethod (const unsigned int channel)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        double cur_timer = sc_time_stamp().to_double();
        unsigned int SetCounterValue = 0;
            if (mIsTimerOFMICNTDRunning[channel]) {
                ...
                mTimer_OFMICNTD[channel]->StartStop(mIsTimerOFMICNTDRunning[channel]);
                mIsTimerOFMICNTDRunning[channel] = true;
                SetCounterValue = mATU5010_TIMERD_SUBFunc->GetOFMICNTDRegValue(channel);
                #if 0
                    re_printf("info", "Write OFMICNTD channel %d: 0x%08x\n", channel, SetCounterValue);
                #endif
...
void ATU5010_TIMERD_SUB::WriteTimerONMICNTDMethod (const unsigned int channel)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        double cur_timer = sc_time_stamp().to_double();
        unsigned int SetCounterValue = 0;
            if (mIsTimerONMICNTDRunning[channel]) {
                ...
                mTimer_ONMICNTD[channel]->StartStop(mIsTimerONMICNTDRunning[channel]);
                mIsTimerONMICNTDRunning[channel] = true ;
                SetCounterValue = mATU5010_TIMERD_SUBFunc->GetONMICNTDRegValue(channel);
                #if 0
                    re_printf("info", "Write ONMICNTD channel %d: 0x%08x\n", channel, SetCounterValue);
                #endif
...
void ATU5010_TIMERD_SUB::WriteTimerOTOMICNTDMethod (const unsigned int channel)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        double cur_timer = sc_time_stamp().to_double();
        unsigned int SetCounterValue = 0;
            if (mIsTimerOTONMICNTDRunning[channel]) {
                ...
                mTimer_OTONMICNTD[channel]->StartStop(mIsTimerOTONMICNTDRunning[channel]);
                mIsTimerOTONMICNTDRunning[channel] = true ;
                SetCounterValue = mATU5010_TIMERD_SUBFunc->GetOTONMICNTDRegValue(channel);
                #if 0
                    re_printf("info", "Write OTOMICNTD channel %d: 0x%08x\n", channel, SetCounterValue);
                #endif
...
// Return value : None
**********************************/
void ATU5010_TIMERD_SUB::StartTimerFunction (unsigned int counter)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        double cur_timer = sc_time_stamp().to_double();
        ...
        if (counter == ATU5010_TIMERD_SUB_AgentController::emCounter1) {
            ...
            if(mOverflowTCNT1D){
                ...
                mATU5010_TIMERD_SUBFunc->SetTCNT1DRegVal(0x0);
                SetCounterValue = mATU5010_TIMERD_SUBFunc->GetTCNT1DRegValue();
            } else if(mCLRCounter1) {
                if (mCLRSEL1DVal == true) {
                    mATU5010_TIMERD_SUBFunc->SetTCNT1DRegVal(0x0);
                } else {
                    mATU5010_TIMERD_SUBFunc->SetTCNT1DRegVal(0x1);
                }
                SetCounterValue = mATU5010_TIMERD_SUBFunc->GetTCNT1DRegValue();
            } else {
                SetCounterValue = mATU5010_TIMERD_SUBFunc->GetTCNT1DRegValue() + 1;
            }
            mCLRCounter1 = false;
            mOverflowTCNT1D = false;
                // set count period
                mTimer_TCNT1D_CUCR1D->SetCounterMode(true);
                mTimer_TCNT1D_CUCR1D->SetCounterPeriod(mCNT1CLKPeriod);
                ...
                for (unsigned int channel = 0; channel < 4; channel ++) {
                    ...
                    mTimer_TCNT1D_OCR1D[channel]->SetCounterPeriod(mCNT1CLKPeriod);
                    ...
                    mTimer_TCNT1D_OCR2D[channel]->SetCounterPeriod(mCNT1CLKPeriod);
                ...
                if(SetCounterValue == mCUCR1DValue) {
                    mCheckCompareMatchCUCR1DEvent.notify(CalculateClockEdge(mPCLKName, false, false, cur_timer,mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emPCLKClockIndex]),mTIMERD->mATU5010->mTimeResolutionUnit);
                }
                for (unsigned int channel = 0; channel < 4; channel ++) {
                    if (mCMPSEL1DVal[channel]==false){ //0: A-> TCNT1; 1:A->TCNT2
                        ...
                        if(SetCounterValue == mOCR1DValue[channel]) {
                            mCmpMatchCNT1OCR1DEvent[channel].notify(mTIMERD->mPCLKPeriod, mTIMERD->mATU5010->mTimeResolutionUnit);
                        }
                    }
                    if (mCMPSEL2DVal[channel]==true){ //0: B-> TCNT2; 1:A->TCNT1
                        ...
                        if(SetCounterValue == mOCR2DValue[channel]) {
                            mCmpMatchCNT1OCR2DEvent[channel].notify(mTIMERD->mPCLKPeriod, mTIMERD->mATU5010->mTimeResolutionUnit);
                        }
                    }
                }
            }
            ...
            if (mOverflowTCNT2D || mCLRBTCNT2D) {
                ...
                mATU5010_TIMERD_SUBFunc->SetTCNT2DRegVal(0x0);
                SetCounterValue = mATU5010_TIMERD_SUBFunc->GetTCNT2DRegValue();
            } else if(mCLRCounter2) {
                if (mCLRSEL2DVal == true) {
                    mATU5010_TIMERD_SUBFunc->SetTCNT2DRegVal(0x0);
                } else {
                    mATU5010_TIMERD_SUBFunc->SetTCNT2DRegVal(0x1);
                }
                SetCounterValue = mATU5010_TIMERD_SUBFunc->GetTCNT2DRegValue();
            } else {
                SetCounterValue = mATU5010_TIMERD_SUBFunc->GetTCNT2DRegValue() + 1;
            }
            mCLRCounter2 = false;
            mOverflowTCNT2D = false;
                // set count period
                ...
                mTimer_TCNT2D_CUCR2D->SetCounterPeriod(mCNT2CLKPeriod);
                ...
                for (unsigned int channel = 0; channel < 4; channel ++) {
                    ...
                    mTimer_TCNT2D_OCR1D[channel]->SetCounterPeriod(mCNT2CLKPeriod);
                    ...
                    mTimer_TCNT2D_OCR2D[channel]->SetCounterPeriod(mCNT2CLKPeriod);
                ...
                if(SetCounterValue == mCUCR2DValue) {
                   mCheckCompareMatchCUCR2DEvent.notify(CalculateClockEdge(mPCLKName, false, false, cur_timer,mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emPCLKClockIndex]),mTIMERD->mATU5010->mTimeResolutionUnit);
                }
                for (unsigned int channel = 0; channel < 4; channel ++) {
                    if (mCMPSEL1DVal[channel]==true){ //0: A-> TCNT1; 1:A->TCNT2
                        mTimer_TCNT2D_OCR1D[channel]->StartStop(mIsTimer2Running);
                        if(SetCounterValue == mOCR1DValue[channel]) {
                            mCmpMatchCNT2OCR1DEvent[channel].notify(mTIMERD->mPCLKPeriod, mTIMERD->mATU5010->mTimeResolutionUnit);
                        }
                    }
                    if (mCMPSEL2DVal[channel]==false){ //0: B-> TCNT2; 1:A->TCNT1
                        mTimer_TCNT2D_OCR2D[channel]->StartStop(mIsTimer2Running);
                        if(SetCounterValue == mOCR2DValue[channel]) {
                            mCmpMatchCNT2OCR2DEvent[channel].notify(mTIMERD->mPCLKPeriod, mTIMERD->mATU5010->mTimeResolutionUnit);
                        }
                    }
                }
            }
...
// Return value : None
**********************************/
void ATU5010_TIMERD_SUB::StopTimerFunction (unsigned int counter, bool reg_update)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if (counter == ATU5010_TIMERD_SUB_AgentController::emCounter1) {
            if (mIsTimer1Running) {
                ...
                // get counter value
                if(reg_update) {
                     mATU5010_TIMERD_SUBFunc->SetTCNT1DRegVal(mTimer_TCNT1D_CUCR1D->GetCounterValue());
                }
                // stop counter
                mTimer_TCNT1D_CUCR1D->StartStop(mIsTimer1Running);
                for (unsigned int channel = 0; channel < 4; channel ++) {
            ...
            if (mIsTimer2Running) {
                ...
                // get counter value
                if(reg_update) {
                    mATU5010_TIMERD_SUBFunc->SetTCNT2DRegVal(mTimer_TCNT2D_CUCR2D->GetCounterValue());
                }
                // stop counter
                mTimer_TCNT2D_CUCR2D->StartStop(mIsTimer2Running);
                for (unsigned int channel = 0; channel < 4; channel ++) {
...
void ATU5010_TIMERD_SUB::KeepCUCR1DValueOvfMethod(void)
{//{{{
    if ((mCUCR1DOvfTime == sc_time_stamp().to_double()) && (mIsCUCR1DOvf)) {
        ...
        (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
            ...
            mATU5010_TIMERD_SUBFunc->SetTCNT1DRegVal(0xFFFFFFFF);
            mCompareMatchCUCR1DEvent.notify(mCNT1CLKPeriod, mTIMERD->mATU5010->mTimeResolutionUnit);
        }
    }
}//}}}
void ATU5010_TIMERD_SUB::KeepCUCR2DValueOvfMethod(void)
{//{{{
    if ((mCUCR2DOvfTime == sc_time_stamp().to_double()) && (mIsCUCR2DOvf)) {
        ...
        (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
            ...
            mATU5010_TIMERD_SUBFunc->SetTCNT2DRegVal(0xFFFFFFFF);
            mCompareMatchCUCR2DEvent.notify(mCNT2CLKPeriod, mTIMERD->mATU5010->mTimeResolutionUnit);
        }
    }
}//}}}
void ATU5010_TIMERD_SUB::PreHandleCmpMatchCUCR1DMethod(void)
{//{{{
...
void ATU5010_TIMERD_SUB::Pre2HandleCmpMatchCUCR1DMethod(void)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if(mOverflowTCNT1D){
        ...
        mCUCR1DMatch = true;
        double cur_timer = sc_time_stamp().to_double();
        mCompareMatchCUCR1DEvent.notify(CalculateClockEdge(mPCLKName, true, false, cur_timer,mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emPCLKClockIndex]),mTIMERD->mATU5010->mTimeResolutionUnit);
    }
}//}}}
void ATU5010_TIMERD_SUB::PreHandleCmpMatchCUCR2DMethod(void)
{//{{{
...
void ATU5010_TIMERD_SUB::Pre2HandleCmpMatchCUCR2DMethod(void)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if(mOverflowTCNT2D){
        ...
        mCUCR2DMatch = true;
        double cur_timer = sc_time_stamp().to_double();
        mCompareMatchCUCR2DEvent.notify(CalculateClockEdge(mPCLKName, true, false, cur_timer,mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emPCLKClockIndex]),mTIMERD->mATU5010->mTimeResolutionUnit);
    }
}//}}}

void ATU5010_TIMERD_SUB::PreHandleCmpMatchCNT1OCR1DMethod (const unsigned int channel)
{//{{{
...
void ATU5010_TIMERD_SUB::PreHandleCmpMatchCNT2OCR1DMethod (const unsigned int channel)
{//{{{
...
void ATU5010_TIMERD_SUB::PreHandleCmpMatchCNT1OCR2DMethod (const unsigned int channel)
{//{{{
...
void ATU5010_TIMERD_SUB::PreHandleCmpMatchCNT2OCR2DMethod (const unsigned int channel)
{//{{{
...
// Main method
void ATU5010_TIMERD_SUB::HandleCmpMatchCUCR1DMethod (void)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        double cur_timer = sc_time_stamp().to_double();
        if(mOverflowTCNT1D || mCUCR1DMatch){
            mCUCR1DMatch = false;
            mATU5010_TIMERD_SUBFunc->TurnOVF1Flag();
...
void ATU5010_TIMERD_SUB::HandleCmpMatchCUCR2DMethod (void)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        double cur_timer = sc_time_stamp().to_double();
        if(mOverflowTCNT2D || mCUCR2DMatch){
            mATU5010_TIMERD_SUBFunc->TurnOVF2Flag();
...
void ATU5010_TIMERD_SUB::HandleCmpMatchCNT1OCR1DMethod (const unsigned int channel)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
...
void ATU5010_TIMERD_SUB::HandleCmpMatchCNT2OCR1DMethod (const unsigned int channel)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
...
void ATU5010_TIMERD_SUB::HandleCmpMatchCNT1OCR2DMethod (const unsigned int channel)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if (mCompareMatchCNT1Enable[channel]) {
...
void ATU5010_TIMERD_SUB::HandleCmpMatchCNT2OCR2DMethod (const unsigned int channel)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if (mCompareMatchCNT2Enable[channel]) {
...
void ATU5010_TIMERD_SUB::HandleCmpMatchCMPB (unsigned int channel)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        mCompareMatchBEvent[channel].notify(); //down counter
        mATU5010_TIMERD_SUBFunc->TurnCMFBFlag(channel);
        mATU5010_TIMERD_SUBFunc->SetICR1DVal(channel,mTimer_TCNT1D_CUCR1D->GetCounterValue());
...
void ATU5010_TIMERD_SUB::HandleCmpMatchCMPA (unsigned int channel)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        mCompareMatchAEvent[channel].notify();//down counter
        mATU5010_TIMERD_SUBFunc->TurnCMFAFlag(channel);
        mATU5010_TIMERD_SUBFunc->SetICR2DVal(channel,mTimer_TCNT2D_CUCR2D->GetCounterValue());
...
void ATU5010_TIMERD_SUB::HandleOutputPortTODAMethod (const unsigned int channel)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
...
void ATU5010_TIMERD_SUB::ChangeOutputPortTODA (const unsigned int channel, unsigned int port_val)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        ...
        if(is_write && !mATU5010_TIMERD_SUBFunc->Get_MIGSELD_MIGSEL(channel)) {
...
// Return value : None
**********************************/
void ATU5010_TIMERD_SUB::HandleInputCaptureMethod (void)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        mTIDSELDVal =  mATU5010_TIMERD_SUBFunc->GetTIDSELDVal();
        ...
        if(input_val){ //capture
            ...
            mInputCaptureEvent.notify(mTIMERD->mPCLKPeriod, mTIMERD->mATU5010->mTimeResolutionUnit);
        }
    }
}//}}}
/*********************************
...
// Return value : None
**********************************/
void ATU5010_TIMERD_SUB::InputCaptureWriteMethod (void)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        mATU5010_TIMERD_SUBFunc->SetOSBRDRegVal(mCaptureValue);
    }
}//}}}
...
// Return value : None
**********************************/
void  ATU5010_TIMERD_SUB::CheckRangeCompareFunction1(unsigned int counter, unsigned int channel)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if (counter == ATU5010_TIMERD_SUB_AgentController::emCounter1) {
            if ((mOCR1DValue[channel] + (double)pow((double)2,(double)mRC1DVal) - 1) > 0xFFFFFFFF) {
                if ((mTimer_TCNT1D_CUCR1D->GetCounterValue()) <= ((mOCR1DValue[channel] + (double)pow((double)2,(double)mRC1DVal) - 1) - 0xFFFFFFFF)) {
                // compare match A
                    mCompareMatchCNT1OCR1[channel] = true;
                    HandleCmpMatchCMPA(channel);
                    mATU5010_TIMERD_SUBFunc->SetOCR1DVal(channel,mTimer_TCNT1D_CUCR1D->GetCounterValue());
            ...
            if ((mOCR1DValue[channel] + (double)pow((double)2,(double)mRC1DVal) - 1 ) > 0xFFFFFFFF) {
                if ((mTimer_TCNT2D_CUCR2D->GetCounterValue()) <= ((mOCR1DValue[channel] + (double)pow((double)2,(double)mRC1DVal) - 1) - 0xFFFFFFFF)) {
                // compare match B
                    mCompareMatchCNT2OCR1[channel] = true;
                    HandleCmpMatchCMPA(channel);
                    mATU5010_TIMERD_SUBFunc->SetOCR1DVal(channel,mTimer_TCNT2D_CUCR2D->GetCounterValue());
                }
            } else {
            if (((mTimer_TCNT2D_CUCR2D->GetCounterValue()) <= (mOCR1DValue[channel] + (double)pow((double)2,(double)mRC1DVal) - 1)) &&
...
// Return value : None
**********************************/
void  ATU5010_TIMERD_SUB::CheckRangeCompareFunction2(unsigned int counter, unsigned int channel)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if (counter == ATU5010_TIMERD_SUB_AgentController::emCounter1) {
            if ((mOCR2DValue[channel] + (double)pow((double)2,(double)mRC2DVal) - 1) > 0xFFFFFFFF) {
                if ((mTimer_TCNT1D_CUCR1D->GetCounterValue()) <= ((mOCR2DValue[channel] + (double)pow((double)2,(double)mRC2DVal) - 1) - 0xFFFFFFFF)) {
                // compare match A
                    mCompareMatchCNT1OCR2[channel] = true;
                    HandleCmpMatchCMPB(channel);
                    mATU5010_TIMERD_SUBFunc->SetOCR2DVal(channel,mTimer_TCNT1D_CUCR1D->GetCounterValue());

                }
            } else {
            if (((mTimer_TCNT1D_CUCR1D->GetCounterValue()) <= (mOCR2DValue[channel] + (double)pow((double)2,(double)mRC2DVal) - 1)) &&
            ...
            if ((mOCR2DValue[channel] + (double)pow((double)2,(double)mRC2DVal) - 1 ) > 0xFFFFFFFF) {
                if ((mTimer_TCNT2D_CUCR2D->GetCounterValue()) <= ((mOCR2DValue[channel] + (double)pow((double)2,(double)mRC2DVal) - 1) - 0xFFFFFFFF)) {
                // compare match B
                    mCompareMatchCNT2OCR2[channel] = true;
                    HandleCmpMatchCMPB(channel);
                    mATU5010_TIMERD_SUBFunc->SetOCR2DVal(channel,mTimer_TCNT2D_CUCR2D->GetCounterValue());
                }
            } else {
...
// Return value : None
**********************************/
void ATU5010_TIMERD_SUB::HandleInputClearCounterMethod (void)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
...
// Return value : None
**********************************/
void ATU5010_TIMERD_SUB::ClockHandleFunction (void)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif"))){
        ...
        switch(mCKSEL1DVal){
         case 0:
           mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter1] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS0ClockIndex];
           PreCNT1CLKPeriod = mTIMERD->mCLKBUS0Period;
           break;
         case 1:
           mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter1] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS1ClockIndex];
           PreCNT1CLKPeriod = mTIMERD->mCLKBUS1Period;
           break;
         case 2:
           mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter1] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS2ClockIndex];
           PreCNT1CLKPeriod = mTIMERD->mCLKBUS2Period;
           break;
         case 3:
           mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter1] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS3ClockIndex];
           PreCNT1CLKPeriod = mTIMERD->mCLKBUS3Period;
           break;
         case 4:
           mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter1] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS4ClockIndex];
           PreCNT1CLKPeriod = mTIMERD->mCLKBUS4Period;
           break;
         case 5:
           mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter1] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS5ClockIndex];
        ...
        switch(mCKSEL2DVal){
          case 0:
            mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter2] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS0ClockIndex];
            PreCNT2CLKPeriod = mTIMERD->mCLKBUS0Period;
            break;
          case 1:
            mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter2] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS1ClockIndex];
            PreCNT2CLKPeriod = mTIMERD->mCLKBUS1Period;
            break;
          case 2:
            mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter2] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS2ClockIndex];
            PreCNT2CLKPeriod = mTIMERD->mCLKBUS2Period;
            break;
          case 3:
            mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter2] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS3ClockIndex];
            PreCNT2CLKPeriod = mTIMERD->mCLKBUS3Period;
            break;
          case 4:
            mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter2] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS4ClockIndex];
            PreCNT2CLKPeriod = mTIMERD->mCLKBUS4Period;
            break;
          case 5:
            mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emCounter2] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS5ClockIndex];
        ...
        switch(mDCSELDVal){
          case 0:
            mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS0ClockIndex];
            PreDCNTCLKPeriod = mTIMERD->mCLKBUS0Period;
            break;
          case 1:
            mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS1ClockIndex];
            PreDCNTCLKPeriod = mTIMERD->mCLKBUS1Period;
            break;
          case 2:
            mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS2ClockIndex];
            PreDCNTCLKPeriod = mTIMERD->mCLKBUS2Period;
            break;
          case 3:
            mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS3ClockIndex];
            PreDCNTCLKPeriod = mTIMERD->mCLKBUS3Period;
            break;
          case 4:
            mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS4ClockIndex];
            PreDCNTCLKPeriod = mTIMERD->mCLKBUS4Period;
            break;
          case 5:
            mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter] = mTIMERD->mInputClockStartPoint[ATU5010_TIMERD_AgentController::emCLKBUS5ClockIndex];
        ...
        if(PreCNT1CLKPeriod==0){ //zero clock condition
            ...
            if (mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emCounter1]){
                mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emCounter1] = false;
            }
        } else if (mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emCounter1]){//count clock selection change
            mCNT1CLKPeriod = PreCNT1CLKPeriod;
            mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emCounter1] = false;
            if (mIsTimer1Running) {
                StopTimerFunction(ATU5010_TIMERD_SUB_AgentController::emCounter1,true);
       ...
       if(PreCNT2CLKPeriod==0){ //zero clock condition
            ...
            if (mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emCounter2]){
                mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emCounter2] = false;
            }
        } else if (mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emCounter2]){//count clock selection change
            mCNT2CLKPeriod = PreCNT2CLKPeriod;
            mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emCounter2] = false;
            if (mIsTimer2Running) {
                StopTimerFunction(ATU5010_TIMERD_SUB_AgentController::emCounter2,true);
       ...
       if(PreDCNTCLKPeriod==0){ //zero clock condition
            ...
            for(unsigned int ch_id=0;ch_id <=3; ch_id++) {
                if(mIsDcntRunning[ch_id]){
                    mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emDownCounter] = true;
                    StopDcntMethod(ch_id);
                    mIsDcntWasRunning[ch_id] = true;
                }
            }
            if (mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emDownCounter]){
                mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emDownCounter] = false;
            }
        } else if (mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emDownCounter]){//count clock selection change
            mDCNTCLKPeriod = PreDCNTCLKPeriod;
            for(unsigned int ch_id=0;ch_id <=3; ch_id++) {
                if(mIsDcntRunning[ch_id]){
                    mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emDownCounter] = true;
                    StopDcntMethod(ch_id);
                    mStartDcntEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,true, cur_timer,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
                }
            }
            mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emDownCounter] = false;
            ...
            for(unsigned int ch_id=0;ch_id <=3; ch_id++) {
                if(mIsDcntRunning[ch_id]){
                    ...
                    mStartDcntEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,false, cur_timer,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
...
***********************************/
void ATU5010_TIMERD_SUB::EnableReset(const bool is_active)
{//{{{
    mATU5010_TIMERD_SUBFunc->EnableReset(is_active);
...
///*********************************
void ATU5010_TIMERD_SUB::WriteTODBMethod (const unsigned int ch_id)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if(mATU5010_TIMERD_SUBFunc->Get_OSELRD_OSELBD(ch_id)) {
            TODB[ch_id]->write(mATU5010_TIMERD_SUBFunc->Get_ODRD_ODBD(ch_id));
        }
        else {
            if(mATU5010_TIMERD_SUBFunc->Get_TOCRD_TONEBD(ch_id) || mATU5010_TIMERD_SUBFunc->Get_TOCCRD_TONEBD(ch_id)) {
                    TODB[ch_id]->write(!mTODB_normal[ch_id]);
                    if(mATU5010_TIMERD_SUBFunc->Get_MIGSELD_MIGSEL(ch_id)) {
...
///*********************************
void ATU5010_TIMERD_SUB::ProcessMIGMethod (const unsigned int ch_id)
{//{{{
    if (!mATU5010_TIMERD_SUBFunc->Get_MIGCRD_MIGEND(ch_id)) { // not enable MIN GUARD
...
///*********************************
void ATU5010_TIMERD_SUB::WriteTODMIGMethod (const unsigned int ch_id)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if(mATU5010_TIMERD_SUBFunc->Get_OSELRD_OSELBD(ch_id)) {
            if(!mATU5010_TIMERD_SUBFunc->Get_MIGSELD_MIGSEL(ch_id)) {
                ...
                if(mOSELADValue[ch_id]) {
                    if(mInterruptREIEDEnable[ch_id]){
                        mInterruptOutputREIEDEvent[ch_id].notify(SC_ZERO_TIME);
                    }
                    mATU5010_TIMERD_SUBFunc->Set_TSR2D_REFD(ch_id, mOSELADValue[ch_id]);
                }
                else {
                    if(mInterruptFEIEDEnable[ch_id]){
                        mInterruptOutputFEIEDEvent[ch_id].notify(SC_ZERO_TIME);
                    }
                    mATU5010_TIMERD_SUBFunc->Set_TSR2D_FEFD(ch_id, mOSELADValue[ch_id]);
                }
            } else {
                TODMIG[ch_id]->write(mATU5010_TIMERD_SUBFunc->Get_ODRD_ODBD(ch_id));
                if(mATU5010_TIMERD_SUBFunc->Get_ODRD_ODBD(ch_id)) {
                    if(mInterruptREIEDEnable[ch_id]){
                        mInterruptOutputREIEDEvent[ch_id].notify(SC_ZERO_TIME);
                    }
                    mATU5010_TIMERD_SUBFunc->Set_TSR2D_REFD(ch_id, mATU5010_TIMERD_SUBFunc->Get_ODRD_ODBD(ch_id));
                }
                else {
                    if(mInterruptFEIEDEnable[ch_id]){
                        mInterruptOutputFEIEDEvent[ch_id].notify(SC_ZERO_TIME);
                    }
                    mATU5010_TIMERD_SUBFunc->Set_TSR2D_FEFD(ch_id, mATU5010_TIMERD_SUBFunc->Get_ODRD_ODBD(ch_id));
                }
            }
        } else {
            TODMIG[ch_id]->write(mTODMIGValue[ch_id]);
            if (mATU5010_TIMERD_SUBFunc->Get_MIGCRD_MIGEND(ch_id)) { // enable MIN GUARD
                if (mTODMIGValue[ch_id]) { // Assert
                    //start counters MIGs
                    mStartONMICNTDEvent[ch_id].notify(SC_ZERO_TIME);
                    mStartOTOMICNTDEvent[ch_id].notify(SC_ZERO_TIME);
                    //capture
                    mATU5010_TIMERD_SUBFunc->Set_CAPD("ONCAPD1", ch_id);
                    mATU5010_TIMERD_SUBFunc->Set_CAPD("ONCAPD2", ch_id);
                    //interrupt
                    if(mInterruptREIEDEnable[ch_id]){
                        mInterruptOutputREIEDEvent[ch_id].notify(SC_ZERO_TIME);
                    }
                    mATU5010_TIMERD_SUBFunc->Set_TSR2D_REFD(ch_id, mTODMIGValue[ch_id]);
                } else { // Deassert
                    //start counters MIGs
                    mStartOFMICNTDEvent[ch_id].notify(SC_ZERO_TIME);
                    //capture
                    mATU5010_TIMERD_SUBFunc->Set_CAPD("OFCAPD1", ch_id);
                    mATU5010_TIMERD_SUBFunc->Set_CAPD("OFCAPD2", ch_id);
                    //interrupt
                    if(mInterruptFEIEDEnable[ch_id]){
                        mInterruptOutputFEIEDEvent[ch_id].notify(SC_ZERO_TIME);
                    }
                    mATU5010_TIMERD_SUBFunc->Set_TSR2D_FEFD(ch_id, mTODMIGValue[ch_id]);
                }
            }
        }
    }
...
///*********************************
void ATU5010_TIMERD_SUB::DeassertTODB_normalMethod (const unsigned int ch_id)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if(!mATU5010_TIMERD_SUBFunc->Get_MIGCRD_MIGEND(ch_id)) { //normal
            if(mInterruptFEIEDEnable[ch_id]){
                mInterruptOutputFEIEDEvent[ch_id].notify(SC_ZERO_TIME);
            }
        }
...
///*********************************
void ATU5010_TIMERD_SUB::AssertTODB_normalMethod (const unsigned int ch_id)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if(!mATU5010_TIMERD_SUBFunc->Get_MIGCRD_MIGEND(ch_id)) { //normal
...
///*********************************
void ATU5010_TIMERD_SUB::Initialize (void)
{//{{{
   ...
   for (unsigned int index = 0; index <= 3; index++) {
    ...
    for(unsigned int ch_id=0;ch_id<ATU5010_TIMERD_SUB_AgentController::emChannel;ch_id++) {
        ...
        mIsDCNTDRegStart[ch_id] = false;
    }
}//}}}

void ATU5010_TIMERD_SUB::InitializePort (void)
{//{{{
...
void ATU5010_TIMERD_SUB::CancelAllEvents (void)
{//{{{
    ...
    for (unsigned int index = 0; index < ATU5010_TIMERD_SUB_AgentController::emChannel; index++) {
        ...
        mWriteOFMICNTDEvent[index].cancel();
        mWriteONMICNTDEvent[index].cancel();
        mWriteOTOMICNTDEvent[index].cancel();
     ...
     for (unsigned int index = 0; index < ATU5010_TIMERD_SUB_AgentController::emDownCounter; index++) {
...
///*********************************
void ATU5010_TIMERD_SUB::StartDCNTDReg (const unsigned int ch_id)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        double current_time = sc_time_stamp().to_double();
        mIsDCNTDRegStart[ch_id] = true;
...
/// Description  : Start DCNTD
///*********************************
void ATU5010_TIMERD_SUB::ClearDSR1D_DSFDMethod (const unsigned int ch_id)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        mATU5010_TIMERD_SUBFunc->Set_DSR1D_DSFD(ch_id,0);
...
///*********************************
void ATU5010_TIMERD_SUB::UpdateDCNTDMethod (const unsigned int ch_id)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        ...
        unsigned int counter_value = mATU5010_TIMERD_SUBFunc->Get_DCNTD(ch_id) - 1;
        unsigned int cm_value = 0xFFFFFFFF;
        double       period = mDCNTCLKPeriod;
        //setup timer
        ...
        //start timer
        double cur_timer = sc_time_stamp().to_double();
        mStartDCNTDEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,false, cur_timer,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
        /////
    }
}//}}}
...
/// Description  : Start DCNTD
///*********************************
void ATU5010_TIMERD_SUB::StartDCNTDMethod (const unsigned int ch_id)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if(mIsDcntinUdfPeriod[ch_id]){
...
/// Description  : Start DCNTD
///*********************************
void ATU5010_TIMERD_SUB::StartDcntMethod (const unsigned int ch_id)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        mIsDcntRunning[ch_id] = true;
...
///*********************************
void ATU5010_TIMERD_SUB::StopDcntMethod (const unsigned int ch_id)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        mIsDcntRunning[ch_id] = false;
        ...
        /////
        if(mChangeCountClock[ATU5010_TIMERD_SUB_AgentController::emDownCounter]){
            mATU5010_TIMERD_SUBFunc->Set_DCNTD(ch_id,GetCounterValue(ATU5010_TIMERD_SUB_AgentController::emDownCounter,ch_id));
        }
        else {
            mATU5010_TIMERD_SUBFunc->Set_DCNTD(ch_id,0);
        }
        ...
        if(mIsDcntinUdfPeriod[ch_id]) {
            mATU5010_TIMERD_SUBFunc->Set_TSRD_UDFD(ch_id,1);
...
///*********************************
void ATU5010_TIMERD_SUB::TriggerDCNTMethod (const unsigned int ch_id)
{//{{{
    mTimer_DCNTD[ch_id]->ClearBitUNF();
    double current_time = sc_time_stamp().to_double();
    if (IsTimerRunning && !(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        if(!mIsDcntRunning[ch_id]) {
            if (mIsDCNTDRegStart[ch_id]) {
                mATU5010_TIMERD_SUBFunc->Set_DSR1D_DSFD(ch_id,1);
                if(mATU5010_TIMERD_SUBFunc->Get_DCNTD(ch_id)!=0) {
                    mStartDcntEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,true, current_time,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
                }
                else {
                    mClearDSR1D_DSFDEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,true, current_time,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
                }
                mIsCMA[ch_id] = false;
                mIsCMB[ch_id] = false;
                mIsDCNTDRegStart[ch_id] = false;
                ...
                switch(mATU5010_TIMERD_SUBFunc->GetDcntTrigger(ch_id)) {
                    case 2:
                        if(mIsCMA[ch_id]) {
                            mATU5010_TIMERD_SUBFunc->Set_DSR1D_DSFD(ch_id,1);
                            mIsCMA[ch_id] = false;
                            if(mATU5010_TIMERD_SUBFunc->Get_DCNTD(ch_id)!=0) {
                                mStartDcntEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,true, current_time,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
                                mATU5010_TIMERD_SUBFunc->Set_DSR2D_DWRFD(ch_id,1);
                            }
                            else {
                                mClearDSR1D_DSFDEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,true, current_time,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
                            }
                    ...
                    case 4:
                        if(mIsCMB[ch_id]) {
                            ...
                            if(mATU5010_TIMERD_SUBFunc->Get_DCNTD(ch_id)!=0) {
                                mStartDcntEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,true, current_time,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
                                mATU5010_TIMERD_SUBFunc->Set_DSR2D_DWRFD(ch_id,2);
                            }
                            else {
                                mClearDSR1D_DSFDEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,true, current_time,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
                            }
                    ...
                    case 6:
                        if(mIsCMA[ch_id] && mIsCMB[ch_id]) {
                            ...
                            mIsCMA[ch_id] = false;
                            mIsCMB[ch_id] = false;
                            mATU5010_TIMERD_SUBFunc->Set_DSR1D_DWFD(ch_id,0);
                            if(mATU5010_TIMERD_SUBFunc->Get_DCNTD(ch_id)!=0) {
                                mStartDcntEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,true, current_time,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
                                mATU5010_TIMERD_SUBFunc->Set_DSR2D_DWRFD(ch_id,3);
                            }
                            else {
                                mClearDSR1D_DSFDEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,true, current_time,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
                            }
                            ...
                            mATU5010_TIMERD_SUBFunc->Set_DSR1D_DWFD(ch_id,1);
                        }
                        break;
                    case 10:
                    case 11:
                        if(mIsCMA[ch_id] || mIsCMB[ch_id]) {
                            ...
                            if(mATU5010_TIMERD_SUBFunc->Get_DCNTD(ch_id)!=0) {
                                mStartDcntEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,true, current_time,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
                                mATU5010_TIMERD_SUBFunc->Set_DSR2D_DWRFD(ch_id, (mIsCMB[ch_id] << 1) + mIsCMA[ch_id]);
                            }
                            else {
                                mClearDSR1D_DSFDEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,true, current_time,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
                            }
            ...
            if(mIsUNF[ch_id]) {
                ...
                mTimer_DCNTD[ch_id]->StartStop(false);
                mATU5010_TIMERD_SUBFunc->Set_DCNTD(ch_id,0);
                mStopDcntEvent[ch_id].notify(CalculateClockEdge(mDCNTCLKName, true,true, current_time,mCountClockStartPoint[ATU5010_TIMERD_SUB_AgentController::emDownCounter]),mTIMERD->mATU5010->mTimeResolutionUnit);
                mIsUNF[ch_id] = false;
            }
            else {
                switch(mATU5010_TIMERD_SUBFunc->GetDcntTrigger(ch_id)) {
...
///*********************************
void ATU5010_TIMERD_SUB::AssertmIsUNFMethod (const unsigned int ch_id)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        mIsUNF[ch_id] = true;
...
/// Description  : Assert mIsCMA variable
///*********************************
void ATU5010_TIMERD_SUB::AssertmIsCMAMethod (const unsigned int ch_id)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        mIsCMA[ch_id] = true;
        mCompareMatchAEventDly[ch_id].notify(mTIMERD->mPCLKPeriod/4, mTIMERD->mATU5010->mTimeResolutionUnit);
    }
}//}}}
...
///*********************************
void ATU5010_TIMERD_SUB::AssertmIsCMBMethod (const unsigned int ch_id)
{//{{{
    if (!(mTIMERD->mATU5010->GetResetStatus("presetz") || mTIMERD->mATU5010->GetResetStatus("presetz_bif") ||
    (!mTIMERD->mATU5010->CheckClockPeriod("CLK_HSB")) || (!mTIMERD->mATU5010->CheckClockPeriod("CLK_LSB")))) {
        mCompareMatchBEventDly[ch_id].notify(mTIMERD->mPCLKPeriod/4, mTIMERD->mATU5010->mTimeResolutionUnit);
        mIsCMB[ch_id] = true;
    }
}//}}}

void ATU5010_TIMERD_SUB::HandleInterruptOVF1Method(void)
{//{{{
    INT_OVF1.write(true);
    mClearInterrupt1Event.notify(mTIMERD->mPCLKPeriod,mTIMERD->mATU5010->mTimeResolutionUnit);
}//}}}

void ATU5010_TIMERD_SUB::HandleInterruptOVF2Method(void)
{//{{{
    INT_OVF2.write(true);
    mClearInterrupt2Event.notify(mTIMERD->mPCLKPeriod,mTIMERD->mATU5010->mTimeResolutionUnit);
}//}}}

void ATU5010_TIMERD_SUB::ClearInterruptOVF1Method(void)
{//{{{
    INT_OVF1.write(false);
}//}}}

void ATU5010_TIMERD_SUB::ClearInterruptOVF2Method(void)
{//{{{
    INT_OVF2.write(false);
}//}}}

void ATU5010_TIMERD_SUB::HandleInterruptCMPAMethod(const unsigned int channel)
{//{{{
  INT_CMPA[channel] -> write(true);
  mClearInterruptOutputCMPAEvent[channel].notify(mTIMERD->mPCLKPeriod,mTIMERD->mATU5010->mTimeResolutionUnit);
}//}}}

void ATU5010_TIMERD_SUB::HandleInterruptCMPBMethod(const unsigned int channel)
{//{{{
  INT_CMPB[channel] -> write(true);
  mClearInterruptOutputCMPBEvent[channel].notify(mTIMERD->mPCLKPeriod,mTIMERD->mATU5010->mTimeResolutionUnit);
}//}}}

void ATU5010_TIMERD_SUB::ClearInterruptCMPAMethod(const unsigned int channel)
{//{{{
  INT_CMPA[channel] -> write(false);
}//}}}

void ATU5010_TIMERD_SUB::ClearInterruptCMPBMethod(const unsigned int channel)
{//{{{
  INT_CMPB[channel] -> write(false);
}//}}}

void ATU5010_TIMERD_SUB::HandleInterruptFEIEDMethod(const unsigned int channel)
{//{{{
    INT_FEIED[channel] -> write(true);
    mClearInterruptOutputFEIEDEvent[channel].notify(mTIMERD->mPCLKPeriod,mTIMERD->mATU5010->mTimeResolutionUnit);
}//}}}

void ATU5010_TIMERD_SUB::HandleInterruptREIEDMethod(const unsigned int channel)
{//{{{
    INT_REIED[channel] -> write(true);
    mClearInterruptOutputREIEDEvent[channel].notify(mTIMERD->mPCLKPeriod,mTIMERD->mATU5010->mTimeResolutionUnit);
}//}}}

void ATU5010_TIMERD_SUB::ClearInterruptFEIEDMethod(const unsigned int channel)
{//{{{
    INT_FEIED[channel] -> write(false);
}//}}}

void ATU5010_TIMERD_SUB::ClearInterruptREIEDMethod(const unsigned int channel)
{//{{{
    INT_REIED[channel] -> write(false);
}//}}}

void ATU5010_TIMERD_SUB::AssertINT_UDFMethod(const unsigned int channel)
{//{{{
    INT_UDF[channel] -> write(true);
    mDeassertINT_UDFEvent[channel].notify(mTIMERD->mPCLKPeriod,mTIMERD->mATU5010->mTimeResolutionUnit);
}//}}}

void ATU5010_TIMERD_SUB::DeassertINT_UDFMethod(const unsigned int channel)
{//{{{
    INT_UDF[channel] -> write(false);
}//}}}

unsigned int ATU5010_TIMERD_SUB::GetTCNTDRegisterVal (unsigned int counter)
{//{{{
   unsigned int val;
   if (counter == ATU5010_TIMERD_SUB_AgentController::emCounter1) {
      val = mATU5010_TIMERD_SUBFunc->GetTCNT1DRegValue();
   } else {
      val = mATU5010_TIMERD_SUBFunc->GetTCNT2DRegValue();
   }
   return val;
}//}}}

void ATU5010_TIMERD_SUB::mOFMIZeroEventFunc(unsigned int chnnl)
{//{{{
    #if 0
...
void ATU5010_TIMERD_SUB::mONMIZeroEventFunc(unsigned int chnnl)
{//{{{
    #if 0
...
void ATU5010_TIMERD_SUB::mOTONMIZeroEventFunc(unsigned int chnnl)
{//{{{
    #if 0
...
bool ATU5010_TIMERD_SUB::mIsOFMICNTZero(unsigned int chnnl)
{//{{{
    ...
    if (mIsTimerOFMICNTDRunning[chnnl] == true) {
        ...
        counter_val = mATU5010_TIMERD_SUBFunc->GetOFMICNTDRegValue(chnnl);
    }
    if (counter_val == 0) {
       return_val = true;
    }
...
bool ATU5010_TIMERD_SUB::mIsONMICNTZero(unsigned int chnnl)
{//{{{
    bool return_val = false;
    unsigned int counter_val = 0x1;
    if (mIsTimerONMICNTDRunning[chnnl] == true) {
        ...
        counter_val = mATU5010_TIMERD_SUBFunc->GetONMICNTDRegValue(chnnl);
    }
    if (counter_val == 0) {
       return_val = true;
    }
...
bool ATU5010_TIMERD_SUB::mIsOTONMICNTZero(unsigned int chnnl)
{//{{{
    bool return_val = false;
    unsigned int counter_val = 0x1;
    if (mIsTimerOTONMICNTDRunning[chnnl] == true) {
        ...
        counter_val = mATU5010_TIMERD_SUBFunc->GetOTONMICNTDRegValue(chnnl);
    }
    if (counter_val == 0) {
       return_val = true;
    }
