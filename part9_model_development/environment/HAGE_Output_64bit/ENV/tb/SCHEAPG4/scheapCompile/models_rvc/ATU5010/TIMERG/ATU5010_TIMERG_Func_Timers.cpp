// -----------------------------------------------------------------------------
// $Id: ATU5010_TIMERG_Func_Timers.cpp,v 1.1 621 2017-06-28 10:51:02Z CuongPH8 $
//
 ...
 */
ATU5010_TIMERG_Func_Timers::ATU5010_TIMERG_Func_Timers(const uint8_t&        iChanNum
...
, is_comp (false)
{
    ...
    for (uint8_t chIdx = 1; chIdx < CHAN_NUM; ++chIdx) {
        /// -# instantiate the others pChannels by invoking the private constructor
        pChannels[chIdx] = new ATU5010_TIMERG_Func_Timers(this
                                                      ,chIdx
    ...
    for (uint8_t channel_id = 0; channel_id < CHAN_NUM; ++channel_id) {
        ...
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERG_Func_Timers::startCnt_syncedThread
                                 ,this
                                 ,channel_id)
    ...
    for (uint8_t channel_id = 0; channel_id < CHAN_NUM; ++channel_id) {
        sc_core::sc_spawn_options opt;
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERG_Func_Timers::compareMatch_syncedThread
                                 ,this
                                 ,channel_id)
    ...
    for (uint8_t channel_id = 0; channel_id < CHAN_NUM; ++channel_id) {
        sc_core::sc_spawn_options opt;
        sc_core::sc_spawn(sc_bind(&ATU5010_TIMERG_Func_Timers::preOverflow_syncedThread
                                 ,this
                                 ,channel_id)
 ...
 */
ATU5010_TIMERG_Func_Timers::ATU5010_TIMERG_Func_Timers(ATU5010_TIMERG_Func_Timers* ipChannel0
 ...
 * Detailed steps as follow
 */
ATU5010_TIMERG_Func_Timers::~ATU5010_TIMERG_Func_Timers() {
 ...
 * Detailed steps as follow:
 */
void ATU5010_TIMERG_Func_Timers::cmpMatchOutputHandling (bool value) {
    #if 0
 ...
 *     the channel *0*
 */
void ATU5010_TIMERG_Func_Timers::interruptHandling () {
    #if 0
    cout << endl << "@[" << std::setfill (' ') << std::setw(7) << sc_time_stamp().to_default_time_units() << "]";
    cout << "[" << "triggering interrupt event" << "]";
    cout << "[" << "compare-match:" << (uint32_t)getSR().gtCMF << "]";
    cout << "[" << "overflow:"      << (uint32_t)getSR().gtOVF << "]";
  //cout << "[" << "underflow:"     << (uint32_t)getSR().gtUNF << "]";
    cout << "[" << "input-capture:" << (uint32_t)getSR().gtICF << "]";
    cout << "[" << "counter-value:" << getCNT()                << "]";
    #endif
 ...
 */
void ATU5010_TIMERG_Func_Timers::enable_reset_async (const bool is_active) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));

    for (uint8_t channel_id = 0; channel_id < CHAN_NUM; ++channel_id) {
        ATU5010_TIMERG_Func_Timers* pChan = pChannels[channel_id];
...
//------------------------------------------------------------------------------
bool ATU5010_TIMERG_Func_Timers::is_event_detected_preSync(const uint8_t&  channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    ATU5010_TIMERG_Func_Timers* pChan = pChannels[channel_id];
...
//------------------------------------------------------------------------------
bool ATU5010_TIMERG_Func_Timers::is_countable_preSync(const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    ATU5010_TIMERG_Func_Timers* pChan = pChannels[channel_id];
    ...
    if (  !pChan->isResetActive_preSync
       ...
       &&  pChan->isStartChan_preSync
       && (pChan->extEventStartMode_preSync
                   ? (pChan->extEventDetectFlag_synced
                           ? 1
                           : pExtEvents_synced[extEventSelect_preSync])
                   : 1)
...
//------------------------------------------------------------------------------
void ATU5010_TIMERG_Func_Timers::set_ext_event_synced(const bool&    value
...
//------------------------------------------------------------------------------
void ATU5010_TIMERG_Func_Timers::set_ext_event_start_mode(const bool&    is_ext_event_sync_mode
                                                      ,const uint8_t& channel_id)
{
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
...
//------------------------------------------------------------------------------
void ATU5010_TIMERG_Func_Timers::ext_event_start_cnt(const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    ATU5010_TIMERG_Func_Timers* pChan = pChannels[channel_id];
...
//------------------------------------------------------------------------------
bool ATU5010_TIMERG_Func_Timers::get_ext_event_detect_flag(const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    return pChannels[channel_id]->extEventDetectFlag_synced;
}
//------------------------------------------------------------------------------
void ATU5010_TIMERG_Func_Timers::set_ext_event_detect_flag(const bool& val, const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
 ...
 * @return none
 */
void ATU5010_TIMERG_Func_Timers::set_str_async(const bool&     isStartChan_preSync  ///< [in] set value
...
//------------------------------------------------------------------------------
void ATU5010_TIMERG_Func_Timers::set_str_mst_async(const bool& isStartMaster) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
 ...
 * @return none
 */
void ATU5010_TIMERG_Func_Timers::set_cnt(const uint32_t& value      ///< [in] set value
                                     ,const uint8_t&  channel_id ///< [in] id of the channel to be set count value
                                     ) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    ATU5010_TIMERG_Func_Timers* pChan = pChannels[channel_id];
 ...
 */
uint32_t ATU5010_TIMERG_Func_Timers::get_cnt(const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
 ...
 * @return none
 */
void ATU5010_TIMERG_Func_Timers::set_cor(const uint32_t& value
                                     ,const uint8_t&  channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    ATU5010_TIMERG_Func_Timers* pChan = pChannels[channel_id];
 ...
 */
uint32_t ATU5010_TIMERG_Func_Timers::get_cor(const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));

    return pChannels[channel_id]->getCOR();
}
//------------------------------------------------------------------------------
void ATU5010_TIMERG_Func_Timers::set_pclk_period(const double& clk_period) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    ...
    for (uint8_t clkIdx = 0; clkIdx < CLK_DIV_NUM; ++clkIdx) {
        pClkDivs_synced   [clkIdx] = (pClkPeriods_synced[CLK_DIV_NUM] <= 0 || pClkPeriods_synced[clkIdx] <= 0)
                                        ? 1
                                        : sc_dt::uint64(floor( 0.5
                                                        +   pClkPeriods_synced[clkIdx]
                                                          / pClkPeriods_synced[CLK_DIV_NUM]));
    }
...
//------------------------------------------------------------------------------
void ATU5010_TIMERG_Func_Timers::set_clk_div_period(const double&  clk_period
                                                ,const uint8_t& clkIdx)
{
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    sc_assert(clkIdx < CLK_DIV_NUM);

    bool isTurnOn            = (pClkPeriods_synced[clkIdx] <= 0) && (clk_period >  0);
    bool isTurnOff           = (pClkPeriods_synced[clkIdx] >  0) && (clk_period <= 0);
    ...
    pClkPeriods_synced[clkIdx] = (clk_period < 0) ? 0 : clk_period;
 ...
 */
//------------------------------------------------------------------------------
void ATU5010_TIMERG_Func_Timers::set_clk_div_sel(const uint8_t&  iClkDivSel
                                             ,const uint8_t&  channel_id)
{
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    ATU5010_TIMERG_Func_Timers* pChan = pChannels[channel_id];

    uint8_t validClkDivSel   = (iClkDivSel < CLK_DIV_NUM) ? iClkDivSel : 0;

    #if 0
    bool isTurnOn            = (pClkPeriods_synced[pChan->clkDivSel_preSync] <= 0) && (pClkPeriods_synced[validClkDivSel] >  0);
    bool isTurnOff           = (pClkPeriods_synced[pChan->clkDivSel_preSync] >  0) && (pClkPeriods_synced[validClkDivSel] <= 0);
 ...
 * @return none
 */
void ATU5010_TIMERG_Func_Timers::set_time_unit(const sc_time_unit& time_unit) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
 ...
 * @return none
 */
bool ATU5010_TIMERG_Func_Timers::get_cmf (const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
 ...
 * @return none
 */
void ATU5010_TIMERG_Func_Timers::clear_cmf (const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    pChannels[channel_id]->CMF = false;
    pChannels[channel_id]->clearCMF();
}
//------------------------------------------------------------------------------
void ATU5010_TIMERG_Func_Timers::set_cmf (const bool& val, const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    if (val) {
        pChannels[channel_id]->CMF = true;
    }
}
//------------------------------------------------------------------------------
 ...
 * @return none
 */
bool ATU5010_TIMERG_Func_Timers::get_ovff (const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
 ...
 * @return none
 */
void ATU5010_TIMERG_Func_Timers::clear_ovff (const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));

    pChannels[channel_id]->OVF = false;
}
//------------------------------------------------------------------------------
void ATU5010_TIMERG_Func_Timers::set_ovff (const bool& val, const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    pChannels[channel_id]->OVF = val;
}
//------------------------------------------------------------------------------
 ...
 * @return none
 */
void ATU5010_TIMERG_Func_Timers::set_rl_en (const bool&    irlen
                                        ,const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
 ...
 * @return none
 */
void ATU5010_TIMERG_Func_Timers::set_rl_val (const uint32_t& irl_val
                                         ,const uint8_t&  channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
 ...
 */
void ATU5010_TIMERG_Func_Timers::common_settings() {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
...
//------------------------------------------------------------------------------
sc_time ATU5010_TIMERG_Func_Timers::to_next_sync_point_pclk() {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    return(to_next_sync_point(CLK_DIV_NUM));
}
//------------------------------------------------------------------------------
sc_time ATU5010_TIMERG_Func_Timers::to_next_sync_point(const uint8_t& clkIdx) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    sc_assert(clkIdx <= CLK_DIV_NUM);
...
//------------------------------------------------------------------------------
bool ATU5010_TIMERG_Func_Timers::is_sync_point (const uint8_t& clkIdx) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    sc_assert(clkIdx <= CLK_DIV_NUM);
    ...
    if (clkIdx > CLK_DIV_NUM) {
        ...
        sc_dt::uint64 round_period_num = (sc_dt::uint64)floor(0.5 + (now - pSyncPoints[clkIdx]) / clk_period);
        sc_dt::uint64 floor_period_num = (sc_dt::uint64)floor(      (now - pSyncPoints[clkIdx]) / clk_period);
        sc_dt::uint64 ceil_period_num  = (sc_dt::uint64)ceil (      (now - pSyncPoints[clkIdx]) / clk_period);
...
/**@brief common settings for all channels
 */
void ATU5010_TIMERG_Func_Timers::startCnt_syncedThread(const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    ATU5010_TIMERG_Func_Timers* pChan = pChannels[channel_id];
    while (1) {
        ...
        if (pChan->getCR().gtSTR) {
            ...
            pChan->setSTR(true);
            pChan->setCNT(pChan->getCounterDirection() == emCountUp
                                ? currentCNT+1
                                : currentCNT-1);
        }
...
//------------------------------------------------------------------------------
void ATU5010_TIMERG_Func_Timers::compareMatch_syncedThread(const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    ATU5010_TIMERG_Func_Timers* pChan = pChannels[channel_id];
    while (1) {
        ...
        pChan->setCNT(pChan->getCR().gtCKS == 1 ? 1 : 0);
        pChan->startCnt_syncedEvent.notify(to_next_sync_point(pChan->clkDivSel_preSync));
...
//------------------------------------------------------------------------------
void ATU5010_TIMERG_Func_Timers::preOverflow_syncedThread(const uint8_t& channel_id) {
    sc_assert((pChannel0 == this) && (pChannels[0] == this));
    ATU5010_TIMERG_Func_Timers* pChan = pChannels[channel_id];
    while (1) {
