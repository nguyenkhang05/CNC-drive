// -----------------------------------------------------------------------------
// $Id: ecccnt_cra_fcc2_regif.cpp,v 1.3 2019/03/05 04:30:03 synguyen Exp $
//
...
//
// Input file : ecccnt_cra_fcc2_regif.txt
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
/// @file ecccnt_cra_fcc2_regif.cpp
/// @brief Register IF class of model ECCCNT_CRA
/// $Id: ecccnt_cra_fcc2_regif.cpp,v 1.3 2019/03/05 04:30:03 synguyen Exp $
...
/// @return none
Cecccnt_cra_fcc2_regif::Cecccnt_cra_fcc2_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    CRAECCCTL = new vpcl::re_register(0x0000, this, "CRAECCCTL", name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_CRAECCCTL  , CRAECCCTL->name(), CRAECCCTL->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(CRAECCCTL  , mCurReg, 0,  4, "8|16|32", "8|16|32");

    mRegList = mCurReg;

    (*CRAECCCTL  ) ( 1,  1, "SECDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CRAECCCTL  ) ( 0,  0, "ECCDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["CRAECCCTL"]["ECCDIS" ] = &Cecccnt_cra_fcc2_regif::cb_CRAECCCTL_ECCDIS;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cecccnt_cra_fcc2_regif::~Cecccnt_cra_fcc2_regif()
{
...
Cecccnt_cra_fcc2_regif::uint Cecccnt_cra_fcc2_regif::bit_select(
...
bool Cecccnt_cra_fcc2_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cecccnt_cra_fcc2_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_cra_fcc2_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                   const unsigned char *p_data,   ///< [in] Writing data
                                   cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_cra_fcc2_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                   unsigned char *p_data,   ///< [out] Reading data
                                   cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_cra_fcc2_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                       const unsigned char *p_data,  ///< [in] Writing data
                                       cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_cra_fcc2_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                       unsigned char *p_data,   ///< [out] Reading data
                                       cuint size,              ///< [in]  Data size (byte)
                                       bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cecccnt_cra_fcc2_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                    unsigned char *p_data,          ///< [out] Reading data
                                    cuint size,                     ///< [in]  Data size (byte)
...
bool Cecccnt_cra_fcc2_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                    const unsigned char *p_data,      ///< [in] Writing data
                                    cuint size,                       ///< [in] Data size (byte)
                                    cuint reg_index,                  ///< [in] Register index
...
bool Cecccnt_cra_fcc2_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cecccnt_cra_fcc2_regif::CommandInit()
{
...
/// @return string
std::string Cecccnt_cra_fcc2_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cecccnt_cra_fcc2_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cecccnt_cra_fcc2_regif::InitLocalVal()
{
    // Initialize local variables
    CRAECCCTL_SECDIS = 0x0;
    CRAECCCTL_ECCDIS = 0x0;
}

/// Update bit value to local value
/// @return None
void Cecccnt_cra_fcc2_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        CRAECCCTL_SECDIS              = (*CRAECCCTL)["SECDIS"   ];
        CRAECCCTL_ECCDIS              = (*CRAECCCTL)["ECCDIS"   ];
        return;
    }
}
...
/// @return None
void Cecccnt_cra_fcc2_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*CRAECCCTL)["SECDIS"   ] = CRAECCCTL_SECDIS;
        (*CRAECCCTL)["ECCDIS"   ] = CRAECCCTL_ECCDIS;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cecccnt_cra_fcc2_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cecccnt_cra_fcc2_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cecccnt_cra_fcc2_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cecccnt_cra_fcc2_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cecccnt_cra_fcc2_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cecccnt_cra_fcc2_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cecccnt_cra_fcc2_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cecccnt_cra_fcc2_regif::uint Cecccnt_cra_fcc2_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cecccnt_cra_fcc2_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cecccnt_cra_fcc2_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cecccnt_cra_fcc2_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cecccnt_cra_fcc2_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cecccnt_cra_fcc2_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cecccnt_cra_fcc2_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cecccnt_cra_fcc2_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
