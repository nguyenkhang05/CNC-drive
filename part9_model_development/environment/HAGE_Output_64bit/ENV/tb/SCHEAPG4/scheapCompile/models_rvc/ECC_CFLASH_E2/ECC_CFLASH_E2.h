// ---------------------------------------------------------------------
// $Id: ECC_CFLASH_E2.h,v 1.1 2019/09/26 10:06:58 landang Exp $
//
...
// Corporation.
// ---------------------------------------------------------------------
#ifndef __ECC_CFLASH_E2_H__
#define __ECC_CFLASH_E2_H__
...
class ECCCNT_CFP_PEn_Func;
class ECCCNT_CFS_Func;
class ECCCNT_CFC_Func;
class MECCCAP_CFL_Func;

class ECC_CFLASH_E2: public sc_module
            ,public ECC_CFLASH_E2_AgentController
            ,public BusSlaveBase<32, PE_MAX_NUM + 3> // PE_NUM socket for CFP, 1 for CFS, 1 for CFC, 1 for CFL (MECCCAP)
...
public:
    ...
    sc_out<bool>            blankerrormaskenable; // blank error maskable

    // ECCCNT CFC
    sc_out<bool>            apedisable;     // address parity error prohibition
    ...
    TlmTargetSocket<32>     *ts_ecccnt_cfc; // target socket
    TlmTargetSocket<32>     *ts_mecccap_cf; // target socket

    ECCCNT_CFP_PEn_Func     **mpECCCNT_CFPFunc;   // Pointer of ECCCNT_CFP_PEn Func
    ECCCNT_CFS_Func         *mpECCCNT_CFSFunc;  // Pointer of ECCCNT_CFS Func
    ECCCNT_CFC_Func         *mpECCCNT_CFCFunc;  // Pointer of ECCCNT_CFC Func
    MECCCAP_CFL_Func        *mpMECCCAP_CFLFunc; // Pointer of MECCCAP_CFL Func
...
private:
    ...
    enum eTgtInfo{
        ...
        ,emMECCCAP_CFTgtId  = PE_MAX_NUM + 2
        ,emNumTgt           = PE_MAX_NUM + 3
    };
    ...
    bool            mBLANKMASKENABLE; // Store value of blankerrormaskenable
    bool            mECCDIS_CFS;    // Store value of eccdisable_cfs
    bool            mSECDIS_CFS;    // Store value of secdisable_cfs
    bool            mAPEDIS_CFC;    // Store value of apedisable
    ...
    sc_event mWriteDisPort_CFCEvent;            // Trigger WriteDisPort_CFCMethod
    ...
    // Overwrite firtual functions of ECC_CFLASH_E2_AgentController
    void WriteDisPort_CFP(unsigned int pe_id, unsigned int value);
    void WriteDisPort_CFS(unsigned int value);
    void WriteDisPort_CFC(unsigned int value);
...
public:
    SC_HAS_PROCESS (ECC_CFLASH_E2);
    ECC_CFLASH_E2 (sc_module_name name, const uint8_t iPE_NUM, const unsigned int rLatency, const unsigned int wLatency);
    ~ECC_CFLASH_E2 ();
