// -----------------------------------------------------------------------------
// $Id: atu5010_timerd_regif.cpp,v 1.0 2016/11/24 10:15:45 binhnguyen Exp $
//
...
//
// Input file : atu5010_timerd_regif.txt
////////////////////////////////////////////////////////////////////////////////
// %MODULE ATU5010_TIMERD
//     #                   name            offset_size
...
// %%BIT         STRD14          14           14          0x0           R|W           TRUE          -
// %%BIT         STRD13          13           13          0x0           R|W           TRUE          -
// %%BIT         STRD12          12           12          0x0           R|W           TRUE          -
// %%BIT         STRD11          11           11          0x0           R|W           TRUE          -
// %%BIT         STRD10          10           10          0x0           R|W           TRUE          -
// %%BIT         STRD9           9            9           0x0           R|W           TRUE          -
...
// 
////////////////////////////////////////////////////////////////////////////////
/// @file atu5010_timerd_regif.cpp
/// @brief Register IF class of model ATU5010_TIMERD
/// $Id$
/// $Date$
/// $Revison$
/// $Author$
...
/// @return none
Catu5010_timerd_regif::Catu5010_timerd_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    TSTRD    = new vpcl::re_register(0x0000, this, "TSTRD"   , name.c_str());
    CCAPSELD = new vpcl::re_register(0x0004, this, "CCAPSELD", name.c_str());
    std::ostringstream str_tmp;
    for(uint i = 0; i < emNUM_CICRD; i++) {
        str_tmp.str("");
        str_tmp<<"CICRD"<< i;
        CICRD   [i] = new vpcl::re_register((0x0020) + 4*i, this, str_tmp.str(), name.c_str());
    }
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_TSTRD_, "TSTRD_", 0x0000);
    CW_TSTRD    = new scml2::bitfield<uint>(TSTRD->name()   , CW_TSTRD_          ,  0, 16);
    cw_instantiate_reg(CW_CCAPSELD_, "CCAPSELD_", 0x0004);
    CW_CCAPSELD = new scml2::bitfield<uint>(CCAPSELD->name(), CW_CCAPSELD_       ,  0,  8);
    for(uint i = 0; i < emNUM_CICRD   ; i++) cw_instantiate_reg(CW_CICRD     [i], CICRD   [i]->name(), CICRD   [i]->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(TSTRD     , mCurReg, 0,  2,    "8|16", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(CCAPSELD  , mCurReg, 0,  1,       "8", "8|16|32");
    for(uint i = 0; i < emNUM_CICRD   ; i++) mCurReg = mRegArray[index++] = new SRegList(CICRD     [i], mCurReg, i,  4, "8|16|32", "8|16|32");

    mRegList = mCurReg;

    (*TSTRD     ) (14, 14, "STRD14"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) (13, 13, "STRD13"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) (12, 12, "STRD12"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) (11, 11, "STRD11"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) (10, 10, "STRD10"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) ( 9,  9, "STRD9"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) ( 8,  8, "STRD8"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) ( 7,  7, "STRD7"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) ( 6,  6, "STRD6"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) ( 5,  5, "STRD5"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) ( 4,  4, "STRD4"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) ( 3,  3, "STRD3"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) ( 2,  2, "STRD2"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) ( 1,  1, "STRD1"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*TSTRD     ) ( 0,  0, "STRD0"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CCAPSELD  ) ( 0,  0, "CCAPSEL" , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    for(uint i = 0; i < emNUM_CICRD; i++) {
        (*CICRD     [i]) (31,  0, "CICRD"   , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    ...
    mWrCbAPI["TSTRD" ]["STRD0" ] = &Catu5010_timerd_regif::cb_TSTRD_STRD0;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Catu5010_timerd_regif::~Catu5010_timerd_regif()
{
...
Catu5010_timerd_regif::uint Catu5010_timerd_regif::bit_select(
...
bool Catu5010_timerd_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Catu5010_timerd_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Catu5010_timerd_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                    const unsigned char *p_data,   ///< [in] Writing data
                                    cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Catu5010_timerd_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                    unsigned char *p_data,   ///< [out] Reading data
                                    cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Catu5010_timerd_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                        const unsigned char *p_data,  ///< [in] Writing data
                                        cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Catu5010_timerd_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                        unsigned char *p_data,   ///< [out] Reading data
                                        cuint size,              ///< [in]  Data size (byte)
                                        bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Catu5010_timerd_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                     unsigned char *p_data,          ///< [out] Reading data
                                     cuint size,                     ///< [in]  Data size (byte)
...
bool Catu5010_timerd_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                     const unsigned char *p_data,      ///< [in] Writing data
                                     cuint size,                       ///< [in] Data size (byte)
                                     cuint reg_index,                  ///< [in] Register index
...
bool Catu5010_timerd_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Catu5010_timerd_regif::CommandInit()
{
...
/// @return string
std::string Catu5010_timerd_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Catu5010_timerd_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Catu5010_timerd_regif::InitLocalVal()
{
    // Initialize local variables
    TSTRD_STRD14 = 0x0;
    TSTRD_STRD13 = 0x0;
...
/// @return None
void Catu5010_timerd_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        TSTRD_STRD14                 = (*TSTRD   )["STRD14"  ];
        TSTRD_STRD13                 = (*TSTRD   )["STRD13"  ];
        TSTRD_STRD12                 = (*TSTRD   )["STRD12"  ];
        TSTRD_STRD11                 = (*TSTRD   )["STRD11"  ];
        TSTRD_STRD10                 = (*TSTRD   )["STRD10"  ];
        TSTRD_STRD9                  = (*TSTRD   )["STRD9"   ];
        TSTRD_STRD8                  = (*TSTRD   )["STRD8"   ];
        TSTRD_STRD7                  = (*TSTRD   )["STRD7"   ];
        TSTRD_STRD6                  = (*TSTRD   )["STRD6"   ];
        TSTRD_STRD5                  = (*TSTRD   )["STRD5"   ];
        TSTRD_STRD4                  = (*TSTRD   )["STRD4"   ];
        TSTRD_STRD3                  = (*TSTRD   )["STRD3"   ];
        TSTRD_STRD2                  = (*TSTRD   )["STRD2"   ];
        TSTRD_STRD1                  = (*TSTRD   )["STRD1"   ];
        TSTRD_STRD0                  = (*TSTRD   )["STRD0"   ];
        return;
    }
    if (addr == 0x0004) {
        CCAPSELD_CCAPSEL             = (*CCAPSELD)["CCAPSEL" ];
        return;
    }
    if ((0x0020 <= addr) && (addr <= (0x0020)+(4*7)) && ((addr-0x0020)%4 == 0)) {
        uint i = (addr - 0x0020)/4;
        CICRD_CICRD                 [i] = (*(CICRD   [i]))["CICRD"   ];
        return;
    }
}
...
/// @return None
void Catu5010_timerd_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*TSTRD   )["STRD14"  ] = TSTRD_STRD14;
        (*TSTRD   )["STRD13"  ] = TSTRD_STRD13;
        (*TSTRD   )["STRD12"  ] = TSTRD_STRD12;
        (*TSTRD   )["STRD11"  ] = TSTRD_STRD11;
        (*TSTRD   )["STRD10"  ] = TSTRD_STRD10;
        (*TSTRD   )["STRD9"   ] = TSTRD_STRD9;
        (*TSTRD   )["STRD8"   ] = TSTRD_STRD8;
        (*TSTRD   )["STRD7"   ] = TSTRD_STRD7;
        (*TSTRD   )["STRD6"   ] = TSTRD_STRD6;
        (*TSTRD   )["STRD5"   ] = TSTRD_STRD5;
        (*TSTRD   )["STRD4"   ] = TSTRD_STRD4;
        (*TSTRD   )["STRD3"   ] = TSTRD_STRD3;
        (*TSTRD   )["STRD2"   ] = TSTRD_STRD2;
        (*TSTRD   )["STRD1"   ] = TSTRD_STRD1;
        (*TSTRD   )["STRD0"   ] = TSTRD_STRD0;
        return;
    }
    if (addr == 0x0004) {
        (*CCAPSELD)["CCAPSEL" ] = CCAPSELD_CCAPSEL;
        return;
    }
    if ((0x0020 <= addr) && (addr <= (0x0020)+(4*7)) && ((addr-0x0020)%4 == 0)) {
        uint i = (addr - 0x0020)/4;
        (*(CICRD   [i]))["CICRD"   ] = CICRD_CICRD[i];
        return;
    }
}
...
/// @return vector
std::vector<std::string> Catu5010_timerd_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Catu5010_timerd_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Catu5010_timerd_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Catu5010_timerd_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Catu5010_timerd_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Catu5010_timerd_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Catu5010_timerd_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Catu5010_timerd_regif::uint Catu5010_timerd_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Catu5010_timerd_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Catu5010_timerd_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Catu5010_timerd_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Catu5010_timerd_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Catu5010_timerd_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Catu5010_timerd_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Catu5010_timerd_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
