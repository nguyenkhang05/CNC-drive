// ---------------------------------------------------------------------
// $Id: ECCCNT_CFS_Func.cpp,v 1.1 2019/09/26 10:06:58 landang Exp $
//
...
//========================================================
/// Constructor
ECCCNT_CFS_Func::ECCCNT_CFS_Func (std::string name, ECC_CFLASH_E2_AgentController* CFLASHAgentController):
                Cecccnt_cfs_regif(name, 32)
                ,BusSlaveFuncIf()
{//{{{
    ...
    mProtectedRegMap.insert(std::pair<unsigned int, std::string>(0x0, "CFSECCCTL"));

    Cecccnt_cfs_regif::set_instance_name(name);
}//}}}

/// Destructor
ECCCNT_CFS_Func::~ECCCNT_CFS_Func()
{//{{{
}//}}}

/// Reset Func and registers
void ECCCNT_CFS_Func::EnableReset(const std::string reset_name, const bool is_active)
{//{{{
    if (reset_name == "sys_resetz"){
        Cecccnt_cfs_regif::EnableReset(is_active);
    }
}//}}}

/// Read API (target socket)
void ECCCNT_CFS_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    // Get extension info
...
void ECCCNT_CFS_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    // Get extension info
    TlmG3mExtension *extension;
    trans.get_extension(extension);
...
void ECCCNT_CFS_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cecccnt_cfs_regif::reg_handle_command(cmd);
                if (msg_return != "") {
            ...
            msg_return = Cecccnt_cfs_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
unsigned int ECCCNT_CFS_Func::GetCFSECCCTLReg()
{//{{{
    return (unsigned int)(*CFSECCCTL);
}//}}}

//========================================================
//============ Callback function  ========================
//========================================================
/// Callback function of CFSECCCTL_BLANKMASKENABLE bit
void ECCCNT_CFS_Func::cb_CFSECCCTL_BLANKMASKENABLE(RegCBstr str)
{//{{{ 
    mpCFLASHAgentController->WriteDisPort_CFS((unsigned int)(*CFSECCCTL));
}//}}}

/// Callback function of CFSECCCTL_ECCDIS bit
void ECCCNT_CFS_Func::cb_CFSECCCTL_ECCDIS(RegCBstr str)
{//{{{ 
    mpCFLASHAgentController->WriteDisPort_CFS((unsigned int)(*CFSECCCTL));
}//}}}
