// -----------------------------------------------------------------------------
// $Id: ecccnt_dmden_regif.cpp,v 1.6 2019/09/06 11:37:29 landang Exp $
//
...
//
// Input file : ./input/DMADTS_RAM/ecccnt_dmden_regif.txt
////////////////////////////////////////////////////////////////////////////////
// %MODULE ecccnt_dmden
//     #              name     offset_size
...
//     ////////////////////////////////////////////////////////////////////////////////
/// @file ecccnt_dmden_regif.cpp
/// @brief Register IF class of model ECCCNT_DMDEN
/// $Id: ecccnt_dmden_regif.cpp,v 1.6 2019/09/06 11:37:29 landang Exp $
...
/// @return none
Cecccnt_dmden_regif::Cecccnt_dmden_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    DEECCCTL       = new vpcl::re_register(0x0000, this, "DEECCCTL"      , name.c_str());
    DEECCTSTCTL    = new vpcl::re_register(0x0010, this, "DEECCTSTCTL"   , name.c_str());
    DEECCTSTDIN0   = new vpcl::re_register(0x0030, this, "DEECCTSTDIN0"  , name.c_str());
    DEECCTSTEIN    = new vpcl::re_register(0x0040, this, "DEECCTSTEIN"   , name.c_str());
    DETDATBFDATAF0 = new vpcl::re_register(0x0050, this, "DETDATBFDATAF0", name.c_str());
    DETDATBFECCF   = new vpcl::re_register(0x0060, this, "DETDATBFECCF"  , name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_DEECCCTL        , DEECCCTL->name()      , DEECCCTL->addr()      );
    cw_instantiate_reg(CW_DEECCTSTCTL     , DEECCTSTCTL->name()   , DEECCTSTCTL->addr()   );
    cw_instantiate_reg(CW_DEECCTSTDIN0    , DEECCTSTDIN0->name()  , DEECCTSTDIN0->addr()  );
    cw_instantiate_reg(CW_DEECCTSTEIN     , DEECCTSTEIN->name()   , DEECCTSTEIN->addr()   );
    cw_instantiate_reg(CW_DETDATBFDATAF0  , DETDATBFDATAF0->name(), DETDATBFDATAF0->addr());
    cw_instantiate_reg(CW_DETDATBFECCF    , DETDATBFECCF->name()  , DETDATBFECCF->addr()  );
    #endif
    mCurReg = mRegArray[index++] = new SRegList(DEECCCTL        , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DEECCTSTCTL     , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DEECCTSTDIN0    , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DEECCTSTEIN     , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DETDATBFDATAF0  , mCurReg, 0,  4,      "32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DETDATBFECCF    , mCurReg, 0,  4,      "32", "8|16|32");

    mRegList = mCurReg;

    (*DEECCCTL        ) ( 1,  1, "SECDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*DEECCCTL        ) ( 0,  0, "ECCDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*DEECCTSTCTL     ) ( 1,  1, "ECCTST"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*DEECCTSTDIN0    ) (31,  0, "DATA"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*DEECCTSTEIN     ) ( 6,  0, "ECC"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*DETDATBFDATAF0  ) (31,  0, "BFDATA"  , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*DETDATBFECCF    ) ( 6,  0, "BFECC0"  , 0x0       , "R"     , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["DEECCCTL"    ]["ECCDIS"      ] = &Cecccnt_dmden_regif::cb_DEECCCTL_ECCDIS;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cecccnt_dmden_regif::~Cecccnt_dmden_regif()
{
...
Cecccnt_dmden_regif::uint Cecccnt_dmden_regif::bit_select(
...
bool Cecccnt_dmden_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cecccnt_dmden_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_dmden_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                     const unsigned char *p_data,   ///< [in] Writing data
                                     cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_dmden_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                     unsigned char *p_data,   ///< [out] Reading data
                                     cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_dmden_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                         const unsigned char *p_data,  ///< [in] Writing data
                                         cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_dmden_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                         unsigned char *p_data,   ///< [out] Reading data
                                         cuint size,              ///< [in]  Data size (byte)
                                         bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cecccnt_dmden_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                      unsigned char *p_data,          ///< [out] Reading data
                                      cuint size,                     ///< [in]  Data size (byte)
...
bool Cecccnt_dmden_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                      const unsigned char *p_data,      ///< [in] Writing data
                                      cuint size,                       ///< [in] Data size (byte)
                                      cuint reg_index,                  ///< [in] Register index
...
bool Cecccnt_dmden_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cecccnt_dmden_regif::CommandInit()
{
...
/// @return string
std::string Cecccnt_dmden_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cecccnt_dmden_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cecccnt_dmden_regif::InitLocalVal()
{
    // Initialize local variables
    DEECCCTL_SECDIS = 0x0;
    DEECCCTL_ECCDIS = 0x0;
    DEECCTSTCTL_ECCTST = 0x0;
    DEECCTSTDIN0_DATA = 0x0;
    DEECCTSTEIN_ECC = 0x0;
    DETDATBFDATAF0_BFDATA = 0x0;
    DETDATBFECCF_BFECC0 = 0x0;
}

/// Update bit value to local value
/// @return None
void Cecccnt_dmden_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        DEECCCTL_SECDIS                    = (*DEECCCTL      )["SECDIS"        ];
        DEECCCTL_ECCDIS                    = (*DEECCCTL      )["ECCDIS"        ];
        return;
    }
    if (addr == 0x0010) {
        DEECCTSTCTL_ECCTST                 = (*DEECCTSTCTL   )["ECCTST"        ];
        return;
    }
    if (addr == 0x0030) {
        DEECCTSTDIN0_DATA                  = (*DEECCTSTDIN0  )["DATA"          ];
        return;
    }
    if (addr == 0x0040) {
        DEECCTSTEIN_ECC                    = (*DEECCTSTEIN   )["ECC"           ];
        return;
    }
    if (addr == 0x0050) {
        DETDATBFDATAF0_BFDATA              = (*DETDATBFDATAF0)["BFDATA"        ];
        return;
    }
    if (addr == 0x0060) {
        DETDATBFECCF_BFECC0                = (*DETDATBFECCF  )["BFECC0"        ];
        return;
    }
}
...
/// @return None
void Cecccnt_dmden_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*DEECCCTL      )["SECDIS"        ] = DEECCCTL_SECDIS;
        (*DEECCCTL      )["ECCDIS"        ] = DEECCCTL_ECCDIS;
        return;
    }
    if (addr == 0x0010) {
        (*DEECCTSTCTL   )["ECCTST"        ] = DEECCTSTCTL_ECCTST;
        return;
    }
    if (addr == 0x0030) {
        (*DEECCTSTDIN0  )["DATA"          ] = DEECCTSTDIN0_DATA;
        return;
    }
    if (addr == 0x0040) {
        (*DEECCTSTEIN   )["ECC"           ] = DEECCTSTEIN_ECC;
        return;
    }
    if (addr == 0x0050) {
        (*DETDATBFDATAF0)["BFDATA"        ] = DETDATBFDATAF0_BFDATA;
        return;
    }
    if (addr == 0x0060) {
        (*DETDATBFECCF  )["BFECC0"        ] = DETDATBFECCF_BFECC0;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cecccnt_dmden_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cecccnt_dmden_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cecccnt_dmden_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cecccnt_dmden_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cecccnt_dmden_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cecccnt_dmden_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cecccnt_dmden_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cecccnt_dmden_regif::uint Cecccnt_dmden_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cecccnt_dmden_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cecccnt_dmden_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cecccnt_dmden_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cecccnt_dmden_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cecccnt_dmden_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cecccnt_dmden_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cecccnt_dmden_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
