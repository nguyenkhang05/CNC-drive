// -----------------------------------------------------------------------------
// $Id: g4_peg_u2a_regif.cpp,v 1.6 2020/01/14 02:39:30 nhutminhdang Exp $
//
...
//
// Input file : ../../input/g4_peg/g4_peg_u2a_regif.txt
////////////////////////////////////////////////////////////////////////////////
// %MODULE G4_PEG_U2A
//     #              name      offset_size
...
// 
////////////////////////////////////////////////////////////////////////////////
/// @file g4_peg_u2a_regif.cpp
/// @brief Register IF class of model G4_PEG_U2A
/// $Id: g4_peg_u2a_regif.cpp,v 1.6 2020/01/14 02:39:30 nhutminhdang Exp $
...
/// @return none
Cg4_peg_u2a_regif::Cg4_peg_u2a_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
...
{
    CommandInit();
    PEGKCPROT = new vpcl::re_register(0x0000, this, "PEGKCPROT", name.c_str());
    std::ostringstream str_tmp;
    for(uint i = 0; i < emNUM_PEGPROT; i++) {
        str_tmp.str("");
        str_tmp<<"PEGPROT"<< i;
        PEGPROT  [i] = new vpcl::re_register((0x0040) + 16*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_PEGSPID; i++) {
        str_tmp.str("");
        str_tmp<<"PEGSPID"<< i;
        PEGSPID  [i] = new vpcl::re_register((0x0044) + 16*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_PEGBAD; i++) {
        str_tmp.str("");
        str_tmp<<"PEGBAD"<< i;
        PEGBAD   [i] = new vpcl::re_register((0x0048) + 16*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_PEGADV; i++) {
        str_tmp.str("");
        str_tmp<<"PEGADV"<< i;
        PEGADV   [i] = new vpcl::re_register((0x004C) + 16*i, this, str_tmp.str(), name.c_str());
    }
    ...
    for(uint i = 0; i < emNUM_PEGSPID;  i++) {
        mRegMap[(0x0044)     + i*16] = index;
        mRegMap[(0x0044 + 1) + i*16] = index;
        mRegMap[(0x0044 + 2) + i*16] = index;
        mRegMap[(0x0044 + 3) + i*16] = index++;
    }
    for(uint i = 0; i < emNUM_PEGBAD;   i++) {
    ...
    for(uint i = 0; i < emNUM_PEGADV;   i++) {
        mRegMap[(0x004C)     + i*16] = index;
        mRegMap[(0x004C + 1) + i*16] = index;
        mRegMap[(0x004C + 2) + i*16] = index;
        mRegMap[(0x004C + 3) + i*16] = index++;
    }
    mTotalRegNum = index;
    mRegArray = new SRegList* [mTotalRegNum];
    index = 0;
    #ifdef USR_CWR_SYSTEMC
    cw_instantiate_reg(CW_PEGKCPROT  , PEGKCPROT->name(), PEGKCPROT->addr());
    for(uint i = 0; i < emNUM_PEGPROT  ; i++) cw_instantiate_reg(CW_PEGPROT    [i], PEGPROT  [i]->name(), PEGPROT  [i]->addr());
    for(uint i = 0; i < emNUM_PEGSPID  ; i++) cw_instantiate_reg(CW_PEGSPID    [i], PEGSPID  [i]->name(), PEGSPID  [i]->addr());
    for(uint i = 0; i < emNUM_PEGBAD   ; i++) cw_instantiate_reg(CW_PEGBAD     [i], PEGBAD   [i]->name(), PEGBAD   [i]->addr());
    for(uint i = 0; i < emNUM_PEGADV   ; i++) cw_instantiate_reg(CW_PEGADV     [i], PEGADV   [i]->name(), PEGADV   [i]->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(PEGKCPROT  , mCurReg, 0,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_PEGPROT  ; i++) mCurReg = mRegArray[index++] = new SRegList(PEGPROT    [i], mCurReg, i,  4, "8|16|32", "8|16|32");
    for(uint i = 0; i < emNUM_PEGSPID  ; i++) mCurReg = mRegArray[index++] = new SRegList(PEGSPID    [i], mCurReg, i,  4, "8|16|32", "8|16|32");
    for(uint i = 0; i < emNUM_PEGBAD   ; i++) mCurReg = mRegArray[index++] = new SRegList(PEGBAD     [i], mCurReg, i,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_PEGADV   ; i++) mCurReg = mRegArray[index++] = new SRegList(PEGADV     [i], mCurReg, i,  4,      "32", "8|16|32");

    mRegList = mCurReg;

    (*PEGKCPROT  ) (31,  1, "KCPROT"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*PEGKCPROT  ) ( 0,  0, "KCE"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    for(uint i = 0; i < emNUM_PEGPROT; i++) {
        (*PEGPROT    [i]) ( 8,  8, "GEN"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PEGPROT    [i]) ( 6,  6, "DBG"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PEGPROT    [i]) ( 4,  4, "UM"      , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PEGPROT    [i]) ( 1,  1, "WG"      , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PEGPROT    [i]) ( 0,  0, "RG"      , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_PEGSPID; i++) {
        (*PEGSPID    [i]) (31,  0, "SPID"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_PEGADV; i++) {
        (*PEGADV     [i]) (25, 12, "ADV"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_PEGBAD; i++) {
        (*PEGBAD     [i]) (31, 12, "BAD"     , 0xFC000   , "R|W"   , vpcl::SPP_ENABLE);
    }
    ...
    mWrCbAPI["PEGKCPROT"]["KCPROT" ] = &Cg4_peg_u2a_regif::cb_PEGKCPROT_KCPROT;

    for(uint i = 0; i < emNUM_PEGPROT; i++) {
        str_tmp.str("");
        str_tmp<<"PEGPROT"<< i;
        mWrCbAPI[str_tmp.str()]["GEN"] = &Cg4_peg_u2a_regif::cb_PEGPROT_GEN;
    }
    for(uint i = 0; i < emNUM_PEGSPID; i++) {
        str_tmp.str("");
        str_tmp<<"PEGSPID"<< i;
        mWrCbAPI[str_tmp.str()]["SPID"] = &Cg4_peg_u2a_regif::cb_PEGSPID_SPID;
    }
    for(uint i = 0; i < emNUM_PEGBAD; i++) {
        str_tmp.str("");
        str_tmp<<"PEGBAD"<< i;
        mWrCbAPI[str_tmp.str()]["BAD"] = &Cg4_peg_u2a_regif::cb_PEGBAD_BAD;
    }
    for(uint i = 0; i < emNUM_PEGADV; i++) {
        str_tmp.str("");
        str_tmp<<"PEGADV"<< i;
        mWrCbAPI[str_tmp.str()]["ADV"] = &Cg4_peg_u2a_regif::cb_PEGADV_ADV;
    }
    InitLocalVal();
}
...
/// @return none
Cg4_peg_u2a_regif::~Cg4_peg_u2a_regif()
{
...
Cg4_peg_u2a_regif::uint Cg4_peg_u2a_regif::bit_select(
...
bool Cg4_peg_u2a_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cg4_peg_u2a_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cg4_peg_u2a_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                   const unsigned char *p_data,   ///< [in] Writing data
                                   cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cg4_peg_u2a_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                   unsigned char *p_data,   ///< [out] Reading data
                                   cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cg4_peg_u2a_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                       const unsigned char *p_data,  ///< [in] Writing data
                                       cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cg4_peg_u2a_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                       unsigned char *p_data,   ///< [out] Reading data
                                       cuint size,              ///< [in]  Data size (byte)
                                       bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cg4_peg_u2a_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                    unsigned char *p_data,          ///< [out] Reading data
                                    cuint size,                     ///< [in]  Data size (byte)
...
bool Cg4_peg_u2a_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                    const unsigned char *p_data,      ///< [in] Writing data
                                    cuint size,                       ///< [in] Data size (byte)
                                    cuint reg_index,                  ///< [in] Register index
...
bool Cg4_peg_u2a_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cg4_peg_u2a_regif::CommandInit()
{
...
/// @return string
std::string Cg4_peg_u2a_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cg4_peg_u2a_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cg4_peg_u2a_regif::InitLocalVal()
{
    // Initialize local variables
    PEGKCPROT_KCPROT = 0x0;
    PEGKCPROT_KCE = 0x0;
    for(uint i = 0; i < emNUM_PEGPROT;  i++) {
        PEGPROT_GEN[i] = 0x0;
        PEGPROT_DBG[i] = 0x0;
...
/// @return None
void Cg4_peg_u2a_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        PEGKCPROT_KCPROT              = (*PEGKCPROT)["KCPROT"   ];
        PEGKCPROT_KCE                 = (*PEGKCPROT)["KCE"      ];
        return;
    }
    if ((0x0040 <= addr) && (addr <= (0x0040)+(16*7)) && ((addr-0x0040)%16 == 0)) {
        uint i = (addr - 0x0040)/16;
        PEGPROT_GEN                  [i] = (*(PEGPROT  [i]))["GEN"      ];
        PEGPROT_DBG                  [i] = (*(PEGPROT  [i]))["DBG"      ];
        PEGPROT_UM                   [i] = (*(PEGPROT  [i]))["UM"       ];
        PEGPROT_WG                   [i] = (*(PEGPROT  [i]))["WG"       ];
        PEGPROT_RG                   [i] = (*(PEGPROT  [i]))["RG"       ];
        return;
    }
    if ((0x0044 <= addr) && (addr <= (0x0044)+(16*7)) && ((addr-0x0044)%16 == 0)) {
        uint i = (addr - 0x0044)/16;
        PEGSPID_SPID                 [i] = (*(PEGSPID  [i]))["SPID"     ];
        return;
    }
    if ((0x0048 <= addr) && (addr <= (0x0048)+(16*7)) && ((addr-0x0048)%16 == 0)) {
        uint i = (addr - 0x0048)/16;
        PEGBAD_BAD                   [i] = (*(PEGBAD   [i]))["BAD"      ];
        return;
    }
    if ((0x004C <= addr) && (addr <= (0x004C)+(16*7)) && ((addr-0x004C)%16 == 0)) {
        uint i = (addr - 0x004C)/16;
        PEGADV_ADV                   [i] = (*(PEGADV   [i]))["ADV"      ];
        return;
    }
}
...
/// @return None
void Cg4_peg_u2a_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*PEGKCPROT)["KCPROT"   ] = PEGKCPROT_KCPROT;
        (*PEGKCPROT)["KCE"      ] = PEGKCPROT_KCE;
        return;
    }
    if ((0x0040 <= addr) && (addr <= (0x0040)+(16*7)) && ((addr-0x0040)%16 == 0)) {
        uint i = (addr - 0x0040)/16;
        (*(PEGPROT  [i]))["GEN"      ] = PEGPROT_GEN[i];
        (*(PEGPROT  [i]))["DBG"      ] = PEGPROT_DBG[i];
        (*(PEGPROT  [i]))["UM"       ] = PEGPROT_UM[i];
        (*(PEGPROT  [i]))["WG"       ] = PEGPROT_WG[i];
        (*(PEGPROT  [i]))["RG"       ] = PEGPROT_RG[i];
        return;
    }
    if ((0x0044 <= addr) && (addr <= (0x0044)+(16*7)) && ((addr-0x0044)%16 == 0)) {
        uint i = (addr - 0x0044)/16;
        (*(PEGSPID  [i]))["SPID"     ] = PEGSPID_SPID[i];
        return;
    }
    if ((0x0048 <= addr) && (addr <= (0x0048)+(16*7)) && ((addr-0x0048)%16 == 0)) {
        uint i = (addr - 0x0048)/16;
        (*(PEGBAD   [i]))["BAD"      ] = PEGBAD_BAD[i];
        return;
    }
    if ((0x004C <= addr) && (addr <= (0x004C)+(16*7)) && ((addr-0x004C)%16 == 0)) {
        uint i = (addr - 0x004C)/16;
        (*(PEGADV   [i]))["ADV"      ] = PEGADV_ADV[i];
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cg4_peg_u2a_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cg4_peg_u2a_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cg4_peg_u2a_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cg4_peg_u2a_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cg4_peg_u2a_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cg4_peg_u2a_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cg4_peg_u2a_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Cg4_peg_u2a_regif::uint Cg4_peg_u2a_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return none
void Cg4_peg_u2a_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cg4_peg_u2a_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cg4_peg_u2a_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cg4_peg_u2a_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Cg4_peg_u2a_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cg4_peg_u2a_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cg4_peg_u2a_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
