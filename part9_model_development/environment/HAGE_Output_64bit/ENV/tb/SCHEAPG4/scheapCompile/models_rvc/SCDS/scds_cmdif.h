...
#endif // EnableDumpResult
    ret += "    SetRegEnable <index> <enable>             Set register implemented or not implemented\n";
    ret += "    SetRegSize <index> <size>                 Set register bit size\n";
    ret += "    SetCLKfreq <clk_name> <clk_freq>          Set clock frequency for pclk port\n";
...
std::string user_def_command(std::vector<std::string>& args)
{
  ...
  if (0) {}
    ...
    if ((int)args.size() > 2 && (int)args.size() <= 3) {
      ...
      ret_val = SetRegEnable(index, enable);
      ret = ret_val;
    }
    else {
      ret = err_msg + "wrong number of arguments" + specified_cmd;
    ...
    if ((int)args.size() > 2 && (int)args.size() <= 3) {
      ...
      if ((int)args.size() >= 3) {
        ...
        if (!str2num(args[2], size)) {
          ret = err_msg + "wrong argument" + specified_cmd;
          return ret;
        }
      }
      ret_val = SetRegSize(index, size);
      ret = ret_val;
    }
    else {
      ret = err_msg + "wrong number of arguments" + specified_cmd;
