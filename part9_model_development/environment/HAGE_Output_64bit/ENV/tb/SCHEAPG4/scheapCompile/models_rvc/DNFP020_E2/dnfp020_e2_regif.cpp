// -----------------------------------------------------------------------------
// $Id: dnfp020_e2_regif.cpp 2241 2017-03-08 02:30:35Z ChinhTD $
//
...
//
// Input file : dnfp020_e2_regif.txt
////////////////////////////////////////////////////////////////////////////////
// %MODULE DNFP020_E2
//     #              name      offset_size
...
// 
////////////////////////////////////////////////////////////////////////////////
/// @file dnfp020_e2_regif.cpp
/// @brief Register IF class of model DNFP020_E2
/// $Id: dnfp020_e2_regif.cpp 2241 2017-03-08 02:30:35Z ChinhTD $
...
/// @return none
Cdnfp020_e2_regif::Cdnfp020_e2_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    DNFP02CTL = new vpcl::re_register(0x0000, this, "DNFP02CTL", name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_DNFP02CTL  , DNFP02CTL->name(), DNFP02CTL->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(DNFP02CTL  , mCurReg, 0,  1,       "8", "8|16|32");

    mRegList = mCurReg;

    (*DNFP02CTL  ) ( 6,  5, "SLST"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*DNFP02CTL  ) ( 2,  0, "PRS"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["DNFP02CTL"]["SLST"   ] = &Cdnfp020_e2_regif::cb_DNFP02CTL_SLST;
    mWrCbAPI["DNFP02CTL"]["PRS"    ] = &Cdnfp020_e2_regif::cb_DNFP02CTL_PRS;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cdnfp020_e2_regif::~Cdnfp020_e2_regif()
{
...
Cdnfp020_e2_regif::uint Cdnfp020_e2_regif::bit_select(
...
bool Cdnfp020_e2_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cdnfp020_e2_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cdnfp020_e2_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                   const unsigned char *p_data,   ///< [in] Writing data
                                   cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cdnfp020_e2_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                   unsigned char *p_data,   ///< [out] Reading data
                                   cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cdnfp020_e2_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                       const unsigned char *p_data,  ///< [in] Writing data
                                       cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cdnfp020_e2_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                       unsigned char *p_data,   ///< [out] Reading data
                                       cuint size,              ///< [in]  Data size (byte)
                                       bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cdnfp020_e2_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                    unsigned char *p_data,          ///< [out] Reading data
                                    cuint size,                     ///< [in]  Data size (byte)
...
bool Cdnfp020_e2_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                    const unsigned char *p_data,      ///< [in] Writing data
                                    cuint size,                       ///< [in] Data size (byte)
                                    cuint reg_index,                  ///< [in] Register index
...
bool Cdnfp020_e2_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cdnfp020_e2_regif::CommandInit()
{
...
/// @return string
std::string Cdnfp020_e2_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cdnfp020_e2_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cdnfp020_e2_regif::InitLocalVal()
{
    // Initialize local variables
    DNFP02CTL_SLST = 0x0;
    DNFP02CTL_PRS = 0x0;
...
/// @return None
void Cdnfp020_e2_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        DNFP02CTL_SLST                = (*DNFP02CTL)["SLST"     ];
        DNFP02CTL_PRS                 = (*DNFP02CTL)["PRS"      ];
        return;
    }
}
...
/// @return None
void Cdnfp020_e2_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*DNFP02CTL)["SLST"     ] = DNFP02CTL_SLST;
        (*DNFP02CTL)["PRS"      ] = DNFP02CTL_PRS;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cdnfp020_e2_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cdnfp020_e2_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cdnfp020_e2_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cdnfp020_e2_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cdnfp020_e2_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cdnfp020_e2_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cdnfp020_e2_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cdnfp020_e2_regif::uint Cdnfp020_e2_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cdnfp020_e2_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cdnfp020_e2_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cdnfp020_e2_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cdnfp020_e2_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
unsigned char Cdnfp020_e2_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
unsigned char Cdnfp020_e2_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(unsigned char));
    return sizeof(unsigned char);
}
void Cdnfp020_e2_regif::cw_instantiate_reg(scml2::reg<unsigned char> * reg, std::string reg_name, int offset)
{
