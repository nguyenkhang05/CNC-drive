// -----------------------------------------------------------------------------
// $Id: ahbfsguard_regif.cpp,v 1.8 2014/11/03 12:54:27 dungvannguyen Exp $
//
...
//
// Input file : ahbfsguard_regif.txt
////////////////////////////////////////////////////////////////////////////////
// %MODULE AHBFSGuard
//     #                   name            offset_size
...
////////////////////////////////////////////////////////////////////////////////
/// @file ahbfsguard_regif.cpp
/// @brief Register IF class of model AHBFSGUARD
/// $Id: ahbfsguard_regif.cpp,v 1.8 2014/11/03 12:54:27 dungvannguyen Exp $
/// $Date: 2014/11/03 12:54:27 $
/// $Revison$
...
/// @return none
Cahbfsguard_regif::Cahbfsguard_regif(std::string name, uint buswidth, uint version, uint filter_number)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    mFilterNumber = filter_number;
    std::ostringstream str_tmp;
    for(uint i = 0; i < mFilterNumber; i++) {
        str_tmp.str("");
        str_tmp<<"PROT"<< i;
        if (version == emCC_CUBE) {
            PROT[i] = new vpcl::re_register((0x0000) + 8*i, this, str_tmp.str(), name.c_str());
        } else {//Other version
            PROT[i] = new vpcl::re_register((0x0000) + 4*i, this, str_tmp.str(), name.c_str());
        }
    }
    unsigned int byte_num = 4;
    if (version == emCC_CUBE) {
        byte_num = 8;
        for(uint i = 0; i < mFilterNumber; i++) {
            str_tmp.str("");
            str_tmp<<"SPID"<< i;
            SPID[i] = new vpcl::re_register((0x0004) + 8*i, this, str_tmp.str(), name.c_str());
        }
    }
    unsigned int total_byte_num = filter_number * byte_num;
    mMaskedBitNumber = 0;
    while ((1 << mMaskedBitNumber) < total_byte_num) {
        mMaskedBitNumber++;
    }
    ...
    uint index = 0;
    mRegMap = new uint [1<<mMaskedBitNumber];
    for (uint i = 0; i < (1<<mMaskedBitNumber); i++) {
        mRegMap[i] = (1<<mMaskedBitNumber);
    }

    for(uint i = 0; i < mFilterNumber; i++) {
        if (version == emCC_CUBE) {
            mRegMap[(0x0000)     + i*8 ] = index;
            mRegMap[(0x0000 + 1) + i*8 ] = index;
            mRegMap[(0x0000 + 2) + i*8 ] = index;
            mRegMap[(0x0000 + 3) + i*8 ] = index++;
        } else {//Other version
    ...
    if (version == emCC_CUBE) {
        for(uint i = 0; i < mFilterNumber; i++) {
            mRegMap[(0x0004)     + i*8 ] = index;
            mRegMap[(0x0004 + 1) + i*8 ] = index;
            mRegMap[(0x0004 + 2) + i*8 ] = index;
            mRegMap[(0x0004 + 3) + i*8 ] = index++;
        }
    }
    mTotalRegNum = index;
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    for(uint i = 0; i < mFilterNumber; i++) cw_instantiate_reg(CW_PROT  [i], PROT[i]->name(), PROT[i]->addr());
    if (version == emCC_CUBE) {
        for(uint i = 0; i < mFilterNumber; i++) cw_instantiate_reg(CW_SPID  [i], SPID[i]->name(), SPID[i]->addr());
    }
    #endif
    for(uint i = 0; i < mFilterNumber; i++) mCurReg = mRegArray[index++] = new SRegList(PROT  [i], mCurReg, i,  4, "8|16|32", "8|16|32");
    if (version == emCC_CUBE) {
        for(uint i = 0; i < mFilterNumber; i++) mCurReg = mRegArray[index++] = new SRegList(SPID  [i], mCurReg, i,  4, "8|16|32", "8|16|32");
    }

    mRegList = mCurReg;

    for(uint i = 0; i < mFilterNumber; i++) {
        (*PROT  [i]) (31, 31, "LOCK"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*PROT  [i]) (26, 26, "VM"      , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*PROT  [i]) (25, 25, "UM"      , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*PROT  [i]) (24, 17, "PEID"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*PROT  [i]) (16,  9, "VCID"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        if (version == emOTHERS) {
            (*PROT[i]) ( 8,  5, "SPID", 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        }
        (*PROT  [i]) ( 4,  4, "DEB"     , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*PROT  [i]) ( 3,  3, "RDPDEF"  , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*PROT  [i]) ( 2,  2, "WRPDEF"  , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*PROT  [i]) ( 1,  1, "RD"      , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*PROT  [i]) ( 0,  0, "WR"      , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    }
    if (version == emCC_CUBE) {
        for(uint i = 0; i < mFilterNumber; i++) {
            (*SPID  [i]) (31,  0, "SPID"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        }
    }
    ...
    for(uint i = 0; i < mFilterNumber; i++) {
        str_tmp.str("");
        str_tmp<<"PROT"<< i;
        mWrCbAPI[str_tmp.str()]["LOCK"] = &Cahbfsguard_regif::cb_PROT_LOCK;
    }
    InitLocalVal();
}
...
/// @return none
Cahbfsguard_regif::~Cahbfsguard_regif()
{
...
Cahbfsguard_regif::uint Cahbfsguard_regif::bit_select(
...
bool Cahbfsguard_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cahbfsguard_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cahbfsguard_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                   const unsigned char *p_data,   ///< [in] Writing data
                                   cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cahbfsguard_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                   unsigned char *p_data,   ///< [out] Reading data
                                   cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cahbfsguard_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                       const unsigned char *p_data,  ///< [in] Writing data
                                       cuint size,                   ///< [in] Data size (byte)
                                       bool IsDbgFunc)               ///< [in] flag indicate reg_rd or reg_rd_dbg
{
    ...
    uint masked_addr = bit_select(tmp_addr, 0, mMaskedBitNumber-1);
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cahbfsguard_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                       unsigned char *p_data,   ///< [out] Reading data
                                       cuint size,              ///< [in]  Data size (byte)
                                       bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cahbfsguard_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                    unsigned char *p_data,          ///< [out] Reading data
                                    cuint size,                     ///< [in]  Data size (byte)
...
bool Cahbfsguard_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                    const unsigned char *p_data,      ///< [in] Writing data
                                    cuint size,                       ///< [in] Data size (byte)
                                    cuint reg_index,                  ///< [in] Register index
...
bool Cahbfsguard_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cahbfsguard_regif::CommandInit()
{
...
/// @return string
std::string Cahbfsguard_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cahbfsguard_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cahbfsguard_regif::InitLocalVal()
{
    // Initialize local variables
    for(uint i = 0; i < mFilterNumber; i++) {
        PROT_LOCK[i] = 0x0;
...
/// @return None
void Cahbfsguard_regif::UpdateLocalVal(cuint addr)
{
    if (mVersion == emCC_CUBE) {
        if ((0x0000 <= addr) && (addr <= (0x0000)+(8*15)) && ((addr-0x0000)%8 == 0)) {
            uint i = (addr - 0x0000)/8;
            PROT_LOCK               [i] = (*(PROT[i]))["LOCK"];
            PROT_VM                 [i] = (*(PROT[i]))["VM"  ];
            PROT_UM                 [i] = (*(PROT[i]))["UM"  ];
            PROT_PEID               [i] = (*(PROT[i]))["PEID"];
            PROT_VCID               [i] = (*(PROT[i]))["VCID"];
            PROT_DEB                [i] = (*(PROT[i]))["DEB" ];
            PROT_RDPDEF             [i] = (*(PROT[i]))["RDPDEF"];
            PROT_WRPDEF             [i] = (*(PROT[i]))["WRPDEF"];
            PROT_RD                 [i] = (*(PROT[i]))["RD"  ];
            PROT_WR                 [i] = (*(PROT[i]))["WR"  ];
            return;
        }
    ...
    if (mVersion == emCC_CUBE) {
        if ((0x0004 <= addr) && (addr <= (0x0004)+(8*15)) && ((addr-0x0004)%8 == 0)) {
            uint i = (addr - 0x0004)/8;
            SPID_SPID               [i] = (*(SPID[i]))["SPID"];
            return;
        }
    }
}
...
/// @return None
void Cahbfsguard_regif::UpdateRegVal(cuint addr)
{
    if (mVersion == emCC_CUBE) {
        if ((0x0000 <= addr) && (addr <= (0x0000)+(8*15)) && ((addr-0x0000)%8 == 0)) {
            uint i = (addr - 0x0000)/8;
            (*(PROT[i]))["LOCK"] = PROT_LOCK[i];
            (*(PROT[i]))["VM"  ] = PROT_VM[i];
            (*(PROT[i]))["UM"  ] = PROT_UM[i];
            (*(PROT[i]))["PEID"] = PROT_PEID[i];
            (*(PROT[i]))["VCID"] = PROT_VCID[i];
            (*(PROT[i]))["DEB" ] = PROT_DEB[i];
            (*(PROT[i]))["RDPDEF"] = PROT_RDPDEF[i];
            (*(PROT[i]))["WRPDEF"] = PROT_WRPDEF[i];
            (*(PROT[i]))["RD"  ] = PROT_RD[i];
            (*(PROT[i]))["WR"  ] = PROT_WR[i];
            return;
        }
...
/// @return vector
std::vector<std::string> Cahbfsguard_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cahbfsguard_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cahbfsguard_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
    if(mDumpRegisterRW == true) {
        ...
        str_tmp<<operation;
        str_tmp<<" Size= "<<size;
        str_tmp<<" Addr= "<<Num2HexStr(addr,mMaskedBitNumber, false);
        if (operation == "R") {
...
int Cahbfsguard_regif::get_reg_index(cuint access_addr)
{
    if (mRegMap[access_addr] != (1<<mMaskedBitNumber)) {
...
vpcl::re_register *Cahbfsguard_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cahbfsguard_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cahbfsguard_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cahbfsguard_regif::uint Cahbfsguard_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cahbfsguard_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cahbfsguard_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cahbfsguard_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cahbfsguard_regif::EnableReset(const bool is_active)
{
    ...
    for (uint i = 0; i < mTotalRegNum; i++) {
        ...
        unsigned int filter_no = mFilterNumber;
        if (is_active) {
           while (filter_no > 0) {
               filter_no--;
               if ((mRegArray[i]->my_p->name() == PROT[filter_no]->name()) ||
                   ((mVersion == emCC_CUBE) && (mRegArray[i]->my_p->name() == SPID[filter_no]->name()))) {
...
#ifdef CWR_SYSTEMC
uint Cahbfsguard_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cahbfsguard_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cahbfsguard_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
