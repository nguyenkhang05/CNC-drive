...
//      $Id: gen_cmdif.py,v 1.26 2013/07/26 06:10:52 sontran Exp
//      $Id: handleCommand.skl,v 1.18 2013/07/26 06:11:11 sontran Exp 
...
//   %%BIT    LBISTREF1b  19      0       0xA5A5A   R        -         -
//
//%REG_NAME  LBISTREF2
//   %%TITLE  name        upper   lower   init      access   support   callback
//   %%BIT    LBISTREF2b  19      0       0x5A5A5   R        -         -
//
//%REG_NAME  MBISTREF1
//   %%TITLE  name        upper   lower   init      access   support   callback
//   %%BIT    MBISTREF1b  19      0       0xAA55A   R        -         -
//
//%REG_NAME  MBISTREF2
//   %%TITLE  name        upper   lower   init      access   support   callback
//   %%BIT    MBISTREF2b  19      0       0x55AA5   R        -         -
//
//%REG_NAME  LBISTSIG1
//   %%TITLE  name        upper   lower   init      access   support   callback
//   %%BIT    LBISTSIG1b  19      0       0x5A5A5   R        -         -
//
//%REG_NAME  LBISTSIG2
//   %%TITLE  name        upper   lower   init      access   support   callback
//   %%BIT    LBISTSIG2b  19      0       0xA5A5A   R        -         -
//
//%REG_NAME  MBISTSIG1
//   %%TITLE  name        upper   lower   init      access   support   callback
//   %%BIT    MBISTSIG1b  19      0       0x55AA5   R        -         -
//
//%REG_NAME  MBISTSIG2
//   %%TITLE  name        upper   lower   init      access   support   callback
//   %%BIT    MBISTSIG2b  19      0       0xAA55A   R        -         -
//
...
#endif // EnableDumpResult
    ret += "    BISTProcessingTime <time>                 The time from receiving the start_bist flag to sending the end_bist flag. ( Default:20  )\n";
...
std::string user_def_command(std::vector<std::string>& args)
{
  ...
  if (0) {}
  else if (args[0] == "BISTProcessingTime") {
    // read mode
    if (0) {;}
    else if ((int)args.size() == 1) {
      std::ostringstream stream;
      stream << BISTProcessingTime;
      ret = stream.str();
    }
    // write mode
    else if ((int)args.size() == 2) {
      {
        ...
        double backup = BISTProcessingTime;
        is >> BISTProcessingTime;
        
        if (is.fail() || !is.eof()) {
          BISTProcessingTime = backup;
