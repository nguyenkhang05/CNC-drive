// -----------------------------------------------------------------------------
// $Id: mecccap_dmden_regif.cpp,v 1.2 2019/03/05 04:17:12 synguyen Exp $
//
...
/// @return none
Cmecccap_dmden_regif::Cmecccap_dmden_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    DE_ERRINT  = new vpcl::re_register(0x0000, this, "DE_ERRINT" , name.c_str());
    DE_SSTCLR  = new vpcl::re_register(0x0010, this, "DE_SSTCLR" , name.c_str());
    DE_DSTCLR  = new vpcl::re_register(0x0014, this, "DE_DSTCLR" , name.c_str());
    DE_OVFCLR  = new vpcl::re_register(0x0018, this, "DE_OVFCLR" , name.c_str());
    DE_SERSTR  = new vpcl::re_register(0x0020, this, "DE_SERSTR" , name.c_str());
    DE_DERSTR  = new vpcl::re_register(0x0024, this, "DE_DERSTR" , name.c_str());
    DE_OVFSTR  = new vpcl::re_register(0x0028, this, "DE_OVFSTR" , name.c_str());
    DE_SERINF  = new vpcl::re_register(0x0030, this, "DE_SERINF" , name.c_str());
    DE_00DEADR = new vpcl::re_register(0x00F0, this, "DE_00DEADR", name.c_str());
    std::ostringstream str_tmp;
    for(uint i = 0; i < emNUM_DE_nSEADR; i++) {
        str_tmp.str("");
        str_tmp<<"DE_nSEADR"<< i;
        DE_nSEADR [i] = new vpcl::re_register((0x0070) + 4*i, this, str_tmp.str(), name.c_str());
    }
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_DE_ERRINT   , DE_ERRINT->name() , DE_ERRINT->addr() );
    cw_instantiate_reg(CW_DE_SSTCLR   , DE_SSTCLR->name() , DE_SSTCLR->addr() );
    cw_instantiate_reg(CW_DE_DSTCLR   , DE_DSTCLR->name() , DE_DSTCLR->addr() );
    cw_instantiate_reg(CW_DE_OVFCLR   , DE_OVFCLR->name() , DE_OVFCLR->addr() );
    cw_instantiate_reg(CW_DE_SERSTR   , DE_SERSTR->name() , DE_SERSTR->addr() );
    cw_instantiate_reg(CW_DE_DERSTR   , DE_DERSTR->name() , DE_DERSTR->addr() );
    cw_instantiate_reg(CW_DE_OVFSTR   , DE_OVFSTR->name() , DE_OVFSTR->addr() );
    cw_instantiate_reg(CW_DE_SERINF   , DE_SERINF->name() , DE_SERINF->addr() );
    for(uint i = 0; i < emNUM_DE_nSEADR ; i++) cw_instantiate_reg(CW_DE_nSEADR   [i], DE_nSEADR [i]->name(), DE_nSEADR [i]->addr());
    cw_instantiate_reg(CW_DE_00DEADR  , DE_00DEADR->name(), DE_00DEADR->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(DE_ERRINT   , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DE_SSTCLR   , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DE_DSTCLR   , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DE_OVFCLR   , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DE_SERSTR   , mCurReg, 0,  4,      "32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DE_DERSTR   , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DE_OVFSTR   , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DE_SERINF   , mCurReg, 0,  4, "8|16|32", "8|16|32");
    for(uint i = 0; i < emNUM_DE_nSEADR ; i++) mCurReg = mRegArray[index++] = new SRegList(DE_nSEADR   [i], mCurReg, i,  4,      "32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(DE_00DEADR  , mCurReg, 0,  4,      "32", "8|16|32");

    mRegList = mCurReg;

    (*DE_ERRINT   ) ( 7,  7, "SEOVFIE" , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
    (*DE_ERRINT   ) ( 1,  1, "DEDIE"   , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
    (*DE_ERRINT   ) ( 0,  0, "SEDIE"   , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
    (*DE_SSTCLR   ) ( 0,  0, "SSTCLR00", 0x0       , "W"     , vpcl::SPP_ENABLE);
    (*DE_DSTCLR   ) ( 0,  0, "DSTCLR00", 0x0       , "W"     , vpcl::SPP_ENABLE);
    (*DE_OVFCLR   ) ( 1,  1, "SERROVFCLR1", 0x0       , "W"     , vpcl::SPP_ENABLE);
    (*DE_OVFCLR   ) ( 0,  0, "SERROVFCLR0", 0x0       , "W"     , vpcl::SPP_ENABLE);
    (*DE_SERSTR   ) ( 0,  0, "SEDF00"  , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*DE_DERSTR   ) ( 1,  1, "DEDF00"  , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*DE_OVFSTR   ) ( 1,  1, "SERROVF1", 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*DE_OVFSTR   ) ( 0,  0, "SERROVF0", 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*DE_SERINF   ) ( 0,  0, "SEDLINF00", 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*DE_00DEADR  ) (31, 27, "DEDL"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*DE_00DEADR  ) (26,  2, "DEADR0"  , 0x0       , "R"     , vpcl::SPP_ENABLE);
    for(uint i = 0; i < emNUM_DE_nSEADR; i++) {
        (*DE_nSEADR   [i]) (31, 27, "SEDL"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*DE_nSEADR   [i]) (26,  2, "SEADR0"  , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    ...
    mWrCbAPI["DE_SSTCLR"]["SSTCLR00"] = &Cmecccap_dmden_regif::cb_DE_SSTCLR_SSTCLR00;
    mWrCbAPI["DE_DSTCLR"]["DSTCLR00"] = &Cmecccap_dmden_regif::cb_DE_DSTCLR_DSTCLR00;
    mWrCbAPI["DE_OVFCLR"]["SERROVFCLR0"] = &Cmecccap_dmden_regif::cb_DE_OVFCLR_SERROVFCLR0;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cmecccap_dmden_regif::~Cmecccap_dmden_regif()
{
...
Cmecccap_dmden_regif::uint Cmecccap_dmden_regif::bit_select(
...
bool Cmecccap_dmden_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cmecccap_dmden_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cmecccap_dmden_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                      const unsigned char *p_data,   ///< [in] Writing data
                                      cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cmecccap_dmden_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                      unsigned char *p_data,   ///< [out] Reading data
                                      cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cmecccap_dmden_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                          const unsigned char *p_data,  ///< [in] Writing data
                                          cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cmecccap_dmden_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                          unsigned char *p_data,   ///< [out] Reading data
                                          cuint size,              ///< [in]  Data size (byte)
                                          bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cmecccap_dmden_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                       unsigned char *p_data,          ///< [out] Reading data
                                       cuint size,                     ///< [in]  Data size (byte)
...
bool Cmecccap_dmden_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                       const unsigned char *p_data,      ///< [in] Writing data
                                       cuint size,                       ///< [in] Data size (byte)
                                       cuint reg_index,                  ///< [in] Register index
...
bool Cmecccap_dmden_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cmecccap_dmden_regif::CommandInit()
{
...
/// @return string
std::string Cmecccap_dmden_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cmecccap_dmden_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cmecccap_dmden_regif::InitLocalVal()
{
    // Initialize local variables
    DE_ERRINT_SEOVFIE = 0x1;
    DE_ERRINT_DEDIE = 0x1;
    DE_ERRINT_SEDIE = 0x1;
    DE_SSTCLR_SSTCLR00 = 0x0;
    DE_DSTCLR_DSTCLR00 = 0x0;
    DE_OVFCLR_SERROVFCLR1 = 0x0;
    DE_OVFCLR_SERROVFCLR0 = 0x0;
    DE_SERSTR_SEDF00 = 0x0;
    DE_DERSTR_DEDF00 = 0x0;
    DE_OVFSTR_SERROVF1 = 0x0;
    DE_OVFSTR_SERROVF0 = 0x0;
    DE_SERINF_SEDLINF00 = 0x0;
    for(uint i = 0; i < emNUM_DE_nSEADR; i++) {
        DE_nSEADR_SEDL[i] = 0x0;
        DE_nSEADR_SEADR0[i] = 0x0;
    }
    DE_00DEADR_DEDL = 0x0;
    DE_00DEADR_DEADR0 = 0x0;
}

/// Update bit value to local value
/// @return None
void Cmecccap_dmden_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        DE_ERRINT_SEOVFIE              = (*DE_ERRINT )["SEOVFIE"   ];
        DE_ERRINT_DEDIE                = (*DE_ERRINT )["DEDIE"     ];
        DE_ERRINT_SEDIE                = (*DE_ERRINT )["SEDIE"     ];
        return;
    }
    if (addr == 0x0010) {
        DE_SSTCLR_SSTCLR00             = (*DE_SSTCLR )["SSTCLR00"  ];
        return;
    }
    if (addr == 0x0014) {
        DE_DSTCLR_DSTCLR00             = (*DE_DSTCLR )["DSTCLR00"  ];
        return;
    }
    if (addr == 0x0018) {
        DE_OVFCLR_SERROVFCLR1          = (*DE_OVFCLR )["SERROVFCLR1"];
        DE_OVFCLR_SERROVFCLR0          = (*DE_OVFCLR )["SERROVFCLR0"];
        return;
    }
    if (addr == 0x0020) {
        DE_SERSTR_SEDF00               = (*DE_SERSTR )["SEDF00"    ];
        return;
    }
    if (addr == 0x0024) {
        DE_DERSTR_DEDF00               = (*DE_DERSTR )["DEDF00"    ];
        return;
    }
    if (addr == 0x0028) {
        DE_OVFSTR_SERROVF1             = (*DE_OVFSTR )["SERROVF1"  ];
        DE_OVFSTR_SERROVF0             = (*DE_OVFSTR )["SERROVF0"  ];
        return;
    }
    if (addr == 0x0030) {
        DE_SERINF_SEDLINF00            = (*DE_SERINF )["SEDLINF00" ];
        return;
    }
    if ((0x0070 <= addr) && (addr <= (0x0070)+(4*31)) && ((addr-0x0070)%4 == 0)) {
        uint i = (addr - 0x0070)/4;
        DE_nSEADR_SEDL                [i] = (*(DE_nSEADR [i]))["SEDL"      ];
        DE_nSEADR_SEADR0              [i] = (*(DE_nSEADR [i]))["SEADR0"    ];
        return;
    }
    if (addr == 0x00F0) {
        DE_00DEADR_DEDL                = (*DE_00DEADR)["DEDL"      ];
        DE_00DEADR_DEADR0              = (*DE_00DEADR)["DEADR0"    ];
        return;
    }
}
...
/// @return None
void Cmecccap_dmden_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*DE_ERRINT )["SEOVFIE"   ] = DE_ERRINT_SEOVFIE;
        (*DE_ERRINT )["DEDIE"     ] = DE_ERRINT_DEDIE;
        (*DE_ERRINT )["SEDIE"     ] = DE_ERRINT_SEDIE;
        return;
    }
    if (addr == 0x0010) {
        (*DE_SSTCLR )["SSTCLR00"  ] = DE_SSTCLR_SSTCLR00;
        return;
    }
    if (addr == 0x0014) {
        (*DE_DSTCLR )["DSTCLR00"  ] = DE_DSTCLR_DSTCLR00;
        return;
    }
    if (addr == 0x0018) {
        (*DE_OVFCLR )["SERROVFCLR1"] = DE_OVFCLR_SERROVFCLR1;
        (*DE_OVFCLR )["SERROVFCLR0"] = DE_OVFCLR_SERROVFCLR0;
        return;
    }
    if (addr == 0x0020) {
        (*DE_SERSTR )["SEDF00"    ] = DE_SERSTR_SEDF00;
        return;
    }
    if (addr == 0x0024) {
        (*DE_DERSTR )["DEDF00"    ] = DE_DERSTR_DEDF00;
        return;
    }
    if (addr == 0x0028) {
        (*DE_OVFSTR )["SERROVF1"  ] = DE_OVFSTR_SERROVF1;
        (*DE_OVFSTR )["SERROVF0"  ] = DE_OVFSTR_SERROVF0;
        return;
    }
    if (addr == 0x0030) {
        (*DE_SERINF )["SEDLINF00" ] = DE_SERINF_SEDLINF00;
        return;
    }
    if ((0x0070 <= addr) && (addr <= (0x0070)+(4*31)) && ((addr-0x0070)%4 == 0)) {
        uint i = (addr - 0x0070)/4;
        (*(DE_nSEADR [i]))["SEDL"      ] = DE_nSEADR_SEDL[i];
        (*(DE_nSEADR [i]))["SEADR0"    ] = DE_nSEADR_SEADR0[i];
        return;
    }
    if (addr == 0x00F0) {
        (*DE_00DEADR)["DEDL"      ] = DE_00DEADR_DEDL;
        (*DE_00DEADR)["DEADR0"    ] = DE_00DEADR_DEADR0;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cmecccap_dmden_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cmecccap_dmden_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cmecccap_dmden_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cmecccap_dmden_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cmecccap_dmden_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cmecccap_dmden_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cmecccap_dmden_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cmecccap_dmden_regif::uint Cmecccap_dmden_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cmecccap_dmden_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cmecccap_dmden_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cmecccap_dmden_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cmecccap_dmden_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cmecccap_dmden_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cmecccap_dmden_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cmecccap_dmden_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
