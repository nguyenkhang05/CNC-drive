// -----------------------------------------------------------------------------
// $Id: tsn_regif.cpp,v 1.0 2014/11/27 06:42:55 binhnguyen Exp $
//
...
//     %%REG    TSNCR      TSNCR       32      8|16|32     32      0x0     W|R     0       TRUE        W
//     %%REG    TSNSTAT    TSNSTAT     -       8|16|32     32      0x4     R       0       TRUE        -
//     %%REG    TSNDIAG    TSNDIAG     32      8|16|32     32      0x8     W|R     0       TRUE        W
// 
// %REG_NAME TSNCR
//     %%TITLE  name           upper   lower   init    access  support     callback
//     %%BIT    TSNEN          0       0       0       W|R     TRUE        W
// 
// %REG_NAME TSNSTAT
//     %%TITLE  name           upper   lower   init    access  support     callback
//     %%BIT    TSNST          0       0       0       R       TRUE        -
// 
// %REG_NAME TSNDIAG
//     %%TITLE  name           upper   lower   init    access  support     callback
//     %%BIT    TSNSELFDIAG    0       0       0       W|R     TRUE        W
////////////////////////////////////////////////////////////////////////////////
/// @file tsn_regif.cpp
/// @brief Register IF class of model TSN
/// $Id: tsn_regif.cpp,v 1.0 2014/11/27 06:42:55 binhnguyen Exp $
/// $Date: 2014/11/27 06:42:55 $
/// $Revison$
/// $Author: binhnguyen $
////////////////////////////////////////////////////////////////////////////////
#include "tsn_regif.h"
...
/// @return none
Ctsn_regif::Ctsn_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    TSNCR   = new vpcl::re_register(0x0000, this, "TSNCR"  , name.c_str());
    TSNSTAT = new vpcl::re_register(0x0004, this, "TSNSTAT", name.c_str());
    TSNDIAG = new vpcl::re_register(0x0008, this, "TSNDIAG", name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_TSNCR    , TSNCR->name()  , TSNCR->addr()  );
    cw_instantiate_reg(CW_TSNSTAT  , TSNSTAT->name(), TSNSTAT->addr());
    cw_instantiate_reg(CW_TSNDIAG  , TSNDIAG->name(), TSNDIAG->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(TSNCR    , mCurReg, 0,  4,      "32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(TSNSTAT  , mCurReg, 0,  4,      "32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(TSNDIAG  , mCurReg, 0,  4,      "32", "8|16|32");

    mRegList = mCurReg;

    (*TSNCR    ) ( 0,  0, "TSNEN"   , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*TSNSTAT  ) ( 0,  0, "TSNST"   , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*TSNDIAG  ) ( 0,  0, "TSNSELFDIAG", 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["TSNCR"]["TSNEN"] = &Ctsn_regif::cb_TSNCR_TSNEN;
    mWrCbAPI["TSNDIAG"]["TSNSELFDIAG"] = &Ctsn_regif::cb_TSNDIAG_TSNSELFDIAG;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Ctsn_regif::~Ctsn_regif()
{
...
Ctsn_regif::uint Ctsn_regif::bit_select(
...
bool Ctsn_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Ctsn_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Ctsn_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                            const unsigned char *p_data,   ///< [in] Writing data
                            cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Ctsn_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                            unsigned char *p_data,   ///< [out] Reading data
                            cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Ctsn_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                const unsigned char *p_data,  ///< [in] Writing data
                                cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Ctsn_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                unsigned char *p_data,   ///< [out] Reading data
                                cuint size,              ///< [in]  Data size (byte)
                                bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Ctsn_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                             unsigned char *p_data,          ///< [out] Reading data
                             cuint size,                     ///< [in]  Data size (byte)
...
bool Ctsn_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                             const unsigned char *p_data,      ///< [in] Writing data
                             cuint size,                       ///< [in] Data size (byte)
                             cuint reg_index,                  ///< [in] Register index
...
bool Ctsn_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return string
std::string Ctsn_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Ctsn_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Ctsn_regif::InitLocalVal()
{
    // Initialize local variables
    TSNCR_TSNEN = 0x0;
    TSNSTAT_TSNST = 0x0;
    TSNDIAG_TSNSELFDIAG = 0x0;
}

/// Update bit value to local value
/// @return None
void Ctsn_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
    ...
    if (addr == 0x0004) {
        TSNSTAT_TSNST               = (*TSNSTAT)["TSNST"  ];
        return;
    }
    if (addr == 0x0008) {
        TSNDIAG_TSNSELFDIAG         = (*TSNDIAG)["TSNSELFDIAG"];
        return;
    }
}
...
/// @return None
void Ctsn_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
    ...
    if (addr == 0x0004) {
        (*TSNSTAT)["TSNST"  ] = TSNSTAT_TSNST;
        return;
    }
    if (addr == 0x0008) {
        (*TSNDIAG)["TSNSELFDIAG"] = TSNDIAG_TSNSELFDIAG;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Ctsn_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Ctsn_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Ctsn_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Ctsn_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Ctsn_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Ctsn_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Ctsn_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Ctsn_regif::uint Ctsn_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Ctsn_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Ctsn_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Ctsn_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Ctsn_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Ctsn_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Ctsn_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Ctsn_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
