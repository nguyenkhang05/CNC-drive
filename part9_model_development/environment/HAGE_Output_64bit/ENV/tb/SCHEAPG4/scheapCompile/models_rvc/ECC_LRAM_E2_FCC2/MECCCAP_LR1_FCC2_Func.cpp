// ---------------------------------------------------------------------
// $Id: MECCCAP_LR1_FCC2_Func.cpp,v 1.8 2019/09/26 10:07:38 landang Exp $
//
...
//========================================================
/// Constructor
MECCCAP_LR1_FCC2_Func::MECCCAP_LR1_FCC2_Func (std::string name, ECC_LRAM_E2_FCC2_AgentController* LRAMAgentController):
                Cmecccap_lr1_fcc2_regif(name, 32)
                ,BusSlaveFuncIf()
{//{{{
    ...
    Cmecccap_lr1_fcc2_regif::set_instance_name(name);
}//}}}

/// Destructor
MECCCAP_LR1_FCC2_Func::~MECCCAP_LR1_FCC2_Func()
{//{{{
}//}}}

/// Reset Func and registers
void MECCCAP_LR1_FCC2_Func::EnableReset(const std::string reset_name, const bool is_active)
{//{{{
    if (reset_name == "sys_resetz"){
        Cmecccap_lr1_fcc2_regif::EnableReset(is_active);
    }
}//}}}

/// Read API (target socket)
void MECCCAP_LR1_FCC2_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void MECCCAP_LR1_FCC2_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void MECCCAP_LR1_FCC2_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cmecccap_lr1_fcc2_regif::reg_handle_command(cmd);
                if (msg_return != "") {
            ...
            msg_return = Cmecccap_lr1_fcc2_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
bool MECCCAP_LR1_FCC2_Func::Set1bitStatusAndAddress(unsigned int edl_code, unsigned int mod_address)
{//{{{
    unsigned int cap_depth = mpLRAMAgentController->GetCapDepth("LR1");
...
void MECCCAP_LR1_FCC2_Func::SetFlagAddressBufferOverflow()
{//{{{
...
bool MECCCAP_LR1_FCC2_Func::CheckHasRoomFor1bit()
{//{{{
    unsigned int cap_depth = mpLRAMAgentController->GetCapDepth("LR1");
    unsigned int status = (unsigned int)(*LR1_SERSTR);
    bool is_find_room = false;
...
/// Set FIFO overflow
void MECCCAP_LR1_FCC2_Func::SetFIFOOverflow ()
{//{{{
...
/// Get SEDL bit
unsigned int MECCCAP_LR1_FCC2_Func::GetSEDLBit(unsigned int index)
{//{{{
    return (unsigned int)(*LR1_nSEADR[index])["SEDL"];
}//}}}

/// Get SEADR bit
unsigned int MECCCAP_LR1_FCC2_Func::GetSEADRBit(unsigned int index)
{//{{{
    return (unsigned int)(*LR1_nSEADR[index])["SEADR0"];
}//}}}

/// Get DEDL bit
unsigned int MECCCAP_LR1_FCC2_Func::GetDEDLBit()
{//{{{
    return (unsigned int)(*LR1_00DEADR)["DEDL"];
}//}}}

/// Get DEADR bit
unsigned int MECCCAP_LR1_FCC2_Func::GetDEADRBit()
{//{{{
    return (unsigned int)(*LR1_00DEADR)["DEADR0"];
}//}}}

/// Get Start mask address
unsigned int MECCCAP_LR1_FCC2_Func::GetERMSA(unsigned int index)
{//{{{
...
unsigned int MECCCAP_LR1_FCC2_Func::GetERMEA(unsigned int index)
{//{{{
    unsigned int ret_val = 0;
    if (index == 0){
...
unsigned int MECCCAP_LR1_FCC2_Func::GetERM_EN(unsigned int index)
{//{{{
    unsigned int ret_val = 0;
    if (index == 0){
...
bool MECCCAP_LR1_FCC2_Func::CheckNoFatalStatus()
{//{{{
    return ((unsigned int)(*LR1_DERSTR) == 0);
}//}}}

/// Set 2bit error status
void MECCCAP_LR1_FCC2_Func::Set2bitErrorStatus()
{//{{{
    (*LR1_DERSTR)["DEDF00"] = 1;
}//}}}

/// Set Fatal error address and edl according
void MECCCAP_LR1_FCC2_Func::SetFatalAddress(unsigned int edl_code, unsigned int mod_address)
{//{{{
...
unsigned int MECCCAP_LR1_FCC2_Func::GetLR1_SERSTR()
{//{{{
    return (unsigned int)(*LR1_SERSTR);
}//}}}

/// Get value of LR1_DERSTR register
unsigned int MECCCAP_LR1_FCC2_Func::GetLR1_DERSTR()
{//{{{
    return (unsigned int)(*LR1_DERSTR);
}//}}}

/// Get value of LR1_OVFSTR register
unsigned int MECCCAP_LR1_FCC2_Func::GetLR1_OVFSTR()
{//{{{
    return (unsigned int)(*LR1_OVFSTR);
}//}}}

/// Write ERRINT_INIT to ERRINT reg after reset
void MECCCAP_LR1_FCC2_Func::WriteERRINT_INIT(unsigned int value)
{//{{{
...
/// Get ERRINT register
unsigned int MECCCAP_LR1_FCC2_Func::GetERRINT()
{//{{{
...
void MECCCAP_LR1_FCC2_Func::cb_LR1_SSTCLR_LR1_SSTCLR(RegCBstr str)
{//{{{ 
...
void MECCCAP_LR1_FCC2_Func::cb_LR1_DSTCLR_DSTCLR00(RegCBstr str)
{//{{{
    // clear DED.
...
void MECCCAP_LR1_FCC2_Func::cb_LR1_OVFCLR_SERROVFCLR0(RegCBstr str)
{//{{{ 
...
void MECCCAP_LR1_FCC2_Func::cb_LR1_ERMSA0_ERMSA(RegCBstr str)
{//{{{ 
...
void MECCCAP_LR1_FCC2_Func::cb_LR1_ERMEA0_ERMEA(RegCBstr str)
{//{{{ 
...
void MECCCAP_LR1_FCC2_Func::cb_LR1_ERMSA1_ERMSA(RegCBstr str)
{//{{{ 
...
void MECCCAP_LR1_FCC2_Func::cb_LR1_ERMEA1_ERMEA(RegCBstr str)
{//{{{ 
