...
/// @return none
Casf_regif::Casf_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
    , cwmem("register", 0x90)
    #endif
{
    CommandInit();
    CTL0 = new vpcl::re_register(0x0080, this, "CTL0", name.c_str());
    CTL1 = new vpcl::re_register(0x0084, this, "CTL1", name.c_str());
    CNT  = new vpcl::re_register(0x008C, this, "CNT" , name.c_str());
    std::ostringstream str_tmp;
    for(uint i = 0; i < emNUM_DR; i++) {
        str_tmp.str("");
        str_tmp<<"DR"<< i;
        DR  [i] = new vpcl::re_register((0x0000) + 4*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_CMP; i++) {
        str_tmp.str("");
        str_tmp<<"CMP"<< i;
        CMP [i] = new vpcl::re_register((0x0040) + 4*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_RTDR; i++) {
        str_tmp.str("");
        str_tmp<<"RTDR"<< i;
        RTDR[i] = new vpcl::re_register((0x0090) + 4*i, this, str_tmp.str(), name.c_str());
    }
    ...
    index = 0;
#ifdef CWR_SYSTEMC
    for(uint i = 0; i < emNUM_DR  ; i++) mCurReg = mRegArray[index++] = new SRegList(DR    [i], mCurReg, i,  4,      "32",      "32", false, &cwmem);
#else
    for(uint i = 0; i < emNUM_DR  ; i++) mCurReg = mRegArray[index++] = new SRegList(DR    [i], mCurReg, i,  4,      "32",      "32");
#endif
    for(uint i = 0; i < emNUM_CMP ; i++) mCurReg = mRegArray[index++] = new SRegList(CMP   [i], mCurReg, i,  2, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(CTL0  , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(CTL1  , mCurReg, 0,  1, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(CNT   , mCurReg, 0,  2,      "16",      "16");
    for(uint i = 0; i < emNUM_RTDR; i++) mCurReg = mRegArray[index++] = new SRegList(RTDR  [i], mCurReg, i,  4,      "32",      "32");

    mRegList = mCurReg;

    (*CTL0  ) (16, 16, "RTD"     , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*CTL0  ) (10,  8, "ACSL"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*CTL0  ) ( 3,  0, "CTAG"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*CTL1  ) ( 0,  0, "ST"      , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*CNT   ) ( 8,  0, "CNT"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    for(uint i = 0; i < emNUM_RTDR; i++) {
        (*RTDR  [i]) (31, 11, "RTDR"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*RTDR  [i]) ( 8,  0, "RTCNT"   , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_DR; i++) {
        (*DR    [i]) (31, 11, "DR"      , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_CMP; i++) {
        (*CMP   [i]) (15, 15, "ASIIE"   , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*CMP   [i]) ( 7,  7, "CHE"     , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*CMP   [i]) ( 4,  0, "CMP"     , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    }
    ...
    mWrCbAPI["CTL0"]["CTAG"] = &Casf_regif::cb_CTL0_CTAG;
    mWrCbAPI["CTL1"]["ST"] = &Casf_regif::cb_CTL1_ST;

    mRdCbAPI["CNT"]["CNT"] = &Casf_regif::cb_CNT_CNT;

    for(uint i = 0; i < emNUM_CMP; i++) {
        str_tmp.str("");
        str_tmp<<"CMP"<< i;
        mWrCbAPI[str_tmp.str()]["CMP"] = &Casf_regif::cb_CMP_CMP;
    }
    for(uint i = 0; i < emNUM_RTDR; i++) {
        str_tmp.str("");
        str_tmp<<"RTDR"<< i;
        mRdCbAPI[str_tmp.str()]["RTCNT"] = &Casf_regif::cb_RTDR_RTCNT;
    }
    InitLocalVal();
}
...
Casf_regif::uint Casf_regif::bit_select(cuint val,    ///< [in] Writting address
...
bool Casf_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Casf_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Casf_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                            const unsigned char *p_data,   ///< [in] Writing data
                            cuint size)                    ///< [in] Data size (byte)
{
    if (size == 0) {
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Casf_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                            unsigned char *p_data,   ///< [out] Reading data
                            cuint size)              ///< [in]  Data size (byte)
{
    if (size == 0) {
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Casf_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                const unsigned char *p_data,  ///< [in] Writing data
                                cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Casf_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                unsigned char *p_data,   ///< [out] Reading data
                                cuint size,              ///< [in]  Data size (byte)
                                bool IsDbgFunc)          ///< [in]  Flag indicate reg_rd or reg_rd_dbg
{
...
bool Casf_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                             unsigned char *p_data,          ///< [out] Reading data
                             cuint size,                     ///< [in]  Data size (byte)
...
bool Casf_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                             const unsigned char *p_data,      ///< [in] Writing data
                             cuint size,                       ///< [in] Data size (byte)
                             cuint reg_index,                  ///< [in] Register index
...
bool Casf_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr, bool is_wr)
{
...
/// @return string
std::string Casf_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Casf_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return None
void Casf_regif::UpdateLocalVal(cuint addr)
{
    ...
    if ((0x0040 <= addr) && (addr <= (0x0040)+(4*15)) && ((addr-0x0040)%4 == 0)) {
        ...
        CMP_ASIIE               [i] = (*(CMP [i]))["ASIIE"];
        CMP_CHE                 [i] = (*(CMP [i]))["CHE" ];
        CMP_CMP                 [i] = (*(CMP [i]))["CMP" ];
        return;
    }
    if ((0x0090 <= addr) && (addr <= (0x0090)+(4*15)) && ((addr-0x0090)%4 == 0)) {
        uint i = (addr - 0x0090)/4;
        RTDR_RTDR               [i] = (*(RTDR[i]))["RTDR"];
        RTDR_RTCNT              [i] = (*(RTDR[i]))["RTCNT"];
        return;
    }
    if (addr == 0x0080) {
        CTL0_RTD                 = (*CTL0)["RTD" ];
...
/// @return None
void Casf_regif::UpdateRegVal(cuint addr)
{
    if ((0x0000 <= addr) && (addr <= (0x0000)+(4*15)) && ((addr-0x0000)%4 == 0)) {
        uint i = (addr - 0x0000)/4;
        (*(DR  [i]))["DR"  ] = DR_DR[i];
    ...
    if ((0x0090 <= addr) && (addr <= (0x0090)+(4*15)) && ((addr-0x0090)%4 == 0)) {
        uint i = (addr - 0x0090)/4;
        (*(RTDR[i]))["RTDR"] = RTDR_RTDR[i];
        (*(RTDR[i]))["RTCNT"] = RTDR_RTCNT[i];
        return;
    }
    if (addr == 0x0080) {
        (*CTL0)["RTD" ] = CTL0_RTD;
...
/// @return vector
std::vector<std::string> Casf_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Casf_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip, bool is_wr_data, uint acc_size)
{
    std::ostringstream str_tmp;
...
/// @return none
void Casf_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
    if (mDumpRegisterRW) {
...
int Casf_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Casf_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Casf_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Casf_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Casf_regif::uint Casf_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
    uint data=0;
...
/// @return none
void Casf_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Casf_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Casf_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Casf_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Casf_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Casf_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    int reg_index = get_reg_index((uint)tag);
    assert(reg_index != (1<<16));
    uint length = mRegArray[reg_index]->length;
...
void Casf_regif::cw_set_all_callback_reg(void)
{
