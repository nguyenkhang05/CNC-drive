...
/// @return none
Csarad_u2b_regif::Csarad_u2b_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    ...
    #endif
{
    ...
    mWrCbAPI["ADSYNSTCR"]["ADSTART" ] = &Csarad_u2b_regif::cb_ADSYNSTCR_ADSTART;
    mWrCbAPI["ADTSYNSTCR"]["ADTSTART"] = &Csarad_u2b_regif::cb_ADTSYNSTCR_ADTSTART;
    mWrCbAPI["VMONVDCR1"]["VDE1"    ] = &Csarad_u2b_regif::cb_VMONVDCR1_VDE1;
    mWrCbAPI["VMONVDCR2"]["VDE2"    ] = &Csarad_u2b_regif::cb_VMONVDCR2_VDE2;

    #ifdef USE_WEB_SIM
    InitializeRegValueID();
    mNotifyRegValueIDAPI = &Csarad_u2b_regif::NotifyRegValueID;
    #endif
}

/// Destructor: delete pointers
/// @return none
Csarad_u2b_regif::~Csarad_u2b_regif()
{
...
Csarad_u2b_regif::uint Csarad_u2b_regif::bit_select(cuint val,    ///< [in] Writting address
...
bool Csarad_u2b_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Csarad_u2b_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Csarad_u2b_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                  const unsigned char *p_data,   ///< [in] Writing data
                                  cuint size)                    ///< [in] Data size (byte)
{
    assert(size != 0);
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Csarad_u2b_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                  unsigned char *p_data,   ///< [out] Reading data
                                  cuint size)              ///< [in]  Data size (byte)
{
    assert(size != 0);
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Csarad_u2b_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                      const unsigned char *p_data,  ///< [in] Writing data
                                      cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Csarad_u2b_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                      unsigned char *p_data,   ///< [out] Reading data
                                      cuint size,              ///< [in]  Data size (byte)
                                      bool IsDbgFunc)          ///< [in]  Flag indicate reg_rd or reg_rd_dbg
{
...
bool Csarad_u2b_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                   unsigned char *p_data,          ///< [out] Reading data
                                   cuint size,                     ///< [in]  Data size (byte)
...
bool Csarad_u2b_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                   const unsigned char *p_data,      ///< [in] Writing data
                                   cuint size,                       ///< [in] Data size (byte)
                                   cuint reg_index,                  ///< [in] Register index
...
bool Csarad_u2b_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr, bool is_wr)
{
...
/// @return none
void Csarad_u2b_regif::CommandInit()
{
...
/// @return string
std::string Csarad_u2b_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Csarad_u2b_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Csarad_u2b_regif::InitializeRegValueID()
{//{{{
...
/// @return none
void Csarad_u2b_regif::CalcTargetRegVal(void)
{//{{{
...
/// @return none
void Csarad_u2b_regif::IssueRegValueID(void)
{//{{{
...
/// @return vector
std::vector<std::string> Csarad_u2b_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Csarad_u2b_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip, bool is_wr_data, uint acc_size)
{
    std::ostringstream str_tmp;
...
/// @return none
void Csarad_u2b_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
    if (mDumpRegisterRW) {
...
int Csarad_u2b_regif::get_reg_index(cuint access_addr)
{
    if (access_addr < (1<<11)) {
...
vpcl::re_register *Csarad_u2b_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Csarad_u2b_regif::next_reg_object()
{
    assert(mCurReg != NULL);
...
/// @return none
void Csarad_u2b_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Csarad_u2b_regif::uint Csarad_u2b_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
    uint data=0;
...
/// @return none
void Csarad_u2b_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Csarad_u2b_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Csarad_u2b_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Csarad_u2b_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
unsigned char Csarad_u2b_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
unsigned char Csarad_u2b_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    int reg_index = get_reg_index((uint)tag);
    assert(reg_index != (1<<16));
    uint length = mRegArray[reg_index]->length;
...
void Csarad_u2b_regif::cw_set_all_callback_reg(void)
{
