// -----------------------------------------------------------------------------
// $Id: ecccnt_cfc_fcc2_regif.cpp,v 1.5 2019/09/26 10:07:02 landang Exp $
//
...
//
// Input file : ecccnt_cfc_fcc2_regif.txt
////////////////////////////////////////////////////////////////////////////////
// %MODULE ecccnt_cfc
//     #              name     offset_size
...
// 
////////////////////////////////////////////////////////////////////////////////
/// @file ecccnt_cfc_fcc2_regif.cpp
/// @brief Register IF class of model ECCCNT_CFC
/// $Id: ecccnt_cfc_fcc2_regif.cpp,v 1.5 2019/09/26 10:07:02 landang Exp $
/// $Date: 2019/09/26 10:07:02 $
/// $Revison$
/// $Author: landang $
////////////////////////////////////////////////////////////////////////////////
...
/// @return none
Cecccnt_cfc_fcc2_regif::Cecccnt_cfc_fcc2_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
    , cwmem("register", scml_memsize(0x4/4))
    #endif
{
    CommandInit();
    CFCECCCTL = new vpcl::re_register(0x0000, this, "CFCECCCTL", name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_CFCECCCTL  , CFCECCCTL->name(), CFCECCCTL->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(CFCECCCTL  , mCurReg, 0,  4, "8|16|32", "8|16|32");

    mRegList = mCurReg;

    (*CFCECCCTL  ) (24, 24, "BLANKMASKENABLE", 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CFCECCCTL  ) ( 2,  2, "APEDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CFCECCCTL  ) ( 1,  1, "SECDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CFCECCCTL  ) ( 0,  0, "ECCDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["CFCECCCTL"]["BLANKMASKENABLE"] = &Cecccnt_cfc_fcc2_regif::cb_CFCECCCTL_BLANKMASKENABLE;
    mWrCbAPI["CFCECCCTL"]["APEDIS" ] = &Cecccnt_cfc_fcc2_regif::cb_CFCECCCTL_APEDIS;
    mWrCbAPI["CFCECCCTL"]["ECCDIS" ] = &Cecccnt_cfc_fcc2_regif::cb_CFCECCCTL_ECCDIS;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cecccnt_cfc_fcc2_regif::~Cecccnt_cfc_fcc2_regif()
{
...
Cecccnt_cfc_fcc2_regif::uint Cecccnt_cfc_fcc2_regif::bit_select(
...
bool Cecccnt_cfc_fcc2_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cecccnt_cfc_fcc2_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_cfc_fcc2_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                   const unsigned char *p_data,   ///< [in] Writing data
                                   cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_cfc_fcc2_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                   unsigned char *p_data,   ///< [out] Reading data
                                   cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_cfc_fcc2_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                       const unsigned char *p_data,  ///< [in] Writing data
                                       cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_cfc_fcc2_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                       unsigned char *p_data,   ///< [out] Reading data
                                       cuint size,              ///< [in]  Data size (byte)
                                       bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cecccnt_cfc_fcc2_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                    unsigned char *p_data,          ///< [out] Reading data
                                    cuint size,                     ///< [in]  Data size (byte)
...
bool Cecccnt_cfc_fcc2_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                    const unsigned char *p_data,      ///< [in] Writing data
                                    cuint size,                       ///< [in] Data size (byte)
                                    cuint reg_index,                  ///< [in] Register index
...
bool Cecccnt_cfc_fcc2_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cecccnt_cfc_fcc2_regif::CommandInit()
{
...
/// @return string
std::string Cecccnt_cfc_fcc2_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cecccnt_cfc_fcc2_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cecccnt_cfc_fcc2_regif::InitLocalVal()
{
    // Initialize local variables
    CFCECCCTL_BLANKMASKENABLE = 0x0;
    CFCECCCTL_APEDIS = 0x0;
    CFCECCCTL_SECDIS = 0x0;
    CFCECCCTL_ECCDIS = 0x0;
}

/// Update bit value to local value
/// @return None
void Cecccnt_cfc_fcc2_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        CFCECCCTL_BLANKMASKENABLE     = (*CFCECCCTL)["BLANKMASKENABLE"];
        CFCECCCTL_APEDIS              = (*CFCECCCTL)["APEDIS"   ];
        CFCECCCTL_SECDIS              = (*CFCECCCTL)["SECDIS"   ];
        CFCECCCTL_ECCDIS              = (*CFCECCCTL)["ECCDIS"   ];
        return;
    }
}
...
/// @return None
void Cecccnt_cfc_fcc2_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*CFCECCCTL)["BLANKMASKENABLE"] = CFCECCCTL_BLANKMASKENABLE;
        (*CFCECCCTL)["APEDIS"   ] = CFCECCCTL_APEDIS;
        (*CFCECCCTL)["SECDIS"   ] = CFCECCCTL_SECDIS;
        (*CFCECCCTL)["ECCDIS"   ] = CFCECCCTL_ECCDIS;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cecccnt_cfc_fcc2_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cecccnt_cfc_fcc2_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cecccnt_cfc_fcc2_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cecccnt_cfc_fcc2_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cecccnt_cfc_fcc2_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cecccnt_cfc_fcc2_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cecccnt_cfc_fcc2_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cecccnt_cfc_fcc2_regif::uint Cecccnt_cfc_fcc2_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cecccnt_cfc_fcc2_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cecccnt_cfc_fcc2_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cecccnt_cfc_fcc2_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cecccnt_cfc_fcc2_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cecccnt_cfc_fcc2_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cecccnt_cfc_fcc2_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cecccnt_cfc_fcc2_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
