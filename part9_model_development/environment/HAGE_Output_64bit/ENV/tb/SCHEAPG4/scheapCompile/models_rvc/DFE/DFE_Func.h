// ---------------------------------------------------------------------
// $Id: DFE_Func.h,v 1.2 2018/09/06 10:17:09 huepham Exp $
//
...
// Corporation.
// ---------------------------------------------------------------------
#ifndef __DFE_FUNC_H__
#define __DFE_FUNC_H__
...
class Cdfe_func: public Cdfe_regif
                ,public BusSlaveFuncIf
{
public:
    SC_HAS_PROCESS (Cdfe_func);
    Cdfe_func(std::string name, unsigned int num_channel, Cdfe_agent_controller* DFEAgentController);
    ~Cdfe_func();
    ...
    unsigned int GetDFEjSTCHn(unsigned int nChan);
    unsigned int GetDFEjST();
    unsigned int GetDFEjEST();
    unsigned int GetDFEjSUBSTCHn(unsigned int nChan);
    unsigned int GetDFEjCAPSTCHn(unsigned int nChan);
    unsigned int GetDFEjCTLBCHn(unsigned int nChan);
...
private:
    ...
    // Variables
    Cdfe_agent_controller   *mDFEAgentController;
    unsigned char           mDMEM[0x600];   // Data memory
    unsigned char           mCMEM[0x300];   // Coefficient memory
    unsigned int            mNumChannel;    // Number of channels
    bool                    mWasReadHOLCH[12];  // Was HOLCH read yet
    bool                    mWasReadHOHCH[12];  // Was HOHCH read yet
    bool                    mCurErrStatus;      // Current error status
    bool                    mPreErrStatus;      // Previous error status
    std::string             mModuleName;        // Module name
    unsigned int            mStartDataPtr[12];  // Start data pointer (used in CMEM, DMEM)
    unsigned int            mCurDataPtr[12];    // Current data pointer (used in CMEM, DMEM)
    unsigned int            mPrePHCH[12];       // Previous value of PHCH reg
    unsigned int            mPrePH20;           // Previous value of PH20 reg
    unsigned int            mPrePH30;           // Previous value of PH30 reg
    unsigned int            mPrePhase[12];      // Previous phase of PH23
    unsigned int            mPHCHPureVal[12];   // Pure value (not converted to Floating point) of PHCH reg
    ...
    void CommonCallbackDFEHOHCHn(unsigned int channel);
    void CommonCallbackDFEHOLCHn(unsigned int channel);
    void UpdateDFEjST();
    void UpdateDFEjEST();
