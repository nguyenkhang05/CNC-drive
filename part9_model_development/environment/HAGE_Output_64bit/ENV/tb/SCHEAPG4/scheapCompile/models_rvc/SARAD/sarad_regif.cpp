...
/// @return none
Csarad_regif::Csarad_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
    , cwmem("register", 0x314)
    #endif
{
    CommandInit();
    ADSYNSTCR  = new vpcl::re_register(0x0300, this, "ADSYNSTCR" , name.c_str());
    ADTSYNSTCR = new vpcl::re_register(0x0304, this, "ADTSYNSTCR", name.c_str());
    RDRVR      = new vpcl::re_register(0x0308, this, "RDRVR"     , name.c_str());
    VMONVDCR1  = new vpcl::re_register(0x030C, this, "VMONVDCR1" , name.c_str());
    VMONVDCR2  = new vpcl::re_register(0x0310, this, "VMONVDCR2" , name.c_str());
    ...
    mCurReg = mRegArray[index++] = new SRegList(RDRVR       , mCurReg, 0,  4,      "32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(VMONVDCR1   , mCurReg, 0,  1,       "8",       "8");
    mCurReg = mRegArray[index++] = new SRegList(VMONVDCR2   , mCurReg, 0,  1,       "8",       "8");

    mRegList = mCurReg;

    (*ADSYNSTCR   ) ( 0,  0, "ADSTART" , 0x0       , "W"     , vpcl::SPP_ENABLE);
    (*ADTSYNSTCR  ) ( 0,  0, "ADTSTART", 0x0       , "W"     , vpcl::SPP_ENABLE);
    (*RDRVR       ) (31, 16, "KEY"     , 0x0       , "W|R"   , vpcl::SPP_DISABLE);
    (*RDRVR       ) ( 0,  0, "RVRS"    , 0x0       , "W|R"   , vpcl::SPP_DISABLE);
    ...
    mWrCbAPI["ADSYNSTCR"]["ADSTART" ] = &Csarad_regif::cb_ADSYNSTCR_ADSTART;
    mWrCbAPI["ADTSYNSTCR"]["ADTSTART"] = &Csarad_regif::cb_ADTSYNSTCR_ADTSTART;
    mWrCbAPI["VMONVDCR1"]["VDE1"    ] = &Csarad_regif::cb_VMONVDCR1_VDE1;
    mWrCbAPI["VMONVDCR2"]["VDE2"    ] = &Csarad_regif::cb_VMONVDCR2_VDE2;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Csarad_regif::~Csarad_regif()
{
...
Csarad_regif::uint Csarad_regif::bit_select(cuint val,    ///< [in] Writting address
...
bool Csarad_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Csarad_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Csarad_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                              const unsigned char *p_data,   ///< [in] Writing data
                              cuint size)                    ///< [in] Data size (byte)
{
    if (size == 0) {
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Csarad_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                              unsigned char *p_data,   ///< [out] Reading data
                              cuint size)              ///< [in]  Data size (byte)
{
    if (size == 0) {
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Csarad_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                  const unsigned char *p_data,  ///< [in] Writing data
                                  cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Csarad_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                  unsigned char *p_data,   ///< [out] Reading data
                                  cuint size,              ///< [in]  Data size (byte)
                                  bool IsDbgFunc)          ///< [in]  Flag indicate reg_rd or reg_rd_dbg
{
...
bool Csarad_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                               unsigned char *p_data,          ///< [out] Reading data
                               cuint size,                     ///< [in]  Data size (byte)
...
bool Csarad_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                               const unsigned char *p_data,      ///< [in] Writing data
                               cuint size,                       ///< [in] Data size (byte)
                               cuint reg_index,                  ///< [in] Register index
...
bool Csarad_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr, bool is_wr)
{
...
/// @return string
std::string Csarad_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Csarad_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Csarad_regif::InitLocalVal()
{
    // Initialize local variables
    ADSYNSTCR_ADSTART = 0x0;
    ADTSYNSTCR_ADTSTART = 0x0;
    RDRVR_KEY  = 0x0;
    RDRVR_RVRS = 0x0;
    VMONVDCR1_VDE1 = 0x0;
    VMONVDCR2_VDE2 = 0x0;
...
/// @return None
void Csarad_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0300) {
        ADSYNSTCR_ADSTART              = (*ADSYNSTCR )["ADSTART"   ];
        return;
    }
    if (addr == 0x0304) {
        ADTSYNSTCR_ADTSTART            = (*ADTSYNSTCR)["ADTSTART"  ];
        return;
    }
    if (addr == 0x0308) {
        RDRVR_KEY                      = (*RDRVR     )["KEY"       ];
        RDRVR_RVRS                     = (*RDRVR     )["RVRS"      ];
        return;
    }
    if (addr == 0x030C) {
        VMONVDCR1_VDE1                 = (*VMONVDCR1 )["VDE1"      ];
        return;
    }
    if (addr == 0x0310) {
        VMONVDCR2_VDE2                 = (*VMONVDCR2 )["VDE2"      ];
        return;
    }
}
...
/// @return None
void Csarad_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0300) {
        (*ADSYNSTCR )["ADSTART"   ] = ADSYNSTCR_ADSTART;
        return;
    }
    if (addr == 0x0304) {
        (*ADTSYNSTCR)["ADTSTART"  ] = ADTSYNSTCR_ADTSTART;
        return;
    }
    if (addr == 0x0308) {
        (*RDRVR     )["KEY"       ] = RDRVR_KEY;
        (*RDRVR     )["RVRS"      ] = RDRVR_RVRS;
        return;
    }
    if (addr == 0x030C) {
        (*VMONVDCR1 )["VDE1"      ] = VMONVDCR1_VDE1;
        return;
    }
    if (addr == 0x0310) {
        (*VMONVDCR2 )["VDE2"      ] = VMONVDCR2_VDE2;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Csarad_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Csarad_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip, bool is_wr_data, uint acc_size)
{
    std::ostringstream str_tmp;
...
/// @return none
void Csarad_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
    if (mDumpRegisterRW) {
...
int Csarad_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Csarad_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Csarad_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Csarad_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Csarad_regif::uint Csarad_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
    uint data=0;
...
/// @return none
void Csarad_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Csarad_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Csarad_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Csarad_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Csarad_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Csarad_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    int reg_index = get_reg_index((uint)tag);
    assert(reg_index != (1<<16));
    uint length = mRegArray[reg_index]->length;
...
void Csarad_regif::cw_set_all_callback_reg(void)
{
