// ---------------------------------------------------------------------
// $Id: ECCCNT_IT_PEn_Func.cpp,v 1.1 2019/09/26 10:07:26 landang Exp $
//
...
//========================================================
/// Constructor
ECCCNT_IT_PEn_Func::ECCCNT_IT_PEn_Func (std::string name, unsigned int pe_id, ECC_ICACHE_E2_AgentController* ICACHEAgentController):
                Cecccnt_it_pen_regif(name, 32)
                ,BusSlaveFuncIf()
{//{{{
    ...
    Cecccnt_it_pen_regif::set_instance_name(name);
}//}}}

/// Destructor
ECCCNT_IT_PEn_Func::~ECCCNT_IT_PEn_Func()
{//{{{
}//}}}

/// Reset Func and registers
void ECCCNT_IT_PEn_Func::EnableReset(const std::string reset_name, const bool is_active)
{//{{{
    if (reset_name == "sys_resetz"){
        Cecccnt_it_pen_regif::EnableReset(is_active);
    }
}//}}}

/// Read API (target socket)
void ECCCNT_IT_PEn_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void ECCCNT_IT_PEn_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void ECCCNT_IT_PEn_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cecccnt_it_pen_regif::reg_handle_command(cmd);
                if (msg_return != "") {
        ...
        } else {//Set handle command to Cecccnt_it_pen_regif class
            msg_return = Cecccnt_it_pen_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
unsigned int ECCCNT_IT_PEn_Func::GetITECCCTLReg()
{//{{{
...
void ECCCNT_IT_PEn_Func::cb_ITECCCTL_ECCDIS(RegCBstr str)
{//{{{ 
