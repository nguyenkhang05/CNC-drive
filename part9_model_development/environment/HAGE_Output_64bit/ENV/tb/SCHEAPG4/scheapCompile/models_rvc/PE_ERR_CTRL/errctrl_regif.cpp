// -----------------------------------------------------------------------------
// $Id: errctrl_regif.cpp,v 1.1 2014/10/08 03:05:00 binhnguyen Exp $
//
...
/// @return none
Cerrctrl_regif::Cerrctrl_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    PGERRSTATCTL = new vpcl::re_register(0x0000, this, "PGERRSTATCTL", name.c_str());
    PGERRSTAT    = new vpcl::re_register(0x0004, this, "PGERRSTAT"   , name.c_str());
    PGERRINFO    = new vpcl::re_register(0x0008, this, "PGERRINFO"   , name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_PGERRSTATCTL  , PGERRSTATCTL->name(), PGERRSTATCTL->addr());
    cw_instantiate_reg(CW_PGERRSTAT     , PGERRSTAT->name()   , PGERRSTAT->addr()   );
    cw_instantiate_reg(CW_PGERRINFO     , PGERRINFO->name()   , PGERRINFO->addr()   );
    #endif
    mCurReg = mRegArray[index++] = new SRegList(PGERRSTATCTL  , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PGERRSTAT     , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PGERRINFO     , mCurReg, 0,  4, "8|16|32", "8|16|32");

    mRegList = mCurReg;

    (*PGERRSTATCTL  ) ( 1,  1, "CLRO"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*PGERRSTATCTL  ) ( 0,  0, "CLRE"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*PGERRSTAT     ) ( 1,  1, "OVF"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PGERRSTAT     ) ( 0,  0, "ERR"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PGERRINFO     ) ( 7,  5, "PEID"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PGERRINFO     ) ( 4,  0, "SPID"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["PGERRSTATCTL"]["CLRE"      ] = &Cerrctrl_regif::cb_PGERRSTATCTL_CLRE;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cerrctrl_regif::~Cerrctrl_regif()
{
...
Cerrctrl_regif::uint Cerrctrl_regif::bit_select(
...
bool Cerrctrl_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cerrctrl_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cerrctrl_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                const unsigned char *p_data,   ///< [in] Writing data
                                cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cerrctrl_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                unsigned char *p_data,   ///< [out] Reading data
                                cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cerrctrl_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                    const unsigned char *p_data,  ///< [in] Writing data
                                    cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cerrctrl_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                    unsigned char *p_data,   ///< [out] Reading data
                                    cuint size,              ///< [in]  Data size (byte)
                                    bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cerrctrl_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                 unsigned char *p_data,          ///< [out] Reading data
                                 cuint size,                     ///< [in]  Data size (byte)
...
bool Cerrctrl_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                 const unsigned char *p_data,      ///< [in] Writing data
                                 cuint size,                       ///< [in] Data size (byte)
                                 cuint reg_index,                  ///< [in] Register index
...
bool Cerrctrl_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cerrctrl_regif::CommandInit()
{
...
/// @return string
std::string Cerrctrl_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cerrctrl_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cerrctrl_regif::InitLocalVal()
{
    // Initialize local variables
    PGERRSTATCTL_CLRO = 0x0;
    PGERRSTATCTL_CLRE = 0x0;
    PGERRSTAT_OVF = 0x0;
    PGERRSTAT_ERR = 0x0;
    PGERRINFO_PEID = 0x0;
    PGERRINFO_SPID = 0x0;
...
/// @return None
void Cerrctrl_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        PGERRSTATCTL_CLRO                = (*PGERRSTATCTL)["CLRO"        ];
        PGERRSTATCTL_CLRE                = (*PGERRSTATCTL)["CLRE"        ];
        return;
    }
    if (addr == 0x0004) {
        PGERRSTAT_OVF                    = (*PGERRSTAT   )["OVF"         ];
        PGERRSTAT_ERR                    = (*PGERRSTAT   )["ERR"         ];
        return;
    }
    if (addr == 0x0008) {
        PGERRINFO_PEID                   = (*PGERRINFO   )["PEID"        ];
        PGERRINFO_SPID                   = (*PGERRINFO   )["SPID"        ];
        return;
    }
}
...
/// @return None
void Cerrctrl_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*PGERRSTATCTL)["CLRO"        ] = PGERRSTATCTL_CLRO;
        (*PGERRSTATCTL)["CLRE"        ] = PGERRSTATCTL_CLRE;
        return;
    }
    if (addr == 0x0004) {
        (*PGERRSTAT   )["OVF"         ] = PGERRSTAT_OVF;
        (*PGERRSTAT   )["ERR"         ] = PGERRSTAT_ERR;
        return;
    }
    if (addr == 0x0008) {
        (*PGERRINFO   )["PEID"        ] = PGERRINFO_PEID;
        (*PGERRINFO   )["SPID"        ] = PGERRINFO_SPID;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cerrctrl_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cerrctrl_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cerrctrl_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cerrctrl_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cerrctrl_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cerrctrl_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cerrctrl_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cerrctrl_regif::uint Cerrctrl_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cerrctrl_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cerrctrl_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cerrctrl_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cerrctrl_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cerrctrl_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cerrctrl_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cerrctrl_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
