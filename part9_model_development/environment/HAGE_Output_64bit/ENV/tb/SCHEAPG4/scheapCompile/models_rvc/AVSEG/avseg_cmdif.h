...
//%MODULE avseg
//    %CMD_BASEID tgt reg
//    %CMD_NODUMP_API CommandCB EnableInsertInput EnableDumpResult DumpProfile ClearProfile AssertReset
//
//    %CMD_ACTION AssertReset <rst_name:std::string> <start_time:double> <period:double>: void @ Assert and deassert reset signal to a target model
//    %CMD_ACTION SetCLKfreq <clk_name:std::string> <clk_freq:uint64_t> <unit:std::string>: void @ Set clock frequency
//    %CMD_ACTION GetCLKfreq <clk_name:std::string>: void @ Get clock frequency

#ifndef AVSEG_CMDIF_H
#define AVSEG_CMDIF_H
...
#if 0 // CommandCB
    // call call back function only when return message does not include error keyword
...
std::string own_handle_command(std::vector<std::string>& args)
{
  ...
  if (args[0] == "help") {
    ...
    ret += "    HWBreak <break-name>                      Enable specified break point (Default: none)\n";
#endif // HWBreak
#if 1 // MessageLevel
...
#endif // EnableDumpResult
    ret += "    AssertReset <rst_name> <start_time> <period>\n";
    ret += "                                              Assert and deassert reset signal to a target model\n";
    ret += "    SetCLKfreq <clk_name> <clk_freq> <unit>   Set clock frequency\n";
    ret += "    GetCLKfreq <clk_name>                     Get clock frequency\n";
...
std::string user_def_command(std::vector<std::string>& args)
{
  ...
  if (0) {}
    ...
    if ((int)args.size() == 4) {
      ...
      std::istringstream str_start_time(args[2]);
      std::istringstream str_period    (args[3]);
      ...
      if (IsTranslateCorrect) {
        AssertReset(rst_name, start_time, period);
      } else {
        ret = err_msg + "wrong argument: " + args[1] + " " + args[2] + " " + args[3]
            + specified_cmd;
      }
    }
    else {
    ...
    if ((int)args.size() > 3 && (int)args.size() <= 4) {
      ...
      SetCLKfreq(clk_name, clk_freq, unit);
    }
    else {
      ret = err_msg + "wrong number of arguments" + specified_cmd;
    }
...
std::string regif_handle_command(std::vector<std::string>& args)
{
  ...
  ret = RegisterHandleCommand(args);
