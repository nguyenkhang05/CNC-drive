// ---------------------------------------------------------------------
// $Id: MECCCAP_CFL_Func.cpp,v 1.1 2019/09/26 10:06:58 landang Exp $
//
...
//========================================================
/// Constructor
MECCCAP_CFL_Func::MECCCAP_CFL_Func (std::string name, ECC_CFLASH_E2_AgentController* CFLASHAgentController):
                Cmecccap_cfl_regif(name, 32)
                ,BusSlaveFuncIf()
{//{{{
    ...
    Cmecccap_cfl_regif::set_instance_name(name);
}//}}}

/// Destructor
MECCCAP_CFL_Func::~MECCCAP_CFL_Func()
{//{{{
}//}}}

/// Reset Func and registers
void MECCCAP_CFL_Func::EnableReset(const std::string reset_name, const bool is_active)
{//{{{
    if (reset_name == "sys_resetz"){
        Cmecccap_cfl_regif::EnableReset(is_active);
    }
}//}}}

/// Read API (target socket)
void MECCCAP_CFL_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    // Get extension info
...
void MECCCAP_CFL_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    // Get extension info
    TlmG3mExtension *extension;
    trans.get_extension(extension);
...
void MECCCAP_CFL_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cmecccap_cfl_regif::reg_handle_command(cmd);
                if (msg_return != "") {
            ...
            msg_return = Cmecccap_cfl_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
bool MECCCAP_CFL_Func::Set1bitStatusAndAddress(unsigned int edl_code, unsigned int mod_address)
{//{{{
    unsigned int cap_depth = mpCFLASHAgentController->GetCapDepth();
...
bool MECCCAP_CFL_Func::CheckHasRoomFor1bit()
{//{{{
    unsigned int cap_depth = mpCFLASHAgentController->GetCapDepth();
    unsigned int status = (unsigned int)(*CF_SERSTR);
    bool is_find_room = false;
...
/// Set FIFO overflow
void MECCCAP_CFL_Func::SetFIFOOverflow ()
{//{{{
...
void MECCCAP_CFL_Func::SetFlagAddressBufferOverflow()
{//{{{
...
/// Get SEDL bit
unsigned int MECCCAP_CFL_Func::GetSEDLBit(unsigned int index)
{//{{{
    return (unsigned int)(*CF_nSEADR[index])["SEDL"];
}//}}}

/// Get SEADR bit
unsigned int MECCCAP_CFL_Func::GetSEADRBit(unsigned int index)
{//{{{
    return (unsigned int)(*CF_nSEADR[index])["SEADR0"];
}//}}}

/// Get DEDL bit
unsigned int MECCCAP_CFL_Func::GetDEDLBit()
{//{{{
    return (unsigned int)(*CF_00DEADR)["DEDL"];
}//}}}

/// Get DEADR bit
unsigned int MECCCAP_CFL_Func::GetDEADRBit()
{//{{{
    return (unsigned int)(*CF_00DEADR)["DEADR0"];
}//}}}

/// Get Start mask address
unsigned int MECCCAP_CFL_Func::GetERMSA(unsigned int index)
{//{{{
...
unsigned int MECCCAP_CFL_Func::GetERMEA(unsigned int index)
{//{{{
    unsigned int ret_val = 0;
    if (index == 0){
...
unsigned int MECCCAP_CFL_Func::GetERM_EN(unsigned int index)
{//{{{
    unsigned int ret_val = 0;
    if (index == 0){
...
bool MECCCAP_CFL_Func::CheckNoFatalStatus()
{//{{{
    return ((unsigned int)(*CF_DERSTR) == 0);
}//}}}

/// Set 2bit error status
void MECCCAP_CFL_Func::Set2bitErrorStatus()
{//{{{
    (*CF_DERSTR)["DEDF00"] = 1;
}//}}}

/// Set address parity error status
void MECCCAP_CFL_Func::SetAddrParityErrorStatus()
{//{{{
    (*CF_DERSTR)["APEF00"] = 1;
}//}}}

/// Set fatal error address and edl code according
void MECCCAP_CFL_Func::SetFatalAddress(unsigned int edl_code, unsigned int mod_address)
{//{{{
...
/// Get ERRINT value
unsigned int MECCCAP_CFL_Func::GetERRINT()
{//{{{
...
void MECCCAP_CFL_Func::cb_CF_SSTCLR_CF_SSTCLR(RegCBstr str)
{//{{{ 
...
void MECCCAP_CFL_Func::cb_CF_DSTCLR_DSTCLR00(RegCBstr str)
{//{{{
    // clear APE, DED.
...
void MECCCAP_CFL_Func::cb_CF_OVFCLR_SERROVFCLR0(RegCBstr str)
{//{{{ 
...
void MECCCAP_CFL_Func::cb_CF_ERMSA0_ERMSA(RegCBstr str)
{//{{{
...
void MECCCAP_CFL_Func::cb_CF_ERMEA0_ERMEA(RegCBstr str)
{//{{{
...
void MECCCAP_CFL_Func::cb_CF_ERMSA1_ERMSA(RegCBstr str)
{//{{{
...
void MECCCAP_CFL_Func::cb_CF_ERMEA1_ERMEA(RegCBstr str)
{//{{{
