// -----------------------------------------------------------------------------
// $Id: ecc_bus_ecccap_regif.cpp,v 1.10 2020/03/30 10:53:33 thunguyen3 Exp $
//
...
/// @return none
    Cecc_bus_ecccap_regif::Cecc_bus_ecccap_regif(std::string name, uint buswidth, std::string err_kind)
:vpcl::reg_super()
#ifdef USR_CWR_SYSTEMC
    , cwmem("register", scml_memsize(0x84/4))
#endif
{
    mErr_Kind = err_kind;
    ...
    if(mErr_Kind != "DD") {
        ASSTCLR = new vpcl::re_register(0x0010, this, "ASSTCLR", name.c_str());
        ADSTCLR = new vpcl::re_register(0x0020, this, "ADSTCLR", name.c_str());
    }
    if(mErr_Kind != "AA") {
        DSSTCLR = new vpcl::re_register(0x0030, this, "DSSTCLR", name.c_str());
        DDSTCLR = new vpcl::re_register(0x0040, this, "DDSTCLR", name.c_str());
    }
    if(mErr_Kind != "DD") {
        ASERSTR = new vpcl::re_register(0x0050, this, "ASERSTR", name.c_str());
        ADERSTR = new vpcl::re_register(0x0060, this, "ADERSTR", name.c_str());
    }
    if(mErr_Kind != "AA") {
        DSERSTR = new vpcl::re_register(0x0070, this, "DSERSTR", name.c_str());
        DDERSTR = new vpcl::re_register(0x0080, this, "DDERSTR", name.c_str());
    }
    ...
    if(mErr_Kind != "DD") {
        cw_instantiate_reg(CW_ASSTCLR  , ASSTCLR->name(), ASSTCLR->addr());
        cw_instantiate_reg(CW_ADSTCLR  , ADSTCLR->name(), ADSTCLR->addr());
    }
    if(mErr_Kind != "AA") {
        cw_instantiate_reg(CW_DSSTCLR  , DSSTCLR->name(), DSSTCLR->addr());
        cw_instantiate_reg(CW_DDSTCLR  , DDSTCLR->name(), DDSTCLR->addr());
    }
    if(mErr_Kind != "DD") {
        cw_instantiate_reg(CW_ASERSTR  , ASERSTR->name(), ASERSTR->addr());
        cw_instantiate_reg(CW_ADERSTR  , ADERSTR->name(), ADERSTR->addr());
    }
    if(mErr_Kind != "AA") {
        cw_instantiate_reg(CW_DSERSTR  , DSERSTR->name(), DSERSTR->addr());
        cw_instantiate_reg(CW_DDERSTR  , DDERSTR->name(), DDERSTR->addr());
    }
#endif
    mCurReg = mRegArray[index++] = new SRegList(ERRINT   , mCurReg, 0,  4, "8|16|32", "8|16|32");
    if(mErr_Kind != "DD") {
        mCurReg = mRegArray[index++] = new SRegList(ASSTCLR  , mCurReg, 0,  4, "8|16|32", "8|16|32");
        mCurReg = mRegArray[index++] = new SRegList(ADSTCLR  , mCurReg, 0,  4, "8|16|32", "8|16|32");
    }
    if(mErr_Kind != "AA") {
        mCurReg = mRegArray[index++] = new SRegList(DSSTCLR  , mCurReg, 0,  4, "8|16|32", "8|16|32");
        mCurReg = mRegArray[index++] = new SRegList(DDSTCLR  , mCurReg, 0,  4, "8|16|32", "8|16|32");
    }
    if(mErr_Kind != "DD") {
        mCurReg = mRegArray[index++] = new SRegList(ASERSTR  , mCurReg, 0,  4,      "32", "8|16|32");
        mCurReg = mRegArray[index++] = new SRegList(ADERSTR  , mCurReg, 0,  4,      "32", "8|16|32");
    }
    if(mErr_Kind != "AA") {
        mCurReg = mRegArray[index++] = new SRegList(DSERSTR  , mCurReg, 0,  4,      "32", "8|16|32");
        mCurReg = mRegArray[index++] = new SRegList(DDERSTR  , mCurReg, 0,  4,      "32", "8|16|32");
    }

    mRegList = mCurReg;
    if(mErr_Kind != "DD") {
        (*ERRINT   ) ( 3,  3, "ADEDIE"  , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
        (*ERRINT   ) ( 2,  2, "ASEDIE"  , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
    }
    if(mErr_Kind != "AA") {
        (*ERRINT   ) ( 1,  1, "DDEDIE"  , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
        (*ERRINT   ) ( 0,  0, "DSEDIE"  , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
    }
    if(mErr_Kind != "DD") {
        (*ASSTCLR  ) (31,  0, "ASSTCLR" , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*ADSTCLR  ) (31,  0, "ADSTCLR" , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    if(mErr_Kind != "AA") {
        (*DSSTCLR  ) (31,  0, "DSSTCLR" , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*DDSTCLR  ) (31,  0, "DDSTCLR" , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    if(mErr_Kind != "DD") {
        (*ASERSTR  ) (31,  0, "ASERSTR" , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*ADERSTR  ) (31,  0, "ADERSTR" , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    if(mErr_Kind != "AA") {
        (*DSERSTR  ) (31,  0, "DSERSTR" , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*DDERSTR  ) (31,  0, "DDERSTR" , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    ...
    if(mErr_Kind != "DD") {
        mWrCbAPI["ERRINT"]["ADEDIE"] = &Cecc_bus_ecccap_regif::cb_ERRINT_ADEDIE;
    }
    if(mErr_Kind != "AA") {
        mWrCbAPI["ERRINT"]["DSEDIE"] = &Cecc_bus_ecccap_regif::cb_ERRINT_DSEDIE;
    }
    if(mErr_Kind != "DD") {
        mWrCbAPI["ASSTCLR"]["ASSTCLR"] = &Cecc_bus_ecccap_regif::cb_ASSTCLR_ASSTCLR;
        mWrCbAPI["ADSTCLR"]["ADSTCLR"] = &Cecc_bus_ecccap_regif::cb_ADSTCLR_ADSTCLR;
    }
    if(mErr_Kind != "AA") {
        mWrCbAPI["DSSTCLR"]["DSSTCLR"] = &Cecc_bus_ecccap_regif::cb_DSSTCLR_DSSTCLR;
        mWrCbAPI["DDSTCLR"]["DDSTCLR"] = &Cecc_bus_ecccap_regif::cb_DDSTCLR_DDSTCLR;
    }
    InitLocalVal();
}
...
/// @return none
Cecc_bus_ecccap_regif::~Cecc_bus_ecccap_regif()
{
...
Cecc_bus_ecccap_regif::uint Cecc_bus_ecccap_regif::bit_select(
...
bool Cecc_bus_ecccap_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cecc_bus_ecccap_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecc_bus_ecccap_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
        const unsigned char *p_data,   ///< [in] Writing data
        cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecc_bus_ecccap_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
        unsigned char *p_data,   ///< [out] Reading data
        cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecc_bus_ecccap_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
        const unsigned char *p_data,  ///< [in] Writing data
        cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecc_bus_ecccap_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
        unsigned char *p_data,   ///< [out] Reading data
        cuint size,              ///< [in]  Data size (byte)
        bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cecc_bus_ecccap_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
        unsigned char *p_data,          ///< [out] Reading data
        cuint size,                     ///< [in]  Data size (byte)
...
bool Cecc_bus_ecccap_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
        const unsigned char *p_data,      ///< [in] Writing data
        cuint size,                       ///< [in] Data size (byte)
        cuint reg_index,                  ///< [in] Register index
...
bool Cecc_bus_ecccap_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cecc_bus_ecccap_regif::CommandInit()
{
...
/// @return string
std::string Cecc_bus_ecccap_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cecc_bus_ecccap_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cecc_bus_ecccap_regif::InitLocalVal()
{
    // Initialize local variables
    ERRINT_ADEDIE = 0x1;
    ERRINT_ASEDIE = 0x1;
...
/// @return None
void Cecc_bus_ecccap_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        if(mErr_Kind != "DD") {
        ...
        if(mErr_Kind != "AA") {
            ERRINT_DDEDIE               = (*ERRINT )["DDEDIE" ];
            ERRINT_DSEDIE               = (*ERRINT )["DSEDIE" ];
        }
        return;
    }
    if(mErr_Kind != "DD") {
        if (addr == 0x0010) {
            ASSTCLR_ASSTCLR             = (*ASSTCLR)["ASSTCLR"];
            return;
        }
        if (addr == 0x0020) {
            ADSTCLR_ADSTCLR             = (*ADSTCLR)["ADSTCLR"];
            return;
        }
    }
    if(mErr_Kind != "AA") {
        if (addr == 0x0030) {
            DSSTCLR_DSSTCLR             = (*DSSTCLR)["DSSTCLR"];
            return;
        }
        if (addr == 0x0040) {
            DDSTCLR_DDSTCLR             = (*DDSTCLR)["DDSTCLR"];
            return;
        }
    }
    if(mErr_Kind != "DD") {
        if (addr == 0x0050) {
            ASERSTR_ASERSTR             = (*ASERSTR)["ASERSTR"];
            return;
        }
        if (addr == 0x0060) {
            ADERSTR_ADERSTR             = (*ADERSTR)["ADERSTR"];
            return;
        }
    }
    if(mErr_Kind != "AA") {
        if (addr == 0x0070) {
            DSERSTR_DSERSTR             = (*DSERSTR)["DSERSTR"];
            return;
        }
        if (addr == 0x0080) {
            DDERSTR_DDERSTR             = (*DDERSTR)["DDERSTR"];
            return;
        }
    }        
}
...
/// @return None
void Cecc_bus_ecccap_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        if(mErr_Kind != "DD") {
        ...
        if(mErr_Kind != "AA") {
            (*ERRINT )["DDEDIE" ] = ERRINT_DDEDIE;
            (*ERRINT )["DSEDIE" ] = ERRINT_DSEDIE;
        }
        return;
    }
    if(mErr_Kind != "DD") {
        if (addr == 0x0010) {
            (*ASSTCLR)["ASSTCLR"] = ASSTCLR_ASSTCLR;
            return;
        }
        if (addr == 0x0020) {
            (*ADSTCLR)["ADSTCLR"] = ADSTCLR_ADSTCLR;
            return;
        }
    }
    if(mErr_Kind != "AA") {
        if (addr == 0x0030) {
            (*DSSTCLR)["DSSTCLR"] = DSSTCLR_DSSTCLR;
            return;
        }
        if (addr == 0x0040) {
            (*DDSTCLR)["DDSTCLR"] = DDSTCLR_DDSTCLR;
            return;
        }
    }
    if(mErr_Kind != "DD") {
        if (addr == 0x0050) {
            (*ASERSTR)["ASERSTR"] = ASERSTR_ASERSTR;
            return;
        }
        if (addr == 0x0060) {
            (*ADERSTR)["ADERSTR"] = ADERSTR_ADERSTR;
            return;
        }
    }
    if(mErr_Kind != "AA") {
        if (addr == 0x0070) {
            (*DSERSTR)["DSERSTR"] = DSERSTR_DSERSTR;
            return;
        }
        if (addr == 0x0080) {
            (*DDERSTR)["DDERSTR"] = DDERSTR_DDERSTR;
            return;
        }
    }
}
...
/// @return vector
std::vector<std::string> Cecc_bus_ecccap_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cecc_bus_ecccap_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cecc_bus_ecccap_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cecc_bus_ecccap_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cecc_bus_ecccap_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cecc_bus_ecccap_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cecc_bus_ecccap_regif::wr_cb(cuint addr, uint data)
{
#ifdef USR_CWR_SYSTEMC
...
/// @return read data
Cecc_bus_ecccap_regif::uint Cecc_bus_ecccap_regif::rd_cb(cuint addr)
{
#ifdef USR_CWR_SYSTEMC
...
/// @return none
void Cecc_bus_ecccap_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cecc_bus_ecccap_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cecc_bus_ecccap_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cecc_bus_ecccap_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Cecc_bus_ecccap_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cecc_bus_ecccap_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cecc_bus_ecccap_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
