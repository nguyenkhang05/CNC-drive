// -----------------------------------------------------------------------------
// $Id: atu5_timerd_sub_regif.cpp 1196 2019-10-17 05:33:04Z chuonghoangle $
//
...
// 
////////////////////////////////////////////////////////////////////////////////
/// @file atu5_timerd_sub_regif.cpp
...
/// @return none
Catu5_timerd_sub_regif::Catu5_timerd_sub_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    ...
    #endif
{
    ...
    for(uint i = 0; i < emNUM_ONCAP1D; i++) {
        str_tmp.str("");
        str_tmp<<"ONCAP1D"<< i;
        ONCAP1D   [i] = new vpcl::re_register((0x0120) + 64*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_ONCAP2D; i++) {
        str_tmp.str("");
        str_tmp<<"ONCAP2D"<< i;
        ONCAP2D   [i] = new vpcl::re_register((0x0124) + 64*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_OFCAP1D; i++) {
        str_tmp.str("");
        str_tmp<<"OFCAP1D"<< i;
        OFCAP1D   [i] = new vpcl::re_register((0x0130) + 64*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_OFCAP2D; i++) {
        str_tmp.str("");
        str_tmp<<"OFCAP2D"<< i;
        OFCAP2D   [i] = new vpcl::re_register((0x0134) + 64*i, this, str_tmp.str(), name.c_str());
    }
    ...
    for(uint i = 0; i < emNUM_ONCAP1D   ; i++) cw_instantiate_reg(CW_ONCAP1D     [i], ONCAP1D   [i]->name(), ONCAP1D   [i]->addr());
    for(uint i = 0; i < emNUM_ONCAP2D   ; i++) cw_instantiate_reg(CW_ONCAP2D     [i], ONCAP2D   [i]->name(), ONCAP2D   [i]->addr());
    for(uint i = 0; i < emNUM_OFCAP1D   ; i++) cw_instantiate_reg(CW_OFCAP1D     [i], OFCAP1D   [i]->name(), OFCAP1D   [i]->addr());
    for(uint i = 0; i < emNUM_OFCAP2D   ; i++) cw_instantiate_reg(CW_OFCAP2D     [i], OFCAP2D   [i]->name(), OFCAP2D   [i]->addr());
    #endif
    ...
    for(uint i = 0; i < emNUM_ONCAP1D   ; i++) mCurReg = mRegArray[index++] = new SRegList(ONCAP1D     [i], mCurReg, i,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_ONCAP2D   ; i++) mCurReg = mRegArray[index++] = new SRegList(ONCAP2D     [i], mCurReg, i,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_OFCAP1D   ; i++) mCurReg = mRegArray[index++] = new SRegList(OFCAP1D     [i], mCurReg, i,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_OFCAP2D   ; i++) mCurReg = mRegArray[index++] = new SRegList(OFCAP2D     [i], mCurReg, i,  4,      "32", "8|16|32");
    ...
    (*TSCR2D      ) (11, 11, "FEFCD3"  , 0x0       , "R|W1:0", vpcl::SPP_ENABLE);
    (*TSCR2D      ) (10, 10, "FEFCD2"  , 0x0       , "R|W1:0", vpcl::SPP_ENABLE);
    (*TSCR2D      ) ( 9,  9, "FEFCD1"  , 0x0       , "R|W1:0", vpcl::SPP_ENABLE);
    (*TSCR2D      ) ( 8,  8, "FEFCD0"  , 0x0       , "R|W1:0", vpcl::SPP_ENABLE);
    (*TSCR2D      ) ( 3,  3, "REFCD3"  , 0x0       , "R|W1:0", vpcl::SPP_ENABLE);
    (*TSCR2D      ) ( 2,  2, "REFCD2"  , 0x0       , "R|W1:0", vpcl::SPP_ENABLE);
    (*TSCR2D      ) ( 1,  1, "REFCD1"  , 0x0       , "R|W1:0", vpcl::SPP_ENABLE);
    (*TSCR2D      ) ( 0,  0, "REFCD0"  , 0x0       , "R|W1:0", vpcl::SPP_ENABLE);
    for(uint i = 0; i < emNUM_ONMIND; i++) {
        (*ONMIND      [i]) (31,  0, "ONMIND"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_ONCAP1D; i++) {
        (*ONCAP1D     [i]) (31,  0, "ONCAP1D" , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_ONCAP2D; i++) {
        (*ONCAP2D     [i]) (31,  0, "ONCAP2D" , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_OCR1D; i++) {
    ...
    for(uint i = 0; i < emNUM_ONMICNTD; i++) {
        (*ONMICNTD    [i]) (31,  0, "ONMICNTD", 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_OTOMIND; i++) {
        (*OTOMIND     [i]) (31,  0, "OTOMIND" , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_OFCAP2D; i++) {
        (*OFCAP2D     [i]) (31,  0, "OFCAP2D" , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_ICR2D; i++) {
    ...
    for(uint i = 0; i < emNUM_OFCAP1D; i++) {
        (*OFCAP1D     [i]) (31,  0, "OFCAP1D" , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_OFMIND; i++) {
        (*OFMIND      [i]) (31,  0, "OFMIND"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_OFMICNTD; i++) {
        (*OFMICNTD    [i]) (31,  0, "OFMICNTD", 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_ICR1D; i++) {
        (*ICR1D       [i]) (31,  0, "ICR1D"   , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_OTONMICNTD; i++) {
        (*OTONMICNTD  [i]) (31,  0, "OTONMICNTD", 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    ...
    mWrCbAPI["TCRD"    ]["OBRED"   ] = &Catu5_timerd_sub_regif::cb_TCRD_OBRED;
    mWrCbAPI["TCCRLRD" ]["CLRSEL2D"] = &Catu5_timerd_sub_regif::cb_TCCRLRD_CLRSEL2D;
    mWrCbAPI["TIOR1D"  ]["OSSD3"   ] = &Catu5_timerd_sub_regif::cb_TIOR1D_OSSD3;
    mWrCbAPI["TIOR2D"  ]["IOBD3"   ] = &Catu5_timerd_sub_regif::cb_TIOR2D_IOBD3;
    mWrCbAPI["TCMPED"  ]["CMPE2D3" ] = &Catu5_timerd_sub_regif::cb_TCMPED_CMPE2D3;
    mWrCbAPI["TIORD"   ]["IO2D3"   ] = &Catu5_timerd_sub_regif::cb_TIORD_IO2D3;
    mWrCbAPI["OSELRD"  ]["OSELBD3" ] = &Catu5_timerd_sub_regif::cb_OSELRD_OSELBD3;
    mWrCbAPI["ODRD"    ]["ODBD3"   ] = &Catu5_timerd_sub_regif::cb_ODRD_ODBD3;
    mWrCbAPI["DSCRD"   ]["DWRFCD0" ] = &Catu5_timerd_sub_regif::cb_DSCRD_DWRFCD0;
    mWrCbAPI["TSCRD"   ]["OVFC2D"  ] = &Catu5_timerd_sub_regif::cb_TSCRD_OVFC2D;
    mWrCbAPI["TOCRD"   ]["TONEBD"  ] = &Catu5_timerd_sub_regif::cb_TOCRD_TONEBD;
    mWrCbAPI["TOCCRD"  ]["TONEBD3" ] = &Catu5_timerd_sub_regif::cb_TOCCRD_TONEBD3;
    mWrCbAPI["TCNT1D"  ]["TCNT1D"  ] = &Catu5_timerd_sub_regif::cb_TCNT1D_TCNT1D;
    mWrCbAPI["TCNT2D"  ]["TCNT2D"  ] = &Catu5_timerd_sub_regif::cb_TCNT2D_TCNT2D;
    mWrCbAPI["CUCR1D"  ]["CUCR1D"  ] = &Catu5_timerd_sub_regif::cb_CUCR1D_CUCR1D;
    mWrCbAPI["CUCR2D"  ]["CUCR2D"  ] = &Catu5_timerd_sub_regif::cb_CUCR2D_CUCR2D;
    mWrCbAPI["RCR1D"   ]["RC1D"    ] = &Catu5_timerd_sub_regif::cb_RCR1D_RC1D;
    mWrCbAPI["RCR2D"   ]["RC2D"    ] = &Catu5_timerd_sub_regif::cb_RCR2D_RC2D;
    mWrCbAPI["TIER1D"  ]["OV2ED"   ] = &Catu5_timerd_sub_regif::cb_TIER1D_OV2ED;
    mWrCbAPI["TIER2D"  ]["UNDED3"  ] = &Catu5_timerd_sub_regif::cb_TIER2D_UNDED3;
    mWrCbAPI["TIER3D"  ]["FEIED3"  ] = &Catu5_timerd_sub_regif::cb_TIER3D_FEIED3;
    mWrCbAPI["TSCR2D"  ]["FEFCD3"  ] = &Catu5_timerd_sub_regif::cb_TSCR2D_FEFCD3;

    for(uint i = 0; i < emNUM_OCR1D; i++) {
        str_tmp.str("");
        str_tmp<<"OCR1D"<< i;
        mWrCbAPI[str_tmp.str()]["OC1D"] = &Catu5_timerd_sub_regif::cb_OCR1D_OC1D;
    }
    for(uint i = 0; i < emNUM_OCR2D; i++) {
        str_tmp.str("");
        str_tmp<<"OCR2D"<< i;
        mWrCbAPI[str_tmp.str()]["OC2D"] = &Catu5_timerd_sub_regif::cb_OCR2D_OC2D;
    }
    InitLocalVal();
}
...
/// @return none
Catu5_timerd_sub_regif::~Catu5_timerd_sub_regif()
{
...
Catu5_timerd_sub_regif::uint Catu5_timerd_sub_regif::bit_select(
...
bool Catu5_timerd_sub_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Catu5_timerd_sub_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Catu5_timerd_sub_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                        const unsigned char *p_data,   ///< [in] Writing data
                                        cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Catu5_timerd_sub_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                        unsigned char *p_data,   ///< [out] Reading data
                                        cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Catu5_timerd_sub_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                            const unsigned char *p_data,  ///< [in] Writing data
                                            cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Catu5_timerd_sub_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                            unsigned char *p_data,   ///< [out] Reading data
                                            cuint size,              ///< [in]  Data size (byte)
                                            bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Catu5_timerd_sub_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                         unsigned char *p_data,          ///< [out] Reading data
                                         cuint size,                     ///< [in]  Data size (byte)
...
bool Catu5_timerd_sub_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                         const unsigned char *p_data,      ///< [in] Writing data
                                         cuint size,                       ///< [in] Data size (byte)
                                         cuint reg_index,                  ///< [in] Register index
...
bool Catu5_timerd_sub_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Catu5_timerd_sub_regif::CommandInit()
{
...
/// @return string
std::string Catu5_timerd_sub_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Catu5_timerd_sub_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Catu5_timerd_sub_regif::InitLocalVal()
{
    // Initialize local variables
    TCRD_OBRED    = 0x0;
    TCRD_C2CED    = 0x0;
    ...
    for(uint i = 0; i < emNUM_ONCAP1D;   i++) {
        ONCAP1D_ONCAP1D[i] = 0x0;
    }
    for(uint i = 0; i < emNUM_ONCAP2D;   i++) {
        ONCAP2D_ONCAP2D[i] = 0x0;
    }
    for(uint i = 0; i < emNUM_OFCAP1D;   i++) {
        OFCAP1D_OFCAP1D[i] = 0x0;
    }
    for(uint i = 0; i < emNUM_OFCAP2D;   i++) {
        OFCAP2D_OFCAP2D[i] = 0x0;
    }
    TSR2D_FEFD3   = 0x0;
    TSR2D_FEFD2   = 0x0;
    TSR2D_FEFD1   = 0x0;
...
/// @return None
void Catu5_timerd_sub_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
...
/// @return None
void Catu5_timerd_sub_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
    ...
    if ((0x0120 <= addr) && (addr <= (0x0120)+(64*3)) && ((addr-0x0120)%64 == 0)) {
        uint i = (addr - 0x0120)/64;
        (*(ONCAP1D   [i]))["ONCAP1D"   ] = ONCAP1D_ONCAP1D[i];
        return;
    }
    if ((0x0124 <= addr) && (addr <= (0x0124)+(64*3)) && ((addr-0x0124)%64 == 0)) {
        uint i = (addr - 0x0124)/64;
        (*(ONCAP2D   [i]))["ONCAP2D"   ] = ONCAP2D_ONCAP2D[i];
        return;
    }
    if ((0x0130 <= addr) && (addr <= (0x0130)+(64*3)) && ((addr-0x0130)%64 == 0)) {
        uint i = (addr - 0x0130)/64;
        (*(OFCAP1D   [i]))["OFCAP1D"   ] = OFCAP1D_OFCAP1D[i];
        return;
    }
    if ((0x0134 <= addr) && (addr <= (0x0134)+(64*3)) && ((addr-0x0134)%64 == 0)) {
        uint i = (addr - 0x0134)/64;
        (*(OFCAP2D   [i]))["OFCAP2D"   ] = OFCAP2D_OFCAP2D[i];
        return;
    }
    if (addr == 0x00D0) {
...
/// @return vector
std::vector<std::string> Catu5_timerd_sub_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Catu5_timerd_sub_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Catu5_timerd_sub_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Catu5_timerd_sub_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Catu5_timerd_sub_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Catu5_timerd_sub_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Catu5_timerd_sub_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Catu5_timerd_sub_regif::uint Catu5_timerd_sub_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return none
void Catu5_timerd_sub_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Catu5_timerd_sub_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Catu5_timerd_sub_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Catu5_timerd_sub_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Catu5_timerd_sub_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Catu5_timerd_sub_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Catu5_timerd_sub_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
