// ---------------------------------------------------------------------
// $Id: MECCCAP_ID_PEn_FCC2_Func.cpp,v 1.9 2019/09/26 10:07:30 landang Exp $
//
...
//========================================================
/// Constructor
MECCCAP_ID_PEn_FCC2_Func::MECCCAP_ID_PEn_FCC2_Func (std::string name, unsigned int pe_id, ECC_ICACHE_E2_FCC2_AgentController* ICACHEAgentController):
                Cmecccap_id_pen_fcc2_regif(name, 32)
                ,BusSlaveFuncIf()
{//{{{
    ...
    Cmecccap_id_pen_fcc2_regif::set_instance_name(name);
}//}}}

/// Destructor
MECCCAP_ID_PEn_FCC2_Func::~MECCCAP_ID_PEn_FCC2_Func()
{//{{{
}//}}}

/// Reset Func and registers
void MECCCAP_ID_PEn_FCC2_Func::EnableReset(const std::string reset_name, const bool is_active)
{//{{{
    if (reset_name == "sys_resetz"){
        Cmecccap_id_pen_fcc2_regif::EnableReset(is_active);
    }
}//}}}

/// Read API (target socket)
void MECCCAP_ID_PEn_FCC2_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void MECCCAP_ID_PEn_FCC2_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void MECCCAP_ID_PEn_FCC2_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cmecccap_id_pen_fcc2_regif::reg_handle_command(cmd);
                if (msg_return != "") {
            ...
            msg_return = Cmecccap_id_pen_fcc2_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
bool MECCCAP_ID_PEn_FCC2_Func::Set1bitStatusAndAddress(unsigned int edl_code, unsigned int mod_address)
{//{{{
    unsigned int cap_depth = mpICACHEAgentController->GetCapDepth("ID");
...
void MECCCAP_ID_PEn_FCC2_Func::SetFlagAddressBufferOverflow()
{//{{{
...
bool MECCCAP_ID_PEn_FCC2_Func::CheckHasRoomFor1bit()
{//{{{
    unsigned int cap_depth = mpICACHEAgentController->GetCapDepth("ID");
    unsigned int status = (unsigned int)(*ID_SERSTR);
    bool is_find_room = false;
...
/// Set FIFO overflow
void MECCCAP_ID_PEn_FCC2_Func::SetFIFOOverflow ()
{//{{{
...
/// Get SEDL bit
unsigned int MECCCAP_ID_PEn_FCC2_Func::GetSEDLBit(unsigned int index)
{//{{{
    return (unsigned int)(*ID_nSEADR[index])["SEDL"];
}//}}}

/// Get SEADR bit
unsigned int MECCCAP_ID_PEn_FCC2_Func::GetSEADRBit(unsigned int index)
{//{{{
    return (unsigned int)(*ID_nSEADR[index])["SEADR0"];
}//}}}

/// Get DEDL bit
unsigned int MECCCAP_ID_PEn_FCC2_Func::GetDEDLBit()
{//{{{
    return (unsigned int)(*ID_00DEADR)["DEDL"];
}//}}}

/// Get DEADR bit
unsigned int MECCCAP_ID_PEn_FCC2_Func::GetDEADRBit()
{//{{{
    return (unsigned int)(*ID_00DEADR)["DEADR0"];
}//}}}

/// Check no fatal status
bool MECCCAP_ID_PEn_FCC2_Func::CheckNoFatalStatus()
{//{{{
    return ((unsigned int)(*ID_DERSTR) == 0);
}//}}}

/// Set 2bit error status
void MECCCAP_ID_PEn_FCC2_Func::Set2bitErrorStatus()
{//{{{
    (*ID_DERSTR)["DEDF00"] = 1;
}//}}}

/// Set address feedback error status
void MECCCAP_ID_PEn_FCC2_Func::SetAddrFeedbackErrorStatus()
{//{{{
    (*ID_DERSTR)["AFEF00"] = 1;
}//}}}

/// Set EDL and error address of fatal
void MECCCAP_ID_PEn_FCC2_Func::SetFatalAddress(unsigned int edl_code, unsigned int mod_address)
{//{{{
...
unsigned int MECCCAP_ID_PEn_FCC2_Func::GetID_SERSTR()
{//{{{
    return (unsigned int)(*ID_SERSTR);
}//}}}

/// Get value of ID_DERSTR register
unsigned int MECCCAP_ID_PEn_FCC2_Func::GetID_DERSTR()
{//{{{
    return (unsigned int)(*ID_DERSTR);
}//}}}

/// Get value of ID_OVFSTR register
unsigned int MECCCAP_ID_PEn_FCC2_Func::GetID_OVFSTR()
{//{{{
    return (unsigned int)(*ID_OVFSTR);
}//}}}

/// Write ERRINT_INIT to ERRINT reg after reset
void MECCCAP_ID_PEn_FCC2_Func::WriteERRINT_INIT(unsigned int value)
{//{{{
...
/// Get ERRINT register
unsigned int MECCCAP_ID_PEn_FCC2_Func::GetERRINT()
{//{{{
...
void MECCCAP_ID_PEn_FCC2_Func::cb_ID_SSTCLR_ID_SSTCLR(RegCBstr str)
{//{{{ 
...
void MECCCAP_ID_PEn_FCC2_Func::cb_ID_DSTCLR_DSTCLR00(RegCBstr str)
{//{{{
    // clear DED, AFE.
...
void MECCCAP_ID_PEn_FCC2_Func::cb_ID_OVFCLR_SERROVFCLR0(RegCBstr str)
{//{{{ 
