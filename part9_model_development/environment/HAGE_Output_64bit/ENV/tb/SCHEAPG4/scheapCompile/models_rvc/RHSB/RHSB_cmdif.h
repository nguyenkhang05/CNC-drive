// -----------------------------------------------------------------------------
// $Id: RHSB_cmdif.h,v 1.3 2018/08/11 07:54:56 ngathunguyen Exp $
//
...
//      gen_cmdif.py,v 1.29 2014/03/13 06:27:32 sontran Exp
//      handleCommand.skl,v 1.20 2014/03/13 06:26:40 sontran Exp 
//      modelinfo_parser.py,v 1.2 2011/03/17 05:08:51 sontran Exp
//
//
// Input file contents
//
//%MODULE RHSB
//  %CMD_NODUMP_API CommandCB DumpProfile ClearProfile AssertReset EnableInsertInput EnableDumpResult
//  %CMD_ACTION SetMessageLevel <msg_lv: std::string>: void @ Set message level function
//  %CMD_ACTION DumpRegisterRW <is_enable: std::string>: void @ Enable/disable dumping register access information
//  %CMD_ACTION DumpInterrupt <is_enable: std::string>: void @ Enable/disable dumping interrupt information
//  %CMD_ACTION AssertReset <reset_name: std::string><start_time: double> <period: double>: void @ Set the reset function
//  %CMD_ACTION SetCLKFreq <clock_name: std::string><freq: sc_dt::uint64><unit: std::string>: void @ Set clock frequency
//  %CMD_ACTION GetCLKFreq <clock_name: std::string>: void @ Get clock frequency
//  %CMD_ACTION ForceRegister <register_name: std::string><value: unsigned int>: void @ Force a value to register
//  %CMD_ACTION ReleaseRegister <register_name: std::string>: void @ Release the forced value of register
//  %CMD_ACTION WriteRegister <register_name: std::string><value: unsigned int>: void @ Write a value to register
//  %CMD_ACTION ReadRegister <register_name: std::string>: void @ Read a value from register
//  %CMD_ACTION ListRegister : void @ list all registers of model
//  %CMD_ACTION Help <type: std::string>: void @ list all registers of model
...
#endif // EnableDumpResult
    ret += "    SetMessageLevel <msg_lv>                 Set message level function\n";
    ret += "    DumpRegisterRW <is_enable>                Enable/disable dumping register access information\n";
    ret += "    DumpInterrupt <is_enable>                 Enable/disable dumping interrupt information\n";
    ret += "    AssertReset <reset_name> <start_time> <period>\n";
    ret += "                                              Set the reset function\n";
    ret += "    SetCLKFreq <clock_name> <freq> <unit>     Set clock frequency\n";
    ret += "    GetCLKFreq <clock_name>                   Get clock frequency\n";
    ret += "    ForceRegister <register_name> <value>     Force a value to register\n";
    ret += "    ReleaseRegister <register_name>           Release the forced value of register\n";
    ret += "    WriteRegister <register_name> <value>     Write a value to register\n";
    ret += "    ReadRegister <register_name>              Read a value from register\n";
    ret += "    ListRegister                              list all registers of model\n";
    ret += "    Help <type>                               list all registers of model\n";
...
std::string user_def_command(std::vector<std::string>& args)
{
  ...
  if (0) {}
  else if (args[0] == "SetMessageLevel") {
    if ((int)args.size() > 1 && (int)args.size() <= 2) {
      std::string msg_lv;
      if ((int)args.size() >= 2) {
        msg_lv = (std::string)args[1];
      }
      SetMessageLevel(msg_lv);
    }
    else {
      ret = err_msg + "wrong number of arguments" + specified_cmd;
    }
  }
  else if (args[0] == "DumpRegisterRW") {
    if ((int)args.size() > 1 && (int)args.size() <= 2) {
      std::string is_enable;
      if ((int)args.size() >= 2) {
        is_enable = (std::string)args[1];
      }
      DumpRegisterRW(is_enable);
    }
    else {
      ret = err_msg + "wrong number of arguments" + specified_cmd;
    }
    ...
    if ((int)args.size() > 3 && (int)args.size() <= 4) {
      ...
      AssertReset(reset_name, start_time, period);
    }
    else {
      ret = err_msg + "wrong number of arguments" + specified_cmd;
    }
    ...
    if ((int)args.size() > 3 && (int)args.size() <= 4) {
      ...
      std::string unit;
      if ((int)args.size() >= 4) {
        unit = (std::string)args[3];
      }
      SetCLKFreq(clock_name, freq, unit);
    }
    else {
      ret = err_msg + "wrong number of arguments" + specified_cmd;
    }
  }
  else if (args[0] == "GetCLKFreq") {
    if ((int)args.size() > 1 && (int)args.size() <= 2) {
      std::string clock_name;
      if ((int)args.size() >= 2) {
        clock_name = (std::string)args[1];
      }
      GetCLKFreq(clock_name);
    }
    else {
      ret = err_msg + "wrong number of arguments" + specified_cmd;
    }
  }
  else if (args[0] == "ForceRegister") {
    if ((int)args.size() > 2 && (int)args.size() <= 3) {
      std::string register_name;
      if ((int)args.size() >= 2) {
        register_name = (std::string)args[1];
      }
      unsigned int value;
      if ((int)args.size() >= 3) {
        if (args[2][0] == '-') {
        ...
        if (!str2num(args[2], value)) {
          ret = err_msg + "wrong argument" + specified_cmd;
          return ret;
        }
      }
      ForceRegister(register_name, value);
    }
    else {
      ret = err_msg + "wrong number of arguments" + specified_cmd;
    }
  }
  else if (args[0] == "ReleaseRegister") {
    if ((int)args.size() > 1 && (int)args.size() <= 2) {
      std::string register_name;
      if ((int)args.size() >= 2) {
        register_name = (std::string)args[1];
      }
      ReleaseRegister(register_name);
    }
    else {
      ret = err_msg + "wrong number of arguments" + specified_cmd;
    }
  }
  else if (args[0] == "WriteRegister") {
    if ((int)args.size() > 2 && (int)args.size() <= 3) {
      std::string register_name;
      if ((int)args.size() >= 2) {
        register_name = (std::string)args[1];
      ...
      WriteRegister(register_name, value);
    }
    else {
      ret = err_msg + "wrong number of arguments" + specified_cmd;
    }
  }
  else if (args[0] == "ReadRegister") {
    if ((int)args.size() > 1 && (int)args.size() <= 2) {
      std::string register_name;
      if ((int)args.size() >= 2) {
        register_name = (std::string)args[1];
