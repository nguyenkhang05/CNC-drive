// -----------------------------------------------------------------------------
// $Id: ecccnt_crcn_fcc2_regif.cpp,v 1.5 2019/09/06 08:48:32 landang Exp $
//
...
// 
////////////////////////////////////////////////////////////////////////////////
/// @file ecccnt_crcn_fcc2_regif.cpp
...
/// @return none
Cecccnt_crcn_fcc2_regif::Cecccnt_crcn_fcc2_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    CRCAFCTL      = new vpcl::re_register(0x0000, this, "CRCAFCTL"     , name.c_str());
    CRCECCTSTCTL  = new vpcl::re_register(0x0010, this, "CRCECCTSTCTL" , name.c_str());
    CRCAFINV      = new vpcl::re_register(0x0020, this, "CRCAFINV"     , name.c_str());
    CRCTDATBFECCF = new vpcl::re_register(0x0060, this, "CRCTDATBFECCF", name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_CRCAFCTL       , CRCAFCTL->name()     , CRCAFCTL->addr()     );
    cw_instantiate_reg(CW_CRCECCTSTCTL   , CRCECCTSTCTL->name() , CRCECCTSTCTL->addr() );
    cw_instantiate_reg(CW_CRCAFINV       , CRCAFINV->name()     , CRCAFINV->addr()     );
    cw_instantiate_reg(CW_CRCTDATBFECCF  , CRCTDATBFECCF->name(), CRCTDATBFECCF->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(CRCAFCTL       , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(CRCECCTSTCTL   , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(CRCAFINV       , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(CRCTDATBFECCF  , mCurReg, 0,  4,      "32", "8|16|32");

    mRegList = mCurReg;

    (*CRCAFCTL       ) ( 3,  3, "AFEDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CRCECCTSTCTL   ) ( 1,  1, "ECCTST"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CRCECCTSTCTL   ) ( 0,  0, "DATSEL"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CRCAFINV       ) (19,  4, "AFINV"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*CRCTDATBFECCF  ) (30, 24, "BFECC3"  , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*CRCTDATBFECCF  ) (22, 16, "BFECC2"  , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*CRCTDATBFECCF  ) (14,  8, "BFECC1"  , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*CRCTDATBFECCF  ) ( 6,  0, "BFECC0"  , 0x0       , "R"     , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["CRCAFCTL"   ]["AFEDIS"     ] = &Cecccnt_crcn_fcc2_regif::cb_CRCAFCTL_AFEDIS;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cecccnt_crcn_fcc2_regif::~Cecccnt_crcn_fcc2_regif()
{
...
Cecccnt_crcn_fcc2_regif::uint Cecccnt_crcn_fcc2_regif::bit_select(
...
bool Cecccnt_crcn_fcc2_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cecccnt_crcn_fcc2_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_crcn_fcc2_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                         const unsigned char *p_data,   ///< [in] Writing data
                                         cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_crcn_fcc2_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                         unsigned char *p_data,   ///< [out] Reading data
                                         cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecccnt_crcn_fcc2_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                             const unsigned char *p_data,  ///< [in] Writing data
                                             cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecccnt_crcn_fcc2_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                             unsigned char *p_data,   ///< [out] Reading data
                                             cuint size,              ///< [in]  Data size (byte)
                                             bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cecccnt_crcn_fcc2_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                          unsigned char *p_data,          ///< [out] Reading data
                                          cuint size,                     ///< [in]  Data size (byte)
...
bool Cecccnt_crcn_fcc2_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                          const unsigned char *p_data,      ///< [in] Writing data
                                          cuint size,                       ///< [in] Data size (byte)
                                          cuint reg_index,                  ///< [in] Register index
...
bool Cecccnt_crcn_fcc2_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cecccnt_crcn_fcc2_regif::CommandInit()
{
...
/// @return string
std::string Cecccnt_crcn_fcc2_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cecccnt_crcn_fcc2_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cecccnt_crcn_fcc2_regif::InitLocalVal()
{
    // Initialize local variables
    CRCAFCTL_AFEDIS = 0x0;
    CRCECCTSTCTL_ECCTST = 0x0;
    CRCECCTSTCTL_DATSEL = 0x0;
    CRCAFINV_AFINV = 0x0;
    CRCTDATBFECCF_BFECC3 = 0x0;
    CRCTDATBFECCF_BFECC2 = 0x0;
    CRCTDATBFECCF_BFECC1 = 0x0;
    CRCTDATBFECCF_BFECC0 = 0x0;
}

/// Update bit value to local value
/// @return None
void Cecccnt_crcn_fcc2_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        CRCAFCTL_AFEDIS                   = (*CRCAFCTL     )["AFEDIS"       ];
        return;
    }
    if (addr == 0x0010) {
        CRCECCTSTCTL_ECCTST               = (*CRCECCTSTCTL )["ECCTST"       ];
        CRCECCTSTCTL_DATSEL               = (*CRCECCTSTCTL )["DATSEL"       ];
        return;
    }
    if (addr == 0x0020) {
        CRCAFINV_AFINV                    = (*CRCAFINV     )["AFINV"        ];
        return;
    }
    if (addr == 0x0060) {
        CRCTDATBFECCF_BFECC3              = (*CRCTDATBFECCF)["BFECC3"       ];
        CRCTDATBFECCF_BFECC2              = (*CRCTDATBFECCF)["BFECC2"       ];
        CRCTDATBFECCF_BFECC1              = (*CRCTDATBFECCF)["BFECC1"       ];
        CRCTDATBFECCF_BFECC0              = (*CRCTDATBFECCF)["BFECC0"       ];
        return;
    }
}
...
/// @return None
void Cecccnt_crcn_fcc2_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*CRCAFCTL     )["AFEDIS"       ] = CRCAFCTL_AFEDIS;
        return;
    }
    if (addr == 0x0010) {
        (*CRCECCTSTCTL )["ECCTST"       ] = CRCECCTSTCTL_ECCTST;
        (*CRCECCTSTCTL )["DATSEL"       ] = CRCECCTSTCTL_DATSEL;
        return;
    }
    if (addr == 0x0020) {
        (*CRCAFINV     )["AFINV"        ] = CRCAFINV_AFINV;
        return;
    }
    if (addr == 0x0060) {
        (*CRCTDATBFECCF)["BFECC3"       ] = CRCTDATBFECCF_BFECC3;
        (*CRCTDATBFECCF)["BFECC2"       ] = CRCTDATBFECCF_BFECC2;
        (*CRCTDATBFECCF)["BFECC1"       ] = CRCTDATBFECCF_BFECC1;
        (*CRCTDATBFECCF)["BFECC0"       ] = CRCTDATBFECCF_BFECC0;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cecccnt_crcn_fcc2_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cecccnt_crcn_fcc2_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cecccnt_crcn_fcc2_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cecccnt_crcn_fcc2_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cecccnt_crcn_fcc2_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cecccnt_crcn_fcc2_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cecccnt_crcn_fcc2_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cecccnt_crcn_fcc2_regif::uint Cecccnt_crcn_fcc2_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cecccnt_crcn_fcc2_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cecccnt_crcn_fcc2_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cecccnt_crcn_fcc2_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cecccnt_crcn_fcc2_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cecccnt_crcn_fcc2_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cecccnt_crcn_fcc2_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cecccnt_crcn_fcc2_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
