// -----------------------------------------------------------------------------
// $Id: dnfp010_e2_sub_regif.cpp 5605 2017-03-08 02:30:35Z ChinhTD $
//
...
//
// Input file : dnfp010_e2_sub_regif.txt
////////////////////////////////////////////////////////////////////////////////
...
// 
////////////////////////////////////////////////////////////////////////////////
/// @file dnfp010_e2_sub_regif.cpp
/// @brief Register IF class of model DNFP010_E2_SUB
/// $Id: dnfp010_e2_sub_regif.cpp 2241 2017-03-08 02:30:35Z ChinhTD $
/// $Date: 2017-03-08 09:30:35 +0700 (Wed, 08 Mar 2017) $
/// $Revison$
/// $Author: ChinhTD $
////////////////////////////////////////////////////////////////////////////////
...
/// @return none
Cdnfp010_e2_sub_regif::Cdnfp010_e2_sub_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    DNFP01CTL = new vpcl::re_register(0x0000, this, "DNFP01CTL", name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_DNFP01CTL  , DNFP01CTL->name(), DNFP01CTL->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(DNFP01CTL  , mCurReg, 0,  1,       "8", "8|16|32");

    mRegList = mCurReg;

    (*DNFP01CTL  ) ( 7,  7, "NFEN"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*DNFP01CTL  ) ( 6,  5, "SLST"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*DNFP01CTL  ) ( 2,  0, "PRS"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["DNFP01CTL"]["SLST"   ] = &Cdnfp010_e2_sub_regif::cb_DNFP01CTL_SLST;
    mWrCbAPI["DNFP01CTL"]["PRS"    ] = &Cdnfp010_e2_sub_regif::cb_DNFP01CTL_PRS;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cdnfp010_e2_sub_regif::~Cdnfp010_e2_sub_regif()
{
...
Cdnfp010_e2_sub_regif::uint Cdnfp010_e2_sub_regif::bit_select(
...
bool Cdnfp010_e2_sub_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cdnfp010_e2_sub_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cdnfp010_e2_sub_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                       const unsigned char *p_data,   ///< [in] Writing data
                                       cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cdnfp010_e2_sub_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                       unsigned char *p_data,   ///< [out] Reading data
                                       cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cdnfp010_e2_sub_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                           const unsigned char *p_data,  ///< [in] Writing data
                                           cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cdnfp010_e2_sub_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                           unsigned char *p_data,   ///< [out] Reading data
                                           cuint size,              ///< [in]  Data size (byte)
                                           bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cdnfp010_e2_sub_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                        unsigned char *p_data,          ///< [out] Reading data
                                        cuint size,                     ///< [in]  Data size (byte)
...
bool Cdnfp010_e2_sub_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                        const unsigned char *p_data,      ///< [in] Writing data
                                        cuint size,                       ///< [in] Data size (byte)
                                        cuint reg_index,                  ///< [in] Register index
...
bool Cdnfp010_e2_sub_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cdnfp010_e2_sub_regif::CommandInit()
{
...
/// @return string
std::string Cdnfp010_e2_sub_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cdnfp010_e2_sub_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cdnfp010_e2_sub_regif::InitLocalVal()
{
    // Initialize local variables
    DNFP01CTL_NFEN = 0x0;
    DNFP01CTL_SLST = 0x0;
...
/// @return None
void Cdnfp010_e2_sub_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        DNFP01CTL_NFEN                = (*DNFP01CTL)["NFEN"     ];
        DNFP01CTL_SLST                = (*DNFP01CTL)["SLST"     ];
        DNFP01CTL_PRS                 = (*DNFP01CTL)["PRS"      ];
        return;
    }
}
...
/// @return None
void Cdnfp010_e2_sub_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*DNFP01CTL)["NFEN"     ] = DNFP01CTL_NFEN;
        (*DNFP01CTL)["SLST"     ] = DNFP01CTL_SLST;
        (*DNFP01CTL)["PRS"      ] = DNFP01CTL_PRS;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cdnfp010_e2_sub_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cdnfp010_e2_sub_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cdnfp010_e2_sub_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cdnfp010_e2_sub_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cdnfp010_e2_sub_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cdnfp010_e2_sub_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cdnfp010_e2_sub_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cdnfp010_e2_sub_regif::uint Cdnfp010_e2_sub_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cdnfp010_e2_sub_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cdnfp010_e2_sub_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cdnfp010_e2_sub_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cdnfp010_e2_sub_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
unsigned char Cdnfp010_e2_sub_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
unsigned char Cdnfp010_e2_sub_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(unsigned char));
    return sizeof(unsigned char);
}
void Cdnfp010_e2_sub_regif::cw_instantiate_reg(scml2::reg<unsigned char> * reg, std::string reg_name, int offset)
{
