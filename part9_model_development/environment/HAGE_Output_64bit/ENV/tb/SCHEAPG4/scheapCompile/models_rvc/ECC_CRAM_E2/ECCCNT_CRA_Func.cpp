// ---------------------------------------------------------------------
// $Id: ECCCNT_CRA_Func.cpp,v 1.1 2019/09/26 10:07:06 landang Exp $
//
...
//========================================================
/// Constructor
ECCCNT_CRA_Func::ECCCNT_CRA_Func (std::string name, ECC_CRAM_E2_AgentController* CRAMAgentController):
                Cecccnt_cra_regif(name, 32)
                ,BusSlaveFuncIf()
{//{{{
    ...
    Cecccnt_cra_regif::set_instance_name(name);
}//}}}

/// Destructor
ECCCNT_CRA_Func::~ECCCNT_CRA_Func()
{//{{{
}//}}}

/// Reset Func and registers
void ECCCNT_CRA_Func::EnableReset(const std::string reset_name, const bool is_active)
{//{{{
    if (reset_name == "sys_resetz"){
        Cecccnt_cra_regif::EnableReset(is_active);
    }
}//}}}

/// Read API (target socket)
void ECCCNT_CRA_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    // Get extension info
...
void ECCCNT_CRA_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    // Get extension info
    TlmG3mExtension *extension;
    trans.get_extension(extension);
...
void ECCCNT_CRA_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cecccnt_cra_regif::reg_handle_command(cmd);
                if (msg_return != "") {
            ...
            msg_return = Cecccnt_cra_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
unsigned int ECCCNT_CRA_Func::GetCRAECCCTLReg()
{//{{{
...
void ECCCNT_CRA_Func::cb_CRAECCCTL_ECCDIS(RegCBstr str)
{//{{{ 
