// -----------------------------------------------------------------------------
// $Id: hwcore_iir_regif.cpp,v 1.1 2020/08/18 07:30:11 minhquocha Exp $
//
...
// Corporation.
// -----------------------------------------------------------------------------
// Description: Copyright of hwcore_iir_regif.cpp
// Ref: {HWCORE_IIR_UD_Copyright_001}
// Description: Register IF generator hwcore_iir_regif.cpp
// Ref: {HWCORE_IIR_UD_ATT1_001, HWCORE_IIR_UD_ATT1_002}
...
////////////////////////////////////////////////////////////////////////////////
/// @file hwcore_iir_regif.cpp
/// @brief Register IF class of model HWCORE_IIR
/// $Id: hwcore_iir_regif.cpp,v 1.1 2020/08/18 07:30:11 minhquocha Exp $
/// $Date: 2020/08/18 07:30:11 $
/// $Revison$
/// $Author: minhquocha $
////////////////////////////////////////////////////////////////////////////////
...
/// @return none
// Ref: {HWCORE_IIR_UD_ATT1_004, HWCORE_IIR_UD_ATT1_007, HWCORE_IIR_UD_ATT1_009, HWCORE_IIR_UD_Registers_001, HWCORE_IIR_UD_Registers_002, HWCORE_IIR_UD_Registers_003, HWCORE_IIR_UD_Registers_004, HWCORE_IIR_UD_Registers_005, HWCORE_IIR_UD_Registers_006, HWCORE_IIR_UD_Registers_007, HWCORE_IIR_UD_Registers_008, HWCORE_IIR_UD_Registers_009, HWCORE_IIR_UD_Registers_010, HWCORE_IIR_UD_Registers_011, HWCORE_IIR_UD_Registers_012, HWCORE_IIR_UD_Registers_013, HWCORE_IIR_UD_Registers_014, HWCORE_IIR_UD_Registers_015}
Chwcore_iir_regif::Chwcore_iir_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef USR_CWR_SYSTEMC
    , cwmem("register", 0x620)
    #endif
{
    CommandInit();
    IIRINIT   = new vpcl::re_register(0x05C4, this, "IIRINIT"  , name.c_str());
    IIRSFT    = new vpcl::re_register(0x05C8, this, "IIRSFT"   , name.c_str());
    IIRRLD    = new vpcl::re_register(0x05CC, this, "IIRRLD"   , name.c_str());
    IIRRSL    = new vpcl::re_register(0x05CD, this, "IIRRSL"   , name.c_str());
    IIRSTAT   = new vpcl::re_register(0x05CE, this, "IIRSTAT"  , name.c_str());
    IIRSTATC  = new vpcl::re_register(0x05CF, this, "IIRSTATC" , name.c_str());
    IIRICOEF  = new vpcl::re_register(0x05D0, this, "IIRICOEF" , name.c_str());
    IIROCOEF  = new vpcl::re_register(0x05E8, this, "IIROCOEF" , name.c_str());
    std::ostringstream str_tmp;
    for(uint i = 0; i < emNUM_IIRCTR; i++) {
        str_tmp.str("");
        str_tmp<<"IIRCTR"<< i;
        IIRCTR   [i] = new vpcl::re_register((0x05C0) + i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_IIRCOEF; i++) {
        str_tmp.str("");
        str_tmp<<"IIRCOEF"<< i;
        IIRCOEF  [i] = new vpcl::re_register((0x05D4) + 2*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_IIRSFTDAT; i++) {
        str_tmp.str("");
        str_tmp<<"IIRSFTDAT"<< i;
        IIRSFTDAT[i] = new vpcl::re_register((0x05F0) + 4*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_IIRZN1DAT; i++) {
        str_tmp.str("");
        str_tmp<<"IIRZN1DAT"<< i;
        IIRZN1DAT[i] = new vpcl::re_register((0x05FC) + 4*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_IIRZN2DAT; i++) {
        str_tmp.str("");
        str_tmp<<"IIRZN2DAT"<< i;
        IIRZN2DAT[i] = new vpcl::re_register((0x0608) + 4*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_IIROUTDAT; i++) {
        str_tmp.str("");
        str_tmp<<"IIROUTDAT"<< i;
        IIROUTDAT[i] = new vpcl::re_register((0x0614) + 4*i, this, str_tmp.str(), name.c_str());
    }
    ...
    for(uint i = 0; i < emNUM_IIRCTR;   i++) {
        mRegMap[(0x05C0)     + i*1 ] = index++;
    }
    mRegMap[0x05C4]     = index++;
    mRegMap[0x05C8]     = index++;
    mRegMap[0x05CC]     = index++;
    mRegMap[0x05CD]     = index++;
    mRegMap[0x05CE]     = index++;
    mRegMap[0x05CF]     = index++;
    mRegMap[0x05D0]     = index;
    mRegMap[0x05D0 + 1] = index;
    mRegMap[0x05D0 + 2] = index;
    mRegMap[0x05D0 + 3] = index++;
    for(uint i = 0; i < emNUM_IIRCOEF;  i++) {
        mRegMap[(0x05D4)     + i*2 ] = index;
        mRegMap[(0x05D4 + 1) + i*2 ] = index++;
    }
    mRegMap[0x05E8]     = index;
    mRegMap[0x05E8 + 1] = index;
    mRegMap[0x05E8 + 2] = index;
    mRegMap[0x05E8 + 3] = index++;
    for(uint i = 0; i < emNUM_IIRSFTDAT; i++) {
        mRegMap[(0x05F0)     + i*4 ] = index;
        mRegMap[(0x05F0 + 1) + i*4 ] = index;
        mRegMap[(0x05F0 + 2) + i*4 ] = index;
        mRegMap[(0x05F0 + 3) + i*4 ] = index++;
    }
    for(uint i = 0; i < emNUM_IIRZN1DAT; i++) {
        mRegMap[(0x05FC)     + i*4 ] = index;
        mRegMap[(0x05FC + 1) + i*4 ] = index;
        mRegMap[(0x05FC + 2) + i*4 ] = index;
        mRegMap[(0x05FC + 3) + i*4 ] = index++;
    }
    for(uint i = 0; i < emNUM_IIRZN2DAT; i++) {
        mRegMap[(0x0608)     + i*4 ] = index;
        mRegMap[(0x0608 + 1) + i*4 ] = index;
        mRegMap[(0x0608 + 2) + i*4 ] = index;
        mRegMap[(0x0608 + 3) + i*4 ] = index++;
    }
    for(uint i = 0; i < emNUM_IIROUTDAT; i++) {
        mRegMap[(0x0614)     + i*4 ] = index;
        mRegMap[(0x0614 + 1) + i*4 ] = index;
        mRegMap[(0x0614 + 2) + i*4 ] = index;
        mRegMap[(0x0614 + 3) + i*4 ] = index++;
    }
    mTotalRegNum = index;
    mRegArray = new SRegList* [mTotalRegNum];
    index = 0;
#ifdef USR_CWR_SYSTEMC
    for(uint i = 0; i < emNUM_IIRCTR   ; i++) mCurReg = mRegArray[index++] = new SRegList(IIRCTR     [i], mCurReg, i,  1,       "8", "8|16|32", false, &cwmem);
#else
    for(uint i = 0; i < emNUM_IIRCTR   ; i++) mCurReg = mRegArray[index++] = new SRegList(IIRCTR     [i], mCurReg, i,  1,       "8", "8|16|32");
#endif
    mCurReg = mRegArray[index++] = new SRegList(IIRINIT    , mCurReg, 0,  1,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(IIRSFT     , mCurReg, 0,  1,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(IIRRLD     , mCurReg, 0,  1,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(IIRRSL     , mCurReg, 0,  1,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(IIRSTAT    , mCurReg, 0,  1,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(IIRSTATC   , mCurReg, 0,  1,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(IIRICOEF   , mCurReg, 0,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_IIRCOEF  ; i++) mCurReg = mRegArray[index++] = new SRegList(IIRCOEF    [i], mCurReg, i,  2,      "16", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(IIROCOEF   , mCurReg, 0,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_IIRSFTDAT; i++) mCurReg = mRegArray[index++] = new SRegList(IIRSFTDAT  [i], mCurReg, i,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_IIRZN1DAT; i++) mCurReg = mRegArray[index++] = new SRegList(IIRZN1DAT  [i], mCurReg, i,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_IIRZN2DAT; i++) mCurReg = mRegArray[index++] = new SRegList(IIRZN2DAT  [i], mCurReg, i,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_IIROUTDAT; i++) mCurReg = mRegArray[index++] = new SRegList(IIROUTDAT  [i], mCurReg, i,  4,      "32", "8|16|32");

    mRegList = mCurReg;

    (*IIRINIT    ) ( 2,  2, "INIT2"   , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*IIRINIT    ) ( 1,  1, "INIT1"   , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*IIRINIT    ) ( 0,  0, "INIT0"   , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*IIRSFT     ) ( 4,  4, "TRG"     , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*IIRSFT     ) ( 1,  0, "TRGSEL"  , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*IIRRLD     ) ( 2,  2, "RLD2"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*IIRRLD     ) ( 1,  1, "RLD1"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*IIRRLD     ) ( 0,  0, "RLD0"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*IIRRSL     ) ( 1,  0, "RSL"     , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*IIRSTAT    ) ( 2,  2, "STAT2"   , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*IIRSTAT    ) ( 1,  1, "STAT1"   , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*IIRSTAT    ) ( 0,  0, "STAT0"   , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*IIRSTATC   ) ( 2,  2, "STATC2"  , 0x0       , "R:0|W" , vpcl::SPP_ENABLE);
    (*IIRSTATC   ) ( 1,  1, "STATC1"  , 0x0       , "R:0|W" , vpcl::SPP_ENABLE);
    (*IIRSTATC   ) ( 0,  0, "STATC0"  , 0x0       , "R:0|W" , vpcl::SPP_ENABLE);
    (*IIRICOEF   ) (31,  0, "DATA"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*IIROCOEF   ) (31,  0, "DATA"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    for(uint i = 0; i < emNUM_IIRZN2DAT; i++) {
        (*IIRZN2DAT  [i]) (31,  0, "DATA"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_IIRCOEF; i++) {
        (*IIRCOEF    [i]) (15,  0, "DATA"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_IIRZN1DAT; i++) {
        (*IIRZN1DAT  [i]) (31,  0, "DATA"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_IIROUTDAT; i++) {
        (*IIROUTDAT  [i]) (31,  0, "DATA"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_IIRSFTDAT; i++) {
        (*IIRSFTDAT  [i]) (31,  0, "DATA"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_IIRCTR; i++) {
        (*IIRCTR     [i]) ( 7,  4, "DATSEL"  , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*IIRCTR     [i]) ( 3,  0, "TRGSEL"  , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    }
    ...
    mWrCbAPI["IIRINIT"]["INIT0"  ] = &Chwcore_iir_regif::cb_IIRINIT_INIT0;
    mWrCbAPI["IIRSFT" ]["TRG"    ] = &Chwcore_iir_regif::cb_IIRSFT_TRG;
    mWrCbAPI["IIRRLD" ]["RLD0"   ] = &Chwcore_iir_regif::cb_IIRRLD_RLD0;
    mWrCbAPI["IIRRSL" ]["RSL"    ] = &Chwcore_iir_regif::cb_IIRRSL_RSL;

    #ifdef USE_WEB_SIM
    InitializeRegValueID();
    mNotifyRegValueIDAPI = &Chwcore_iir_regif::NotifyRegValueID;
    #endif
}

/// Destructor: delete pointers
/// @return none
Chwcore_iir_regif::~Chwcore_iir_regif()
{
...
Chwcore_iir_regif::uint Chwcore_iir_regif::bit_select(cuint val,    ///< [in] Writting address
...
// Ref: {HWCORE_IIR_UD_Direction_019, HWCORE_IIR_UD_Direction_018}
bool Chwcore_iir_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
// Ref: {HWCORE_IIR_UD_Direction_019, HWCORE_IIR_UD_Direction_018}
bool Chwcore_iir_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
// Ref: {HWCORE_IIR_UD_Direction_019, HWCORE_IIR_UD_Direction_018}
bool Chwcore_iir_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                   const unsigned char *p_data,   ///< [in] Writing data
                                   cuint size)                    ///< [in] Data size (byte)
{
    assert(size != 0);
...
// Ref: {HWCORE_IIR_UD_Direction_019, HWCORE_IIR_UD_Direction_018}
bool Chwcore_iir_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                   unsigned char *p_data,   ///< [out] Reading data
                                   cuint size)              ///< [in]  Data size (byte)
{
    assert(size != 0);
...
// Ref: {HWCORE_IIR_UD_Direction_020, HWCORE_IIR_UD_Direction_025, HWCORE_IIR_UD_Direction_018}
bool Chwcore_iir_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                       const unsigned char *p_data,  ///< [in] Writing data
                                       cuint size,                   ///< [in] Data size (byte)
...
// Ref: {HWCORE_IIR_UD_Direction_020, HWCORE_IIR_UD_Direction_024, HWCORE_IIR_UD_Direction_018}
bool Chwcore_iir_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                       unsigned char *p_data,   ///< [out] Reading data
                                       cuint size,              ///< [in]  Data size (byte)
                                       bool IsDbgFunc)          ///< [in]  Flag indicate reg_rd or reg_rd_dbg
{
...
// Ref: {HWCORE_IIR_UD_Direction_030, HWCORE_IIR_UD_Direction_018}
bool Chwcore_iir_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                    unsigned char *p_data,          ///< [out] Reading data
                                    cuint size,                     ///< [in]  Data size (byte)
...
// Ref: {HWCORE_IIR_UD_Direction_031, HWCORE_IIR_UD_Direction_018}
bool Chwcore_iir_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                    const unsigned char *p_data,      ///< [in] Writing data
                                    cuint size,                       ///< [in] Data size (byte)
                                    cuint reg_index,                  ///< [in] Register index
...
// Ref: {HWCORE_IIR_UD_Direction_020, HWCORE_IIR_UD_Direction_019, HWCORE_IIR_UD_Direction_018}
bool Chwcore_iir_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr, bool is_wr)
{
...
/// @return none
void Chwcore_iir_regif::CommandInit()
{
...
/// @return string
// Ref: {HWCORE_IIR_UD_Direction_022, HWCORE_IIR_UD_Direction_023, HWCORE_IIR_UD_Direction_018}
std::string Chwcore_iir_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Chwcore_iir_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Chwcore_iir_regif::InitializeRegValueID()
{//{{{
...
/// @return none
void Chwcore_iir_regif::CalcTargetRegVal(void)
{//{{{
...
/// @return none
void Chwcore_iir_regif::IssueRegValueID(void)
{//{{{
...
/// @return vector
std::vector<std::string> Chwcore_iir_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Chwcore_iir_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip, bool is_wr_data, uint acc_size)
{
    std::ostringstream str_tmp;
...
/// @return none
void Chwcore_iir_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
    if (mDumpRegisterRW) {
...
int Chwcore_iir_regif::get_reg_index(cuint access_addr)
{
    if (access_addr < (1<<11)) {
...
vpcl::re_register *Chwcore_iir_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Chwcore_iir_regif::next_reg_object()
{
    assert(mCurReg != NULL);
...
/// @return none
void Chwcore_iir_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Chwcore_iir_regif::uint Chwcore_iir_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
    uint data=0;
...
/// @return none
void Chwcore_iir_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Chwcore_iir_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Chwcore_iir_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
// Ref: {HWCORE_IIR_UD_Direction_036, HWCORE_IIR_UD_Direction_018}
void Chwcore_iir_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Chwcore_iir_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Chwcore_iir_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    int reg_index = get_reg_index((uint)tag);
    assert(reg_index != (1<<16));
    uint length = mRegArray[reg_index]->length;
...
void Chwcore_iir_regif::cw_set_all_callback_reg(void)
{
