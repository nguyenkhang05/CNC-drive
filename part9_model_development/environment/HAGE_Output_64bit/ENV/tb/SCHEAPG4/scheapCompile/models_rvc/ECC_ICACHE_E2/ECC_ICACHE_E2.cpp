// ---------------------------------------------------------------------
// $Id: ECC_ICACHE_E2.cpp,v 1.4 2019/10/23 08:29:08 landang Exp $
//
...
#include "ECC_def.h"
#include "ECC_ICACHE_E2.h"
#include "ECCCNT_IT_PEn_Func.h"
#include "ECCCNT_ID_PEn_Func.h"
#include "MECCCAP_IT_PEn_Func.h"
#include "MECCCAP_ID_PEn_Func.h"

/// Constructor and Destructor

ECC_ICACHE_E2::ECC_ICACHE_E2(sc_module_name name, const uint8_t iPE_NUM, const unsigned int rLatency, const unsigned int wLatency)
        :sc_module(name)
        ,ECC_ICACHE_E2_AgentController()
        ,PE_NUM(iPE_NUM)
        // Clock/Reset
        ,pclk("pclk")
        ,presetn("presetn")
...
{//{{{ 
    ...
    mpECCCNT_ITFunc = new ECCCNT_IT_PEn_Func*[PE_MAX_NUM];
    mpMECCCAP_ITFunc = new MECCCAP_IT_PEn_Func*[PE_MAX_NUM];
    mpECCCNT_IDFunc = new ECCCNT_ID_PEn_Func*[PE_MAX_NUM];
    mpMECCCAP_IDFunc = new MECCCAP_ID_PEn_Func*[PE_MAX_NUM];

    for (unsigned int i = 0; i < PE_MAX_NUM; i++){
        mpECCCNT_ITFunc[i] = new ECCCNT_IT_PEn_Func((std::string)name, i, this);
        mpMECCCAP_ITFunc[i] = new MECCCAP_IT_PEn_Func((std::string)name, i, this);
        mpECCCNT_IDFunc[i] = new ECCCNT_ID_PEn_Func((std::string)name, i, this);
        mpMECCCAP_IDFunc[i] = new MECCCAP_ID_PEn_Func((std::string)name, i, this);
    }
    ...
    for (unsigned int reset_id = 0; reset_id < emNumReset; reset_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::HandleResetHardMethod, this, reset_id),
    ...
    for (unsigned int reset_id = 0; reset_id < emNumReset; reset_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::HandleResetCmdMethod, this, reset_id),
    ...
    for (unsigned int reset_id = 0; reset_id < emNumReset; reset_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::CancelResetCmdMethod, this, reset_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::WriteDisPort_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::WriteDisPort_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::WriteERROVFOUT0_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::WriteERROVFOUT1_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::NegateERROVFOUT0_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::NegateERROVFOUT1_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::WriteSEDERROUT_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::AssertSEDERROUT_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::NegateSEDERROUT_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::NegateDEDERROUT_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::WriteDEDERROUT_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::NegateAFEERROUT_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::WriteAFEERROUT_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::Process1bitError_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::ProcessFatalError_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::SetFatalErrorStatus_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::NotifyFatalError_ITMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::WriteERROVFOUT0_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::WriteERROVFOUT1_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::NegateERROVFOUT0_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::NegateERROVFOUT1_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::WriteSEDERROUT_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::AssertSEDERROUT_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::NegateSEDERROUT_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::NegateDEDERROUT_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::WriteDEDERROUT_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::NegateAFEERROUT_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::WriteAFEERROUT_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::Process1bitError_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::ProcessFatalError_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::SetFatalErrorStatus_IDMethod, this, pe_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_ICACHE_E2::NotifyFatalError_IDMethod, this, pe_id),
...
{//{{{
    ...
    for (unsigned int i = 0; i < PE_NUM; i++){
        if (eccdisable_it[i] != NULL){
            delete eccdisable_it[i];
        }
        if (afedisable_it[i] != NULL){
            delete afedisable_it[i];
        }
        if (eccdisable_id[i] != NULL){
            delete eccdisable_id[i];
        }
        if (afedisable_id[i] != NULL){
            delete afedisable_id[i];
        }
        if (errovfout1_IT[i] != NULL){
            delete errovfout1_IT[i];
        }
        if (errovfout0_IT[i] != NULL){
            delete errovfout0_IT[i];
        }
        if (dederrout_IT[i] != NULL){
            delete dederrout_IT[i];
        }
        if (afeerrout_IT[i] != NULL){
            delete afeerrout_IT[i];
        }
        if (sederrout_IT[i] != NULL){
            delete sederrout_IT[i];
        }
        if (errovfout1_ID[i] != NULL){
            delete errovfout1_ID[i];
        }
        if (errovfout0_ID[i] != NULL){
            delete errovfout0_ID[i];
        }
        if (dederrout_ID[i] != NULL){
            delete dederrout_ID[i];
        }
        if (afeerrout_ID[i] != NULL){
            delete afeerrout_ID[i];
        }
        if (sederrout_ID[i] != NULL){
...
void ECC_ICACHE_E2::CreateEDLMap()
{//{{{
    std::string edl_name;
...
void ECC_ICACHE_E2::Initialize(void)
{//{{{
    for (unsigned int i = 0; i < PE_NUM; i++){
...
void ECC_ICACHE_E2::WriteDisPort_IT(unsigned int pe_id, unsigned int value)
{//{{{
...
void ECC_ICACHE_E2::WriteDisPort_ID(unsigned int pe_id, unsigned int value)
{//{{{
...
unsigned int ECC_ICACHE_E2::GetCapDepth(std::string sub_name)
{//{{{
...
void ECC_ICACHE_E2::AssertERROVFOUT0_IT(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::AssertERROVFOUT1_IT(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::AssertERROVFOUT0_ID(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::AssertERROVFOUT1_ID(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::MonitorpclkMethod()
{//{{{
...
void ECC_ICACHE_E2::Monitorsys_clkMethod()
{//{{{
...
void ECC_ICACHE_E2::MonitorpresetnMethod()
{//{{{
...
void ECC_ICACHE_E2::Monitorsys_resetzMethod()
{//{{{
...
void ECC_ICACHE_E2::HandleResetHardMethod (const unsigned int reset_id)
{//{{{
...
void ECC_ICACHE_E2::HandleResetCmdMethod (const unsigned int reset_id)
{//{{{
...
void ECC_ICACHE_E2::CancelResetCmdMethod (const unsigned int reset_id)
{//{{{
    std::string reset_name = "presetn";
    if (reset_id == empresetnId) {
        reset_name = "presetn";
...
void ECC_ICACHE_E2::EnableReset (const std::string reset_name, const bool is_active)
{//{{{
...
void ECC_ICACHE_E2::CancelEvents()
{//{{{
    for (unsigned int i = 0; i < PE_NUM; i++){
...
bool ECC_ICACHE_E2::GetResetStatus (const std::string reset_name)
{//{{{
...
bool ECC_ICACHE_E2::CheckClockPeriod (const std::string clock_name)
{//{{{
...
void ECC_ICACHE_E2::MonitorkeyopenMethod()
{//{{{
...
void ECC_ICACHE_E2::WriteDisPort_ITMethod(unsigned int pe_id)
{//{{{ 
...
void ECC_ICACHE_E2::WriteERROVFOUT0_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::WriteERROVFOUT1_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::NegateERROVFOUT0_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::NegateERROVFOUT1_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::WriteSEDERROUT_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::AssertSEDERROUT_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::NegateSEDERROUT_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::NegateDEDERROUT_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::WriteDEDERROUT_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::WriteDisPort_IDMethod(unsigned int pe_id)
{//{{{ 
...
void ECC_ICACHE_E2::WriteERROVFOUT0_IDMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::WriteERROVFOUT1_IDMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::NegateERROVFOUT0_IDMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::NegateERROVFOUT1_IDMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::WriteSEDERROUT_IDMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::AssertSEDERROUT_IDMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::NegateSEDERROUT_IDMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::NegateDEDERROUT_IDMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::WriteDEDERROUT_IDMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::NegateAFEERROUT_IDMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::WriteAFEERROUT_IDMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::NegateAFEERROUT_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::WriteAFEERROUT_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::Process1bitError_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::Process1bitError_IDMethod(unsigned int pe_id)
{//{{{
    // Update 1bit error FIFO
...
void ECC_ICACHE_E2::Set1bitErrorStatusThread()
{//{{{
    while (true){
...
bool ECC_ICACHE_E2::CheckAnyFIFOHasElement()
{//{{{
    for (unsigned int i = 0; i < PE_NUM; i++){
...
void ECC_ICACHE_E2::ProcessFatalError_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::ProcessFatalError_IDMethod(unsigned int pe_id)
{//{{{
    bool is_update_status = false;
    double current_time = sc_time_stamp().to_double();
    unsigned int mod_address = 0;
...
void ECC_ICACHE_E2::SetFatalErrorStatus_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::SetFatalErrorStatus_IDMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::NotifyFatalError_ITMethod(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::NotifyFatalError_IDMethod(unsigned int pe_id)
{//{{{
...
bool ECC_ICACHE_E2::CheckNoFatalStatus_IT(unsigned int pe_id)
{//{{{
...
bool ECC_ICACHE_E2::CheckNoFatalStatus_ID(unsigned int pe_id)
{//{{{
...
bool ECC_ICACHE_E2::Check1bitAddressNotCaptured_IT(unsigned int pe_id, unsigned int mod_address)
{//{{{
...
bool ECC_ICACHE_E2::Check1bitAddressNotCaptured_ID(unsigned int pe_id, unsigned int mod_address)
{//{{{
...
void ECC_ICACHE_E2::Update1bitErrorList_IT(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::Update1bitErrorList_ID(unsigned int pe_id)
{//{{{
    bool is_same_port = false;
...
void ECC_ICACHE_E2::UpdateFatalErrorList_IT(unsigned int pe_id)
{//{{{
...
void ECC_ICACHE_E2::UpdateFatalErrorList_ID(unsigned int pe_id)
{//{{{
    bool is_same_port_and_type = false;
    // Update new address if havingpe_id change in slave port
    for (unsigned int i = 0; i < mSizeFatalErrorList_ID[pe_id]; i++){
...
std::string ECC_ICACHE_E2::CombineStringUint(std::string name, unsigned int index)
{//{{{
...
unsigned int ECC_ICACHE_E2::ShiftAddress(std::string sub_name, unsigned int address)
{//{{{
    sc_assert((sub_name == "IT") || (sub_name == "ID"));
...
void ECC_ICACHE_E2::SetMessageLevel (const std::string msg_lv)
{//{{{
...
void ECC_ICACHE_E2::DumpRegisterRW (const std::string is_enable)
{//{{{
    std::vector<std::string> cmd;
    cmd.push_back("reg");
...
void ECC_ICACHE_E2::DumpInterrupt (const std::string is_enable)
{//{{{
    ...
    if (word_vector.size() == 1) {
        if (word_vector[0] == "true") {
            ...
            re_printf("warning","Invalid argument: ECC_ICACHE_E2_DumpInterrupt %s\n", is_enable.c_str());
        }
        ...
        re_printf("info","ECC_ICACHE_E2_DumpInterrupt %s\n",temp.c_str());
    } else {
        re_printf("warning","Invalid argument: ECC_ICACHE_E2_DumpInterrupt %s\n", is_enable.c_str());
    }
}//}}}
...
void ECC_ICACHE_E2::AWIDTH_MSB(const std::string sub_name, const unsigned int value)
{//{{{
    if (sub_name == "IT"){
        if ((mAWIDTH_LSB_IT <= value) && (value <= MAX_ADDWIDTH_MSB)){
            mAWIDTH_MSB_IT = value;
        }else{
            re_printf("warning", "Invalid AWIDTH_MSB (%d) of the ECC_ICACHE_E2_AWIDTH_MSB parameter. Valid range is %d-%d. Value %d is set as default.\n", value, mAWIDTH_LSB_IT, MAX_ADDWIDTH_MSB, DEFAULT_AWIDTH_MSB_IT);
        }
    }else if (sub_name == "ID"){
        ...
        re_printf("warning", "Invalid name of sub-block (%s) of the ECC_ICACHE_E2_AWIDTH_MSB parameter. Valid name of sub-block is IT or ID.\n", sub_name.c_str());
    }
}//}}}

/// Set AWIDTH_LSB

void ECC_ICACHE_E2::AWIDTH_LSB(const std::string sub_name, const unsigned int value)
{//{{{
    if (sub_name == "IT"){
        if ((MIN_ADDWIDTH_LSB <= (int)value) && (value <= mAWIDTH_MSB_IT)){
            mAWIDTH_LSB_IT = value;
        }else{
            re_printf("warning", "Invalid AWIDTH_LSB (%d) of the ECC_ICACHE_E2_AWIDTH_LSB parameter. Valid range is %d-%d. Value %d is set as default.\n", value, MIN_ADDWIDTH_LSB, mAWIDTH_MSB_IT, DEFAULT_AWIDTH_LSB_IT);
        }
    }else if (sub_name == "ID"){
        ...
        re_printf("warning", "Invalid name of sub-block (%s) of the ECC_ICACHE_E2_AWIDTH_LSB parameter. Valid name of sub-block is IT or ID.\n", sub_name.c_str());
    }
}//}}}

/// Set CAP_DEPTH

void ECC_ICACHE_E2::CAP_DEPTH(const std::string sub_name, const unsigned int value)
{//{{{
    if (sub_name == "IT"){
        if ((MIN_CAP_DEPTH <= value) && (value <= MAX_CAP_DEPTH)){
            mCAP_DEPTH_IT = value;
        }else{
            re_printf("warning", "Invalid CAP_DEPTH (%d) of the ECC_ICACHE_E2_CAP_DEPTH parameter. Valid range is %d-%d. Value %d is set as default.\n", value, MIN_CAP_DEPTH, MAX_CAP_DEPTH, DEFAULT_CAP_DEPTH_IT);
        }
    }else if (sub_name == "ID"){
        if ((MIN_CAP_DEPTH <= value) && (value <= MAX_CAP_DEPTH)){
            mCAP_DEPTH_ID = value;
        ...
        re_printf("warning", "Invalid name of sub-block (%s) of the ECC_ICACHE_E2_CAP_DEPTH parameter. Valid name of sub-block is IT or ID.\n", sub_name.c_str());
    }
}//}}}

/// Set ERRINT_INIT

void ECC_ICACHE_E2::ERRINT_INIT(const std::string sub_name, const unsigned int value)
{//{{{
    if (sub_name == "IT"){
        if ((MIN_ERRINT_INIT <= (int)value) && (value <= MAX_ERRINT_INIT)){
            mERRINT_INIT_IT = value;
        }else{
            re_printf("warning", "Invalid ERRINT_INIT (%d) of the ECC_ICACHE_E2_ERRINT_INIT parameter. Valid range is %d-%d. Value %d is set as default.\n", value, MIN_ERRINT_INIT, MAX_ERRINT_INIT, DEFAULT_ERRINT_INIT_IT);
        }
    }else if (sub_name == "ID"){
        if ((MIN_ERRINT_INIT <= (int)value) && (value <= MAX_ERRINT_INIT)){
        ...
        re_printf("warning", "Invalid name of sub-block (%s) of the ECC_ICACHE_E2_ERRINT_INIT parameter. Valid name of sub-block is IT or ID.\n", sub_name.c_str());
    }
}//}}}

/// Calculate EDL code

std::string ECC_ICACHE_E2::CalculateEDLCode()
{//{{{
...
void ECC_ICACHE_E2::IssueError(const unsigned int errortype)
{//{{{
    ...
    if ((mIsEDLConfig) && (mIsAddrConfig)){
        if (((errortype == em1bitError) || (errortype == em2bitError) || (errortype == emAddressFeedbackError)) && ((mCurConfigSubName == "IT") || (mCurConfigSubName == "ID"))){
            ...
            re_printf("warning", "Invalid error type (%d) of ECC_ICACHE_E2_IssueError command.\n", errortype);
        }
    }else{
...
void ECC_ICACHE_E2::SetErrorAddr(const unsigned int addr_value)
{//{{{
...
void ECC_ICACHE_E2::SetEDL (const std::string edl, const unsigned int pe_index)
{//{{{
    ...
    if (!mIsEDLConfig){
        re_printf("warning", "Invalid EDL (%s, %d) of ECC_ICACHE_E2_SetEDL command.\n", edl.c_str(), pe_index);
    }
}//}}}

/// Dump Status info

void ECC_ICACHE_E2::DumpStatusInfo()
{//{{{
    // For IT - PEn
    for (unsigned int i = 0; i < PE_NUM; i++){
        const char *type = CombineStringUint("StatInfo IT - PE", i).c_str();
        DumpInfo(type, "Info [%20s] (%10s):\n", sc_time_stamp().to_string().c_str(), this->name());
        DumpInfo(type, "  IT_SERSTR: 0x%08X\n", mpMECCCAP_ITFunc[i]->GetIT_SERSTR());
        DumpInfo(type, "  IT_DERSTR: 0x%08X\n", mpMECCCAP_ITFunc[i]->GetIT_DERSTR());
        DumpInfo(type, "  IT_OVFSTR: 0x%08X\n", mpMECCCAP_ITFunc[i]->GetIT_OVFSTR());
        for (unsigned int k = 0; k < mCAP_DEPTH_IT; k++){
            ...
            DumpInfo(type, "  IT_%2dSEADR: SEDL = 0x%02X, SEADR = 0x%08X\n", k, sedl, seadr);
        }
        ...
        DumpInfo(type, "  IT_00DEADR: DEDL = 0x%02X, DEADR = 0x%08X\n", dedl, deadr);
    }

    // For ID - PEn
    for (unsigned int i = 0; i < PE_NUM; i++){
        const char *type = CombineStringUint("StatInfo ID - PE", i).c_str();
        DumpInfo(type, "Info [%20s] (%10s):\n", sc_time_stamp().to_string().c_str(), this->name());
        DumpInfo(type, "  ID_SERSTR: 0x%08X\n", mpMECCCAP_IDFunc[i]->GetID_SERSTR());
        DumpInfo(type, "  ID_DERSTR: 0x%08X\n", mpMECCCAP_IDFunc[i]->GetID_DERSTR());
        DumpInfo(type, "  ID_OVFSTR: 0x%08X\n", mpMECCCAP_IDFunc[i]->GetID_OVFSTR());
        for (unsigned int k = 0; k < mCAP_DEPTH_ID; k++){
            ...
            DumpInfo(type, "  ID_%2dSEADR: SEDL = 0x%02X, SEADR = 0x%08X\n", k, sedl, seadr);
        }
        ...
        DumpInfo(type, "  ID_00DEADR: DEDL = 0x%02X, DEADR = 0x%08X\n", dedl, deadr);
    }
}//}}}

/// Common dump info message

void ECC_ICACHE_E2::DumpInfo(const char *type, const char *message, ...)
{//{{{
    if ((message != NULL) && (type != NULL)){
        printf("PROFILE(%s): ECC_ICACHE_E2: ", type);
        // Print body
        va_list argptr;
...
void ECC_ICACHE_E2::AssertReset (const std::string reset_name, const double start_time, const double period)
{//{{{
    if (reset_name == "presetn") {
...
void ECC_ICACHE_E2::SetCLKFreq (const std::string clock_name, const sc_dt::uint64 freq, const std::string unit)
{//{{{
    std::vector<std::string> word_vector;
...
void ECC_ICACHE_E2::GetCLKFreq (const std::string clock_name)
{//{{{
    std::vector<std::string> word_vector;
    SeparateString(word_vector, clock_name);
    if (word_vector.size() == 1) {
...
void ECC_ICACHE_E2::ForceRegister (const std::string block_name, const unsigned int block_index, const std::string reg_name, const unsigned int reg_value)
{//{{{ 
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ECC_ICACHE_E2::ReleaseRegister (const std::string block_name, const unsigned int block_index, const std::string reg_name)
{//{{{
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ECC_ICACHE_E2::WriteRegister (const std::string block_name, const unsigned int block_index, const std::string reg_name, const unsigned int reg_value)
{//{{{
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ECC_ICACHE_E2::ReadRegister (const std::string block_name, const unsigned int block_index, const std::string reg_name)
{//{{{
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ECC_ICACHE_E2::ListRegister (void)
{//{{{
    std::vector<std::string> cmd;
    cmd.push_back("reg");
    for (unsigned int i = 0; i < PE_NUM; i++){
...
void ECC_ICACHE_E2::Help (const std::string type)
{//{{{
    std::vector<std::string> word_vector;
    SeparateString(word_vector, type);
    if (word_vector.size() == 1) {
        if (word_vector[0] == "parameters") {
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_MessageLevel (\"ECC_ICACHE_E2 instance\", \"fatal|error|warning|info\")                          Set debug message level (Default: fatal|error)");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_DumpRegisterRW (\"ECC_ICACHE_E2 instance\", \"true/false\")                                      Enable/disable dumping access register (Default: false)");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_DumpInterrupt (\"ECC_ICACHE_E2 instance\", \"true/false\")                                       Enable/disable dumping interrupt info (Default: false)");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_AWIDTH_MSB (\"ECC_ICACHE_E2 instance\", \"sub_name\", value)                                     Set address bit width MSB for IT or ID (Default: 11 for IT, ID). Valid range to be set: 7-26");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_AWIDTH_LSB (\"ECC_ICACHE_E2 instance\", \"sub_name\", value)                                     Set address bit width LSB for IT or ID (Default: 5 for IT, 3 for ID). Valid range to be set: 2-4");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_CAP_DEPTH (\"ECC_ICACHE_E2 instance\", \"sub_name\", value)                                      Set number of address registers for IT or ID (Default: 1 for both IT, ID). Valid range to be set: 1-32");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_ERRINT_INIT (\"ECC_ICACHE_E2 instance\", \"sub_name\", value)                                    Set initial value of ERRINT register for IT or ID (Default: 0x0 for IT, ID). Valid range to be set: 0x0-0xFF");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_EnableRegisterMessage (\"ECC_ICACHE_E2 instance\", \"true/false\")             Enable/disable info/warning/error message of register IF (Default: true)");
        } else if (word_vector[0] == "commands") {
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_IssueError (\"ECC_ICACHE_E2 instance\", error_type)                                              Issue pseudo error to the ECC_ICACHE_E2 model");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_SetErrorAddr (\"ECC_ICACHE_E2 instance\", addr_value)                                            Set error detection address");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_SetEDL (\"ECC_ICACHE_E2 instance\", \"edl\", pe_index)                                           Set error detection location");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_DumpStatusInfo (\"ECC_ICACHE_E2 instance\")                                                      Dump information of the error status registers of the ECC_ICACHE_E2 model");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_AssertReset (\"ECC_ICACHE_E2 instance\", \"reset_name\", start_time, period)                     Assert and de-assert reset signal to the ECC_ICACHE_E2 model");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_SetCLKFreq (\"ECC_ICACHE_E2 instance\", \"clock_name\", freq, \"unit\")                          Set clock frequency value to ECC_ICACHE_E2 model. Valid clock_name is pclk or sys_clk");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_GetCLKFreq (\"ECC_ICACHE_E2 instance\", \"clock_name\")                                          Get clock frequency value of ECC_ICACHE_E2 model. Valid clock_name is pclk or sys_clk");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_ForceRegister (\"ECC_ICACHE_E2 instance\", \"block_name\", block_index, \"reg_name\", reg_value) Force register with setting value");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_ReleaseRegister (\"ECC_ICACHE_E2 instance\", \"block_name\", block_index, \"reg_name\")          Release register from force value");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_WriteRegister (\"ECC_ICACHE_E2 instance\", \"block_name\", block_index, \"reg_name\", reg_value) Write a value to a register");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_ReadRegister (\"ECC_ICACHE_E2 instance\", \"block_name\", block_index, \"reg_name\")             Read a value from a register");
            SC_REPORT_INFO(this->basename(),"ECC_ICACHE_E2_ListRegister (\"ECC_ICACHE_E2 instance\")                                                        Dump name of all registers");
        } else {
...
void ECC_ICACHE_E2::EnableRegisterMessage(const std::string is_enable)
{//{{{
    std::vector<std::string> word_vector;
    SeparateString(word_vector, is_enable);
    if (word_vector.size() == 1) {
        if (word_vector[0] == "true") {
            ...
            re_printf("warning","Invalid argument: ECC_ICACHE_E2_EnableRegisterMessage %s.\n", is_enable.c_str());
        }
    } else if (word_vector.size() == 0) {
        std::string temp = "false";
        ...
        re_printf("info","ECC_ICACHE_E2_EnableRegisterMessage %s\n", temp.c_str());
    } else {
        re_printf("warning","Invalid argument: ECC_ICACHE_E2_EnableRegisterMessage %s.\n", is_enable.c_str());
    }
}//}}}
