// -----------------------------------------------------------------------------
// $Id: mspi000_int_factor_regif.cpp,v 1.4 2020/03/11 00:52:43 ducla Exp $
//
...
/// @return none
Cint_factor_regif::Cint_factor_regif(std::string name, uint buswidth, uint ch_num)
    :vpcl::reg_super()
    #ifdef USR_CWR_SYSTEMC
    , cwmem("register", scml_memsize(0x9C/4))
    #endif
{
    mCh_num = ch_num;
    CommandInit();
    std::ostringstream str_tmp;
    for(uint i = 0; i < emNUM_MSPInINTFk; i++) {
        str_tmp.str("");
        str_tmp<<"MSPInINTFk"<< i;
        MSPInINTFk  [i] = new vpcl::re_register((0x0000) + 48*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_MSPInINTMSKk; i++) {
        str_tmp.str("");
        str_tmp<<"MSPInINTMSKk"<< i;
        MSPInINTMSKk[i] = new vpcl::re_register((0x0004) + 48*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_MSPInINTFCk; i++) {
        str_tmp.str("");
        str_tmp<<"MSPInINTFCk"<< i;
        MSPInINTFCk [i] = new vpcl::re_register((0x0008) + 48*i, this, str_tmp.str(), name.c_str());
    }
    ...
    for(uint i = 0; i < emNUM_MSPInINTFk;  i++) {
        mRegMap[(0x0000)     + i*48] = index;
        mRegMap[(0x0000 + 1) + i*48] = index;
        mRegMap[(0x0000 + 2) + i*48] = index;
        mRegMap[(0x0000 + 3) + i*48] = index++;
    }
    for(uint i = 0; i < emNUM_MSPInINTMSKk; i++) {
        mRegMap[(0x0004)     + i*48] = index;
        mRegMap[(0x0004 + 1) + i*48] = index;
        mRegMap[(0x0004 + 2) + i*48] = index;
        mRegMap[(0x0004 + 3) + i*48] = index++;
    }
    for(uint i = 0; i < emNUM_MSPInINTFCk; i++) {
        mRegMap[(0x0008)     + i*48] = index;
        mRegMap[(0x0008 + 1) + i*48] = index;
        mRegMap[(0x0008 + 2) + i*48] = index;
        mRegMap[(0x0008 + 3) + i*48] = index++;
    }
    mTotalRegNum = index;
    mRegArray = new SRegList* [mTotalRegNum];
    index = 0;
    #ifdef USR_CWR_SYSTEMC
    for(uint i = 0; i < emNUM_MSPInINTFk  ; i++) cw_instantiate_reg(CW_MSPInINTFk    [i], MSPInINTFk  [i]->name(), MSPInINTFk  [i]->addr());
    for(uint i = 0; i < emNUM_MSPInINTMSKk; i++) cw_instantiate_reg(CW_MSPInINTMSKk  [i], MSPInINTMSKk[i]->name(), MSPInINTMSKk[i]->addr());
    for(uint i = 0; i < emNUM_MSPInINTFCk ; i++) cw_instantiate_reg(CW_MSPInINTFCk   [i], MSPInINTFCk [i]->name(), MSPInINTFCk [i]->addr());
    #endif
    for(uint i = 0; i < emNUM_MSPInINTFk  ; i++) mCurReg = mRegArray[index++] = new SRegList(MSPInINTFk    [i], mCurReg, i,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_MSPInINTMSKk; i++) mCurReg = mRegArray[index++] = new SRegList(MSPInINTMSKk  [i], mCurReg, i,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_MSPInINTFCk ; i++) mCurReg = mRegArray[index++] = new SRegList(MSPInINTFCk   [i], mCurReg, i,  4,      "32", "8|16|32");

    mRegList = mCurReg;

    for(uint i = 0; i < emNUM_MSPInINTFCk; i++) {
        (*MSPInINTFCk   [i]) ((mCh_num-1),  0, "MSPInINTFCk", 0x0       , "R|W", vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_MSPInINTMSKk; i++) {
        (*MSPInINTMSKk  [i]) ((mCh_num - 1),  0, "MSPInINTMSKk", ((1<< mCh_num)-1), "R|W"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_MSPInINTFk; i++) {
        (*MSPInINTFk    [i]) ((mCh_num - 1),  0, "MSPInINTFk", 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    ...
    for(uint i = 0; i < emNUM_MSPInINTMSKk; i++) {
        str_tmp.str("");
        str_tmp<<"MSPInINTMSKk"<< i;
        mWrCbAPI[str_tmp.str()]["MSPInINTMSKk"] = &Cint_factor_regif::cb_MSPInINTMSKk_MSPInINTMSKk;
    }
    for(uint i = 0; i < emNUM_MSPInINTFCk; i++) {
        str_tmp.str("");
        str_tmp<<"MSPInINTFCk"<< i;
        mWrCbAPI[str_tmp.str()]["MSPInINTFCk"] = &Cint_factor_regif::cb_MSPInINTFCk_MSPInINTFCk;
    }
    InitLocalVal();
}
...
/// @return none
Cint_factor_regif::~Cint_factor_regif()
{
...
Cint_factor_regif::uint Cint_factor_regif::bit_select(
...
bool Cint_factor_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cint_factor_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cint_factor_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                   const unsigned char *p_data,   ///< [in] Writing data
                                   cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cint_factor_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                   unsigned char *p_data,   ///< [out] Reading data
                                   cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cint_factor_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                       const unsigned char *p_data,  ///< [in] Writing data
                                       cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cint_factor_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                       unsigned char *p_data,   ///< [out] Reading data
                                       cuint size,              ///< [in]  Data size (byte)
                                       bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cint_factor_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                    unsigned char *p_data,          ///< [out] Reading data
                                    cuint size,                     ///< [in]  Data size (byte)
...
bool Cint_factor_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                    const unsigned char *p_data,      ///< [in] Writing data
                                    cuint size,                       ///< [in] Data size (byte)
                                    cuint reg_index,                  ///< [in] Register index
...
bool Cint_factor_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cint_factor_regif::CommandInit()
{
...
/// @return string
std::string Cint_factor_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cint_factor_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cint_factor_regif::InitLocalVal()
{
    // Initialize local variables
    for(uint i = 0; i < emNUM_MSPInINTFk;  i++) {
        MSPInINTFk_MSPInINTFk[i] = 0x0;
    }
    for(uint i = 0; i < emNUM_MSPInINTMSKk; i++) {
        MSPInINTMSKk_MSPInINTMSKk[i] = 0xFF;
    }
    for(uint i = 0; i < emNUM_MSPInINTFCk; i++) {
        MSPInINTFCk_MSPInINTFCk[i] = 0x0;
    }
}

/// Update bit value to local value
/// @return None
void Cint_factor_regif::UpdateLocalVal(cuint addr)
{
    if ((0x0000 <= addr) && (addr <= (0x0000)+(48*3)) && ((addr-0x0000)%48 == 0)) {
        uint i = (addr - 0x0000)/48;
        MSPInINTFk_MSPInINTFk           [i] = (*(MSPInINTFk  [i]))["MSPInINTFk"  ];
        return;
    }
    if ((0x0004 <= addr) && (addr <= (0x0004)+(48*3)) && ((addr-0x0004)%48 == 0)) {
        uint i = (addr - 0x0004)/48;
        MSPInINTMSKk_MSPInINTMSKk       [i] = (*(MSPInINTMSKk[i]))["MSPInINTMSKk"];
        return;
    }
    if ((0x0008 <= addr) && (addr <= (0x0008)+(48*3)) && ((addr-0x0008)%48 == 0)) {
        uint i = (addr - 0x0008)/48;
        MSPInINTFCk_MSPInINTFCk         [i] = (*(MSPInINTFCk [i]))["MSPInINTFCk" ];
        return;
    }
}
...
/// @return None
void Cint_factor_regif::UpdateRegVal(cuint addr)
{
    if ((0x0000 <= addr) && (addr <= (0x0000)+(48*3)) && ((addr-0x0000)%48 == 0)) {
        uint i = (addr - 0x0000)/48;
        (*(MSPInINTFk  [i]))["MSPInINTFk"  ] = MSPInINTFk_MSPInINTFk[i];
        return;
    }
    if ((0x0004 <= addr) && (addr <= (0x0004)+(48*3)) && ((addr-0x0004)%48 == 0)) {
        uint i = (addr - 0x0004)/48;
        (*(MSPInINTMSKk[i]))["MSPInINTMSKk"] = MSPInINTMSKk_MSPInINTMSKk[i];
        return;
    }
    if ((0x0008 <= addr) && (addr <= (0x0008)+(48*3)) && ((addr-0x0008)%48 == 0)) {
        uint i = (addr - 0x0008)/48;
        (*(MSPInINTFCk [i]))["MSPInINTFCk" ] = MSPInINTFCk_MSPInINTFCk[i];
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cint_factor_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cint_factor_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cint_factor_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cint_factor_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cint_factor_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cint_factor_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cint_factor_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Cint_factor_regif::uint Cint_factor_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return none
void Cint_factor_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cint_factor_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cint_factor_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cint_factor_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Cint_factor_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cint_factor_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cint_factor_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
