// -----------------------------------------------------------------------------
// $Id: cadc_regif.cpp,v 1.1 2018/11/19 02:50:58 chanle Exp $
//
...
/// @return none
Ccadc_regif::Ccadc_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    CADCADGCR = new vpcl::re_register(0x0004, this, "CADCADGCR", name.c_str());
    CADCTDCR  = new vpcl::re_register(0x000C, this, "CADCTDCR" , name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_CADCADGCR  , CADCADGCR->name(), CADCADGCR->addr());
    cw_instantiate_reg(CW_CADCTDCR   , CADCTDCR->name() , CADCTDCR->addr() );
    #endif
    mCurReg = mRegArray[index++] = new SRegList(CADCADGCR  , mCurReg, 0,  1,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(CADCTDCR   , mCurReg, 0,  1,       "8", "8|16|32");

    mRegList = mCurReg;

    (*CADCADGCR  ) ( 5,  5, "CADCODDE", 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*CADCADGCR  ) ( 4,  4, "CADCODE" , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*CADCADGCR  ) ( 0,  0, "CADCUNSND", 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*CADCTDCR   ) ( 0,  0, "CADCTDE" , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["CADCTDCR"]["CADCTDE"] = &Ccadc_regif::cb_CADCTDCR_CADCTDE;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Ccadc_regif::~Ccadc_regif()
{
...
Ccadc_regif::uint Ccadc_regif::bit_select(
...
bool Ccadc_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Ccadc_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Ccadc_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                             const unsigned char *p_data,   ///< [in] Writing data
                             cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Ccadc_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                             unsigned char *p_data,   ///< [out] Reading data
                             cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Ccadc_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                 const unsigned char *p_data,  ///< [in] Writing data
                                 cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Ccadc_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                 unsigned char *p_data,   ///< [out] Reading data
                                 cuint size,              ///< [in]  Data size (byte)
                                 bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Ccadc_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                              unsigned char *p_data,          ///< [out] Reading data
                              cuint size,                     ///< [in]  Data size (byte)
...
bool Ccadc_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                              const unsigned char *p_data,      ///< [in] Writing data
                              cuint size,                       ///< [in] Data size (byte)
                              cuint reg_index,                  ///< [in] Register index
...
bool Ccadc_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return string
std::string Ccadc_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Ccadc_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Ccadc_regif::InitLocalVal()
{
    // Initialize local variables
    CADCADGCR_CADCODDE = 0x0;
    CADCADGCR_CADCODE = 0x0;
    CADCADGCR_CADCUNSND = 0x0;
    CADCTDCR_CADCTDE = 0x0;
}

/// Update bit value to local value
/// @return None
void Ccadc_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0004) {
        CADCADGCR_CADCODDE            = (*CADCADGCR)["CADCODDE" ];
        CADCADGCR_CADCODE             = (*CADCADGCR)["CADCODE"  ];
        CADCADGCR_CADCUNSND           = (*CADCADGCR)["CADCUNSND"];
        return;
    }
    if (addr == 0x000C) {
        CADCTDCR_CADCTDE              = (*CADCTDCR )["CADCTDE"  ];
        return;
    }
}
...
/// @return None
void Ccadc_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0004) {
        (*CADCADGCR)["CADCODDE" ] = CADCADGCR_CADCODDE;
        (*CADCADGCR)["CADCODE"  ] = CADCADGCR_CADCODE;
        (*CADCADGCR)["CADCUNSND"] = CADCADGCR_CADCUNSND;
        return;
    }
    if (addr == 0x000C) {
        (*CADCTDCR )["CADCTDE"  ] = CADCTDCR_CADCTDE;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Ccadc_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Ccadc_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Ccadc_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Ccadc_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Ccadc_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Ccadc_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Ccadc_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Ccadc_regif::uint Ccadc_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Ccadc_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Ccadc_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Ccadc_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Ccadc_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
unsigned char Ccadc_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
unsigned char Ccadc_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(unsigned char));
    return sizeof(unsigned char);
}
void Ccadc_regif::cw_instantiate_reg(scml2::reg<unsigned char> * reg, std::string reg_name, int offset)
{
