// -----------------------------------------------------------------------------
// $Id: ecc_bus_apec_regif.cpp,v 1.7 2020/04/01 06:41:19 thunguyen3 Exp $
//
...
/// @return none
Cecc_bus_apec_regif::Cecc_bus_apec_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
...
{
    CommandInit();
    KCPROT = new vpcl::re_register(0x0000, this, "KCPROT", name.c_str());
    ...
    mWrCbAPI["KCPROT"]["KCE" ] = &Cecc_bus_apec_regif::cb_KCPROT_KCE;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cecc_bus_apec_regif::~Cecc_bus_apec_regif()
{
...
Cecc_bus_apec_regif::uint Cecc_bus_apec_regif::bit_select(
...
bool Cecc_bus_apec_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cecc_bus_apec_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecc_bus_apec_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                     const unsigned char *p_data,   ///< [in] Writing data
                                     cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecc_bus_apec_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                     unsigned char *p_data,   ///< [out] Reading data
                                     cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecc_bus_apec_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                         const unsigned char *p_data,  ///< [in] Writing data
                                         cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecc_bus_apec_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                         unsigned char *p_data,   ///< [out] Reading data
                                         cuint size,              ///< [in]  Data size (byte)
                                         bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cecc_bus_apec_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                      unsigned char *p_data,          ///< [out] Reading data
                                      cuint size,                     ///< [in]  Data size (byte)
...
bool Cecc_bus_apec_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                      const unsigned char *p_data,      ///< [in] Writing data
                                      cuint size,                       ///< [in] Data size (byte)
                                      cuint reg_index,                  ///< [in] Register index
...
bool Cecc_bus_apec_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cecc_bus_apec_regif::CommandInit()
{
...
/// @return string
std::string Cecc_bus_apec_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cecc_bus_apec_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cecc_bus_apec_regif::InitLocalVal()
{
    // Initialize local variables
    KCPROT_KCPROT = 0x0;
    KCPROT_KCE = 0x0;
...
/// @return None
void Cecc_bus_apec_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
...
/// @return None
void Cecc_bus_apec_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
...
/// @return vector
std::vector<std::string> Cecc_bus_apec_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cecc_bus_apec_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cecc_bus_apec_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cecc_bus_apec_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cecc_bus_apec_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cecc_bus_apec_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cecc_bus_apec_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Cecc_bus_apec_regif::uint Cecc_bus_apec_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return none
void Cecc_bus_apec_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cecc_bus_apec_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cecc_bus_apec_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cecc_bus_apec_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Cecc_bus_apec_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cecc_bus_apec_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cecc_bus_apec_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
