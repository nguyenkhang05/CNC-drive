...
//   %CMD_RW EnableTransInfo <enable:bool=false> @ Enable/disable information display when SCI3 is turned on/off
//   %CMD_RW TransTimeCalcMode <enable:bool=true> @ Transfer time calculation mode
//   %CMD_RW TransferLatency <latency:double=0> @ Transfer latency [ns]
//   %CMD_ACTION DumpStatInfo : void @ Dumps statistical information of SCI3
...
#endif // EnableDumpResult
    ret += "    DumpInterrupt <enable>                    Enable/disable interrupt information display when an interrupt is sent ( Default:false  )\n";
    ret += "    EnableTransInfo <enable>                  Enable/disable information display when SCI3 is turned on/off ( Default:false  )\n";
    ret += "    TransTimeCalcMode <enable>                Transfer time calculation mode ( Default:true  )\n";
    ret += "    TransferLatency <latency>                 Transfer latency [ns] ( Default:0  )\n";
    ret += "    DumpStatInfo                              Dumps statistical information of SCI3\n";
    ret += "    SetCLKfreq <clk_name> <clk_freq>          Set a new clock frequency [Hz]\n";
...
std::string user_def_command(std::vector<std::string>& args)
{
  ...
  else if (args[0] == "EnableTransInfo") {
    // read mode
    if (0) {;}
    else if ((int)args.size() == 1) {
      if (EnableTransInfo) {
  ...
  else if (args[0] == "TransTimeCalcMode") {
    // read mode
    if (0) {;}
    else if ((int)args.size() == 1) {
  ...
  else if (args[0] == "TransferLatency") {
    // read mode
    if (0) {;}
    else if ((int)args.size() == 1) {
      std::ostringstream stream;
      stream << TransferLatency;
      ret = stream.str();
    }
    // write mode
    else if ((int)args.size() == 2) {
      {
        ...
        double backup = TransferLatency;
        is >> TransferLatency;
        
        if (is.fail() || !is.eof()) {
          TransferLatency = backup;
...
bool DumpInterrupt;
bool EnableTransInfo;
bool TransTimeCalcMode;
double TransferLatency;
