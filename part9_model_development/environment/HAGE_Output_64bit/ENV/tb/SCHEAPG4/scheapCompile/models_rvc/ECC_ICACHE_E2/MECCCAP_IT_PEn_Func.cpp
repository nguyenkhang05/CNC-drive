// ---------------------------------------------------------------------
// $Id: MECCCAP_IT_PEn_Func.cpp,v 1.1 2019/09/26 10:07:26 landang Exp $
//
...
//========================================================
/// Constructor
MECCCAP_IT_PEn_Func::MECCCAP_IT_PEn_Func (std::string name, unsigned int pe_id, ECC_ICACHE_E2_AgentController* ICACHEAgentController):
                Cmecccap_it_pen_regif(name, 32)
                ,BusSlaveFuncIf()
{//{{{
    ...
    Cmecccap_it_pen_regif::set_instance_name(name);
}//}}}

/// Destructor
MECCCAP_IT_PEn_Func::~MECCCAP_IT_PEn_Func()
{//{{{
}//}}}

/// Reset Func and registers
void MECCCAP_IT_PEn_Func::EnableReset(const std::string reset_name, const bool is_active)
{//{{{
    if (reset_name == "sys_resetz"){
        Cmecccap_it_pen_regif::EnableReset(is_active);
    }
}//}}}

/// Read API (target socket)
void MECCCAP_IT_PEn_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void MECCCAP_IT_PEn_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void MECCCAP_IT_PEn_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cmecccap_it_pen_regif::reg_handle_command(cmd);
                if (msg_return != "") {
            ...
            msg_return = Cmecccap_it_pen_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
bool MECCCAP_IT_PEn_Func::Set1bitStatusAndAddress(unsigned int edl_code, unsigned int mod_address)
{//{{{
    unsigned int cap_depth = mpICACHEAgentController->GetCapDepth("IT");
...
void MECCCAP_IT_PEn_Func::SetFlagAddressBufferOverflow()
{//{{{
...
bool MECCCAP_IT_PEn_Func::CheckHasRoomFor1bit()
{//{{{
    unsigned int cap_depth = mpICACHEAgentController->GetCapDepth("IT");
    unsigned int status = (unsigned int)(*IT_SERSTR);
    bool is_find_room = false;
...
/// Set FIFO overflow
void MECCCAP_IT_PEn_Func::SetFIFOOverflow ()
{//{{{
...
/// Get SEDL bit
unsigned int MECCCAP_IT_PEn_Func::GetSEDLBit(unsigned int index)
{//{{{
    return (unsigned int)(*IT_nSEADR[index])["SEDL"];
}//}}}

/// Get SEADR bit
unsigned int MECCCAP_IT_PEn_Func::GetSEADRBit(unsigned int index)
{//{{{
    return (unsigned int)(*IT_nSEADR[index])["SEADR0"];
}//}}}

/// Get DEDL bit
unsigned int MECCCAP_IT_PEn_Func::GetDEDLBit()
{//{{{
    return (unsigned int)(*IT_00DEADR)["DEDL"];
}//}}}

/// Get DEADR bit
unsigned int MECCCAP_IT_PEn_Func::GetDEADRBit()
{//{{{
    return (unsigned int)(*IT_00DEADR)["DEADR0"];
}//}}}

/// Check no fatal status
bool MECCCAP_IT_PEn_Func::CheckNoFatalStatus()
{//{{{
    return ((unsigned int)(*IT_DERSTR) == 0);
}//}}}

/// Set 2bit error status
void MECCCAP_IT_PEn_Func::Set2bitErrorStatus()
{//{{{
    (*IT_DERSTR)["DEDF00"] = 1;
}//}}}

/// Set address feedback error status
void MECCCAP_IT_PEn_Func::SetAddrFeedbackErrorStatus()
{//{{{
    (*IT_DERSTR)["AFEF00"] = 1;
}//}}}

/// Set EDL and error address of fatal
void MECCCAP_IT_PEn_Func::SetFatalAddress(unsigned int edl_code, unsigned int mod_address)
{//{{{
...
unsigned int MECCCAP_IT_PEn_Func::GetIT_SERSTR()
{//{{{
    return (unsigned int)(*IT_SERSTR);
}//}}}

/// Get value of IT_DERSTR register
unsigned int MECCCAP_IT_PEn_Func::GetIT_DERSTR()
{//{{{
    return (unsigned int)(*IT_DERSTR);
}//}}}

/// Get value of IT_OVFSTR register
unsigned int MECCCAP_IT_PEn_Func::GetIT_OVFSTR()
{//{{{
    return (unsigned int)(*IT_OVFSTR);
}//}}}

/// Write ERRINT_INIT to ERRINT reg after reset
void MECCCAP_IT_PEn_Func::WriteERRINT_INIT(unsigned int value)
{//{{{
...
/// Get ERRINT register
unsigned int MECCCAP_IT_PEn_Func::GetERRINT()
{//{{{
...
void MECCCAP_IT_PEn_Func::cb_IT_SSTCLR_IT_SSTCLR(RegCBstr str)
{//{{{ 
...
void MECCCAP_IT_PEn_Func::cb_IT_DSTCLR_DSTCLR00(RegCBstr str)
{//{{{
    // clear DED, AFE.
...
void MECCCAP_IT_PEn_Func::cb_IT_OVFCLR_SERROVFCLR0(RegCBstr str)
{//{{{ 
