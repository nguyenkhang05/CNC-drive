// ---------------------------------------------------------------------
// $Id: APBFSGuard.cpp,v 1.12 2014/09/25 08:51:11 dungvannguyen Exp $
//
...
///Destructor of APBFSGuard class
APBFSGuard::~APBFSGuard()
{//{{{
}//}}}

///Reset all registers
void APBFSGuard::Reset(const bool is_active)
{//{{{
    Capbfsguard_regif::EnableReset(is_active);
}//}}}

///Check access ability to peripherals
bool APBFSGuard::CheckAccessibility(TlmBasicPayload &trans, const bool debug, bool &is_legal_trans)
{//{{{
    bool status = false;
    bool peid_flt = false;
    bool vcid_flt = false;
    ...
    if (flt_id < 0x10) {
        ...
        if (mVersion == emCC_CUBE) {
            if (((trans_property == "R") && ((*PROT[flt_id])["RDPDEF"] == 0)) || /* default read access */
               ((trans_property == "W") && ((*PROT[flt_id])["WRPDEF"] == 0)) || /* default trans_property access */
            ...
            if (((trans_property == "R") && ((*PROT[flt_id])["RDPDEF"] == 1)) || /* default read access */
               ((trans_property == "W") && ((*PROT[flt_id])["WRPDEF"] == 1)) || /* default trans_property access */
        ...
        mAgentController->print_msg("error","Address 0x%X is out of controlled-range of APG.\n", addr);
...
///Set the information (id, address area) to filters
void APBFSGuard::SetFilterInfo(const bool is_set,
...
void APBFSGuard::DumpStatusInfo(void)
{//{{{
    mAgentController->print_msg("info","APBFSGuard status information\n");
    for (unsigned int index = 0; index < mFilterNo; index++) {
...
///Set parameters and command to apbfsguard_regif class
std::string APBFSGuard::RegisterHandler(const std::vector<std::string> &args)
{//{{{
    std::string ret = "";
    if ((args.size() == 1) && (args[0] == "reg")) { // list registers
        printf("List registers of APBFSGUARD\n");
...
void APBFSGuard::read( unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
///Virtual function of BusSlaveFuncIf class
void APBFSGuard::write( unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    if (debug == false) {
...
///Virtual function of apbfsguard_regif class
void APBFSGuard::cb_PROT_LOCK(RegCBstr str)
{//{{{
...
void APBFSGuard::SetRegInitValue(const unsigned int filter_id, const std::string reg_name, const unsigned int init_val)
{//{{{
