...
std::string own_handle_command(std::vector<std::string>& args)
{
  ...
  if (args[0] == "help") {
    ...
    ret += "    RamReadData <arg>                         Read data from RAM to set the E710TED register ( Default:0  )\n";
    ret += "    RamReadEccPart <arg>                      Read data of ECC part from RAM to set the E710TRC.ECHORD register ( Default:0  )\n";
    ret += "    RamWriteEccPart <arg>                     Write data of ECC part from RAM to set the E710TRC.ECERDB register ( Default:0  )\n";
    ret += "    EncodeResult <arg>                        Encode result to set the E710TRC.ECECRD register ( Default:0  )\n";
    ret += "    DecodeResult <arg>                        Decode result to set the E710TRC.ECSYND register ( Default:0  )\n";
    ret += "    EccErrorAddress <arg>                     Read address detecting ECC error to set the E70EAD register ( Default:0  )\n";
    ret += "    NumOfErrorStage <arg>                     Number of stages of ECC error address register (set value from 0 to 8) ( Default:1  )\n";
    ret += "    AccEnErrJudg <arg>                        Access enable signal of error judgment enable bit ( Default:false  )\n";
    ret += "    TwoBitsErrJudg <arg>                      2bits error judgment enable signal ( Default:true  )\n";
    ret += "    SetCLKfreq <clk_name> <frequency>         Setup the new clock frequency\n";
    ret += "    EccError <arg>                            Set the status of ECC change to EccError and assert corresponding interrupt (set value 1/2)\n";
...
std::string user_def_command(std::vector<std::string>& args)
{
  ...
  if (0) {}
    ...
    // write mode
      ...
      if (args[1].substr(0, 1) == "-") {
        ...
        unsigned int backup = RamReadData;
        if (args[1].substr(0, 2) == "0x") {
            is >> std::hex >> RamReadData;
        }
        else if (args[1].substr(0, 1) == "0") {
            is >> std::oct >> RamReadData;
        }
        else {
            is >> std::dec >> RamReadData;
        }
        if (is.fail() || !is.eof()) {
          RamReadData = backup;
          ret += err_msg + "wrong argument: " + (std::string)args[1] + specified_cmd;
  ...
  else if (args[0] == "RamReadEccPart") {
    // read mode
    if (0) {;}
    else if ((int)args.size() == 1) {
      std::ostringstream stream;
      stream << (unsigned int)RamReadEccPart;
      ret = stream.str();
    }
    // write mode
    else if ((int)args.size() == 2) {
      if (args[1].substr(0, 1) == "-") {
        ...
        std::istringstream is(args[1]);
        unsigned int tmp;
        unsigned char backup = RamReadEccPart;
        if (args[1].substr(0, 2) == "0x") {
            is >> std::hex >> tmp;
        }
        else if (args[1].substr(0, 1) == "0") {
        ...
        RamReadEccPart = (unsigned char)tmp;
        if (is.fail() || !is.eof()) {
          RamReadEccPart = backup;
          ret += err_msg + "wrong argument: " + (std::string)args[1] + specified_cmd;
        }
      }
    }
  ...
  else if (args[0] == "RamWriteEccPart") {
    // read mode
    if (0) {;}
    else if ((int)args.size() == 1) {
      std::ostringstream stream;
      stream << (unsigned int)RamWriteEccPart;
      ret = stream.str();
    }
    // write mode
    else if ((int)args.size() == 2) {
      if (args[1].substr(0, 1) == "-") {
        ...
        std::istringstream is(args[1]);
        unsigned int tmp;
        unsigned char backup = RamWriteEccPart;
        if (args[1].substr(0, 2) == "0x") {
            is >> std::hex >> tmp;
        }
        else if (args[1].substr(0, 1) == "0") {
        ...
        RamWriteEccPart = (unsigned char)tmp;
        if (is.fail() || !is.eof()) {
          RamWriteEccPart = backup;
          ret += err_msg + "wrong argument: " + (std::string)args[1] + specified_cmd;
        }
      }
    }
  ...
  else if (args[0] == "EncodeResult") {
    // read mode
    if (0) {;}
    else if ((int)args.size() == 1) {
      std::ostringstream stream;
      stream << (unsigned int)EncodeResult;
      ret = stream.str();
    }
    // write mode
    else if ((int)args.size() == 2) {
      if (args[1].substr(0, 1) == "-") {
        ...
        std::istringstream is(args[1]);
        unsigned int tmp;
        unsigned char backup = EncodeResult;
        if (args[1].substr(0, 2) == "0x") {
            is >> std::hex >> tmp;
        }
        else if (args[1].substr(0, 1) == "0") {
        ...
        EncodeResult = (unsigned char)tmp;
        if (is.fail() || !is.eof()) {
          EncodeResult = backup;
          ret += err_msg + "wrong argument: " + (std::string)args[1] + specified_cmd;
        }
  ...
  else if (args[0] == "DecodeResult") {
    // read mode
    if (0) {;}
    else if ((int)args.size() == 1) {
      std::ostringstream stream;
      stream << (unsigned int)DecodeResult;
      ret = stream.str();
    }
    // write mode
    else if ((int)args.size() == 2) {
      if (args[1].substr(0, 1) == "-") {
        ...
        std::istringstream is(args[1]);
        unsigned int tmp;
        unsigned char backup = DecodeResult;
        if (args[1].substr(0, 2) == "0x") {
            is >> std::hex >> tmp;
        }
        else if (args[1].substr(0, 1) == "0") {
        ...
        DecodeResult = (unsigned char)tmp;
        if (is.fail() || !is.eof()) {
          DecodeResult = backup;
          ret += err_msg + "wrong argument: " + (std::string)args[1] + specified_cmd;
        }
  ...
  else if (args[0] == "EccErrorAddress") {
    // read mode
    if (0) {;}
    else if ((int)args.size() == 1) {
      std::ostringstream stream;
      stream << EccErrorAddress;
      ret = stream.str();
    }
    // write mode
    else if ((int)args.size() == 2) {
      if (args[1].substr(0, 1) == "-") {
        ...
        std::istringstream is(args[1]);
        unsigned int backup = EccErrorAddress;
        if (args[1].substr(0, 2) == "0x") {
            is >> std::hex >> EccErrorAddress;
        }
        else if (args[1].substr(0, 1) == "0") {
            is >> std::oct >> EccErrorAddress;
        }
        else {
            is >> std::dec >> EccErrorAddress;
        }
        if (is.fail() || !is.eof()) {
          EccErrorAddress = backup;
          ret += err_msg + "wrong argument: " + (std::string)args[1] + specified_cmd;
        }
      }
    }
  ...
  else if (args[0] == "NumOfErrorStage") {
    // read mode
    if (0) {;}
    else if ((int)args.size() == 1) {
      std::ostringstream stream;
      stream << (unsigned int)NumOfErrorStage;
      ret = stream.str();
    }
    // write mode
    else if ((int)args.size() == 2) {
      if (args[1].substr(0, 1) == "-") {
        ...
        std::istringstream is(args[1]);
        unsigned int tmp;
        unsigned char backup = NumOfErrorStage;
        if (args[1].substr(0, 2) == "0x") {
            is >> std::hex >> tmp;
        }
        else if (args[1].substr(0, 1) == "0") {
        ...
        NumOfErrorStage = (unsigned char)tmp;
        if (is.fail() || !is.eof()) {
          NumOfErrorStage = backup;
          ret += err_msg + "wrong argument: " + (std::string)args[1] + specified_cmd;
        }
      }
    }
  ...
  else if (args[0] == "EccError") {
    if ((int)args.size() > 1 && (int)args.size() <= 2) {
      unsigned int err_det_set;
      if ((int)args.size() >= 2) {
        if (args[1][0] == '-') {
          ret = err_msg + "wrong argument" + specified_cmd;
          return ret;
        }
        if (!str2num(args[1], err_det_set)) {
          ret = err_msg + "wrong argument" + specified_cmd;
          return ret;
        }
      }
      EccError(err_det_set);
    }
    else {
      ret = err_msg + "wrong number of arguments" + specified_cmd;
    }
...
bool DumpInterrupt;
unsigned int RamReadData;
unsigned char RamReadEccPart;
unsigned char RamWriteEccPart;
unsigned char EncodeResult;
unsigned char DecodeResult;
unsigned int EccErrorAddress;
unsigned char NumOfErrorStage;
bool AccEnErrJudg;
bool TwoBitsErrJudg;
