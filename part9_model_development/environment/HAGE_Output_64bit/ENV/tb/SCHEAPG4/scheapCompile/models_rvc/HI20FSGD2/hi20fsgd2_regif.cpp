// -----------------------------------------------------------------------------
// $Id: hi20fsgd2_regif.cpp,v 1.3 2019/11/28 09:56:42 hadinhtran Exp $
//
...
/// @return none
Chi20fsgd2_regif::Chi20fsgd2_regif(std::string name, uint buswidth, uint ch_num)
    :vpcl::reg_super()
    ...
    #endif
{
    ...
    for(uint i = 0; i < mCh_num; i++) {
        str_tmp.str("");
        str_tmp<<"PBGPROT0"<< i;
        PBGPROT0[i] = new vpcl::re_register((0x0000) + 8*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < mCh_num; i++) {
        str_tmp.str("");
        str_tmp<<"PBGPROT1"<< i;
        PBGPROT1[i] = new vpcl::re_register((0x0004) + 8*i, this, str_tmp.str(), name.c_str());
    }
    ...
    index = 0;
    #ifdef USR_CWR_SYSTEMC
    for(uint i = 0; i < mCh_num; i++) cw_instantiate_reg(CW_PBGPROT0  [i], PBGPROT0[i]->name(), PBGPROT0[i]->addr());
    for(uint i = 0; i < mCh_num; i++) cw_instantiate_reg(CW_PBGPROT1  [i], PBGPROT1[i]->name(), PBGPROT1[i]->addr());
    #endif
    for(uint i = 0; i < mCh_num; i++) mCurReg = mRegArray[index++] = new SRegList(PBGPROT0  [i], mCurReg, i,  4, "8|16|32", "8|16|32");
    for(uint i = 0; i < mCh_num; i++) mCurReg = mRegArray[index++] = new SRegList(PBGPROT1  [i], mCurReg, i,  4, "8|16|32", "8|16|32");

    mRegList = mCurReg;

    for(uint i = 0; i < mCh_num; i++) {
        (*PBGPROT0  [i]) (31, 31, "LOCK"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PBGPROT0  [i]) ( 8,  8, "GEN"     , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
        (*PBGPROT0  [i]) ( 7,  7, "SEC"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PBGPROT0  [i]) ( 6,  6, "DBG"     , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
        (*PBGPROT0  [i]) ( 4,  4, "UM"      , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PBGPROT0  [i]) ( 1,  1, "WG"      , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PBGPROT0  [i]) ( 0,  0, "RG"      , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < mCh_num; i++) {
        (*PBGPROT1  [i]) (31,  0, "SPID"    , 0xF       , "R|W"   , vpcl::SPP_ENABLE);
    }
    ...
    for(uint i = 0; i < mCh_num; i++) {
        str_tmp.str("");
        str_tmp<<"PBGPROT0"<< i;
        mWrCbAPI[str_tmp.str()]["LOCK"] = &Chi20fsgd2_regif::cb_PBGPROT0_LOCK;
    }
    for(uint i = 0; i < mCh_num; i++) {
        str_tmp.str("");
        str_tmp<<"PBGPROT1"<< i;
        mWrCbAPI[str_tmp.str()]["SPID"] = &Chi20fsgd2_regif::cb_PBGPROT1_SPID;
    }
    InitLocalVal();
}
...
/// @return none
Chi20fsgd2_regif::~Chi20fsgd2_regif()
{
...
Chi20fsgd2_regif::uint Chi20fsgd2_regif::bit_select(
...
bool Chi20fsgd2_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Chi20fsgd2_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Chi20fsgd2_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                  const unsigned char *p_data,   ///< [in] Writing data
                                  cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Chi20fsgd2_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                  unsigned char *p_data,   ///< [out] Reading data
                                  cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Chi20fsgd2_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                      const unsigned char *p_data,  ///< [in] Writing data
                                      cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Chi20fsgd2_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                      unsigned char *p_data,   ///< [out] Reading data
                                      cuint size,              ///< [in]  Data size (byte)
                                      bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Chi20fsgd2_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                   unsigned char *p_data,          ///< [out] Reading data
                                   cuint size,                     ///< [in]  Data size (byte)
...
bool Chi20fsgd2_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                   const unsigned char *p_data,      ///< [in] Writing data
                                   cuint size,                       ///< [in] Data size (byte)
                                   cuint reg_index,                  ///< [in] Register index
...
bool Chi20fsgd2_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Chi20fsgd2_regif::CommandInit()
{
...
/// @return string
std::string Chi20fsgd2_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Chi20fsgd2_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Chi20fsgd2_regif::InitLocalVal()
{
    // Initialize local variables
    for(uint i = 0; i < mCh_num; i++) {
        PBGPROT0_LOCK[i] = 0x0;
...
/// @return None
void Chi20fsgd2_regif::UpdateLocalVal(cuint addr)
{
...
/// @return None
void Chi20fsgd2_regif::UpdateRegVal(cuint addr)
{
    if ((0x0000 <= addr) && (addr <= (0x0000)+(8*15)) && ((addr-0x0000)%8 == 0)) {
        uint i = (addr - 0x0000)/8;
...
/// @return vector
std::vector<std::string> Chi20fsgd2_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Chi20fsgd2_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Chi20fsgd2_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Chi20fsgd2_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Chi20fsgd2_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Chi20fsgd2_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Chi20fsgd2_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Chi20fsgd2_regif::uint Chi20fsgd2_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return none
void Chi20fsgd2_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Chi20fsgd2_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Chi20fsgd2_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Chi20fsgd2_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Chi20fsgd2_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Chi20fsgd2_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Chi20fsgd2_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
