// -----------------------------------------------------------------------------
// $Id: ATU5_TIMERG.cpp 1196 2019-10-17 05:33:04Z chuonghoangle $
//
 ...
 */
ATU5_TIMERG::ATU5_TIMERG(sc_module_name iname)
: sc_module (iname)
...
, pFuncModel (NULL)
{
    ...
    pFuncModel = new ATU5_TIMERG_Func ((string)iname, this);
    ...
    for (uint8_t evIdx = 0; evIdx < CONFIG::EVT_NUM; evIdx++) {
        ...
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERG::events_bus_scdmth
                                 ,this
                                 ,evIdx)
    ...
    for (uint8_t sblkIdx = 0; sblkIdx < CONFIG::SUBBLOCK_NUM; sblkIdx++) {
        sc_core::sc_spawn_options opt;
        sc_core::sc_spawn(sc_bind(&ATU5_TIMERG::cmi_g_scdmth
                                 ,this
                                 ,sblkIdx)
 ...
 */
void ATU5_TIMERG::setPCLKPeriod (const double& tperiod) {
    pFuncModel->setPCLKPeriod(tperiod);
}
//------------------------------------------------------------------------------
/**@brief the method handles PCLK
 */
void ATU5_TIMERG::setTimeUnit (const sc_time_unit& tunit) {
 ...
 */
void ATU5_TIMERG::setClkBusPeriod (const double& tperiod, const uint8_t& clkIdx) {
    /// save input values
 ...
 * @param event_idx index of desired event
 */
void ATU5_TIMERG::events_bus_scdmth(const uint8_t& evIdx) {
    while (1) {
        wait(events[evIdx]->value_changed_event());
        wait(SC_ZERO_TIME); // add by chuonghoangle 10/13/2018 to prevent racing condition of timer B's AGCKTIM and AGCKM
 ...
 */
void ATU5_TIMERG::cmi_g_scdmth(const uint8_t& sblk_id) {
    while (1) {
 ...
 * @param is_active
 */
void ATU5_TIMERG::EnableReset (const bool is_active) {
 ...
 *        when (ATU5_Base top wrapper) asserting reset
 * @note this replace the use of reset port
 * @param is_active
 */
void ATU5_TIMERG::Initialize () {
 ...
 *        or asserting reset (ATU5_Base top wrapper)
 * @note this replace the use of reset port
 * @param is_active
 */
void ATU5_TIMERG::CancelEvents () {}
//------------------------------------------------------------------------------
/// @todo comment me
void ATU5_TIMERG::EnableTIG(const bool& is_enable) {
    /// save input
