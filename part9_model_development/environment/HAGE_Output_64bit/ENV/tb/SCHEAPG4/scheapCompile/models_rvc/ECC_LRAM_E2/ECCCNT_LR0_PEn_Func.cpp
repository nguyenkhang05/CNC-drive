// ---------------------------------------------------------------------
// $Id: ECCCNT_LR0_PEn_Func.cpp,v 1.1 2019/09/26 10:07:33 landang Exp $
//
...
//========================================================
/// Constructor
ECCCNT_LR0_PEn_Func::ECCCNT_LR0_PEn_Func (std::string name, unsigned int pe_id, ECC_LRAM_E2_AgentController* LRAMAgentController):
                Cecccnt_lr0_pen_regif(name, 32)
                ,BusSlaveFuncIf()
{//{{{
    ...
    Cecccnt_lr0_pen_regif::set_instance_name(name);
}//}}}

/// Destructor
ECCCNT_LR0_PEn_Func::~ECCCNT_LR0_PEn_Func()
{//{{{
}//}}}

/// Reset Func and registers
void ECCCNT_LR0_PEn_Func::EnableReset(const std::string reset_name, const bool is_active)
{//{{{
    if (reset_name == "sys_resetz"){
        Cecccnt_lr0_pen_regif::EnableReset(is_active);
    }
}//}}}

/// Read API (target socket)
void ECCCNT_LR0_PEn_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void ECCCNT_LR0_PEn_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void ECCCNT_LR0_PEn_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cecccnt_lr0_pen_regif::reg_handle_command(cmd);
                if (msg_return != "") {
            ...
            msg_return = Cecccnt_lr0_pen_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
unsigned int ECCCNT_LR0_PEn_Func::GetLRECCCTLReg()
{//{{{
...
void ECCCNT_LR0_PEn_Func::cb_LRECCCTL_ECCDIS(RegCBstr str)
{//{{{ 
