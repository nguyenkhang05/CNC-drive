// ---------------------------------------------------------------------
// $Id: ECCCNT_CFC_FCC2_Func.cpp,v 1.7 2019/09/26 10:07:02 landang Exp $
//
...
//========================================================
/// Constructor
ECCCNT_CFC_FCC2_Func::ECCCNT_CFC_FCC2_Func (std::string name, unsigned int cfc_id, ECC_CFLASH_E2_FCC2_AgentController* CFLASHAgentController):
                Cecccnt_cfc_fcc2_regif(name, 32)
                ,BusSlaveFuncIf()
{//{{{
    ...
    mModuleName = name;
    mCfcId = cfc_id;
    mProtectedRegMap.insert(std::pair<unsigned int, std::string>(0x0, "CFCECCCTL"));
    Cecccnt_cfc_fcc2_regif::set_instance_name(name);
}//}}}

/// Destructor
ECCCNT_CFC_FCC2_Func::~ECCCNT_CFC_FCC2_Func()
{//{{{
}//}}}

/// Reset Func and registers
void ECCCNT_CFC_FCC2_Func::EnableReset(const std::string reset_name, const bool is_active)
{//{{{
    if (reset_name == "sys_resetz"){
        Cecccnt_cfc_fcc2_regif::EnableReset(is_active);
    }
}//}}}

/// Read API (target socket)
void ECCCNT_CFC_FCC2_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    // Get extension info
...
void ECCCNT_CFC_FCC2_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void ECCCNT_CFC_FCC2_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cecccnt_cfc_fcc2_regif::reg_handle_command(cmd);
                if (msg_return != "") {
            ...
            msg_return = Cecccnt_cfc_fcc2_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
/// Get value of CFCECCCTL register
unsigned int ECCCNT_CFC_FCC2_Func::GetCFCECCCTLReg()
{//{{{
    return (unsigned int)(*CFCECCCTL);
}//}}}

//========================================================
//============ Callback function  ========================
//========================================================
/// Callback function of CFCECCCTL_APEDIS bit
void ECCCNT_CFC_FCC2_Func::cb_CFCECCCTL_APEDIS(RegCBstr str)
{//{{{ 
    mpCFLASHAgentController->WriteDisPort_CFC(mCfcId, (unsigned int)(*CFCECCCTL));
}//}}}

/// Callback function of CFCECCCTL_BLANKMASKENABLE bit
void ECCCNT_CFC_FCC2_Func::cb_CFCECCCTL_BLANKMASKENABLE(RegCBstr str)
{//{{{
    mpCFLASHAgentController->WriteDisPort_CFC(mCfcId,(unsigned int)(*CFCECCCTL));
}//}}}

/// Callback function of CFCECCCTL_ECCDIS bit
void ECCCNT_CFC_FCC2_Func::cb_CFCECCCTL_ECCDIS(RegCBstr str)
{//{{{
    mpCFLASHAgentController->WriteDisPort_CFC(mCfcId,(unsigned int)(*CFCECCCTL));
}//}}}
