// ---------------------------------------------------------------------
// $Id: ATU5010_GLUE.cpp,v 1.0 2016/11/24 10:15:45 binhnguyen Exp $
//
...
// Corporation.
// ---------------------------------------------------------------------
#include "ATU5010_GLUE.h"
#include "ATU5010_GLUE_Func.h"

///Constructor of ATU5010_GLUE class
ATU5010_GLUE::ATU5010_GLUE (sc_module_name name,
            unsigned int rLatency,
            unsigned int wLatency):
            sc_module(name),
            ATU5010_GLUE_AgentController(),
            BusSlaveBase<32,1>(),
            PCLK("PCLK"),
            PRESETZ("PRESETZ"),
            // Input ports
...
{//{{{
    ...
    mATU5010_GLUE_Func = new ATU5010_GLUE_Func((std::string)name, this);
    sc_assert(mATU5010_GLUE_Func != NULL);
    mBusSlaveIf32[0]->setFuncModulePtr(mATU5010_GLUE_Func);
    ...
    SC_METHOD(WriteSLIC00_73Method);
    dont_initialize();
    sensitive << IMIGC00;
    sensitive << IMIGC01;
    sensitive << IMIGC02;
    ...
    sensitive << IMIOC72;
    sensitive << IMIOC73;
    //sensitive << mINTSELCregChangeEvent[0];
    
    SC_METHOD(WriteSLIC80_133Method);
    dont_initialize();
    sensitive << IMIGC80;
    sensitive << IMIGC81;
    sensitive << IMIGC82;
    ...
    sensitive << IMIOC132;
    sensitive << IMIOC133;
    //sensitive << mINTSELCregChangeEvent[1];
    ...
    for (unsigned int i = 0; i < emSLIDNum; i++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_GLUE::WriteSLIDxMethod, this, i),
    ...
    for (unsigned int i = 0; i < emSLDCDNum; i++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_GLUE::WriteSLDCDxMethod, this, i),
    ...
    for (unsigned int i = 0; i < emTQNum; i++) {
        for (unsigned int j = 0; j < emDFEFactorNum; j++) {
            ...
            opt.dont_initialize();
            sc_core::sc_spawn(sc_bind(&ATU5010_GLUE::WriteDFxMethod, this, i * emDFEFactorNum + j),
    ...
    for (unsigned int i = 0; i < emDSTSNum; i++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_GLUE::WriteDSTSxMethod, this, i),
    ...
    for (unsigned int i = 0; i < emP5Num; i++) {
        sc_core::sc_spawn_options opt;
        opt.spawn_method();
        switch (i) {
            case 0:     // TGC0
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ATU5010_GLUE::WriteP5xMethod, this, i),
...
///Destructor of ATU5010_GLUE class
ATU5010_GLUE::~ATU5010_GLUE (void)
{//{{{
    delete mATU5010_GLUE_Func;
}//}}}

///Assert reset by software
void ATU5010_GLUE::AssertReset (const std::string reset_name, const double start_time, const double period)
{//{{{
...
void ATU5010_GLUE::SetCLKFreq (const std::string clock_name, const sc_dt::uint64 freq, const std::string unit)
{//{{{
...
///Get clock value
void ATU5010_GLUE::GetCLKFreq (const std::string clock_name)
{//{{{
    std::vector<std::string> word_vector;
    SeparateString(word_vector, clock_name);
    if (word_vector.size() == 1) {
...
void ATU5010_GLUE::ForceRegister (const std::string reg_name, const unsigned int reg_value)
{//{{{
    std::vector<std::string> word_vector;
    ...
    if (reg_name != "") {
        ...
        if (word_vector.size() == 1) {
            ...
            mATU5010_GLUE_Func->RegisterHandler(cmd);
        } else {
...
void ATU5010_GLUE::ReleaseRegister (const std::string reg_name)
{//{{{
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ATU5010_GLUE::WriteRegister (const std::string reg_name, const unsigned int reg_value)
{//{{{
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ATU5010_GLUE::ReadRegister (const std::string reg_name)
{//{{{
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ATU5010_GLUE::ListRegister (void)
{//{{{
    std::vector<std::string> cmd;
    cmd.push_back("reg");
    mATU5010_GLUE_Func->RegisterHandler(cmd);
...
void ATU5010_GLUE::Help (const std::string type)
{//{{{
    std::vector<std::string> word_vector;
    SeparateString(word_vector, type);
    if (word_vector.size() == 1) {
        if (word_vector[0] == "parameters") {
            SC_REPORT_INFO(this->basename(),"ATU5010_GLUE_MessageLevel (fatal|error|warning|info)          Set debug message level (Default: fatal|error).");
            SC_REPORT_INFO(this->basename(),"ATU5010_GLUE_DumpRegisterRW (true/false)                      Enable/disable dumping access register (Default: false).");
        } else if (word_vector[0] == "commands") {
            SC_REPORT_INFO(this->basename(),"ATU5010_GLUE_AssertReset (reset_name, start_time, period)     Assert and de-assert reset signal.");
            SC_REPORT_INFO(this->basename(),"ATU5010_GLUE_SetCLKFreq (clock_name, freq, unit)              Set clock frequency value of model.");
            SC_REPORT_INFO(this->basename(),"ATU5010_GLUE_GetCLKFreq (clock_name)                          Get clock frequency value of model.");
            SC_REPORT_INFO(this->basename(),"ATU5010_GLUE_ForceRegister (reg_name, reg_value)              Force ATU5010_GLUE’s register with setting value.");
            SC_REPORT_INFO(this->basename(),"ATU5010_GLUE_ReleaseRegister (reg_name)                       Release ATU5010_GLUE’s register from force value.");
            SC_REPORT_INFO(this->basename(),"ATU5010_GLUE_WriteRegister (reg_name, reg_value)              Write a value to a ATU5010_GLUE’s register.");
            SC_REPORT_INFO(this->basename(),"ATU5010_GLUE_ReadRegister (reg_name)                          Read a value from a ATU5010_GLUE’s register.");
            SC_REPORT_INFO(this->basename(),"ATU5010_GLUE_ListRegister ()                                  Dump name of model's registers.");
        } else {
            re_printf("warning","The name (%s) of ATU5010_GLUE_Help argument is wrong (commands or parameters)\n", type.c_str());
        }
    } else {
        re_printf("warning","The name (%s) of ATU5010_GLUE_Help argument is wrong (commands or parameters)\n", type.c_str());
    }
...
void ATU5010_GLUE::SetMessageLevel (const std::string msg_lv)
{//{{{
    std::vector<std::string> cmd;
...
void ATU5010_GLUE::DumpRegisterRW (const std::string is_enable)
{//{{{
    std::vector<std::string> cmd;
    cmd.push_back("reg");
...
void ATU5010_GLUE::ConvertClockFreq (sc_dt::uint64 &freq_out, std::string &unit_out,
...
void ATU5010_GLUE::SeparateString (std::vector<std::string> &vtr, const std::string msg)
{//{{{
...
///Get time resolution
void ATU5010_GLUE::GetTimeResolution (sc_dt::uint64 &resolution_value, sc_time_unit &resolution_unit)
{//{{{
...
///Check reset status
bool ATU5010_GLUE::GetResetStatus (const std::string reset_name)
{//{{{
...
bool ATU5010_GLUE::CheckClockPeriod (const std::string clock_name)
{//{{{
...
///Handle PCLK signal
void ATU5010_GLUE::HandlePCLKSignalMethod (void)
{//{{{
...
void ATU5010_GLUE::HandlePRESETZSignalMethod (void)
{//{{{
...
void ATU5010_GLUE::HandleResetHardMethod (void)
{//{{{
...
void ATU5010_GLUE::HandleResetCmdMethod (void)
{//{{{
...
void ATU5010_GLUE::CancelResetCmdMethod (void)
{//{{{
    std::string reset_name = "PRESETZ";
    mIsResetCmdActive = false;
    mIsResetCmdReceive = false;
...
void ATU5010_GLUE::EnableReset (const std::string reset_name, const bool is_active)
{//{{{
    mATU5010_GLUE_Func->Reset(is_active);
    if (is_active) {
...
void ATU5010_GLUE::WriteSLIAxMethod (void)
{//{{{
    bool value[8];
    ...
    unsigned int int_sel_a = mATU5010_GLUE_Func->GetRegSelectSetting(emINTSELA);
...
void ATU5010_GLUE::WriteSLDBxMethod (void)
{//{{{
    bool value[9];
    ...
    unsigned int dma_sel_b = mATU5010_GLUE_Func->GetRegSelectSetting(emDMASELB);
    unsigned int temp[4];
...
void ATU5010_GLUE::WriteSLIC00_73Method (void)
{//{{{
    bool value[32][2];
    value[0][0] = IMIGC00.read();
    value[0][1] = IMIOC00.read();
    value[1][0] = IMIGC01.read();
    value[1][1] = IMIOC01.read();
    value[2][0] = IMIGC02.read();
    value[2][1] = IMIOC02.read();
    value[3][0] = IMIGC03.read();
    value[3][1] = IMIOC03.read();
    value[4][0] = IMIGC10.read();
    value[4][1] = IMIOC10.read();
    value[5][0] = IMIGC11.read();
    value[5][1] = IMIOC11.read();
    value[6][0] = IMIGC12.read();
    value[6][1] = IMIOC12.read();
    value[7][0] = IMIGC13.read();
    value[7][1] = IMIOC13.read();
    value[8][0] = IMIGC20.read();
    value[8][1] = IMIOC20.read();
    value[9][0] = IMIGC21.read();
    value[9][1] = IMIOC21.read();
    value[10][0] = IMIGC22.read();
    value[10][1] = IMIOC22.read();
    value[11][0] = IMIGC23.read();
    value[11][1] = IMIOC23.read();
    value[12][0] = IMIGC30.read();
    value[12][1] = IMIOC30.read();
    value[13][0] = IMIGC31.read();
    value[13][1] = IMIOC31.read();
    value[14][0] = IMIGC32.read();
    value[14][1] = IMIOC32.read();
    value[15][0] = IMIGC33.read();
    value[15][1] = IMIOC33.read();
    value[16][0] = IMIGC40.read();
    value[16][1] = IMIOC40.read();
    value[17][0] = IMIGC41.read();
    value[17][1] = IMIOC41.read();
    value[18][0] = IMIGC42.read();
    value[18][1] = IMIOC42.read();
    value[19][0] = IMIGC43.read();
    value[19][1] = IMIOC43.read();
    value[20][0] = IMIGC50.read();
    value[20][1] = IMIOC50.read();
    value[21][0] = IMIGC51.read();
    value[21][1] = IMIOC51.read();
    value[22][0] = IMIGC52.read();
    value[22][1] = IMIOC52.read();
    value[23][0] = IMIGC53.read();
    value[23][1] = IMIOC53.read();
    value[24][0] = IMIGC60.read();
    value[24][1] = IMIOC60.read();
    value[25][0] = IMIGC61.read();
    value[25][1] = IMIOC61.read();
    value[26][0] = IMIGC62.read();
    value[26][1] = IMIOC62.read();
    value[27][0] = IMIGC63.read();
    value[27][1] = IMIOC63.read();
    value[28][0] = IMIGC70.read();
    value[28][1] = IMIOC70.read();
    value[29][0] = IMIGC71.read();
    value[29][1] = IMIOC71.read();
    value[30][0] = IMIGC72.read();
    value[30][1] = IMIOC72.read();
    value[31][0] = IMIGC73.read();
    value[31][1] = IMIOC73.read();
    
    //unsigned int int_sel_c = mATU5010_GLUE_Func->GetRegSelectSetting(emINTSELC0);

    //SLIC00.write(value[0][int_sel_c  & 0x1]);
    //SLIC01.write(value[1][(int_sel_c >> 1) & 0x1]);
    //SLIC02.write(value[2][(int_sel_c >> 2) & 0x1]);
    //SLIC03.write(value[3][(int_sel_c >> 3) & 0x1]);
    //SLIC10.write(value[4][(int_sel_c >> 4) & 0x1]);
    //SLIC11.write(value[5][(int_sel_c >> 5) & 0x1]);
    //SLIC12.write(value[6][(int_sel_c >> 6) & 0x1]);
    //SLIC13.write(value[7][(int_sel_c >> 7) & 0x1]);
    //SLIC20.write(value[8][(int_sel_c >> 8) & 0x1]);
    //SLIC21.write(value[9][(int_sel_c >> 9) & 0x1]);
    //SLIC22.write(value[10][(int_sel_c >> 10) & 0x1]);
    //SLIC23.write(value[11][(int_sel_c >> 11) & 0x1]);
    //SLIC30.write(value[12][(int_sel_c >> 12) & 0x1]);
    //SLIC31.write(value[13][(int_sel_c >> 13) & 0x1]);
    //SLIC32.write(value[14][(int_sel_c >> 14) & 0x1]);
    //SLIC33.write(value[15][(int_sel_c >> 15) & 0x1]);
    //SLIC40.write(value[16][(int_sel_c >> 16) & 0x1]);
    //SLIC41.write(value[17][(int_sel_c >> 17) & 0x1]);
    //SLIC42.write(value[18][(int_sel_c >> 18) & 0x1]);
    //SLIC43.write(value[19][(int_sel_c >> 19) & 0x1]);
    //SLIC50.write(value[20][(int_sel_c >> 20) & 0x1]);
    //SLIC51.write(value[21][(int_sel_c >> 21) & 0x1]);
    //SLIC52.write(value[22][(int_sel_c >> 22) & 0x1]);
    //SLIC53.write(value[23][(int_sel_c >> 23) & 0x1]);
    //SLIC60.write(value[24][(int_sel_c >> 24) & 0x1]);
    //SLIC61.write(value[25][(int_sel_c >> 25) & 0x1]);
    //SLIC62.write(value[26][(int_sel_c >> 26) & 0x1]);
    //SLIC63.write(value[27][(int_sel_c >> 27) & 0x1]);
    //SLIC70.write(value[28][(int_sel_c >> 28) & 0x1]);
    //SLIC71.write(value[29][(int_sel_c >> 29) & 0x1]);
    //SLIC72.write(value[30][(int_sel_c >> 30) & 0x1]);
    //SLIC73.write(value[31][(int_sel_c >> 31) & 0x1]);

    SLIC00.write(value[0][0] || value [0][1]);
    SLIC01.write(value[1][0] || value [1][1]);
    SLIC02.write(value[2][0] || value [2][1]);
    SLIC03.write(value[3][0] || value [3][1]);
    SLIC10.write(value[4][0] || value [4][1]);
    SLIC11.write(value[5][0] || value [5][1]);
    SLIC12.write(value[6][0] || value [6][1]);
    SLIC13.write(value[7][0] || value [7][1]);
    SLIC20.write(value[8][0] || value [8][1]);
    SLIC21.write(value[9][0] || value [9][1]);
    SLIC22.write(value[10][0] || value [10][1]);
    SLIC23.write(value[11][0] || value [11][1]);
    SLIC30.write(value[12][0] || value [12][1]);
    SLIC31.write(value[13][0] || value [13][1]);
    SLIC32.write(value[14][0] || value [14][1]);
    SLIC33.write(value[15][0] || value [15][1]);
    SLIC40.write(value[16][0] || value [16][1]);
    SLIC41.write(value[17][0] || value [17][1]);
    SLIC42.write(value[18][0] || value [18][1]);
    SLIC43.write(value[19][0] || value [19][1]);
    SLIC50.write(value[20][0] || value [20][1]);
    SLIC51.write(value[21][0] || value [21][1]);
    SLIC52.write(value[22][0] || value [22][1]);
    SLIC53.write(value[23][0] || value [23][1]);
    SLIC60.write(value[24][0] || value [24][1]);
    SLIC61.write(value[25][0] || value [25][1]);
    SLIC62.write(value[26][0] || value [26][1]);
    SLIC63.write(value[27][0] || value [27][1]);
    SLIC70.write(value[28][0] || value [28][1]);
    SLIC71.write(value[29][0] || value [29][1]);
    SLIC72.write(value[30][0] || value [30][1]);
    SLIC73.write(value[31][0] || value [31][1]);
    
}//}}}


void ATU5010_GLUE::WriteSLIC80_133Method (void)
{//{{{
    bool value[24][2];
    value[0][0] = IMIGC80.read();
    value[0][1] = IMIOC80.read();
    value[1][0] = IMIGC81.read();
    value[1][1] = IMIOC81.read();
    value[2][0] = IMIGC82.read();
    value[2][1] = IMIOC82.read();
    value[3][0] = IMIGC83.read();
    value[3][1] = IMIOC83.read();
    value[4][0] = IMIGC90.read();
    value[4][1] = IMIOC90.read();
    value[5][0] = IMIGC91.read();
    value[5][1] = IMIOC91.read();
    value[6][0] = IMIGC92.read();
    value[6][1] = IMIOC92.read();
    value[7][0] = IMIGC93.read();
    value[7][1] = IMIOC93.read();
    value[8][0] = IMIGC100.read();
    value[8][1] = IMIOC100.read();
    value[9][0] = IMIGC101.read();
    value[9][1] = IMIOC101.read();
    value[10][0] = IMIGC102.read();
    value[10][1] = IMIOC102.read();
    value[11][0] = IMIGC103.read();
    value[11][1] = IMIOC103.read();
    
    value[12][0] = IMIGC110.read();
    value[12][1] = IMIOC110.read();
    value[13][0] = IMIGC111.read();
    value[13][1] = IMIOC111.read();
    value[14][0] = IMIGC112.read();
    value[14][1] = IMIOC112.read();
    value[15][0] = IMIGC113.read();
    value[15][1] = IMIOC113.read();
    value[16][0] = IMIGC120.read();
    value[16][1] = IMIOC120.read();
    value[17][0] = IMIGC121.read();
    value[17][1] = IMIOC121.read();
    value[18][0] = IMIGC122.read();
    value[18][1] = IMIOC122.read();
    value[19][0] = IMIGC123.read();
    value[19][1] = IMIOC123.read();
    value[20][0] = IMIGC130.read();
    value[20][1] = IMIOC130.read();
    value[21][0] = IMIGC131.read();
    value[21][1] = IMIOC131.read();
    value[22][0] = IMIGC132.read();
    value[22][1] = IMIOC132.read();
    value[23][0] = IMIGC133.read();
    value[23][1] = IMIOC133.read();

    //unsigned int int_sel_c = mATU5010_GLUE_Func->GetRegSelectSetting(emINTSELC1);

    //SLIC80.write(value[0][int_sel_c & 0x1]);
    //SLIC81.write(value[1][(int_sel_c >> 1) & 0x1]);
    //SLIC82.write(value[2][(int_sel_c >> 2) & 0x1]);
    //SLIC83.write(value[3][(int_sel_c >> 3) & 0x1]);
    //SLIC90.write(value[4][(int_sel_c >> 4) & 0x1]);
    //SLIC91.write(value[5][(int_sel_c >> 5) & 0x1]);
    //SLIC92.write(value[6][(int_sel_c >> 6) & 0x1]);
    //SLIC93.write(value[7][(int_sel_c >> 7) & 0x1]);
    //SLIC100.write(value[8][(int_sel_c >> 8) & 0x1]);
    //SLIC101.write(value[9][(int_sel_c >> 9) & 0x1]);
    //SLIC102.write(value[10][(int_sel_c >> 10) & 0x1]);
    //SLIC103.write(value[11][(int_sel_c >> 11) & 0x1]);
    //SLIC110.write(value[12][(int_sel_c >> 12) & 0x1]);
    //SLIC111.write(value[13][(int_sel_c >> 13) & 0x1]);
    //SLIC112.write(value[14][(int_sel_c >> 14) & 0x1]);
    //SLIC113.write(value[15][(int_sel_c >> 15) & 0x1]);
    //SLIC120.write(value[16][(int_sel_c >> 16) & 0x1]);
    //SLIC121.write(value[17][(int_sel_c >> 17) & 0x1]);
    //SLIC122.write(value[18][(int_sel_c >> 18) & 0x1]);
    //SLIC123.write(value[19][(int_sel_c >> 19) & 0x1]);
    //SLIC130.write(value[20][(int_sel_c >> 20) & 0x1]);
    //SLIC131.write(value[21][(int_sel_c >> 21) & 0x1]);
    //SLIC132.write(value[22][(int_sel_c >> 22) & 0x1]);
    //SLIC133.write(value[23][(int_sel_c >> 23) & 0x1]);
    
    SLIC80.write(value[0][0] || value [0][1]);
    SLIC81.write(value[1][0] || value [1][1]);
    SLIC82.write(value[2][0] || value [2][1]);
    SLIC83.write(value[3][0] || value [3][1]);
    SLIC90.write(value[4][0] || value [4][1]);
    SLIC91.write(value[5][0] || value [5][1]);
    SLIC92.write(value[6][0] || value [6][1]);
    SLIC93.write(value[7][0] || value [7][1]);
    SLIC100.write(value[8][0] || value [8][1]);
    SLIC101.write(value[9][0] || value [9][1]);
    SLIC102.write(value[10][0] || value [10][1]);
    SLIC103.write(value[11][0] || value [11][1]);
    SLIC110.write(value[12][0] || value [12][1]);
    SLIC111.write(value[13][0] || value [13][1]);
    SLIC112.write(value[14][0] || value [14][1]);
    SLIC113.write(value[15][0] || value [15][1]);
    SLIC120.write(value[16][0] || value [16][1]);
    SLIC121.write(value[17][0] || value [17][1]);
    SLIC122.write(value[18][0] || value [18][1]);
    SLIC123.write(value[19][0] || value [19][1]);
    SLIC130.write(value[20][0] || value [20][1]);
    SLIC131.write(value[21][0] || value [21][1]);
    SLIC132.write(value[22][0] || value [22][1]);
    SLIC133.write(value[23][0] || value [23][1]);

}//}}}

void ATU5010_GLUE::WriteSLIDxMethod (unsigned int id)
{//{{{
    bool value[6];
    switch (id) {
        case 0:
    ...
    unsigned int int_sel_d = mATU5010_GLUE_Func->GetRegSelectSetting(emINTSELD0 + (id >> 3));
...
void ATU5010_GLUE::WriteSLDCDxMethod (unsigned int id)
{//{{{
    bool value[7];
    switch (id) {
        case 0:
    ...
    unsigned int int_sel_cd = mATU5010_GLUE_Func->GetRegSelectSetting(emDMASELCD0 + (id >> 3));
...
void ATU5010_GLUE::WriteSLDExMethod (void)
{//{{{
    bool value[10][4];
    ...
    unsigned int dma_sel_e = mATU5010_GLUE_Func->GetRegSelectSetting(emDMASELE);
...
void ATU5010_GLUE::WriteOVFMethod (void)
{//{{{
...
void ATU5010_GLUE::WriteDFxMethod (unsigned int id)
{//{{{
    ...
    unsigned int dfe_sel = mATU5010_GLUE_Func->GetRegSelectSetting(emDFESEL0 + reg_id);
    bool int_en = (bool)((mATU5010_GLUE_Func->GetRegSelectSetting(emDFEENTQ0 + reg_id) >> id_mod) & 0x1);
    unsigned bit_val = 0;
    if (id_mod < 6) {
        // DFDx
...
void ATU5010_GLUE::WriteDEG1TRGMethod (void)
{//{{{
    bool value[8];
    ...
    unsigned int bit_val = mATU5010_GLUE_Func->GetRegSelectSetting(emDFESELD1T) & 0x7;
    DEG1TRG.write(value[bit_val]);
}//}}}

void ATU5010_GLUE::WriteDSTSxMethod (unsigned int id)
{//{{{
    bool value[4];
    switch (id) {
        case 0:
    ...
    unsigned int ds_sel_dsts = mATU5010_GLUE_Func->GetRegSelectSetting(emDSSELDSTS);
...
void ATU5010_GLUE::WriteCATSMethod (void)
{//{{{
    bool value[4];
    value[0] = TOC22.read();
    value[1] = TOD22A.read();
    ...
    unsigned int bit_val = mATU5010_GLUE_Func->GetRegSelectSetting(emCASELCATS) & 0x3;
    CATS.write(value[bit_val]);
}//}}}

void ATU5010_GLUE::WriteP5xMethod (unsigned int id)
{//{{{
    bool value[4];
    switch (id) {
        case 0:
    ...
    unsigned int p5s_sel = mATU5010_GLUE_Func->GetRegSelectSetting(emP5SSEL);
    unsigned int bit_val = (p5s_sel >> (id * 2)) & 0x3;
    
    switch (id) {
        case 0:
            P5TGC0.write(value[bit_val]);
            break;
        case 1:
...
void ATU5010_GLUE::WriteICIAxMethod (void)
{//{{{
...
void ATU5010_GLUE::WriteCIBxMethod (void)
{//{{{
...
void ATU5010_GLUE::WriteCMIExMethod (void)
{//{{{
...
void ATU5010_GLUE::WriteICIFxMethod (void)
{//{{{
...
void ATU5010_GLUE::WriteCMIGxMethod (void)
{//{{{
...
void ATU5010_GLUE::WriteDMAREQxMethod (void)
{//{{{
...
void ATU5010_GLUE::NotifyEvent (const unsigned int event, const unsigned int channel)
{//{{{
    switch (event) {
        case emINTSELA :
            ...
            break;
        //case emINTSELC0 :
        //    mINTSELCregChangeEvent[channel].notify(SC_ZERO_TIME);
        //    break;
        case emINTSELD0 :
