// -----------------------------------------------------------------------------
// $Id: ahberrslv_uimpl_regif.cpp,v 1.4 2014/10/03 03:16:28 dungvannguyen Exp $
//
...
//
// Input file : ahberrslv_uimpl_regif.txt
////////////////////////////////////////////////////////////////////////////////
// %MODULE AHBERRSLV_UIMPL
//     #                   name            offset_size
...
//     %%BIT    VM          7      7      0     R       TRUE     -
//     %%BIT    UM          6      6      0     R       TRUE     -
//     %%BIT    SIZE        3      1      0     R       TRUE     -
//     %%BIT    WRITE       0      0      0     R       TRUE     -
////////////////////////////////////////////////////////////////////////////////
/// @file ahberrslv_uimpl_regif.cpp
/// @brief Register IF class of model AHBERRSLV_UIMPL
/// $Id: ahberrslv_uimpl_regif.cpp,v 1.4 2014/10/03 03:16:28 dungvannguyen Exp $
/// $Date: 2014/10/03 03:16:28 $
/// $Revison$
...
/// @return none
Cahberrslv_uimpl_regif::Cahberrslv_uimpl_regif(std::string name, uint buswidth, uint version)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    CTL  = new vpcl::re_register(0x0000, this, "CTL" , name.c_str());
    STAT = new vpcl::re_register(0x0004, this, "STAT", name.c_str());
    ADDR = new vpcl::re_register(0x0008, this, "ADDR", name.c_str());
    TYPE = new vpcl::re_register(0x000C, this, "TYPE", name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_CTL   , CTL->name() , CTL->addr() );
    cw_instantiate_reg(CW_STAT  , STAT->name(), STAT->addr());
    cw_instantiate_reg(CW_ADDR  , ADDR->name(), ADDR->addr());
    cw_instantiate_reg(CW_TYPE  , TYPE->name(), TYPE->addr());
    #endif
    if (version == emCC_CUBE) {
        mCurReg = mRegArray[index++] = new SRegList(CTL   , mCurReg, 0,  4, "8|16|32", "8|16|32");
        mCurReg = mRegArray[index++] = new SRegList(STAT  , mCurReg, 0,  4, "8|16|32", "8|16|32");
        mCurReg = mRegArray[index++] = new SRegList(ADDR  , mCurReg, 0,  4,      "32", "8|16|32");
        mCurReg = mRegArray[index++] = new SRegList(TYPE  , mCurReg, 0,  4,      "16", "8|16|32");
    } else {//Other specification
        mCurReg = mRegArray[index++] = new SRegList(CTL   , mCurReg, 0,  4,      "32", "8|16|32");
        mCurReg = mRegArray[index++] = new SRegList(STAT  , mCurReg, 0,  4,      "32", "8|16|32");
        mCurReg = mRegArray[index++] = new SRegList(ADDR  , mCurReg, 0,  4,      "32", "8|16|32");
        mCurReg = mRegArray[index++] = new SRegList(TYPE  , mCurReg, 0,  4,      "32", "8|16|32");
    }

    mRegList = mCurReg;

    (*CTL   ) ( 1,  1, "CLRO"    , 0x0       , "W"     , vpcl::SPP_ENABLE);
    (*CTL   ) ( 0,  0, "CLRE"    , 0x0       , "W"     , vpcl::SPP_ENABLE);
    (*STAT  ) ( 1,  1, "OVF"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*STAT  ) ( 0,  0, "ERR"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*ADDR  ) (31,  0, "ADDR"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    if (version == emCC_CUBE) {
        (*TYPE  ) (20, 16, "SPID"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    } else {//Other specification
        (*TYPE  ) (9, 8, "SPID"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    (*TYPE  ) (15, 13, "PEID"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*TYPE  ) (12, 10, "VCID"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*TYPE  ) ( 7,  7, "VM"      , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*TYPE  ) ( 6,  6, "UM"      , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*TYPE  ) ( 3,  1, "SIZE"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*TYPE  ) ( 0,  0, "WRITE"   , 0x0       , "R"     , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["CTL"]["CLRE"] = &Cahberrslv_uimpl_regif::cb_CTL_CLRE;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cahberrslv_uimpl_regif::~Cahberrslv_uimpl_regif()
{
...
Cahberrslv_uimpl_regif::uint Cahberrslv_uimpl_regif::bit_select(
...
bool Cahberrslv_uimpl_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cahberrslv_uimpl_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cahberrslv_uimpl_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                        const unsigned char *p_data,   ///< [in] Writing data
                                        cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cahberrslv_uimpl_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                        unsigned char *p_data,   ///< [out] Reading data
                                        cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cahberrslv_uimpl_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                            const unsigned char *p_data,  ///< [in] Writing data
                                            cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cahberrslv_uimpl_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                            unsigned char *p_data,   ///< [out] Reading data
                                            cuint size,              ///< [in]  Data size (byte)
                                            bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cahberrslv_uimpl_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                         unsigned char *p_data,          ///< [out] Reading data
                                         cuint size,                     ///< [in]  Data size (byte)
...
bool Cahberrslv_uimpl_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                         const unsigned char *p_data,      ///< [in] Writing data
                                         cuint size,                       ///< [in] Data size (byte)
                                         cuint reg_index,                  ///< [in] Register index
...
bool Cahberrslv_uimpl_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cahberrslv_uimpl_regif::CommandInit()
{
...
/// @return string
std::string Cahberrslv_uimpl_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cahberrslv_uimpl_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cahberrslv_uimpl_regif::InitLocalVal()
{
    // Initialize local variables
    CTL_CLRO = 0x0;
    CTL_CLRE = 0x0;
...
/// @return None
void Cahberrslv_uimpl_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        CTL_CLRO                 = (*CTL )["CLRO"];
        CTL_CLRE                 = (*CTL )["CLRE"];
...
/// @return None
void Cahberrslv_uimpl_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*CTL )["CLRO"] = CTL_CLRO;
        (*CTL )["CLRE"] = CTL_CLRE;
...
/// @return vector
std::vector<std::string> Cahberrslv_uimpl_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cahberrslv_uimpl_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cahberrslv_uimpl_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cahberrslv_uimpl_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cahberrslv_uimpl_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cahberrslv_uimpl_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cahberrslv_uimpl_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cahberrslv_uimpl_regif::uint Cahberrslv_uimpl_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cahberrslv_uimpl_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cahberrslv_uimpl_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cahberrslv_uimpl_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cahberrslv_uimpl_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cahberrslv_uimpl_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cahberrslv_uimpl_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cahberrslv_uimpl_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
