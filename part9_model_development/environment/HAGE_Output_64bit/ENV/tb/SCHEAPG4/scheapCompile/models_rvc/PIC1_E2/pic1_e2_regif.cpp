// -----------------------------------------------------------------------------
// $Id: pic1_e2_regif.cpp 3395 2017-06-29 08:02:34Z ChinhTD $
//
...
/// @return none
Cpic1_e2_regif::Cpic1_e2_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
    , cwmem("register", scml_memsize(0x12/2))
    #endif
{
    CommandInit();
    PIC1SST   = new vpcl::re_register(0x0004, this, "PIC1SST"  , name.c_str());
    PIC1SSER0 = new vpcl::re_register(0x0010, this, "PIC1SSER0", name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_PIC1SST_, "PIC1SST_", 0x0004);
    CW_PIC1SST   = new scml2::bitfield<unsigned short>(PIC1SST->name()  , CW_PIC1SST_          ,  0,  8);
    cw_instantiate_reg(CW_PIC1SSER0  , PIC1SSER0->name(), PIC1SSER0->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(PIC1SST    , mCurReg, 0,  1,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PIC1SSER0  , mCurReg, 0,  2,      "16", "8|16|32");

    mRegList = mCurReg;

    (*PIC1SST    ) ( 0,  0, "SYNCTRG" , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*PIC1SSER0  ) ( 6,  6, "PIC1SSER006", 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*PIC1SSER0  ) ( 5,  5, "PIC1SSER005", 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*PIC1SSER0  ) ( 4,  4, "PIC1SSER004", 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*PIC1SSER0  ) ( 3,  3, "PIC1SSER003", 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*PIC1SSER0  ) ( 2,  2, "PIC1SSER002", 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*PIC1SSER0  ) ( 1,  1, "PIC1SSER001", 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*PIC1SSER0  ) ( 0,  0, "PIC1SSER000", 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["PIC1SST"]["SYNCTRG"] = &Cpic1_e2_regif::cb_PIC1SST_SYNCTRG;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cpic1_e2_regif::~Cpic1_e2_regif()
{
...
Cpic1_e2_regif::uint Cpic1_e2_regif::bit_select(
...
bool Cpic1_e2_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cpic1_e2_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cpic1_e2_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                    const unsigned char *p_data,   ///< [in] Writing data
                                    cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cpic1_e2_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                    unsigned char *p_data,   ///< [out] Reading data
                                    cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cpic1_e2_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                        const unsigned char *p_data,  ///< [in] Writing data
                                        cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cpic1_e2_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                        unsigned char *p_data,   ///< [out] Reading data
                                        cuint size,              ///< [in]  Data size (byte)
                                        bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cpic1_e2_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                     unsigned char *p_data,          ///< [out] Reading data
                                     cuint size,                     ///< [in]  Data size (byte)
...
bool Cpic1_e2_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                     const unsigned char *p_data,      ///< [in] Writing data
                                     cuint size,                       ///< [in] Data size (byte)
                                     cuint reg_index,                  ///< [in] Register index
...
bool Cpic1_e2_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cpic1_e2_regif::CommandInit()
{
...
/// @return string
std::string Cpic1_e2_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cpic1_e2_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cpic1_e2_regif::InitLocalVal()
{
    // Initialize local variables
    PIC1SST_SYNCTRG = 0x0;
    PIC1SSER0_PIC1SSER006 = 0x0;
    PIC1SSER0_PIC1SSER005 = 0x0;
    PIC1SSER0_PIC1SSER004 = 0x0;
    PIC1SSER0_PIC1SSER003 = 0x0;
    PIC1SSER0_PIC1SSER002 = 0x0;
    PIC1SSER0_PIC1SSER001 = 0x0;
    PIC1SSER0_PIC1SSER000 = 0x0;
}

/// Update bit value to local value
/// @return None
void Cpic1_e2_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0004) {
        PIC1SST_SYNCTRG               = (*PIC1SST  )["SYNCTRG"  ];
        return;
    }
    if (addr == 0x0010) {
        PIC1SSER0_PIC1SSER006         = (*PIC1SSER0)["PIC1SSER006"];
        PIC1SSER0_PIC1SSER005         = (*PIC1SSER0)["PIC1SSER005"];
        PIC1SSER0_PIC1SSER004         = (*PIC1SSER0)["PIC1SSER004"];
        PIC1SSER0_PIC1SSER003         = (*PIC1SSER0)["PIC1SSER003"];
        PIC1SSER0_PIC1SSER002         = (*PIC1SSER0)["PIC1SSER002"];
        PIC1SSER0_PIC1SSER001         = (*PIC1SSER0)["PIC1SSER001"];
        PIC1SSER0_PIC1SSER000         = (*PIC1SSER0)["PIC1SSER000"];
        return;
    }
}
...
/// @return None
void Cpic1_e2_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0004) {
        (*PIC1SST  )["SYNCTRG"  ] = PIC1SST_SYNCTRG;
        return;
    }
    if (addr == 0x0010) {
        (*PIC1SSER0)["PIC1SSER006"] = PIC1SSER0_PIC1SSER006;
        (*PIC1SSER0)["PIC1SSER005"] = PIC1SSER0_PIC1SSER005;
        (*PIC1SSER0)["PIC1SSER004"] = PIC1SSER0_PIC1SSER004;
        (*PIC1SSER0)["PIC1SSER003"] = PIC1SSER0_PIC1SSER003;
        (*PIC1SSER0)["PIC1SSER002"] = PIC1SSER0_PIC1SSER002;
        (*PIC1SSER0)["PIC1SSER001"] = PIC1SSER0_PIC1SSER001;
        (*PIC1SSER0)["PIC1SSER000"] = PIC1SSER0_PIC1SSER000;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cpic1_e2_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cpic1_e2_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cpic1_e2_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cpic1_e2_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cpic1_e2_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cpic1_e2_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cpic1_e2_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cpic1_e2_regif::uint Cpic1_e2_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cpic1_e2_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cpic1_e2_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cpic1_e2_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cpic1_e2_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
unsigned short Cpic1_e2_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
unsigned short Cpic1_e2_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(unsigned short));
    return sizeof(unsigned short);
}
void Cpic1_e2_regif::cw_instantiate_reg(scml2::reg<unsigned short> * reg, std::string reg_name, int offset)
{
