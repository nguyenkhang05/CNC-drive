// ---------------------------------------------------------------------
// $Id: ECCCNT_DTS_Func.cpp,v 1.6 2019/09/26 10:07:20 landang Exp $
//
...
//========================================================
/// Constructor
ECCCNT_DTS_Func::ECCCNT_DTS_Func (std::string name, ECC_DMADTS_RAM_E2_AgentController* DMADTS_RAMAgentController):
                Cecccnt_dts_regif(name, 32)
                ,BusSlaveFuncIf()
{//{{{
    ...
    mProtectedRegMap.insert(std::pair<unsigned int, std::string>(0x0, "DRECCCTL"));

    Cecccnt_dts_regif::set_instance_name(name);
}//}}}

/// Destructor
ECCCNT_DTS_Func::~ECCCNT_DTS_Func()
{//{{{
}//}}}

/// Reset Func and registers
void ECCCNT_DTS_Func::EnableReset(const std::string reset_name, const bool is_active)
{//{{{
    if (reset_name == "sys_resetz"){
        Cecccnt_dts_regif::EnableReset(is_active);
    }
}//}}}

/// Read API (target socket)
void ECCCNT_DTS_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    // Get extension info
...
void ECCCNT_DTS_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
...
void ECCCNT_DTS_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cecccnt_dts_regif::reg_handle_command(cmd);
                if (msg_return != "") {
            ...
            msg_return = Cecccnt_dts_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
unsigned int ECCCNT_DTS_Func::GetDRECCCTLReg()
{//{{{
    return (unsigned int)(*DRECCCTL);
}//}}}
...
void ECCCNT_DTS_Func::cb_DRECCCTL_ECCDIS(RegCBstr str)
{//{{{ 
    mpDMADTS_RAMAgentController->WriteDisPort_DR((unsigned int)(*DRECCCTL));
}//}}}
