// -----------------------------------------------------------------------------
// $Id: g4_pegcap_e2x_regif.cpp,v 1.6 2020/01/14 02:39:30 nhutminhdang Exp $
//
...
/// @return none
Cg4_pegcap_e2x_regif::Cg4_pegcap_e2x_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
...
{
    CommandInit();
    PEGERRCLR  = new vpcl::re_register(0x0000, this, "PEGERRCLR" , name.c_str());
    PEGERRSTAT = new vpcl::re_register(0x0004, this, "PEGERRSTAT", name.c_str());
    ...
    index = 0;
    #ifdef USR_CWR_SYSTEMC
    cw_instantiate_reg(CW_PEGERRCLR   , PEGERRCLR->name() , PEGERRCLR->addr() );
    cw_instantiate_reg(CW_PEGERRSTAT  , PEGERRSTAT->name(), PEGERRSTAT->addr());
    ...
    #endif
    mCurReg = mRegArray[index++] = new SRegList(PEGERRCLR   , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PEGERRSTAT  , mCurReg, 0,  4,      "32", "8|16|32");
    ...
    (*PEGERRCLR   ) ( 1,  1, "CLRO"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*PEGERRCLR   ) ( 0,  0, "CLRE"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*PEGERRSTAT  ) ( 1,  1, "OVF"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PEGERRSTAT  ) ( 0,  0, "ERR"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["PEGERRCLR"]["CLRO"    ] = &Cg4_pegcap_e2x_regif::cb_PEGERRCLR_CLRO;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cg4_pegcap_e2x_regif::~Cg4_pegcap_e2x_regif()
{
...
Cg4_pegcap_e2x_regif::uint Cg4_pegcap_e2x_regif::bit_select(
...
bool Cg4_pegcap_e2x_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cg4_pegcap_e2x_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cg4_pegcap_e2x_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                      const unsigned char *p_data,   ///< [in] Writing data
                                      cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cg4_pegcap_e2x_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                      unsigned char *p_data,   ///< [out] Reading data
                                      cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cg4_pegcap_e2x_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                          const unsigned char *p_data,  ///< [in] Writing data
                                          cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cg4_pegcap_e2x_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                          unsigned char *p_data,   ///< [out] Reading data
                                          cuint size,              ///< [in]  Data size (byte)
                                          bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cg4_pegcap_e2x_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                       unsigned char *p_data,          ///< [out] Reading data
                                       cuint size,                     ///< [in]  Data size (byte)
...
bool Cg4_pegcap_e2x_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                       const unsigned char *p_data,      ///< [in] Writing data
                                       cuint size,                       ///< [in] Data size (byte)
                                       cuint reg_index,                  ///< [in] Register index
...
bool Cg4_pegcap_e2x_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cg4_pegcap_e2x_regif::CommandInit()
{
...
/// @return string
std::string Cg4_pegcap_e2x_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cg4_pegcap_e2x_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cg4_pegcap_e2x_regif::InitLocalVal()
{
    // Initialize local variables
    PEGERRCLR_CLRO = 0x0;
    PEGERRCLR_CLRE = 0x0;
...
/// @return None
void Cg4_pegcap_e2x_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        PEGERRCLR_CLRO                 = (*PEGERRCLR )["CLRO"      ];
        PEGERRCLR_CLRE                 = (*PEGERRCLR )["CLRE"      ];
        return;
    }
    if (addr == 0x0004) {
        PEGERRSTAT_OVF                 = (*PEGERRSTAT)["OVF"       ];
        PEGERRSTAT_ERR                 = (*PEGERRSTAT)["ERR"       ];
        return;
    }
    if (addr == 0x0008) {
...
/// @return None
void Cg4_pegcap_e2x_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*PEGERRCLR )["CLRO"      ] = PEGERRCLR_CLRO;
        (*PEGERRCLR )["CLRE"      ] = PEGERRCLR_CLRE;
        return;
    }
    if (addr == 0x0004) {
        (*PEGERRSTAT)["OVF"       ] = PEGERRSTAT_OVF;
        (*PEGERRSTAT)["ERR"       ] = PEGERRSTAT_ERR;
        return;
    }
    if (addr == 0x0008) {
...
/// @return vector
std::vector<std::string> Cg4_pegcap_e2x_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cg4_pegcap_e2x_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cg4_pegcap_e2x_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cg4_pegcap_e2x_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cg4_pegcap_e2x_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cg4_pegcap_e2x_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cg4_pegcap_e2x_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Cg4_pegcap_e2x_regif::uint Cg4_pegcap_e2x_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return none
void Cg4_pegcap_e2x_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cg4_pegcap_e2x_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cg4_pegcap_e2x_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cg4_pegcap_e2x_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Cg4_pegcap_e2x_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cg4_pegcap_e2x_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cg4_pegcap_e2x_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
