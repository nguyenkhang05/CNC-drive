...
//      $Id: gen_cmdif.py,v 1.28 2013/08/08 02:30:52 sontran Exp
//      $Id: handleCommand.skl,v 1.19 2013/07/31 06:05:12 sontran Exp 
    ...
    if (((int)args.size() == 2) || ((int)args.size() == 3)){
        ...
        for (unsigned int port_index = 0; port_index < this->mForcedPortArray.size(); port_index++){
            if (args[1] == this->mForcedPortArray[port_index]->basename()){
                ...
                //combine output signal of Cecm_wp if force port successfully
                if (write_success){
        ...
        if (port_in_list == false){
            ret = err_msg + "The port should be among sgati, sgatnmi, sgaterroz, sgatresz, dtmtrgout" + specified_cmd;
        }
    }else{
        ret = err_msg + "wrong number of arguments" + specified_cmd;
    }
