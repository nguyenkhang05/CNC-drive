// -----------------------------------------------------------------------------
// $Id: pbguard_regif.cpp,v 1.8 2019/06/16 06:02:16 nhuvo Exp $
//
...
/// @return none
Cpbguard_regif::Cpbguard_regif(std::string name, uint buswidth, unsigned int ch_num)
    :vpcl::reg_super()
    ...
    #endif
{
    CommandInit();
    mChNum = ch_num;
    PBGERRCLR  = new vpcl::re_register(0x0000, this, "PBGERRCLR" , name.c_str());
    PBGERRSTAT = new vpcl::re_register(0x0004, this, "PBGERRSTAT", name.c_str());
    PBGERRADDR = new vpcl::re_register(0x0008, this, "PBGERRADDR", name.c_str());
    PBGERRTYPE = new vpcl::re_register(0x000C, this, "PBGERRTYPE", name.c_str());
    PBGKCPROT  = new vpcl::re_register(0x0010, this, "PBGKCPROT" , name.c_str());
    std::ostringstream str_tmp;
    re_printf("info", "Number of channel is: %d .\n", mChNum);
    for(uint i = 0; i < mChNum; i++) {
        str_tmp.str("");
        str_tmp<<"PBGPROT0"<< i;
        PBGPROT0  [i] = new vpcl::re_register((0x0080) + 8*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < mChNum; i++) {
        str_tmp.str("");
        str_tmp<<"PBGPROT1"<< i;
        PBGPROT1  [i] = new vpcl::re_register((0x0084) + 8*i, this, str_tmp.str(), name.c_str());
    }
    ...
    index = 0;
    #ifdef USR_CWR_SYSTEMC
    cw_instantiate_reg(CW_PBGERRCLR   , PBGERRCLR->name() , PBGERRCLR->addr() );
    cw_instantiate_reg(CW_PBGERRSTAT  , PBGERRSTAT->name(), PBGERRSTAT->addr());
    cw_instantiate_reg(CW_PBGERRADDR  , PBGERRADDR->name(), PBGERRADDR->addr());
    cw_instantiate_reg(CW_PBGERRTYPE  , PBGERRTYPE->name(), PBGERRTYPE->addr());
    cw_instantiate_reg(CW_PBGKCPROT   , PBGKCPROT->name() , PBGKCPROT->addr() );
    for(uint i = 0; i < mChNum  ; i++) cw_instantiate_reg(CW_PBGPROT0    [i], PBGPROT0  [i]->name(), PBGPROT0  [i]->addr());
    for(uint i = 0; i < mChNum  ; i++) cw_instantiate_reg(CW_PBGPROT1    [i], PBGPROT1  [i]->name(), PBGPROT1  [i]->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(PBGERRCLR   , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PBGERRSTAT  , mCurReg, 0,  4,    "8|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PBGERRADDR  , mCurReg, 0,  4,      "32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PBGERRTYPE  , mCurReg, 0,  4,   "16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PBGKCPROT   , mCurReg, 0,  4,      "32", "8|16|32");
    for(uint i = 0; i < mChNum  ; i++) mCurReg = mRegArray[index++] = new SRegList(PBGPROT0    [i], mCurReg, i,  4, "8|16|32", "8|16|32");
    for(uint i = 0; i < mChNum  ; i++) mCurReg = mRegArray[index++] = new SRegList(PBGPROT1    [i], mCurReg, i,  4, "8|16|32", "8|16|32");

    mRegList = mCurReg;

    (*PBGERRCLR   ) ( 1,  1, "CLRO"    , 0x0       , "R|W1:0", vpcl::SPP_ENABLE);
    (*PBGERRCLR   ) ( 0,  0, "CLRE"    , 0x0       , "R|W1:0", vpcl::SPP_ENABLE);
    (*PBGERRSTAT  ) ( 1,  1, "OVF"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PBGERRSTAT  ) ( 0,  0, "ERR"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PBGERRADDR  ) (31,  0, "ADDR"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PBGERRTYPE  ) (13, 13, "SEC"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PBGERRTYPE  ) (12, 12, "DBG"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PBGERRTYPE  ) (11, 11, "UM"      , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PBGERRTYPE  ) (10,  6, "SPI"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PBGERRTYPE  ) ( 0,  0, "WRITE"   , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PBGKCPROT   ) (31,  1, "KCPROT"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*PBGKCPROT   ) ( 0,  0, "KCE"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    for(uint i = 0; i < mChNum; i++) {
        (*PBGPROT0    [i]) (31, 31, "LOCK"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PBGPROT0    [i]) ( 8,  8, "GEN"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PBGPROT0    [i]) ( 7,  7, "SEC"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PBGPROT0    [i]) ( 6,  6, "DBG"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PBGPROT0    [i]) ( 4,  4, "UM"      , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PBGPROT0    [i]) ( 1,  1, "WG"      , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PBGPROT0    [i]) ( 0,  0, "RG"      , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < mChNum; i++) {
        (*PBGPROT1    [i]) (31,  0, "SPID"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    ...
    mWrCbAPI["PBGERRCLR"]["CLRO"    ] = &Cpbguard_regif::cb_PBGERRCLR_CLRO;
    mWrCbAPI["PBGKCPROT"]["KCPROT"  ] = &Cpbguard_regif::cb_PBGKCPROT_KCPROT;

    for(uint i = 0; i < mChNum; i++) {
        str_tmp.str("");
        str_tmp<<"PBGPROT0"<< i;
        mWrCbAPI[str_tmp.str()]["LOCK"] = &Cpbguard_regif::cb_PBGPROT0_LOCK;
    }
    for(uint i = 0; i < mChNum; i++) {
        str_tmp.str("");
        str_tmp<<"PBGPROT1"<< i;
        mWrCbAPI[str_tmp.str()]["SPID"] = &Cpbguard_regif::cb_PBGPROT1_SPID;
    }
    InitLocalVal();
}
...
Cpbguard_regif::uint Cpbguard_regif::bit_select(
...
bool Cpbguard_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cpbguard_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cpbguard_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                const unsigned char *p_data,   ///< [in] Writing data
                                cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cpbguard_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                unsigned char *p_data,   ///< [out] Reading data
                                cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cpbguard_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                    const unsigned char *p_data,  ///< [in] Writing data
                                    cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cpbguard_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                    unsigned char *p_data,   ///< [out] Reading data
                                    cuint size,              ///< [in]  Data size (byte)
                                    bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cpbguard_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                 unsigned char *p_data,          ///< [out] Reading data
                                 cuint size,                     ///< [in]  Data size (byte)
                                 ...
                                 bool IsDbgFunc)                 ///< [in] flag indicate reg_rd or reg_rd_dbg
{
    ...
    if (size < reg_length) {
        uint read_data = 0;
        if (((reg_length%2 == 0) && (reg_length%size == 0) && ((addr&mask_size)%size == 0)) || (IsDbgFunc == false)) {
...
bool Cpbguard_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                 const unsigned char *p_data,      ///< [in] Writing data
                                 cuint size,                       ///< [in] Data size (byte)
                                 cuint reg_index,                  ///< [in] Register index
...
bool Cpbguard_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cpbguard_regif::CommandInit()
{
...
/// @return string
std::string Cpbguard_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cpbguard_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cpbguard_regif::InitLocalVal()
{
    // Initialize local variables
    PBGERRCLR_CLRO = 0x0;
    PBGERRCLR_CLRE = 0x0;
    ...
    PBGERRTYPE_UM = 0x0;
    PBGERRTYPE_SPI = 0x0;
    PBGERRTYPE_WRITE = 0x0;
    PBGKCPROT_KCPROT = 0x0;
    PBGKCPROT_KCE = 0x0;
    for(uint i = 0; i < mChNum;  i++) {
        PBGPROT0_LOCK[i] = 0x0;
        PBGPROT0_GEN[i] = 0x0;
...
/// @return None
void Cpbguard_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        PBGERRCLR_CLRO                 = (*PBGERRCLR )["CLRO"      ];
        PBGERRCLR_CLRE                 = (*PBGERRCLR )["CLRE"      ];
        return;
    }
    if (addr == 0x0004) {
        PBGERRSTAT_OVF                 = (*PBGERRSTAT)["OVF"       ];
        PBGERRSTAT_ERR                 = (*PBGERRSTAT)["ERR"       ];
        return;
    }
    if (addr == 0x0008) {
        PBGERRADDR_ADDR                = (*PBGERRADDR)["ADDR"      ];
        return;
    }
    if (addr == 0x000C) {
        PBGERRTYPE_SEC                 = (*PBGERRTYPE)["SEC"       ];
        PBGERRTYPE_DBG                 = (*PBGERRTYPE)["DBG"       ];
        PBGERRTYPE_UM                  = (*PBGERRTYPE)["UM"        ];
        PBGERRTYPE_SPI                 = (*PBGERRTYPE)["SPI"       ];
        PBGERRTYPE_WRITE               = (*PBGERRTYPE)["WRITE"     ];
        return;
    }
    if (addr == 0x0010) {
        PBGKCPROT_KCPROT               = (*PBGKCPROT )["KCPROT"    ];
        PBGKCPROT_KCE                  = (*PBGKCPROT )["KCE"       ];
        return;
    }
    if ((0x0080 <= addr) && (addr <= (0x0080)+(8*(mChNum-1))) && ((addr-0x0080)%8 == 0)) {
        uint i = (addr - 0x0080)/8;
        PBGPROT0_LOCK                 [i] = (*(PBGPROT0  [i]))["LOCK"      ];
        PBGPROT0_GEN                  [i] = (*(PBGPROT0  [i]))["GEN"       ];
        PBGPROT0_SEC                  [i] = (*(PBGPROT0  [i]))["SEC"       ];
        PBGPROT0_DBG                  [i] = (*(PBGPROT0  [i]))["DBG"       ];
        PBGPROT0_UM                   [i] = (*(PBGPROT0  [i]))["UM"        ];
        PBGPROT0_WG                   [i] = (*(PBGPROT0  [i]))["WG"        ];
        PBGPROT0_RG                   [i] = (*(PBGPROT0  [i]))["RG"        ];
        return;
    }
    if ((0x0084 <= addr) && (addr <= (0x0084)+(8*(mChNum-1))) && ((addr-0x0084)%8 == 0)) {
        uint i = (addr - 0x0084)/8;
        PBGPROT1_SPID                 [i] = (*(PBGPROT1  [i]))["SPID"      ];
        return;
    }
}
...
/// @return None
void Cpbguard_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*PBGERRCLR )["CLRO"      ] = PBGERRCLR_CLRO;
        (*PBGERRCLR )["CLRE"      ] = PBGERRCLR_CLRE;
        return;
    }
    if (addr == 0x0004) {
        (*PBGERRSTAT)["OVF"       ] = PBGERRSTAT_OVF;
        (*PBGERRSTAT)["ERR"       ] = PBGERRSTAT_ERR;
        return;
    }
    if (addr == 0x0008) {
        (*PBGERRADDR)["ADDR"      ] = PBGERRADDR_ADDR;
        return;
    }
    if (addr == 0x000C) {
        (*PBGERRTYPE)["SEC"       ] = PBGERRTYPE_SEC;
        (*PBGERRTYPE)["DBG"       ] = PBGERRTYPE_DBG;
        (*PBGERRTYPE)["UM"        ] = PBGERRTYPE_UM;
        (*PBGERRTYPE)["SPI"       ] = PBGERRTYPE_SPI;
        (*PBGERRTYPE)["WRITE"     ] = PBGERRTYPE_WRITE;
        return;
    }
    if (addr == 0x0010) {
        (*PBGKCPROT )["KCPROT"    ] = PBGKCPROT_KCPROT;
        (*PBGKCPROT )["KCE"       ] = PBGKCPROT_KCE;
        return;
    }
    if ((0x0080 <= addr) && (addr <= (0x0080)+(8*(mChNum-1))) && ((addr-0x0080)%8 == 0)) {
        uint i = (addr - 0x0080)/8;
        (*(PBGPROT0  [i]))["LOCK"      ] = PBGPROT0_LOCK[i];
        (*(PBGPROT0  [i]))["GEN"       ] = PBGPROT0_GEN[i];
        (*(PBGPROT0  [i]))["SEC"       ] = PBGPROT0_SEC[i];
        (*(PBGPROT0  [i]))["DBG"       ] = PBGPROT0_DBG[i];
        (*(PBGPROT0  [i]))["UM"        ] = PBGPROT0_UM[i];
        (*(PBGPROT0  [i]))["WG"        ] = PBGPROT0_WG[i];
        (*(PBGPROT0  [i]))["RG"        ] = PBGPROT0_RG[i];
        return;
    }
    if ((0x0084 <= addr) && (addr <= (0x0084)+(8*(mChNum-1))) && ((addr-0x0084)%8 == 0)) {
        uint i = (addr - 0x0084)/8;
        (*(PBGPROT1  [i]))["SPID"      ] = PBGPROT1_SPID[i];
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cpbguard_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cpbguard_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cpbguard_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cpbguard_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cpbguard_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cpbguard_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cpbguard_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Cpbguard_regif::uint Cpbguard_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return none
void Cpbguard_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cpbguard_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cpbguard_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cpbguard_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Cpbguard_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cpbguard_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cpbguard_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
