// ---------------------------------------------------------------------
// $Id: spidctl_regif.cpp,v 1.2 2020/02/06 04:48:44 khoaanhnguyen Exp $
//
...
// Corporation.
// ---------------------------------------------------------------------
// Description: Copyright of spidctl_regif.cpp
// Ref: {SPIDCTL_DD_Copyright_001}
// Description: Register IF generator spidctl_regif.cpp
// Ref: {SPIDCTL_DD_ATT1_001, SPIDCTL_DD_ATT1_002}
// This file is generated by Register I/F generator
//    gen_regif.py 1.9 2013/12/04 09:51:36 sontran
//    gen_regif_class.py 1.60 2014/10/24 09:42:15 ducduong
//    regif_cpp.skl 1.53 2014/10/13 03:19:16 sontran
//
// Input file : input/spidctl/spidctl_regif.txt
// Ref: {SPIDCTL_DD_ATT1_008, SPIDCTL_DD_Registers_005}
////////////////////////////////////////////////////////////////////////////////
// %MODULE SPIDCTL
//     #              name     offset_size
...
//     %%REG      BMSPID           BMSPID              8|16|32 8|16|32     32      0x0                     0               31          -               -            R|W      0x0         TRUE     -
//     %%REG      BMSPIDMSK        BMSPIDMSK           8|16|32 8|16|32     32      0x100                   0               31          -               -            R|W      0xFFFFFFFF  TRUE     -
//     %%REG      BMSPIDMSKLOCK    BMSPIDMSKLOCK       8|16|32 8|16|32     32      0x200                   0               31          -               -            R|W      0x0         TRUE     -
...
//     ////////////////////////////////////////////////////////////////////////////////
/// @file spidctl_regif.cpp
/// @brief Register IF class of model SPIDCTL
/// $Id: spidctl_regif.cpp,v 1.2 2020/02/06 04:48:44 khoaanhnguyen Exp $
/// $Date: 2020/02/06 04:48:44 $
/// $Revison$
...
// Description: Constructor of Register IF class: define registers and bits
// Ref: {SPIDCTL_DD_ATT1_004, SPIDCTL_DD_ATT1_009}
/// @return none
Cspidctl_regif::Cspidctl_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef USR_CWR_SYSTEMC
    , cwmem("register", scml_memsize(0x304/4))
    #endif
{
    CommandInit();
    SPIDKCPROT    = new vpcl::re_register(0x0300, this, "SPIDKCPROT"   , name.c_str());
    std::ostringstream str_tmp;
    for(uint i = 0; i < emNUM_BMSPID; i++) {
        str_tmp.str("");
        str_tmp<<"BMSPID"<< i;
        BMSPID       [i] = new vpcl::re_register((0x0000) + 4*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_BMSPIDMSK; i++) {
        str_tmp.str("");
        str_tmp<<"BMSPIDMSK"<< i;
        BMSPIDMSK    [i] = new vpcl::re_register((0x0100) + 4*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_BMSPIDMSKLOCK; i++) {
        str_tmp.str("");
        str_tmp<<"BMSPIDMSKLOCK"<< i;
        BMSPIDMSKLOCK[i] = new vpcl::re_register((0x0200) + 4*i, this, str_tmp.str(), name.c_str());
    }
    ...
    index = 0;
    #ifdef USR_CWR_SYSTEMC
    for(uint i = 0; i < emNUM_BMSPID       ; i++) cw_instantiate_reg(CW_BMSPID         [i], BMSPID       [i]->name(), BMSPID       [i]->addr());
    for(uint i = 0; i < emNUM_BMSPIDMSK    ; i++) cw_instantiate_reg(CW_BMSPIDMSK      [i], BMSPIDMSK    [i]->name(), BMSPIDMSK    [i]->addr());
    for(uint i = 0; i < emNUM_BMSPIDMSKLOCK; i++) cw_instantiate_reg(CW_BMSPIDMSKLOCK  [i], BMSPIDMSKLOCK[i]->name(), BMSPIDMSKLOCK[i]->addr());
    cw_instantiate_reg(CW_SPIDKCPROT     , SPIDKCPROT->name()   , SPIDKCPROT->addr()   );
    #endif
    for(uint i = 0; i < emNUM_BMSPID       ; i++) mCurReg = mRegArray[index++] = new SRegList(BMSPID         [i], mCurReg, i,  4, "8|16|32", "8|16|32");
    for(uint i = 0; i < emNUM_BMSPIDMSK    ; i++) mCurReg = mRegArray[index++] = new SRegList(BMSPIDMSK      [i], mCurReg, i,  4, "8|16|32", "8|16|32");
    for(uint i = 0; i < emNUM_BMSPIDMSKLOCK; i++) mCurReg = mRegArray[index++] = new SRegList(BMSPIDMSKLOCK  [i], mCurReg, i,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(SPIDKCPROT     , mCurReg, 0,  4,      "32", "8|16|32");

    mRegList = mCurReg;

    (*SPIDKCPROT     ) (31,  1, "KCPROT"  , 0x0       , "W"     , vpcl::SPP_ENABLE);
    (*SPIDKCPROT     ) ( 0,  0, "KCE"     , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    for(uint i = 0; i < emNUM_BMSPID; i++) {
        (*BMSPID         [i]) ( 4,  0, "SPID"    , i       , "R|W"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_BMSPIDMSKLOCK; i++) {
        (*BMSPIDMSKLOCK  [i]) ( 0,  0, "LOCK"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_BMSPIDMSK; i++) {
        (*BMSPIDMSK      [i]) (31,  0, "SPIDMSK" , 0xFFFFFFFF, "R|W"   , vpcl::SPP_ENABLE);
    }
    ...
    mWrCbAPI["SPIDKCPROT" ]["KCE"        ] = &Cspidctl_regif::cb_SPIDKCPROT_KCE;

    for(uint i = 0; i < emNUM_BMSPID; i++) {
        str_tmp.str("");
        str_tmp<<"BMSPID"<< i;
        mWrCbAPI[str_tmp.str()]["SPID"] = &Cspidctl_regif::cb_BMSPID_SPID;
    }
    for(uint i = 0; i < emNUM_BMSPIDMSK; i++) {
        str_tmp.str("");
        str_tmp<<"BMSPIDMSK"<< i;
        mWrCbAPI[str_tmp.str()]["SPIDMSK"] = &Cspidctl_regif::cb_BMSPIDMSK_SPIDMSK;
    }
    for(uint i = 0; i < emNUM_BMSPIDMSKLOCK; i++) {
        str_tmp.str("");
        str_tmp<<"BMSPIDMSKLOCK"<< i;
        mWrCbAPI[str_tmp.str()]["LOCK"] = &Cspidctl_regif::cb_BMSPIDMSKLOCK_LOCK;
    }
    InitLocalVal();
}

// Description: Destructor: delete pointers
/// @return none
Cspidctl_regif::~Cspidctl_regif()
{
...
// Description: Mask unselected bit
/// @return selected value
Cspidctl_regif::uint Cspidctl_regif::bit_select(
...
// Description: Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cspidctl_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
// Description: Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cspidctl_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
// Description: Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cspidctl_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                const unsigned char *p_data,   ///< [in] Writing data
                                cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
// Description: Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cspidctl_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                unsigned char *p_data,   ///< [out] Reading data
                                cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
// Description: Write the value to requested module register
// Ref: {SPIDCTL_DD_Direction_032, SPIDCTL_DD_Direction_022, SPIDCTL_DD_Direction_017}
/// @return true if writing transaction is finished normally
bool Cspidctl_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                    const unsigned char *p_data,  ///< [in] Writing data
                                    cuint size,                   ///< [in] Data size (byte)
...
// Description: Read the register value of requested module
// Ref: {SPIDCTL_DD_Direction_021, SPIDCTL_DD_Direction_017}
/// @return true if reading transaction is finished normally
bool Cspidctl_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                    unsigned char *p_data,   ///< [out] Reading data
                                    cuint size,              ///< [in]  Data size (byte)
                                    bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
// Description: Process reading function
// Ref: {SPIDCTL_DD_Direction_027}
/// @return true if reading successfully
bool Cspidctl_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                 unsigned char *p_data,          ///< [out] Reading data
                                 cuint size,                     ///< [in]  Data size (byte)
...
// Description: Process writing function
// Ref: {SPIDCTL_DD_Direction_031, SPIDCTL_DD_Direction_033}
/// @return true if writing successfully
bool Cspidctl_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                 const unsigned char *p_data,      ///< [in] Writing data
                                 cuint size,                       ///< [in] Data size (byte)
                                 cuint reg_index,                  ///< [in] Register index
...
// Description: Check access size
// Ref: {SPIDCTL_DD_Direction_016}
/// @@return true if detect_size equal expect_size
bool Cspidctl_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
// Description: Initialize reg_handle_command variables
/// @return none
void Cspidctl_regif::CommandInit()
{
...
// Description: Process reg_handle_command command
// Ref: {SPIDCTL_DD_Direction_019, SPIDCTL_DD_Direction_020, SPIDCTL_DD_Direction_049}
/// @return string
std::string Cspidctl_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
// Description: process reg_handle_command of "reg" parameter
/// @return string
std::string Cspidctl_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cspidctl_regif::InitLocalVal()
{
    ...
    for(uint i = 0; i < emNUM_BMSPIDMSK;    i++) {
        BMSPIDMSK_SPIDMSK[i] = 0xFFFFFFFF;
    }
    for(uint i = 0; i < emNUM_BMSPIDMSKLOCK; i++) {
        BMSPIDMSKLOCK_LOCK[i] = 0x0;
    }
    SPIDKCPROT_KCPROT = 0x0;
    SPIDKCPROT_KCE    = 0x0;
}

// Description: Update bit value to local value
/// @return None
void Cspidctl_regif::UpdateLocalVal(cuint addr)
{
    if ((addr <= (0x0000)+(4*31)) && ((addr-0x0000)%4 == 0)) {
        uint i = (addr - 0x0000)/4;
        BMSPID_SPID                      [i] = (*(BMSPID       [i]))["SPID"         ];
        return;
    }
    if ((0x0100 <= addr) && (addr <= (0x0100)+(4*31)) && ((addr-0x0100)%4 == 0)) {
        uint i = (addr - 0x0100)/4;
        BMSPIDMSK_SPIDMSK                [i] = (*(BMSPIDMSK    [i]))["SPIDMSK"      ];
        return;
    }
    if ((0x0200 <= addr) && (addr <= (0x0200)+(4*31)) && ((addr-0x0200)%4 == 0)) {
        uint i = (addr - 0x0200)/4;
        BMSPIDMSKLOCK_LOCK               [i] = (*(BMSPIDMSKLOCK[i]))["LOCK"         ];
        return;
    }
    if (addr == 0x0300) {
        SPIDKCPROT_KCPROT                 = (*SPIDKCPROT   )["KCPROT"       ];
        SPIDKCPROT_KCE                    = (*SPIDKCPROT   )["KCE"          ];
        return;
    }
}

// Description: Update local value to bit value
/// @return None
void Cspidctl_regif::UpdateRegVal(cuint addr)
{
    if ((addr <= (0x0000)+(4*31)) && ((addr-0x0000)%4 == 0)) {
        uint i = (addr - 0x0000)/4;
        (*(BMSPID       [i]))["SPID"         ] = BMSPID_SPID[i];
        return;
    }
    if ((0x0100 <= addr) && (addr <= (0x0100)+(4*31)) && ((addr-0x0100)%4 == 0)) {
        uint i = (addr - 0x0100)/4;
        (*(BMSPIDMSK    [i]))["SPIDMSK"      ] = BMSPIDMSK_SPIDMSK[i];
        return;
    }
    if ((0x0200 <= addr) && (addr <= (0x0200)+(4*31)) && ((addr-0x0200)%4 == 0)) {
        uint i = (addr - 0x0200)/4;
        (*(BMSPIDMSKLOCK[i]))["LOCK"         ] = BMSPIDMSKLOCK_LOCK[i];
        return;
    }
    if (addr == 0x0300) {
        (*SPIDKCPROT   )["KCPROT"       ] = SPIDKCPROT_KCPROT;
        (*SPIDKCPROT   )["KCE"          ] = SPIDKCPROT_KCE;
        return;
    }
}

// Description: convert string to vector
/// @return vector
std::vector<std::string> Cspidctl_regif::Str2Vec(std::string str, const char sep)
{
...
// Description: Align nuber to hexadecimal format
/// @return aligned number
std::string Cspidctl_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
// Description: Dump register access information
/// @return none
void Cspidctl_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
// Description: find register pointer based on accessed address
/// @return accessed register pointer
int Cspidctl_regif::get_reg_index(cuint access_addr)
{
...
// Description: find first register pointer
/// @return first register pointer
vpcl::re_register *Cspidctl_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}

// Description: find next register pointer
/// @return next register pointer
vpcl::re_register *Cspidctl_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
// Description: write callback function of CoWare
/// @return none
void Cspidctl_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
// Description: read callback function of CoWare
/// @return read data
Cspidctl_regif::uint Cspidctl_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
// Description: print message function
/// @return none
void Cspidctl_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
// Description: Get file name and line number
/// @return none
void Cspidctl_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
    mLineNum  = line_number;
}

// Description: API for instance registration
/// @return none
void Cspidctl_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}

// Description: Method to change value of mIsReset
// Ref: {SPIDCTL_DD_Direction_038}
/// @return none
void Cspidctl_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Cspidctl_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cspidctl_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cspidctl_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
