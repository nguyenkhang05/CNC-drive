// ---------------------------------------------------------------------
// $Id: ECC_CFLASH_E2.cpp,v 1.4 2019/10/23 08:28:53 landang Exp $
//
...
#include "ECC_def.h"
#include "ECC_CFLASH_E2.h"
#include "ECCCNT_CFP_PEn_Func.h"
#include "ECCCNT_CFS_Func.h"
#include "ECCCNT_CFC_Func.h"
#include "MECCCAP_CFL_Func.h"

/// Constructor and Destructor
ECC_CFLASH_E2::ECC_CFLASH_E2(sc_module_name name, const uint8_t iPE_NUM, const unsigned int rLatency, const unsigned int wLatency)
        :sc_module(name)
        ,ECC_CFLASH_E2_AgentController()
        ,rvc_common_model()
        ,PE_NUM(iPE_NUM)
        // Clock/Reset
        ,pclk("pclk")
        ...
        ,blankerrormaskenable("blankerrormaskenable")
        // ECCCNT CFC
        ,apedisable("apedisable")
        // MECCCAP CFL
...
{//{{{
    ...
    mpECCCNT_CFPFunc = new ECCCNT_CFP_PEn_Func*[PE_MAX_NUM];
    for (unsigned int i = 0; i < PE_MAX_NUM; i++){
        mpECCCNT_CFPFunc[i] = new ECCCNT_CFP_PEn_Func((std::string)name, i, this);
    }
    mpECCCNT_CFSFunc = new ECCCNT_CFS_Func((std::string)name, this);
    mpECCCNT_CFCFunc = new ECCCNT_CFC_Func((std::string)name, this);
    mpMECCCAP_CFLFunc = new MECCCAP_CFL_Func((std::string)name, this);
    
    // Configure Slave socket
    BusSlaveBase<32, emNumTgt>::setSlaveResetPort32(&presetn, &presetn, &presetn, &presetn, &presetn, &presetn, &presetn, 
                                                    &presetn, &presetn, &presetn, &presetn);
    BusSlaveBase<32, emNumTgt>::setSlaveFreqPort32(&pclk, &pclk, &pclk, &pclk, &pclk, &pclk, &pclk, &pclk, &pclk, &pclk, &pclk);
    BusSlaveBase<32, emNumTgt>::setTargetSocket32("ts_ecccnt_cfp_0", "ts_ecccnt_cfp_1", "ts_ecccnt_cfp_2", "ts_ecccnt_cfp_3", 
                                                  "ts_ecccnt_cfp_4", "ts_ecccnt_cfp_5", "ts_ecccnt_cfp_6", "ts_ecccnt_cfp_7", "ts_ecccnt_cfs", "ts_ecccnt_cfc", "ts_mecccap_cf");
    ...
    ts_ecccnt_cfc = BusSlaveBase<32, emNumTgt>::tSocket32[emECCCNT_CFCTgtId];
    BusSlaveBase<32, emNumTgt>::mBusSlaveIf32[emECCCNT_CFCTgtId]->setFuncModulePtr(mpECCCNT_CFCFunc);
    BusSlaveBase<32, emNumTgt>::mBusSlaveIf32[emECCCNT_CFCTgtId]->setBusProtocol(BUS_APB);
    BusSlaveBase<32, emNumTgt>::mBusSlaveIf32[emECCCNT_CFCTgtId]->setReadLatency(rLatency);
    BusSlaveBase<32, emNumTgt>::mBusSlaveIf32[emECCCNT_CFCTgtId]->setWriteLatency(wLatency);
    ...
    blankerrormaskenable.initialize(false);
    apedisable.initialize(false);
    ...
    for (unsigned int reset_id = 0; reset_id < emNumReset; reset_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_CFLASH_E2::HandleResetHardMethod, this, reset_id),
    ...
    for (unsigned int reset_id = 0; reset_id < emNumReset; reset_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_CFLASH_E2::HandleResetCmdMethod, this, reset_id),
    ...
    for (unsigned int reset_id = 0; reset_id < emNumReset; reset_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_CFLASH_E2::CancelResetCmdMethod, this, reset_id),
    ...
    for (unsigned int pe_id = 0; pe_id < PE_NUM; pe_id++) {
        ...
        opt.dont_initialize();
        sc_core::sc_spawn(sc_bind(&ECC_CFLASH_E2::WriteDisPort_CFPMethod, this, pe_id),
    ...
    SC_METHOD(WriteDisPort_CFCMethod);
    dont_initialize();
    sensitive << mWriteDisPort_CFCEvent;
...
/// Destructor
ECC_CFLASH_E2::~ECC_CFLASH_E2()
{//{{{
    for (unsigned int i = 0; i < PE_MAX_NUM; i++){
        delete mpECCCNT_CFPFunc[i];
    }
    delete mpECCCNT_CFSFunc;
    delete mpECCCNT_CFCFunc;
    delete mpMECCCAP_CFLFunc;

    for (unsigned int i = 0; i < PE_NUM; i++){
        if (eccdisable_cfp[i] != NULL){
            delete eccdisable_cfp[i];
        }
        if (secdisable_cfp[i] != NULL){
...
void ECC_CFLASH_E2::CreateEDLMap()
{//{{{
...
void ECC_CFLASH_E2::Initialize(void)
{//{{{
    unsigned int cfseccctl  = mpECCCNT_CFSFunc->GetCFSECCCTLReg();
    unsigned int cfcapctl   = mpECCCNT_CFCFunc->GetCFCAPCTLReg();
    for (unsigned int i = 0; i < PE_NUM; i++){
    ...
    mBLANKMASKENABLE        = (bool)((cfseccctl >> 24) & 0x1);// get BLANKMASKENABLE
    mECCDIS_CFS             = (bool)(cfseccctl & 0x1);// get ECCDIS_CFS
    mSECDIS_CFS             = (bool)((cfseccctl >> 1) & 0x1);// get SECDIS_CFS
    mAPEDIS_CFC             = (bool)((cfcapctl >> 2) & 0x1);// get APEDIS_CFC
...
void ECC_CFLASH_E2::WriteDisPort_CFP(unsigned int pe_id, unsigned int value)
{//{{{
...
void ECC_CFLASH_E2::WriteDisPort_CFS(unsigned int value)
{//{{{
    mECCDIS_CFS = (bool)(value & 0x1);
    mSECDIS_CFS = (bool)((value >> 1) & 0x1);
    mBLANKMASKENABLE = (bool)((value >> 24) & 0x1);
...
void ECC_CFLASH_E2::WriteDisPort_CFC(unsigned int value)
{//{{{
    mAPEDIS_CFC = (bool)((value >> 2) & 0x1);
    mWriteDisPort_CFCEvent.cancel();
    double current_time = sc_time_stamp().to_double();
    mWriteDisPort_CFCEvent.notify(CalculateCLKPosEdge((double) mClkPeriod[empclkId], false, current_time, mStartClkTime[empclkId]), mTimeResolutionUnit);
}//}}} 

/// Get CAP DEPTH
unsigned int ECC_CFLASH_E2::GetCapDepth()
{//{{{
    return mCAP_DEPTH;
}//}}}

/// Assert errovfout0_CF port
void ECC_CFLASH_E2::AssertERROVFOUT0_CF()
{//{{{
    mERROVFOUT0_CF = true;
    double current_time = sc_time_stamp().to_double();
...
void ECC_CFLASH_E2::AssertERROVFOUT1_CF()
{//{{{
    mERROVFOUT1_CF = true;
    double current_time = sc_time_stamp().to_double();
...
/// Get keycode value
bool ECC_CFLASH_E2::GetKeyOpen()
{//{{{
    return mKeyOpen;
}//}}}

/// Get AWIDTH MSB value
unsigned int ECC_CFLASH_E2::GetAWIDTH_MSB()
{//{{{
    return mAWIDTH_MSB;
}//}}}
...
//========================================================
/// Monitor pclk clock
void ECC_CFLASH_E2::MonitorpclkMethod()
{//{{{
...
void ECC_CFLASH_E2::Monitorsys_clkMethod()
{//{{{
...
void ECC_CFLASH_E2::MonitorpresetnMethod()
{//{{{
...
/// Monitor presetn port
void ECC_CFLASH_E2::Monitorsys_resetzMethod()
{//{{{
...
void ECC_CFLASH_E2::HandleResetHardMethod (const unsigned int reset_id)
{//{{{
...
void ECC_CFLASH_E2::HandleResetCmdMethod (const unsigned int reset_id)
{//{{{
...
void ECC_CFLASH_E2::CancelResetCmdMethod (const unsigned int reset_id)
{//{{{
    std::string reset_name = "presetn";
    if (reset_id == empresetnId) {
        reset_name = "presetn";
...
void ECC_CFLASH_E2::EnableReset (const std::string reset_name, const bool is_active)
{//{{{
    ...
    mpECCCNT_CFCFunc->EnableReset(reset_name, is_active);
...
void ECC_CFLASH_E2::CancelEvents()
{//{{{
    for (unsigned int i = 0; i < PE_NUM; i++){
...
/// Get reset status
bool ECC_CFLASH_E2::GetResetStatus (const std::string reset_name)
{//{{{
...
/// Check clock period 
bool ECC_CFLASH_E2::CheckClockPeriod (const std::string clock_name)
{//{{{
...
void ECC_CFLASH_E2::MonitorkeyopenMethod()
{//{{{
    mKeyOpen = keyopen.read();
}//}}}

/// Write prohibition port of CFP
void ECC_CFLASH_E2::WriteDisPort_CFPMethod(unsigned int pe_id)
{//{{{ 
...
void ECC_CFLASH_E2::WriteDisPort_CFSMethod()
{//{{{
    blankerrormaskenable.write(mBLANKMASKENABLE);
    eccdisable_cfs.write(mECCDIS_CFS);
    secdisable_cfs.write(mSECDIS_CFS);
}//}}}

/// Write apedisable port
void ECC_CFLASH_E2::WriteDisPort_CFCMethod()
{//{{{
    apedisable.write(mAPEDIS_CFC);
}//}}}

/// Write errovfout0_CF port
void ECC_CFLASH_E2::WriteERROVFOUT0_CFMethod()
{//{{{
...
void ECC_CFLASH_E2::WriteERROVFOUT1_CFMethod()
{//{{{
...
void ECC_CFLASH_E2::WriteSEDERROUT_CFMethod()
{//{{{
...
void ECC_CFLASH_E2::AssertSEDERROUT_CFMethod()
{//{{{
...
void ECC_CFLASH_E2::NegateSEDRROUT_CFMethod()
{//{{{
    mSEDERROUT_CF = false;
    mWriteSEDERROUT_CFEvent.notify();
}//}}}
...
void ECC_CFLASH_E2::NegateERROVFOUT0_CFMethod()
{//{{{
    mERROVFOUT0_CF = false;
    mWriteERROVFOUT0_CFEvent.notify();
}//}}}
...
void ECC_CFLASH_E2::NegateERROVFOUT1_CFMethod()
{//{{{
    mERROVFOUT1_CF = false;
    mWriteERROVFOUT1_CFEvent.notify();
}//}}}
...
/// Process 1bit error 
void ECC_CFLASH_E2::Set1bitErrorStatusThread()
{//{{{
    while (true){
...
void ECC_CFLASH_E2::Process1bitErrorMethod()
{//{{{
...
void ECC_CFLASH_E2::ProcessFatalErrorMethod()
{//{{{
...
void ECC_CFLASH_E2::SetFatalErrorStatusMethod()
{//{{{
    if (mHas2bitError){
...
/// Notify fatal error
void ECC_CFLASH_E2::NotifyFatalErrorMethod()
{//{{{
...
void ECC_CFLASH_E2::NegateDEDERROUT_CFMethod()
{//{{{
    mDEDERROUT_CF = false;
    mWriteDEDERROUT_CFEvent.notify();
}//}}}
...
void ECC_CFLASH_E2::NegateAPEERROUT_CFMethod()
{//{{{
    mAPEERROUT_CF = false;
    mWriteAPEERROUT_CFEvent.notify();
}//}}}
...
void ECC_CFLASH_E2::WriteDEDERROUT_CFMethod()
{//{{{
...
void ECC_CFLASH_E2::WriteAPEERROUT_CFMethod()
{//{{{
...
bool ECC_CFLASH_E2::CheckNoFatalStatus()
{//{{{
    return mpMECCCAP_CFLFunc->CheckNoFatalStatus();
}//}}}

/// Check an address is masked or not
bool ECC_CFLASH_E2::CheckMaskAddress( const unsigned int address)
{//{{{
...
bool ECC_CFLASH_E2::Check1bitAddressNotCaptured(unsigned int mod_address)
{//{{{ 
...
void ECC_CFLASH_E2::Update1bitErrorList()
{//{{{
...
void ECC_CFLASH_E2::UpdateFatalErrorList()
{//{{{
...
std::string ECC_CFLASH_E2::CombineStringUint(std::string name, unsigned int index)
{//{{{
...
unsigned int ECC_CFLASH_E2::ShiftAddress(unsigned int address)
{//{{{
...
void ECC_CFLASH_E2::SetMessageLevel (const std::string msg_lv)
{//{{{
    ...
    if (true == mEnableRegisterMessage) {
        ...
        mpECCCNT_CFCFunc->RegisterHandler(cmd);
...
void ECC_CFLASH_E2::DumpRegisterRW (const std::string is_enable)
{//{{{
    std::vector<std::string> cmd;
    cmd.push_back("reg");
...
void ECC_CFLASH_E2::DumpInterrupt (const std::string is_enable)
{//{{{
    ...
    if (word_vector.size() == 1) {
        if (word_vector[0] == "true") {
            ...
            re_printf("warning","Invalid argument: ECC_CFLASH_E2_DumpInterrupt %s\n", is_enable.c_str());
        }
        ...
        re_printf("info","ECC_CFLASH_E2_DumpInterrupt %s\n",temp.c_str());
    } else {
        re_printf("warning","Invalid argument: ECC_CFLASH_E2_DumpInterrupt %s\n", is_enable.c_str());
    }
}//}}}
...
/// Set AWIDTH_MSB
void ECC_CFLASH_E2::AWIDTH_MSB(const unsigned int value)
{//{{{
    if ((mAWIDTH_LSB < value) && (value <= MAX_ADDWIDTH_MSB)){
        mAWIDTH_MSB = value;
    }else{
        re_printf("warning", "Invalid AWIDTH_MSB (%d) of the ECC_CFLASH_E2_AWIDTH_MSB parameter. Valid range is %d-%d. Value %d is set as default.\n", value, mAWIDTH_LSB, MAX_ADDWIDTH_MSB, DEFAULT_AWIDTH_MSB);
    }
}//}}}

/// Set AWIDTH_LSB
void ECC_CFLASH_E2::AWIDTH_LSB(const unsigned int value)
{//{{{
    if ((MIN_ADDWIDTH_LSB <= (int)value) && (value < mAWIDTH_MSB)){
        mAWIDTH_LSB = value;
    }else{
        re_printf("warning", "Invalid AWIDTH_LSB (%d) of the ECC_CFLASH_E2_AWIDTH_LSB parameter. Valid range is %d-%d. Value %d is set as default.\n", value, MIN_ADDWIDTH_LSB, mAWIDTH_MSB, DEFAULT_AWIDTH_LSB);
    }
}//}}}

/// Set CAP_DEPTH
void ECC_CFLASH_E2::CAP_DEPTH(const unsigned int value)
{//{{{
    if ((MIN_CAP_DEPTH <= value) && (value <= MAX_CAP_DEPTH)){
        mCAP_DEPTH = value;
    }else{
        re_printf("warning", "Invalid CAP_DEPTH (%d) of the ECC_CFLASH_E2_CAP_DEPTH parameter. Valid range is %d-%d. Value %d is set as default.\n", value, MIN_CAP_DEPTH, MAX_CAP_DEPTH, DEFAULT_CAP_DEPTH);
    }
}//}}}

/// Set ERRINT_INIT
void ECC_CFLASH_E2::ERRINT_INIT(const unsigned int value)
{//{{{
    if ((MIN_ERRINT_INIT <= (int)value) && (value <= MAX_ERRINT_INIT)){
        mERRINT_INIT = value;
    }else{
        re_printf("warning", "Invalid ERRINT_INIT (%d) of the ECC_CFLASH_E2_ERRINT_INIT parameter. Valid range is %d-%d. Value %d is set as default.\n", value, MIN_ERRINT_INIT, MAX_ERRINT_INIT, DEFAULT_ERRINT_INIT);
    }
}//}}}

/// Issue error to ECC
void ECC_CFLASH_E2::IssueError(const unsigned int errortype)
{//{{{
    ...
    if ((errortype == em1bitError) || (errortype == em2bitError) || (errortype == emAddressParityError)){
        ...
        re_printf("warning", "Invalid error type (%d) of ECC_CFLASH_E2_IssueError command.\n", errortype);
    }
}//}}}

/// Config error address
void ECC_CFLASH_E2::SetErrorAddr(const unsigned int addr_value)
{//{{{
...
void ECC_CFLASH_E2::SetEDL (const std::string edl, const unsigned int pe_index)
{//{{{
    ...
    if (!mIsEDLConfig){
        re_printf("warning", "Invalid EDL (%s, %d) of ECC_CFLASH_E2_SetEDL command.\n", edl.c_str(), pe_index);
    }
}//}}}

/// Dump Status info
void ECC_CFLASH_E2::DumpStatusInfo()
{//{{{
    const char *type = "StatInfo";
    DumpInfo(type, "Info [%20s] (%10s):\n", sc_time_stamp().to_string().c_str(), this->name());
    DumpInfo(type, "  CF_SERSTR: 0x%08X\n", mpMECCCAP_CFLFunc->GetCF_SERSTR());
    DumpInfo(type, "  CF_DERSTR: 0x%08X\n", mpMECCCAP_CFLFunc->GetCF_DERSTR());
    DumpInfo(type, "  CF_OVFSTR: 0x%08X\n", mpMECCCAP_CFLFunc->GetCF_OVFSTR());
    for (unsigned int i = 0; i < mCAP_DEPTH; i++){
        unsigned int sedl = mpMECCCAP_CFLFunc->GetSEDLBit(i);
        unsigned int seadr = mpMECCCAP_CFLFunc->GetSEADRBit(i);
        DumpInfo(type, "  CF_%2dSEADR: SEDL = 0x%02X, SEADR = 0x%08X\n", i, sedl, seadr);
    }
    unsigned int dedl = mpMECCCAP_CFLFunc->GetDEDLBit();
    unsigned int deadr = mpMECCCAP_CFLFunc->GetDEADRBit();
    DumpInfo(type, "  CF_00DEADR: DEDL = 0x%02X, DEADR = 0x%08X\n", dedl, deadr);
}//}}}

/// Common dump info message
void ECC_CFLASH_E2::DumpInfo(const char *type, const char *message, ...)
{//{{{
    if ((message != NULL) && (type != NULL)){
        printf("PROFILE(%s): ECC_CFLASH_E2: ", type);
        // Print body
        va_list argptr;
...
void ECC_CFLASH_E2::AssertReset (const std::string reset_name, const double start_time, const double period)
{//{{{
    if (reset_name == "presetn") {
...
void ECC_CFLASH_E2::SetCLKFreq (const std::string clock_name, const sc_dt::uint64 freq, const std::string unit)
{//{{{
    std::vector<std::string> word_vector;
...
/// Get clock value
void ECC_CFLASH_E2::GetCLKFreq (const std::string clock_name)
{//{{{
    std::vector<std::string> word_vector;
    SeparateString(word_vector, clock_name);
    if (word_vector.size() == 1) {
...
void ECC_CFLASH_E2::ForceRegister (const std::string block_name, const unsigned int block_index, const std::string reg_name, const unsigned int reg_value)
{//{{{ 
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ECC_CFLASH_E2::ReleaseRegister (const std::string block_name, const unsigned int block_index, const std::string reg_name)
{//{{{
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ECC_CFLASH_E2::WriteRegister (const std::string block_name, const unsigned int block_index, const std::string reg_name, const unsigned int reg_value)
{//{{{
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ECC_CFLASH_E2::ReadRegister (const std::string block_name, const unsigned int block_index, const std::string reg_name)
{//{{{
    std::vector<std::string> word_vector;
    std::vector<std::string> cmd;
    if (reg_name != "") {
...
void ECC_CFLASH_E2::ListRegister (void)
{//{{{
    std::vector<std::string> cmd;
    cmd.push_back("reg");
    for (unsigned int i = 0; i < PE_NUM; i++){
...
void ECC_CFLASH_E2::Help (const std::string type)
{//{{{
    std::vector<std::string> word_vector;
    SeparateString(word_vector, type);
    if (word_vector.size() == 1) {
        if (word_vector[0] == "parameters") {
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_MessageLevel (\"ECC_CFLASH_E2 instance\", \"fatal|error|warning|info\")                          Set debug message level (Default: fatal|error)");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_DumpRegisterRW (\"ECC_CFLASH_E2 instance\", \"true/false\")                                      Enable/disable dumping access register (Default: false)");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_DumpInterrupt (\"ECC_CFLASH_E2 instance\", \"true/false\")                                       Enable/disable dumping interrupt info (Default: false)");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_AWIDTH_MSB (\"ECC_CFLASH_E2 instance\", value)                                                   Set address bit width MSB (Default: 25). Valid range to be set: 7-26");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_AWIDTH_LSB (\"ECC_CFLASH_E2 instance\", value)                                                   Set address bit width LSB (Default: 2). Valid range to be set: 2-4");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_CAP_DEPTH (\"ECC_CFLASH_E2 instance\", value)                                                    Set number of address registers (Default: 4). Valid range to be set: 1-32");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_ERRINT_INIT (\"ECC_CFLASH_E2 instance\", value)                                                  Set initial value of ERRINT register (Default: 0x87). Valid range to be set: 0x0-0xFF");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_EnableRegisterMessage (\"ECC_CFLASH_E2 instance\", \"true/false\")                               Enable/disable info/warning/error message of register IF (Default: true)");
        } else if (word_vector[0] == "commands") {
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_IssueError (\"ECC_CFLASH_E2 instance\", error_type)                                              Issue pseudo error to the ECC_CFLASH_E2 model");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_SetErrorAddr (\"ECC_CFLASH_E2 instance\", addr_value)                                            Set error detection address");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_SetEDL (\"ECC_CFLASH_E2 instance\", \"edl\", pe_index)                                           Set error detection location");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_DumpStatusInfo (\"ECC_CFLASH_E2 instance\")                                                      Dump information of the error status registers of the ECC_CFLASH_E2 model");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_AssertReset (\"ECC_CFLASH_E2 instance\", \"reset_name\", start_time, period)                     Assert and de-assert reset signal to the ECC_CFLASH_E2 model");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_SetCLKFreq (\"ECC_CFLASH_E2 instance\", \"clock_name\", freq, \"unit\")                          Set clock frequency value to ECC_CFLASH_E2 model. Valid clock_name is pclk or sys_clk");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_GetCLKFreq (\"ECC_CFLASH_E2 instance\", \"clock_name\")                                          Get clock frequency value of ECC_CFLASH_E2 model. Valid clock_name is pclk or sys_clk");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_ForceRegister (\"ECC_CFLASH_E2 instance\", \"block_name\", block_index, \"reg_name\", reg_value) Force register with setting value");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_ReleaseRegister (\"ECC_CFLASH_E2 instance\", \"block_name\", block_index, \"reg_name\")          Release register from force value");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_WriteRegister (\"ECC_CFLASH_E2 instance\", \"block_name\", block_index, \"reg_name\", reg_value) Write a value to a register");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_ReadRegister (\"ECC_CFLASH_E2 instance\", \"block_name\", block_index, \"reg_name\")             Read a value from a register");
            SC_REPORT_INFO(this->basename(),"ECC_CFLASH_E2_ListRegister (\"ECC_CFLASH_E2 instance\")                                                        Dump name of all registers");
        } else {
...
void ECC_CFLASH_E2::EnableRegisterMessage(const std::string is_enable)
{//{{{
    std::vector<std::string> word_vector;
    SeparateString(word_vector, is_enable);
    if (word_vector.size() == 1) {
        if (word_vector[0] == "true") {
            ...
            re_printf("warning","Invalid argument: ECC_CFLASH_E2_EnableRegisterMessage %s.\n", is_enable.c_str());
        }
    } else if (word_vector.size() == 0) {
        std::string temp = "false";
        ...
        re_printf("info","ECC_CFLASH_E2_EnableRegisterMessage %s\n", temp.c_str());
    } else {
        re_printf("warning","Invalid argument: ECC_CFLASH_E2_EnableRegisterMessage %s.\n", is_enable.c_str());
    }
}//}}}
