// -----------------------------------------------------------------------------
// $Id: atu5_timerc_regif.cpp 1196 2019-10-17 05:33:04Z chuonghoangle $
//
...
/// @return none
Catu5_timerc_regif::Catu5_timerc_regif(std::string name, uint buswidth, unsigned int subblock)
    :vpcl::reg_super()
    #ifdef USR_CWR_SYSTEMC
    , cwmem("register", scml_memsize((0x10 + subblock/2 *2)/2))
    #endif
{
    msbC = subblock;
    CommandInit();
    ...
    NCCRC = new vpcl::re_register*[msbC];
    for(uint i = 0; i < msbC; i++) {
        str_tmp.str("");
        str_tmp<<"NCCRC"<< i;
    ...
    CW_NCCRC_ = new scml2::bitfield<unsigned short> *[msbC];
    CW_NCCRC_NCCRC_ = new scml2::reg<unsigned short> *[msbC/2]
    for(uint i = 0; i < msbC  ; i++) {
        if (i/2 == 0) && ( i <= msbC - 2)
        {
            std::ostringstream inst_NCCRC_NCCRC_name;
            inst_sub_name << "NCCRC_" << i << "_NCCRC_" <<i+1 ;
            cw_instantiate_reg(CW_NCCRC_NCCRC_[i/2],inst_NCCRC_NCCRC_name.str().c_str() , 0x0008 + i * 0x2);
            CW_NCCRC  [i] = new scml2::bitfield<unsigned short>(NCCRC  [i]->name(), CW_NCCRC_NCCRC_[i/2],  0,  8);
            CW_NCCRC  [i+1] = new scml2::bitfield<unsigned short>(NCCRC  [i+1]->name(), CW_NCCRC_NCCRC_[i/2],  8,  8);
        }
        if  (i/2 == 0) && ( i == msbC - 1)
        {
            cw_instantiate_reg(CW_NCCRC_[i], NCCRC  [i]->name(), NCCRC  [i]->addr());
            CW_NCCRC  [i] = new scml2::bitfield<unsigned short>(NCCRC->name()  , CW_NCCRC_  [i],  0,  8);        
        }      
    }
    ...
    for(uint i = 0; i < msbC  ; i++) mCurReg = mRegArray[index++] = new SRegList(NCCRC    [i], mCurReg, i,  1,       "8", "8|16|32");

    mRegList = mCurReg;

    (*TSTRC    ) (msbC - 1,  0, "STRC"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*NCMCR1C  ) (msbC - 1,  0, "NCM1C"   , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*NCMCR2C  ) (msbC - 1,  0, "NCM2C"   , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    for(uint i = 0; i < msbC; i++) {
    ...
    mWrCbAPI["TSTRC"]["STRC" ] = &Catu5_timerc_regif::cb_TSTRC_STRC;
    mWrCbAPI["NCMCR1C"]["NCM1C"] = &Catu5_timerc_regif::cb_NCMCR1C_NCM1C;
    mWrCbAPI["NCMCR2C"]["NCM2C"] = &Catu5_timerc_regif::cb_NCMCR2C_NCM2C;

    for(uint i = 0; i < msbC; i++) {
        str_tmp.str("");
        str_tmp<<"NCCRC"<< i;
        mWrCbAPI[str_tmp.str()]["NCEC3"] = &Catu5_timerc_regif::cb_NCCRC_NCEC3;
    }
    InitLocalVal();
}
...
/// @return none
Catu5_timerc_regif::~Catu5_timerc_regif()
{
...
Catu5_timerc_regif::uint Catu5_timerc_regif::bit_select(
...
bool Catu5_timerc_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Catu5_timerc_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Catu5_timerc_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                    const unsigned char *p_data,   ///< [in] Writing data
                                    cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Catu5_timerc_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                    unsigned char *p_data,   ///< [out] Reading data
                                    cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Catu5_timerc_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                        const unsigned char *p_data,  ///< [in] Writing data
                                        cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Catu5_timerc_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                        unsigned char *p_data,   ///< [out] Reading data
                                        cuint size,              ///< [in]  Data size (byte)
                                        bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Catu5_timerc_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                     unsigned char *p_data,          ///< [out] Reading data
                                     cuint size,                     ///< [in]  Data size (byte)
...
bool Catu5_timerc_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                     const unsigned char *p_data,      ///< [in] Writing data
                                     cuint size,                       ///< [in] Data size (byte)
                                     cuint reg_index,                  ///< [in] Register index
...
bool Catu5_timerc_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Catu5_timerc_regif::CommandInit()
{
...
/// @return string
std::string Catu5_timerc_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Catu5_timerc_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Catu5_timerc_regif::InitLocalVal()
{
    // Initialize local variables
    TSTRC_STRC = 0x0;
    NCMCR1C_NCM1C = 0x0;
    ...
    NCCRC_NCKC3 = new uint[msbC];
    NCCRC_NCKC2 = new uint[msbC];
    NCCRC_NCKC1 = new uint[msbC];
    NCCRC_NCKC0 = new uint[msbC];
    NCCRC_NCEC3 = new uint[msbC];
    NCCRC_NCEC2 = new uint[msbC];
    NCCRC_NCEC1 = new uint[msbC];
    NCCRC_NCEC0 = new uint[msbC];
    for(uint i = 0; i < msbC;  i++) {
        NCCRC_NCKC3[i] = 0x0;
        NCCRC_NCKC2[i] = 0x0;
        NCCRC_NCKC1[i] = 0x0;
...
/// @return None
void Catu5_timerc_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
    ...
    if ((0x0008 <= addr) && (addr <= (0x0008)+(1*(msbC-1))) && ((addr-0x0008)%1 == 0)) {
...
/// @return None
void Catu5_timerc_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
...
/// @return vector
std::vector<std::string> Catu5_timerc_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Catu5_timerc_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Catu5_timerc_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Catu5_timerc_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Catu5_timerc_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Catu5_timerc_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Catu5_timerc_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Catu5_timerc_regif::uint Catu5_timerc_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return none
void Catu5_timerc_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Catu5_timerc_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Catu5_timerc_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Catu5_timerc_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
unsigned short Catu5_timerc_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
unsigned short Catu5_timerc_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(unsigned short));
    return sizeof(unsigned short);
}
void Catu5_timerc_regif::cw_instantiate_reg(scml2::reg<unsigned short> * reg, std::string reg_name, int offset)
{
