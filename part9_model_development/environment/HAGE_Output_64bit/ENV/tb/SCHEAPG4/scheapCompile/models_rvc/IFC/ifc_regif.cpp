...
/// @return none
Cifc_regif::Cifc_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
    , cwmem("register", 0x1C0)
    #endif
{
    CommandInit();
    std::ostringstream str_tmp;
    for(uint i = 0; i < emNUM_DR0; i++) {
        str_tmp.str("");
        str_tmp<<"DR0"<< i;
        DR0[i] = new vpcl::re_register((0x0000) + 4*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_DR1; i++) {
        str_tmp.str("");
        str_tmp<<"DR1"<< i;
        DR1[i] = new vpcl::re_register((0x0100) + 4*i, this, str_tmp.str(), name.c_str());
    }
    ...
    index = 0;
#ifdef CWR_SYSTEMC
    for(uint i = 0; i < emNUM_DR0; i++) mCurReg = mRegArray[index++] = new SRegList(DR0  [i], mCurReg, i,  4,      "32",      "32", false, &cwmem);
#else
    for(uint i = 0; i < emNUM_DR0; i++) mCurReg = mRegArray[index++] = new SRegList(DR0  [i], mCurReg, i,  4,      "32",      "32");
#endif
    for(uint i = 0; i < emNUM_DR1; i++) mCurReg = mRegArray[index++] = new SRegList(DR1  [i], mCurReg, i,  4,      "32",      "32");

    mRegList = mCurReg;

    for(uint i = 0; i < emNUM_DR1; i++) {
        (*DR1  [i]) (31, 31, "S"       , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*DR1  [i]) (30, 23, "EXP"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*DR1  [i]) (22,  0, "FRACTION", 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_DR0; i++) {
        (*DR0  [i]) (31, 31, "S"       , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*DR0  [i]) (30, 23, "EXP"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*DR0  [i]) (22,  0, "FRACTION", 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
...
Cifc_regif::uint Cifc_regif::bit_select(cuint val,    ///< [in] Writting address
...
bool Cifc_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cifc_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cifc_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                            const unsigned char *p_data,   ///< [in] Writing data
                            cuint size)                    ///< [in] Data size (byte)
{
    if (size == 0) {
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cifc_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                            unsigned char *p_data,   ///< [out] Reading data
                            cuint size)              ///< [in]  Data size (byte)
{
    if (size == 0) {
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cifc_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                const unsigned char *p_data,  ///< [in] Writing data
                                cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cifc_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                unsigned char *p_data,   ///< [out] Reading data
                                cuint size,              ///< [in]  Data size (byte)
                                bool IsDbgFunc)          ///< [in]  Flag indicate reg_rd or reg_rd_dbg
{
...
bool Cifc_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                             unsigned char *p_data,          ///< [out] Reading data
                             cuint size,                     ///< [in]  Data size (byte)
...
bool Cifc_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                             const unsigned char *p_data,      ///< [in] Writing data
                             cuint size,                       ///< [in] Data size (byte)
                             cuint reg_index,                  ///< [in] Register index
...
bool Cifc_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr, bool is_wr)
{
...
/// @return string
std::string Cifc_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cifc_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return None
void Cifc_regif::UpdateLocalVal(cuint addr)
{
    if ((0x0000 <= addr) && (addr <= (0x0000)+(4*47)) && ((addr-0x0000)%4 == 0)) {
        uint i = (addr - 0x0000)/4;
        DR0_S                  [i] = (*(DR0[i]))["S"  ];
        DR0_EXP                [i] = (*(DR0[i]))["EXP"];
        DR0_FRACTION           [i] = (*(DR0[i]))["FRACTION"];
        return;
    }
    if ((0x0100 <= addr) && (addr <= (0x0100)+(4*47)) && ((addr-0x0100)%4 == 0)) {
        uint i = (addr - 0x0100)/4;
        DR1_S                  [i] = (*(DR1[i]))["S"  ];
        DR1_EXP                [i] = (*(DR1[i]))["EXP"];
        DR1_FRACTION           [i] = (*(DR1[i]))["FRACTION"];
        return;
    }
}
...
/// @return None
void Cifc_regif::UpdateRegVal(cuint addr)
{
    if ((0x0000 <= addr) && (addr <= (0x0000)+(4*47)) && ((addr-0x0000)%4 == 0)) {
        uint i = (addr - 0x0000)/4;
        (*(DR0[i]))["S"  ] = DR0_S[i];
        (*(DR0[i]))["EXP"] = DR0_EXP[i];
        (*(DR0[i]))["FRACTION"] = DR0_FRACTION[i];
        return;
    }
    if ((0x0100 <= addr) && (addr <= (0x0100)+(4*47)) && ((addr-0x0100)%4 == 0)) {
        uint i = (addr - 0x0100)/4;
...
/// @return vector
std::vector<std::string> Cifc_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cifc_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip, bool is_wr_data, uint acc_size)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cifc_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
    if (mDumpRegisterRW) {
...
int Cifc_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cifc_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cifc_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cifc_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cifc_regif::uint Cifc_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
    uint data=0;
...
/// @return none
void Cifc_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cifc_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cifc_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cifc_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cifc_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cifc_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    int reg_index = get_reg_index((uint)tag);
    assert(reg_index != (1<<16));
    uint length = mRegArray[reg_index]->length;
...
void Cifc_regif::cw_set_all_callback_reg(void)
{
