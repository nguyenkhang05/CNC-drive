// -----------------------------------------------------------------------------
// $Id: ecc_apb_regif.cpp,v 1.3 2018/01/08 09:21:31 chanle Exp $
//
...
/// @return none
Cecc_apb_regif::Cecc_apb_regif(std::string name, std::string kind, uint buswidth)// revised by chanle 20170103
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
    , cwmem("register", scml_memsize(0x54/4))
    #endif
{
    mKind   = kind; // added by chanle 20170103

    CommandInit();

    // --- revised by chanle 20170103
    if ((mKind == "CF_A") || (mKind == "IFU_D") || (mKind == "LSUS_DA") || (mKind == "LSUM_D") || (mKind == "PDMA_D") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
        ECCCTL  = new vpcl::re_register(0x0000, this, "ECCCTL" , name.c_str());
    }
    //--------------
    ERRINT  = new vpcl::re_register(0x0004, this, "ERRINT" , name.c_str());
    ERSTCLR = new vpcl::re_register(0x0008, this, "ERSTCLR", name.c_str());
    OVFSTR  = new vpcl::re_register(0x000C, this, "OVFSTR" , name.c_str());
    ERSTR   = new vpcl::re_register(0x0010, this, "ERSTR"  , name.c_str());
    EADR    = new vpcl::re_register(0x0050, this, "EADR"   , name.c_str());
    ...
    cw_instantiate_reg(CW_ERSTCLR  , ERSTCLR->name(), ERSTCLR->addr());
    cw_instantiate_reg(CW_OVFSTR   , OVFSTR->name() , OVFSTR->addr() );
    cw_instantiate_reg(CW_ERSTR    , ERSTR->name()  , ERSTR->addr()  );
    cw_instantiate_reg(CW_EADR     , EADR->name()   , EADR->addr()   );
    #endif
    // ---- revised by chanle 20170103
    if ((mKind == "CF_A") || (mKind == "IFU_D") || (mKind == "LSUS_DA") || (mKind == "LSUM_D") || (mKind == "PDMA_D") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
    ...
    mCurReg = mRegArray[index++] = new SRegList(ERSTCLR  , mCurReg, 0,  4, "8|16|32", "8|16|32");
    ...
    mCurReg = mRegArray[index++] = new SRegList(EADR     , mCurReg, 0,  4,      "32", "8|16|32");

    mRegList = mCurReg;

    // ---- revised by chanle 20170103
    if ((mKind == "CF_A") || (mKind == "IFU_D") || (mKind == "LSUS_DA") || (mKind == "LSUM_D") || (mKind == "PDMA_D") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
        (*ECCCTL   ) (15, 14, "PROT"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        if ((mKind == "CF_A") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) { // "LSUS_DA" is specical case, both address and data in SECDIS/ECCDIS
            (*ECCCTL   ) ( 3,  3, "ASECDIS" , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
            (*ECCCTL   ) ( 2,  2, "AECCDIS" , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        }
        if ((mKind == "IFU_D") || (mKind == "LSUS_DA") || (mKind == "LSUM_D") || (mKind == "PDMA_D") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
            (*ECCCTL   ) ( 1,  1, "SECDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
            (*ECCCTL   ) ( 0,  0, "ECCDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        }
    }
    if ((mKind == "CF_A") || (mKind == "LSUS_DA") || (mKind == "SG7_DA") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
        (*ERRINT   ) ( 5,  5, "ADEDIE"  , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
        (*ERRINT   ) ( 4,  4, "ASEDIE"  , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
    }
    if ((mKind == "IFU_D") || (mKind == "LSUS_DA") || (mKind == "LSUM_D") || (mKind == "SGn_D") || (mKind == "SG7_DA") || (mKind == "PDMA_D") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
        (*ERRINT   ) ( 1,  1, "DEDIE"   , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
        (*ERRINT   ) ( 0,  0, "SEDIE"   , 0x1       , "R|W"   , vpcl::SPP_ENABLE);
    }
    (*ERSTCLR  ) ( 0,  0, "SSTCLR"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*OVFSTR   ) ( 0,  0, "ERROVF"  , 0x0       , "R"     , vpcl::SPP_ENABLE);
    if ((mKind == "CF_A") || (mKind == "LSUS_DA") || (mKind == "SG7_DA") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
        (*ERSTR    ) ( 7,  7, "ADEDF"   , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*ERSTR    ) ( 6,  6, "ASEDF"   , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    if ((mKind == "IFU_D") || (mKind == "LSUS_DA") || (mKind == "LSUM_D") || (mKind == "SGn_D") || (mKind == "SG7_DA") || (mKind == "PDMA_D") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
        (*ERSTR    ) ( 1,  1, "DEDF"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*ERSTR    ) ( 0,  0, "SEDF"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    //--------------
    (*EADR     ) (31,  0, "EADR"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["ECCCTL"]["PROT" ] = &Cecc_apb_regif::cb_ECCCTL_PROT;
    mWrCbAPI["ERSTCLR"]["SSTCLR"] = &Cecc_apb_regif::cb_ERSTCLR_SSTCLR;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cecc_apb_regif::~Cecc_apb_regif()
{
...
Cecc_apb_regif::uint Cecc_apb_regif::bit_select(
...
bool Cecc_apb_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cecc_apb_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecc_apb_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                const unsigned char *p_data,   ///< [in] Writing data
                                cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecc_apb_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                unsigned char *p_data,   ///< [out] Reading data
                                cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecc_apb_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                    const unsigned char *p_data,  ///< [in] Writing data
                                    cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecc_apb_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                    unsigned char *p_data,   ///< [out] Reading data
                                    cuint size,              ///< [in]  Data size (byte)
                                    bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cecc_apb_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                 unsigned char *p_data,          ///< [out] Reading data
                                 cuint size,                     ///< [in]  Data size (byte)
...
bool Cecc_apb_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                 const unsigned char *p_data,      ///< [in] Writing data
                                 cuint size,                       ///< [in] Data size (byte)
                                 cuint reg_index,                  ///< [in] Register index
...
bool Cecc_apb_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cecc_apb_regif::CommandInit()
{
...
/// @return string
std::string Cecc_apb_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cecc_apb_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cecc_apb_regif::InitLocalVal()
{
    // Initialize local variables
    // ---- revised by chanle 20170103
    if ((mKind == "CF_A") || (mKind == "IFU_D") || (mKind == "LSUS_DA") || (mKind == "LSUM_D") || (mKind == "PDMA_D") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
...
/// @return None
void Cecc_apb_regif::UpdateLocalVal(cuint addr)
{
    // ---- revised by chanle 20170103
    if (addr == 0x0000) {
        if ((mKind == "CF_A") || (mKind == "IFU_D") || (mKind == "LSUS_DA") || (mKind == "LSUM_D") || (mKind == "PDMA_D") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
            ECCCTL_PROT                 = (*ECCCTL )["PROT"   ];
            if ((mKind == "CF_A") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) { // "LSUS_DA" is specical case, both address and data in SECDIS/ECCDIS
    ...
    if (addr == 0x0008) {
        ERSTCLR_SSTCLR              = (*ERSTCLR)["SSTCLR" ];
        return;
    }
    if (addr == 0x000C) {
        OVFSTR_ERROVF               = (*OVFSTR )["ERROVF" ];
        return;
    }
    if (addr == 0x0010) {
        if ((mKind == "CF_A") || (mKind == "LSUS_DA") || (mKind == "SG7_DA") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
            ERSTR_ADEDF                 = (*ERSTR  )["ADEDF"  ];
            ERSTR_ASEDF                 = (*ERSTR  )["ASEDF"  ];
        }
        if ((mKind == "IFU_D") || (mKind == "LSUS_DA") || (mKind == "LSUM_D") || (mKind == "SGn_D") || (mKind == "SG7_DA") || (mKind == "PDMA_D") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
            ERSTR_DEDF                  = (*ERSTR  )["DEDF"   ];
            ERSTR_SEDF                  = (*ERSTR  )["SEDF"   ];
        }
        return;
    }
    //--------------
...
/// @return None
void Cecc_apb_regif::UpdateRegVal(cuint addr)
{
    // ---- revised by chanle 20170103
    if (addr == 0x0000) {
        if ((mKind == "CF_A") || (mKind == "IFU_D") || (mKind == "LSUS_DA") || (mKind == "LSUM_D") || (mKind == "PDMA_D") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
            (*ECCCTL )["PROT"   ] = ECCCTL_PROT;
            if ((mKind == "CF_A") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) { // "LSUS_DA" is specical case, both address and data in SECDIS/ECCDIS
    ...
    if (addr == 0x0008) {
        (*ERSTCLR)["SSTCLR" ] = ERSTCLR_SSTCLR;
        return;
    }
    if (addr == 0x000C) {
        (*OVFSTR )["ERROVF" ] = OVFSTR_ERROVF;
        return;
    }
    if (addr == 0x0010) {
        if ((mKind == "CF_A") || (mKind == "LSUS_DA") || (mKind == "SG7_DA") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
            (*ERSTR  )["ADEDF"  ] = ERSTR_ADEDF;
            (*ERSTR  )["ASEDF"  ] = ERSTR_ASEDF;
        }
        if ((mKind == "IFU_D") || (mKind == "LSUS_DA") || (mKind == "LSUM_D") || (mKind == "SGn_D") || (mKind == "SG7_DA") || (mKind == "PDMA_D") || (mKind == "APEC0_DA") || (mKind == "APECn_DA")) {
            (*ERSTR  )["DEDF"   ] = ERSTR_DEDF;
            (*ERSTR  )["SEDF"   ] = ERSTR_SEDF;
        }
        return;
    }
    //--------------
...
/// @return vector
std::vector<std::string> Cecc_apb_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cecc_apb_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cecc_apb_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cecc_apb_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cecc_apb_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cecc_apb_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cecc_apb_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cecc_apb_regif::uint Cecc_apb_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cecc_apb_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cecc_apb_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cecc_apb_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cecc_apb_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cecc_apb_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cecc_apb_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cecc_apb_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
