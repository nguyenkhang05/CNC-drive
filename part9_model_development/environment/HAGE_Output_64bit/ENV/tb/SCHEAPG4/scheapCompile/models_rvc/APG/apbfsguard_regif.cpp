// -----------------------------------------------------------------------------
// $Id: apbfsguard_regif.cpp,v 1.14 2014/11/03 12:53:30 dungvannguyen Exp $
//
...
////////////////////////////////////////////////////////////////////////////////
/// @file apbfsguard_regif.cpp
/// @brief Register IF class of model APBFSGUARD
/// $Id: apbfsguard_regif.cpp,v 1.14 2014/11/03 12:53:30 dungvannguyen Exp $
/// $Date: 2014/11/03 12:53:30 $
/// $Revison$
...
/// @return none
Capbfsguard_regif::Capbfsguard_regif(std::string name, uint buswidth, uint version, uint filter_number)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
    ...
    #endif
{
    ...
    for(uint i = 0; i < mFilterNumber; i++) {
        str_tmp.str("");
        str_tmp<<"PROT"<< i;
        mWrCbAPI[str_tmp.str()]["LOCK"] = &Capbfsguard_regif::cb_PROT_LOCK;
    }
    InitLocalVal();
}
...
/// @return none
Capbfsguard_regif::~Capbfsguard_regif()
{
...
Capbfsguard_regif::uint Capbfsguard_regif::bit_select(
...
bool Capbfsguard_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Capbfsguard_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Capbfsguard_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                   const unsigned char *p_data,   ///< [in] Writing data
                                   cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Capbfsguard_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                   unsigned char *p_data,   ///< [out] Reading data
                                   cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Capbfsguard_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                       const unsigned char *p_data,  ///< [in] Writing data
                                       cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Capbfsguard_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                       unsigned char *p_data,   ///< [out] Reading data
                                       cuint size,              ///< [in]  Data size (byte)
                                       bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Capbfsguard_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                    unsigned char *p_data,          ///< [out] Reading data
                                    cuint size,                     ///< [in]  Data size (byte)
...
bool Capbfsguard_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                    const unsigned char *p_data,      ///< [in] Writing data
                                    cuint size,                       ///< [in] Data size (byte)
                                    cuint reg_index,                  ///< [in] Register index
...
bool Capbfsguard_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Capbfsguard_regif::CommandInit()
{
...
/// @return string
std::string Capbfsguard_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Capbfsguard_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Capbfsguard_regif::InitLocalVal()
{
    // Initialize local variables
    for(uint i = 0; i < mFilterNumber; i++) {
        PROT_LOCK[i] = 0x0;
...
/// @return None
void Capbfsguard_regif::UpdateLocalVal(cuint addr)
{
...
/// @return None
void Capbfsguard_regif::UpdateRegVal(cuint addr)
{
    if (mVersion == emCC_CUBE) {
        if ((0x0000 <= addr) && (addr <= (0x0000)+(8*15)) && ((addr-0x0000)%8 == 0)) {
            uint i = (addr - 0x0000)/8;
...
/// @return vector
std::vector<std::string> Capbfsguard_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Capbfsguard_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Capbfsguard_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Capbfsguard_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Capbfsguard_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Capbfsguard_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Capbfsguard_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Capbfsguard_regif::uint Capbfsguard_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Capbfsguard_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Capbfsguard_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Capbfsguard_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Capbfsguard_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Capbfsguard_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Capbfsguard_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Capbfsguard_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
