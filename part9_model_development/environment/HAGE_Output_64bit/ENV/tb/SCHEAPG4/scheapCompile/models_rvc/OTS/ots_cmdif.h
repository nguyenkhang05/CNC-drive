...
std::string own_handle_command(std::vector<std::string>& args)
{
  ...
  if (args[0] == "help") {
    ...
    ret += "    AccelerationRate <rate>                   rate of conversion time ( Default:1  )\n";
    ret += "    MaxTemperature <max_temp>                 maximum temperature setting in OTDR ( Default:0x3BD8  )\n";
    ret += "    MinTemperature <min_temp>                 minimum temperature setting in OTDR ( Default:0x200F  )\n";
    ret += "    DumpStatInfo                              Dump the statistical information about OTS model activity\n";
    ret += "    SetTemp <temperature>                     Set current temperature for OTS model\n";
...
std::string user_def_command(std::vector<std::string>& args)
{
  ...
  else if (args[0] == "AccelerationRate") {
    // read mode
    if (0) {;}
    else if ((int)args.size() == 1) {
      std::ostringstream stream;
      stream << AccelerationRate;
      ret = stream.str();
    }
    // write mode
    else if ((int)args.size() == 2) {
  ...
  else if (args[0] == "MinTemperature") {
    // read mode
    if (0) {;}
    else if ((int)args.size() == 1) {
      std::ostringstream stream;
      stream << MinTemperature;
      ret = stream.str();
    }
    // write mode
    else if ((int)args.size() == 2) {
...
void CommandInit(std::string name = "") {
  ...
  DumpInterrupt = false;
  AccelerationRate = 1;
  MaxTemperature = 0x3BD8;
  MinTemperature = 0x200F;
