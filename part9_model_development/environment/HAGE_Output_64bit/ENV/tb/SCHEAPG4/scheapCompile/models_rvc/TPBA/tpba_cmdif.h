...
std::string own_handle_command(std::vector<std::string>& args)
{
  ...
  if (args[0] == "help") {
    ...
    ret += "    EnableTransInfo <arg>                     Enable/disable information display when CNT0 match CB0 or CB2 ( Default:false  )\n";
    ret += "    DumpStatInfo                              Dumps status information about TPBA\n";
    ret += "    UpdateRegisters                           copy the latest counter value of general timer to counter registers of tpba\n";
...
std::string user_def_command(std::vector<std::string>& args)
{
  ...
  else if (args[0] == "UpdateRegisters") {
    if ((int)args.size() > 0 && (int)args.size() <= 1) {
      UpdateRegisters();
    }
    else {
