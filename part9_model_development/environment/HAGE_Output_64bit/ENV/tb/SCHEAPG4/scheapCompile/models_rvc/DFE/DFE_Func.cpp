// ---------------------------------------------------------------------
// $Id: DFE_Func.cpp,v 1.3 2018/09/06 10:18:20 huepham Exp $
//
...
//========================================================
Cdfe_func::Cdfe_func (std::string name, unsigned int num_channel, Cdfe_agent_controller* DFEAgentController):
                Cdfe_regif(name, 32, num_channel)
                ,BusSlaveFuncIf()
{//{{{
    assert(DFEAgentController != NULL);
    mDFEAgentController = DFEAgentController;
    mDFEAgentController->CommandInit(name);
    Cdfe_regif::set_instance_name(name);
    mNumChannel = num_channel;
    mModuleName = name;
    // Initialize
    Initialize();
    memset(mDMEM, 0, 0x600);
    memset(mCMEM, 0, 0x300);
}//}}}
...
void Cdfe_func::Initialize(void)
{//{{{
    mCurErrStatus = false;
    mPreErrStatus = false;
...
void Cdfe_func::EnableReset(const bool is_active)
{//{{{
    if (is_active){
        Initialize();
    }
    Cdfe_regif::EnableReset(is_active);
}//}}}

void Cdfe_func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    ...
    if ((0 <= addr) && (addr < emStartOffsetCMEM)){
        // Check non-existed
        if ((((4*mNumChannel) <= addr) && (addr < 0x30)) // DFEjCTLACH
        || (((0x40+4*mNumChannel) <= addr) && (addr < 0x70)) // DFEjCTLBCH
        || (((0x80+4*mNumChannel) <= addr) && (addr < 0xB0)) // DFEjDOCH
        || (((0xC0+4*mNumChannel) <= addr) && (addr < 0xF0)) // DFEjPHCH
        || (((0x100+4*mNumChannel) <= addr) && (addr < 0x130)) // DFEjPHINDCH
        || (((0x140+4*mNumChannel) <= addr) && (addr < 0x170)) // DFEjSTCH
        || (((0x180+4*mNumChannel) <= addr) && (addr < 0x1B0)) // DFEjCLRSTCH
        || (((0x1C0+4*mNumChannel) <= addr) && (addr < 0x1F0)) // DFEjERMCH
        || (((0x200+4*mNumChannel) <= addr) && (addr < 0x230)) // DFEjTRGCH
        || (((0x240+4*mNumChannel) <= addr) && (addr < 0x270)) // DFEjTRHCH
        || (((0x320+4*mNumChannel) <= addr) && (addr < 0x350)) // DFEjHOHCH and DFEjHOLCH
        || (((0x360+2*mNumChannel) <= addr) && (addr < 0x378)) // DFEjHOLMCH
        || (((0x380+4*mNumChannel) <= addr) && (addr < 0x3B0)) // DFEjGAINCH
        || (((0x3C0+4*mNumChannel) <= addr) && (addr < 0x3F0))){ // DFEjTMTRGCH
...
void Cdfe_func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    unsigned int full_addr = (unsigned int)trans.get_address();
    unsigned int addr = (unsigned int)trans.get_address() & 0x3FFF;
    unsigned int size = (unsigned int)trans.get_data_length();
...
void Cdfe_func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Cdfe_regif::reg_handle_command(cmd);
                if (msg_return != "") {
            ...
            msg_return = Cdfe_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
bool Cdfe_func::CheckValidAccessMem(eMEMInfo mem_type, unsigned int addr, unsigned int size, bool is_wr, bool debug)
{//{{{
    bool ret = false;
...
//============ Dispatch circuit ==========================
//========================================================
unsigned int Cdfe_func::GetENBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjCTLACHn[channel])["EN"];
}//}}}

void Cdfe_func::SetTRHCHReg(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjTRHCHn[channel]) = value;
}//}}}

void Cdfe_func::SetFITSBit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjTRHCHn[channel])["FITS"] = value;
}//}}}

void Cdfe_func::SetMETSBit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjTRHCHn[channel])["METS"] = value;
}//}}}

void Cdfe_func::SetMITSBit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjTRHCHn[channel])["MITS"] = value;
}//}}}

void Cdfe_func::SetPETSBit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjTRHCHn[channel])["PETS"] = value;
}//}}}

void Cdfe_func::SetPITSBit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjTRHCHn[channel])["PITS"] = value;
}//}}}

void Cdfe_func::SetPMETSBit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjTRHCHn[channel])["PMETS"] = value;
}//}}}

void Cdfe_func::SetPMITSBit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjTRHCHn[channel])["PMITS"] = value;
}//}}}

void Cdfe_func::SetVALIDBit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjSTCHn[channel])["VALID"] = value;
    UpdateDFEjST();
}//}}}

unsigned int Cdfe_func::GetVALIDBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjSTCHn[channel])["VALID"];
}//}}}

void Cdfe_func::SetDIOWBit(unsigned int channel, unsigned int value)
{//{{{
...
void Cdfe_func::SetDOOWBit(unsigned int channel, unsigned int value)
{//{{{
...
unsigned int Cdfe_func::GetSWInputTag()
{//{{{
    return (unsigned int)(*DFEjDI)["TAG"];
}//}}}

unsigned int Cdfe_func::GetSWInputData()
{//{{{
    return (unsigned int)(*DFEjDI)["DI"];
}//}}}

bool Cdfe_func::CheckMatchTag(unsigned int in_tag, unsigned int channel_id)
{//{{{
...
unsigned short Cdfe_func::GetCoefficient(unsigned int channel_id, unsigned int index)
{//{{{
...
bool Cdfe_func::SetCoeffMem(unsigned int offset, unsigned int value)
{//{{{
    bool ret = false;
...
bool Cdfe_func::GetCoeffMem(unsigned int offset, unsigned int& data)
{//{{{
    bool ret = false;
    if ((offset + 4) <= (mNumChannel*0x40)){
...
bool Cdfe_func::SetDataMem(unsigned int offset, unsigned int value)
{//{{{
    bool ret = false;
...
bool Cdfe_func::GetDataMem(unsigned int offset, unsigned int& data)
{//{{{ 
    bool ret = false;
    if ((offset + 4) <= (mNumChannel*0x80)){
...
unsigned short Cdfe_func::GetFIRBuffer(unsigned int channel_id, unsigned int num_taps)
{//{{{
    unsigned short ret = 0;
...
unsigned int Cdfe_func::GetIIRCirBuffer(unsigned int channel_id, unsigned int index)
{//{{{
    unsigned int ret = 0;
...
void Cdfe_func::CopyInitBufToCirBuf(unsigned int channel)
{//{{{
...
unsigned int Cdfe_func::GetGAIN2Bit(unsigned int channel_id)
{//{{{
    return (unsigned int)(*DFEjGAINCHn[channel_id])["GAIN2"];
}//}}}

unsigned int Cdfe_func::GetGAIN3Bit(unsigned int channel_id)
{//{{{
    return (unsigned int)(*DFEjGAINCHn[channel_id])["GAIN3"];
}//}}}

// Shift IIR buffer (circular) for 1 stage
void Cdfe_func::UpdateIIRBufferOneStage(unsigned int channel_id, unsigned int input)
{//{{{
...
void Cdfe_func::UpdateIIRBufferTwoStage(unsigned int channel, unsigned int input1, unsigned int input2)
{//{{{
    // Get w11, w21
    unsigned int w11 = 0;
    unsigned int w21 = 0;
...
void Cdfe_func::UpdateIIRBufferThreeStage(unsigned int channel, unsigned int input1, unsigned int input2, unsigned int input3)
{//{{{
    // Get w11, w21
    unsigned int w11 = 0;
    unsigned int w21 = 0;
...
bool Cdfe_func::IsFIRFixedPoint(unsigned int channel_id)
{//{{{
    return ((unsigned int)(*DFEjCTLACHn[channel_id])["FMT"] == 0);
}//}}}

void Cdfe_func::UpdateFIRBuffer(unsigned int channel_id, unsigned short xn, unsigned int num_taps)
{//{{{
    // Store data to next data pointer
...
bool Cdfe_func::IsIntermediateConvertFloat(unsigned int channel_id)
{//{{{
...
bool Cdfe_func::IsFIRSelect(unsigned int channel)
{//{{{
...
bool Cdfe_func::IsIIRSelect(unsigned int channel)
{//{{{
    unsigned int cmd = (unsigned int)(*DFEjCTLACHn[channel])["CMD"];
    if (0x8 <= cmd){
        return true;
...
unsigned int Cdfe_func::GetFEBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjTRGCHn[channel])["FE"];
}//}}}

unsigned int Cdfe_func::GetFITMTRGBit(unsigned int trigger)
{//{{{
    unsigned int ret = 0;
    if (trigger == 0){
...
int Cdfe_func::GetFTBit(unsigned int channel)
{//{ {{
    int ret = 0;
...
unsigned int Cdfe_func::GetCMDBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjCTLACHn[channel])["CMD"];
}//}}}

unsigned int Cdfe_func::GetFMTBit(unsigned int channel)
{//{{{
...
//============ Accumulate circuit ========================
//========================================================
bool Cdfe_func::IsAbsolutePerform(unsigned int channel)
{//{{{
    unsigned int abs = (unsigned int)(*DFEjCTLBCHn[channel])["ABS"];
    return (abs == 1);
}//}}}

void Cdfe_func::SetAERBit(unsigned int channel, unsigned int value)
{//{{{
...
unsigned int Cdfe_func::GetAccumExpectedCount(unsigned int channel)
{//{{{
...
bool Cdfe_func::IsAccumEnable(unsigned int channel)
{//{{{ 
...
bool Cdfe_func::IsDecimateEnable(unsigned int channel)
{//{{{ 
    unsigned int en = (unsigned int)(*DFEjCTLACHn[channel])["EN"];
    unsigned int prcsa = (unsigned int)(*DFEjCTLBCHn[channel])["PRCSA"];
    return ((en == 1) && (prcsa == 2));
...
unsigned int Cdfe_func::GetDeciCATAGBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjCTLACHn[channel])["CATAG"];
}//}}}

void Cdfe_func::SetCERBit(unsigned int channel, unsigned int value)
{//{{{
...
void Cdfe_func::SetGERBit(unsigned int channel, unsigned int value)
{//{{{
...
void Cdfe_func::SetMERBit(unsigned int channel, unsigned int value)
{//{{{
...
unsigned int Cdfe_func::GetAEBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjTRGCHn[channel])["AE"];
}//}}}

unsigned int Cdfe_func::GetAFEBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjTRGCHn[channel])["AFE"];
}//}}}

unsigned int Cdfe_func::GetMITMTRGBit(unsigned int trigger)
{//{{{
    unsigned int ret = 0;
    if (trigger == 0){
...
int Cdfe_func::GetATBit(unsigned int channel)
{//{{{
    int ret = 0;
...
unsigned int Cdfe_func::GetCAENBit(unsigned int channel)
{//{{{
    return ((unsigned int)(*DFEjCTLACHn[channel])["CAEN"]);
}//}}}

unsigned int Cdfe_func::GetPRCSABit(unsigned int channel)
{//{{{
...
//============ Subtraction circuit =======================
//========================================================
void Cdfe_func::SetSUBTRHCHReg(unsigned int sub_id, unsigned int value)
{//{{{
    (*DFEjSUBTRHCHn[sub_id]) = value;
}//}}}

unsigned int Cdfe_func::GetMinuendChannelNum(unsigned int sub_id)
{//{{{
    return (unsigned int)(*DFEjSUBCTLCHn[sub_id])["MINCH"];
}//}}}

unsigned int Cdfe_func::GetSubtrahendChannelNum(unsigned int sub_id)
{//{{{
    return (unsigned int)(*DFEjSUBCTLCHn[sub_id])["SUBCH"];
}//}}}

void Cdfe_func::SetSUBFBit(unsigned int sub_id, unsigned int value)
{//{{{
    (*DFEjSUBSTCHn[sub_id])["SUBF"] = value;
}//}}}

unsigned int Cdfe_func::GetSIEOBit(unsigned int sub_id)
{//{{{
    return (unsigned int)(*DFEjSUBCTLCHn[sub_id])["SIEO"];
}//}}}

unsigned int Cdfe_func::GetSUBFBit(unsigned int sub_id)
{//{{{
    return (unsigned int)(*DFEjSUBSTCHn[sub_id])["SUBF"];
}//}}}

void Cdfe_func::SetMINFBit(unsigned int sub_id, unsigned int value)
{//{{{
    (*DFEjSUBSTCHn[sub_id])["MINF"] = value;
}//}}}

unsigned int Cdfe_func::GetMINFBit(unsigned int sub_id)
{//{{{
    return (unsigned int)(*DFEjSUBSTCHn[sub_id])["MINF"];
}//}}}

void Cdfe_func::SetSubDataOutReg(unsigned int sub_id, unsigned int value)
{//{{{
    (*DFEjSUBDOCHn[sub_id]) = value;
}//}}}

unsigned int Cdfe_func::GetSubCAEN(unsigned int sub_id)
{//{{{
    return (unsigned int)(*DFEjSUBCTLCHn[sub_id])["CAEN"];
}//}}}

bool Cdfe_func::IsFPConvertSubValue(unsigned int sub_id)
{//{{{
    return ((unsigned int)(*DFEjSUBCTLCHn[sub_id])["SFMT"] == 1);
}//}}}

void Cdfe_func::SetSDOENBit(unsigned int sub_id, unsigned int value)
{//{{{
    (*DFEjSUBSTCHn[sub_id])["SDOEN"] = value;
}//}}}

unsigned int Cdfe_func::GetSDOENBit(unsigned int sub_id)
{//{{{
    return (unsigned int)(*DFEjSUBSTCHn[sub_id])["SDOEN"];
}//}}}

void Cdfe_func::SetSDOOWBit(unsigned int sub_id, unsigned int value)
{//{{{
...
void Cdfe_func::SetSCERBit(unsigned int sub_id, unsigned int value)
{//{{{
...
void Cdfe_func::SetSGERBit(unsigned int sub_id, unsigned int value)
{//{{{
...
unsigned int Cdfe_func::GetSubCATAGBit(unsigned int sub_id)
{//{{{
    return (unsigned int)(*DFEjSUBCTLCHn[sub_id])["CATAG"];
}//}}}

unsigned int Cdfe_func::GetSBEBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjSUBTRGCHn[channel])["SBE"];
}//}}}

unsigned int Cdfe_func::GetSBFEBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjSUBTRGCHn[channel])["SBFE"];
}//}}}

unsigned int Cdfe_func::GetSBTBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjSUBTRGCHn[channel])["SBT"];
}//}}}

unsigned int Cdfe_func::GetSUBTMTRGBit(unsigned int trigger)
{//{{{
    unsigned int ret = 0;
    if (trigger == 0){
...
unsigned int Cdfe_func::GetSENBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjSUBCTLCHn[channel])["SEN"];
}//}}}

void Cdfe_func::SetSITSBit(unsigned int subid, unsigned int value)
{//{{{
    (*DFEjSUBTRHCHn[subid])["SITS"] = value;
}//}}}

void Cdfe_func::SetSETSBit(unsigned int subid, unsigned int value)
{//{{{
    (*DFEjSUBTRHCHn[subid])["SETS"] = value;
}//}}}

bool Cdfe_func::CheckSubValidSetting(unsigned int subid)
{//{{{
    bool can_run = true;
...
//============ Capture circuit    ========================
//========================================================
unsigned int Cdfe_func::GetCENBit(unsigned int capid)
{//{{{
    return (unsigned int)(*DFEjCAPCTLCHn[capid])["CEN"];
}//}}}

unsigned int Cdfe_func::GetCAPCHBit(unsigned int capid)
{//{{{
    return (unsigned int)(*DFEjCAPCTLCHn[capid])["CAPCH"];
}//}}}

void Cdfe_func::CaptureToCAPDOCHReg(unsigned int channel, unsigned int capid)
{//{{{
...
//========================================================
void Cdfe_func::StoreIntermediateValue(unsigned int channel_id, unsigned int high, unsigned int low)
{//{{{
...
//============ Output circuit ============================
//========================================================
bool Cdfe_func::IsFPConvertOutputValue(unsigned int channel)
{//{{{
    return ((unsigned int)(*DFEjCTLBCHn[channel])["PRCSC"] == 1);
}//}}}

unsigned int Cdfe_func::GetIEOBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjCTLACHn[channel])["IEO"];
}//}}}

void Cdfe_func::SetDOCHReg(unsigned int channel, unsigned int data)
{//{{{
    (*DFEjDOCHn[channel]) = data;
}//}}}

unsigned int Cdfe_func::GetDOCHReg(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjDOCHn[channel]);
}//}}}

void Cdfe_func::SetDOENBit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjSTCHn[channel])["DOEN"] = value;
}//}}}

unsigned int Cdfe_func::GetDOENBit(unsigned int channel)
{//{{{
...
//============ PH circuit ================================
//========================================================
void Cdfe_func::SetPHEBit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjSTCHn[channel])["PHE"] = value;
}//}}}

unsigned int Cdfe_func::GetIEPBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjCTLACHn[channel])["IEP"];
}//}}}

void Cdfe_func::SetDISBBit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjCTLBCHn[channel])["DISB"] = value;
}//}}}

void Cdfe_func::SetDISABit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjCTLBCHn[channel])["DISA"] = value;
}//}}}

unsigned int Cdfe_func::GetPRCSBBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjCTLBCHn[channel])["PRCSB"];
}//}}}

unsigned int Cdfe_func::GetPHInitVal(unsigned int phase, unsigned int channel)
{//{{{
...
unsigned int Cdfe_func::GetPHPSBit(unsigned int ph_phase, unsigned int channel)
{//{{{
    unsigned int phps = 0;
    if (ph_phase == 1){
...
unsigned int Cdfe_func::GetPHUPDCHBit(unsigned int phuid)
{//{{{
    return (unsigned int)(*DFEjPHUPDCn[phuid])["PHUPDCH"];
}//}}}

unsigned int Cdfe_func::GetCNSLEBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjCTLACHn[channel])["CNSLE"];
}//}}}

unsigned int Cdfe_func::GetCNSLBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjCTLACHn[channel])["CNSL"];
}//}}}

unsigned int Cdfe_func::GetOEPHUPDBit(unsigned int phuid)
{//{{{
    return (unsigned int)(*DFEjPHUPDCn[phuid])["OEPHUPD"];
}//}}}

unsigned int Cdfe_func::GetPH23SLBit(unsigned int phuid)
{//{{{
    return (unsigned int)(*DFEjPHUPDCn[phuid])["PH23SL"];
}//}}}

unsigned int Cdfe_func::GetPH23EBit(unsigned int phuid)
{//{{{
    return (unsigned int)(*DFEjPHUPDCn[phuid])["PH23E"];
}//}}}

void Cdfe_func::SetPHCHReg(unsigned int phase, unsigned int channel, unsigned int value)
{//{{{
    if (phase == 1){
...
void Cdfe_func::SetPHCHPureVal(unsigned int phase, unsigned int channel, unsigned int value)
{//{{{
    if (phase == 1){
...
void Cdfe_func::SetPrePHCH(unsigned int phase, unsigned int channel)
{//{{{
    if (phase == 1){
...
unsigned int Cdfe_func::GetPrePHCHReg(unsigned int channel)
{//{{{ 
    unsigned int ret = 0;
...
unsigned int Cdfe_func::GetCompareOperator(unsigned int phase, unsigned int channel)
{//{{{ 
    unsigned int ret = 0;
    if (phase == 1){
...
void Cdfe_func::SetPHINDBit(unsigned int ph_phase, unsigned int channel, unsigned int value)
{//{{{
    if (ph_phase == 1){
...
void Cdfe_func::SetPHIOFBit(unsigned int ph_phase, unsigned int channel, unsigned int value)
{//{{{
    if (ph_phase == 1){
...
void Cdfe_func::SetPHINDCHReg(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjPHINDCHn[channel]) = value;
}//}}}

void Cdfe_func::SetPH2IND0Reg(unsigned int value)
{//{{{
    (*DFEjPH2IND0) = value;
}//}}}

void Cdfe_func::SetPH3IND0Reg(unsigned int value)
{//{{{
    (*DFEjPH3IND0) = value;
}//}}}

unsigned int Cdfe_func::GetPHPFMTBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjCTLBCHn[channel])["PFMT"];
}//}}}

unsigned int Cdfe_func::GetPH23PENBit()
{//{{{
    return (unsigned int)(*DFEjPH23CCTL0)["PEN"];
}//}}}

unsigned int Cdfe_func::GetPH23CHSBit()
{//{{{
    return (unsigned int)(*DFEjPH23CCTL0)["CHS"];
}//}}}

unsigned int Cdfe_func::GetPH23STBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjSTCHn[channel])["PH23ST"];
}//}}}

void Cdfe_func::SetPH23STBit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjSTCHn[channel])["PH23ST"] = value;
}//}}}

void Cdfe_func::SetPH20Reg(unsigned int value)
{//{{{
    (*DFEjPH20) = value;
}//}}}

void Cdfe_func::SetPH30Reg(unsigned int value)
{//{{{
    (*DFEjPH30) = value;
}//}}}

unsigned int Cdfe_func::GetCPCSBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjCTLBCHn[channel])["CPCS"];
}//}}}

unsigned int Cdfe_func::GetAlpha(unsigned int ph_phase, unsigned int channel)
{//{{{
    unsigned int ofsl = 0;
...
unsigned int Cdfe_func::GetPMFEBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjTRGCHn[channel])["PMFE"];
}//}}}

unsigned int Cdfe_func::GetPMEBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjTRGCHn[channel])["PME"];
}//}}}

unsigned int Cdfe_func::GetPEBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjTRGCHn[channel])["PE"];
}//}}}

unsigned int Cdfe_func::GetPFEBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjTRGCHn[channel])["PFE"];
}//}}}

unsigned int Cdfe_func::GetPMTBit(unsigned int channel)
{//{{{
    return (unsigned int)(*DFEjTMTRGCHn[channel])["PMT"];
}//}}}

unsigned int Cdfe_func::GetPMITMTRGBit(unsigned int trigger)
{//{{{
    unsigned int ret = 0;
    if (trigger == 0){
...
unsigned int Cdfe_func::GetPITMTRGBit(unsigned int trigger)
{//{{{
    unsigned int ret = 0;
    if (trigger == 0){
...
int Cdfe_func::GetPTBit(unsigned int channel)
{//{{{
    int ret = 0;
...
void Cdfe_func::SetCNDBit(unsigned int channel, unsigned int value)
{//{{{
    (*DFEjSTCHn[channel])["CND"] = value;
}//}}}

unsigned int Cdfe_func::GetCPRegWithSELB1Bit(unsigned int channel)
{//{{{
...
unsigned int Cdfe_func::GetCHSBit()
{//{{{
    return (unsigned int)(*DFEjPH23CCTL0)["CHS"];
}//}}}
//========================================================
...
//========================================================
void Cdfe_func::UpdateErrorStatus()
{//{{{
    ...
    if (mCurErrStatus && (!mPreErrStatus)){
        mDFEAgentController->RaiseINTDFEERR();
    }
}//}}}

bool Cdfe_func::CheckSettingDFEjCTLACHn(unsigned int channel)
{//{{{
    bool can_run = true;
...
bool Cdfe_func::CheckSettingDFEjCTLBCHn(unsigned int channel)
{//{{{
    bool can_run = true;
...
bool Cdfe_func::CheckSettingDFEjTRGCHn(unsigned int channel)
{//{{{
    bool can_run = true;
...
bool Cdfe_func::CheckSettingDFEjPMITRG()
{//{{{
    bool can_run = true;
...
bool Cdfe_func::CheckSettingDFEjPITRG()
{//{{{
    bool can_run = true;
...
bool Cdfe_func::CheckSettingDFEjMITRG()
{//{{{
    bool can_run = true;
...
bool Cdfe_func::CheckSettingDFEjFITRG()
{//{{{
    bool can_run = true;
...
bool Cdfe_func::CheckSettingDFEjPHUPDCn()
{//{{{
    bool can_run = true;
    for (unsigned int i = 0; i < 4; i++){
        unsigned int phupdch = (unsigned int)(*DFEjPHUPDCn[i])["PHUPDCH"];

        if (!((0x0 <= phupdch) && (phupdch <= 0xB))){
...
bool Cdfe_func::CheckSettingDFEjTMTRGCHn(unsigned int channel)
{//{{{
    bool can_run = true;
...
bool Cdfe_func::CheckSettingDFEjSUBCTLCHn(unsigned int channel)
{//{{{
    bool can_run = true;
...
bool Cdfe_func::CheckSettingDFEjSUBTRGCHn(unsigned int channel)
{//{{{
    bool can_run = true;
...
bool Cdfe_func::CheckSettingDFEjSUBTRG()
{//{{{
    bool can_run = true;
...
void Cdfe_func::CommonCallbackDFEHOHCHn(unsigned int channel)
{//{{{
    unsigned int hofs = (unsigned int)(*DFEjCTLBCHn[channel])["HOFS"];
...
void Cdfe_func::CommonCallbackDFEHOLCHn(unsigned int channel)
{//{{{
...
void Cdfe_func::cb_DFEjCTLACHn_EN(RegCBstr str)
{//{{{
    bool can_run = true;
    ...
    if (en == 0){
        // Not stop now. Wait finish remain task then stop. Cdfe class will update variable at the end of operation.
    }else{// en == 1
        ...
        mDFEAgentController->UpdateChannelEnable(str.channel, can_run);
    }

    // Auto initialize => must put after invoking UpdateChannelEnable() IMPORTANT
    if ((pre_en_aime != en_aime) && (en_aime == 0x3)){
        mDFEAgentController->AutoInitialize(str.channel);
    }
}//}}}

void Cdfe_func::cb_DFEjCTLBCHn_PRCSA(RegCBstr str)
{//{{{
    unsigned int valid = (unsigned int)(*DFEjSTCHn[str.channel])["VALID"];
    ...
    unsigned int pre_hofs = (str.pre_data >> 7) & 0x1;
        unsigned int disb = (unsigned int)(*DFEjCTLBCHn[str.channel])["DISB"];
        unsigned int disa = (unsigned int)(*DFEjCTLBCHn[str.channel])["DISA"];
    ...
    if (disa == 0x1) {
        //Disable Accumulation/Decimation Initialize/Disable flag when DISA bit is written 1
        mDFEAgentController->SetAccumDeciInitFlag(false, str.channel);
    }
    if (disb == 0x1) {
        //Disable PH initialize flag when DISB bit is written 1
        mDFEAgentController->SetPeakHoldInitFlag(false, str.channel);
    }
}//}}}

void Cdfe_func::cb_DFEjDOCHn_DFEjDOCHn(RegCBstr str)
{//{{{
    // FIR
...
void Cdfe_func::cb_DFEjPHINDCHn_PHIOF(RegCBstr str)
{//{{{
    if (str.channel >=0 && str.channel <= 9 ) {
        ...
        re_printf("warning", "Peak-Hold Index Register Control Select not supported by channels outside range");
        }
}//}}}

void Cdfe_func::cb_DFEjCLRSTCHn_CLRDIOW(RegCBstr str)
{//{{{
    if (str.is_wr){
        // CLRPHIOW bit
...
void Cdfe_func::cb_DFEjDI_DI(RegCBstr str)
{//{{{
    mDFEAgentController->StartSWInput();
}//}}}

void Cdfe_func::cb_DFEjTRG_TRGA(RegCBstr str)
{//{{{
    if (str.is_wr){
        if ((str.data & 0x1) == 1){
            mDFEAgentController->IssueSWTrigger();
        }
    }
}//}}}

void Cdfe_func::cb_DFEjHOLCH0_DFEjHOLCH0(RegCBstr str)
{//{{{
    CommonCallbackDFEHOLCHn(0);
}//}}} 

void Cdfe_func::cb_DFEjHOHCH0_GURD(RegCBstr str)
{//{{{
    CommonCallbackDFEHOHCHn(0);
}//}}} 

void Cdfe_func::cb_DFEjHOLCH2_DFEjHOLCH2(RegCBstr str)
{//{{{
    CommonCallbackDFEHOLCHn(2);
}//}}} 

void Cdfe_func::cb_DFEjHOHCH2_GURD(RegCBstr str)
{//{{{
    CommonCallbackDFEHOHCHn(2);
}//}}} 

void Cdfe_func::cb_DFEjHOLCH4_DFEjHOLCH4(RegCBstr str)
{//{{{
    CommonCallbackDFEHOLCHn(4);
}//}}} 

void Cdfe_func::cb_DFEjHOHCH4_GURD(RegCBstr str)
{//{{{
    CommonCallbackDFEHOHCHn(4);
}//}}} 

void Cdfe_func::cb_DFEjHOLCH6_DFEjHOLCH6(RegCBstr str)
{//{{{
    CommonCallbackDFEHOLCHn(6);
}//}}} 

void Cdfe_func::cb_DFEjHOHCH6_GURD(RegCBstr str)
{//{{{
    CommonCallbackDFEHOHCHn(6);
}//}}} 

void Cdfe_func::cb_DFEjHOLCH8_DFEjHOLCH8(RegCBstr str)
{//{{{
    CommonCallbackDFEHOLCHn(8);
}//}}} 

void Cdfe_func::cb_DFEjHOHCH8_GURD(RegCBstr str)
{//{{{
    CommonCallbackDFEHOHCHn(8);
}//}}} 

void Cdfe_func::cb_DFEjHOLCH10_DFEjHOLCH10(RegCBstr str)
{//{{{
    CommonCallbackDFEHOLCHn(10);
}//}}} 

void Cdfe_func::cb_DFEjHOHCH10_GURD(RegCBstr str)
{//{{{
    CommonCallbackDFEHOHCHn(10);
}//}}} 

void Cdfe_func::cb_DFEjPH23CCTL0_PEN(RegCBstr str)
{//{{{
...
void Cdfe_func::cb_DFEjPH2CTL0_CN2SLB2(RegCBstr str)
{//{{{ 
...
void Cdfe_func::cb_DFEjPH3CTL0_CN3SLB2(RegCBstr str)
{//{{{ 
    unsigned int pen = (unsigned int)(*DFEjPH23CCTL0)["PEN"];
...
void Cdfe_func::cb_DFEjSUBCTLCHn_SEN(RegCBstr str)
{//{{{
    bool can_run = true;
    ...
    if (sen == 0){
        // Not stop now. Cdfe will stop after finish operation (for channel)
    }else{// sen == 1
...
void Cdfe_func::cb_DFEjSUBDOCHn_DFEjSUBDOCHn(RegCBstr str)
{//{{{
    (*DFEjSUBSTCHn[str.channel])["SDOEN"] = 0;
}//}}}

void Cdfe_func::cb_DFEjSUBCLRSTCHn_CLRSDOOW(RegCBstr str)
{//{{{
    if (str.is_wr){
        // CLRSDOEN bit
        if (((str.data >> 8)&0x1) == 1){
            (*DFEjSUBSTCHn[str.channel])["SDOEN"] = 0;
            (*DFEjSUBCLRSTCHn[str.channel])["CLRSDOEN"] = 0;
        }
        // CLRSCER bit
        if (((str.data >> 5)&0x1) == 1){
...
void Cdfe_func::cb_DFEjSUBTRGCHn_SBT(RegCBstr str)
{//{{{
    unsigned int sbe = (unsigned int)(*DFEjSUBTRGCHn[str.channel])["SBE"];
    unsigned int pre_sbe = (str.pre_data >> 6) & 0x3;
    if ((pre_sbe == 0) && (sbe != 0)){
        mDFEAgentController->DisableSubtractionFlag(str.channel);
    }
}//}}}

void Cdfe_func::cb_DFEjCAPCTLCHn_CEN(RegCBstr str)
{//{{{
    ...
    if (cen == 0){
        // Not stop now. Cdfe will stop after finish operation (channel)
    }else{// cen == 1
        ...
        mDFEAgentController->UpdateCaptureEnable(str.channel, true);
    }
}//}}}

void Cdfe_func::cb_DFEjCAPDOCHn_DFEjCAPDOCHn(RegCBstr str)
{//{{{
    (*DFEjCAPSTCHn[str.channel])["CDOEN"] = 0;
}//}}}

void Cdfe_func::cb_DFEjCAPCLRSTCHn_CLRCDOOW(RegCBstr str)
{//{{{
    if (str.is_wr){
        // CLRCDOEN bit
        if (((str.data >> 8)&0x1) == 1){
...
unsigned int Cdfe_func::GetDFEjSTCHn(unsigned int nChan)
{//{{{
    return (unsigned int)(*DFEjSTCHn[nChan]);
}//}}}

unsigned int Cdfe_func::GetDFEjST()
{//{{{
    return (unsigned int)(*DFEjST);
}//}}}

unsigned int Cdfe_func::GetDFEjEST()
{//{{{
    return (unsigned int)(*DFEjEST);
}//}}}

unsigned int Cdfe_func::GetDFEjSUBSTCHn(unsigned int nChan)
{//{{{
    return (unsigned int)(*DFEjSUBSTCHn[nChan]);
}//}}}

unsigned int Cdfe_func::GetDFEjCAPSTCHn(unsigned int nChan)
{//{{{
    return (unsigned int)(*DFEjCAPSTCHn[nChan]);
}//}}}

unsigned int Cdfe_func::GetDFEjCTLBCHn(unsigned int nChan)
{//{{{
    return (unsigned int)(*DFEjCTLBCHn[nChan]);
}//}}}
