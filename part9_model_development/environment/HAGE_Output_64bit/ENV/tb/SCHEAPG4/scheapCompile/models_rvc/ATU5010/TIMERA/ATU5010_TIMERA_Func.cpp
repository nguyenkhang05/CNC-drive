// ---------------------------------------------------------------------
// $Id: ATU5010_TIMERA_Func.cpp,v 1.0 2016/11/24 10:15:45 binhnguyen Exp $
//
...
/*********************************
// Function     : ATU5010_TIMERA_Func
// Description  : Constructor of ATU5010_TIMERA_Func class
// Parameter    :
//      name            Module name
//      Parent          Indicate base class
// Return value : None
**********************************/
ATU5010_TIMERA_Func::ATU5010_TIMERA_Func(std::string name, ATU5010_TIMERA_Base *ATU5010_TIA_Base_ptr):
                             Catu5010_timera_regif(name,32)
{//{{{
    assert(ATU5010_TIA_Base_ptr != NULL);
    mATU5010_TIA_Base = ATU5010_TIA_Base_ptr;
    mATU5010_TIA_Base->CommandInit(name);
    Catu5010_timera_regif::set_instance_name(name);
    mModuleName = name;
}//}}}

/*********************************
// Function     : ~ATU5010_TIMERA_Func
// Description  : Destructor of ATU5010_TIMERA_Func class
// Parameter    : None
// Return value : None
**********************************/
ATU5010_TIMERA_Func::~ATU5010_TIMERA_Func (void)
{//{{{
}//}}}
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::read (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    unsigned int addr = (unsigned int)(trans.get_address()-0x200)&0x1FF;
    memset(trans.get_data_ptr(), 0, trans.get_data_length());
    if (addr == 0x18) { //Read TILRA
        (*TILRA) = mATU5010_TIA_Base->GetTILRAVal();
    }
    if ((addr == 0x20)&&(mATU5010_TIA_Base->IsTimerRunning())) { //Read TCNTA
        SetTCNTA(mATU5010_TIA_Base->GetCounterValue());
    }
    for (unsigned int i = 0; i < 8; i++) {
        if ((addr == (0x40 + (i*0x20)))&&(mATU5010_TIA_Base->IsNoiseCancelDuration(i))) { //Read NCNTA
            SetNCNTA(mATU5010_TIA_Base->GetNoiseCounterValue(i),i);
        }
    }
    if (debug == false) {
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::write (unsigned int offsetAddress, TlmBasicPayload& trans, BusTime_t* t, bool debug)
{//{{{
    unsigned int addr = (unsigned int)(trans.get_address()-0x200)&0x1FF;
    if (debug == false) {
...
// Description  : Set parameters or commands to Catu5010_timera_regif class
// Parameter    :
//      args            Argument of parameter/command
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::RegisterHandler (const std::vector<std::string> &args)
{//{{{
    ...
    if (args.size() == 1) {//Dump registers name
        ...
        if (args[1] == "*") {//Set message level for each register
            ...
            while (reg_p != NULL) {
                ...
                msg_return = Catu5010_timera_regif::reg_handle_command(cmd);
                if (msg_return != "") {
        ...
        } else {//Set handle command to Catu5010_timera_regif class
            msg_return = Catu5010_timera_regif::reg_handle_command(args);
            if (msg_return != "") {
                SC_REPORT_INFO(mModuleName.c_str(), msg_return.c_str());
            }
        }
...
// Description  : Reset ATU5010_TIMERA's registers
// Parameter    :
//      is_active           Active state of reset
// Return value : Register value
**********************************/
void ATU5010_TIMERA_Func::EnableReset (const bool is_active)
{//{{{
    Catu5010_timera_regif::EnableReset(is_active);
}//}}}
...
**********************************/
unsigned int ATU5010_TIMERA_Func::GetCLKBusSel (void)
{//{{{
...
**********************************/
unsigned int ATU5010_TIMERA_Func::GetNoiseCancelMode (const unsigned int chan_num)
{//{{{
    unsigned int ret = 0;
    if ((((*NCMCR1A)>>chan_num)&0x1) == 0x1){
        if ((((*NCMCR2A)>>chan_num)&0x1) == 0) {
            ret = 1;
        } else {
            ret = 2;
        }
...
**********************************/
unsigned int ATU5010_TIMERA_Func::GetNoiseCLKSel (const unsigned int chan_num)
{//{{{
    unsigned int ret = 0;
    unsigned int cur_nckga = (*TIOR2A)["NCKGA"]&0xFF;
    unsigned int cur_ncka = (*TIOR2A)["NCKA"]&0xFF;
...
**********************************/
unsigned int ATU5010_TIMERA_Func::GetTCNTA (void)
{//{{{
...
**********************************/
unsigned int ATU5010_TIMERA_Func::GetNCNTA (const unsigned int chan_num)
{//{{{
...
**********************************/
unsigned int ATU5010_TIMERA_Func::GetICRA2SEL (void)
{//{{{
...
**********************************/
bool ATU5010_TIMERA_Func::IsOutEvent (const std::string event_name, const unsigned int chan_num, const bool is_raisingedge)
{//{{{
    bool ret = false;
    ...
    if ((edge_sel == 0x1 && is_raisingedge) || (edge_sel == 0x2 && !is_raisingedge) || (edge_sel == 0x3)){
        if (event_name == "ev_out_1_0") {
            if ((*TCR2A)["EVOSELE1E"] == 1) {
                if ((((*TCR2A)["EVOSELE1"]>>chan_num)&0x1) == 1) {
                    ret = true;
                }
            } else {
            ...
            if ((((*TCR2AS1)["EVOSELE1S1"]>>chan_num)&0x1) == 1) {
                ret = true;
            }
        } else if (event_name == "ev_out_1_2") {
            if ((((*TCR2AS2)["EVOSELE1S2"]>>chan_num)&0x1) == 1) {
                ret = true;
            }
...
**********************************/
bool ATU5010_TIMERA_Func::IsNoiseCancelEn (const unsigned int chan_num)
{//{{{
...
**********************************/
bool ATU5010_TIMERA_Func::IsInterruptEnable (const std::string bit_name)
{//{{{
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::UpdateTimerStatus (const bool is_captured, const unsigned int chan_num)
{//{{{
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::UpdateICRAVal (const unsigned int value, const unsigned int chan_num, const bool is_icra2x)
{//{{{
    if (is_icra2x) {
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::SetTCNTA (unsigned int value)
{//{{{
    (*TCNTA) = value;
}//}}}
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::SetNCNTA (unsigned int value, const unsigned int chan_num)
{//{{{
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::cb_TCR1A_CKSELA(RegCBstr str)
{//{{{
    if (mATU5010_TIA_Base->IsTimerRunning()||((*TCR1A)["CKSELA"] > 5)) {
        mATU5010_TIA_Base->print_msg("warning","Can not change selected counting clock.\n");
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::cb_TSCRA_ICFCA(RegCBstr str)
{//{{{
    if ((*TSCRA)["OVFCA"] == 1) {
        (*TSRA)["OVFA"] = 0;
    }
    if ((*TSCRA)["ICFCA"] != 0) {
        ...
        unsigned int mask = tscra_val & 0x000080FF;
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::cb_NCMCR1A_NCM1A(RegCBstr str)
{//{{{
    ...
    for (unsigned int chan_num = 0; chan_num < 8; chan_num++) {
        ...
        if (((pre_ncm1a>>chan_num)&0x1) != ((cur_ncm1a>>chan_num)&0x1)) {
            if (mATU5010_TIA_Base->IsNoiseCancelDuration(chan_num)) {
                mATU5010_TIA_Base->print_msg("warning","Can not change operation mode of noise canceler in channel %d.\n",chan_num);
                (*NCMCR1A)["NCM1A"] = (cur_ncm1a & (~((unsigned int)1 << chan_num)) & 0xFF) | (((pre_ncm1a>>chan_num)&0x1)<<chan_num);
            } else {
                mATU5010_TIA_Base->SetNoiseCancelMode(chan_num);
            }
        }
    }
}//}}}
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::cb_NCMCR2A_NCM2A(RegCBstr str)
{//{{{
    unsigned int pre_ncm2a = str.pre_data;
    for (unsigned int chan_num = 0; chan_num < 8; chan_num++) {
        unsigned int cur_ncm2a = (*NCMCR2A)["NCM2A"];
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::cb_TIOR2A_NCEA(RegCBstr str)
{//{{{
    ...
    for (unsigned int chan_num = 0; chan_num < 8; chan_num++) {
        ...
        if ((((str.pre_data>>chan_num)&0x1) == 0x1) && ((((*TIOR2A)["NCEA"]>>chan_num)&0x1) == 0x1)) {
            if ((((cur_nckga>>chan_num)&0x1) != ((pre_nckga>>chan_num)&0x1)) ||
                ...
                mATU5010_TIA_Base->print_msg("warning","Can not change selected counting clock of NCNTA%d when noise cancellation function for TIA%d is enabled.\n",chan_num, chan_num);
            ...
            mATU5010_TIA_Base->EnableNoiseCancel(chan_num, true);
            mATU5010_TIA_Base->SetNoiseCancelMode(chan_num);
        } else if ((((*TIOR2A)["NCEA"]>>chan_num)&0x1) == 0x0) {
            mATU5010_TIA_Base->EnableNoiseCancel(chan_num, false);
        }
    }
}//}}}
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::cb_TCNTA_CNTA(RegCBstr str)
{//{{{
    mATU5010_TIA_Base->UpdateCounterValue();
}//}}}
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::cb_NCNTA_NCNTA(const unsigned int chan_num)
{//{{{
    mATU5010_TIA_Base->UpdateNoiseCounterValue(chan_num);
}//}}}
...
// Return value : None
**********************************/
void ATU5010_TIMERA_Func::cb_NCRA_NCTA(const unsigned int chan_num)
{//{{{
    mATU5010_TIA_Base->SetNoiseCORVal(chan_num, (unsigned int)(*NCRA[chan_num])["NCTA"]);
}//}}}
