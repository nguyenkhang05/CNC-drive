// -----------------------------------------------------------------------------
// $Id: pwseld0_regif.cpp,v 1.4 2020/01/15 13:23:55 landang Exp $
//
...
/// @return none
Cpwseld0_regif::Cpwseld0_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef USR_CWR_SYSTEMC
    , cwmem("register", scml_memsize(0x480/4))
    #endif
{
    CommandInit();
    PWS0CTL    = new vpcl::re_register(0x0000, this, "PWS0CTL"   , name.c_str());
    PWS0STR    = new vpcl::re_register(0x0004, this, "PWS0STR"   , name.c_str());
    PWS0STC    = new vpcl::re_register(0x0008, this, "PWS0STC"   , name.c_str());
    PWS0EMU    = new vpcl::re_register(0x000C, this, "PWS0EMU"   , name.c_str());
    std::ostringstream str_tmp;
    for(uint i = 0; i < emNUM_PWS0QUE; i++) {
        str_tmp.str("");
        str_tmp<<"PWS0QUE"<< i;
        PWS0QUE   [i] = new vpcl::re_register((0x0020) + 4*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_PWS0PVCR; i++) {
        str_tmp.str("");
        str_tmp<<"PWS0PVCR"<< i;
        PWS0PVCR  [i] = new vpcl::re_register((0x0100) + 4*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_PWS0PWDDIR; i++) {
        str_tmp.str("");
        str_tmp<<"PWS0PWDDIR"<< i;
        PWS0PWDDIR[i] = new vpcl::re_register((0x0300) + 4*i, this, str_tmp.str(), name.c_str());
    }
    ...
    for(uint i = 0; i < emNUM_PWS0PWDDIR; i++) {
        mRegMap[(0x0300)     + i*4 ] = index;
        mRegMap[(0x0300 + 1) + i*4 ] = index;
        mRegMap[(0x0300 + 2) + i*4 ] = index;
        mRegMap[(0x0300 + 3) + i*4 ] = index++;
    }
    mTotalRegNum = index;
    mRegArray = new SRegList* [mTotalRegNum];
    index = 0;
    #ifdef USR_CWR_SYSTEMC
    cw_instantiate_reg(CW_PWS0CTL     , PWS0CTL->name()   , PWS0CTL->addr()   );
    cw_instantiate_reg(CW_PWS0STR     , PWS0STR->name()   , PWS0STR->addr()   );
    cw_instantiate_reg(CW_PWS0STC     , PWS0STC->name()   , PWS0STC->addr()   );
    cw_instantiate_reg(CW_PWS0EMU     , PWS0EMU->name()   , PWS0EMU->addr()   );
    for(uint i = 0; i < emNUM_PWS0QUE   ; i++) cw_instantiate_reg(CW_PWS0QUE     [i], PWS0QUE   [i]->name(), PWS0QUE   [i]->addr());
    for(uint i = 0; i < emNUM_PWS0PVCR  ; i++) cw_instantiate_reg(CW_PWS0PVCR    [i], PWS0PVCR  [i]->name(), PWS0PVCR  [i]->addr());
    for(uint i = 0; i < emNUM_PWS0PWDDIR; i++) cw_instantiate_reg(CW_PWS0PWDDIR  [i], PWS0PWDDIR[i]->name(), PWS0PWDDIR[i]->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(PWS0CTL     , mCurReg, 0,  4,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PWS0STR     , mCurReg, 0,  4,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PWS0STC     , mCurReg, 0,  4,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PWS0EMU     , mCurReg, 0,  4,       "8", "8|16|32");
    for(uint i = 0; i < emNUM_PWS0QUE   ; i++) mCurReg = mRegArray[index++] = new SRegList(PWS0QUE     [i], mCurReg, i,  4,       "8", "8|16|32");
    for(uint i = 0; i < emNUM_PWS0PVCR  ; i++) mCurReg = mRegArray[index++] = new SRegList(PWS0PVCR    [i], mCurReg, i,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_PWS0PWDDIR; i++) mCurReg = mRegArray[index++] = new SRegList(PWS0PWDDIR  [i], mCurReg, i,  4,      "32", "8|16|32");

    mRegList = mCurReg;

    (*PWS0CTL     ) ( 7,  7, "ARSE"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*PWS0CTL     ) ( 0,  0, "ENBL"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*PWS0STR     ) ( 1,  1, "QFL"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PWS0STR     ) ( 0,  0, "QNE"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PWS0STC     ) ( 1,  1, "CLFL"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*PWS0STC     ) ( 0,  0, "CLNE"    , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*PWS0EMU     ) ( 0,  0, "SVSDIS"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    for(uint i = 0; i < emNUM_PWS0QUE; i++) {
        (*PWS0QUE     [i]) ( 6,  0, "QUE"     , 0x7F      , "R"     , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_PWS0PWDDIR; i++) {
        (*PWS0PWDDIR  [i]) (31, 31, "MP"      , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*PWS0PWDDIR  [i]) (30, 28, "MPXV"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*PWS0PWDDIR  [i]) (26, 26, "OWE"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*PWS0PWDDIR  [i]) (25, 25, "WFLG"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*PWS0PWDDIR  [i]) (21, 16, "ID"      , 0x0       , "R"     , vpcl::SPP_ENABLE);
        (*PWS0PWDDIR  [i]) (15,  0, "PWDDR"   , 0x0       , "R"     , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_PWS0PVCR; i++) {
        (*PWS0PVCR    [i]) (19, 16, "WTTSx"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PWS0PVCR    [i]) (15, 15, "MPXEx"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PWS0PVCR    [i]) (14, 12, "MPXVx"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PWS0PVCR    [i]) (11,  8, "VCULLMTBSx", 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PWS0PVCR    [i]) ( 7,  6, "SLADx"   , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
        (*PWS0PVCR    [i]) ( 5,  0, "GCTRLx"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    }
    ...
    mWrCbAPI["PWS0CTL" ]["ENBL"    ] = &Cpwseld0_regif::cb_PWS0CTL_ENBL;
    mWrCbAPI["PWS0STC" ]["CLNE"    ] = &Cpwseld0_regif::cb_PWS0STC_CLNE;
    mWrCbAPI["PWS0EMU" ]["SVSDIS"  ] = &Cpwseld0_regif::cb_PWS0EMU_SVSDIS;

    for(uint i = 0; i < emNUM_PWS0PVCR; i++) {
        str_tmp.str("");
        str_tmp<<"PWS0PVCR"<< i;
        mWrCbAPI[str_tmp.str()]["GCTRLx"] = &Cpwseld0_regif::cb_PWS0PVCR_GCTRLx;
    }
    for(uint i = 0; i < emNUM_PWS0PWDDIR; i++) {
        str_tmp.str("");
        str_tmp<<"PWS0PWDDIR"<< i;
        mRdCbAPI[str_tmp.str()]["PWDDR"] = &Cpwseld0_regif::cb_PWS0PWDDIR_PWDDR;
    }
    InitLocalVal();
}
...
Cpwseld0_regif::uint Cpwseld0_regif::bit_select(
...
bool Cpwseld0_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cpwseld0_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cpwseld0_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                const unsigned char *p_data,   ///< [in] Writing data
                                cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cpwseld0_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                unsigned char *p_data,   ///< [out] Reading data
                                cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cpwseld0_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                    const unsigned char *p_data,  ///< [in] Writing data
                                    cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cpwseld0_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                    unsigned char *p_data,   ///< [out] Reading data
                                    cuint size,              ///< [in]  Data size (byte)
                                    bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cpwseld0_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                 unsigned char *p_data,          ///< [out] Reading data
                                 cuint size,                     ///< [in]  Data size (byte)
...
bool Cpwseld0_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                 const unsigned char *p_data,      ///< [in] Writing data
                                 cuint size,                       ///< [in] Data size (byte)
                                 cuint reg_index,                  ///< [in] Register index
...
bool Cpwseld0_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cpwseld0_regif::CommandInit()
{
...
/// @return string
std::string Cpwseld0_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cpwseld0_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cpwseld0_regif::InitLocalVal()
{
    // Initialize local variables
    PWS0CTL_ARSE   = 0x0;
    PWS0CTL_ENBL   = 0x0;
    ...
    for(uint i = 0; i < emNUM_PWS0PVCR;  i++) {
        PWS0PVCR_WTTSx[i] = 0x0;
        PWS0PVCR_MPXEx[i] = 0x0;
        PWS0PVCR_MPXVx[i] = 0x0;
        PWS0PVCR_VCULLMTBSx[i] = 0x0;
        PWS0PVCR_SLADx[i] = 0x0;
        PWS0PVCR_GCTRLx[i] = 0x0;
    }
    for(uint i = 0; i < emNUM_PWS0PWDDIR; i++) {
        PWS0PWDDIR_MP [i] = 0x0;
        PWS0PWDDIR_MPXV[i] = 0x0;
        PWS0PWDDIR_OWE[i] = 0x0;
        PWS0PWDDIR_WFLG[i] = 0x0;
        PWS0PWDDIR_ID [i] = 0x0;
        PWS0PWDDIR_PWDDR[i] = 0x0;
    }
}

/// Update bit value to local value
/// @return None
void Cpwseld0_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        PWS0CTL_ARSE                   = (*PWS0CTL   )["ARSE"      ];
        PWS0CTL_ENBL                   = (*PWS0CTL   )["ENBL"      ];
        return;
    }
    if (addr == 0x0004) {
        PWS0STR_QFL                    = (*PWS0STR   )["QFL"       ];
        PWS0STR_QNE                    = (*PWS0STR   )["QNE"       ];
        return;
    }
    if (addr == 0x0008) {
        PWS0STC_CLFL                   = (*PWS0STC   )["CLFL"      ];
        PWS0STC_CLNE                   = (*PWS0STC   )["CLNE"      ];
        return;
    }
    if (addr == 0x000C) {
        PWS0EMU_SVSDIS                 = (*PWS0EMU   )["SVSDIS"    ];
        return;
    }
    if ((0x0020 <= addr) && (addr <= (0x0020)+(4*7)) && ((addr-0x0020)%4 == 0)) {
        uint i = (addr - 0x0020)/4;
        PWS0QUE_QUE                   [i] = (*(PWS0QUE   [i]))["QUE"       ];
        return;
    }
    if ((0x0100 <= addr) && (addr <= (0x0100)+(4*95)) && ((addr-0x0100)%4 == 0)) {
        uint i = (addr - 0x0100)/4;
        PWS0PVCR_WTTSx                [i] = (*(PWS0PVCR  [i]))["WTTSx"     ];
        PWS0PVCR_MPXEx                [i] = (*(PWS0PVCR  [i]))["MPXEx"     ];
        PWS0PVCR_MPXVx                [i] = (*(PWS0PVCR  [i]))["MPXVx"     ];
        PWS0PVCR_VCULLMTBSx           [i] = (*(PWS0PVCR  [i]))["VCULLMTBSx"];
        PWS0PVCR_SLADx                [i] = (*(PWS0PVCR  [i]))["SLADx"     ];
        PWS0PVCR_GCTRLx               [i] = (*(PWS0PVCR  [i]))["GCTRLx"    ];
        return;
    }
    if ((0x0300 <= addr) && (addr <= (0x0300)+(4*95)) && ((addr-0x0300)%4 == 0)) {
        uint i = (addr - 0x0300)/4;
        PWS0PWDDIR_MP                 [i] = (*(PWS0PWDDIR[i]))["MP"        ];
        PWS0PWDDIR_MPXV               [i] = (*(PWS0PWDDIR[i]))["MPXV"      ];
        PWS0PWDDIR_OWE                [i] = (*(PWS0PWDDIR[i]))["OWE"       ];
        PWS0PWDDIR_WFLG               [i] = (*(PWS0PWDDIR[i]))["WFLG"      ];
        PWS0PWDDIR_ID                 [i] = (*(PWS0PWDDIR[i]))["ID"        ];
        PWS0PWDDIR_PWDDR              [i] = (*(PWS0PWDDIR[i]))["PWDDR"     ];
        return;
    }
}
...
/// @return None
void Cpwseld0_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*PWS0CTL   )["ARSE"      ] = PWS0CTL_ARSE;
        (*PWS0CTL   )["ENBL"      ] = PWS0CTL_ENBL;
        return;
    }
    if (addr == 0x0004) {
        (*PWS0STR   )["QFL"       ] = PWS0STR_QFL;
        (*PWS0STR   )["QNE"       ] = PWS0STR_QNE;
        return;
    }
    if (addr == 0x0008) {
        (*PWS0STC   )["CLFL"      ] = PWS0STC_CLFL;
        (*PWS0STC   )["CLNE"      ] = PWS0STC_CLNE;
        return;
    }
    if (addr == 0x000C) {
        (*PWS0EMU   )["SVSDIS"    ] = PWS0EMU_SVSDIS;
        return;
    }
    if ((0x0020 <= addr) && (addr <= (0x0020)+(4*7)) && ((addr-0x0020)%4 == 0)) {
        uint i = (addr - 0x0020)/4;
        (*(PWS0QUE   [i]))["QUE"       ] = PWS0QUE_QUE[i];
        return;
    }
    if ((0x0100 <= addr) && (addr <= (0x0100)+(4*95)) && ((addr-0x0100)%4 == 0)) {
        uint i = (addr - 0x0100)/4;
        (*(PWS0PVCR  [i]))["WTTSx"     ] = PWS0PVCR_WTTSx[i];
        (*(PWS0PVCR  [i]))["MPXEx"     ] = PWS0PVCR_MPXEx[i];
        (*(PWS0PVCR  [i]))["MPXVx"     ] = PWS0PVCR_MPXVx[i];
        (*(PWS0PVCR  [i]))["VCULLMTBSx"] = PWS0PVCR_VCULLMTBSx[i];
        (*(PWS0PVCR  [i]))["SLADx"     ] = PWS0PVCR_SLADx[i];
        (*(PWS0PVCR  [i]))["GCTRLx"    ] = PWS0PVCR_GCTRLx[i];
        return;
    }
    if ((0x0300 <= addr) && (addr <= (0x0300)+(4*95)) && ((addr-0x0300)%4 == 0)) {
        uint i = (addr - 0x0300)/4;
        (*(PWS0PWDDIR[i]))["MP"        ] = PWS0PWDDIR_MP[i];
        (*(PWS0PWDDIR[i]))["MPXV"      ] = PWS0PWDDIR_MPXV[i];
        (*(PWS0PWDDIR[i]))["OWE"       ] = PWS0PWDDIR_OWE[i];
        (*(PWS0PWDDIR[i]))["WFLG"      ] = PWS0PWDDIR_WFLG[i];
        (*(PWS0PWDDIR[i]))["ID"        ] = PWS0PWDDIR_ID[i];
        (*(PWS0PWDDIR[i]))["PWDDR"     ] = PWS0PWDDIR_PWDDR[i];
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cpwseld0_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cpwseld0_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cpwseld0_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
    if(mDumpRegisterRW == true) {
        ...
        str_tmp<<operation;
        str_tmp<<" Size= "<<size;
        str_tmp<<" Addr= "<<Num2HexStr(addr,11, false);
        if (operation == "R") {
...
int Cpwseld0_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cpwseld0_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cpwseld0_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cpwseld0_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Cpwseld0_regif::uint Cpwseld0_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return none
void Cpwseld0_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cpwseld0_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cpwseld0_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cpwseld0_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Cpwseld0_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cpwseld0_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cpwseld0_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
