// -----------------------------------------------------------------------------
// $Id: ecc_ethernet_e2_regif.cpp,v 1.2 2019/03/05 04:17:14 synguyen Exp $
//
...
//
// Input file : ecc_ethernet_e2_regif.txt
////////////////////////////////////////////////////////////////////////////////
...
//     %%BIT    EC1ECP             5        5      0       R|W     TRUE        W
////////////////////////////////////////////////////////////////////////////////
/// @file ecc_ethernet_e2_regif.cpp
/// @brief Register IF class of model ECC_ETHERNET_E2
/// $Id: ecc_ethernet_e2_regif.cpp,v 1.2 2019/03/05 04:17:14 synguyen Exp $
/// $Date: 2019/03/05 04:17:14 $
/// $Revison$
/// $Author: synguyen $
////////////////////////////////////////////////////////////////////////////////
...
/// @return none
Cecc_ethernet_e2_regif::Cecc_ethernet_e2_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
...
{
    CommandInit();
    ETCTXECCCTL = new vpcl::re_register(0x0000, this, "ETCTXECCCTL", name.c_str());
    ETCRXECCCTL = new vpcl::re_register(0x0004, this, "ETCRXECCCTL", name.c_str());
    ...
    index = 0;
    #ifdef CWR_SYSTEMC
    cw_instantiate_reg(CW_ETCTXECCCTL  , ETCTXECCCTL->name(), ETCTXECCCTL->addr());
    cw_instantiate_reg(CW_ETCRXECCCTL  , ETCRXECCCTL->name(), ETCRXECCCTL->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(ETCTXECCCTL  , mCurReg, 0,  4, "8|16|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(ETCRXECCCTL  , mCurReg, 0,  4, "8|16|32", "8|16|32");

    mRegList = mCurReg;

    (*ETCTXECCCTL  ) ( 5,  5, "EC1ECP"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    (*ETCRXECCCTL  ) ( 5,  5, "EC1ECP"  , 0x0       , "R|W"   , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["ETCTXECCCTL"]["EC1ECP"   ] = &Cecc_ethernet_e2_regif::cb_ETCTXECCCTL_EC1ECP;
    mWrCbAPI["ETCRXECCCTL"]["EC1ECP"   ] = &Cecc_ethernet_e2_regif::cb_ETCRXECCCTL_EC1ECP;
    InitLocalVal();
}

/// Destructor: delete pointers
/// @return none
Cecc_ethernet_e2_regif::~Cecc_ethernet_e2_regif()
{
...
Cecc_ethernet_e2_regif::uint Cecc_ethernet_e2_regif::bit_select(
...
bool Cecc_ethernet_e2_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cecc_ethernet_e2_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecc_ethernet_e2_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                        const unsigned char *p_data,   ///< [in] Writing data
                                        cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecc_ethernet_e2_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                        unsigned char *p_data,   ///< [out] Reading data
                                        cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cecc_ethernet_e2_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                            const unsigned char *p_data,  ///< [in] Writing data
                                            cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cecc_ethernet_e2_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                            unsigned char *p_data,   ///< [out] Reading data
                                            cuint size,              ///< [in]  Data size (byte)
                                            bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cecc_ethernet_e2_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                         unsigned char *p_data,          ///< [out] Reading data
                                         cuint size,                     ///< [in]  Data size (byte)
...
bool Cecc_ethernet_e2_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                         const unsigned char *p_data,      ///< [in] Writing data
                                         cuint size,                       ///< [in] Data size (byte)
                                         cuint reg_index,                  ///< [in] Register index
...
bool Cecc_ethernet_e2_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return none
void Cecc_ethernet_e2_regif::CommandInit()
{
...
/// @return string
std::string Cecc_ethernet_e2_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cecc_ethernet_e2_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cecc_ethernet_e2_regif::InitLocalVal()
{
    // Initialize local variables
    ETCTXECCCTL_EC1ECP = 0x0;
    ETCRXECCCTL_EC1ECP = 0x0;
}

/// Update bit value to local value
/// @return None
void Cecc_ethernet_e2_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        ETCTXECCCTL_EC1ECP              = (*ETCTXECCCTL)["EC1ECP"     ];
        return;
    }
    if (addr == 0x0004) {
        ETCRXECCCTL_EC1ECP              = (*ETCRXECCCTL)["EC1ECP"     ];
        return;
    }
}
...
/// @return None
void Cecc_ethernet_e2_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*ETCTXECCCTL)["EC1ECP"     ] = ETCTXECCCTL_EC1ECP;
        return;
    }
    if (addr == 0x0004) {
        (*ETCRXECCCTL)["EC1ECP"     ] = ETCRXECCCTL_EC1ECP;
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cecc_ethernet_e2_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cecc_ethernet_e2_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cecc_ethernet_e2_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cecc_ethernet_e2_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cecc_ethernet_e2_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cecc_ethernet_e2_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cecc_ethernet_e2_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cecc_ethernet_e2_regif::uint Cecc_ethernet_e2_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
...
/// @return none
void Cecc_ethernet_e2_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cecc_ethernet_e2_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cecc_ethernet_e2_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cecc_ethernet_e2_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cecc_ethernet_e2_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cecc_ethernet_e2_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cecc_ethernet_e2_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
