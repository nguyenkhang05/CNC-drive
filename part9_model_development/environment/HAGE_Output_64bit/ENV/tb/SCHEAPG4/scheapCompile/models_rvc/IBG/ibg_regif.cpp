// -----------------------------------------------------------------------------
// $Id: ibg_regif.cpp,v 1.7 2019/07/15 08:43:08 nhuvo Exp $
//
...
/// @return none
Cibg_regif::Cibg_regif(std::string name, uint buswidth, uint iChannel)
    :vpcl::reg_super()
    ,channelNum(iChannel)
    ,strModelName(name)
...
{
    CommandInit();
    GERRCLR  = new vpcl::re_register(0x0000, this, "GERRCLR" , name.c_str());
    GERRSTAT = new vpcl::re_register(0x0004, this, "GERRSTAT", name.c_str());
    ...
    GKCPROT  = new vpcl::re_register(0x0010, this, "GKCPROT" , name.c_str());
    std::ostringstream str_tmp;
    for(uint i = 0; i < channelNum; i++) {
        str_tmp.str("");
        str_tmp<<"GPROT0_"<< i;
        GPROT0  [i] = new vpcl::re_register((0x0080) + 8*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < channelNum; i++) {
        str_tmp.str("");
        str_tmp<<"GPROT1_"<< i;
        GPROT1  [i] = new vpcl::re_register((0x0084) + 8*i, this, str_tmp.str(), name.c_str());
    }
    ...
    index = 0;
    #ifdef USR_CWR_SYSTEMC
    cw_instantiate_reg(CW_GERRCLR   , GERRCLR->name() , GERRCLR->addr() );
    cw_instantiate_reg(CW_GERRSTAT  , GERRSTAT->name(), GERRSTAT->addr());
    ...
    for(uint i = 0; i < channelNum ; i++) cw_instantiate_reg(CW_GPROT0    [i], GPROT0  [i]->name(), GPROT0  [i]->addr());
    for(uint i = 0; i < channelNum ; i++) cw_instantiate_reg(CW_GPROT1    [i], GPROT1  [i]->name(), GPROT1  [i]->addr());
    #endif
    mCurReg = mRegArray[index++] = new SRegList(GERRCLR   , mCurReg, 0,  4,    "8|32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(GERRSTAT  , mCurReg, 0,  4,    "8|32", "8|16|32");
    ...
    for(uint i = 0; i < channelNum ; i++) mCurReg = mRegArray[index++] = new SRegList(GPROT0    [i], mCurReg, i,  4, "8|16|32", "8|16|32");
    for(uint i = 0; i < channelNum ; i++) mCurReg = mRegArray[index++] = new SRegList(GPROT1    [i], mCurReg, i,  4, "8|16|32", "8|16|32");

    mRegList = mCurReg;

    (*GERRCLR   ) ( 0,  0, "CLRE"    , 0x0       , "R|W1:0", vpcl::SPP_ENABLE);
    (*GERRCLR   ) ( 1,  1, "CLRO"    , 0x0       , "R|W1:0", vpcl::SPP_ENABLE);
    (*GERRSTAT  ) ( 0,  0, "ERR"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*GERRSTAT  ) ( 1,  1, "OVF"     , 0x0       , "R"     , vpcl::SPP_ENABLE);
    ...
    (*GKCPROT   ) (31,  1, "KCPROT"  , 0x0       , "W"     , vpcl::SPP_ENABLE);
    for(uint i = 0; i < channelNum; i++) {
    ...
    mWrCbAPI["GERRCLR"]["CLRE"  ] = &Cibg_regif::cb_GERRCLR_CLRE;
    mWrCbAPI["GERRCLR"]["CLRO"  ] = &Cibg_regif::cb_GERRCLR_CLRO;
    mWrCbAPI["GKCPROT"]["KCE"   ] = &Cibg_regif::cb_GKCPROT_KCE;
    mWrCbAPI["GKCPROT"]["KCPROT"] = &Cibg_regif::cb_GKCPROT_KCPROT;

    mRdCbAPI["GERRCLR"]["CLRE"  ] = &Cibg_regif::cb_GERRCLR_CLRE;
    mRdCbAPI["GERRCLR"]["CLRO"  ] = &Cibg_regif::cb_GERRCLR_CLRO;
    mRdCbAPI["GKCPROT"]["KCE"   ] = &Cibg_regif::cb_GKCPROT_KCE;
    mRdCbAPI["GKCPROT"]["KCPROT"] = &Cibg_regif::cb_GKCPROT_KCPROT;

    for(uint i = 0; i < channelNum; i++) {
        str_tmp.str("");
        str_tmp<<"GPROT0_"<< i;
        mWrCbAPI[str_tmp.str()]["RG"] = &Cibg_regif::cb_GPROT0_RG;
        mWrCbAPI[str_tmp.str()]["WG"] = &Cibg_regif::cb_GPROT0_WG;
        mRdCbAPI[str_tmp.str()]["RG"] = &Cibg_regif::cb_GPROT0_RG;
        mRdCbAPI[str_tmp.str()]["WG"] = &Cibg_regif::cb_GPROT0_WG;
    }
    for(uint i = 0; i < channelNum; i++) {
        str_tmp.str("");
        str_tmp<<"GPROT1_"<< i;
        mWrCbAPI[str_tmp.str()]["SPID"] = &Cibg_regif::cb_GPROT1_SPID;
        mRdCbAPI[str_tmp.str()]["SPID"] = &Cibg_regif::cb_GPROT1_SPID;
    }
    InitLocalVal();
}
...
Cibg_regif::uint Cibg_regif::bit_select(
...
bool Cibg_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cibg_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cibg_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                            const unsigned char *p_data,   ///< [in] Writing data
                            cuint size)                    ///< [in] Data size (byte)
{
    if(size == 0) return false;
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cibg_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                            unsigned char *p_data,   ///< [out] Reading data
                            cuint size)              ///< [in]  Data size (byte)
{
    if(size == 0) return false;
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cibg_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                const unsigned char *p_data,  ///< [in] Writing data
                                cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cibg_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                unsigned char *p_data,   ///< [out] Reading data
                                cuint size,              ///< [in]  Data size (byte)
                                bool IsDbgFunc)          ///< [in] flag indicate reg_rd or reg_rd_dbg
{
...
bool Cibg_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                             unsigned char *p_data,          ///< [out] Reading data
                             cuint size,                     ///< [in]  Data size (byte)
...
bool Cibg_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                             const unsigned char *p_data,      ///< [in] Writing data
                             cuint size,                       ///< [in] Data size (byte)
                             cuint reg_index,                  ///< [in] Register index
...
bool Cibg_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr)
{
...
/// @return string
std::string Cibg_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cibg_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return None
void Cibg_regif::UpdateLocalVal(cuint addr)
{
    if (addr == 0x0000) {
        GERRCLR_CLRE                 = (*GERRCLR )["CLRE"    ];
        GERRCLR_CLRO                 = (*GERRCLR )["CLRO"    ];
        return;
    }
    if (addr == 0x0004) {
        GERRSTAT_ERR                 = (*GERRSTAT)["ERR"     ];
        GERRSTAT_OVF                 = (*GERRSTAT)["OVF"     ];
        return;
    }
    if (addr == 0x0008) {
...
/// @return None
void Cibg_regif::UpdateRegVal(cuint addr)
{
    if (addr == 0x0000) {
        (*GERRCLR )["CLRE"    ] = GERRCLR_CLRE;
        (*GERRCLR )["CLRO"    ] = GERRCLR_CLRO;
        return;
    }
    if (addr == 0x0004) {
        (*GERRSTAT)["ERR"     ] = GERRSTAT_ERR;
        (*GERRSTAT)["OVF"     ] = GERRSTAT_OVF;
        return;
    }
    if (addr == 0x0008) {
...
/// @return vector
std::vector<std::string> Cibg_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cibg_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cibg_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
...
int Cibg_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cibg_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cibg_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cibg_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Cibg_regif::uint Cibg_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return none
void Cibg_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cibg_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cibg_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cibg_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Cibg_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cibg_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    unsigned char* ptr = (unsigned char*)trans.get_data_ptr();
    reg_wr_dbg((cuint)tag, ptr, sizeof(uint));
    return sizeof(uint);
}
void Cibg_regif::cw_instantiate_reg(scml2::reg<uint> * reg, std::string reg_name, int offset)
{
