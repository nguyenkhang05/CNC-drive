// -----------------------------------------------------------------------------
// $Id: hwcore_measure_regif.cpp,v 1.2 2020/09/01 10:06:12 phuongtran Exp $
//
...
//    regif_cpp.skl
//
// Description: Copyright of hwcore_measure_regif.cpp
// Ref: {HWCORE_MEASURE_UD_Copyright_001}
// Description: Register IF generator hwcore_measure_regif.cpp
// Ref: {HWCORE_MEASURE_UD_ATT1_001, HWCORE_MEASURE_UD_ATT1_002}
// This file is generated by Register I/F generator
//    gen_regif.py 1.9 2013/12/04 09:51:36 sontran
//    gen_regif_class.py 1.60 2014/10/24 09:42:15 ducduong
//    regif_cpp.skl 1.53 2014/10/13 03:19:16 sontran
//
// Input file : measure_regif.txt
// Description:
// Ref: {HWCORE_MEASURE_UD_ATT1_008, HWCORE_MEASURE_UD_Registers_006}
////////////////////////////////////////////////////////////////////////////////
// %MODULE HWCORE_MEASURE
//     #                   name            offset_size
//     %%REG_INSTANCE      reg_def         11
// 
// %REG_CHANNEL reg_def
//     %%TITLE  name          reg_name      wsize    rsize    length  offset  factor_start  factor_end  factor_index  factor_step  access init  support callback callbackDBG
//     %%REG    PMT2CTR      PMT2CTR      8        8|16|32    8       0x06A0    -             -           -             -            W|R    0     TRUE    -       -
//     %%REG    PMT2SFT      PMT2SFT      8        8|16|32    8       0x06A4    -             -           -             -            W|R    0     TRUE    -       -
//     %%REG    PMT2CNT      PMT2CNT      32       8|16|32    32      0x06A8    -             -           -             -            W|R    0     TRUE    -       -
//     %%REG    PMT2CAP      PMT2CAP      32       8|16|32    32      0x06AC    -             -           -             -              R    0     TRUE    -       -
//     %%REG    PMT2INVL     PMT2INVL     8        8|16|32    8       0x06B0    -             -           -             -            W|R    0x1F  TRUE    -       -
// 
// %REG_NAME PMT2CTR 
//     %%TITLE  name        upper  lower  init  access  support  callback
//     %%BIT    EN          0      0      0     W|R     TRUE     W
// 
// %REG_NAME PMT2SFT
//     %%TITLE  name        upper  lower  init  access  support  callback
//     %%BIT    SCAPTRG     0      0      0     W|R     TRUE     W
// 
// %REG_NAME PMT2CNT
//     %%TITLE  name        upper  lower  init  access  support  callback callbackDBG
//     %%BIT    DATA        31     0      0     W|R     TRUE     W|R      W|R
// 
// %REG_NAME PMT2CAP
//     %%TITLE  name        upper  lower  init  access  support  callback
...
// %REG_NAME PMT2INVL
//     %%TITLE  name        upper  lower  init  access  support  callback
//     %%BIT    DATA        4      0      0x1F  W|R     TRUE     W
////////////////////////////////////////////////////////////////////////////////
/// @file hwcore_measure_regif.cpp
/// @brief Register IF class of model HWCORE_MEASURE
/// $Id: hwcore_measure_regif.cpp,v 1.2 2020/09/01 10:06:12 phuongtran Exp $
/// $Date: 2020/09/01 10:06:12 $
/// $Revison$
/// $Author: phuongtran $
////////////////////////////////////////////////////////////////////////////////
...
// Ref: {HWCORE_MEASURE_UD_ATT1_004, HWCORE_MEASURE_UD_ATT1_009}
/// @return none
Chwcore_measure_regif::Chwcore_measure_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef USR_CWR_SYSTEMC
    , cwmem("register", 0x6B4)
    #endif
{
    CommandInit();
    PMT2CTR  = new vpcl::re_register(0x06A0, this, "PMT2CTR" , name.c_str());
    PMT2SFT  = new vpcl::re_register(0x06A4, this, "PMT2SFT" , name.c_str());
    PMT2CNT  = new vpcl::re_register(0x06A8, this, "PMT2CNT" , name.c_str());
    PMT2CAP  = new vpcl::re_register(0x06AC, this, "PMT2CAP" , name.c_str());
    PMT2INVL = new vpcl::re_register(0x06B0, this, "PMT2INVL", name.c_str());
    ...
    mRegMap[0x06A8]     = index;
    mRegMap[0x06A8 + 1] = index;
    mRegMap[0x06A8 + 2] = index;
    mRegMap[0x06A8 + 3] = index++;
    mRegMap[0x06AC]     = index;
    mRegMap[0x06AC + 1] = index;
    mRegMap[0x06AC + 2] = index;
    mRegMap[0x06AC + 3] = index++;
    mRegMap[0x06B0]     = index++;
    mTotalRegNum = index;
    mRegArray = new SRegList* [mTotalRegNum];
    index = 0;
#ifdef USR_CWR_SYSTEMC
    mCurReg = mRegArray[index++] = new SRegList(PMT2CTR   , mCurReg, 0,  1,       "8", "8|16|32", false, &cwmem);
#else
    mCurReg = mRegArray[index++] = new SRegList(PMT2CTR   , mCurReg, 0,  1,       "8", "8|16|32");
#endif
    mCurReg = mRegArray[index++] = new SRegList(PMT2SFT   , mCurReg, 0,  1,       "8", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PMT2CNT   , mCurReg, 0,  4,      "32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PMT2CAP   , mCurReg, 0,  4,      "32", "8|16|32");
    mCurReg = mRegArray[index++] = new SRegList(PMT2INVL  , mCurReg, 0,  1,       "8", "8|16|32");

    mRegList = mCurReg;

    (*PMT2CTR   ) ( 0,  0, "EN"      , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*PMT2SFT   ) ( 0,  0, "SCAPTRG" , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*PMT2CNT   ) (31,  0, "DATA"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    (*PMT2CAP   ) (31,  0, "DATA"    , 0x0       , "R"     , vpcl::SPP_ENABLE);
    (*PMT2INVL  ) ( 4,  0, "DATA"    , 0x1F      , "W|R"   , vpcl::SPP_ENABLE);
    ...
    mWrCbAPI["PMT2CTR"]["EN"    ] = &Chwcore_measure_regif::cb_PMT2CTR_EN;
    mWrCbAPI["PMT2SFT"]["SCAPTRG"] = &Chwcore_measure_regif::cb_PMT2SFT_SCAPTRG;
    mWrCbAPI["PMT2CNT"]["DATA"  ] = &Chwcore_measure_regif::cb_PMT2CNT_DATA;
    mWrCbAPI["PMT2INVL"]["DATA"  ] = &Chwcore_measure_regif::cb_PMT2INVL_DATA;

    mRdCbAPI["PMT2CNT"]["DATA"  ] = &Chwcore_measure_regif::cb_PMT2CNT_DATA;

    mWrDbgCbAPI["PMT2CNT"]["DATA"  ] = &Chwcore_measure_regif::cbDBG_PMT2CNT_DATA;

    mRdDbgCbAPI["PMT2CNT"]["DATA"  ] = &Chwcore_measure_regif::cbDBG_PMT2CNT_DATA;

    #ifdef USE_WEB_SIM
    InitializeRegValueID();
    mNotifyRegValueIDAPI = &Chwcore_measure_regif::NotifyRegValueID;
    #endif
}

/// Destructor: delete pointers
/// @return none
Chwcore_measure_regif::~Chwcore_measure_regif()
{
...
Chwcore_measure_regif::uint Chwcore_measure_regif::bit_select(cuint val,    ///< [in] Writting address
...
// Ref: {HWCORE_MEASURE_UD_Direction_024, HWCORE_MEASURE_UD_Direction_019}
/// @return true if writing transaction is finished normally
bool Chwcore_measure_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
// Ref: {HWCORE_MEASURE_UD_Direction_023, HWCORE_MEASURE_UD_Direction_019}
/// @return true if reading transaction is finished normally
bool Chwcore_measure_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// @return true if writing transaction is finished normally
bool Chwcore_measure_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                                       const unsigned char *p_data,   ///< [in] Writing data
                                       cuint size)                    ///< [in] Data size (byte)
{
    assert(size != 0);
...
/// @return true if reading transaction is finished normally
bool Chwcore_measure_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                                       unsigned char *p_data,   ///< [out] Reading data
                                       cuint size)              ///< [in]  Data size (byte)
{
    assert(size != 0);
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Chwcore_measure_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                           const unsigned char *p_data,  ///< [in] Writing data
                                           cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Chwcore_measure_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                           unsigned char *p_data,   ///< [out] Reading data
                                           cuint size,              ///< [in]  Data size (byte)
                                           bool IsDbgFunc)          ///< [in]  Flag indicate reg_rd or reg_rd_dbg
{
...
// Ref: {HWCORE_MEASURE_UD_Direction_029}
/// @return true if reading successfully
bool Chwcore_measure_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                                        unsigned char *p_data,          ///< [out] Reading data
                                        cuint size,                     ///< [in]  Data size (byte)
...
// Ref: {HWCORE_MEASURE_UD_Direction_030}
/// @return true if writing successfully
bool Chwcore_measure_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                                        const unsigned char *p_data,      ///< [in] Writing data
                                        cuint size,                       ///< [in] Data size (byte)
                                        cuint reg_index,                  ///< [in] Register index
                                        bool IsDbgFunc)                   ///< [in] Flag indicate reg_wr or reg_wr_dbg
{
    ...
    if (!mRegArray[reg_index]->block || IsDbgFunc) {
        ...
        for (vpcl::bit_info *it=Register->mBitInfo ; it!=NULL ; it=it->next) {
            ...
            if (IsDbgFunc) {
                if ((mWrDbgCbAPI[Register->name()][it->name()]) != NULL) {
                    (this->*(mWrDbgCbAPI[Register->name()][it->name()]))(RegCBstr(mRegArray[reg_index]->channel, true, size, pre_data, data));
                }
            }
        }
    } else {
...
// Ref: {HWCORE_MEASURE_UD_Direction_018}
/// @@return true if detect_size equal expect_size
bool Chwcore_measure_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr, bool is_wr)
{
...
/// @return none
void Chwcore_measure_regif::CommandInit()
{
...
/// @return string
std::string Chwcore_measure_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
// Ref: {HWCORE_MEASURE_UD_Direction_021, HWCORE_MEASURE_UD_Direction_022, HWCORE_MEASURE_UD_Direction_015}
/// @return string
std::string Chwcore_measure_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Chwcore_measure_regif::InitializeRegValueID()
{//{{{
...
/// @return none
void Chwcore_measure_regif::CalcTargetRegVal(void)
{//{{{
...
/// @return none
void Chwcore_measure_regif::IssueRegValueID(void)
{//{{{
...
/// @return vector
std::vector<std::string> Chwcore_measure_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Chwcore_measure_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip, bool is_wr_data, uint acc_size)
{
    std::ostringstream str_tmp;
...
/// @return none
void Chwcore_measure_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
    if (mDumpRegisterRW) {
...
int Chwcore_measure_regif::get_reg_index(cuint access_addr)
{
    if (access_addr < (1<<11)) {
...
vpcl::re_register *Chwcore_measure_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Chwcore_measure_regif::next_reg_object()
{
    assert(mCurReg != NULL);
...
/// @return none
void Chwcore_measure_regif::wr_cb(cuint addr, uint data)
{
    #ifdef USR_CWR_SYSTEMC
...
/// @return read data
Chwcore_measure_regif::uint Chwcore_measure_regif::rd_cb(cuint addr)
{
    #ifdef USR_CWR_SYSTEMC
    uint data=0;
...
/// @return none
void Chwcore_measure_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Chwcore_measure_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Chwcore_measure_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}

// Description: Method to change value of mIsReset
// Ref: {HWCORE_MEASURE_UD_Direction_035}
/// @return none
void Chwcore_measure_regif::EnableReset(const bool is_active)
{
...
#ifdef USR_CWR_SYSTEMC
uint Chwcore_measure_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Chwcore_measure_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    int reg_index = get_reg_index((uint)tag);
    assert(reg_index != (1<<16));
    uint length = mRegArray[reg_index]->length;
...
void Chwcore_measure_regif::cw_set_all_callback_reg(void)
{
