...
/// @return none
Cabfg_regif::Cabfg_regif(std::string name, uint buswidth)
    :vpcl::reg_super()
    #ifdef CWR_SYSTEMC
    , cwmem("register", 0x2FC)
    #endif
{
    CommandInit();
    std::ostringstream str_tmp;
    for(uint i = 0; i < emNUM_BFGCR; i++) {
        str_tmp.str("");
        str_tmp<<"BFGCR"<< i;
        BFGCR[i] = new vpcl::re_register((0x0000) + 16*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_CNTCR; i++) {
        str_tmp.str("");
        str_tmp<<"CNTCR"<< i;
        CNTCR[i] = new vpcl::re_register((0x0004) + 16*i, this, str_tmp.str(), name.c_str());
    }
    for(uint i = 0; i < emNUM_BFSR; i++) {
        str_tmp.str("");
        str_tmp<<"BFSR"<< i;
        BFSR [i] = new vpcl::re_register((0x0008) + 16*i, this, str_tmp.str(), name.c_str());
    }
    ...
    for(uint i = 0; i < emNUM_BFGCR; i++) {
        mRegMap[(0x0000)     + i*16] = index;
        mRegMap[(0x0000 + 1) + i*16] = index;
        mRegMap[(0x0000 + 2) + i*16] = index;
        mRegMap[(0x0000 + 3) + i*16] = index++;
    }
    for(uint i = 0; i < emNUM_CNTCR; i++) {
        mRegMap[(0x0004)     + i*16] = index;
        mRegMap[(0x0004 + 1) + i*16] = index;
        mRegMap[(0x0004 + 2) + i*16] = index;
        mRegMap[(0x0004 + 3) + i*16] = index++;
    }
    for(uint i = 0; i < emNUM_BFSR; i++) {
        mRegMap[(0x0008)     + i*16] = index;
        mRegMap[(0x0008 + 1) + i*16] = index;
        mRegMap[(0x0008 + 2) + i*16] = index;
        mRegMap[(0x0008 + 3) + i*16] = index++;
    }
    mTotalRegNum = index;
    mRegArray = new SRegList* [mTotalRegNum];
    index = 0;
#ifdef CWR_SYSTEMC
    for(uint i = 0; i < emNUM_BFGCR; i++) mCurReg = mRegArray[index++] = new SRegList(BFGCR  [i], mCurReg, i,  4,      "32", "8|16|32", false, &cwmem);
#else
    for(uint i = 0; i < emNUM_BFGCR; i++) mCurReg = mRegArray[index++] = new SRegList(BFGCR  [i], mCurReg, i,  4,      "32", "8|16|32");
#endif
    for(uint i = 0; i < emNUM_CNTCR; i++) mCurReg = mRegArray[index++] = new SRegList(CNTCR  [i], mCurReg, i,  4,      "32", "8|16|32");
    for(uint i = 0; i < emNUM_BFSR ; i++) mCurReg = mRegArray[index++] = new SRegList(BFSR   [i], mCurReg, i,  4,      "32", "8|16|32");

    mRegList = mCurReg;

    for(uint i = 0; i < emNUM_BFSR; i++) {
        (*BFSR   [i]) ( 0,  0, "BFS"     , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_BFGCR; i++) {
        (*BFGCR  [i]) (25, 24, "BTGC"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*BFGCR  [i]) (17, 16, "BPGC"    , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*BFGCR  [i]) ( 7,  0, "CHS"     , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
    }
    for(uint i = 0; i < emNUM_CNTCR; i++) {
        (*CNTCR  [i]) (28, 28, "ENB"     , 0x0       , "W|R"   , vpcl::SPP_ENABLE);
        (*CNTCR  [i]) (11,  8, "NRMCNT"  , 0x1       , "W|R"   , vpcl::SPP_ENABLE);
        (*CNTCR  [i]) ( 3,  0, "ERRCNT"  , 0x1       , "W|R"   , vpcl::SPP_ENABLE);
    }
    ...
    for(uint i = 0; i < emNUM_CNTCR; i++) {
        str_tmp.str("");
        str_tmp<<"CNTCR"<< i;
        mWrCbAPI[str_tmp.str()]["ENB"] = &Cabfg_regif::cb_CNTCR_ENB;
    }
    for(uint i = 0; i < emNUM_BFSR; i++) {
        str_tmp.str("");
        str_tmp<<"BFSR"<< i;
        mWrCbAPI[str_tmp.str()]["BFS"] = &Cabfg_regif::cb_BFSR_BFS;
    }
    InitLocalVal();
    #ifdef USE_WEB_SIM
    InitializeRegValueID();
    mNotifyRegValueIDAPI = &Cabfg_regif::NotifyRegValueID;
    #endif
}

/// Destructor: delete pointers
/// @return none
Cabfg_regif::~Cabfg_regif()
{
...
Cabfg_regif::uint Cabfg_regif::bit_select(cuint val,    ///< [in] Writting address
...
bool Cabfg_regif::reg_wr(cuint addr,                    ///< [in] Writting address
...
bool Cabfg_regif::reg_rd(cuint addr,              ///< [in]  Reading address
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cabfg_regif::reg_wr_dbg(cuint addr,                    ///< [in] Writting address
                             const unsigned char *p_data,   ///< [in] Writing data
                             cuint size)                    ///< [in] Data size (byte)
{
    if (size == 0) {
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cabfg_regif::reg_rd_dbg(cuint addr,              ///< [in]  Reading address
                             unsigned char *p_data,   ///< [out] Reading data
                             cuint size)              ///< [in]  Data size (byte)
{
    if (size == 0) {
...
/// Write the value to requested module register
/// @return true if writing transaction is finished normally
bool Cabfg_regif::reg_wr_process(cuint addr,                   ///< [in] Writting address
                                 const unsigned char *p_data,  ///< [in] Writing data
                                 cuint size,                   ///< [in] Data size (byte)
...
/// Read the register value of requested module
/// @return true if reading transaction is finished normally
bool Cabfg_regif::reg_rd_process(cuint addr,              ///< [in]  Reading address
                                 unsigned char *p_data,   ///< [out] Reading data
                                 cuint size,              ///< [in]  Data size (byte)
                                 bool IsDbgFunc)          ///< [in]  Flag indicate reg_rd or reg_rd_dbg
{
...
bool Cabfg_regif::reg_rd_func(cuint addr,                     ///< [in]  Reading address
                              unsigned char *p_data,          ///< [out] Reading data
                              cuint size,                     ///< [in]  Data size (byte)
...
bool Cabfg_regif::reg_wr_func(cuint addr,                       ///< [in] Writting address
                              const unsigned char *p_data,      ///< [in] Writing data
                              cuint size,                       ///< [in] Data size (byte)
                              cuint reg_index,                  ///< [in] Register index
...
bool Cabfg_regif::ChkSize(std::string expect_size, cuint detect_size, cuint addr, bool is_wr)
{
...
/// @return string
std::string Cabfg_regif::reg_handle_command(const std::vector<std::string>& args)
{
    std::string ret = "";
...
/// @return string
std::string Cabfg_regif::AccessRegCommand(const std::vector<std::string>& args, vpcl::re_register *Register, bool &BlockReg)
{
    std::string ret = "";
    char *err_p = NULL;
    std::string err_msg = "Error ("   + mInstName + ") ";   // error message
...
/// @return none
void Cabfg_regif::InitializeRegValueID()
{//{{{
...
/// @return none
void Cabfg_regif::CalcTargetRegVal(void)
{//{{{
...
/// @return none
void Cabfg_regif::IssueRegValueID(void)
{//{{{
...
/// @return None
void Cabfg_regif::UpdateLocalVal(cuint addr)
{
    if ((0x0000 <= addr) && (addr <= (0x0000)+(16*47)) && ((addr-0x0000)%16 == 0)) {
        uint i = (addr - 0x0000)/16;
        BFGCR_BTGC               [i] = (*(BFGCR[i]))["BTGC" ];
        BFGCR_BPGC               [i] = (*(BFGCR[i]))["BPGC" ];
        BFGCR_CHS                [i] = (*(BFGCR[i]))["CHS"  ];
        return;
    }
    if ((0x0004 <= addr) && (addr <= (0x0004)+(16*47)) && ((addr-0x0004)%16 == 0)) {
        uint i = (addr - 0x0004)/16;
        CNTCR_ENB                [i] = (*(CNTCR[i]))["ENB"  ];
        CNTCR_NRMCNT             [i] = (*(CNTCR[i]))["NRMCNT"];
        CNTCR_ERRCNT             [i] = (*(CNTCR[i]))["ERRCNT"];
        return;
    }
    if ((0x0008 <= addr) && (addr <= (0x0008)+(16*47)) && ((addr-0x0008)%16 == 0)) {
        uint i = (addr - 0x0008)/16;
        BFSR_BFS                 [i] = (*(BFSR [i]))["BFS"  ];
        return;
    }
}
...
/// @return None
void Cabfg_regif::UpdateRegVal(cuint addr)
{
    if ((0x0000 <= addr) && (addr <= (0x0000)+(16*47)) && ((addr-0x0000)%16 == 0)) {
        uint i = (addr - 0x0000)/16;
        (*(BFGCR[i]))["BTGC" ] = BFGCR_BTGC[i];
        (*(BFGCR[i]))["BPGC" ] = BFGCR_BPGC[i];
        (*(BFGCR[i]))["CHS"  ] = BFGCR_CHS[i];
        return;
    }
    if ((0x0004 <= addr) && (addr <= (0x0004)+(16*47)) && ((addr-0x0004)%16 == 0)) {
        uint i = (addr - 0x0004)/16;
        (*(CNTCR[i]))["ENB"  ] = CNTCR_ENB[i];
        (*(CNTCR[i]))["NRMCNT"] = CNTCR_NRMCNT[i];
        (*(CNTCR[i]))["ERRCNT"] = CNTCR_ERRCNT[i];
        return;
    }
    if ((0x0008 <= addr) && (addr <= (0x0008)+(16*47)) && ((addr-0x0008)%16 == 0)) {
        uint i = (addr - 0x0008)/16;
        (*(BFSR [i]))["BFS"  ] = BFSR_BFS[i];
        return;
    }
}
...
/// @return vector
std::vector<std::string> Cabfg_regif::Str2Vec(std::string str, const char sep)
{
...
std::string Cabfg_regif::Num2HexStr(cuint num, cuint num_length, bool space_strip, bool is_wr_data, uint acc_size)
{
    std::ostringstream str_tmp;
...
/// @return none
void Cabfg_regif::DumpRegMsg(const std::string operation, const std::string RegName, const std::string BitName, cuint size, cuint addr, cuint wr_data, cuint pre_data, cuint data, cuint reg_length)
{
    if (mDumpRegisterRW) {
...
int Cabfg_regif::get_reg_index(cuint access_addr)
{
...
vpcl::re_register *Cabfg_regif::first_reg_object()
{
    mCurReg = mRegList;
    return mCurReg->my_p;
}
...
vpcl::re_register *Cabfg_regif::next_reg_object()
{
    if (mCurReg != NULL) {
...
/// @return none
void Cabfg_regif::wr_cb(cuint addr, uint data)
{
    #ifdef CWR_SYSTEMC
...
/// @return read data
Cabfg_regif::uint Cabfg_regif::rd_cb(cuint addr)
{
    #ifdef CWR_SYSTEMC
    uint data=0;
...
/// @return none
void Cabfg_regif::_re_printf(std::string group, const char *message, ...)
{
    // message group check
    if (group == "DumpRegMsg") {
        group = "info";
...
/// @return none
void Cabfg_regif::get_fileline(std::string filename, int line_number)
{
    mFileName = filename;
...
/// @return none
void Cabfg_regif::set_instance_name(std::string InstName)
{
    mInstName = InstName;
}
...
/// @return none
void Cabfg_regif::EnableReset(const bool is_active)
{
...
#ifdef CWR_SYSTEMC
uint Cabfg_regif::cw_rd_cb(tlm::tlm_generic_payload& trans, int tag)
{
...
uint Cabfg_regif::cw_wr_cb(tlm::tlm_generic_payload& trans, int tag)
{
    int reg_index = get_reg_index((uint)tag);
    assert(reg_index != (1<<16));
    uint length = mRegArray[reg_index]->length;
...
void Cabfg_regif::cw_set_all_callback_reg(void)
{
