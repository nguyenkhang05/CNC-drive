...
if (($TIMING_MODE !~ "lt" && $TIMING_MODE !~ "at") || ($PAT !~ "pat_1" && $PAT !~ "pat_2")) then
    ...
    echo "============================="
    echo "Usage:    run_all_osci.csh <pat> <timing_mode> <model> <product_line>"
...
if (-e ../../tb/SCHEAPG4/scheapCompile/build/TOPV02/sim.x) then
    echo
    ...
    if (($cmd =~ "y") || ($cmd =~ "Y")) then
        set compile_env
        rm -f ../../tb/SCHEAPG4/scheapCompile/build/TOPV02/sim.x
    endif
else
    set compile_env
endif
...
if ($?run_sim) then
   ######Generate simulation environment
    ...
    if ($memcheck =~ "memcheck") then 
        perl gen_sim_osci.pl -m ${MODEL}_${PRODUCT} -t $TIMING_MODE -p $PAT -exe ../tb/SCHEAPG4/scheapCompile/build/TOPV02/sim.x -memcheck ${MODEL}_${PRODUCT}
    else
        perl gen_sim_osci.pl -m ${MODEL}_${PRODUCT} -t $TIMING_MODE -p $PAT -exe ../tb/SCHEAPG4/scheapCompile/build/TOPV02/sim.x
    endif
    popd
    echo
    ...
    echo
    pushd ../../sim/
    foreach sim_x (`find -iname "sim.x"`)
       rm -f $sim_x
    end
    ...
    popd
    if (-e ../../sim_osci_${PAT}_${MODEL}_${PRODUCT}_${TIMING_MODE}) then
       rm -rf ../../sim_osci_${PAT}_${MODEL}_${PRODUCT}_${TIMING_MODE}
    endif
    mkdir ../../sim_osci_${PAT}_${MODEL}_${PRODUCT}_${TIMING_MODE}
    cp -rf ../../sim/* ../../sim_osci_${PAT}_${MODEL}_${PRODUCT}_${TIMING_MODE}/
    echo
