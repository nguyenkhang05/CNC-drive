...
#        $COMMAND_LINE[1] = $COMMAND_LINE[0]." -vcd"." @_[1]"."$VCD_DIR/$category_dir";
    ...
    if ($category_name eq "all") {
        ...
        if ($VERSION ne "") {
			$CHECK_RESULT = $CHECK_RESULT."perl check_result_usk.pl -m $MODEL -v $VERSION -t $TIME_MODE -p $PAT\n"; #added '-p $PAT' by nhuvo 20180615
        } else {
            $CHECK_RESULT = $CHECK_RESULT."perl check_result_usk.pl -m $MODEL -t $TIME_MODE -p $PAT\n"; #added '-p $PAT' by nhuvo 20180615
        }
    }
    else {
        $CHECK_RESULT = "pushd @_[1]$CHECK_RESULT_DIR\n";
        if ($VERSION ne "") {
            $CHECK_RESULT = $CHECK_RESULT."perl check_result_usk.pl -c $category_dir -m $MODEL -v $VERSION -t $TIME_MODE -p $PAT\n"; #added '-p $PAT' by nhuvo 20180615
        } else {
            $CHECK_RESULT = $CHECK_RESULT."perl check_result_usk.pl -c $category_dir -m $MODEL -t $TIME_MODE -p $PAT\n"; #added '-p $PAT' by nhuvo 20180615
        }
    }
