    ...
    for (my $i=0; $i<=$#TM_PATH_SEL ;$i++) {
        ...
        if ($MISS_LOG[$i] == 0) {
            ...
            if ($no_script == 1) {
                ...
                if ($LOG_RESULT[$i] == 1) {
                    if ($VERSION ne "") {
                        if (open (EXPECTED_LOG, "<$SCRIPT_DIR/check_result/expected_log_usk_$PAT\_$MODEL\_$VERSION\_$TIME_MODE/$TM_PATH_SEL[$i].log")) { # Added '$PAT\_' by nhuvo 20180615
                            ...
                            print "Cannot open $SCRIPT_DIR/check_result/expected_log_usk_$PAT\_$MODEL\_$VERSION\_$TIME_MODE/$TM_PATH_SEL[$i].log\n"; # Added '$PAT\_' by nhuvo 20180615
                            $LOG_RESULT[$i] = 0;
                        }
                        # if ($TM_PATH_SEL[$i] =~ /tp03_06/ || $TM_PATH_SEL[$i] =~ /tp03_03_04/ || $TM_PATH_SEL[$i] =~ /tp03_03_07/ || $TM_PATH_SEL[$i] =~ /tp03_03_10/ || $TM_PATH_SEL[$i] =~ /tp04_01_04/){ ### Added by NganTran - Aug 15th, 2015 to check trace log
                            # if (open (EXPECTED_TRACE_LOG, "<$SCRIPT_DIR/check_result/expected_log_usk_$MODEL\_$VERSION\_$TIME_MODE/$TM_PATH_SEL[$i]_trace.log")) {
                            ...
                            # } else {
                                # print "Cannot open $SCRIPT_DIR/check_result/expected_log_usk_$MODEL\_$VERSION\_$TIME_MODE/$TM_PATH_SEL[$i]_trace.log\n";
                                # $LOG_RESULT[$i] = 0;
                            # }
                        # }
                    } else {
                        if (open (EXPECTED_LOG, "<$SCRIPT_DIR/check_result/expected_log_usk_$PAT\_$MODEL\_$TIME_MODE/$TM_PATH_SEL[$i].log")) { # Added '$PAT\_' by nhuvo 20180615
                            if (open (CHECKED_LOG, "<$LOG_DIR/$TM_PATH_SEL[$i].log")) {
                                my @expected_result = <EXPECTED_LOG>;
                                my @checked_result = <CHECKED_LOG>;
                            ...
                            print "Cannot open $SCRIPT_DIR/check_result/expected_log_usk_$PAT\_$MODEL\_$TIME_MODE/$TM_PATH_SEL[$i].log\n"; # Added '$PAT\_' by nhuvo 20180615
                            $LOG_RESULT[$i] = 0;
                        }
                        # if ($TM_PATH_SEL[$i] =~ /tp03_06/ || $TM_PATH_SEL[$i] =~ /tp03_03_04/ || $TM_PATH_SEL[$i] =~ /tp03_03_07/ || $TM_PATH_SEL[$i] =~ /tp03_03_10/ || $TM_PATH_SEL[$i] =~ /tp04_01_04/){ ### Added by NganTran - Aug 15th, 2015 to check trace log
                            # if (open (EXPECTED_TRACE_LOG, "<$SCRIPT_DIR/check_result/expected_log_usk_$MODEL\_$TIME_MODE/$TM_PATH_SEL[$i]_trace.log")) {
                                # if (open (TRACE_LOG_FILE, "$LOG_DIR/$TM_PATH_SEL[$i]_trace.log")) {
                                    # my @expected_result = <EXPECTED_TRACE_LOG>;
                                    # my @checked_result = <TRACE_LOG_FILE>;
                                    # for(my $my_index = 0; $my_index < $#expected_result; $my_index++) {
                                ...
                                # close EXPECTED_TRACE_LOG;
                            # } else {
                                # print "Cannot open $SCRIPT_DIR/check_result/expected_log_usk_$MODEL\_$TIME_MODE/$TM_PATH_SEL[$i]_trace.log\n";
                                # $LOG_RESULT[$i] = 0;
                            # }
                        # }
                    }
...
#                    my $no_script = 0;
#                    if ($VERSION ne "") { 
#                        open (EXPECTED_LOG, "<$SCRIPT_DIR/check_result/expected_log_usk_$PAT\_$MODEL\_$MODULE\_$VERSION\_$TIME_MODE/$TM_PATH_SEL[$i].log"); # Added '$PAT\_' by nhuvo 20180615
#                    } else {
#                        open (EXPECTED_LOG, "<$SCRIPT_DIR/check_result/expected_log_usk_$PAT\_$MODEL\_$MODULE\_$TIME_MODE/$TM_PATH_SEL[$i].log"); # Added '$PAT\_' by nhuvo 20180615
#                    }
...
#    if ($P_PASS_FOUND == 1) {
        ...
        my $EXP = "";
        if ($VERSION ne "") {
            $EXP = "$SCRIPT_DIR/check_result/expected_log_usk_$PAT\_$MODEL\_$VERSION\_$TIME_MODE/$TM_PATH_SEL[$i].log"; # Added '$PAT\_' by nhuvo 20180615
        } else {
            $EXP = "$SCRIPT_DIR/check_result/expected_log_usk_$PAT\_$MODEL\_$TIME_MODE/$TM_PATH_SEL[$i].log"; # Added '$PAT\_' by nhuvo 20180615
        }
