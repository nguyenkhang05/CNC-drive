// ---------------------------------------------------------------------
// $Id: SPIDCTL_Func.cpp,v 1.3 2021/11/18 07:22:52 minhquocha Exp $
//
...
#define func_re_printf mpSPIDCTLAgentController->common_re_printf
...
// Ref: {SPIDCTL_UD_ATT1_006, SPIDCTL_UD_ReferenceDoc_001, SPIDCTL_UD_Direction_030, SPIDCTL_UD_Flow_004}
    ...
    if ((!mpSPIDCTLAgentController->CheckClockPeriod()) || (mpSPIDCTLAgentController->GetResetStatus())) {
        if (pExtInfo != NULL) {
            if (!(debug && (pExtInfo->getPeId() == DEBUG_MASTER_BMID))) {
                func_re_printf("warning", "Cannot read register when clock is 0 or in reset state.\n");
                return;
            }
        }else {
            func_re_printf("warning", "Cannot read register when clock is 0 or in reset state.\n");
...
// Ref: {SPIDCTL_UD_ATT1_006, SPIDCTL_UD_ReferenceDoc_001, SPIDCTL_UD_Direction_029, SPIDCTL_UD_Flow_005}
...
// Ref: {SPIDCTL_UD_ATT1_007, SPIDCTL_UD_Registers_001}
...
// Ref: {SPIDCTL_UD_ATT1_007, SPIDCTL_UD_Registers_002}
...
// Ref: {SPIDCTL_UD_Registers_001, SPIDCTL_UD_Flow_006, SPIDCTL_UD_Feature_003}
...
// Ref: {SPIDCTL_UD_Feature_006, SPIDCTL_UD_Registers_002, SPIDCTL_UD_Flow_007, SPIDCTL_UD_Direction_047, SPIDCTL_UD_Direction_048}
...
{//{{{
    ...
    if (true == mpSPIDCTLAgentController->IsKeyCodeEnable()) { // KCE bit is 1
        if (false == mpSPIDCTLAgentController->IsMaskLocked(str.channel)) { // LOCK bit is 0
            ...
            func_re_printf("warning", "Cannot write to BMSPIDMSK%02d when BMSPIDMSKLOCK%02d is locked.\n", str.channel, str.channel);
        }
    }else{
        func_re_printf("warning", "Cannot write to %s register while key code is disabled.\n", register_name.c_str());
    }
    (*BMSPIDMSK[str.channel])["SPIDMSK"] = str.pre_data;
}//}}}

// Description: Callback function of SPID Mask Lock bit
// Ref: {SPIDCTL_UD_Registers_003, SPIDCTL_UD_Flow_008}
...
// Ref: {SPIDCTL_UD_Registers_004, SPIDCTL_UD_Flow_009}
...
// Description: set initial values for BMSPIDn registers
void SPIDCTL_Func::InitializeBMSPIDn(void)
{
    std::map<unsigned int, unsigned int>::iterator it;
    for(unsigned int i = 0; i < MAX_SPID; i++) {
        if ((it = mpSPIDCTLAgentController->mSPIDInitValMap.find(i)) != mpSPIDCTLAgentController->mSPIDInitValMap.end()) {
            (*BMSPID[it->first]).UpdateBitInitValue("SPID", it->second);
            (*BMSPID[it->first]) = it->second;
        } else {
            if(i <= 31) {
                (*BMSPID[i]).UpdateBitInitValue("SPID", i);
                (*BMSPID[i]) = i;
            } else {
                (*BMSPID[i]).UpdateBitInitValue("SPID", 31);
                (*BMSPID[i]) = 31;
            }
        }
    }
